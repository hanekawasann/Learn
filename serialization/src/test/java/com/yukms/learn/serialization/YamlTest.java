...
public class YamlTest extends BaseTest {
    ...
    public void test_general_obj() throws IOException {
        ...
        people.setAge(18);
        YamlWriter writer = new YamlWriter(new FileWriter("src/test/resource/yaml/generalObj.yaml"), CONFIG);
        writer.write(people);
        writer.close();

        YamlReader reader = new YamlReader(new FileReader("src/test/resource/yaml/generalObj.yaml"), CONFIG);
    ...
    public void test_subclass_obj() throws IOException {
        ...
        YamlWriter writer = new YamlWriter(new FileWriter("src/test/resource/yaml/subclassObj.yaml"), CONFIG);
        writer.write(people);
        writer.close();

        YamlReader reader = new YamlReader(new FileReader("src/test/resource/yaml/subclassObj.yaml"), CONFIG);
    ...
    public void test_draw_dot() throws IOException {
        ...
        people.add(teacher);
        people.add(teacher);
        YamlWriter writer = new YamlWriter(new FileWriter("src/test/resource/yaml/drawDot.yaml"), CONFIG);
        writer.write(people);
        writer.close();

        YamlReader reader = new YamlReader(new FileReader("src/test/resource/yaml/drawDot.yaml"), CONFIG);
        List<People> read = (List<People>) reader.read();
        Assert.assertEquals(2, read.size());
        Teacher read_0 = (Teacher) read.get(0);
        Assert.assertEquals("teacher", read_0.getName());
    ...
    public void test_null_obj() throws IOException {
        YamlWriter writer = new YamlWriter(new FileWriter("src/test/resource/yaml/nullObj.yaml"), CONFIG);
        writer.write(null);
        writer.close();
    }
    ...
    public void test_exception() throws IOException {
        YamlWriter writer = new YamlWriter(new FileWriter("src/test/resource/yaml/exception.yaml"));
    ...
    public void test_no_getter_object() throws IOException {
        ...
        YamlWriter writer = new YamlWriter(new FileWriter("src/test/resource/yaml/noGetterObject.yaml"));
        writer.write(object);
        writer.close();
    }
}
