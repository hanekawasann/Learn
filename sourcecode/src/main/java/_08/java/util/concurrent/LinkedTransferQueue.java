...
public class LinkedTransferQueue<E> extends AbstractQueue<E> implements TransferQueue<E>, java.io.Serializable {
    ...
    private E firstDataItem() {
        for (Node p = head; p != null; p = succ(p)) {
            Object item = p.item;
            if (p.isData) {
                if (item != null && item != p) { return LinkedTransferQueue.cast(item); }
    ...
    final class Itr implements Iterator<E> {
        ...
        private void advance(Node prev) {
            ...
            for (Node p = prev, s, n; ; ) {
                ...
                // assert s.isMatched();
                if (p == null) { p = s; } else if ((n = s.next) == null) { break; } else if (s == n) {
                    p = null;
                } else { p.casNext(s, n); }
            }
            nextNode = null;
    ...
    private void sweep() {
        for (Node p = head, s, n; p != null && (s = p.next) != null; ) {
            ...
            { p = s; } else if ((n = s.next) == null) // trailing node is pinned
            { break; } else if (s == n)    // stale
            // No need to also check for p == s, since that implies s == n
            { p = head; } else { p.casNext(s, n); }
        }
    }
