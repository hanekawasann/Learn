...
public class DoubleAccumulator extends Striped64 implements Serializable {
        ...
        if ((as = cells) != null ||
            ...
            if (as == null || (m = as.length - 1) < 0 || (a = as[getProbe() & m]) == null || !(uncontended =
                (r = Double.doubleToRawLongBits(function.applyAsDouble(Double.longBitsToDouble(v = a.value), x))) ==
                    v || a.cas(v, r))) { doubleAccumulate(x, function, uncontended); }
        }
    }
    ...
    public double get() {
        ...
        if (as != null) {
            for (int i = 0; i < as.length; ++i) {
                if ((a = as[i]) != null) { result = function.applyAsDouble(result, Double.longBitsToDouble(a.value)); }
            }
        }
        return result;
    }
    ...
    public void reset() {
        ...
        if (as != null) {
            for (int i = 0; i < as.length; ++i) {
                if ((a = as[i]) != null) { a.value = identity; }
            }
        }
    }
