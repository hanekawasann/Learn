...
class MutableBigInteger {
    ...
    void addDisjoint(MutableBigInteger addend, int n) {
        ...
        int resultLen = (intLen > y ? intLen : y);
        int[] result;
        if (value.length < resultLen) { result = new int[resultLen]; } else {
            result = value;
    ...
    void multiply(MutableBigInteger y, MutableBigInteger z) {
        ...
        for (int i = xLen - 2; i >= 0; i--) {
            ...
            for (int j = yLen - 1, k = yLen + i; j >= 0; j--, k--) {
                long product = (y.value[j + y.offset] & LONG_MASK) * (value[i + offset] & LONG_MASK) +
    ...
    int divideOneWord(int divisor, MutableBigInteger quotient) {
        ...
        quotient.normalize();
        // Unnormalize
        if (shift > 0) { return rem % divisor; } else { return rem; }
    }
    ...
    long divide(long v, MutableBigInteger quotient) {
        ...
        if (d == 0) { return divideOneWord((int) v, quotient) & LONG_MASK; } else {
    ...
    private MutableBigInteger divideMagnitude(MutableBigInteger div, MutableBigInteger quotient,
        ...
        if (qhat != 0) {
            ...
            if (needRemainder) { borrow = mulsub(rem.value, divisor, qhat, dlen, limit - 1 + rem.offset); } else {
