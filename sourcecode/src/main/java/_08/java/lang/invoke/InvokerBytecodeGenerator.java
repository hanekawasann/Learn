...
class InvokerBytecodeGenerator {
    ...
    private InvokerBytecodeGenerator(LambdaForm lambdaForm, int localsMapSize, String className, String invokerName,
    ...
    String constantPlaceholder(Object arg) {
        ...
        int index = cw.newConst(cpPlaceholder);  // TODO check if aready in the constant pool
    ...
    private static MemberName resolveInvokerMember(Class<?> invokerClass, String name, MethodType type) {
        ...
        try {
            member = MEMBERNAME_FACTORY
                .resolveOrFail(REF_invokeStatic, member, HOST_CLASS, ReflectiveOperationException.class);
    ...
    private void classFilePrologue() {
        ...
        cw.visit(Opcodes.V1_8, Opcodes.ACC_PUBLIC + Opcodes.ACC_FINAL + Opcodes.ACC_SUPER, className, null, superName,
            null);
    ...
    private void emitImplicitConversion(char ptype, Class<?> pclass) {
        switch (ptype) {
            case 'L':
                if (VerifyType.isNullConversion(Object.class, pclass)) { return; }
                if (isStaticallyNameable(pclass)) {
                    ...
                    if (pclass.isArray()) { mv.visitTypeInsn(Opcodes.CHECKCAST, OBJARY); }
                }
                return;
            case 'I':
    ...
    static boolean isStaticallyInvocable(MemberName member) {
        ...
        if (!isStaticallyNameable(mtype.returnType())) { return false; }
        for (Class<?> ptype : mtype.parameterArray()) { if (!isStaticallyNameable(ptype)) { return false; } }
        if (!member.isPrivate() && VerifyAccess.isSamePackage(MethodHandle.class, cls)) {
            return true;   // in java.lang.invoke package
        }
        return member.isPublic() && isStaticallyNameable(cls);
    }
    ...
    static boolean isStaticallyNameable(Class<?> cls) {
        ...
        if (cls.getClassLoader() != Object.class.getClassLoader()) { return false; }
        if (VerifyAccess.isSamePackage(MethodHandle.class, cls)) { return true; }
        if (!Modifier.isPublic(cls.getModifiers())) { return false; }
        for (Class<?> pkgcls : STATICALLY_INVOCABLE_PACKAGES) {
            if (VerifyAccess.isSamePackage(pkgcls, cls)) { return true; }
        }
        return false;
    }
    ...
    private boolean isSelectAlternative(MemberName member) {
        return member != null && member.getDeclaringClass() == MethodHandleImpl.class &&
    ...
    private void emitPrimCast(char from, char to) {
        ...
        if (wfrom.isSubwordOrInt()) {
            ...
            if (wto.isSubwordOrInt()) {
                ...
                switch (from) {
                    case 'J':
                        if (to == 'F') { mv.visitInsn(Opcodes.L2F); } else if (to == 'D') {
                            mv.visitInsn(Opcodes.L2D);
                        } else { error = true; }
                        break;
                    case 'F':
                        if (to == 'J') { mv.visitInsn(Opcodes.F2L); } else if (to == 'D') {
                            mv.visitInsn(Opcodes.F2D);
                        } else { error = true; }
                        break;
                    case 'D':
                        if (to == 'J') { mv.visitInsn(Opcodes.D2L); } else if (to == 'F') {
    ...
    private static String basicTypeCharSignature(String prefix, MethodType type) {
        ...
        for (Class<?> ptype : type.parameterList()) { buf.append(Wrapper.forBasicType(ptype).basicTypeChar()); }
