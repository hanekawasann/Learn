...
/*non-public*/ abstract class MethodHandleImpl {
    ...
    static MethodHandle makeArrayElementAccessor(Class<?> arrayClass, boolean isSetter) {
        ...
        names[names.length - 1] = new Name(accessor.asType(srcType), args);
    ...
    static final class ArrayAccessor {
        ...
        static MethodType type(Class<?> arrayClass, boolean isSetter) {
            ...
            if (!elemClass.isPrimitive()) {
                ...
                if (USE_WEAKLY_TYPED_ARRAY_ACCESSORS) { arrayArgClass = Object.class; }
            }
            if (!needCast(arrayClass)) {
                return !isSetter ? MethodType.methodType(elemClass, arrayArgClass, int.class)
                    : MethodType.methodType(void.class, arrayArgClass, int.class, elemClass);
            } else {
                Class<?> classArgClass = Class.class;
                if (USE_WEAKLY_TYPED_ARRAY_ACCESSORS) { classArgClass = Object.class; }
                return !isSetter ? MethodType.methodType(Object.class, classArgClass, arrayArgClass, int.class)
        ...
        static MethodType correctType(Class<?> arrayClass, boolean isSetter) {
            Class<?> elemClass = arrayClass.getComponentType();
            return !isSetter ? MethodType.methodType(elemClass, arrayClass, int.class)
        ...
        for (int i = 0; i < INARG_COUNT; i++) {
            ...
            if (src.isPrimitive()) {
                if (dst.isPrimitive()) {
                    ...
                    if (dst == w.wrapperType()) { fn = boxMethod; } else {
    ...
    static class AsVarargsCollector extends MethodHandle {
        ...
        @Override
        MethodHandle reinvokerTarget() { return target; }
        ...
        public MethodHandle asTypeUncached(MethodType newType) {
            ...
            if (acc != null && acc.type().parameterCount() == newArity) { return asTypeCache = acc.asType(newType); }
        ...
        MethodHandle viewAsType(MethodType newType) {
            if (newType.lastParameterType() != type().lastParameterType()) { throw new InternalError(); }
    ...
    static MethodHandle makeSpreadArguments(MethodHandle target, Class<?> spreadArgType, int spreadArgPos,
        int spreadArgCount) {
        ...
        static {
            try {
                NF_checkSpreadArgument = new NamedFunction(
                    MethodHandleImpl.class.getDeclaredMethod("checkSpreadArgument", Object.class, int.class));
    ...
    static MethodHandle makeCollectArguments(MethodHandle target, MethodHandle collector, int collectArgPos,
        ...
        names[collectNamePos] = new Name(collector, collectorArgs);
        ...
        names[targetNamePos] = new Name(target, targetArgs);
    ...
    static MethodHandle makeGuardWithCatch(MethodHandle target, Class<? extends Throwable> exType,
        ...
        if (nargs < GuardWithCatch.INVOKES.length) {
            ...
            MethodType catcherType = ctype.changeParameterType(0, Throwable.class).changeReturnType(Object.class);
    ...
    static MethodHandle throwException() {
        ...
        try {
            mh = IMPL_LOOKUP.findStatic(MethodHandleImpl.class, "throwException",
    ...
    static MethodHandle fakeMethodHandleInvoke(MemberName method) {
        ...
        MethodType type = MethodType
            .methodType(Object.class, UnsupportedOperationException.class, MethodHandle.class, Object[].class);
        mh = throwException(type);
        mh = mh.bindTo(new UnsupportedOperationException("cannot reflectively invoke MethodHandle"));
        if (!method.getInvocationType().equals(mh.type())) { throw new InternalError(method.toString()); }
    ...
    private static class BindCaller {
        static MethodHandle bindCaller(MethodHandle mh, Class<?> hostClass) {
            ...
            if (hostClass == null ||
                (hostClass.isArray() || hostClass.isPrimitive() || hostClass.getName().startsWith("java.") ||
        ...
        static {
            ...
            try {
                MH_checkCallerClass = IMPL_LOOKUP.findStatic(THIS_CLASS, "checkCallerClass",
    ...
    static MethodHandle makeWrappedMember(MethodHandle target, MemberName member) {
        if (member.equals(target.internalMemberName())) { return target; }
