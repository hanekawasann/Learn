...
class Invokers {
    ...
    private boolean checkInvoker(MethodHandle invoker) {
        assert (targetType.invokerType().equals(invoker.type())) : java.util.Arrays
            .asList(targetType, targetType.invokerType(), invoker);
    ...
    /*non-public*/ MethodHandle spreadInvoker(int leadingArgCount) {
        ...
        if (targetType.parameterSlotCount() <= MethodType.MAX_MH_INVOKER_ARITY) {
            ...
            try {
                makeSpreader = IMPL_LOOKUP.findVirtual(MethodHandle.class, "asSpreader",
        ...
        if (invoker == null) {
            try {
                THROW_UCS = invoker = IMPL_LOOKUP
                    .findStatic(CallSite.class, "uninitializedCallSite", MethodType.methodType(Empty.class));
            } catch (ReflectiveOperationException ex) {
                throw newInternalError(ex);
            }
        }
    ...
    private static LambdaForm invokeHandleForm(MethodType mtype, boolean customized, int which) {
        ...
        if (isCached) { lform = mtype.form().setCachedLambdaForm(which, lform); }
        return lform;
    }
    ...
    void checkExactType(Object mhObj, Object expectedObj) {
        ...
        if (actual != expected) { throw newWrongMethodTypeException(expected, actual); }
    }
    ...
    private static LambdaForm callSiteForm(MethodType mtype, boolean skipCallSite) {
        ...
        if (!skipCallSite) { names[CALL_MH] = new Name(NF_getCallSiteTarget, names[CSITE_ARG]); }
    ...
    static {
        try {
            NF_checkExactType = new NamedFunction(
                Invokers.class.getDeclaredMethod("checkExactType", Object.class, Object.class));
            NF_checkGenericType = new NamedFunction(
                Invokers.class.getDeclaredMethod("checkGenericType", Object.class, Object.class));
            NF_asType = new NamedFunction(MethodHandle.class.getDeclaredMethod("asType", MethodType.class));
            NF_getCallSiteTarget = new NamedFunction(
                Invokers.class.getDeclaredMethod("getCallSiteTarget", Object.class));
