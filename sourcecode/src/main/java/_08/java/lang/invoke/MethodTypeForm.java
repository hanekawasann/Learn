...
final class MethodTypeForm {
    ...
    @Stable
    String typeString;           // argument type signature characters
    @Stable
    MethodHandle genericInvoker; // JVM hook for inexact invoke
    @Stable
    MethodHandle basicInvoker;   // cached instance of MH.invokeBasic
    @Stable
    MethodHandle namedFunctionInvoker; // cached helper for LF.NamedFunction
    ...
    static final int LF_INVVIRTUAL = 0,  // DMH invokeVirtual
        LF_INVSTATIC = 1, LF_INVSPECIAL = 2, LF_NEWINVSPECIAL = 3, LF_INVINTERFACE = 4, LF_INVSTATIC_INIT
        = 5,  // DMH invokeStatic with <clinit> barrier
        LF_INTERPRET = 6,  // LF interpreter
        LF_COUNTER = 7,  // CMH wrapper
        LF_REINVOKE = 8,  // other wrapper
        LF_EX_LINKER = 9,  // invokeExact_MT
        LF_EX_INVOKER = 10,  // invokeExact MH
        LF_GEN_LINKER = 11, LF_GEN_INVOKER = 12, LF_CS_LINKER = 13,  // linkToCallSite_CS
    ...
    public MethodHandle basicInvoker() {
        assert (erasedType == basicType) :
            "erasedType: " + erasedType + " != basicType: " + basicType;  // primitives must be flattened also
    ...
    protected MethodTypeForm(MethodType erasedType) {
        ...
        if (rt != Object.class) {
            ...
            if (w.isSubwordOrInt() && rt != int.class) { bt = int.class; }
            // adjust #slots, #args
            if (rt == void.class) { rtypeCount = rslotCount = 0; } else { rslotCount += lrc; }
        }
        if (epts == bpts && bt == rt) {
