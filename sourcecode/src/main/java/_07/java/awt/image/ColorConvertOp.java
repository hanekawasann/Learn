...
public class ColorConvertOp implements BufferedImageOp, RasterOp {
    ...
    private final BufferedImage ICCBIFilter(BufferedImage src, ColorSpace srcColorSpace, BufferedImage dest,
    ...
    public final WritableRaster filter(Raster src, WritableRaster dest) {
        ...
        if (dest == null) {
            ...
            if (dest.getNumBands() != profileList[nProfiles - 1].getNumComponents()) {
                throw new IllegalArgumentException(
                    "Numbers of destination Raster bands and destination " + "color space components do not match");
            }
        }
    ...
    private BufferedImage createCompatibleDestImage(BufferedImage src, ColorModel destCM, ColorSpace destCS) {
        ...
        int w = src.getWidth();
        int h = src.getHeight();
        image = new BufferedImage(destCM, destCM.createCompatibleWritableRaster(w, h), destCM.isAlphaPremultiplied(),
            null);
        return image;
    }
    ...
    public WritableRaster createCompatibleDestRaster(Raster src) {
        ...
        WritableRaster dest = Raster
            .createInterleavedRaster(DataBuffer.TYPE_BYTE, src.getWidth(), src.getHeight(), ncomponents,
    ...
    private int getRenderingIntent(ICC_Profile profile) {
        ...
        return (((header[index] & 0xff) << 24) | ((header[index + 1] & 0xff) << 16) |
            ((header[index + 2] & 0xff) << 8) | (header[index + 3] & 0xff));
    }
    ...
    private final BufferedImage nonICCBIFilter(BufferedImage src, ColorSpace srcColorSpace, BufferedImage dst,
    ...
    private final WritableRaster nonICCRasterFilter(Raster src, WritableRaster dst) {
        ...
        srcIsFloat = (srcTransferType == DataBuffer.TYPE_FLOAT) || (srcTransferType == DataBuffer.TYPE_DOUBLE);
        dstIsFloat = (dstTransferType == DataBuffer.TYPE_FLOAT) || (dstTransferType == DataBuffer.TYPE_DOUBLE);
        int w = src.getWidth();
        int h = src.getHeight();
