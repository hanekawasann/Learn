    ...
    */
    ...
    private TextLineComponent[] makeComponentsOnRange(int startPos, int limitPos) {
        ...
        while (linePos < limitPos) {
            ...
            components[newCompIndex++] = fComponents[tlcIndex]
                .getSubset(start - tlcStart, limit - tlcStart, subsetFlag);
            linePos += (limit - start);
            if (linePos == breakPt) {
                breakPt = limitPos;
                subsetFlag = fIsDirectionLTR ? TextLineComponent.LEFT_TO_RIGHT : TextLineComponent.RIGHT_TO_LEFT;
    ...
    private TextLine makeTextLineOnRange(int startPos, int limitPos) {
        ...
        return new TextLine(fFrc, components, fBaselineOffsets, fChars, startPos, limitPos, charsLtoV, charLevels,
            fIsDirectionLTR);
