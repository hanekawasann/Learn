...
public class Socket implements java.io.Closeable {
    ...
    public Socket(Proxy proxy) {
        ...
        if (p.type() == Proxy.Type.SOCKS) {
            ...
            if (security != null) {
                ...
                if (epoint.isUnresolved()) { security.checkConnect(epoint.getHostName(), epoint.getPort()); } else {
    ...
    public Socket(InetAddress address, int port, InetAddress localAddr, int localPort) throws IOException {
        this(address != null ? new InetSocketAddress(address, port) : null, new InetSocketAddress(localAddr, localPort),
            true);
    }
    ...
    public void connect(SocketAddress endpoint, int timeout) throws IOException {
        ...
        SecurityManager security = System.getSecurityManager();
        if (security != null) {
            if (epoint.isUnresolved()) { security.checkConnect(epoint.getHostName(), port); } else {
                security.checkConnect(addr.getHostAddress(), port);
            }
        }
        if (!created) { createImpl(true); }
        if (!oldImpl) { impl.connect(epoint, timeout); } else if (timeout == 0) {
            if (epoint.isUnresolved()) { impl.connect(addr.getHostName(), port); } else { impl.connect(addr, port); }
    ...
    public String toString() {
        try {
            if (isConnected()) {
                return "Socket[addr=" + getImpl().getInetAddress() + ",port=" + getImpl().getPort() + ",localport=" +
     ...
     * will have no effect.
     *
     * @param connectionTime An <tt>int</tt> expressing the relative importance of a short
     *                       connection time
     * @param latency        An <tt>int</tt> expressing the relative importance of low
     *                       latency
     * @param bandwidth      An <tt>int</tt> expressing the relative importance of high
     *                       bandwidth
     * @since 1.5
     */
