...
class AdapterMethodHandle extends BoundMethodHandle {
    ...
    private AdapterMethodHandle(MethodHandle target, MethodType newType, long conv) {
    ...
    static boolean canPairwiseConvert(MethodType newType, MethodType oldType, int level) {
        ...
        if (len != oldType.parameterCount()) { return false; }
        ...
        if (!VerifyType.isNullConversion(ret, exp)) {
            ...
            if (!canConvertArgument(ret, exp, level)) { return false; }
        }
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (!canConvertArgument(src, dst, level)) { return false; }
        }
    ...
    static boolean canConvertArgument(Class<?> src, Class<?> dst, int level) {
        ...
        if (VerifyType.isNullConversion(src, dst)) {
            ...
            if (dst.isPrimitive()) { return canPrimCast(src, dst); } else { return canBoxArgument(src, dst); }
        } else {
            if (dst.isPrimitive()) { return canUnboxArgument(src, dst, level); } else {
    ...
    static MethodHandle makePairwiseConvert(MethodType newType, MethodHandle target, int level) {
        ...
        if (!canPairwiseConvert(newType, oldType, level)) { return null; }
    ...
    private static boolean isTrivialConversion(Class<?> src, Class<?> dst, int level) {
        ...
        if (level > 1) {
            return true;  // explicitCastArguments
        }
    ...
    static MethodHandle makePermutation(MethodType newType, MethodHandle target, int[] argumentMap) {
        ...
        for (int i = 0; i < argumentMap.length; i++) {
            ...
            if (pos != i) { nullPermutation = false; }
            if (pos < 0 || pos >= newType.parameterCount()) {
                argumentMap = new int[0];
                break;
            }
        }
        ...
        if (nullPermutation) {
            ...
            // well, that was easy
            if (res == null) { throw newIllegalArgumentException("cannot convert pairwise: " + newType); }
            return res;
        }
    ...
    private static long makeConv(int convOp, int argnum, int src, int dest) {
        ...
        return ((long) argnum << 32 | (long) convOp << CONV_OP_SHIFT | src << CONV_SRC_TYPE_SHIFT |
            dest << CONV_DEST_TYPE_SHIFT | insertStackMove(stackMove));
    }
    ...
    private static long makeDupConv(int convOp, int argnum, int stackMove) {
        ...
        byte src = 0, dest = 0;
        return ((long) argnum << 32 | (long) convOp << CONV_OP_SHIFT | (int) src << CONV_SRC_TYPE_SHIFT |
            (int) dest << CONV_DEST_TYPE_SHIFT | insertStackMove(stackMove));
    }
    ...
    private static long makeSwapConv(int convOp, int srcArg, byte srcType, int destSlot, byte destType) {
        ...
        return ((long) srcArg << 32 | (long) convOp << CONV_OP_SHIFT | (int) srcType << CONV_SRC_TYPE_SHIFT |
            (int) destType << CONV_DEST_TYPE_SHIFT | destSlot << CONV_VMINFO_SHIFT);
    }
    ...
    private static int diffTypes(MethodType adapterType, MethodType targetType, boolean raw) {
        ...
        if (nargs != targetType.parameterCount()) { return -1; }
    ...
    private static int diffParamTypes(MethodType adapterType, int astart, MethodType targetType, int tstart, int nargs,
        boolean raw) {
        assert (nargs >= 0);
        int res = 0;
        for (int i = 0; i < nargs; i++) {
            ...
            if ((!raw ? VerifyType.canPassUnchecked(src, dest) : VerifyType.canPassRaw(src, dest)) <= 0) {
    ...
    static MethodHandle makeRetype(MethodType newType, MethodHandle target, boolean raw) {
        ...
        if (!canRetype(newType, oldType, raw)) { return null; }
    ...
    static class AsVarargsCollector extends AdapterMethodHandle {
        ...
        public MethodHandle asType(MethodType newType) {
            ...
            // check cache
            if (cache.type().parameterCount() == newArity) { return cache.asType(newType); }
    ...
    static boolean canCheckCast(MethodType newType, MethodType targetType, int arg, Class<?> castType) {
        ...
        if (!canCheckCast(src, castType) || !VerifyType.isNullConversion(castType, dst)) { return false; }
    ...
    static MethodHandle makeCheckCast(MethodType newType, MethodHandle target, int arg, Class<?> castType) {
        if (!canCheckCast(newType, target.type(), arg, castType)) { return null; }
    ...
    static boolean canPrimCast(MethodType newType, MethodType targetType, int arg, Class<?> convType) {
        ...
        Class<?> src = newType.parameterType(arg);
        Class<?> dst = targetType.parameterType(arg);
        if (!canPrimCast(src, convType) || !VerifyType.isNullConversion(convType, dst)) { return false; }
        int diff = diffTypes(newType, targetType, false);
    ...
    static MethodHandle makePrimCast(MethodType newType, MethodHandle target, int arg, Class<?> convType) {
        Class<?> src = newType.parameterType(arg);
        if (canPrimCast(src, convType)) { return makePrimCastOnly(newType, target, arg, convType); }
        Class<?> dst = convType;
        boolean sflt = Wrapper.forPrimitiveType(src).isFloating();
        boolean dflt = Wrapper.forPrimitiveType(dst).isFloating();
        if (sflt | dflt) {
            ...
            if (sflt) {
                convMethod = ((src == double.class) ? ValueConversions.convertFromDouble(dst)
                    : ValueConversions.convertFromFloat(dst));
            } else {
                convMethod = ((dst == double.class) ? ValueConversions.convertToDouble(src)
    ...
    static MethodHandle makePrimCastOnly(MethodType newType, MethodHandle target, int arg, Class<?> convType) {
        MethodType oldType = target.type();
        if (!canPrimCast(newType, oldType, arg, convType)) { return null; }
        Class<?> src = newType.parameterType(arg);
    ...
    static boolean canUnboxArgument(MethodType newType, MethodType targetType, int arg, Class<?> convType, int level) {
        ...
        if (!canCheckCast(src, boxType) || boxType == convType || !VerifyType.isNullConversion(convType, dst)) {
            return false;
        }
        int diff = diffTypes(newType, targetType, false);
        return (diff == arg + 1);  // arg is sole non-trivial diff
     ...
     * Return null if this cannot be done.
     */
    static MethodHandle makeUnboxArgument(MethodType newType, MethodHandle target, int arg, Class<?> convType,
        int level) {
        MethodType oldType = target.type();
        Class<?> src = newType.parameterType(arg);
        ...
        if (!canUnboxArgument(newType, oldType, arg, convType, level)) { return null; }
        ...
        if (!VerifyType.isNullConversion(src, boxType)) {
            ...
            if (level != 0) {
                ...
                if (src == Object.class || !Wrapper.isWrapperType(src)) {
                    ...
                    MethodHandle unboxMethod = (level == 1 ? ValueConversions.unbox(dst)
        ...
        if (castDone == newType) { return adapter; }
    ...
    static boolean canBoxArgument(MethodType newType, MethodType targetType, int arg, Class<?> convType) {
        ...
        Class<?> boxType = Wrapper.asWrapperType(convType);
        convType = Wrapper.asPrimitiveType(convType);
        if (!canCheckCast(boxType, dst) || boxType == convType || !VerifyType.isNullConversion(src, convType)) {
            return false;
        }
        int diff = diffTypes(newType, targetType, false);
        return (diff == arg + 1);  // arg is sole non-trivial diff
    ...
    static boolean canDropArguments(MethodType newType, MethodType targetType, int dropArgPos, int dropArgCount) {
        if (dropArgCount == 0) { return canRetypeOnly(newType, targetType); }
        if (!convOpSupported(OP_DROP_ARGS)) { return false; }
        if (diffReturnTypes(newType, targetType, false) != 0) { return false; }
        int nptypes = newType.parameterCount();
        // parameter types must be the same up to the drop point
        if (dropArgPos != 0 && diffParamTypes(newType, 0, targetType, 0, dropArgPos, false) != 0) { return false; }
        int afterPos = dropArgPos + dropArgCount;
        int afterCount = nptypes - afterPos;
        if (dropArgPos < 0 || dropArgPos >= nptypes || dropArgCount < 1 || afterPos > nptypes ||
            targetType.parameterCount() != nptypes - dropArgCount) { return false; }
        // parameter types after the drop point must also be the same
        return afterCount == 0 || diffParamTypes(newType, afterPos, targetType, dropArgPos, afterCount, false) == 0;
    }
    ...
    static MethodHandle makeDropArguments(MethodType newType, MethodHandle target, int dropArgPos, int dropArgCount) {
        if (dropArgCount == 0) { return makeRetypeOnly(newType, target); }
        if (!canDropArguments(newType, target.type(), dropArgPos, dropArgCount)) { return null; }
    ...
    static boolean canDupArguments(MethodType newType, MethodType targetType, int dupArgPos, int dupArgCount) {
        ...
        if (diffReturnTypes(newType, targetType, false) != 0) { return false; }
        int nptypes = newType.parameterCount();
        if (dupArgCount < 0 || dupArgPos + dupArgCount > nptypes) { return false; }
        if (targetType.parameterCount() != nptypes + dupArgCount) { return false; }
        // parameter types must be the same up to the duplicated arguments
        if (diffParamTypes(newType, 0, targetType, 0, nptypes, false) != 0) { return false; }
        // duplicated types must be, well, duplicates
        return diffParamTypes(newType, dupArgPos, targetType, nptypes, dupArgCount, false) == 0;
    }
    ...
    static MethodHandle makeDupArguments(MethodType newType, MethodHandle target, int dupArgPos, int dupArgCount) {
        if (!canDupArguments(newType, target.type(), dupArgPos, dupArgCount)) { return null; }
        if (dupArgCount == 0) { return target; }
    ...
    static boolean canSwapArguments(MethodType newType, MethodType targetType, int swapArg1, int swapArg2) {
        ...
        if (swapArg1 >= swapArg2) {
            return false;  // caller resp
        }
        int nptypes = newType.parameterCount();
        if (targetType.parameterCount() != nptypes) { return false; }
        if (swapArg1 < 0 || swapArg2 >= nptypes) { return false; }
        if (diffParamTypes(newType, 0, targetType, 0, swapArg1, false) != 0) { return false; }
        if (diffParamTypes(newType, swapArg1, targetType, swapArg2, 1, false) != 0) { return false; }
        if (diffParamTypes(newType, swapArg1 + 1, targetType, swapArg1 + 1, swapArg2 - swapArg1 - 1, false) != 0) {
            return false;
        }
        if (diffParamTypes(newType, swapArg2, targetType, swapArg1, 1, false) != 0) { return false; }
        return diffParamTypes(newType, swapArg2 + 1, targetType, swapArg2 + 1, nptypes - swapArg2 - 1, false) == 0;
    }
    ...
    static MethodHandle makeSwapArguments(MethodType newType, MethodHandle target, int swapArg1, int swapArg2) {
        if (swapArg1 == swapArg2) { return target; }
        if (swapArg1 > swapArg2) {
            int t = swapArg1;
            swapArg1 = swapArg2;
            swapArg2 = t;
        }
        if (type2size(newType.parameterType(swapArg1)) != type2size(newType.parameterType(swapArg2))) {
        ...
        if (!canSwapArguments(newType, target.type(), swapArg1, swapArg2)) { return null; }
    ...
    static int positiveRotation(int argCount, int rotateBy) {
        assert (argCount > 0);
        if (rotateBy >= 0) {
            if (rotateBy < argCount) { return rotateBy; }
    ...
    static boolean canRotateArguments(MethodType newType, MethodType targetType, int firstArg, int argCount,
        int rotateBy) {
        ...
        if (diffParamTypes(newType, 0, targetType, 0, firstArg, false) != 0) { return false; }
        ...
        return diffParamTypes(newType, firstArg + newChunk1, targetType, firstArg, newChunk2, false) == 0;
    }
     ...
     * Return null if this is not possible.
     */
    static MethodHandle makeRotateArguments(MethodType newType, MethodHandle target, int firstArg, int argCount,
        int rotateBy) {
        rotateBy = positiveRotation(argCount, rotateBy);
        if (!canRotateArguments(newType, target.type(), firstArg, argCount, rotateBy)) { return null; }
    ...
    static boolean canSpreadArguments(MethodType newType, MethodType targetType, Class<?> spreadArgType,
        int spreadArgPos, int spreadArgCount) {
        if (!convOpSupported(OP_SPREAD_ARGS)) { return false; }
        if (diffReturnTypes(newType, targetType, false) != 0) { return false; }
        int nptypes = newType.parameterCount();
        // parameter types must be the same up to the spread point
        if (spreadArgPos != 0 && diffParamTypes(newType, 0, targetType, 0, spreadArgPos, false) != 0) { return false; }
        int afterPos = spreadArgPos + spreadArgCount;
        int afterCount = nptypes - (spreadArgPos + 1);
        if (spreadArgPos < 0 || spreadArgPos >= nptypes || spreadArgCount < 0 ||
            targetType.parameterCount() != afterPos + afterCount) { return false; }
        // parameter types after the spread point must also be the same
        if (afterCount != 0 &&
            diffParamTypes(newType, spreadArgPos + 1, targetType, afterPos, afterCount, false) != 0) { return false; }
        // match the array element type to the spread arg types
        Class<?> rawSpreadArgType = newType.parameterType(spreadArgPos);
        if (rawSpreadArgType != spreadArgType && !canCheckCast(rawSpreadArgType, spreadArgType)) { return false; }
        for (int i = 0; i < spreadArgCount; i++) {
            Class<?> src = VerifyType.spreadArgElementType(spreadArgType, i);
            Class<?> dst = targetType.parameterType(spreadArgPos + i);
            if (src == null || !canConvertArgument(src, dst, 1)) { return false; }
        }
        return true;
    }
    ...
    static MethodHandle makeSpreadArguments(MethodType newType, MethodHandle target, Class<?> spreadArgType,
        int spreadArgPos, int spreadArgCount) {
        ...
        for (int i = 0; i < spreadArgCount; i++) {
            ...
            if (dest == T_VOID) { dest = dest2; } else if (dest != dest2) { dest = T_VOID; }
    ...
    static boolean canCollectArguments(MethodType targetType, MethodType collectorType, int collectArgPos,
    ...
    static MethodHandle makeCollectArguments(MethodHandle target, MethodHandle collector, int collectArgPos,
        ...
        if (!retainOriginalArgs) {
            ...
            assert (diffParamTypes(newType, collectArgPos, targetType, collectValCount, collectArgCount, false) ==
                0) : Arrays.asList(target, collector, collectArgPos, retainOriginalArgs);
        }
        ...
        assert (collectSlot == targetType
            .parameterSlotDepth(collectArgPos + collectValCount + (retainOriginalArgs ? collectArgCount : 0)));
        ...
        for (int i = 0; i < collectArgCount; i++) {
            ...
            if (src == T_VOID) { src = src2; } else if (src != src2) { src = T_VOID; }
            if (src == T_VOID) { break; }
        }
        int stackMove = collectValSlots;  // push 0..2 results
        if (!retainOriginalArgs) {
            stackMove -= slotCount; // pop N arguments
        }
        int lastCollectArg = keep2InPos - 1;
        long conv = makeSpreadConv(retainOriginalArgs ? OP_FOLD_ARGS : OP_COLLECT_ARGS, lastCollectArg, src, dest,
            stackMove);
