...
/*non-public*/ final class MemberName implements Member, Cloneable {
    ...
    public MethodType getMethodOrFieldType() {
        if (isInvocable()) { return getMethodType(); }
        if (isGetter()) { return MethodType.methodType(getFieldType()); }
        if (isSetter()) { return MethodType.methodType(void.class, getFieldType()); }
    ...
    public MethodType getInvocationType() {
        ...
        if (isConstructor() && getReferenceKind() == REF_newInvokeSpecial) { return itype.changeReturnType(clazz); }
    ...
    public String getSignature() {
        ...
        if (isInvocable()) { return BytecodeDescriptor.unparse(getMethodType()); } else {
    ...
    private boolean referenceKindIsConsistent() {
        ...
        if (isField()) {
            ...
            assert !clazz.isInterface() ||
                (refKind == REF_invokeInterface || refKind == REF_invokeStatic || refKind == REF_invokeSpecial ||
    ...
    private boolean isObjectPublicMethod() {
        ...
        if (name.equals("hashCode") && mtype.returnType() == int.class && mtype.parameterCount() == 0) { return true; }
        return name.equals("equals") && mtype.returnType() == boolean.class && mtype.parameterCount() == 1 &&
    ...
    public boolean isAccessibleFrom(Class<?> lookupClass) {
        return VerifyAccess.isMemberAccessible(this.getDeclaringClass(), this.getDeclaringClass(), flags, lookupClass,
            ALL_ACCESS | MethodHandles.Lookup.PACKAGE);
    }
    ...
    public MemberName(Method m, boolean wantSpecial) {
        ...
        if (clazz == null) {  // MHN.init failed
            if (m.getDeclaringClass() == MethodHandle.class && isMethodHandleInvokeName(m.getName())) {
                ...
                if (isMethodHandleInvoke()) { return; }
            }
            throw new LinkageError(m.toString());
        }
        assert (isResolved() && this.clazz != null);
        this.name = m.getName();
        if (this.type == null) { this.type = new Object[] { m.getReturnType(), m.getParameterTypes() }; }
        if (wantSpecial) {
            if (isAbstract()) { throw new AbstractMethodError(this.toString()); }
    ...
    public MemberName asNormalOriginal() {
        ...
        if (newRefKind == refKind) { return this; }
    ...
    public MemberName(Constructor<?> ctor) {
        ...
        if (this.type == null) { this.type = new Object[] { void.class, ctor.getParameterTypes() }; }
    }
    ...
    public boolean equals(MemberName that) {
        ...
        return this.clazz == that.clazz && this.getReferenceKind() == that.getReferenceKind() &&
            Objects.equals(this.name, that.name) && Objects.equals(this.getType(), that.getType());
    }
    ...
    public MemberName(byte refKind, Class<?> defClass, String name, Object type) {
        ...
        if (MethodHandleNatives.refKindIsField(refKind)) {
            kindFlags = IS_FIELD;
            if (!(type instanceof Class)) { throw newIllegalArgumentException("not a field type"); }
        } else if (MethodHandleNatives.refKindIsMethod(refKind)) {
            kindFlags = IS_METHOD;
            if (!(type instanceof MethodType)) { throw newIllegalArgumentException("not a method type"); }
    ...
    private void initResolved(boolean isResolved) {
        ...
        if (!isResolved) { this.resolution = this; }
    ...
    void checkForTypeAlias() {
        if (isInvocable()) {
            MethodType type;
            if (this.type instanceof MethodType) { type = (MethodType) this.type; } else {
            ...
            Class<?> type;
            if (this.type instanceof Class<?>) { type = (Class<?>) this.type; } else {
    ...
    private String message() {
        if (isResolved()) { return "no access"; } else if (isConstructor()) {
    ...
    public ReflectiveOperationException makeAccessException() {
        ...
        } else if (isConstructor()) { ex = new NoSuchMethodException(message); } else if (isMethod()) {
            ex = new NoSuchMethodException(message);
        } else {
            ex = new NoSuchFieldException(message);
        }
        if (resolution instanceof Throwable) { ex.initCause((Throwable) resolution); }
        return ex;
    }
    ...
    /*non-public*/ static class Factory {
        ...
        /// Queries
        List<MemberName> getMembers(Class<?> defc, String matchName, Object matchType, int matchFlags,
            Class<?> lookupClass) {
            matchFlags &= ALLOWED_FLAGS;
            String matchSig = null;
            if (matchType != null) {
                matchSig = BytecodeDescriptor.unparse(matchType);
                if (matchSig.startsWith("(")) { matchFlags &= ~(ALL_KINDS & ~IS_INVOCABLE); } else {
            ...
            for (; ; ) {
                bufCount = MethodHandleNatives
                    .getMembers(defc, matchName, matchSig, matchFlags, lookupClass, totalCount, buf);
         ...
         * Otherwise a fresh copy of the given member is returned, with modifier bits filled in.
         */
        public <NoSuchMemberException extends ReflectiveOperationException> MemberName resolveOrFail(byte refKind,
            MemberName m, Class<?> lookupClass, Class<NoSuchMemberException> nsmClass)
         ...
         * Inaccessible members are not added to the last.
         */
        public List<MemberName> getMethods(Class<?> defc, boolean searchSupers, String name, MethodType type,
            Class<?> lookupClass) {
         ...
         * Inaccessible members are not added to the last.
         */
        public List<MemberName> getFields(Class<?> defc, boolean searchSupers, String name, Class<?> type,
            Class<?> lookupClass) {
