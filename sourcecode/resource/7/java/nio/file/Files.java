...
public final class Files {
    ...
    public static Path createFile(Path path, FileAttribute<?>... attrs) throws IOException {
        EnumSet<StandardOpenOption> options = EnumSet.<StandardOpenOption>of(StandardOpenOption.CREATE_NEW,
    ...
    private static class FileTypeDetectors {
        static final FileTypeDetector defaultFileTypeDetector = sun.nio.fs.DefaultFileTypeDetector.create();
    ...
    private static byte[] read(InputStream source, int initialSize) throws IOException {
        ...
        while ((n = source.read(buf, nread, rem)) > 0) {
            ...
            if (rem == 0) {
                ...
                if (newCapacity < 0) {
                    if (capacity == Integer.MAX_VALUE) { throw new OutOfMemoryError("Required array size too large"); }
    ...
    public static byte[] readAllBytes(Path path) throws IOException {
        long size = size(path);
        if (size > (long) Integer.MAX_VALUE) { throw new OutOfMemoryError("Required array size too large"); }
