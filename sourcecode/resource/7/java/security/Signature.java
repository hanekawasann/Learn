...
public abstract class Signature extends SignatureSpi {
    ...
    private final static List<ServiceId> rsaIds = Arrays.asList(
        new ServiceId[] { new ServiceId("Signature", "NONEwithRSA"), new ServiceId("Cipher", "RSA/ECB/PKCS1Padding"),
        ...
        if (result == null) {
            try {
                ...
                if ((debug != null) && (r == false)) {
                    ...
                    debug.println(
                        "Delayed provider selection may not be " + "available for algorithm " + s.getAlgorithm());
                }
    ...
    private static Signature getInstanceRSA(Provider p) throws NoSuchAlgorithmException {
        ...
        try {
            ...
            throw new NoSuchAlgorithmException("no such algorithm: " + RSA_SIGNATURE + " for provider " + p.getName(),
                e);
        }
    }
     ...
     *                                  encoded or of the wrong type, if this signature algorithm is unable to
     *                                  process the input data provided, etc.
     * @throws IllegalArgumentException if the <code>signature</code>
        ...
        void chooseFirstProvider() {
            ...
            synchronized (lock) {
                ...
                if (debug != null) {
                    ...
                    if (w >= 0) {
                        debug.println(
                            "Signature.init() not first method " + "called, disabling delayed provider selection");
                        if (w == 0) {
