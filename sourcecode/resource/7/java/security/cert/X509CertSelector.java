...
public class X509CertSelector implements CertSelector {
    ...
    private final static ObjectIdentifier ANY_EXTENDED_KEY_USAGE = ObjectIdentifier
    ...
    public boolean match(Certificate cert) {
        ...
        boolean result = matchBasicConstraints(xcert) && matchKeyUsage(xcert) && matchExtendedKeyUsage(xcert) &&
            matchSubjectKeyID(xcert) && matchAuthorityKeyID(xcert) && matchPrivateKeyValid(xcert) &&
            matchSubjectPublicKeyAlgID(xcert) && matchPolicy(xcert) && matchSubjectAlternativeNames(xcert) &&
            matchPathToNames(xcert) && matchNameConstraints(xcert);
    ...
    private boolean matchPrivateKeyValid(X509Certificate xcert) {
        ...
        try {
            ...
            if (debug != null) {
                String time = "n/a";
                try {
                    Date notAfter = (Date) ext.get(PrivateKeyUsageExtension.NOT_AFTER);
                    time = notAfter.toString();
                } catch (CertificateException ex) {
                    // not able to retrieve notAfter value
                }
                debug.println(
                    "X509CertSelector.match: private key usage not " + "within validity date; ext.NOT_After: " + time +
                        "; X509CertSelector: " + this.toString());
                e1.printStackTrace();
            }
            return false;
            ...
            if (debug != null) {
                String time = "n/a";
                try {
                    Date notBefore = (Date) ext.get(PrivateKeyUsageExtension.NOT_BEFORE);
                    time = notBefore.toString();
                } catch (CertificateException ex) {
                    // not able to retrieve notBefore value
                }
                debug.println(
                    "X509CertSelector.match: private key usage not " + "within validity date; ext.NOT_BEFORE: " + time +
                        "; X509CertSelector: " + this.toString());
                e2.printStackTrace();
            }
            return false;
            ...
            if (debug != null) {
                debug.println(
                    "X509CertSelector.match: CertificateException " + "in private key usage check; X509CertSelector: " +
                        this.toString());
                e3.printStackTrace();
            }
            return false;
            ...
            if (debug != null) {
                debug.println(
                    "X509CertSelector.match: IOException in " + "private key usage check; X509CertSelector: " +
                        this.toString());
                e4.printStackTrace();
            }
            return false;
    ...
    private boolean matchSubjectPublicKeyAlgID(X509Certificate xcert) {
        ...
        try {
            ...
            if (debug != null) {
                debug.println("X509CertSelector.match: subjectPublicKeyAlgID = " + subjectPublicKeyAlgID +
                    ", xcert subjectPublicKeyAlgID = " + algID.getOID());
            }
    ...
    private boolean matchExtendedKeyUsage(X509Certificate xcert) {
        ...
        try {
            ...
            if (ext != null) {
                Vector<ObjectIdentifier> certKeyPurposeVector = (Vector<ObjectIdentifier>) ext
    ...
    private boolean matchSubjectAlternativeNames(X509Certificate xcert) {
        ...
        try {
            SubjectAlternativeNameExtension sanExt = (SubjectAlternativeNameExtension) getExtensionObject(xcert,
                SUBJECT_ALT_NAME_ID);
            if (sanExt == null) {
                if (debug != null) {
            ...
            GeneralNames certNames = (GeneralNames) sanExt.get(SubjectAlternativeNameExtension.SUBJECT_NAME);
            ...
            while (i.hasNext()) {
                ...
                if (!found && (matchAllSubjectAltNames || !i.hasNext())) {
                    if (debug != null) {
                        debug.println(
                            "X509CertSelector.match: subject alternative " + "name " + matchName + " not found");
                    }
                    return false;
    ...
    private boolean matchPolicy(X509Certificate xcert) {
        ...
        try {
            CertificatePoliciesExtension ext = (CertificatePoliciesExtension) getExtensionObject(xcert,
                CERT_POLICIES_ID);
            if (ext == null) {
                if (debug != null) {
    ...
    private boolean matchPathToNames(X509Certificate xcert) {
        ...
        try {
            ...
            GeneralSubtrees permitted = (GeneralSubtrees) ext.get(NameConstraintsExtension.PERMITTED_SUBTREES);
            GeneralSubtrees excluded = (GeneralSubtrees) ext.get(NameConstraintsExtension.EXCLUDED_SUBTREES);
            if (excluded != null) {
                ...
                if (excludedName.getType() == pathToName.getType()) {
                    switch (pathToName.constrains(excludedName)) {
                        ...
                        case GeneralNameInterface.NAME_MATCH:
                            if (debug != null) {
                                debug.println(
                                    "X509CertSelector.match: name constraints " + "inhibit path to specified name");
            ...
            if (!permittedNameFound && nameTypeFound) {
                if (debug != null) {
                    debug.println("X509CertSelector.match: " + "name constraints inhibit path to specified name; " +
                        "permitted names of type " + pathToName.getType() + ": " + names);
                }
                return false;
            }
        }
    ...
    private boolean matchBasicConstraints(X509Certificate xcert) {
        ...
        if (basicConstraints == -2) {
            ...
            if (maxPathLen < basicConstraints) {
                if (debug != null) {
                    debug.println(
                        "X509CertSelector.match: maxPathLen too small (" + maxPathLen + " < " + basicConstraints + ")");
                }
                return false;
            }
        }
    ...
    public Object clone() {
        try {
            ...
            if (subjectAlternativeNames != null) {
                copy.subjectAlternativeNames = (Set<List<?>>) cloneSet(subjectAlternativeNames);
                copy.subjectAlternativeGeneralNames = (Set<GeneralNameInterface>) cloneSet(
                    subjectAlternativeGeneralNames);
            }
            if (pathToGeneralNames != null) {
                copy.pathToNames = (Set<List<?>>) cloneSet(pathToNames);
                copy.pathToGeneralNames = (Set<GeneralNameInterface>) cloneSet(pathToGeneralNames);
            }
            return copy;
