...
/*non-public*/ final class MemberName implements Member, Cloneable {
    ...
    static final int  // for MethodHandleNatives.getMembers
        SEARCH_SUPERCLASSES = MN_SEARCH_SUPERCLASSES, SEARCH_INTERFACES = MN_SEARCH_INTERFACES;
    ...
    public ReflectiveOperationException makeAccessException() {
        ...
        if (isResolved()) { return new IllegalAccessException(message); } else if (isConstructor()) {
    ...
    /*non-public*/ static class Factory {
        ...
        boolean resolveInPlace(MemberName m, boolean searchSupers, Class<?> lookupClass) {
            if (m.name == null || m.type == null) {  // find unique non-overloaded name
                ...
                } else if (m.isConstructor()) { choices = getConstructors(defc, lookupClass); } else if (m.isField()) {
            ...
            MemberName[] buf = { m };
            int n = MethodHandleNatives
                .getMembers(m.getDeclaringClass(), m.getName(), matchSig, matchFlags, lookupClass, 0, buf);
            if (n != 1) { return false; }
            return m.isResolved();
        }
         ...
         * Otherwise a fresh copy of the given member is returned, with modifier bits filled in.
         */
        public <NoSuchMemberException extends ReflectiveOperationException> MemberName resolveOrFail(MemberName m,
            boolean searchSupers, Class<?> lookupClass, Class<NoSuchMemberException> nsmClass)
