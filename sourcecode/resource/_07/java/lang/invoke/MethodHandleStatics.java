...
/*non-public*/ class MethodHandleStatics {
    ...
    static String getNameString(MethodHandle target, MethodType type) {
        if (type == null) { type = target.type(); }
        MemberName name = null;
        if (target != null) { name = MethodHandleNatives.getMethodName(target); }
        if (name == null) { return "invoke" + type; }
        return name.getName() + type;
    }

    /*non-public*/
    static String getNameString(MethodHandle target, MethodHandle typeHolder) {
        return getNameString(target, typeHolder == null ? null : typeHolder.type());
    }
    ...
    static Error uncaughtException(Exception ex) {
        Error err = new InternalError("uncaught exception", ex);
        return err;
    }
