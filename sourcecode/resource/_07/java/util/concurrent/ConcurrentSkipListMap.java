...
public class ConcurrentSkipListMap<K, V> extends AbstractMap<K, V>
    ...
    private Comparable<? super K> comparable(Object key) throws ClassCastException {
        ...
        if (comparator != null) { return new ComparableUsingComparator<K>((K) key, comparator); } else {
    ...
    int compare(K k1, K k2) throws ClassCastException {
        ...
        if (cmp != null) { return cmp.compare(k1, k2); } else { return ((Comparable<? super K>) k1).compareTo(k2); }
    }
    ...
    private Node<K, V> findPredecessorOfLast() {
        for (; ; ) {
            ...
            for (; ; ) {
                ...
                if ((d = q.down) != null) { q = d; } else { return q.node; }
            }
        }
    }
    ...
    static final class SubMap<K, V> extends AbstractMap<K, V>
        ...
        private ConcurrentSkipListMap.Node<K, V> loNode() {
            if (lo == null) { return m.findFirst(); } else if (loInclusive) {
                return m.findNear(lo, ConcurrentSkipListMap.GT | ConcurrentSkipListMap.EQ);
            } else { return m.findNear(lo, ConcurrentSkipListMap.GT); }
        }
        ...
        private ConcurrentSkipListMap.Node<K, V> hiNode() {
            if (hi == null) { return m.findLast(); } else if (hiInclusive) {
                return m.findNear(hi, ConcurrentSkipListMap.LT | ConcurrentSkipListMap.EQ);
            } else { return m.findNear(hi, ConcurrentSkipListMap.LT); }
        }
        ...
        private K lowestKey() {
            ...
            if (isBeforeEnd(n)) { return n.key; } else { throw new NoSuchElementException(); }
        }
        ...
        public Comparator<? super K> comparator() {
            ...
            if (isDescending) { return Collections.reverseOrder(cmp); } else { return cmp; }
        }
        ...
        public Map.Entry<K, V> ceilingEntry(K key) {
            return getNearEntry(key, (ConcurrentSkipListMap.GT | ConcurrentSkipListMap.EQ));
        }

        public K ceilingKey(K key) {
            return getNearKey(key, (ConcurrentSkipListMap.GT | ConcurrentSkipListMap.EQ));
        }

        public Map.Entry<K, V> lowerEntry(K key) {
            return getNearEntry(key, (ConcurrentSkipListMap.LT));
        }

        public K lowerKey(K key) {
            return getNearKey(key, (ConcurrentSkipListMap.LT));
        }

        public Map.Entry<K, V> floorEntry(K key) {
            return getNearEntry(key, (ConcurrentSkipListMap.LT | ConcurrentSkipListMap.EQ));
        }

        public K floorKey(K key) {
            return getNearKey(key, (ConcurrentSkipListMap.LT | ConcurrentSkipListMap.EQ));
        }

        public Map.Entry<K, V> higherEntry(K key) {
            return getNearEntry(key, (ConcurrentSkipListMap.GT));
        }

        public K higherKey(K key) {
            return getNearKey(key, (ConcurrentSkipListMap.GT));
        }
        ...
        abstract class SubMapIter<T> implements Iterator<T> {
            ...
            SubMapIter() {
                for (; ; ) {
                    ...
                    Object x = next.value;
                    if (x != null && x != next) {
                        if (!inBounds(next.key)) { next = null; } else { nextValue = (V) x; }
                        break;
                    }
                }
            }
            ...
            final void advance() {
                if (next == null) { throw new NoSuchElementException(); }
                lastReturned = next;
                if (isDescending) { descend(); } else { ascend(); }
            }
            ...
            private void ascend() {
                for (; ; ) {
                    ...
                    Object x = next.value;
                    if (x != null && x != next) {
                        if (tooHigh(next.key)) { next = null; } else { nextValue = (V) x; }
                        break;
                    }
                }
            }
            ...
            private void descend() {
                for (; ; ) {
                    ...
                    Object x = next.value;
                    if (x != null && x != next) {
                        if (tooLow(next.key)) { next = null; } else { nextValue = (V) x; }
                        break;
                    }
                }
            }
