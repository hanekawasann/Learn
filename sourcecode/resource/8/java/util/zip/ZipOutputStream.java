...
public class ZipOutputStream extends DeflaterOutputStream implements ZipConstants {
                    ...
                    if ((e.flag & 8) == 0) {
                        ...
                        if (e.size != def.getBytesRead()) {
                            throw new ZipException(
                                "invalid entry size (expected " + e.size + " but got " + def.getBytesRead() +
                                    " bytes)");
                        }
                        ...
                        if (e.crc != crc.getValue()) {
                            throw new ZipException(
                                "invalid entry CRC-32 (expected 0x" + Long.toHexString(e.crc) + " but got 0x" +
    ...
    private void writeLOC(XEntry xentry) throws IOException {
        ...
        if (flagEXTT != 0) {
            ...
            writeByte(flagEXTT);
            if (e.mtime != null) { writeInt(fileTimeToUnixTime(e.mtime)); }
            if (e.atime != null) { writeInt(fileTimeToUnixTime(e.atime)); }
            if (e.ctime != null) { writeInt(fileTimeToUnixTime(e.ctime)); }
        }
        writeExtra(e.extra);
        locoff = written;
    }
