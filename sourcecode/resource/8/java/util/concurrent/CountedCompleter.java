...
public abstract class CountedCompleter<T> extends ForkJoinTask<T> {
    ...
    public final CountedCompleter<?> firstComplete() {
        for (int c; ; ) {
            if ((c = pending) == 0) { return this; } else if (U.compareAndSwapInt(this, PENDING, c, c - 1)) {
                return null;
            }
        }
    }
    ...
    public final CountedCompleter<?> nextComplete() {
        CountedCompleter<?> p;
        if ((p = completer) != null) { return p.firstComplete(); } else {
            quietlyComplete();
            return null;
        }
    }
    ...
    public final void helpComplete(int maxTasks) {
        ...
        if (maxTasks > 0 && status >= 0) {
            ...
            } else { ForkJoinPool.common.externalHelpComplete(this, maxTasks); }
        }
    }
