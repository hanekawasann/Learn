...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
    ...
    private int setCompletion(int completion) {
        for (int s; ; ) {
            ...
            if (U.compareAndSwapInt(this, STATUS, s, s | completion)) {
                if ((s >>> 16) != 0) { synchronized (this) { notifyAll(); } }
                return completion;
            }
        }
    }
    ...
    private int externalInterruptibleAwaitDone() throws InterruptedException {
        ...
        while ((s = status) >= 0) {
            if (U.compareAndSwapInt(this, STATUS, s, s | SIGNAL)) {
                synchronized (this) {
                    if (status >= 0) { wait(); } else { notifyAll(); }
                }
            }
        }
        return s;
    ...
    private int doJoin() {
        ...
        return (s = status) < 0 ? s : ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) ?
            (w = (wt = (ForkJoinWorkerThread) t).workQueue).
                tryUnpush(this) && (s = doExec()) < 0 ? s : wt.pool.awaitJoin(w, this) : externalAwaitDone();
    }
    ...
    private int doInvoke() {
        ...
        ForkJoinWorkerThread wt;
        return (s = doExec()) < 0 ? s
            : ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) ? (wt = (ForkJoinWorkerThread) t).pool
                .awaitJoin(wt.workQueue, this) : externalAwaitDone();
    }
    ...
    private void clearExceptionalCompletion() {
        ...
        try {
            ...
            while (e != null) {
                ...
                if (e.get() == this) {
                    if (pred == null) { t[i] = next; } else { pred.next = next; }
                    break;
                }
                pred = e;
                e = next;
        ...
        if (false && e.thrower != Thread.currentThread().getId()) {
            ...
            try {
                ...
                for (int i = 0; i < cs.length; ++i) {
                    ...
                    if (ps.length == 0) { noArgCtor = c; } else if (ps.length == 1 && ps[0] == Throwable.class) {
    ...
    static void rethrow(Throwable ex) {
        if (ex != null) { ForkJoinTask.<RuntimeException>uncheckedThrow(ex); }
    }
    ...
    public static void invokeAll(ForkJoinTask<?>... tasks) {
        ...
        for (int i = last; i >= 0; --i) {
            ...
            } else if (i != 0) { t.fork(); } else if (t.doInvoke() < NORMAL && ex == null) { ex = t.getException(); }
        }
        for (int i = 1; i <= last; ++i) {
            ForkJoinTask<?> t = tasks[i];
            if (t != null) {
                if (ex != null) { t.cancel(false); } else if (t.doJoin() < NORMAL) { ex = t.getException(); }
            }
        }
    ...
    public final V get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
        ...
        if ((s = status) >= 0 && ns > 0L) {
            ...
            try {
                while ((s = status) >= 0) {
                    if (w != null && w.qlock < 0) { cancelIgnoringExceptions(this); } else if (!canBlock) {
                        if (p == null || p.tryCompensate(p.ctl)) { canBlock = true; }
                    } else {
    ...
    public static void helpQuiesce() {
        ...
        } else { ForkJoinPool.quiesceCommonPool(); }
    }
    ...
    public void reinitialize() {
        if ((status & DONE_MASK) == EXCEPTIONAL) { clearExceptionalCompletion(); } else { status = 0; }
    }
    ...
    public boolean tryUnfork() {
        Thread t;
        return (((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) ? ((ForkJoinWorkerThread) t).workQueue
            .tryUnpush(this) : ForkJoinPool.common.tryExternalUnpush(this));
    }
    ...
    protected static ForkJoinTask<?> pollTask() {
        Thread t;
        ForkJoinWorkerThread wt;
        return ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) ? (wt = (ForkJoinWorkerThread) t).pool
            .nextTaskFor(wt.workQueue) : null;
    }
