...
public class Exchanger<V> {
    ...
    private final Object slotExchange(Object item, boolean timed, long ns) {
        ...
        while ((v = p.match) == null) {
            if (spins > 0) {
                ...
                h ^= h >>> 3;
                h ^= h << 10;
                if (h == 0) { h = SPINS | (int) t.getId(); } else if (h < 0 && (--spins & ((SPINS >>> 1) - 1)) == 0) {
                    Thread.yield();
                }
            } else if (slot != p) { spins = SPINS; } else if (!t.isInterrupted() && arena == null &&
    ...
    public V exchange(V x) throws InterruptedException {
        ...
        if ((arena != null || (v = slotExchange(item, false, 0L)) == null) &&
            ((Thread.interrupted() || // disambiguates null return
                (v = arenaExchange(item, false, 0L)) == null))) { throw new InterruptedException(); }
    ...
    public V exchange(V x, long timeout, TimeUnit unit) throws InterruptedException, TimeoutException {
        ...
        if (v == TIMED_OUT) { throw new TimeoutException(); }
        return (v == NULL_ITEM) ? null : (V) v;
    }
    ...
    static {
        ...
        if ((s & (s - 1)) != 0 || s > (1 << ASHIFT)) { throw new Error("Unsupported array scale"); }
    }
