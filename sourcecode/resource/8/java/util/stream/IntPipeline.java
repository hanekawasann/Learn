...
abstract class IntPipeline<E_IN> extends AbstractPipeline<E_IN, Integer, IntStream> implements IntStream {
    ...
    private static IntConsumer adapt(Sink<Integer> sink) {
        if (sink instanceof IntConsumer) {
            ...
            if (Tripwire.ENABLED) { Tripwire.trip(AbstractPipeline.class, "using IntStream.adapt(Sink<Integer> s)"); }
            return sink::accept;
        }
    }
    ...
    @Override
    final <P_IN> Node<Integer> evaluateToNode(PipelineHelper<Integer> helper, Spliterator<P_IN> spliterator,
        boolean flattenTree, IntFunction<Integer[]> generator) {
        return Nodes.collectInt(helper, spliterator, flattenTree);
    }

    @Override
    final <P_IN> Spliterator<Integer> wrap(PipelineHelper<Integer> ph, Supplier<Spliterator<P_IN>> supplier,
        boolean isParallel) {
                ...
                return new Sink.ChainedInt<Integer>(sink) {
                    ...
                    public void accept(int t) {
                        try (IntStream result = mapper.apply(t)) {
                            ...
                            if (result != null) { result.sequential().forEach(i -> downstream.accept(i)); }
                        }
                    }
                };
            }
    ...
    public final IntStream skip(long n) {
        ...
        if (n == 0) { return this; } else { return SliceOps.makeInt(this, n, -1); }
    }
    ...
    abstract static class StatefulOp<E_IN> extends IntPipeline<E_IN> {
        ...
        @Override
        abstract <P_IN> Node<Integer> opEvaluateParallel(PipelineHelper<Integer> helper, Spliterator<P_IN> spliterator,
