...
class SpinedBuffer<E> extends AbstractSpinedBuffer implements Consumer<E>, Iterable<E> {
    ...
    public E get(long index) {
        ...
        if (spineIndex == 0) {
            if (index < elementIndex) { return curChunk[((int) index)]; } else {
    ...
    public void copyInto(E[] array, int offset) {
        ...
        if (spineIndex == 0) { System.arraycopy(curChunk, 0, array, offset, elementIndex); } else {
            // full chunks
    ...
    private static final int SPLITERATOR_CHARACTERISTICS = Spliterator.SIZED | Spliterator.ORDERED |
        ...
        class Splitr implements Spliterator<E> {
            ...
            public Spliterator<E> trySplit() {
                if (splSpineIndex < lastSpineIndex) {
                    ...
                    Spliterator<E> ret = new Splitr(splSpineIndex, lastSpineIndex - 1, splElementIndex,
    ...
    abstract static class OfPrimitive<E, T_ARR, T_CONS> extends AbstractSpinedBuffer implements Iterable<E> {
        ...
        protected long capacity() {
            return (spineIndex == 0) ? arrayLength(curChunk)
        ...
        protected int chunkFor(long index) {
            if (spineIndex == 0) {
                if (index < elementIndex) { return 0; } else {
                    throw new IndexOutOfBoundsException(Long.toString(index));
                }
            }
        ...
        abstract class BaseSpliterator<T_SPLITR extends Spliterator.OfPrimitive<E, T_CONS, T_SPLITR>>
            ...
            BaseSpliterator(int firstSpineIndex, int lastSpineIndex, int firstSpineElementIndex,
            ...
            abstract T_SPLITR newSpliterator(int firstSpineIndex, int lastSpineIndex, int firstSpineElementIndex,
                int lastSpineElementFence);
            ...
            public T_SPLITR trySplit() {
                if (splSpineIndex < lastSpineIndex) {
                    // split just before last chunk (if it is full this means 50:50 split)
                    T_SPLITR ret = newSpliterator(splSpineIndex, lastSpineIndex - 1, splElementIndex,
                        arrayLength(spine[lastSpineIndex - 1]));
    ...
    static class OfInt extends SpinedBuffer.OfPrimitive<Integer, int[], IntConsumer> implements IntConsumer {
        ...
        public void forEach(Consumer<? super Integer> consumer) {
            if (consumer instanceof IntConsumer) {
                ...
                if (Tripwire.ENABLED) { Tripwire.trip(getClass(), "{0} calling SpinedBuffer.OfInt.forEach(Consumer)"); }
        ...
        public int get(long index) {
            ...
            if (spineIndex == 0 && ch == 0) { return curChunk[(int) index]; } else {
        ...
        public Spliterator.OfInt spliterator() {
            class Splitr extends BaseSpliterator<Spliterator.OfInt> implements Spliterator.OfInt {
                ...
                @Override
                Splitr newSpliterator(int firstSpineIndex, int lastSpineIndex, int firstSpineElementIndex,
                    int lastSpineElementFence) {
        ...
        public String toString() {
            ...
            if (array.length < 200) {
                return String.format("%s[length=%d, chunks=%d]%s", getClass().getSimpleName(), array.length, spineIndex,
                    Arrays.toString(array));
            } else {
                int[] array2 = Arrays.copyOf(array, 200);
                return String
                    .format("%s[length=%d, chunks=%d]%s...", getClass().getSimpleName(), array.length, spineIndex,
