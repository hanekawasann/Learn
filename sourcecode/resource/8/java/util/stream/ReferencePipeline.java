...
abstract class ReferencePipeline<P_IN, P_OUT> extends AbstractPipeline<P_IN, P_OUT, Stream<P_OUT>>
    ...
    @Override
    final <P_IN> Node<P_OUT> evaluateToNode(PipelineHelper<P_OUT> helper, Spliterator<P_IN> spliterator,
        boolean flattenTree, IntFunction<P_OUT[]> generator) {
        return Nodes.collect(helper, spliterator, flattenTree, generator);
    }

    @Override
    final <P_IN> Spliterator<P_OUT> wrap(PipelineHelper<P_OUT> ph, Supplier<Spliterator<P_IN>> supplier,
        boolean isParallel) {
                ...
                return new Sink.ChainedReference<P_OUT, R>(sink) {
                    ...
                    public void accept(P_OUT u) {
                        try (Stream<? extends R> result = mapper.apply(u)) {
                            ...
                            if (result != null) { result.sequential().forEach(downstream); }
                        }
                    }
                };
            }
                ...
                return new Sink.ChainedReference<P_OUT, Integer>(sink) {
                    ...
                    public void accept(P_OUT u) {
                        try (IntStream result = mapper.apply(u)) {
                            // We can do better that this too; optimize for depth=0 case and just grab spliterator and forEach it
                            if (result != null) { result.sequential().forEach(downstreamAsInt); }
                        }
                    }
                };
            }
                ...
                return new Sink.ChainedReference<P_OUT, Double>(sink) {
                    ...
                    public void accept(P_OUT u) {
                        try (DoubleStream result = mapper.apply(u)) {
                            // We can do better that this too; optimize for depth=0 case and just grab spliterator and forEach it
                            if (result != null) { result.sequential().forEach(downstreamAsDouble); }
                        }
                    }
                };
            }
                ...
                return new Sink.ChainedReference<P_OUT, Long>(sink) {
                    ...
                    public void accept(P_OUT u) {
                        try (LongStream result = mapper.apply(u)) {
                            // We can do better that this too; optimize for depth=0 case and just grab spliterator and forEach it
                            if (result != null) { result.sequential().forEach(downstreamAsLong); }
                        }
                    }
                };
            }
    ...
    public final Stream<P_OUT> skip(long n) {
        ...
        if (n == 0) { return this; } else { return SliceOps.makeRef(this, n, -1); }
    }
    ...
    abstract static class StatefulOp<E_IN, E_OUT> extends ReferencePipeline<E_IN, E_OUT> {
        ...
        @Override
        abstract <P_IN> Node<E_OUT> opEvaluateParallel(PipelineHelper<E_OUT> helper, Spliterator<P_IN> spliterator,
