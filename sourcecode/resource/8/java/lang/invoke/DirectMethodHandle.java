...
class DirectMethodHandle extends MethodHandle {
    ...
    MethodHandle bindArgument(int pos, char basicType, Object value) {
        ...
        if (pos == 0 && basicType == 'L') {
            ...
            if (concrete != null) { return concrete.bindReceiver(value); }
        }
    ...
    MethodHandle bindReceiver(Object receiver) {
        ...
        if (concrete != null) { return concrete.bindReceiver(receiver); }
        ...
        if (receiver != null) {
            switch (member.getReferenceKind()) {
                ...
                case REF_invokeVirtual:
                    ...
                    MemberName concrete = new MemberName(concreteClass, member.getName(), member.getMethodType(),
                        REF_invokeSpecial);
    ...
    private static LambdaForm preparedLambdaForm(MemberName m) {
        ...
        assert (lform.methodType().dropParameterTypes(0, 1).equals(m.getInvocationType().basicType())) : Arrays
            .asList(m, m.getInvocationType().basicType(), lform, lform.methodType());
        return lform;
    }
    ...
    private void ensureInitialized() {
        if (checkInitialized(member)) {
            ...
            if (member.isField()) { updateForm(preparedFieldLambdaForm(member)); } else {
    ...
    static class Constructor extends DirectMethodHandle {
        ...
        private Constructor(MethodType mtype, LambdaForm form, MemberName constructor, MemberName initMethod,
            Class<?> instanceClass) {
    ...
    static class StaticAccessor extends DirectMethodHandle {
        ...
        private StaticAccessor(MethodType mtype, LambdaForm form, MemberName member, Object staticBase,
            long staticOffset) {
            super(mtype, form, member);
            this.fieldType = member.getFieldType();
    ...
    private static byte AF_GETFIELD = 0, AF_PUTFIELD = 1, AF_GETSTATIC = 2, AF_PUTSTATIC = 3, AF_GETSTATIC_INIT = 4,
        AF_PUTSTATIC_INIT = 5, AF_LIMIT = 6;
    // Enumerate the different field kinds using Wrapper,
    // with an extra case added for checked references.
    private static int FT_LAST_WRAPPER = Wrapper.values().length - 1, FT_UNCHECKED_REF = Wrapper.OBJECT.ordinal(),
        FT_CHECKED_REF = FT_LAST_WRAPPER + 1, FT_LIMIT = FT_LAST_WRAPPER + 2;
    ...
    private static int ftypeKind(Class<?> ftype) {
        if (ftype.isPrimitive()) { return Wrapper.forPrimitiveType(ftype).ordinal(); } else if (VerifyType
            .isNullReferenceConversion(Object.class, ftype)) { return FT_UNCHECKED_REF; } else {
    ...
    private static LambdaForm makePreparedFieldLambdaForm(byte formOp, boolean isVolatile, int ftypeKind) {
        ...
        if (isGetter) { linkerType = MethodType.methodType(ft, Object.class, long.class); } else {
        ...
        MethodType mtype;
        if (isGetter) { mtype = MethodType.methodType(ft); } else { mtype = MethodType.methodType(void.class, ft); }
        ...
        Name[] names = arguments(nameCursor - ARG_LIMIT, mtype.invokerType());
        if (needsInit) { names[INIT_BAR] = new Name(Lazy.NF_ensureInitialized, names[DMH_THIS]); }
    ...
    private static class Lazy {
        static final NamedFunction NF_internalMemberName, NF_internalMemberNameEnsureInit, NF_ensureInitialized,
            NF_fieldOffset, NF_checkBase, NF_staticBase, NF_staticOffset, NF_checkCast, NF_allocateInstance,
            NF_constructorMethod;

        static {
            try {
                NamedFunction nfs[] = { NF_internalMemberName = new NamedFunction(
                    DirectMethodHandle.class.getDeclaredMethod("internalMemberName", Object.class)),
                    NF_internalMemberNameEnsureInit = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("internalMemberNameEnsureInit", Object.class)),
                    NF_ensureInitialized = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("ensureInitialized", Object.class)),
                    NF_fieldOffset = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("fieldOffset", Object.class)),
                    NF_checkBase = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("checkBase", Object.class)),
                    NF_staticBase = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("staticBase", Object.class)),
                    NF_staticOffset = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("staticOffset", Object.class)),
                    NF_checkCast = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("checkCast", Object.class, Object.class)),
                    NF_allocateInstance = new NamedFunction(
                        DirectMethodHandle.class.getDeclaredMethod("allocateInstance", Object.class)),
                    NF_constructorMethod = new NamedFunction(
