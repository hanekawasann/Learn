...
public final class MethodType implements java.io.Serializable {
    ...
    static void checkSlotCount(int count) {
        ...
        if ((count & MAX_JVM_ARITY) != count) { throw newIllegalArgumentException("bad parameter count " + count); }
    }
    ...
    public MethodType insertParameterTypes(int num, Class<?>... ptypesToInsert) {
        ...
        if (num < 0 || num > len) { throw newIndexOutOfBoundsException(num); }
    ...
    /*non-public*/ MethodType replaceParameterTypes(int start, int end, Class<?>... ptypesToInsert) {
        if (start == end) { return insertParameterTypes(start, ptypesToInsert); }
        int len = ptypes.length;
        if (!(0 <= start && start <= end && end <= len)) {
            throw newIndexOutOfBoundsException("start=" + start + " end=" + end);
        }
        int ilen = ptypesToInsert.length;
        if (ilen == 0) { return dropParameterTypes(start, end); }
    ...
    boolean isViewableAs(MethodType newType) {
        if (!VerifyType.isNullConversion(returnType(), newType.returnType())) { return false; }
        int argc = parameterCount();
        if (argc != newType.parameterCount()) { return false; }
        for (int i = 0; i < argc; i++) {
            if (!VerifyType.isNullConversion(newType.parameterType(i), parameterType(i))) { return false; }
        }
        return true;
    }
    ...
    static boolean canConvert(Class<?> src, Class<?> dst) {
        ...
        if (src.isPrimitive()) {
            ...
            if (src == void.class) {
                return true;  //or !dst.isPrimitive()?
            }
