...
public class MethodHandles {
    ...
    public static final class Lookup {
        ...
        private MethodHandle findVirtualForMH(String name, MethodType type) {
            ...
            if ("invoke".equals(name)) { return invoker(type); }
            if ("invokeExact".equals(name)) { return exactInvoker(type); }
        ...
        public MethodHandle unreflectSpecial(Method m, Class<?> specialCaller) throws IllegalAccessException {
            ...
            return specialLookup.getDirectMethodNoSecurityManager(REF_invokeSpecial, method.getDeclaringClass(), method,
                findBoundCallerClass(method));
        }
        ...
        private MethodHandle unreflectField(Field f, boolean isSetter) throws IllegalAccessException {
            ...
            assert (isSetter ? MethodHandleNatives.refKindIsSetter(field.getReferenceKind())
        ...
        void checkMethod(byte refKind, Class<?> refc, MemberName m) throws IllegalAccessException {
            ...
            String message;
            if (m.isConstructor()) { message = "expected a method, not a constructor"; } else if (!m.isMethod()) {
        ...
        void checkAccess(byte refKind, Class<?> refc, MemberName m) throws IllegalAccessException {
            assert (m.referenceKindIsConsistentWith(refKind) && MethodHandleNatives.refKindIsValid(refKind) &&
                (MethodHandleNatives.refKindIsField(refKind) == m.isField()));
            int allowedModes = this.allowedModes;
            if (allowedModes == TRUSTED) { return; }
            int mods = m.getModifiers();
            if (Modifier.isProtected(mods) && refKind == REF_invokeVirtual && m.getDeclaringClass() == Object.class &&
                m.getName().equals("clone") && refc.isArray()) {
            ...
            if ((requestedModes & allowedModes) != 0) {
                ...
                if ((requestedModes & PROTECTED) != 0 && (allowedModes & PACKAGE) != 0 &&
                    VerifyAccess.isSamePackage(m.getDeclaringClass(), lookupClass())) { return; }
            }
        ...
        String accessFailedMessage(Class<?> refc, MemberName m) {
            ...
            boolean classOK = (Modifier.isPublic(defc.getModifiers()) &&
                (defc == refc || Modifier.isPublic(refc.getModifiers())));
            if (!classOK && (allowedModes & PACKAGE) != 0) {
                classOK = (VerifyAccess.isClassAccessible(defc, lookupClass(), ALL_MODES) &&
                    (defc == refc || VerifyAccess.isClassAccessible(refc, lookupClass(), ALL_MODES)));
            }
            if (!classOK) { return "class is not public"; }
            if (Modifier.isPublic(mods)) {
                return "access to public member failed";  // (how?)
            }
            if (Modifier.isPrivate(mods)) { return "member is private"; }
            if (Modifier.isProtected(mods)) { return "member is protected"; }
        ...
        private void checkSpecialCaller(Class<?> specialCaller) throws IllegalAccessException {
            ...
            if (!hasPrivateAccess() || (specialCaller != lookupClass() &&
                !(ALLOW_NESTMATE_ACCESS && VerifyAccess.isSamePackageMember(specialCaller, lookupClass())))) {
        ...
        private boolean restrictProtectedReceiver(MemberName method) {
            ...
            if (!method.isProtected() || method.isStatic() || allowedModes == TRUSTED ||
                method.getDeclaringClass() == lookupClass() ||
                VerifyAccess.isSamePackage(method.getDeclaringClass(), lookupClass()) || (ALLOW_NESTMATE_ACCESS &&
                VerifyAccess.isSamePackageMember(method.getDeclaringClass(), lookupClass()))) { return false; }
            return true;
        }

        private MethodHandle restrictReceiver(MemberName method, MethodHandle mh, Class<?> caller)
        ...
        private MethodHandle getDirectMethodNoSecurityManager(byte refKind, Class<?> refc, MemberName method,
            Class<?> callerClass) throws IllegalAccessException {
            final boolean doRestrict = true;
        ...
        private MethodHandle getDirectMethodCommon(byte refKind, Class<?> refc, MemberName method,
            boolean checkSecurity, boolean doRestrict, Class<?> callerClass) throws IllegalAccessException {
            checkMethod(refKind, refc, method);
            // Optionally check with the security manager; this isn't needed for unreflect* calls.
            if (checkSecurity) { checkSecurityManager(refc, method); }
            assert (!method.isMethodHandleInvoke());

            Class<?> refcAsSuper;
            if (refKind == REF_invokeSpecial && refc != lookupClass() && !refc.isInterface() &&
                refc != (refcAsSuper = lookupClass().getSuperclass()) && refc.isAssignableFrom(lookupClass())) {
                ...
                MemberName m2 = new MemberName(refcAsSuper, method.getName(), method.getMethodType(),
                    REF_invokeSpecial);
        ...
        private MethodHandle maybeBindCaller(MemberName method, MethodHandle mh, Class<?> callerClass)
            throws IllegalAccessException {
            if (allowedModes == TRUSTED || !MethodHandleNatives.isCallerSensitive(method)) { return mh; }
        ...
        private MethodHandle getDirectFieldNoSecurityManager(byte refKind, Class<?> refc, MemberName field)
            throws IllegalAccessException {
            final boolean checkSecurity = false;  // not needed for reflection or for linking CONSTANT_MH constants
            return getDirectFieldCommon(refKind, refc, field, checkSecurity);
        }
        ...
        private MethodHandle getDirectFieldCommon(byte refKind, Class<?> refc, MemberName field, boolean checkSecurity)
            throws IllegalAccessException {
            checkField(refKind, refc, field);
            // Optionally check with the security manager; this isn't needed for unreflect* calls.
            if (checkSecurity) { checkSecurityManager(refc, field); }
            ...
            if (doRestrict) { mh = restrictReceiver(field, mh, lookupClass()); }
            return mh;
        }
        ...
        private MethodHandle getDirectConstructorCommon(Class<?> refc, MemberName ctor, boolean checkSecurity)
            throws IllegalAccessException {
            assert (ctor.isConstructor());
            checkAccess(REF_newInvokeSpecial, refc, ctor);
            // Optionally check with the security manager; this isn't needed for unreflect* calls.
            if (checkSecurity) { checkSecurityManager(refc, ctor); }
        ...
        private boolean canBeCached(byte refKind, Class<?> defc, MemberName member) {
            ...
            if (!Modifier.isPublic(defc.getModifiers()) ||
                !Modifier.isPublic(member.getDeclaringClass().getModifiers()) || !member.isPublic() ||
            ...
            try {
                MemberName resolved2 = publicLookup()
                    .resolveOrFail(refKind, new MemberName(refKind, defc, member.getName(), member.getType()));
    ...
    public static MethodHandle constant(Class<?> type, Object value) {
        if (type.isPrimitive()) {
            if (type == void.class) { throw newIllegalArgumentException("void type"); }
    ...
    public static MethodHandle identity(Class<?> type) {
        if (type == void.class) { throw newIllegalArgumentException("void type"); } else if (type == Object.class) {
    ...
    public static MethodHandle insertArguments(MethodHandle target, int pos, Object... values) {
        ...
        if (inargs < 0) { throw newIllegalArgumentException("too many values to insert"); }
        if (pos < 0 || pos > inargs) { throw newIllegalArgumentException("no argument type to append"); }
    ...
    public static MethodHandle dropArguments(MethodHandle target, int pos, List<Class<?>> valueTypes) {
        ...
        if (pos < 0 || pos >= inargs) { throw newIllegalArgumentException("no argument type to remove"); }
    ...
    public static MethodHandle filterArguments(MethodHandle target, int pos, MethodHandle... filters) {
        ...
        if (pos + filters.length > maxPos) { throw newIllegalArgumentException("too many filters"); }
    ...
    public static MethodHandle collectArguments(MethodHandle target, int pos, MethodHandle filter) {
        MethodType targetType = target.type();
        MethodType filterType = filter.type();
        if (filterType.returnType() != void.class && filterType.returnType() != targetType.parameterType(pos)) {
            throw newIllegalArgumentException("target and filter types do not match", targetType, filterType);
        }
        return MethodHandleImpl.makeCollectArguments(target, filter, pos, false);
    }
    ...
    public static MethodHandle filterReturnValue(MethodHandle target, MethodHandle filter) {
        ...
        if (filterValues == 0 ? (rtype != void.class) : (rtype != filterType.parameterType(0))) {
    ...
    public static MethodHandle foldArguments(MethodHandle target, MethodHandle combiner) {
        ...
        if (ok && !(combinerType.parameterList()
            .equals(targetType.parameterList().subList(afterInsertPos, afterInsertPos + foldArgs)))) { ok = false; }
        if (ok && foldVals != 0 && !combinerType.returnType().equals(targetType.parameterType(0))) { ok = false; }
        if (!ok) { throw misMatchedTypes("target and combiner types", targetType, combinerType); }
    ...
    public static MethodHandle guardWithTest(MethodHandle test, MethodHandle target, MethodHandle fallback) {
        ...
        if (!ttype.equals(ftype)) { throw misMatchedTypes("target and fallback types", ttype, ftype); }
    ...
    public static MethodHandle throwException(Class<?> returnType, Class<? extends Throwable> exType) {
        if (!Throwable.class.isAssignableFrom(exType)) { throw new ClassCastException(exType.getName()); }
