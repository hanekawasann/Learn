...
class LambdaForm {
    ...
    private static Name[] buildNames(Name[] formals, Name[] temps, Name result) {
        ...
        if (result != null) { names[length - 1] = result; }
        return names;
    }
    ...
    private static int fixResult(int result, Name[] names) {
        if (result >= 0) {
            if (names[result].type == 'V') { return -1; }
    ...
    private static boolean namesOK(int arity, Name[] names) {
        for (int i = 0; i < names.length; i++) {
            ...
            if (i < arity) { assert (n.isParam()) : n + " is not param at " + i; } else {
    ...
    private void normalize() {
        ...
        if (oldNames != null) {
            ...
            if (startFixing <= changesStart) { startFixing = changesStart + 1; }
    ...
    static boolean isValidSignature(String sig) {
        ...
        for (int i = 0; i < siglen; i++) {
            ...
            char c = sig.charAt(i);
            if (c == 'V') { return (i == siglen - 1 && arity == siglen - 2); }
    ...
    MemberName compileToBytecode() {
        ...
        try {
            ...
            if (TRACE_INTERPRETER) { traceInterpreter("compileToBytecode", this); }
            isCompiled = true;
            return vmentry;
    ...
    private static Map<String, LambdaForm> computeInitialPreparedForms() {
        ...
        for (MemberName m : MemberName.getFactory().getMethods(LambdaForm.class, false, null, null, null)) {
            ...
            if (mt.parameterCount() > 0 && mt.parameterType(0) == MethodHandle.class &&
    ...
    private static boolean valueMatches(char tc, Class<?> type, Object x) {
        ...
        if (type == void.class) {
            tc = 'V';   // can drop any kind of value
        }
    ...
    Object interpretWithArguments(Object... argumentValues) throws Throwable {
        if (TRACE_INTERPRETER) { return interpretWithArgumentsTracing(argumentValues); }
    ...
    Object interpretName(Name name, Object[] values) throws Throwable {
        if (TRACE_INTERPRETER) { traceInterpreter("| interpretName", name.debugString(), (Object[]) null); }
    ...
    */
    ...
    static void traceInterpreter(String event, Object obj, Object... args) {
        if (TRACE_INTERPRETER) {
            System.out.println("LFI: " + event + " " + (obj != null ? obj : "") +
    ...
    private boolean isEmpty() {
        if (result < 0) { return (names.length == arity); } else if (result == arity && names.length == arity + 1) {
    ...
    LambdaForm bind(Name name, Name binding, BoundMethodHandle.SpeciesData oldData,
        ...
        if (bindCache != null) {
            ...
            if (form != null) {
                assert (form.contains(binding)) :
                    "form << " + form + " >> does not contain binding << " + binding + " >>";
                return form;
            }
        } else {
        ...
        for (int i = 0; i < names2.length; i++) {
            ...
            if (n.function != null && n.function.memberDeclaringClassOrNull() == oldData.clazz) {
                ...
                for (int j = 0; j < oldData.getters.length; j++) {
                    if (oldGetter == oldData.getters[j]) { newGetter = newData.getters[j]; }
                }
                if (newGetter != null) {
        ...
        int result2 = result;
        if (result2 == pos) { result2 = insPos; } else if (result2 > pos && result2 <= insPos) { result2 -= 1; }
    ...
    LambdaForm addArguments(int pos, char... types) {
        ...
        int result2 = result;
        if (result2 >= arity) { result2 += inTypes; }
    ...
    LambdaForm permuteArguments(int skip, int[] reorder, char[] types) {
        ...
        for (int j = pos; j < outArgs; j++) {
            ...
            if (n2 == null) { names2[skip + i] = n2 = new Name(types[i]); } else { assert (n2.type == types[i]); }
        ...
        for (int i = skip + pos; i < arity2; i++) {
            if (names2[i] == null) { names2[i] = argument(i, types[i - skip]); }
        }
    ...
    static class NamedFunction {
        ...
        @Override
        public int hashCode() {
            if (member != null) { return member.hashCode(); }
        ...
        static void initializeInvokers() {
            for (MemberName m : MemberName.getFactory().getMethods(NamedFunction.class, false, null, null, null)) {
                ...
                if (type.equals(INVOKER_METHOD_TYPE) && m.getName().startsWith("invoke_")) {
                    ...
                    if (LambdaForm.signatureReturn(sig) == 'V') { srcType = srcType.changeReturnType(void.class); }
        ...
        static final MethodType INVOKER_METHOD_TYPE = MethodType
        ...
        private static MethodHandle computeInvoker(MethodTypeForm typeForm) {
            ...
            if (!mh.type().equals(INVOKER_METHOD_TYPE)) { throw new InternalError(mh.debugString()); }
        ...
        MethodType methodType() {
            if (resolvedHandle != null) { return resolvedHandle.type(); } else
    ...
    static final class Name {
        ...
        Name(NamedFunction function, Object... arguments) {
            ...
            assert (arguments.length == function.arity()) :
                "arity mismatch: arguments.length=" + arguments.length + " == function.arity()=" + function.arity() +
                    " in " + debugString();
            for (int i = 0; i < arguments.length; i++) {
        ...
        void resolve() {
            if (function != null) { function.resolve(); }
        }
        ...
        Name replaceNames(Name[] oldNames, Name[] newNames, int start, int end) {
            ...
            for (int j = 0; j < arguments.length; j++) {
                if (arguments[j] instanceof Name) {
                    ...
                    if (check >= 0 && check < newNames.length && n == newNames[check]) { continue eachArg; }
                    // n might not have the correct index: n != oldNames[n.index].
                    for (int i = start; i < end; i++) {
                        if (n == oldNames[i]) {
                            if (n == newNames[i]) { continue eachArg; }
                            if (!replaced) {
                                replaced = true;
                                arguments = arguments.clone();
                            }
        ...
        void internArguments() {
            ...
            for (int j = 0; j < arguments.length; j++) {
                if (arguments[j] instanceof Name) {
                    Name n = (Name) arguments[j];
                    if (n.isParam() && n.index < INTERNED_ARGUMENT_LIMIT) { arguments[j] = internArgument(n); }
                }
            }
        }
        ...
        public String exprString() {
            ...
            for (Object a : arguments) {
                buf.append(cma);
                cma = ",";
                if (a instanceof Name || a instanceof Integer) { buf.append(a); } else {
        ...
        public boolean equals(Name that) {
            ...
            return
                ...
                this.type == that.type && this.function.equals(that.function) &&
        ...
        @Override
        public int hashCode() {
            if (isParam()) { return index | (type << 8); }
    ...
    static Name argument(int which, char type) {
        ...
        if (tn < 0 || which >= INTERNED_ARGUMENT_LIMIT) { return new Name(which, type); }
    ...
    static {
        for (int tn = 0; tn < ALL_TYPES.length(); tn++) {
            ...
            MemberName zmem = new MemberName(LambdaForm.class, "zero" + bt, MethodType.methodType(wrap.primitiveType()),
                REF_invokeStatic);
            try {
    ...
    static {
        if (USE_PREDEFINED_INTERPRET_METHODS) { PREPARED_FORMS.putAll(computeInitialPreparedForms()); }
    }
