...
/* non-public */ abstract class BoundMethodHandle extends MethodHandle {
        ...
        try {
            switch (xtype) {
                case 'L':
                    ...
                    return (BoundMethodHandle) SpeciesData.EMPTY.extendWithType('L').constructor[0]
                        .invokeBasic(type, form, x);
                case 'I':
                    return (BoundMethodHandle) SpeciesData.EMPTY.extendWithType('I').constructor[0]
                        .invokeBasic(type, form, ValueConversions.widenSubword(x));
                case 'J':
                    return (BoundMethodHandle) SpeciesData.EMPTY.extendWithType('J').constructor[0]
                        .invokeBasic(type, form, (long) x);
                case 'F':
                    return (BoundMethodHandle) SpeciesData.EMPTY.extendWithType('F').constructor[0]
                        .invokeBasic(type, form, (float) x);
                case 'D':
                    return (BoundMethodHandle) SpeciesData.EMPTY.extendWithType('D').constructor[0]
                        .invokeBasic(type, form, (double) x);
                default:
    ...
    MethodHandle cloneExtend(MethodType type, LambdaForm form, char xtype, Object x) {
        try {
            switch (xtype) {
                case 'L':
                    return cloneExtendL(type, form, x);
                case 'I':
                    return cloneExtendI(type, form, ValueConversions.widenSubword(x));
                case 'J':
                    return cloneExtendJ(type, form, (long) x);
                case 'F':
                    return cloneExtendF(type, form, (float) x);
                case 'D':
                    return cloneExtendD(type, form, (double) x);
            }
        } catch (Throwable t) {
            throw newInternalError(t);
        }
    ...
    static final class Species_L extends BoundMethodHandle {
        ...
        // The following is a grossly irregular hack:
        @Override
        MethodHandle reinvokerTarget() { return (MethodHandle) argL0; }
        ...
        public final BoundMethodHandle cloneExtendL(MethodType mt, LambdaForm lf, Object narg) throws Throwable {
            return (BoundMethodHandle) SPECIES_DATA.extendWithIndex(INDEX_L).constructor[0]
        ...
        public final BoundMethodHandle cloneExtendI(MethodType mt, LambdaForm lf, int narg) throws Throwable {
            return (BoundMethodHandle) SPECIES_DATA.extendWithIndex(INDEX_I).constructor[0]
                .invokeBasic(mt, lf, argL0, narg);
        }

        @Override
        public final BoundMethodHandle cloneExtendJ(MethodType mt, LambdaForm lf, long narg) throws Throwable {
            return (BoundMethodHandle) SPECIES_DATA.extendWithIndex(INDEX_J).constructor[0]
                .invokeBasic(mt, lf, argL0, narg);
        }

        @Override
        public final BoundMethodHandle cloneExtendF(MethodType mt, LambdaForm lf, float narg) throws Throwable {
            return (BoundMethodHandle) SPECIES_DATA.extendWithIndex(INDEX_F).constructor[0]
                .invokeBasic(mt, lf, argL0, narg);
        }

        @Override
        public final BoundMethodHandle cloneExtendD(MethodType mt, LambdaForm lf, double narg) throws Throwable {
            return (BoundMethodHandle) SPECIES_DATA.extendWithIndex(INDEX_D).constructor[0]
                .invokeBasic(mt, lf, argL0, narg);
        }
    }
...
*/
    ...
    static class SpeciesData {
        ...
        private static SpeciesData get(String types) {
            ...
            synchronized (d) {
                ...
                if (lookupCache(types).isPlaceholder()) { Factory.generateConcreteBMHClass(types); }
            }
    ...
    static class Factory {
        ...
        static final Class<?>[] TYPES = new Class<?>[] { Object.class, int.class, long.class, float.class,
            double.class };
        ...
        static Class<? extends BoundMethodHandle> generateConcreteBMHClass(String types) {
            ...
            for (Class<?> c : TYPES) {
                ...
                mv = cw.visitMethod(ACC_PUBLIC + ACC_FINAL, "cloneExtend" + t, makeSignature(String.valueOf(t), false),
                    null, E_THROWABLE);
                mv.visitCode();
            ...
            Class<? extends BoundMethodHandle> bmhClass =
                ...
                UNSAFE.defineClass(className, classFile, 0, classFile.length, BoundMethodHandle.class.getClassLoader(),
        ...
        static MethodHandle makeCbmhCtor(Class<? extends BoundMethodHandle> cbmh, String types) {
            try {
                return linkConstructor(LOOKUP
                    .findConstructor(cbmh, MethodType.fromMethodDescriptorString(makeSignature(types, true), null)));
        ...
        private static MethodHandle linkConstructor(MethodHandle cmh) {
            ...
            final MethodType linkerMT = ctorMT.changeParameterType(0, BoundMethodHandle.class)
            ...
            try {
                linkerMN = MemberName.getFactory()
                    .resolveOrFail(REF_invokeStatic, linkerMN, null, NoSuchMethodException.class);
