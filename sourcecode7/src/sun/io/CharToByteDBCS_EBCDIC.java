...
public abstract class CharToByteDBCS_EBCDIC extends CharToByteConverter {
    ...
    public int flush(byte[] output, int outStart, int outEnd)
        ...
        if (currentState == DBCS) {
            if (outStart >= outEnd) { throw new ConversionBufferFullException(); }
            output[outStart] = SI;
            bytesOut++;
        }
    ...
    public int convert(char[] input, int inOff, int inEnd, byte[] output, int outOff, int outEnd)
        ...
        while (charOff < inEnd) {
            ...
            if (currentState == DBCS && outputByte[0] == 0x00) {
                if (byteOff >= outEnd) { throw new ConversionBufferFullException(); }
                currentState = SBCS;
                output[byteOff++] = SI;
            } else if (currentState == SBCS && outputByte[0] != 0x00) {
                if (byteOff >= outEnd) {
            ...
            if (currentState == DBCS) { spaceNeeded = 2; } else { spaceNeeded = 1; }

            if (byteOff + spaceNeeded > outEnd) {
                throw new ConversionBufferFullException();
            }

            if (currentState == SBCS) { output[byteOff++] = outputByte[1]; } else {
