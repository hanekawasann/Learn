...
public abstract class CharToByteConverter {
     ...
     * @throws MalformedInputException       if the input buffer contains any
     ...
     * @throws UnknownCharacterException     for any character that
     *                                       that cannot be converted to the external character encoding. Thrown
     *                                       only when converter is not in substitution mode.
     * @throws ConversionBufferFullException if output array is filled prior
     *                                       to converting all the input.
     */
    public abstract int convert(char[] input, int inStart, int inEnd, byte[] output, int outStart, int outEnd)
     ...
     * to converting all the input.
     */
    public int convertAny(char[] input, int inStart, int inEnd, byte[] output, int outStart, int outEnd)
        ...
        while (localInOff < inEnd) {
            try {
                int discard = convert(input, localInOff, inEnd, output, localOutOff, outEnd);
                return (nextByteIndex() - outStart);
            } catch (MalformedInputException e) {
                byte[] s = subBytes;
                int subSize = s.length;
                localOutOff = nextByteIndex();
                if ((localOutOff + subSize) > outEnd) { throw new ConversionBufferFullException(); }
                for (int i = 0; i < subSize; i++) { output[localOutOff++] = s[i]; }
     ...
     * @param outStart start writing to output array at this offset.
     * @param outEnd   stop writing to output array at this offset (exclusive).
     * @throws MalformedInputException       if the output to be flushed contained
     ...
     * @throws ConversionBufferFullException if output array is filled
    ...
    public int flushAny(byte[] output, int outStart, int outEnd) throws ConversionBufferFullException {
        ...
        try {
            ...
            if ((outStart + subSize) > outEnd) { throw new ConversionBufferFullException(); }
            for (int i = 0; i < subSize; i++) { output[outIndex++] = s[i]; }
     ...
     * @throws IllegalArgumentException if given byte array is longer than
