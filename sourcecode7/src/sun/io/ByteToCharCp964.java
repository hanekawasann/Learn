...
public class ByteToCharCp964 extends ByteToCharConverter {
    ...
    public int convert(byte[] input, int inOff, int inEnd, char[] output, int outOff, int outEnd)
        ...
        while (byteOff < inEnd) {
            ...
            switch (state) {
                case G0:
                    if (byte1 == SS2) { state = G2; } else if (byte1 == SS3) {
                        badInputLength = 1;
                        throw new MalformedInputException();
                    } else if (byte1 <= 0x9f)               // valid single byte
                    { outputChar = byteToCharTable.charAt(byte1); } else if (byte1 < 0xa1 || byte1 > 0xfe) {
                        badInputLength = 1;
                        throw new MalformedInputException();
                ...
                case G2:
                    ...
                    if (byte1 == 0xa2) { mappingTableG2 = mappingTableG2a2; } else if (byte1 == 0xac) {
