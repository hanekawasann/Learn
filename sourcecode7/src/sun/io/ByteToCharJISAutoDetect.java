...
public class ByteToCharJISAutoDetect extends ByteToCharConverter {
    ...
    public int flush(char[] output, int outStart, int outEnd)
        ...
        badInputLength = 0;
        if (detectedConv != null) { return detectedConv.flush(output, outStart, outEnd); } else {
    ...
    public int convert(byte[] input, int inOff, int inEnd, char[] output, int outOff, int outEnd)
        ...
        try {
            if (detectedConv == null) {
                        ...
                        if (cnt + 1 < inEnd) {
                            ...
                            if ((byte1 == SS3) && JISAutoDetect.canBeEUCJP(secondmask)) {
                                if (cnt + 1 < inEnd) {
                                    ...
                                    if (!JISAutoDetect.canBeEUCJP(maskTable2[nextbyte])) { convName = SJISName; }
                                } else { convName = SJISName; }
                            }
                            if (JISAutoDetect.canBeEUCKana(firstmask, secondmask)) { euckana++; }
                        } else {
                ...
                if (nonAsciiFound && (convName == null)) {
                    if ((euckana > 1) || (ss2count > 1)) { convName = EUCJPName; } else { convName = SJISName; }
                }
        ...
        if (detectedConv != null) {
            try {
                num += detectedConv.convert(input, inOff + num, inEnd, output, outOff + num, outEnd);
            } finally {
            ...
            try {
                num += defaultConv.convert(input, inOff + num, inEnd, output, outOff + num, outEnd);
            } finally {
