...
public abstract class CharToByteEUC extends CharToByteConverter {
    ...
    public int convert(char[] input, int inOff, int inEnd, byte[] output, int outOff, int outEnd)
        ...
        while (charOff < inEnd) {
            ...
            // Is this an unaccompanied low surrogate?
                ...
                if (index < 7500) { theChars = index2; } else if (index < 15000) {
                    index = index - 7500;
                    theChars = index2a;
                } else if (index < 22500) {
                    index = index - 15000;
            ...
            for (spaceNeeded = outputByte.length; spaceNeeded > 1; spaceNeeded--) {
                if (outputByte[oindex++] != 0x00) { break; }
            }

            if (byteOff + spaceNeeded > outEnd) { throw new ConversionBufferFullException(); }
    ...
    public boolean canConvert(char ch) {
        ...
        if (theChars.charAt(2 * index) != '\u0000' || theChars.charAt(2 * index + 1) != '\u0000') { return (true); }
