    ...
    public int convert(byte[] in, int inOff, int inEnd, char[] out, int outOff, int outEnd)
        ...
        if (usesMark && !started) {     /* Read initial byte-order mark */
            if (inI < inEnd) {
                ...
                if (c == BYTE_ORDER_MARK) { bo = BIG; } else if (c == REVERSED_MARK) { bo = LITTLE; }
        ...
        while (inI < inEnd) {
            ...
            char c;
            if (byteOrder == BIG) { c = (char) ((b1 << 8) | b2); } else { c = (char) ((b2 << 8) | b1); }

            if (c == REVERSED_MARK) { throw new MalformedInputException("Reversed byte-order mark"); }

            if (outI >= outEnd) { throw new ConversionBufferFullException(); }
            out[outI++] = c;
            byteOff = inI;
            charOff = outI;
