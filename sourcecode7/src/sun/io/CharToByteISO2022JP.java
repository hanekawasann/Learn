...
public class CharToByteISO2022JP extends CharToByteJIS0208 {
    ...
    public int convert(char[] input, int inOff, int inEnd, byte[] output, int outOff, int outEnd)
                ...
                if (inputChar >= '\uDC00' && inputChar <= '\uDFFF') {
                    ...
                    if (subMode) {
                        if (currentMode != subBytesMode) {
                            System.arraycopy(subBytesEscape, 0, outputByte, 0, subBytesEscape.length);
                            outputSize = subBytesEscape.length;
                            System.arraycopy(subBytes, 0, outputByte, outputSize, subBytes.length);
        ...
        if (currentMode != ASCII) {
            if (byteOff + 3 > outEnd) { throw new ConversionBufferFullException(); }
