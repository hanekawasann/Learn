...
public class ByteToCharGB18030 extends ByteToCharGB18030DB {
    ...
    public int convert(byte[] input, int inOff, int inEnd, char[] output, int outOff, int outEnd)
        ...
        if (savedSize != 0) {
            ...
            if (((savedBytes[0] & 0xFF) < 0x81 || savedBytes[0] > 0xFE) ||
                (savedSize > 1 && (savedBytes[1] & 0xFF) < 0x30) ||
                (savedSize > 2 && ((savedBytes[2] & 0xFF) < 0x81 || (savedBytes[2] & 0xFF) > 0xFE))) {
                badInputLength = 0;
                throw new MalformedInputException();
            }
        ...
        while (readOff < inEnd) {
            ...
            if ((byte1 & (byte) 0x80) == 0) { // US-ASCII range
                ...
                if (subMode) { outputChar = subChars[0]; } else {
                    badInputLength = 1;
            ...
            switch (currentState) {
                ...
                case GB18030_FOUR_BYTE:
                    int offset = (((byte1 - 0x81) * 10 + (byte2 - 0x30)) * 126 + byte3 - 0x81) * 10 + byte4 - 0x30;
                    ...
                    if (offset <= 0x4A62) { output[charOff++] = getChar(offset); } else if (offset > 0x4A62 &&
                        offset <= 0x82BC) {
                        output[charOff++] = (char) (offset + 0x5543);
                    } else if (offset >= 0x82BD && offset <= 0x830D) { output[charOff++] = getChar(offset); } else if (
                        offset >= 0x830D && offset <= 0x93A8) {
                        output[charOff++] = (char) (offset + 0x6557);
                    } else if (offset >= 0x93A9 && offset <= 0x99FB) { output[charOff++] = getChar(offset); }
                    // Supplemental UCS planes handled via surrogates
                    else if (offset >= 0x2E248 && offset < 0x12E248) {
                        if (offset >= 0x12E248) {
                            if (subMode) { return subChars[0]; } else {
                                badInputLength = 4;
                                throw new UnknownCharacterException();
                            }
                        }
    ...
    private char getChar(int offset) throws UnknownCharacterException {
        ...
        if (((byte1 < 0) || (byte1 > getOuter().length)) || ((byte2 < start) || (byte2 > end))) {
            if (subMode) { return subChars[0]; } else {
                badInputLength = 1;
                throw new UnknownCharacterException();
            }
