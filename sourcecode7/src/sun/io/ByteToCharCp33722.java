        ...
        while (byteOff < inEnd) {
            ...
            switch (state) {
                case G0:
                    ...
                    { state = G2; } else if (byte1 == SS3)                  // drop into G3 set
                    { state = G3; } else if (byte1 <= 0x9f)               // valid single byte
                    { outputChar = byteToCharTable.charAt(byte1); } else if (byte1 < 0xa1 || byte1 > 0xfe) {  // valid G1 set 1st byte
                        badInputLength = 1;
                        throw new MalformedInputException();
                    } else {
