...
public class ByteToCharISO2022CN extends ByteToCharConverter {
    ...
    public void reset() {
        ...
        for (i = 0; i < remainByte.length; i++) { remainByte[i] = 0; }
    }
    ...
    private byte[] savetyGetSrc(byte[] input, int inOff, int inEnd, int nbytes) {
        ...
        if (inOff <= (inEnd - nbytes + 1)) { tmp = new byte[nbytes]; } else { tmp = new byte[inEnd - inOff]; }

        for (i = 0; i < tmp.length; i++) { tmp[i] = input[inOff + i]; }
        return tmp;
    }
    ...
    public final int convert(byte[] input, int inOff, int inEnd, char[] output, int outOff, int outEnd)
        ...
        while (byteOff < inEnd) {
            ...
            switch (remainByte[0]) {
                case ISO_SO:
                    shiftout = true;
                    if (remainIndex > 0) { System.arraycopy(remainByte, 1, remainByte, 0, remainIndex); }
                    remainIndex--;
                    break;
                case ISO_SI:
                    shiftout = false;
                    ...
                    break;
                case ISO_ESC:
                    byte tmp[] = savetyGetSrc(input, byteOff, inEnd, (maximumDesignatorLength - remainIndex));
                    System.arraycopy(tmp, 0, remainByte, remainIndex + 1, tmp.length);

                    remainIndex += tmp.length;
                    byteOff += tmp.length;
                    if (tmp.length < (maximumDesignatorLength - remainIndex)) { break; }
                    ...
                    if (i == SODesignator.length) {
                        ...
                        if (i == SS2Designator.length) {
                            ...
                            if (i == SS3Designator.length) {
                                switch (remainByte[1]) {
                                    case ISO_SS2_7:
                                        output[charOff] = getUnicode(remainByte[2], remainByte[3], SS2Flag);
                                        charOff++;
                                        DesignatorLength = 3;
                                        break;
                                    case ISO_SS3_7:
                                        output[charOff] = getUnicode(remainByte[2], remainByte[3], SS3Flag);
                                        charOff++;
                                        DesignatorLength = 3;
                                        break;
                                    default:
                    ...
                    if (!shiftout) {
                        ...
                        switch (state) {
                            case G0:
                                ...
                                break;
                            case G1:
                                output[charOff] = getUnicode(firstByte, remainByte[0], SOFlag);
                                charOff++;
                                state = G0;
                                break;
                        }
    ...
    public ByteToCharISO2022CN() {
        ...
        try {
            SOConverter[0] = SOConverter[1] = ByteToCharConverter.getConverter("GB2312");
            SOConverter[2] = SS2Converter[0] = SS3Converter[0] = ByteToCharConverter.getConverter("CNS11643");
        } catch (Exception e) {}
        ;
    }
