...
public class CharToByteUnicode extends CharToByteConverter {
    ...
    public CharToByteUnicode() {
        String enc = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("sun.io.unicode.encoding", "UnicodeBig"));
        if (enc.equals("UnicodeBig")) { byteOrder = BIG; } else if (enc.equals("UnicodeLittle")) {
            byteOrder = LITTLE;
        } else { byteOrder = BIG; }
    }
    ...
    public int convert(char in[], int inOff, int inEnd, byte out[], int outOff, int outEnd)
        ...
        if (inOff >= inEnd) { return 0; }

        int inI = inOff, outI = outOff, outTop = outEnd - 2;

        if (usesMark && !markWritten) {
            if (outI > outTop) { throw new ConversionBufferFullException(); }
            if (byteOrder == BIG) {
