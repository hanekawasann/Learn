...
public abstract class CharToByteISO2022 extends CharToByteConverter {
    ...
    protected String SODesignator, SS2Designator = null, SS3Designator = null;
    ...
    private int unicodeToNative(char unicode, byte ebyte[]) {
        ...
        try {
            converted = codeConverter.convert(convChar, 0, 1, convByte, 0, codeConverter.getMaxBytesPerChar());
        } catch (Exception e) {
            return -1;
        }
    ...
    public int convert(char[] input, int inOff, int inEnd, byte[] output, int outOff, int outEnd)
        ...
        for (charOff = inOff; charOff < inEnd; charOff++) {
            ...
            if (outEnd - byteOff < outputSize) { throw new ConversionBufferFullException(); }

            for (int i = 0; i < outputSize; i++) { output[byteOff++] = outputByte[i]; }
