...
public class StreamRemoteCall implements RemoteCall {
     ...
     *                exceptional return.
     * @throws StreamCorruptedException If result stream previously
     *                                  acquired
     * @throws IOException              For any other problem with I/O.
     */
    public ObjectOutput getResultStream(boolean success) throws IOException {
        /* make sure result code only marshaled once. */
        if (resultStarted) { throw new StreamCorruptedException("result already in progress"); } else {
            resultStarted = true;
        }
        ...
        if (success)            //
        { out.writeByte(TransportConstants.NormalReturn); } else {
    ...
    protected void exceptionReceivedFromServer(Exception ex) throws Exception {
        ...
        if (UnicastRef.clientCallLog.isLoggable(Log.BRIEF)) {
            ...
            UnicastRef.clientCallLog.log(Log.BRIEF,
                "outbound call " + "received exception: [" + ep.getHost() + ":" + ep.getPort() + "] exception: ", ex);
        }
