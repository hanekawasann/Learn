...
public class RMIGenerator implements RMIConstants, Generator {
    ...
    private void generate() {
        ...
        try {
            IndentingWriter out = new IndentingWriter(new OutputStreamWriter(new FileOutputStream(stubFile)));
            writeStub(out);
            out.close();
            if (env.verbose()) {
        ...
        if (version == STUB_VERSION_1_1 || version == STUB_VERSION_FAT) {
            ...
            try {
                IndentingWriter out = new IndentingWriter(new OutputStreamWriter(new FileOutputStream(skeletonFile)));
                writeSkeleton(out);
                out.close();
                if (env.verbose()) {
                    env.output(Main.getText("rmic.wrote", skeletonFile.getPath()));
                }
     ...
     */
    protected static File sourceFileForClass(Identifier className, Identifier outputClassName, File destDir,
        ...
        if (outputName.endsWith("_Skel")) {
            ...
            if (temp.exists()) {
                ...
                if (!env.getMain().iiopGeneration) {
                    ...
                    env.error(0, "warn.rmic.tie.found", classNameStr, temp.getAbsolutePath());
                }
            }
        }
     ...
     */
    private RMIGenerator(BatchEnvironment env, ClassDefinition cdef, File destDir, RemoteClass remoteClass, int version)
         ...
         */
        defException = env.getClassDeclaration(idJavaLangException).
            getClassDefinition(env);
        defRemoteException = env.getClassDeclaration(idRemoteException).
            getClassDefinition(env);
        defRuntimeException = env.getClassDeclaration(idJavaLangRuntimeException).
            getClassDefinition(env);
    }
    ...
    private void writeStub(IndentingWriter p) throws IOException {
         ...
         */
        p.plnI("public final class " + Names.mangleClass(stubClassName.getName()));
    ...
    private void writeStubConstructors(IndentingWriter p) throws IOException {
        ...
        p.plnI("public " + Names.mangleClass(stubClassName.getName()) + "(" + idRemoteRef + " ref) {");
        p.pln("super(ref);");
        p.pOln("}");
    }
    ...
    private void writeStubMethod(IndentingWriter p, int opnum) throws IOException {
        ...
        if (version == STUB_VERSION_1_1 || version == STUB_VERSION_FAT) {
            p.pln(idRemoteCall + " call = ref.newCall((" + idRemoteObject + ") this, operations, " + opnum +
                ", interfaceHash);");
         ...
         */
        if (catchList.size() > 0) {
            for (Enumeration enumeration = catchList.elements(); enumeration.hasMoreElements(); ) {
        ...
        for (int i = 0; i < exceptions.length; i++) {
            ...
            try {
                ...
                for (int j = 0; j < uniqueList.size(); ) {
                    ClassDefinition def = (ClassDefinition) uniqueList.elementAt(j);
    ...
    private void writeSkeleton(IndentingWriter p) throws IOException {
         ...
         */
        p.plnI("public final class " + Names.mangleClass(skeletonClassName.getName()));
        ...
        if (version == STUB_VERSION_FAT) {
            ...
            if (remoteMethods.length > 0) {
                for (int opnum = 0; opnum < remoteMethods.length; opnum++) {
                    ...
                    p.plnI("if (hash == " + remoteMethods[opnum].getMethodHash() + "L) {");
                    p.pln("opnum = " + opnum + ";");
                }
                p.pOlnI("} else {");
            }
    ...
    private void writeOperationsArray(IndentingWriter p) throws IOException {
        ...
        for (int i = 0; i < remoteMethods.length; i++) {
            if (i > 0) { p.pln(","); }
            p.p("new " + idOperation + "(\"" + remoteMethods[i].getOperationString() + "\")");
        }
        p.pln();
        p.pOln("};");
    }
    ...
    private void writeInterfaceHash(IndentingWriter p) throws IOException {
        p.pln("private static final long interfaceHash = " + remoteClass.getInterfaceHash() + "L;");
    }
    ...
    private void writeMethodFieldDeclarations(IndentingWriter p) throws IOException {
        for (int i = 0; i < methodFieldNames.length; i++) {
            p.pln("private static java.lang.reflect.Method " + methodFieldNames[i] + ";");
        }
    }
    ...
    private void writeMethodFieldInitializers(IndentingWriter p) throws IOException {
        for (int i = 0; i < methodFieldNames.length; i++) {
            ...
            p.p(def.getClassDefinition().getName() + ".class.getMethod(\"" + methodName +
                "\", new java.lang.Class[] {");
     ...
     */
    private static void writeMarshalArgument(IndentingWriter p, String streamName, Type type, String name)
        throws IOException {
        int typeCode = type.getTypeCode();
        switch (typeCode) {
            case TC_BOOLEAN:
     ...
     */
    private static void writeMarshalArguments(IndentingWriter p, String streamName, Type[] types, String[] names)
        throws IOException {
     ...
     */
    private static boolean writeUnmarshalArgument(IndentingWriter p, String streamName, Type type, String name)
        throws IOException {
     ...
     * and discarded).
     */
    private static boolean writeUnmarshalArguments(IndentingWriter p, String streamName, Type[] types, String[] names)
        throws IOException {
        if (types.length != names.length) {
            throw new Error("paramter type and name arrays different sizes");
        }
