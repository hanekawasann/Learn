...
public class Main implements sun.rmi.rmic.Constants {
    ...
    public synchronized boolean compile(String argv[]) {
        ...
        for (int i = 0; i < argv.length; i++) {
            if (argv[i].equals("-Xnew")) {
                return (new sun.rmi.rmic.newrmic.Main(out, program)).compile(argv);
            }
        }
    ...
    public BatchEnvironment getEnv() {
        ...
        ClassPath classPath = BatchEnvironment.createClassPath(classPathString, sysClassPathArg, extDirsArg);
    ...
    public boolean doCompile() {
        ...
        try {
            ...
            for (int i = classes.size() - 1; i >= 0; i--) {
                Identifier implClassName = Identifier.lookup((String) classes.elementAt(i));
     ...
     */
    public boolean compileClass(ClassDeclaration c, ByteArrayOutputStream buf, BatchEnvironment env)
