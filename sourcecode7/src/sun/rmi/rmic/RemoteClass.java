...
public class RemoteClass implements sun.rmi.rmic.RMIConstants {
    ...
    private boolean initialize() {
        ...
        for (Enumeration<ClassDefinition> enumeration = remotesImplemented.elements(); enumeration
            .hasMoreElements(); ) {
                    ...
                    if (implMethod != null) {           // should not be null
                        ...
                        for (int i = 0; i < exceptions.length; i++) {
                            if (!defException.superClassOf(env, exceptions[i])) {
                                env.error(0, "rmic.must.only.throw.exception", implMethod.toString(),
    ...
    public class Method implements Cloneable {
        ...
        private Method mergeWith(Method other) {
            if (!getName().equals(other.getName()) || !getType().equals(other.getType())) {
                throw new Error("attempt to merge method \"" + other.getNameAndDescriptor() + "\" with \"" +
         ...
         */
        private void collectCompatibleExceptions(ClassDeclaration[] from, ClassDeclaration[] with,
            Vector<ClassDeclaration> list) throws ClassNotFound {
