...
public class UnicastServerRef extends UnicastRef implements ServerRef, Dispatcher {
    ...
    public static final boolean logCalls = AccessController
        .doPrivileged(new GetBooleanAction("java.rmi.server.logCalls"));
    ...
    private static final boolean wantExceptionLog = AccessController
        .doPrivileged(new GetBooleanAction("sun.rmi.server.exceptionTrace"));
     ...
     */
    private static final boolean suppressStackTraces = AccessController
        .doPrivileged(new GetBooleanAction("sun.rmi.server.suppressStackTraces"));
    ...
    private void logCall(Remote obj, Object method) {
        if (callLog.isLoggable(Log.VERBOSE)) {
            ...
            callLog.log(Log.VERBOSE,
                "[" + clientHost + ": " + obj.getClass().getName() + ref.getObjID().toString() + ": " + method + "]");
        }
    }
    ...
    private void logCallException(Throwable e) {
        ...
        if (wantExceptionLog) {
            ...
            synchronized (log) {
                log.println();
                log.println("Exception dispatching call to " + ref.getObjID() + " in thread \"" +
