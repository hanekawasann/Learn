            ...
            switch (nb) {
                case 1:
                    ...
                    if ((b1 >> 1) == -2) {
                        ...
                        if (src.remaining() < 5) { return CoderResult.UNDERFLOW; }
                ...
                case 3:
                    ...
                    return CoderResult.malformedForLength(
                        ((b1 == (byte) 0xe0 && (b2 & 0xe0) == 0x80) || isNotContinuation(b2)) ? 1 : 2);
        ...
        private CoderResult decodeArrayLoop(ByteBuffer src, CharBuffer dst) {
            ...
            while (sp < sl) {
                ...
                if (b1 >= 0) {
                    ...
                    int b2 = sa[sp + 1];
                    if (isMalformed2(b1, b2)) { return malformed(src, sp, dst, dp, 2); }
                    da[dp++] = (char) (((b1 << 6) ^ b2) ^ (((byte) 0xC0 << 6) ^ ((byte) 0x80 << 0)));
                    sp += 2;
                } else if ((b1 >> 4) == -2) {
                    // 3 bytes, 16 bits: 1110xxxx 10xxxxxx 10xxxxxx
                    if (sl - sp < 3 || dp >= dl) { return xflow(src, sp, sl, dst, dp, 3); }
                    int b2 = sa[sp + 1];
                    int b3 = sa[sp + 2];
                    if (isMalformed3(b1, b2, b3)) { return malformed(src, sp, dst, dp, 3); }
                    da[dp++] = (char) ((b1 << 12) ^ (b2 << 6) ^
                        (b3 ^ (((byte) 0xE0 << 12) ^ ((byte) 0x80 << 6) ^ ((byte) 0x80 << 0))));
                    sp += 3;
                } else if ((b1 >> 3) == -2) {
                    // 4 bytes, 21 bits: 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
                    if (sl - sp < 4 || dl - dp < 2) { return xflow(src, sp, sl, dst, dp, 4); }
                    int b2 = sa[sp + 1];
                    int b3 = sa[sp + 2];
                    int b4 = sa[sp + 3];
                    int uc = ((b1 << 18) ^ (b2 << 12) ^ (b3 << 6) ^
                        (b4 ^ (((byte) 0xF0 << 18) ^ ((byte) 0x80 << 12) ^ ((byte) 0x80 << 6) ^ ((byte) 0x80 << 0))));
        ...
        private CoderResult decodeBufferLoop(ByteBuffer src, CharBuffer dst) {
            ...
            while (mark < limit) {
                ...
                if (b1 >= 0) {
                    ...
                    int b2 = src.get();
                    if (isMalformed2(b1, b2)) { return malformed(src, mark, 2); }
                    dst.put((char) (((b1 << 6) ^ b2) ^ (((byte) 0xC0 << 6) ^ ((byte) 0x80 << 0))));
                    mark += 2;
                } else if ((b1 >> 4) == -2) {
                    // 3 bytes, 16 bits: 1110xxxx 10xxxxxx 10xxxxxx
                    if (limit - mark < 3 || dst.remaining() < 1) { return xflow(src, mark, 3); }
                    int b2 = src.get();
                    int b3 = src.get();
                    if (isMalformed3(b1, b2, b3)) { return malformed(src, mark, 3); }
                    dst.put((char) ((b1 << 12) ^ (b2 << 6) ^
                        (b3 ^ (((byte) 0xE0 << 12) ^ ((byte) 0x80 << 6) ^ ((byte) 0x80 << 0)))));
                    mark += 3;
                } else if ((b1 >> 3) == -2) {
                    // 4 bytes, 21 bits: 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
                    if (limit - mark < 4 || dst.remaining() < 2) { return xflow(src, mark, 4); }
                    int b2 = src.get();
                    int b3 = src.get();
                    int b4 = src.get();
                    int uc = ((b1 << 18) ^ (b2 << 12) ^ (b3 << 6) ^
                ...
                if (b1 >= 0) {
                    ...
                    if (sp + 2 < sl) {
                        ...
                        int uc = ((b1 << 18) ^ (b2 << 12) ^ (b3 << 6) ^ (b4 ^
                            (((byte) 0xF0 << 18) ^ ((byte) 0x80 << 12) ^ ((byte) 0x80 << 6) ^ ((byte) 0x80 << 0))));
                        if (isMalformed4(b2, b3, b4) ||
                            // shortest form check
                            !Character.isSupplementaryCodePoint(uc)) {
                            if (malformedInputAction() != CodingErrorAction.REPLACE) { return -1; }
