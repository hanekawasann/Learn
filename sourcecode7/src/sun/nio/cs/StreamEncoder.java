...
public class StreamEncoder extends Writer {
    ...
    public void flushBuffer() throws IOException {
        synchronized (lock) {
            if (isOpen()) { implFlushBuffer(); } else { throw new IOException("Stream closed"); }
        }
    }
    ...
    private void writeBytes() throws IOException {
        ...
        if (rem > 0) {
            if (ch != null) {
                if (ch.write(bb) != rem) { assert false : rem; }
            } else {
    ...
    private void flushLeftoverChar(CharBuffer cb, boolean endOfInput) throws IOException {
        ...
        if (lcb == null) { lcb = CharBuffer.allocate(2); } else { lcb.clear(); }
    ...
    void implWrite(char cbuf[], int off, int len) throws IOException {
        ...
        if (haveLeftoverChar) { flushLeftoverChar(cb, false); }
    ...
    void implClose() throws IOException {
        ...
        try {
            ...
            if (bb.position() > 0) { writeBytes(); }
            if (ch != null) { ch.close(); } else { out.close(); }
        } catch (IOException x) {
            encoder.reset();
            throw x;
        }
