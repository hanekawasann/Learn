...
public class AbstractCharsetProvider extends CharsetProvider {
     ...
     */
    private Map<String, SoftReference<Charset>> cache = new TreeMap<>(
    ...
    private static <K, V> void put(Map<K, V> m, K name, V value) {
        if (!m.containsKey(name)) { m.put(name, value); }
    }
    ...
    protected void charset(String name, String className, String[] aliases) {
        synchronized (this) {
            ...
            for (int i = 0; i < aliases.length; i++) { put(aliasMap, aliases[i], name); }
    ...
    protected void deleteCharset(String name, String[] aliases) {
        synchronized (this) {
            ...
            for (int i = 0; i < aliases.length; i++) { remove(aliasMap, aliases[i]); }
