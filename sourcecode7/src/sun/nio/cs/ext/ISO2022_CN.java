...
public class ISO2022_CN extends Charset implements HistoricallyNamedCharset {
    ...
    static class Decoder extends CharsetDecoder {
        ...
        private char cnsDecode(byte byte1, byte byte2, byte SS) {
            ...
            char[] ret = cnsDecoder.toUnicode(byte1 & 0xff, byte2 & 0xff, p);
            if (ret == null || ret.length == 2) { return REPLACE_CHAR; }
            return ret[0];
        }
        ...
        private char SODecode(byte byte1, byte byte2, byte SOD) {
            ...
            byte2 |= MSB;
            if (SOD == SODesigGB) {
                return gb2312Decoder.decodeDouble(byte1 & 0xff, byte2 & 0xff);
            } else {    // SOD == SODesigCNS
                char[] ret = cnsDecoder.toUnicode(byte1 & 0xff, byte2 & 0xff, 0);
                if (ret == null) { return REPLACE_CHAR; }
                return ret[0];
            }
        }
                ...
                while (src.hasRemaining()) {
                    ...
                    while (b1 == ISO_ESC || b1 == ISO_SO || b1 == ISO_SI) {
                        if (b1 == ISO_ESC) {  // ESC
                            ...
                            if ((b2 & (byte) 0x80) != 0) { return CoderResult.malformedForLength(inputSize); }
                            ...
                            if (b2 == (byte) 0x24) {
                                ...
                                if ((b3 & (byte) 0x80) != 0) { return CoderResult.malformedForLength(inputSize); }
                                if (b3 == 'A') {              // "$A"
                                    currentSODesig = SODesigGB;
                                } else if (b3 == ')') {
                                    if (src.remaining() < 1) { return CoderResult.UNDERFLOW; }
                        ...
                        if (b1 == ISO_ESC) {  // ESC
                            ...
                            if (b2 == (byte) 0x24) {
                                ...
                                if (b3 == 'A') {              // "$A"
                                    ...
                                    if (sp + 4 > sl) { return CoderResult.UNDERFLOW; }
                                    b4 = sa[sp + 3];
                                    inputSize++;
