...
class UTF_32Coder {
    ...
    protected static class Decoder extends CharsetDecoder {
        ...
        private int getCP(ByteBuffer src) {
            return (currentBO == BIG) ? (((src.get() & 0xff) << 24) | ((src.get() & 0xff) << 16) |
                ((src.get() & 0xff) << 8) | (src.get() & 0xff))
                : ((src.get() & 0xff) | ((src.get() & 0xff) << 8) | ((src.get() & 0xff) << 16) |
                    ((src.get() & 0xff) << 24));
        }
        ...
        protected CoderResult decodeLoop(ByteBuffer src, CharBuffer dst) {
            ...
            try {
                if (currentBO == NONE) {
                    cp = ((src.get() & 0xff) << 24) | ((src.get() & 0xff) << 16) | ((src.get() & 0xff) << 8) |
                        (src.get() & 0xff);
                    if (cp == BOM_BIG && expectedBO != LITTLE) {
                        ...
                        if (expectedBO == NONE) { currentBO = BIG; } else { currentBO = expectedBO; }
                        src.position(mark);
                    }
                }
