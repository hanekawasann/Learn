...
public abstract class SingleByteEncoder extends CharsetEncoder {
    ...
    public boolean canEncode(char c) {
        char testEncode = index2.charAt(index1[(c & mask1) >> shift] + (c & mask2));
        return testEncode != '\u0000' || c == '\u0000';
    }
    ...
    private CoderResult encodeArrayLoop(CharBuffer src, ByteBuffer dst) {
        ...
        try {
            while (sp < sl) {
                ...
                if (c >= '\uFFFE') { return CoderResult.unmappableForLength(1); }
                if (dl - dp < 1) { return CoderResult.OVERFLOW; }

                char e = index2.charAt(index1[(c & mask1) >> shift] + (c & mask2));

                // If output byte is zero because input char is zero
                // then character is mappable, o.w. fail
                if (e == '\u0000' && c != '\u0000') { return CoderResult.unmappableForLength(1); }
    ...
    protected CoderResult encodeLoop(CharBuffer src, ByteBuffer dst) {
        if (true && src.hasArray() && dst.hasArray()) { return encodeArrayLoop(src, dst); } else {
