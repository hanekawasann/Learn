...
public class DatagramSocketAdaptor extends DatagramSocket {
    ...
    private void connectInternal(SocketAddress remote) throws SocketException {
        ...
        if (port < 0 || port > 0xFFFF) { throw new IllegalArgumentException("connect: " + port); }
        if (remote == null) { throw new IllegalArgumentException("connect: null address"); }
        if (isClosed()) { return; }
        try {
            dc.connect(remote);
        } catch (Exception x) {
    ...
    public void connect(SocketAddress remote) throws SocketException {
        if (remote == null) { throw new IllegalArgumentException("Address can't be null"); }
    ...
    public void send(DatagramPacket p) throws IOException {
        synchronized (dc.blockingLock()) {
            if (!dc.isBlocking()) { throw new IllegalBlockingModeException(); }
            try {
                synchronized (p) {
                    ...
                    if (dc.isConnected()) {
                        if (p.getAddress() == null) {
                            ...
                            InetSocketAddress isa = (InetSocketAddress) dc.remoteAddress();
    ...
    private SocketAddress receive(ByteBuffer bb) throws IOException {
        ...
        try {
            ...
            if ((sender = dc.receive(bb)) != null) { return sender; }
            sel = Util.getTemporarySelector(dc);
            sk = dc.register(sel, SelectionKey.OP_READ);
            long to = timeout;
            for (; ; ) {
                if (!dc.isOpen()) { throw new ClosedChannelException(); }
            ...
            if (dc.isOpen()) { dc.configureBlocking(true); }
