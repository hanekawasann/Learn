...
class IOUtil {
    ...
    static int write(FileDescriptor fd, ByteBuffer src, long position, NativeDispatcher nd, Object lock)
        throws IOException {
        if (src instanceof DirectBuffer) { return writeFromNativeBuffer(fd, src, position, nd, lock); }
    ...
    private static int writeFromNativeBuffer(FileDescriptor fd, ByteBuffer bb, long position, NativeDispatcher nd,
        ...
        if (rem == 0) { return 0; }
        if (position != -1) {
            written = nd.pwrite(fd, ((DirectBuffer) bb).address() + pos, rem, position, lock);
        } else {
    ...
    static long write(FileDescriptor fd, ByteBuffer[] bufs, int offset, int length, NativeDispatcher nd)
        ...
        try {
            ...
            for (int j = 0; j < iov_len; j++) {
                ...
                if (shadow != null) { Util.offerLastTemporaryDirectBuffer(shadow); }
                vec.clearRefs(j);
            }
    ...
    static int read(FileDescriptor fd, ByteBuffer dst, long position, NativeDispatcher nd, Object lock)
        throws IOException {
        if (dst.isReadOnly()) { throw new IllegalArgumentException("Read-only buffer"); }
        if (dst instanceof DirectBuffer) { return readIntoNativeBuffer(fd, dst, position, nd, lock); }
    ...
    private static int readIntoNativeBuffer(FileDescriptor fd, ByteBuffer bb, long position, NativeDispatcher nd,
        ...
        int n = 0;
        if (position != -1) {
            n = nd.pread(fd, ((DirectBuffer) bb).address() + pos, rem, position, lock);
        } else {
    ...
    static long read(FileDescriptor fd, ByteBuffer[] bufs, int offset, int length, NativeDispatcher nd)
        ...
        try {
            ...
            for (int i = offset; i < count; i++) {
                ByteBuffer buf = bufs[i];
                if (buf.isReadOnly()) { throw new IllegalArgumentException("Read-only buffer"); }
                int pos = buf.position();
                int lim = buf.limit();
                assert (pos <= lim);
                int rem = (pos <= lim ? lim - pos : 0);
