...
class Util {
    ...
    private static ThreadLocal<SelectorWrapper> localSelectorWrapper = new ThreadLocal<SelectorWrapper>();
    ...
    static Selector getTemporarySelector(SelectableChannel sc) throws IOException {
        ...
        Selector sel = null;
        if (ref == null || ((selWrapper = ref.get()) == null) || ((sel = selWrapper.get()) == null) ||
    ...
    static ByteBuffer[] subsequence(ByteBuffer[] bs, int offset, int length) {
        if ((offset == 0) && (length == bs.length)) { return bs; }
        int n = length;
        ByteBuffer[] bs2 = new ByteBuffer[n];
        for (int i = 0; i < n; i++) { bs2[i] = bs[offset + i]; }
        return bs2;
    }
    ...
    static MappedByteBuffer newMappedByteBuffer(int size, long addr, FileDescriptor fd, Runnable unmapper) {
        ...
        try {
            dbb = (MappedByteBuffer) directByteBufferConstructor
                .newInstance(new Object[] { new Integer(size), new Long(addr), fd, unmapper });
