...
abstract class AsynchronousChannelGroupImpl extends AsynchronousChannelGroup implements Executor {
    ...
    private static final int internalThreadCount = AccessController
        .doPrivileged(new GetIntegerAction("sun.nio.ch.internalThreadPoolSize", 1));
    ...
    AsynchronousChannelGroupImpl(AsynchronousChannelProvider provider, ThreadPool pool) {
        ...
        this.timeoutExecutor = (ScheduledThreadPoolExecutor) Executors
            .newScheduledThreadPool(1, ThreadPool.defaultThreadFactory());
    ...
    final void detachFromThreadPool() {
        if (shutdown.getAndSet(true)) { throw new AssertionError("Already shutdown"); }
        if (!isEmpty()) { throw new AssertionError("Group not empty"); }
        shutdownHandlerTasks();
    }
