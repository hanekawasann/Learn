...
final class PendingFuture<V, A> implements Future<V> {
    ...
    void setFailure(Throwable x) {
        if (!(x instanceof IOException) && !(x instanceof SecurityException)) { x = new IOException(x); }
        synchronized (this) {
            if (haveResult) { return; }
            exc = x;
            haveResult = true;
    ...
    public V get() throws ExecutionException, InterruptedException {
        if (!haveResult) {
            ...
            if (needToWait) { latch.await(); }
        }
        if (exc != null) {
            if (exc == CANCELLED) { throw new CancellationException(); }
    ...
    public V get(long timeout, TimeUnit unit) throws ExecutionException, InterruptedException, TimeoutException {
        if (!haveResult) {
            boolean needToWait = prepareForWait();
            if (needToWait) { if (!latch.await(timeout, unit)) { throw new TimeoutException(); } }
        }
        if (exc != null) {
            if (exc == CANCELLED) { throw new CancellationException(); }
            throw new ExecutionException(exc);
    ...
    public boolean cancel(boolean mayInterruptIfRunning) {
        synchronized (this) {
            ...
            // notify channel
            if (channel() instanceof Cancellable) { ((Cancellable) channel()).onCancel(this); }
