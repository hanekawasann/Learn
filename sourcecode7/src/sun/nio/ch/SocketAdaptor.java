...
public class SocketAdaptor extends Socket {
    ...
    public void connect(SocketAddress remote, int timeout) throws IOException {
        if (remote == null) { throw new IllegalArgumentException("connect: The address can't be null"); }
        ...
        synchronized (sc.blockingLock()) {
            ...
            try {
                ...
                try {
                    if (sc.connect(remote)) { return; }
                    sel = Util.getTemporarySelector(sc);
                    sk = sc.register(sel, SelectionKey.OP_CONNECT);
                    long to = timeout;
                    for (; ; ) {
                        if (!sc.isOpen()) { throw new ClosedChannelException(); }
                        long st = System.currentTimeMillis();
                        int ns = sel.select(to);
                        if (ns > 0 && sk.isConnectable() && sc.finishConnect()) { break; }
                    ...
                    if (sc.isOpen()) { sc.configureBlocking(true); }
                    if (sel != null) { Util.releaseTemporarySelector(sel); }
                }
    ...
    public InetAddress getLocalAddress() {
        if (sc.isOpen()) {
            ...
            if (local != null) { return ((InetSocketAddress) local).getAddress(); }
        }
    ...
    private class SocketInputStream extends ChannelInputStream {
        ...
        protected int read(ByteBuffer bb) throws IOException {
            synchronized (sc.blockingLock()) {
                if (!sc.isBlocking()) { throw new IllegalBlockingModeException(); }
                if (timeout == 0) { return sc.read(bb); }
                ...
                try {
                    ...
                    for (; ; ) {
                        ...
                        sel.selectedKeys().remove(sk);
                        to -= System.currentTimeMillis() - st;
                        if (to <= 0) { throw new SocketTimeoutException(); }
                    }
                } finally {
                    if (sk != null) { sk.cancel(); }
                    if (sc.isOpen()) { sc.configureBlocking(true); }
    ...
    public InputStream getInputStream() throws IOException {
        if (!sc.isOpen()) { throw new SocketException("Socket is closed"); }
        if (!sc.isConnected()) { throw new SocketException("Socket is not connected"); }
        if (!sc.isInputOpen()) { throw new SocketException("Socket input is shutdown"); }
    ...
    public OutputStream getOutputStream() throws IOException {
        if (!sc.isOpen()) { throw new SocketException("Socket is closed"); }
        if (!sc.isConnected()) { throw new SocketException("Socket is not connected"); }
        if (!sc.isOutputOpen()) { throw new SocketException("Socket output is shutdown"); }
    ...
    public void setSendBufferSize(int size) throws SocketException {
        ...
        if (size <= 0) { throw new IllegalArgumentException("Invalid send size"); }
