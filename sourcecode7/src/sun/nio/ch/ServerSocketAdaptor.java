...
public class ServerSocketAdaptor                        // package-private
    ...
    public Socket accept() throws IOException {
        synchronized (ssc.blockingLock()) {
            if (!ssc.isBound()) { throw new IllegalBlockingModeException(); }
            try {
                if (timeout == 0) {
                    SocketChannel sc = ssc.accept();
                    if (sc == null && !ssc.isBlocking()) { throw new IllegalBlockingModeException(); }
                    return sc.socket();
                }
                ...
                try {
                    SocketChannel sc;
                    if ((sc = ssc.accept()) != null) { return sc.socket(); }
                    ...
                    for (; ; ) {
                        ...
                        if (ns > 0 && sk.isAcceptable() && ((sc = ssc.accept()) != null)) { return sc.socket(); }
                    ...
                    if (ssc.isOpen()) { ssc.configureBlocking(true); }
    ...
    public void setReceiveBufferSize(int size) throws SocketException {
        ...
        if (size <= 0) { throw new IllegalArgumentException("size cannot be 0 or negative"); }
        try {
