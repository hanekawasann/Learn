...
class IOVecWrapper {
    ...
    void putBase(int i, long base) {
        ...
        if (addressSize == 4) { vecArray.putInt(offset, (int) base); } else { vecArray.putLong(offset, base); }
    }

    void putLen(int i, long len) {
        int offset = SIZE_IOVEC * i + LEN_OFFSET;
        if (addressSize == 4) { vecArray.putInt(offset, (int) len); } else { vecArray.putLong(offset, len); }
    }
