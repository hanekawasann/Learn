...
public class ParseUtil {
    ...
    public static String encodePath(String path, boolean flag) {
        ...
        for (int i = 0; i < n; i++) {
            char c = pathCC[i];
            if ((!flag && c == '/') || (flag && c == File.separatorChar)) { retCC[retLen++] = '/'; } else {
                if (c <= 0x007F) {
                    if (c >= 'a' && c <= 'z' || c >= 'A' && c <= 'Z' || c >= '0' && c <= '9') {
                        retCC[retLen++] = c;
                    } else if (encodedInPath.get(c)) { retLen = escape(retCC, c, retLen); } else {
                        retCC[retLen++] = c;
                    }
                } else if (c > 0x07FF) {
    ...
    public static String decode(String s) {
        ...
        CharsetDecoder dec = ThreadLocalCoders.decoderFor("UTF-8").onMalformedInput(CodingErrorAction.REPORT)
    ...
    private static void appendSchemeSpecificPart(StringBuffer sb, String opaquePart, String authority, String userInfo,
    ...
    private static void appendEncoded(StringBuffer sb, char c) {
        ...
        while (bb.hasRemaining()) {
            int b = bb.get() & 0xff;
            if (b >= 0x80) { appendEscape(sb, (byte) b); } else { sb.append((char) b); }
        }
    }
