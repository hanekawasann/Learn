...
public class GopherClient extends NetworkClient implements Runnable {
    ...
    static {
        useGopherProxy = java.security.AccessController
            .doPrivileged(new sun.security.action.GetBooleanAction("gopherProxySet")).booleanValue();

        gopherProxyHost = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("gopherProxyHost"));

        gopherProxyPort = java.security.AccessController
            .doPrivileged(new sun.security.action.GetIntegerAction("gopherProxyPort", 80)).intValue();
    }
    ...
    public static boolean getUseGopherProxy() {
        return java.security.AccessController.doPrivileged(new GetBooleanAction("gopherProxySet")).booleanValue();
    }
    ...
    public static String getGopherProxyHost() {
        String host = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("gopherProxyHost"));
        if ("".equals(host)) {
            host = null;
        }
    ...
    InputStream openStream(URL u) throws IOException {
        ...
        if (gtype != '7') {
            ...
            serverOutput.print(decodePercent(gkey.substring(0, i) + "\t" + gkey.substring(i + 1) + "\r\n"));
            serverOutput.flush();
            msgh.add("content-type", "text/html");
        } else {
            msgh.add("content-type", "text/html");
    ...
    private String decodePercent(String s) {
        ...
        for (int sp = 0; sp < limit; sp++) {
            ...
            if (c == '%' && sp + 2 < limit) {
                ...
                if ('0' <= s1 && s1 <= '9') { s1 = s1 - '0'; } else if ('a' <= s1 && s1 <= 'f') {
                    s1 = s1 - 'a' + 10;
                } else if ('A' <= s1 && s1 <= 'F') {
                    s1 = s1 - 'A' + 10;
                } else { s1 = -1; }
                if ('0' <= s2 && s2 <= '9') { s2 = s2 - '0'; } else if ('a' <= s2 && s2 <= 'f') {
                    s2 = s2 - 'a' + 10;
                } else if ('A' <= s2 && s2 <= 'F') {
                    s2 = s2 - 'A' + 10;
                } else { s2 = -1; }
    ...
    public void run() {
        ...
        try {
            if (gtype == '7' && (qpos = gkey.indexOf('?')) < 0) {
                ...
                String title = null;
                if (gtype == '7') {
                    title = "Results of searching for \"" + gkey.substring(qpos + 1) + "\" on " + u.getHost();
                } else { title = "Gopher directory " + gkey + " from " + u.getHost(); }
                ...
                while ((s = ds.readLine()) != null) {
                    ...
                    ps.print("<dt><a href=\"gopher://" + host + port + "/" + s.substring(0, 1) +
                        encodePercent(s.substring(t1 + 1, t2)) + "\">\n");
