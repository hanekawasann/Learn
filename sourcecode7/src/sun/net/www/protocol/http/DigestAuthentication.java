...
class DigestAuthentication extends AuthenticationInfo {
     ...
     *
     * @param requestURI the Request-URI from the HTTP request line
     * @param method     the HTTP method
     * @return the value of the HTTP header this authentication wants set
     */
    ...
    public boolean setHeaders(HttpURLConnection conn, HeaderParser p, String raw) {
        ...
        String uri = "";
        String method;
        if (type == PROXY_AUTHENTICATION && conn.tunnelState() == HttpURLConnection.TunnelState.SETUP) {
    ...
    private String getHeaderValueImpl(String uri, String method) {
        ...
        if (nccount != -1) {
            ...
            if (len < 8) { ncstring = zeroPad[len] + ncstring; }
        }
        ...
        String value = authMethod + " username=\"" + pw.getUserName() + "\", realm=\"" + realm + "\", nonce=\"" +
            nonce + ncfield + ", uri=\"" + uri + "\", response=\"" + response + "\", algorithm=\"" + algorithm;
        if (opaque != null) {
