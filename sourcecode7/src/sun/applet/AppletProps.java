...
class AppletProps extends Frame {
    ...
    void reset() {
        ...
        if (security != null) { security.reset(); }

        String proxyhost = (String) AccessController.doPrivileged(new GetPropertyAction("http.proxyHost"));
        String proxyport = (String) AccessController.doPrivileged(new GetPropertyAction("http.proxyPort"));

        Boolean tmp = (Boolean) AccessController.doPrivileged(new GetBooleanAction("package.restrict.access.sun"));
    ...
    void apply() {
        ...
        if (proxyHostValue.length() != 0) {
            ...
            if (proxyPortNumber <= 0) {
                ...
                (new AppletPropsErrorDialog(this, amh.getMessage("title.invalidproxy"),
                    amh.getMessage("label.invalidproxy"), amh.getMessage("button.ok"))).show();
                return;
            }
            /* end 4066402 */
...
class AppletPropsErrorDialog extends Dialog {
    public AppletPropsErrorDialog(Frame parent, String title, String message, String buttonText) {
        ...
        move(fRect.x + ((fRect.width - dDim.width) / 2), fRect.y + ((fRect.height - dDim.height) / 2));
    }
