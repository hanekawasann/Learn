...
public class TrieIterator implements RangeValueIterator {
     ...
     * @throws IllegalArgumentException throw when argument is null.
     */
    ...
    private final void calculateNextSupplementaryElement(Element element) {
        ...
        while (nextLead < TRAIL_SURROGATE_MIN_VALUE_) {
            ...
            int leadBlock = m_trie_.m_index_[nextLead >> Trie.INDEX_STAGE_1_SHIFT_] << Trie.INDEX_STAGE_2_SHIFT_;
            if (leadBlock == m_trie_.m_dataOffset_) {
                ...
                m_nextCodepoint_ = UCharacterProperty
                    .getRawSupplementary((char) nextLead, (char) UTF16.TRAIL_SURROGATE_MIN_VALUE);
                continue;
            }
            ...
            m_nextIndex_ = m_trie_.m_dataManipulate_
                .getFoldingOffset(m_trie_.getValue(leadBlock + (nextLead & Trie.INDEX_STAGE_3_MASK_)));
            if (m_nextIndex_ <= 0) {
    ...
    private final boolean checkBlock(int currentBlock, int currentValue) {
        m_nextBlock_ = m_trie_.m_index_[m_nextIndex_] << Trie.INDEX_STAGE_2_SHIFT_;
