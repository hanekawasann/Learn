...
public class BidiBase {
    ...
    /* character array representing the current text */ char[] text;

    /* length of the current text */ int originalLength;
    ...
    /* indicators for whether memory may be allocated after construction */ boolean mayAllocateText;
    boolean mayAllocateRuns;

    /* arrays with one value per text-character */ byte[] dirPropsMemory = new byte[1];
    byte[] levelsMemory = new byte[1];
    byte[] dirProps;
    byte[] levels;

    /* must block separators receive level 0? */ boolean orderParagraphsLTR;

    /* the paragraph level */ byte paraLevel;

    /* original paraLevel when contextual */
    /* must be one of DEFAULT_xxx or 0 if not contextual */ byte defaultParaLevel;

    /* the following is set in setPara, used in processPropertySeq */

    ImpTabPair impTabPair;  /* reference to levels state table pair */

    /* the overall paragraph or line directionality*/ byte direction;

    /* flags is a bit set for which directional properties are in the text */ int flags;

    /* lastArabicPos is index to the last AL in the text, -1 if none */ int lastArabicPos;

    /* characters after trailingWSStart are WS and are */
    /* implicitly at the paraLevel (rule (L1)) - levels may not reflect that */ int trailingWSStart;

    /* fields for paragraph handling */ int paraCount;       /* set in getDirProps() */
    int[] parasMemory = new int[1];
    int[] paras;           /* limits of paragraphs, filled in
                                          ResolveExplicitLevels() or CheckExplicitLevels() */

    /* for single paragraph text, we only need a tiny array of paras (no allocation) */ int[] simpleParas = { 0 };

    /* fields for line reordering */ int runCount;     /* ==-1: runs not set up yet */
    BidiRun[] runsMemory = new BidiRun[0];
    BidiRun[] runs;

    /* for non-mixed text, we only need a tiny array of runs (no allocation) */ BidiRun[] simpleRuns = {
        new BidiRun() };

    /* mapping of runs in logical order to visual order */ int[] logicalToVisualRunsMap;

    /* flag to indicate that the map has been updated */ boolean isGoodLogicalToVisualRunsMap;

    /* for inverse Bidi with insertion of directional marks */ InsertPoints insertPoints = new InsertPoints();

    /* for option OPTION_REMOVE_CONTROLS */ int controlCount;
     ...
     */
    static final int MASK_LTR = DirPropFlag(L) | DirPropFlag(EN) | DirPropFlag(AN) | DirPropFlag(LRE) |
        DirPropFlag(LRO);
    ...
    byte GetParaLevelAt(int index) {
        return (defaultParaLevel != 0) ? (byte) (dirProps[index] >> CONTEXT_RTL_SHIFT) : paraLevel;
    }
    ...
    private byte resolveExplicitLevels() {
        ...
        if ((dirct != MIXED) && (paraCount == 1)) {
        ...
        } else if ((paraCount == 1) && ((flags & MASK_EXPLICIT) == 0)) {
            ...
            for (i = 0; i < length; ++i) {
                ...
                switch (dirProp) {
                    ...
                    case LRO:
                        /* (X3, X5) */
                        newLevel = (byte) ((embeddingLevel + 2) &
                            ~(INTERNAL_LEVEL_OVERRIDE | 1)); /* least greater even level */
                    ...
                    case RLO:
                        /* (X2, X4) */
                        newLevel = (byte) (((embeddingLevel & ~INTERNAL_LEVEL_OVERRIDE) + 1) |
                    ...
                    case PDF:
                        ...
                        if (countOver61 > 0) {
                            ...
                            (embeddingLevel & ~INTERNAL_LEVEL_OVERRIDE) != MAX_EXPLICIT_LEVEL) {
    ...
    private static final ImpTabPair impTab_INVERSE_LIKE_DIRECT_WITH_MARKS = new ImpTabPair(
        impTabL_INVERSE_LIKE_DIRECT_WITH_MARKS, impTabR_INVERSE_LIKE_DIRECT_WITH_MARKS, impAct0, impAct2);
    ...
    private static final ImpTabPair impTab_INVERSE_FOR_NUMBERS_SPECIAL_WITH_MARKS = new ImpTabPair(
        impTabL_INVERSE_FOR_NUMBERS_SPECIAL_WITH_MARKS, impTabR_INVERSE_LIKE_DIRECT_WITH_MARKS, impAct0, impAct2);
            ...
            switch (actionSeq) {
                ...
                case 4:                     /* R/AL after possible relevant EN/AN */
                    ...
                    if (insertPoints.points.length > 0)
                        /* remove all non confirmed insert points */ { insertPoints.size = insertPoints.confirmed; }
                    levState.startON = -1;
    ...
    public static boolean requiresBidi(char[] text, int start, int limit) {
        final int RTLMask = (1 << Bidi.DIRECTION_RIGHT_TO_LEFT | 1 << AL | 1 << RLE | 1 << RLO | 1 << AN);
    ...
    public static void reorderVisually(byte[] levels, int levelStart, Object[] objects, int objectStart, int count) {
        ...
        if (0 > objectStart || objects.length <= objectStart) {
            throw new IllegalArgumentException(
                "Value objectStart " + levelStart + " is out of range 0 to " + (objects.length - 1));
        }
        if (0 > count || objects.length < (objectStart + count)) {
            throw new IllegalArgumentException(
                "Value count " + levelStart + " is out of range 0 to " + (objects.length - objectStart));
        }
    ...
    private static class TextAttributeConstants {
         ...
         */
        static final Boolean RUN_DIRECTION_LTR = (clazz == null) ? Boolean.FALSE
    ...
    private static class NumericShapings {
        ...
        private static final Method shapeMethod = getMethod(clazz, "shape", char[].class, int.class, int.class);
        ...
        static void shape(Object shaper, char[] text, int start, int count) {
            if (shapeMethod == null) { throw new AssertionError("Should not get here"); }
            try {
