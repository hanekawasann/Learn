...
public final class IntHashtable {
    ...
    public boolean equals(Object that) {
        ...
        for (int i = 0; i < keyList.length; ++i) {
            int key = keyList[i];
            if (key > MAX_UNUSED && other.get(key) != values[i]) { return false; }
        }
        return true;
    }
    ...
    private int find(int key) {
        if (key <= MAX_UNUSED) { throw new IllegalArgumentException("key can't be less than 0xFFFFFFFE"); }
    ...
    private static final int[] PRIMES = { 17, 37, 67, 131, 257, 521, 1031, 2053, 4099, 8209, 16411, 32771, 65537,
        131101, 262147, 524309, 1048583, 2097169, 4194319, 8388617, 16777259, 33554467, 67108879, 134217757, 268435459,
