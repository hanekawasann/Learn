...
public abstract class KeyboardFocusManagerPeerImpl implements KeyboardFocusManagerPeer {
    ...
    private static final PlatformLogger focusLog = PlatformLogger
        .getLogger("sun.awt.focus.KeyboardFocusManagerPeerImpl");
     ...
     */
    public static boolean deliverFocus(Component lightweightChild, Component target, boolean temporary,
        ...
        Component currentFocusOwner) // provided by the descendant peers
    {
        ...
        if (currentOwner != null) {
            ...
            if (focusLog.isLoggable(PlatformLogger.FINER)) { focusLog.finer("Posting focus event: " + fl); }
            SunToolkit.postPriorityEvent(fl);
        }

        FocusEvent fg = new CausedFocusEvent(lightweightChild, FocusEvent.FOCUS_GAINED, false, currentOwner, cause);

        if (focusLog.isLoggable(PlatformLogger.FINER)) { focusLog.finer("Posting focus event: " + fg); }
    ...
    // WARNING: Don't call it on the Toolkit thread.
    public static int shouldNativelyFocusHeavyweight(Component heavyweight, Component descendant, boolean temporary,
        boolean focusedWindowChangeAllowed, long time, CausedFocusEvent.Cause cause) {
        return kfmAccessor
            .shouldNativelyFocusHeavyweight(heavyweight, descendant, temporary, focusedWindowChangeAllowed, time,
                cause);
    }
    ...
    // WARNING: Don't call it on the Toolkit thread.
    public static boolean processSynchronousLightweightTransfer(Component heavyweight, Component descendant,
        boolean temporary, boolean focusedWindowChangeAllowed, long time) {
        return kfmAccessor
            .processSynchronousLightweightTransfer(heavyweight, descendant, temporary, focusedWindowChangeAllowed,
                time);
    }
}
