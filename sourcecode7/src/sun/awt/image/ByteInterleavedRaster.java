...
public class ByteInterleavedRaster extends ByteComponentRaster {
     ...
     */
    public ByteInterleavedRaster(SampleModel sampleModel, DataBuffer dataBuffer, Rectangle aRegion, Point origin,
        ...
        if (sampleModel instanceof PixelInterleavedSampleModel ||
            ...
            throw new RasterFormatException(
                "ByteInterleavedRasters must " + "have PixelInterleavedSampleModel, SinglePixelPackedSampleModel" +
                    " or interleaved ComponentSampleModel.  Sample model is " + sampleModel);
        }
        this.bandOffset = this.dataOffsets[0];

        this.dbOffsetPacked = dataBuffer.getOffset() - sampleModelTranslateY * scanlineStride -
    ...
    public byte[] getByteData(int x, int y, int w, int h, int band, byte[] outData) {
        ...
        int yoff = (y - minY) * scanlineStride + (x - minX) * pixelStride + dataOffsets[band];
        int xoff;
        int off = 0;
        int xstart;
        int ystart;
    ...
    private void setDataElements(int dstX, int dstY, int srcX, int srcY, int width, int height, Raster inRaster) {
        ...
        if (inRaster instanceof ByteInterleavedRaster) {
            ...
            if (inOrder && bct.inOrder && pixelStride == bct.pixelStride) {
                ...
                int dstOffset = dataOffsets[0] + (dstY - minY) * scanlineStride + (dstX - minX) * pixelStride;
    ...
    public WritableRaster createWritableChild(int x, int y, int width, int height, int x0, int y0, int[] bandList) {
        ...
        if (bandList != null) { sm = sampleModel.createSubsetSampleModel(bandList); } else { sm = sampleModel; }

        int deltaX = x0 - x;
        int deltaY = y0 - y;

        return new ByteInterleavedRaster(sm, dataBuffer, new Rectangle(x0, y0, width, height),
    ...
    private void verify(boolean strictCheck) {
        ...
        for (int i = 0; i < numDataElements; i++) {
            size = (height - 1) * scanlineStride + (width - 1) * pixelStride + dataOffsets[i];
            if (size > maxSize) {
                maxSize = size;
            }
        }
    ...
    public String toString() {
        return new String(
            "ByteInterleavedRaster: width = " + width + " height = " + height + " #numDataElements " + numDataElements
