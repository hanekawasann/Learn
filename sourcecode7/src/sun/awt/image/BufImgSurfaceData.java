...
public class BufImgSurfaceData extends SurfaceData {
    ...
    public static SurfaceData createData(BufferedImage bufImg) {
        ...
        switch (type) {
            ...
            default: {
                ...
                if (raster instanceof IntegerComponentRaster && raster.getNumDataElements() == 1 &&
                    ...
                    if (cm instanceof DirectColorModel) {
                        ...
                        if (numBands == 3 && aMask == 0 && rMask == DCM_RGBX_RED_MASK && gMask == DCM_RGBX_GREEN_MASK &&
                            bMask == DCM_RGBX_BLUE_MASK) {
                            sType = SurfaceType.IntRgbx;
                        } else if (numBands == 4 && aMask == DCM_ARGBBM_ALPHA_MASK && rMask == DCM_ARGBBM_RED_MASK &&
                            gMask == DCM_ARGBBM_GREEN_MASK && bMask == DCM_ARGBBM_BLUE_MASK) {
                    ...
                    break;
                } else if (raster instanceof ShortComponentRaster && raster.getNumDataElements() == 1 &&
                    ...
                    if (cm instanceof DirectColorModel) {
                        ...
                        int gMask = dcm.getGreenMask();
                        int bMask = dcm.getBlueMask();
                        if (numBands == 3 && aMask == 0 && rMask == DCM_555X_RED_MASK && gMask == DCM_555X_GREEN_MASK &&
                            bMask == DCM_555X_BLUE_MASK) {
                            sType = SurfaceType.Ushort555Rgbx;
                        } else if (numBands == 4 && aMask == DCM_4444_ALPHA_MASK && rMask == DCM_4444_RED_MASK &&
                            gMask == DCM_4444_GREEN_MASK && bMask == DCM_4444_BLUE_MASK) {
    ...
    public static SurfaceData createDataIC(BufferedImage bImg, SurfaceType sType) {
        ...
        bisd.initRaster(icRaster.getDataStorage(), icRaster.getDataOffset(0) * 4, 0, icRaster.getWidth(),
            icRaster.getHeight(), icRaster.getPixelStride() * 4, icRaster.getScanlineStride() * 4, null);
        return bisd;
    }
    ...
    public static SurfaceData createDataSC(BufferedImage bImg, SurfaceType sType, IndexColorModel icm) {
        ...
        bisd.initRaster(scRaster.getDataStorage(), scRaster.getDataOffset(0) * 2, 0, scRaster.getWidth(),
            scRaster.getHeight(), scRaster.getPixelStride() * 2, scRaster.getScanlineStride() * 2, icm);
        return bisd;
    }
    ...
    public static SurfaceData createDataBC(BufferedImage bImg, SurfaceType sType, int primaryBank) {
        ...
        bisd.initRaster(bcRaster.getDataStorage(), bcRaster.getDataOffset(primaryBank), 0, bcRaster.getWidth(),
            bcRaster.getHeight(), bcRaster.getPixelStride(), bcRaster.getScanlineStride(), icm);
        return bisd;
    }
    ...
    public static SurfaceData createDataBP(BufferedImage bImg, SurfaceType sType) {
        ...
        ColorModel cm = bImg.getColorModel();
        IndexColorModel icm = ((cm instanceof IndexColorModel) ? (IndexColorModel) cm : null);
        bisd.initRaster(bpRaster.getDataStorage(), bpRaster.getDataBitOffset() / 8, bpRaster.getDataBitOffset() & 7,
            bpRaster.getWidth(), bpRaster.getHeight(), 0, bpRaster.getScanlineStride(), icm);
        return bisd;
    }

    public RenderLoops getRenderLoops(SunGraphics2D sg2d) {
        if (sg2d.paintState <= sg2d.PAINT_ALPHACOLOR && sg2d.compositeState <= sg2d.COMP_ISCOPY) {
            return solidloops;
        }
     ...
     */
    protected native void initRaster(Object theArray, int offset, int bitoffset, int width, int height, int pixStr,
        int scanStr, IndexColorModel icm);
