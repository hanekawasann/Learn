...
public class ImageRepresentation extends ImageWatched implements ImageConsumer {
     ...
     */
    protected BufferedImage createImage(ColorModel cm, WritableRaster raster, boolean isRasterPremultiplied,
    ...
    void createBufferedImage() {
        ...
        if ((cmodel == ColorModel.getRGBdefault()) || (type == BufferedImage.TYPE_INT_RGB) ||
    ...
    private void convertToRGB() {
        ...
        if (cmodel instanceof IndexColorModel && biRaster instanceof ByteComponentRaster &&
    ...
    private native void setICMpixels(int x, int y, int w, int h, int[] lut, byte[] pix, int off, int scansize,
        IntegerComponentRaster ict);

    private native int setDiffICM(int x, int y, int w, int h, int[] lut, int transPix, int numLut, IndexColorModel icm,
        byte[] pix, int off, int scansize, ByteComponentRaster bct, int chanOff);
    ...
    public void setPixels(int x, int y, int w, int h, ColorModel model, byte pix[], int off, int scansize) {
        ...
        synchronized (this) {
            ...
            if (isSameCM && (cmodel != model) && (srcLUT != null) && (model instanceof IndexColorModel) &&
                (biRaster instanceof ByteComponentRaster)) {
                IndexColorModel icm = (IndexColorModel) model;
                ByteComponentRaster bct = (ByteComponentRaster) biRaster;
                int numlut = numSrcLUT;
                if (setDiffICM(x, y, w, h, srcLUT, srcLUTtransIndex, numSrcLUT, icm, pix, off, scansize, bct,
                    ...
                    if (numlut != numSrcLUT) {
                        ...
                        icm = new IndexColorModel(nbits, numSrcLUT, srcLUT, 0, hasAlpha, srcLUTtransIndex,
            ...
            if (isDefaultBI) {
                ...
                if (srcLUT != null && model instanceof IndexColorModel) {
                    ...
                    if (s_useNative) {
                        ...
                        setICMpixels(x, y, w, h, srcLUT, pix, off, scansize, iraster);
                        iraster.markDirty();
                    } else {
            ...
            } else if ((cmodel == model) && (biRaster instanceof ByteComponentRaster) &&
                (biRaster.getNumDataElements() == 1)) {
    ...
    private synchronized void checkConsumption() {
        if (isWatcherListEmpty() && numWaiters == 0 && ((availinfo & ImageObserver.ALLBITS) == 0)) {
            dispose();
        }
    }
    ...
    public boolean drawToBufImage(Graphics g, ToolkitImage img, int x, int y, int w, int h, Color bg,
        ImageObserver iw) {
    ...
    public boolean drawToBufImage(Graphics g, ToolkitImage img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1,
        int sx2, int sy2, Color bg, ImageObserver iw) {
    ...
    synchronized void abort() {
        ...
        availinfo &= ~(ImageObserver.SOMEBITS | ImageObserver.FRAMEBITS | ImageObserver.ALLBITS | ImageObserver.ERROR);
    }
