...
public abstract class FontConfiguration {
    ...
    public boolean fontFilesArePresent() {
        ...
        Boolean exists = (Boolean) java.security.AccessController.doPrivileged(new java.security.PrivilegedAction() {
            public Object run() {
                try {
    ...
    protected void getInstalledFallbackFonts(String javaLib) {
        ...
        if (fallbackDir.exists() && fallbackDir.isDirectory()) {
            ...
            for (int i = 0; i < numTTFs; i++) {
                installedFallbackFontFiles[i] = fallbackDir + File.separator + ttfs[i];
            }
    ...
    private short getInitELC() {
        ...
        if (elcIDs.containsKey(elc = initEncoding + "." + language + "." + country) ||
            elcIDs.containsKey(elc = initEncoding + "." + language) || elcIDs.containsKey(elc = initEncoding)) {
            ...
            for (int styleIndex = 0; styleIndex < NUM_STYLES; styleIndex++) {
                ...
                for (index = 0; index < coreScripts.length; index++) {
                    ...
                    if (preferLocaleFonts && localeMap != null && fontManager.usingAlternateFontforJALocales()) {
                        nameIDs[index] = remapLocaleMap(fontIndex, styleIndex, coreScripts[index], nameIDs[index]);
                    }
                    if (preferPropFonts) {
                ...
                for (int i = 0; i < fallbackScripts.length; i++) {
                    short id = getComponentFontID(fallbackScripts[i], fontIndex, styleIndex);
                    if (preferLocaleFonts && localeMap != null && fontManager.usingAlternateFontforJALocales()) {
    ...
    private short remapProportional(int fontIndex, short id) {
        if (preferPropFonts && table_proportionals.length != 0 && fontIndex != 2 &&         //"monospaced"
    ...
    protected static final String[] publicFontNames = { Font.SERIF, Font.SANS_SERIF, Font.MONOSPACED, Font.DIALOG,
        Font.DIALOG_INPUT };
    ...
    public static boolean isLogicalFontFaceNameLC(String fontName) {
        ...
        if (period >= 0) {
            ...
            return isLogicalFontFamilyName(familyName) && isLogicalFontStyleName(styleName);
        } else {
    ...
    private FontDescriptor[][][] fontDescriptors = new FontDescriptor[NUM_FONTS][NUM_STYLES][];
    ...
    private CharsetEncoder getFontCharsetEncoder(final String charsetName, String fontName) {
        ...
        if (!charsetName.startsWith("sun.awt.") && !charsetName.equals("default")) {
            ...
            Class fcc = (Class) AccessController.doPrivileged(new PrivilegedAction() {
                public Object run() {
                    try {
                        return Class.forName(charsetName, true, Thread.currentThread().getContextClassLoader());
            ...
            for (int styleIndex = 0; styleIndex < NUM_STYLES; styleIndex++) {
                ...
                */
                result[fontIndex * NUM_STYLES + styleIndex] = new CompositeFontDescriptor(faceName,
                    compCoreNum[fontIndex], componentFaceNames, componentFileNames, exclusionRanges,
    ...
    */
        ...
        if (numCoreFonts == -1) {
            ...
            for (int j = 0; j < fallback.length; j++) {
                if (fallback[j] != null) {
                    ...
                    short fileid = getComponentFileID(fontid);
                    componentFonts[i] = getComponentFontName(fontid);
                    filenamesMap.put(componentFonts[i], mapFileName(getComponentFileName(fileid)));
                    i++;
                }
            }
        }
    ...
    private static void sanityCheck() {
        ...
        for (int ii = 1; ii < table_filenames.length; ii++) {
            if (table_filenames[ii] == -1) {
                ...
                if (osName.contains("Windows")) {
                    System.err.println(
                        "\n Error: <filename." + getString(table_componentFontNameIDs[ii]) + "> entry is missing!!!");
                    errors++;
                } else {
            ...
            if (fid == 0) {
                ...
                for (int iii = 0; iii < NUM_FONTS; iii++) {
                    for (int iij = 0; iij < NUM_STYLES; iij++) {
                        ...
                        if (ffid == 0) {
                            System.err.println("\n Error: <" + getFontName(iii) + "." + getStyleName(iij) + "." +
                                getString(table_scriptIDs[ii]) + "> entry is missing!!!");
                            errors++;
                        }
                    }
                }
        ...
        if (errors != 0) {
            System.err.println(
                "!!THERE ARE " + errors + " ERROR(S) IN " + "THE FONTCONFIG FILE, PLEASE CHECK ITS CONTENT!!\n");
            System.exit(1);
        }
    }
    ...
    private static void dump() {
        ...
        for (int ii = 0; ii < table_scriptIDs.length; ii++) {
            ...
            if (fid >= 0) {
                System.out.println(
                    "  allfonts." + getString(table_scriptIDs[ii]) + "=" + getString(table_componentFontNameIDs[fid]));
            }
        }
        for (int ii = 0; ii < table_scriptIDs.length; ii++) {
            short fid = table_scriptFonts[ii];
            if (fid < 0) {
                ...
                for (int iii = 0; iii < NUM_FONTS; iii++) {
                    for (int iij = 0; iij < NUM_STYLES; iij++) {
                        ...
                        short ffid = table_scriptFonts[fid + jj];
                        System.out.println(
                            "  " + getFontName(iii) + "." + getStyleName(iij) + "." + getString(table_scriptIDs[ii]) +
        ...
        for (int ii = 0; ii < table_filenames.length; ii++) {
            if (table_filenames[ii] == -1) {
                ...
                System.out.println("  " + ii + " : " + getString(table_fontfileNameIDs[table_filenames[ii]]));
            }
        }
        System.out.println("\n----awtfontpaths---------");
        for (int ii = 0; ii < table_awtfontpaths.length; ii++) {
            System.out.println("  " + getString(table_scriptIDs[ii]) + " : " + getString(table_awtfontpaths[ii]));
        }
        System.out.println("\n----proportionals--------");
    ...
    protected static String getString(short stringID) {
        ...
        */
        ...
        if (stringCache[stringID] == null) {
            stringCache[stringID] = new String(table_stringTable, table_stringIDs[stringID],
    ...
    static class PropertiesHandler {
        ...
        private void initBinaryTable() {
            ...
            if (scriptAllfontsMotif.size() != 0 || scriptFontsMotif.size() != 0) {
                ...
                for (Entry<Short, Short> entry : scriptAllfontsMotif.entrySet()) {
                    table_scriptFontsMotif[entry.getKey().intValue()] = (short) entry.getValue();
                }
        ...
        private void parseProperty(String key, String value) {
            if (key.startsWith("filename.")) {
                ...
                proportionals.put(getID(componentFontNameIDs, key), getID(componentFontNameIDs, value));
            } else {
                ...
                pnids[nameID.intValue() * NUM_STYLES + styleID.intValue()] = getID(componentFontNameIDs, value);
                /*
