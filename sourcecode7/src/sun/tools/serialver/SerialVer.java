...
public class SerialVer extends Applet {
     ...
     * specified classpath.
     */
    static void initializeLoader(String cp) throws MalformedURLException, IOException {
        URL[] urls;
        StringTokenizer st = new StringTokenizer(cp, File.pathSeparator);
        int count = st.countTokens();
        urls = new URL[count];
        for (int i = 0; i < count; i++) {
            urls[i] = ParseUtil.fileToEncodedURL(new File(new File(st.nextToken()).getCanonicalPath()));
        }
    ...
    static String resolveClass(String classname) throws ClassNotFoundException {
        ...
        if (desc != null) {
            return "    static final long serialVersionUID = " + desc.getSerialVersionUID() + "L;";
        } else {
            return null;
        }
    }
        ...
        if (!show) {
            ...
            for (i = i; i < args.length; i++) {
                try {
                    ...
                    if (syntax != null) { System.out.println(args[i] + ":" + syntax); } else {
