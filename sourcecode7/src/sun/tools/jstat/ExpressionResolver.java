...
public class ExpressionResolver implements ExpressionEvaluator {
    ...
    public Object evaluate(Expression e) throws MonitorException {
        ...
        if (e instanceof Identifier) {
            ...
            if (m == null) {
                System.err.println("Warning: Unresolved Symbol: " + id.getName() + " substituted NaN");
                return new Literal(new Double(Double.NaN));
            }
            if (m.getVariability() == Variability.CONSTANT) {
                if (debug) {
                    System.out
                        .println("Converting constant " + id.getName() + " to literal with value " + m.getValue());
                }
        ...
        if (l != null && r != null) {
            if ((l instanceof Literal) && (r instanceof Literal)) {
                ...
                double result = e.getOperator().eval(ln.doubleValue(), rn.doubleValue());
                if (debug) {
                    System.out.println(
                        "Converting expression " + e + " (left = " + ln.doubleValue() + ")" + " (right = " +
                            rn.doubleValue() + ")" + " to literal value " + result);
                }
