...
public final class BinaryConstantPool implements Constants {
            ...
            case CONSTANT_INTERFACEMETHOD:
                try {
                    ...
                    for (MemberDefinition field = clazz.getFirstMatch(id);
                        ...
                        if ((constant_type == CONSTANT_FIELD) ? (field_type == type)
    ...
    public int indexObject(Object obj, Environment env) {
        if (indexHashObject == null) { createIndexHash(env); }
        Integer result = (Integer) indexHashObject.get(obj);
        if (result == null) {
            throw new IndexOutOfBoundsException(
                "Cannot find object " + obj + " of type " + obj.getClass() + " in constant pool");
        }
    ...
    public void write(DataOutputStream out, Environment env) throws IOException {
        ...
        if (MoreStuff != null) { length += MoreStuff.size(); }
