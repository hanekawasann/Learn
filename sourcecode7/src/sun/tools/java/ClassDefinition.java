...
public class ClassDefinition implements Constants {
     ...
     * Constructor
     */
    protected ClassDefinition(Object source, long where, ClassDeclaration declaration, int modifiers,
        IdentifierToken superClass, IdentifierToken interfaces[]) {
        this.source = source;
        this.where = where;
    ...
    public static boolean containsDeprecated(String documentation) {
        ...
        doScan:
        for (int scan = 0; (scan = documentation.indexOf(paraDeprecated, scan)) >= 0; scan += paraDeprecated.length()) {
    ...
    public boolean permitInlinedAccess(Environment env, MemberDefinition f) throws ClassNotFound {
        return (env.opt() && (f.clazz.getClassDeclaration().equals(declaration))) ||
    ...
    public boolean protectedAccess(Environment env, MemberDefinition f, Type accessorType) throws ClassNotFound {
        ...
        return
            ...
            f.isStatic() || // allow array.clone()
                (accessorType.isType(TC_ARRAY) && (f.getName() == idClone) &&
                    (f.getType().getArgumentTypes().length == 0)) || // <accessorType> is a subtype of the current class
                (accessorType.isType(TC_CLASS) &&
                    env.getClassDefinition(accessorType.getClassName()).subClassOf(env, getClassDeclaration())) ||
                // we are accessing the field from a friendly class (same package)
                (getName().getQualifier().equals(f.getClassDeclaration().getName().getQualifier()));
    }
     ...
     */
    private MemberDefinition getVariable0(Environment env, Identifier nm, ClassDefinition source, boolean showPrivate,
        boolean showPackage) throws AmbiguousMember, ClassNotFound {
     ...
     */
    private MemberDefinition matchMethod(Environment env, ClassDefinition accessor, Identifier methodName,
        Type[] argumentTypes, boolean isAnonConstCall, Identifier accessPackage) throws AmbiguousMember, ClassNotFound {
        ...
        while (methods.hasNext()) {
            ...
            if (accessor != null) {
                ...
                if (method.isPrivate() ||
                    (method.isPackagePrivate() && accessPackage != null && !inSamePackage(accessPackage))) {
    ...
    public MemberDefinition matchMethod(Environment env, ClassDefinition accessor, Identifier methodName,
        Type[] argumentTypes) throws AmbiguousMember, ClassNotFound {
     ...
     */
    private void collectOneClass(Environment env, ClassDeclaration parent, MethodSet myMethods, MethodSet allMethods,
            ...
            while (methods.hasNext()) {
                ...
                if (method.isPackagePrivate() && !inSamePackage(method.getClassDeclaration())) {
                    ...
                    if (override != null && this instanceof sun.tools.javac.SourceClass) {
                        ...
                        env.error(method.getWhere(), "warn.no.override.access", override,
                            override.getClassDeclaration(), method.getClassDeclaration());
                    }
                ...
                if (override != null) {
                    ...
                    if (formerMethod == null) {
                        ...
                        env.error(this.where, "nontrivial.meet", method, formerMethod.getClassDefinition(),
    ...
    private boolean diagnoseMismatch(Environment env, Identifier nm, Type argTypes[], int start, int haveMatch[],
        Type margType[]) throws ClassNotFound {
        // look in the current class
        ...
        for (f = getFirstMatch(nm); f != null; f = f.getNextMatch()) {
            ...
            if (fArgTypes.length == argTypes.length) {
                ...
                for (int i = start; i < argTypes.length; i++) {
                    ...
                    if (margType[i] == null) { margType[i] = ft; } else if (margType[i] != ft) { haveMatch[i] |= 1; }
                }
            }
        }
        ...
        ClassDeclaration sup = getSuperClass();
        if (sup != null) {
            if (sup.getClassDefinition(env).diagnoseMismatch(env, nm, argTypes, start, haveMatch, margType)) {
                haveOne = true;
            }
        }
        return haveOne;
