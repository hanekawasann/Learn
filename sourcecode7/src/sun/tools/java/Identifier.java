...
public final class Identifier implements Constants {
    ...
    public static Identifier lookupInner(Identifier c, Identifier n) {
        Identifier id;
        if (c.isInner()) {
            if (c.name.charAt(c.name.length() - 1) == INNERCLASS_PREFIX) { id = lookup(c.name + n); } else {
                id = lookup(c, n);
            }
        } else {
    ...
    public boolean isQualified() {
        if (value == null) {
            int idot = ipos;
            if (idot <= 0) { idot = name.length(); } else {
    ...
    public Identifier getTail() {
        ...
        if (id == this) { return idNull; } else { return Identifier.lookup(name.substring(id.name.length() + 1)); }
    }
