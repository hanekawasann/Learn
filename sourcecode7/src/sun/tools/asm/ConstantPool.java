...
public final class ConstantPool implements RuntimeConstants {
    ...
    private static String sortKey(ConstantPoolData f) {
        if (f instanceof NumberConstantData) {
            ...
            int key = 3;
            if (num instanceof Integer) { key = 0; } else if (num instanceof Float) {
                key = 1;
        ...
        if (f instanceof ClassConstantData) { return ((ClassConstantData) f).name; }
    ...
    private static void xsort(ConstantPoolData ff[], String ss[], int left, int right) {
        ...
        int r = right;
        while (l < r) {
            while (l <= right && ss[l].compareTo(pivot) <= 0) { l++; }
            while (r >= left && ss[r].compareTo(pivot) > 0) { r--; }
            if (l < r) {
