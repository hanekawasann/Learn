...
public final class Assembler implements Constants {
    ...
    static int[] arrayModifiers = { M_PUBLIC, M_PRIVATE, M_PROTECTED, M_ABSTRACT, M_FINAL, M_INTERFACE };
    static int[] arrayModifiersOpc = { PUBLIC, PRIVATE, PROTECTED, ABSTRACT, FINAL, INTERFACE };
    //end JCOV
    ...
    void balance(Label lbl, int depth) {
        for (Instruction inst = lbl; inst != null; inst = inst.next) {
            ...
            switch (inst.opc) {
                ...
                case opc_fstore:
                case opc_astore: {
                    int v = ((inst.value instanceof Number) ? ((Number) inst.value).intValue()
     ...
     */
    void writeExceptions(Environment env, DataOutputStream out, ConstantPool tab, Instruction first, Instruction last)
        throws IOException {
     ...
     */
    public void writeCoverageTable(Environment env, ClassDefinition c, DataOutputStream out, ConstantPool tab,
        long whereField) throws IOException {
