...
public class XMBeanAttributes extends XTable {
    ...
    private final static String[] columnNames = { Resources.getText("Name"), Resources.getText("Value") };
    ...
    public synchronized Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
        ...
        if (row >= getRowCount()) { return null; } else { return super.prepareRenderer(renderer, row, column); }
    }
    ...
    void updateRowHeight(Object obj, int row) {
        ...
        if (obj instanceof ZoomedCell) {
            cell = (ZoomedCell) obj;
            if (cell.isInited()) { setRowHeight(row, cell.getHeight()); } else if (rowMinHeight != -1) {
                setRowHeight(row, rowMinHeight);
            }
        } else if (rowMinHeight != -1) { setRowHeight(row, rowMinHeight); }
    }
    ...
    public final boolean editCellAt(final int row, final int column, EventObject e) {
        if (LOGGER.isLoggable(Level.FINER)) {
            LOGGER.finer("editCellAt(row=" + row + ", col=" + column + ", e=" + e + ")");
        }
        if (JConsole.isDebug()) {
        ...
        if (retVal) {
            final TableCellEditor tableCellEditor = getColumnModel().getColumn(column).getCellEditor();
    ...
    public void setValueAt(Object value, int row, int column) {
        if (!isCellError(row, column) && isColumnEditable(column) && isWritable(row) &&
            Utils.isEditableType(getClassName(row))) {
            if (JConsole.isDebug()) {
                System.err
                    .println("validating [row=" + row + ", column=" + column + "]: " + getValueName(row) + "=" + value);
            }
        ...
        if (isColumnEditable(column)) {
            ...
            if (value != null) {
                ...
                if (isAttributeViewable(row, VALUE_COLUMN)) {
                    tip = Resources.getText("Double click to expand/collapse") + ". " + tip;
                }
            }
    ...
    public synchronized boolean isCellError(int row, int col) {
        return (isColumnEditable(col) && (unavailableAttributes.containsKey(getValueName(row))));
    }
    ...
    public synchronized boolean isAttributeViewable(int row, int col) {
        ...
        if (col == VALUE_COLUMN) {
            Object obj = getModel().getValueAt(row, col);
            if (obj instanceof ZoomedCell) { isViewable = true; }
        }
    ...
    private Runnable doLoadAttributes(final XMBean mbean, MBeanInfo infoOrNull) throws JMException, IOException {
        ...
        try {
            ...
            if (JConsole.isDebug()) {
                System.err.println(
                    "Error calling getAttributes() on MBean \"" + mbean.getObjectName() + "\". JConsole will " +
                        "try to get them individually calling " + "getAttribute() instead. Exception:");
            ...
            for (int i = 0; i < attrsInfo.length; i++) {
                ...
                try {
                    ...
                    Object value = mbean.getMBeanServerConnection().
        ...
        try {
            ...
            for (int i = 0; i < att_length; i++) {
                ...
                } else { attrs.put(attribute.getName(), attribute.getValue()); }
            ...
            if (att_length < attrsInfo.length) {
                for (int i = 0; i < attrsInfo.length; i++) {
                    ...
                    if (!attrs.containsKey(attributeInfo.getName()) && !viewableAttrs.containsKey(attributeInfo.
                        getName()) && !unavailableAttrs.containsKey(attributeInfo.
                        getName())) {
            ...
            for (int i = 0; i < attrsInfo.length; i++) {
                MBeanAttributeInfo attributeInfo = attrsInfo[i];
                if (attributeInfo.isReadable()) {
                    unavailableAttrs.put(attributeInfo.getName(), Utils.getActualException(e).
                        toString());
                }
            }
        }
    ...
    ZoomedCell updateZoomedCell(int row, int col) {
        ...
        if (obj instanceof ZoomedCell) {
            ...
            if (!cell.isInited()) {
                ...
                String attributeName = (String) getModel().getValueAt(row, NAME_COLUMN);
        ...
        synchronized (viewersCache) {
            ...
            if (viewers.containsKey(attribute)) {
                ...
                if (cell.isMaximized() && cell.getType() != XDataViewer.NUMERIC) {
                    ...
                    Component comp = mbeansTab.getDataViewer()
                        .createAttributeViewer(value, mbean, attribute, XMBeanAttributes.this);
    ...
    protected void addTableData(DefaultTableModel tableModel, XMBean mbean, MBeanAttributeInfo[] attributesInfo,
        HashMap<String, Object> attributes, HashMap<String, Object> unavailableAttributes,
        ...
        for (int i = 0; i < attributesInfo.length; i++) {
            ...
            if (rowData[0] != null) {
                ...
                if (str.length() > col1Width) { col1Width = str.length(); }
            }
            if (rowData[1] != null) {
                str = rowData[1].toString();
                if (str.length() > col2Width) { col2Width = str.length(); }
            }
        }
        ...
        public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row,
            ...
            if (column == VALUE_COLUMN) {
                ...
                if (obj instanceof ZoomedCell) {
                    ...
                    if (isCellError(row, column) || !isWritable(row) || !Utils.isEditableType(getClassName(row))) {
    ...
    class ZoomedCell {
        ...
        int getHeight() {
            if (minimized) { return minHeight; } else {
        ...
        public String toString() {
            ...
            if (value.getClass().isArray()) {
                String name = Utils.getArrayClassName(value.getClass().getName());
                int length = Array.getLength(value);
                return name + "[" + length + "]";
            }

            if (value instanceof CompositeData || value instanceof TabularData) { return value.getClass().getName(); }

            return value.toString();
        }

        TableCellRenderer getRenderer() {
            if (minimized) { return minRenderer; } else { return maxRenderer; }
        }
    ...
    class AttributesListener implements TableModelListener {
        ...
        public void tableChanged(final TableModelEvent e) {
            ...
            if (isColumnEditable(e.getColumn())) {
                ...
                if (LOGGER.isLoggable(Level.FINER)) {
                    LOGGER.finer(
                        "tableChanged: firstRow=" + e.getFirstRow() + ", lastRow=" + e.getLastRow() + ", column=" +
                            e.getColumn() + ", value=" + tableValue);
                }
                // if it's a String, try construct new value
                // using the defined type.
                if (tableValue instanceof String) {
                    try {
                        tableValue = Utils.createObjectFromString(getClassName(e.getFirstRow()), // type
                            (String) tableValue);// value
                    } catch (Throwable ex) {
                        popupAndLog(ex, "tableChanged", "Problem setting attribute");
                    }
                }
                final String attributeName = getValueName(e.getFirstRow());
                final Attribute attribute = new Attribute(attributeName, tableValue);
        ...
        private void setAttribute(final Attribute attribute, final String method) {
            final SwingWorker<Void, Void> setAttribute = new SwingWorker<Void, Void>() {
                @Override
                protected Void doInBackground() throws Exception {
                    try {
                        if (JConsole.isDebug()) {
                            System.err
                                .println("setAttribute(" + attribute.getName() + "=" + attribute.getValue() + ")");
                        }
                ...
                protected void done() {
                    try {
                        ...
                        if (JConsole.isDebug()) { x.printStackTrace(); }
                    }
                    refreshAttributes(false);
                }
