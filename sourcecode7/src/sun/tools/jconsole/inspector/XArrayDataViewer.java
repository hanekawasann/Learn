...
class XArrayDataViewer {
    ...
    public static Component loadArray(Object value) {
        ...
        if (isViewableValue(value)) {
            ...
            Color oddRowColor = new Color(red < 20 ? red + 20 : red - 20, green < 20 ? green + 20 : green - 20,
                blue < 20 ? blue + 20 : blue - 20);
            String oddRowColorStr = "rgb(" + oddRowColor.getRed() + "," + oddRowColor.getGreen() + "," +
                oddRowColor.getBlue() + ")";
            ...
            for (int i = 0; i < arr.length; i++) {
                if (i % 2 == 0) {
                    sb.append("<tr style=\"background-color: " + evenRowColorStr + "\"><td><pre>" +
                        (arr[i] == null ? arr[i] : htmlize(arr[i].toString())) + "</pre></td></tr>");
                } else {
                    sb.append("<tr style=\"background-color: " + oddRowColorStr + "\"><td><pre>" +
                        (arr[i] == null ? arr[i] : htmlize(arr[i].toString())) + "</pre></td></tr>");
                }
            }
            if (arr.length == 0) {
