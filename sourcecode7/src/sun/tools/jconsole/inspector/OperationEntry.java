    ...
    private void setPanel(boolean isCallable, JButton button, XOperations xoperations) {
        try {
            ...
            for (int i = 0; i < params.length; i++) {
                ...
                String defaultTextValue = Utils.getDefaultValue(params[i].getType());
                int fieldWidth = defaultTextValue.length();
                if (fieldWidth > 15) { fieldWidth = 15; } else if (fieldWidth < 10) { fieldWidth = 10; }

                add(inputs[i] = new XTextField(Utils.getReadableClassName(defaultTextValue),
                    Utils.getClass(params[i].getType()), fieldWidth, isCallable, button, xoperations));
                inputs[i].setHorizontalAlignment(SwingConstants.CENTER);

                if (i < params.length - 1) { add(new JLabel(",", JLabel.CENTER)); }
            }
            add(new JLabel(")", JLabel.CENTER));
            validate();
            doLayout();
