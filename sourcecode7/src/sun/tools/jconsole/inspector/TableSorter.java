...
public class TableSorter extends DefaultTableModel implements MouseListener {
    ...
    private void removeListeners() {
        for (TableModelListener tnl : evtListenerList) { super.removeTableModelListener(tnl); }
    }

    private void restoreListeners() {
        for (TableModelListener tnl : evtListenerList) { super.addTableModelListener(tnl); }
    }
    ...
    private void sort(int column, boolean isAscending) {
        final XMBeanAttributes attrs = (tableView instanceof XMBeanAttributes) ? (XMBeanAttributes) tableView : null;
        ...
        //
        if (attrs != null && attrs.isEditing()) { attrs.cancelCellEditing(); }
    ...
    private boolean compareS(Object s1, Object s2, boolean isAscending) {
        if (isAscending) { return (compare(s1, s2) > 0); } else { return (compare(s1, s2) < 0); }
    }
    ...
    private void quickSort(int lo0, int hi0, int key, boolean isAscending) {
        ...
        if (hi0 > lo0) {
            ...
            while (lo <= hi) {
                 ...
                 * from the left Index.
                 */
                while ((lo < hi0) && (compareS(mid, getValueAt(lo, key), isAscending))) { ++lo; }

                /* find an element that is smaller than or equal to
                 * the partition element starting from the right Index.
                 */
                while ((hi > lo0) && (compareG(mid, getValueAt(hi, key), isAscending))) { --hi; }
             ...
             */
            if (lo0 < hi) { quickSort(lo0, hi, key, isAscending); }

            /* If the left index has not reached the right
             * side of array
             * must now sort the right partition.
             */
            if (lo <= hi0) { quickSort(lo, hi0, key, isAscending); }
        }
    }
