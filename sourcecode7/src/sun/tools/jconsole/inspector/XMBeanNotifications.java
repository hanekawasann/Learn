...
public class XMBeanNotifications extends JTable implements NotificationListener {
    ...
    private final static String[] columnNames = { Resources.getText("TimeStamp"), Resources.getText("Type"),
        Resources.getText("UserData"), Resources.getText("SeqNum"), Resources.getText("Message"),
        Resources.getText("Event"), Resources.getText("Source") };
    ...
    public final static String NOTIFICATION_RECEIVED_EVENT = "jconsole.xnotification.received";
    ...
    public synchronized TableCellRenderer getCellRenderer(int row, int column) {
        ...
        if (cell != null && cell.isInited()) {
            ...
            renderer = (DefaultTableCellRenderer) super.getCellRenderer(row, column);
        }

        if (cell != null) {
            toolTip = Resources.getText("Double click to expand/collapse") + ". " + cell.toString();
        } else {
            Object val = ((DefaultTableModel) getModel()).getValueAt(row, column);
            if (val != null) {
    ...
    public long getReceivedNotifications(XMBean mbean) {
        XMBeanNotificationsListener listener = listeners.get(mbean.getObjectName());
        if (listener == null) {
            return 0;
        } else {
        ...
        if (!subscribed) {
            try {
                mbean.getMBeanServerConnection()
                    .addNotificationListener(MBeanServerDelegate.DELEGATE_NAME, this, null, null);
                subscribed = true;
            } catch (Exception e) {
                if (JConsole.isDebug()) {
    ...
    // Call on EDT
    void fireNotificationReceived(XMBeanNotificationsListener listener, XMBean mbean, DefaultMutableTreeNode node,
        Object[] rowData, long received) {
        if (enabled) {
        ...
        Notification notif = new Notification(NOTIFICATION_RECEIVED_EVENT, this, 0);
    ...
    class UserDataCell {
        ...
        public String toString() {
            ...
            if (userData.getClass().isArray()) {
                String name = Utils.getArrayClassName(userData.getClass().getName());
                int length = Array.getLength(userData);
                return name + "[" + length + "]";
            }

            if (userData instanceof CompositeData || userData instanceof TabularData) {
    ...
    class XMBeanNotificationsListener implements NotificationListener {
        ...
        public XMBeanNotificationsListener(XMBeanNotifications notifications, XMBean xmbean,
            DefaultMutableTreeNode node, String[] columnNames) {
        ...
        public synchronized void unregister() {
            try {
                xmbean.getMBeanServerConnection().removeNotificationListener(xmbean.getObjectName(), this, null, null);
            } catch (Exception e) {
                if (JConsole.isDebug()) {
        ...
        public synchronized void register(DefaultMutableTreeNode node) {
            ...
            this.node = node;
            try {
                xmbean.getMBeanServerConnection().addNotificationListener(xmbean.getObjectName(), this, null, null);
                unregistered = false;
            } catch (Exception e) {
                if (JConsole.isDebug()) {
                ...
                public void run() {
                    synchronized (XMBeanNotificationsListener.this) {
                        try {
                            ...
                            Object[] rowData = { time, n.getType(), (cell == null ? userData : cell),
                                n.getSequenceNumber(), n.getMessage(), n, n.getSource() };
                            received++;
                            data.add(0, rowData);

                            notifications
                                .fireNotificationReceived(XMBeanNotificationsListener.this, xmbean, node, rowData,
                                    received);
                        } catch (Exception e) {
                            if (JConsole.isDebug()) {
