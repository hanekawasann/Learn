...
class MemoryTab extends Tab implements ActionListener, ItemListener {
    ...
    */
    ...
    public MemoryTab(VMPanel vmPanel) {
        ...
        controlPanel.add(new LabeledComponent(getText("Chart:"), getMnemonicInt("Chart:"), plotterChoice));

        // Range control
        timeComboBox = new TimeComboBox();
        controlPanel.add(new LabeledComponent(getText("Time Range:"), getMnemonicInt("Time Range:"), timeComboBox));
        ...
        bottomPanel
            .setBorder(new CompoundBorder(new TitledBorder(getText("Details")), new EmptyBorder(10, 10, 10, 10)));
    ...
    private void createPlotters() throws IOException {
        ...
        setAccessibleName(heapPlotter, getText("MemoryTab.heapPlotter.accessibleName"));
        setAccessibleName(nonHeapPlotter, getText("MemoryTab.nonHeapPlotter.accessibleName"));
        ...
        for (ObjectName objectName : objectNames) {
            ...
            if (type.equals("MemoryPool")) {
                String name = getText("MemoryPoolLabel", objectName.getKeyProperty("name"));
                // Heap or non-heap?
    ...
    private String formatDetails() {
        ...
        try {
            ...
            for (GarbageCollectorMXBean garbageCollectorMBean : garbageCollectors) {
                ...
                String str = getText("GC time details", justify(formatTime(gcTime), 14), gcName,
    ...
    private class PoolPlotter extends Plotter {
        ...
        public PoolPlotter(ObjectName objectName, String name, boolean isHeap) {
            ...
            setAccessibleName(this, getText("MemoryTab.poolPlotter.accessibleName", name));
        }
        ...
        protected class AccessiblePoolChart extends AccessibleJPanel {
            public String getAccessibleName() {
                ...
                for (PoolPlotter poolPlotter : poolPlotters) {
                    ...
                    keyValueList += getText("Plotter.accessibleName.keyAndValue", poolPlotter.toString(), value);
                    if (poolPlotter.threshold > 0L) {
                        ...
                        if (poolPlotter.value > poolPlotter.threshold) {
                            keyValueList += getText("MemoryTab.poolChart.aboveThreshold", threshold);
                        } else {
                            keyValueList += getText("MemoryTab.poolChart.belowThreshold", threshold);
                        }
                    }
                }
    ...
    private static class MemoryOverviewPanel extends OverviewPanel {
        ...
        private void updateMemoryInfo(long used, long committed, long max) {
            getInfoLabel().setText(getText(infoLabelFormat, formatBytes(used, true), formatBytes(committed, true),
