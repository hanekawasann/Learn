...
public class SourceClass extends ClassDefinition {
     ...
     * Constructor
     */
    public SourceClass(Environment env, long where, ClassDeclaration declaration, String documentation, int modifiers,
        IdentifierToken superClass, IdentifierToken interfaces[], SourceClass outerClass, Identifier localName) {
        ...
        if (localName != null) { setLocalName(localName); }
        ...
        if (thisName != idNull) {
            ...
            for (ClassDefinition scope = outerClass; scope != null; scope = scope.getOuterClass()) {
                Identifier outerName = scope.getLocalName();
                if (thisName.equals(outerName)) { env.error(where, "inner.redefined", thisName); }
            }
        }
    }
    ...
    public void addMember(Environment env, MemberDefinition f) {
        ...
        if (f.isMethod()) {
            ...
            if (f.getClassDefinition().isInterface() &&
                (f.isStatic() || f.isSynchronized() || f.isNative() || f.isFinal() || f.isPrivate() ||
                    f.isProtected())) {
                env.error(f.getWhere(), "intf.modifier.method", f);
                f.subModifiers(M_STATIC | M_SYNCHRONIZED | M_NATIVE | M_FINAL | M_PRIVATE);
            }
            if (f.isTransient()) {
            ...
            if (arguments != null) {
                ...
                for (int i = 0; i < argTypes.length; i++) {
                    ...
                    if (argTypes[i].isType(TC_VOID) || argTypes[i].isVoidArray()) {
                        env.error(where, "void.argument", arg);
                    }
                }
            }
        } else if (f.isInnerClass()) {
            if (f.isVolatile() || f.isTransient() || f.isNative() || f.isSynchronized()) {
                env.error(f.getWhere(), "inner.modifier", f);
                f.subModifiers(M_VOLATILE | M_TRANSIENT | M_NATIVE | M_SYNCHRONIZED);
            }
            // same check as for fields, below:
            if (f.getClassDefinition().isInterface() && (f.isPrivate() || f.isProtected())) {
        ...
        if (!f.isInitializer()) {
            for (MemberDefinition f2 = getFirstMatch(f.getName()); f2 != null; f2 = f2.getNextMatch()) {
                if (f.isVariable() && f2.isVariable()) {
                    env.error(f.getWhere(), "var.multidef", f, f2);
                    return;
                } else if (f.isInnerClass() && f2.isInnerClass() && !f.getInnerClass().isLocal() &&
    ...
    public void check(Environment env) throws ClassNotFound {
        ...
        if (isInsideLocal()) {
            ...
            if (tracing) { env.dtEvent("SourceClass.check: INSIDE LOCAL " + getOuterClass().getName()); }
            getOuterClass().check(env);
        } else {
            if (isInnerClass()) {
                if (tracing) { env.dtEvent("SourceClass.check: INNER CLASS " + getOuterClass().getName()); }
                // Make sure the outer is checked first.
                ((SourceClass) getOuterClass()).maybeCheck(env);
            }
            Vset vset = new Vset();
            Context ctx = null;
            if (tracing) { env.dtEvent("SourceClass.check: CHECK INTERNAL " + getName()); }
    ...
    private Vset checkInternal(Environment env, Context ctx, Vset vset) throws ClassNotFound {
        ...
        for (int i = 0; i < interfaces.length; i++) {
            ...
            if (interfaceIds != null && interfaceIds.length == interfaces.length) {
        ...
        if (!isInnerClass() && !isInsideLocal()) {
            ...
            try {
                ...
                if (ID != getName()) { env.error(where, "class.multidef.import", simpleName, ID); }
    ...
    private void checkSupers(Environment env) throws ClassNotFound {
        ...
        for (int i = 0; i < interfaces.length; i++) {
            ...
            ClassDeclaration newInterfaces[] = new ClassDeclaration[interfaces.length - 1];
            System.arraycopy(interfaces, 0, newInterfaces, 0, i);
            System.arraycopy(interfaces, i + 1, newInterfaces, i, newInterfaces.length - i);
     ...
     * rest of the members.
     */
    private Vset checkMembers(Environment env, Context ctx, Vset vset) throws ClassNotFound {
        ...
        if (!isInterface() && !isAbstract() && mustBeAbstract(env)) {
            ...
            while (iter.hasNext()) {
                ...
                env.error(where, "abstract.class.cannot.override", getClassDeclaration(), method,
            ...
            while (iter.hasNext()) {
                ...
                if (method.isAbstract()) {
                    env.error(where, "abstract.class", getClassDeclaration(), method,
                        method.getDefiningClassDeclaration());
                }
            }
        }
    ...
    private void checkBlankFinals(Environment env, Context ctxInit, Vset vset, boolean isStatic) {
        for (int i = 0; i < ctxInit.getVarNumber(); i++) {
            if (!vset.testVar(i)) {
                MemberDefinition ff = ctxInit.getElement(i);
                if (ff != null && ff.isBlankFinal() && ff.isStatic() == isStatic && ff.getClassDefinition() == this) {
                    env.error(ff.getWhere(), "final.var.not.initialized", ff.getName());
                }
            }
        }
    }
    ...
    protected void basicCheck(Environment env) throws ClassNotFound {
        ...
        if (!isInterface()) {
            ...
            if (!hasConstructor()) {
                ...
                int accessModifiers = getModifiers() & (isInnerClass() ? (M_PUBLIC | M_PROTECTED) : M_PUBLIC);
                env.makeMemberDefinition(env, getWhere(), this, null, accessModifiers, t, idInit, null, null, code);
            }
        }
    ...
    protected void addMirandaMethods(Environment env, Iterator mirandas) {
        ...
        while (mirandas.hasNext()) {
            MemberDefinition method = (MemberDefinition) mirandas.next();
    ...
    public void resolveTypeStructure(Environment env) {
        ...
        if (tracing) { env.dtEnter("SourceClass.resolveTypeStructure: " + getName()); }
        ...
        if (oc != null && oc instanceof SourceClass && !((SourceClass) oc).resolved) {
        ...
        if (resolved || resolving) {
            if (tracing) { env.dtExit("SourceClass.resolveTypeStructure: OK " + getName()); }
            return;
        }
        ...
        if (tracing) { env.dtEvent("SourceClass.resolveTypeStructure: RESOLVING " + getName()); }
        ...
        for (MemberDefinition f = getFirstMember(); f != null; f = f.getNextMember()) {
            if (f instanceof SourceMember) { ((SourceMember) f).resolveTypeStructure(env); }
        }
        ...
        if (tracing) { env.dtExit("SourceClass.resolveTypeStructure: " + getName()); }
    }

    protected void resolveSupers(Environment env) {
        if (tracing) { env.dtEnter("SourceClass.resolveSupers: " + this); }
        ...
        if (superClassId != null && superClass == null) {
            ...
            if (superClass == getClassDeclaration() && getName().equals(idJavaLangObject)) {
                superClass = null;
                superClassId = null;
            }
        }
        ...
        if (tracing) { env.dtExit("SourceClass.resolveSupers: " + this); }
    }

    private ClassDeclaration resolveSuper(Environment env, IdentifierToken t) {
        Identifier name = t.getName();
        if (tracing) { env.dtEnter("SourceClass.resolveSuper: " + name); }
        if (isInnerClass()) { name = outerClass.resolveName(env, name); } else { name = env.resolveName(name); }
     ...
     */
    public Vset checkLocalClass(Environment env, Context ctx, Vset vset, ClassDefinition sup, Expression args[],
     ...
     */
    public Vset checkInsideClass(Environment env, Context ctx, Vset vset) throws ClassNotFound {
     ...
     */
    private void resolveAnonymousStructure(Environment env, ClassDefinition sup, Expression args[], Type argTypes[])
        throws ClassNotFound {

        if (tracing) { env.dtEvent("SourceClass.resolveAnonymousStructure: " + this + ", super " + sup); }
        ...
        *---------------------*/
        ...
        for (int i = 0; i < names.length; i++) {
            names[i] = new IdentifierToken(args[i].getWhere(), Identifier.lookup("$" + i));
        }
        ...
        if (outerArg == 0) {
            ...
            superExp = new SuperExpression(where, new IdentifierExpression(names[0]));
        }
        Expression superCall = new MethodExpression(where, superExp, idInit, superArgs);
        ...
        env.makeMemberDefinition(env, where, this, null, mod, t, idInit, names, null, code);
    }
    ...
    private static int classModifierBits[] = { ACC_PUBLIC, ACC_PRIVATE, ACC_PROTECTED, ACC_STATIC, ACC_FINAL,
        ACC_INTERFACE, ACC_ABSTRACT, ACC_SUPER, M_ANONYMOUS, M_LOCAL, M_STRICTFP, ACC_STRICT };

    private static String classModifierNames[] = { "PUBLIC", "PRIVATE", "PROTECTED", "STATIC", "FINAL", "INTERFACE",
        "ABSTRACT", "SUPER", "ANONYMOUS", "LOCAL", "STRICTFP", "STRICT" };
    ...
    private MemberDefinition getAccessMember(Environment env, Context ctx, MemberDefinition field, boolean isUpdate,
        boolean isSuper) {
        ...
        if (field.isConstructor()) {
            ...
            if (dummyType == null) {
                // Create dummy class.
                IdentifierToken sup = new IdentifierToken(0, idJavaLangObject);
                ...
                ClassDefinition dummyClass = toplevelEnv
                    .makeClassDefinition(toplevelEnv, 0, t, null, mod, sup, interfaces, outerMostClass);
                // Check the class.
                ...
                Type argTypesX[] = {};
                try {
                    ClassDefinition supcls = toplevelEnv.getClassDefinition(idJavaLangObject);
                    dummyClass.checkLocalClass(toplevelEnv, null, new Vset(), supcls, argsX, argTypesX);
                } catch (ClassNotFound ee) {}
                ;
                // Get class type.
        ...
        if (isStatic) {
            ...
            if (!isMethod) {
                ...
                if (field.isConstructor()) {
                    ...
                    MemberDefinition outerThisArg = ((SourceMember) field).getOuterThisArg();
        ...
        SourceMember newf = (SourceMember) env
            .makeMemberDefinition(env, where, this, null, mod, t, anm, names, field.getExceptionIds(), code);
    ...
    SourceClass findLookupContext() {
        ...
        for (MemberDefinition f = getFirstMember(); f != null; f = f.getNextMember()) {
            if (f.isInnerClass()) {
                SourceClass lc = ((SourceClass) f.getInnerClass()).findLookupContext();
                if (lc != null) {
                    return lc;
                }
            }
    ...
    public MemberDefinition getClassLiteralLookup(long fwhere) {
        ...
        if (isInterface()) {
            ...
            if (c == null) {
                ...
                c = (SourceClass) toplevelEnv
                    .makeClassDefinition(toplevelEnv, fwhere, t, null, mod, sup, interfaces, this);
            }
        }
        ...
        Statement handler = new CatchStatement(w, new TypeExpression(w, ctyp), new IdentifierToken(idForName),
        ...
        lookup = toplevelEnv
            .makeMemberDefinition(toplevelEnv, w, c, null, M_STATIC | M_SYNTHETIC, mtype, idDClass, args, null, body);
     ...
     * Compile this class
     */
    public void compile(OutputStream out) throws InterruptedException, IOException {
    ...
    protected void compileClass(Environment env, OutputStream out) throws IOException, ClassNotFound {
        Vector variables = new Vector();
        Vector methods = new Vector();
        Vector innerClasses = new Vector();
        CompilerMember init = new CompilerMember(
            new MemberDefinition(getWhere(), this, M_STATIC, Type.tMethod(Type.tVoid), idClassInit, null, null),
            new Assembler());
        ...
        for (int i = ncsize; --i >= 0; ) { innerClasses.addElement(innerClasses.elementAt(i)); }
        for (int i = ncsize; --i >= 0; ) { innerClasses.removeElementAt(i); }
        ...
        for (SourceMember field = (SourceMember) getFirstMember();
            field != null; field = (SourceMember) field.getNextMember()) {
            ...
            try {
                if (field.isMethod()) {
                    haveExceptions |= (field.getExceptions(env).length > 0);
                    ...
                    if (field.isInitializer()) {
                        ...
                        CompilerMember f = new CompilerMember(field, new Assembler());
                    ...
                    haveConstantValue |= (field.getInitialValue() != null);
                }
            } catch (CompilerError ee) {
                ee.printStackTrace();
                env.error(field, 0, "generic", field.getClassDeclaration() + ":" + field + "@" + ee.toString(), null,
                    null);
            }
        }
        if (!init.asm.empty()) {
        ...
        CompilerMember[] ordered_methods = new CompilerMember[methods.size()];
        methods.copyInto(ordered_methods);
        java.util.Arrays.sort(ordered_methods);
        for (int i = 0; i < methods.size(); i++) { methods.setElementAt(ordered_methods[i], i); }
        ...
        for (Enumeration e = innerClasses.elements(); e.hasMoreElements(); ) {
        ...
        if (env.dumpModifiers()) {
            ...
            Identifier nm = Identifier.lookup(cn.getQualifier(), cn.getFlatName());
            System.out.println();
        ...
        for (Enumeration e = variables.elements(); e.hasMoreElements(); ) {
            ...
            if (val != null) {
                ...
                data.writeInt(2);
                data.writeShort(
                    tab.index((val instanceof String) ? new StringExpression(f.field.getWhere(), (String) val) : val));
            }
            if (dep) {
        ...
        for (Enumeration e = methods.elements(); e.hasMoreElements(); ) {
            ...
            if (!f.asm.empty()) {
                ...
                //JCOV
                if (env.coverage()) {
                    f.asm.writeCoverageTable(env, (ClassDefinition) this, new DataOutputStream(attbuf), tab,
                        f.field.getWhere());
        ...
        if (innerClasses.size() > 0) {
            ...
            for (Enumeration e = innerClasses.elements(); e.hasMoreElements(); ) {
                ...
                int imods = inner.getInnerClassMember().getModifiers() & ACCM_INNERCLASS;
                ...
                if (env.dumpModifiers()) {
                    ...
                    Identifier nm = Identifier.lookup(fn.getQualifier(), fn.getFlatName());
    ...
    public void printClassDependencies(Environment env) {
        ...
        if (toplevelEnv.print_dependencies()) {
            ...
            System.out.println("CLASS:" + src + "," + startLine + "," + endLine + "," + className);
            ...
            for (Enumeration e = deps.elements(); e.hasMoreElements(); ) {
                ...
                String depName = Type.mangleInnerType(data.getName()).toString();
