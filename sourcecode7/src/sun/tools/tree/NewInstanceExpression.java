...
public class NewInstanceExpression extends NaryExpression {
    ...
    public NewInstanceExpression(long where, Expression right, Expression args[], Expression outerArg,
        ClassDefinition body) {
        ...
        try {
            ...
            if (body != null) {
                ...
                if (constructor != null) {
                    ...
                    if (tracing) {
                        env.dtEvent("NewInstanceExpression.checkValue: ANON CLASS " + body + " SUPER " + def);
                    }
            ...
            if (field == null) {
                ...
                if (anyInit != null &&
                    new MethodExpression(where, right, anyInit, args).diagnoseMismatch(env, args, argTypes)) {
                    return vset;
                }
            ...
            if (field.isProtected() &&
                !(sourceClass.getName().getQualifier().equals(field.getClassDeclaration().getName().getQualifier()))) {
     ...
     */
    public static Expression[] insertOuterLink(Environment env, Context ctx, long where, ClassDefinition def,
        Expression outerArg, Expression args[]) {
    ...
    public Expression inlineValue(Environment env, Context ctx) {
        ...
        try {
            if (outerArg != null) {
                if (outerArg.type.isType(TC_VOID)) { outerArg = outerArg.inline(env, ctx); } else {
            ...
            if (false && env.opt() && field.isInlineable(env, false) && (!ctx.field.isInitializer()) &&
                ctx.field.isMethod() && (ctx.getInlineMemberContext(field) == null)) {
