...
public class FieldExpression extends UnaryExpression {
    ...
    *-------------------------------------------------------*/
    ...
    private Vset checkCommon(Environment env, Context ctx, Vset vset, Hashtable exp, UnaryExpression loc,
        boolean isLHS) {
        ...
        try {
            ...
            if (field.isProtected() && !(right instanceof SuperExpression
                // Extension of JLS 6.6.2 for qualified 'super'.
                || (right instanceof FieldExpression && ((FieldExpression) right).id == idSuper)) &&
                !sourceClass.protectedAccess(env, field, right.type)) {
                env.error(where, "invalid.protected.field.use", field.getName(), field.getClassDeclaration(),
                    right.type);
                return vset;
            }
            ...
            if (sourceClass == ctxClass) {
                ...
                if (declarer.isPackagePrivate() &&
                    !declarer.getName().getQualifier().equals(sourceClass.getName().getQualifier())) {
    ...
    private static MemberDefinition getClassLiteralCache(Environment env, Context ctx, String className,
        ClassDefinition c) {
        ...
        return env.makeMemberDefinition(env, c.getWhere(), c, null, M_STATIC | M_SYNTHETIC, // M_PRIVATE,
            Type.tClassDesc, fname, null, null, null);
    }

    private Expression makeClassLiteralCacheRef(Environment env, Context ctx, MemberDefinition lookup,
        MemberDefinition cfld, String className) {
    ...
    private Expression makeClassLiteralInlineRef(Environment env, Context ctx, MemberDefinition lookup,
        String className) {
        Expression lcls = new TypeExpression(where, lookup.getClassDefinition().getType());
        Expression name = new StringExpression(where, className);
        Expression namearg[] = { name };
    ...
    public boolean isConstant() {
        ...
        if ((field != null) &&
            (right == null || right instanceof TypeExpression || (right.op == THIS && right.where == where))) {
