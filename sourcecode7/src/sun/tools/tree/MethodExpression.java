...
public class MethodExpression extends NaryExpression {
    ...
    public Vset checkValue(Environment env, Context ctx, Vset vset, Hashtable exp) {
        ...
        try {
                    ...
                    if (m.getClassDefinition() != cdef) {
                        ...
                        while ((cdef2 = cdef2.getOuterClass()) != null) {
                            ...
                            if (m2 != null && m2.getClassDefinition() == cdef2) {
                                env.error(where, "inherited.hides.method", id, cdef.getClassDeclaration(),
            ...
            if (field.isProtected() && !(right == null) && !(right instanceof SuperExpression
                ...
                env.error(where, "invalid.protected.method.use", field.getName(), field.getClassDeclaration(),
                    right.type);
                return vset;
            }
            ...
            if (sourceClass == ctxClass) {
                ...
                if (!field.isConstructor() && declarer.isPackagePrivate() &&
    ...
    boolean diagnoseMismatch(Environment env, Expression args[], Type argTypes[]) throws ClassNotFound {
        ...
        while (start < argTypes.length) {
            ...
            if (castOK) { env.error(args[i].where, "explicit.cast.needed", opName, argTypes[i], ttype); } else {
    ...
    public Expression inline(Environment env, Context ctx) {
        ...
        try {
            ...
            if (env.opt() && field.isInlineable(env, clazz.isFinal()) &&
                ...
                ctxClass.permitInlinedAccess(env, field.getClassDeclaration()) &&
                ctxClass.permitInlinedAccess(env, field) &&
                (right == null || ctxClass.permitInlinedAccess(env, env.getClassDeclaration(right.type))) &&

                ((id == null) || !id.equals(idInit)) && (!ctx.field.isInitializer()) && ctx.field.isMethod() &&
    ...
    public Expression inlineValue(Environment env, Context ctx) {
        ...
        try {
            ...
            if (env.opt() && field.isInlineable(env, clazz.isFinal()) &&
                ...
                (!ctx.field.isInitializer()) && ctx.field.isMethod() && (ctx.getInlineMemberContext(field) == null)) {
                Statement s = (Statement) field.getValue(env);
                if ((s == null) || (s.costInline(MAXINLINECOST, env, ctx) < MAXINLINECOST)) {
        ...
        for (MemberDefinition f = clazz.getFirstMember(); f != null; f = f.getNextMember()) {
            if ((f.isVariable() || f.isInitializer()) && !f.isStatic()) {
                ...
                if (f.isUplevelValue()) {
                    ...
                    IdentifierExpression arg = new IdentifierExpression(where, f.getName());
                    if (!arg.bind(env, ctx)) {
    ...
    public void codeValue(Environment env, Context ctx, Assembler asm) {
        if (implementation != null) { throw new CompilerError("codeValue"); }
