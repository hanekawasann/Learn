...
public class TrueTypeGlyphMapper extends CharToGlyphMapper {
    ...
    public TrueTypeGlyphMapper(TrueTypeFont font) {
        ...
        if (FontUtilities.isSolaris && isJAlocale && font.supportsJA()) {
            ...
            if (FontUtilities.isSolaris8 && getGlyphFromCMAP(JA_WAVE_DASH_CHAR) == missingGlyph) {
    ...
    private char getGlyphFromCMAP(int charCode) {
        try {
            ...
            if (glyphCode < numGlyphs || glyphCode >= FileFontStrike.INVISIBLE_GLYPHS) {
                return glyphCode;
            } else {
                if (FontUtilities.isLogging()) {
                    FontUtilities.getLogger().warning(
                        font + " out of range glyph id=" + Integer.toHexString((int) glyphCode) + " for char " +
    ...
    public void charsToGlyphs(int count, int[] unicodes, int[] glyphs) {
        for (int i = 0; i < count; i++) {
            ...
            if (font.checkUseNatives() && glyphs[i] < font.glyphToCharMap.length) {
    ...
    boolean hasSupplementaryChars() {
        return cmap instanceof CMap.CMapFormat8 || cmap instanceof CMap.CMapFormat10 ||
