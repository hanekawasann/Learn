...
public final class GlyphLayout {
    ...
    public static interface LayoutEngine {
         ...
         */
        public void layout(FontStrikeDesc sd, float[] mat, int gmask, int baseIndex, TextRecord text, int typo_flags,
    ...
    private static final class SDCache {
        ...
        private SDCache(Font font, FontRenderContext frc) {
            ...
            dtx.setTransform(dtx.getScaleX(), dtx.getShearY(), dtx.getShearX(), dtx.getScaleY(), 0, 0);
            if (!dtx.isIdentity()) {
                try {
            ...
            if (font.isTransformed()) {
                ...
                gtx.setTransform(gtx.getScaleX(), gtx.getShearY(), gtx.getShearX(), gtx.getScaleY(), 0, 0);
             ...
             * AA mode is 'gasp'.
             */
            int aa = FontStrikeDesc
                .getAAHintIntVal(frc.getAntiAliasingHint(), FontUtilities.getFont2D(font), (int) Math.abs(ptSize));
        ...
        private static final class SDKey {
            ...
            public boolean equals(Object o) {
                try {
                    SDKey rhs = (SDKey) o;
                    return hash == rhs.hash && font.equals(rhs.font) && frc.equals(rhs.frc);
                } catch (ClassCastException e) {
                }
                return false;
            }
        ...
        public static SDCache get(Font font, FontRenderContext frc) {
            ...
            if (frc.isTransformed()) {
                ...
                if (transform.getTranslateX() != 0 || transform.getTranslateY() != 0) {
                    transform = new AffineTransform(transform.getScaleX(), transform.getShearY(), transform.getShearX(),
     ...
     */
    public StandardGlyphVector layout(Font font, FontRenderContext frc, char[] text, int offset, int count, int flags,
        StandardGlyphVector result) {
    ...
    private final class EngineRecord {
        ...
        void init(int start, int limit, Font2D font, int script, int lang, int gmask) {
            ...
            for (int i = start; i < limit; ++i) {
                ...
                if (isHighSurrogate((char) ch) && i < limit - 1 && isLowSurrogate(_textRecord.text[i + 1])) {
                    // rare case
                    ch = toCodePoint((char) ch, _textRecord.text[++i]); // inc
                }
                int gc = getType(ch);
                if (gc == NON_SPACING_MARK || gc == ENCLOSING_MARK ||
