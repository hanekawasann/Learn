...
public class FileFontStrike extends PhysicalStrike {
    ...
    FileFontStrike(FileFont fileFont, FontStrikeDesc desc) {
         ...
         * requires it. However
         */
        if (Double.isNaN(matrix[0]) || Double.isNaN(matrix[1]) || Double.isNaN(matrix[2]) || Double.isNaN(matrix[3]) ||
            fileFont.getScaler() == null) {
            pScalerContext = NullFontScaler.getNullScalerContext();
        } else {
            pScalerContext = fileFont.getScaler()
                .createScalerContext(matrix, desc.aaHint, desc.fmHint, boldness, italic, disableHinting);
        }
        ...
        segmentedCache = (numGlyphs > SEGSIZE << 3) ||
            ((numGlyphs > SEGSIZE << 1) && (!isSimpleTx || ptSize != iSize || iSize < 6 || iSize > 36));
         ...
         */
        if (FontUtilities.isWindows && isXPorLater && !FontUtilities.useT2K && !GraphicsEnvironment.isHeadless() &&
            !fileFont.useJavaRasterizer &&
            (desc.aaHint == INTVAL_TEXT_ANTIALIAS_LCD_HRGB || desc.aaHint == INTVAL_TEXT_ANTIALIAS_LCD_HBGR) &&
            (matrix[1] == 0.0 && matrix[2] == 0.0 && matrix[0] == matrix[3] && matrix[0] >= 3.0 &&
                matrix[0] <= 100.0) && !((TrueTypeFont) fileFont).useEmbeddedBitmapsForSize(intPtSize)) {
            useNatives = true;
            ...
            if (matrix[1] == 0.0 && matrix[2] == 0.0 && matrix[0] >= 6.0 && matrix[0] <= 36.0 &&
                matrix[0] == matrix[3]) {
                useNatives = true;
        ...
        if (FontUtilities.isLogging() && FontUtilities.isWindows) {
            FontUtilities.getLogger().info(
                "Strike for " + fileFont + " at size = " + intPtSize + " use natives = " + useNatives +
                    " useJavaRasteriser = " + fileFont.useJavaRasterizer + " AAHint = " + desc.aaHint +
                    " Has Embedded bitmaps = " + ((TrueTypeFont) fileFont).
         ...
         */
        double maxSz = 48.0;
        getImageWithAdvance = Math.abs(at.getScaleX()) <= maxSz && Math.abs(at.getScaleY()) <= maxSz &&
            Math.abs(at.getShearX()) <= maxSz && Math.abs(at.getShearY()) <= maxSz;
     ...
     */
    private native long _getGlyphImageFromWindows(String family, int style, int size, int glyphCode,
        boolean fracMetrics);
        ...
        if ((glyphPtr = getCachedGlyphPtr(glyphCode)) != 0L) {
            ...
            if (useNatives) {
                ...
                if (glyphPtr == 0L && FontUtilities.isLogging()) {
                    FontUtilities.getLogger().info("Strike for " + fileFont + " at size = " + intPtSize +
    ...
    void getGlyphImageBounds(int glyphCode, Point2D.Float pt, Rectangle result) {
         ...
         */
        if ((desc.aaHint == INTVAL_TEXT_ANTIALIAS_LCD_HRGB || desc.aaHint == INTVAL_TEXT_ANTIALIAS_LCD_HBGR) &&
            topLeftX <= -2.0f) {
    ...
    private int getGlyphImageMinX(long ptr, int origMinX) {
        ...
        if (StrikeCache.nativeAddressSize == 4) {
            pixelData = 0xffffffff & StrikeCache.unsafe.getInt(ptr + StrikeCache.pixelDataOffset);
        } else {
            pixelData = StrikeCache.unsafe.getLong(ptr + StrikeCache.pixelDataOffset);
        }
        if (pixelData == 0L) {
            return origMinX;
        }
