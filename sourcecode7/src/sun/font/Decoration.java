...
public class Decoration {
    ...
    private static final int VALUES_MASK = AttributeValues
        .getMask(EFOREGROUND, EBACKGROUND, ESWAP_COLORS, ESTRIKETHROUGH, EUNDERLINE, EINPUT_METHOD_HIGHLIGHT,
    ...
    public static Decoration getDecoration(AttributeValues values) {
        ...
        return new DecorationImpl(values.getForeground(), values.getBackground(), values.getSwapColors(),
            values.getStrikethrough(), Underline.getUnderline(values.getUnderline()),
    ...
    private static final class DecorationImpl extends Decoration {
        ...
        DecorationImpl(Paint foreground, Paint background, boolean swapColors, boolean strikethrough,
            Underline stdUnderline, Underline imUnderline) {
        ...
        public boolean equals(Object rhs) {
            ...
            if (!(swapColors == other.swapColors && strikethrough == other.strikethrough)) {
                return false;
            }
        ...
        private void drawTextAndEmbellishments(Label label, Graphics2D g2d, float x, float y) {
            ...
            if (strikethrough) {
                ...
                g2d.setStroke(new BasicStroke(cm.strikethroughThickness, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));
        ...
        public void drawTextAndDecorations(Label label, Graphics2D g2d, float x, float y) {
            ...
            if (fgPaint == null && bgPaint == null && swapColors == false) {
                ...
                if (swapColors) {
                    ...
                    if (bgPaint == null) {
                        if (background instanceof Color) {
                            ...
                            int brightness = 33 * bg.getRed() + 53 * bg.getGreen() + 14 * bg.getBlue();
                ...
                if (background != null) {
                    ...
                    bgArea = new Rectangle2D.Float(x + (float) bgArea.getX(), y + (float) bgArea.getY(),
                        (float) bgArea.getWidth(), (float) bgArea.getHeight());
        ...
        Shape getOutline(Label label, float x, float y) {
            ...
            if (stdUnderline != null) {
                Shape ul = stdUnderline.getUnderlineShape(ulThickness, x1, x2, y + ulOffset);
                area = new Area(ul);
            }

            if (strikethrough) {
                Stroke stStroke = new BasicStroke(cm.strikethroughThickness, BasicStroke.CAP_BUTT,
            ...
            if (imUnderline != null) {
                Shape ul = imUnderline.getUnderlineShape(ulThickness, x1, x2, y + ulOffset);
