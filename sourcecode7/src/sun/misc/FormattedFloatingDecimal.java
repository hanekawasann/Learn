...
public class FormattedFloatingDecimal {
    ...
    private FormattedFloatingDecimal(boolean negSign, int decExponent, char[] digits, int n, boolean e, int precision,
        Form form) {
        isNegative = negSign;
        isExceptional = e;
    ...
    private int checkExponent(int length) {
        if (length >= nDigits || length < 0) { return decExponent; }
    ...
    public int getChars(char[] result) {
        ...
        if (isExceptional) {
            ...
            if (exp > 0 && ((form == Form.COMPATIBLE && (exp < 8)) || (form == Form.DECIMAL_FLOAT))) {
                // print digits.digits.
                ...
                if (charLength < exp) {
                    ...
                    for (int nz = 0; nz < charLength; nz++) { result[i++] = '0'; }
            ...
            } else if (exp <= 0 && ((form == Form.COMPATIBLE && exp > -3) || (form == Form.DECIMAL_FLOAT))) {
                // print 0.0* digits
                result[i++] = '0';
                if (exp != 0) {
                    ...
                    if (t > 0) {
                        result[i++] = '.';
                        for (int nz = 0; nz < t; nz++) { result[i++] = '0'; }
                    }
                }
                ...
                if (exp <= 0) {
                    ...
                    if (form != Form.COMPATIBLE) { result[i++] = '+'; }
                    e = exp - 1;
                }
                // decExponent has 1, 2, or 3, digits
                if (e <= 9) {
                    if (form != Form.COMPATIBLE) { result[i++] = '0'; }
                    result[i++] = (char) (e + '0');
                } else if (e <= 99) {
