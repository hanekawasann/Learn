...
public class FpUtils {
    ...
    public static int getExponent(double d) {
         ...
         */
        return (int) (
            ((Double.doubleToRawLongBits(d) & DoubleConsts.EXP_BIT_MASK) >> (DoubleConsts.SIGNIFICAND_WIDTH - 1)) -
        ...
        switch (exponent) {
            ...
            case FloatConsts.MIN_EXPONENT - 1:        // zero or subnormal
                if (f == 0.0f) {
                    ...
                    exponent++;
                    assert (exponent >= FloatConsts.MIN_EXPONENT - (FloatConsts.SIGNIFICAND_WIDTH - 1) &&
    ...
    public static double scalb(double d, int scale_factor) {
        ...
        final int MAX_SCALE = DoubleConsts.MAX_EXPONENT + -DoubleConsts.MIN_EXPONENT + DoubleConsts.SIGNIFICAND_WIDTH +
            1;
        int exp_adjust = 0;
    ...
    public static double nextAfter(double start, double direction) {
        ...
        if (isNaN(start) || isNaN(direction)) {
            ...
            if (direction > start) { // Calculate next greater value
                ...
                if (transducer > 0L) { --transducer; } else if (transducer < 0L) { ++transducer; }
                /*
    ...
    public static float nextAfter(float start, double direction) {
        ...
        if (isNaN(start) || isNaN(direction)) {
            ...
            if (direction > start) {// Calculate next greater value
                ...
                assert direction < start;
                if (transducer > 0) { --transducer; } else if (transducer < 0) { ++transducer; }
                /*
                 * transducer==0, the result is -MIN_VALUE
                 *
                 * The transition from zero (implicitly
    ...
    public static double nextUp(double d) {
        if (isNaN(d) || d == Double.POSITIVE_INFINITY) { return d; } else {
            d += 0.0d;
    ...
    public static float nextUp(float f) {
        if (isNaN(f) || f == FloatConsts.POSITIVE_INFINITY) { return f; } else {
            f += 0.0f;
    ...
    public static double nextDown(double d) {
        if (isNaN(d) || d == Double.NEGATIVE_INFINITY) { return d; } else {
            if (d == 0.0) { return -Double.MIN_VALUE; } else {
                return Double.longBitsToDouble(Double.doubleToRawLongBits(d) + ((d > 0.0d) ? -1L : +1L));
            }
        }
    }
    ...
    public static double nextDown(float f) {
        if (isNaN(f) || f == Float.NEGATIVE_INFINITY) { return f; } else {
            if (f == 0.0f) { return -Float.MIN_VALUE; } else {
                return Float.intBitsToFloat(Float.floatToRawIntBits(f) + ((f > 0.0f) ? -1 : +1));
            }
        }
    }
        ...
        switch (exp) {
            ...
            default:
                ...
                if (exp >= FloatConsts.MIN_EXPONENT) {
                    ...
                    // number of positions
                    return Float
                        .intBitsToFloat(1 << (exp - (FloatConsts.MIN_EXPONENT - (FloatConsts.SIGNIFICAND_WIDTH - 1))));
                }
        }
    }
