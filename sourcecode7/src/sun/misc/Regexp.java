...
public class Regexp {
    ...
    public Regexp(String s) {
        ...
        if (firstst < 0) {
            ...
            prefixLen = firstst;
            if (firstst == 0) { prefix = null; } else { prefix = s.substring(0, firstst); }
            suffixLen = s.length() - lastst - 1;
            if (suffixLen == 0) { suffix = null; } else { suffix = s.substring(lastst + 1); }
            int nmids = 0;
            int pos = firstst;
    ...
    boolean matches(String s, int offset, int len) {
        if (exact) { return len == totalLen && exp.regionMatches(ignoreCase, 0, s, offset, len); }
        ...
        for (int i = 0; i < nmids; i++) {
            String ms = mids[i];
            int ml = ms.length();
            while (spos + ml <= limit && !ms.regionMatches(ignoreCase, 0, s, spos, ml)) { spos++; }
