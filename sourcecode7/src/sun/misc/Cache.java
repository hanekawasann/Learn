        ...
        for (int i = oldCapacity; i-- > 0; ) {
            for (CacheEntry old = oldTable[i]; old != null; ) {
                ...
                if (e.check() != null) {
                    ...
                    newTable[index] = e;
                } else { count--;    /* remove entries that have disappeared */ }
            }
        }
    }
    ...
    public synchronized Object put(Object key, Object value) {
        ...
        for (CacheEntry e = tab[index]; e != null; e = e.next) {
            if ((e.hash == hash) && e.key.equals(key)) {
                ...
                e.setThing(value);
                return old;
            } else if (e.check() == null) { ne = e;         /* reuse old flushed value */ }
        }
...
class CacheEnumerator implements Enumeration {
    ...
    public boolean hasMoreElements() {
        while (index >= 0) {
            while (entry != null) { if (entry.check() != null) { return true; } else { entry = entry.next; } }
    ...
    public Object nextElement() {
        while (index >= 0) {
            ...
            if (entry != null) {
                CacheEntry e = entry;
                entry = e.next;
                if (e.check() != null) { return keys ? e.key : e.check(); }
            }
        }
