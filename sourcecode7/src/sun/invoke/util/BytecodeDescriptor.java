...
public class BytecodeDescriptor {
    ...
    static List<Class<?>> parseMethod(String bytecodeSignature, int start, int end, ClassLoader loader) {
        ...
        if (i[0] < end && str.charAt(i[0]) == '(') {
            ...
            while (i[0] < end && str.charAt(i[0]) != ')') {
                ...
                if (pt == null || pt == void.class) { parseError(str, "bad argument type"); }
                ptypes.add(pt);
            }
            ++i[0];  // skip ')'
        } else {
        ...
        if (rtype == null || i[0] != end) { parseError(str, "bad return type"); }
        ptypes.add(rtype);
        return ptypes;
    }
    ...
    static private Class<?> parseSig(String str, int[] i, int end, ClassLoader loader) {
        ...
        if (c == 'L') {
            ...
            if (t != null) { t = java.lang.reflect.Array.newInstance(t, 0).getClass(); }
            return t;
        } else {
    ...
    public static String unparse(Object type) {
        if (type instanceof Class<?>) { return unparse((Class<?>) type); }
        if (type instanceof MethodType) { return unparse((MethodType) type); }
        return (String) type;
    }
    ...
    public static String unparseMethod(Class<?> rtype, List<Class<?>> ptypes) {
        StringBuilder sb = new StringBuilder();
        sb.append('(');
        for (Class<?> pt : ptypes) { unparseSig(pt, sb); }
        sb.append(')');
        unparseSig(rtype, sb);
        return sb.toString();
    }
