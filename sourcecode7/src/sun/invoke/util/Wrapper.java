...
public enum Wrapper {
    ...
    public String detailString() {
        return simpleName + java.util.Arrays
            .asList(wrapperType, primitiveType, basicTypeChar, zero, "0x" + Integer.toHexString(format));
    }
    ...
    private static abstract class Format {
        ...
        static final int SLOT_MASK = ((1 << (SIZE_SHIFT - SLOT_SHIFT)) - 1), SIZE_MASK = (
            (1 << (KIND_SHIFT - SIZE_SHIFT)) - 1);

        static int format(int kind, int size, int slots) {
            assert (((kind >> KIND_SHIFT) << KIND_SHIFT) == kind);
            assert ((size & (size - 1)) == 0); // power of two
            assert ((kind == SIGNED) ? (size > 0)
                : (kind == UNSIGNED) ? (size > 0) : (kind == FLOATING) ? (size == 32 || size == 64) : false);
            assert ((slots == 2) ? (size == 64) : (slots == 1) ? (size <= 32) : false);
            return kind | (size << SIZE_SHIFT) | (slots << SLOT_SHIFT);
        }

        static final int INT = SIGNED | (32 << SIZE_SHIFT) | (1 << SLOT_SHIFT), SHORT = SIGNED | (16 << SIZE_SHIFT) |
            (1 << SLOT_SHIFT), BOOLEAN = UNSIGNED | (1 << SIZE_SHIFT) | (1 << SLOT_SHIFT), CHAR = UNSIGNED |
            (16 << SIZE_SHIFT) | (1 << SLOT_SHIFT), FLOAT = FLOATING | (32 << SIZE_SHIFT) | (1 << SLOT_SHIFT), VOID =
            UNSIGNED | (0 << SIZE_SHIFT) | (0 << SLOT_SHIFT), NUM_MASK = (-1) << SIZE_SHIFT;
        ...
        for (Wrapper w : values()) {
            ...
            if (w.isSigned()) {
                for (Wrapper x : values()) {
                    if (w == x) { continue; }
                    if (x.isFloating()) { assert (!w.isConvertibleFrom(x)); } else if (x.isSigned()) {
                        if (w.compareTo(x) < 0) { assert (!w.isConvertibleFrom(x)); } else {
            ...
            if (w.isFloating()) {
                for (Wrapper x : values()) {
                    if (w == x) { continue; }
                    if (x.isSigned()) { assert (w.isConvertibleFrom(x)); } else if (x.isFloating()) {
                        if (w.compareTo(x) < 0) { assert (!w.isConvertibleFrom(x)); } else {
                            assert (w.isConvertibleFrom(x));
                        }
                    }
    ...
    public static char basicTypeChar(Class<?> type) {
        if (!type.isPrimitive()) { return 'L'; } else { return forPrimitiveType(type).basicTypeChar(); }
    }
    ...
    public Object wrap(Object x) {
        ...
        switch (basicTypeChar) {
            ...
            case 'Z':
                return Boolean.valueOf(boolValue(xn.longValue()));
        }
    ...
    public Object wrap(int x) {
        ...
        switch (basicTypeChar) {
            ...
            case 'Z':
                return Boolean.valueOf(boolValue(x));
        }
        throw new InternalError("bad wrapper");
    }
    ...
    public Object wrapRaw(long x) {
        switch (basicTypeChar) {
            ...
            case 'D':
                return Double.valueOf(Double.longBitsToDouble(x));
            case 'L':  // same as 'J':
            case 'J':
                return (Long) x;
        }
    ...
    public long unwrapRaw(Object x) {
        switch (basicTypeChar) {
            ...
            case 'L':
                throw newIllegalArgumentException("cannot unwrap from sobject type");
            case 'V':
                return 0;
            case 'I':
            ...
            case 'B':
                return (byte) (Byte) x;
            case 'C':
                return (char) (Character) x;
            case 'Z':
                return (boolean) (Boolean) x ? 1 : 0;
        }
        throw new InternalError("bad wrapper");
    }
