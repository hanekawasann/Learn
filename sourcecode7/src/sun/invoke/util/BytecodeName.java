...
public class BytecodeName {
    ...
    private static String appendAll(Object[] components) {
        ...
        for (Object c : components) {
            if (c instanceof String) { slen += String.valueOf(c).length(); } else { slen += 1; }
        }
    ...
    public static boolean isSafeBytecodeName(String s) {
        ...
        for (char xc : DANGEROUS_CHARS_A) {
            if (xc == ESCAPE_C) {
                continue;  // not really that dangerous
            }
    ...
    static boolean isSpecial(char c) {
        if ((c >>> 6) < SPECIAL_BITMAP.length) { return ((SPECIAL_BITMAP[c >>> 6] >> c) & 1) != 0; } else {
            return false;
        }
    }
