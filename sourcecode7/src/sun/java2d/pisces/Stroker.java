...
final class Stroker implements PathConsumer2D {
    ...
    private static void computeOffset(final float lx, final float ly, final float w, final float[] m) {
    ...
    private void drawRoundJoin(float x, float y, float omx, float omy, float mx, float my, boolean rev,
        float threshold) {
    ...
    private void drawBezApproxForArc(final float cx, final float cy, final float omx, final float omy, final float mx,
    ...
    private void drawRoundCap(float cx, float cy, float mx, float my) {
        ...
        emitCurveTo(cx + mx, cy + my, cx + mx - C * my, cy + my + C * mx, cx - my + C * mx, cy + mx + C * my, cx - my,
            cy + mx, false);
        emitCurveTo(cx - my, cy + mx, cx - my - C * mx, cy + mx - C * my, cx - mx - C * my, cy - my + C * mx, cx - mx,
            cy - my, false);
    }
    ...
    private void computeIntersection(final float x0, final float y0, final float x1, final float y1, final float x0p,
    ...
    private void drawMiter(final float pdx, final float pdy, final float x0, final float y0, final float dx,
        ...
        computeIntersection((x0 - pdx) + omx, (y0 - pdy) + omy, x0 + omx, y0 + omy, (dx + x0) + mx, (dy + y0) + my,
            x0 + mx, y0 + my, miter, 0);
    ...
    private void emitQuadTo(final float x0, final float y0, final float x1, final float y1, final float x2,
    ...
    private void emitCurveTo(final float x0, final float y0, final float x1, final float y1, final float x2,
    ...
    private void drawJoin(float pdx, float pdy, float x0, float y0, float dx, float dy, float omx, float omy, float mx,
        float my) {
        if (prev != DRAWING_OP_TO) {
    ...
    private static boolean within(final float x1, final float y1, final float x2, final float y2, final float ERR) {
        assert ERR > 0 : "";
    ...
    private int computeOffsetQuad(float[] pts, final int off, float[] leftOff, float[] rightOff) {
        ...
        leftOff[0] = x1 + offset[0][0];
        leftOff[1] = y1 + offset[0][1];
        leftOff[4] = x3 + offset[1][0];
        leftOff[5] = y3 + offset[1][1];
        rightOff[0] = x1 - offset[0][0];
        rightOff[1] = y1 - offset[0][1];
        rightOff[4] = x3 - offset[1][0];
        rightOff[5] = y3 - offset[1][1];
    ...
    private static final class PolyStack {
        ...
        public void pop(PathConsumer2D io) {
            ...
            switch (type) {
                case 8:
                    io.curveTo(curves[end + 0], curves[end + 1], curves[end + 2], curves[end + 3], curves[end + 4],
                        curves[end + 5]);
                    break;
                case 6:
