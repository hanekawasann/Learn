...
public abstract class BufferedRenderPipe implements PixelDrawPipe, PixelFillPipe, ShapeDrawPipe, ParallelogramPipe {
    ...
    protected void drawPoly(final SunGraphics2D sg2d, final int[] xPoints, final int[] yPoints, final int nPoints,
    ...
    protected abstract void drawPoly(int[] xPoints, int[] yPoints, int nPoints, boolean isClosed, int transX,
        int transY);
    ...
    private native int fillSpans(RenderQueue rq, long buf, int pos, int limit, SpanIterator si, long iterator,
    ...
    protected void fillSpans(SunGraphics2D sg2d, SpanIterator si, int transx, int transy) {
        ...
        try {
            ...
            int newpos = fillSpans(rq, buf.getAddress(), buf.position(), buf.capacity(), si, si.getNativeIterator(),
                transx, transy);
            buf.position(newpos);
        } finally {
            rq.unlock();
    ...
    public void fill(SunGraphics2D sg2d, Shape s) {
        ...
        try {
            ...
            Region clip = sg2d.getCompClip();
            ssi.setOutputAreaXYXY(clip.getLoX() - transx, clip.getLoY() - transy, clip.getHiX() - transx,
                clip.getHiY() - transy);
