...
public class LCMSTransform implements ColorTransform {
    ...
    public LCMSTransform(ColorTransform[] transforms) {
        ...
        for (int i = 0; i < transforms.length; i++) {
            ...
            System.arraycopy(curTrans.profileIDs, 0, profileIDs, j, curTrans.profileIDs.length);
    ...
    private synchronized void doTransform(LCMSImageLayout in, LCMSImageLayout out) {
        // update native transfrom if needed
        if (ID == 0L || inFormatter != in.pixelType || outFormatter != out.pixelType) {
            ...
            ID = LCMS.createNativeTransform(profileIDs, renderType, inFormatter, outFormatter, disposerReferent);
        }
    ...
    public void colorConvert(BufferedImage src, BufferedImage dst) {
        ...
        if (precision == 8) {
            ...
            srcIL = new LCMSImageLayout(srcLine, srcLine.length / getNumInComponents(),
                LCMSImageLayout.CHANNELS_SH(getNumInComponents()) | LCMSImageLayout.BYTES_SH(1), getNumInComponents());
            dstIL = new LCMSImageLayout(dstLine, dstLine.length / getNumOutComponents(),
    ...
    public void colorConvert(Raster src, WritableRaster dst, float[] srcMinVal, float[] srcMaxVal, float[] dstMinVal,
        float[] dstMaxVal) {
        LCMSImageLayout srcIL, dstIL;
        ...
        int idx;
        srcIL = new LCMSImageLayout(srcLine, srcLine.length / getNumInComponents(),
            LCMSImageLayout.CHANNELS_SH(getNumInComponents()) | LCMSImageLayout.BYTES_SH(2), getNumInComponents() * 2);
    ...
    public short[] colorConvert(short[] src, short[] dst) {
        ...
        LCMSImageLayout srcIL = new LCMSImageLayout(src, src.length / getNumInComponents(),
            LCMSImageLayout.CHANNELS_SH(getNumInComponents()) | LCMSImageLayout.BYTES_SH(2), getNumInComponents() * 2);

        LCMSImageLayout dstIL = new LCMSImageLayout(dst, dst.length / getNumOutComponents(),
            LCMSImageLayout.CHANNELS_SH(getNumOutComponents()) | LCMSImageLayout.BYTES_SH(2),
            getNumOutComponents() * 2);
    ...
    public byte[] colorConvert(byte[] src, byte[] dst) {
        ...
        LCMSImageLayout dstIL = new LCMSImageLayout(dst, dst.length / getNumOutComponents(),
            LCMSImageLayout.CHANNELS_SH(getNumOutComponents()) | LCMSImageLayout.BYTES_SH(1), getNumOutComponents());
