...
public class CMSManager {
    ...
    public static class CMMTracer implements PCMM {
        ...
        public int getTagSize(long profileID, int tagSignature) {
            System.err.printf(cName + ".getTagSize(ID=%x, TagSig=%s)", profileID, signatureToString(tagSignature));
        ...
        public void getTagData(long profileID, int tagSignature, byte[] data) {
            System.err.printf(cName + ".getTagData(ID=%x, TagSig=%s)", profileID, signatureToString(tagSignature));
            System.err.println(" requested " + data.length + " byte(s)");
            tcmm.getTagData(profileID, tagSignature, data);
        }

        public void setTagData(long profileID, int tagSignature, byte[] data) {
            System.err.print(cName + ".setTagData(ID=" + profileID + ", TagSig=" + tagSignature + ")");
        ...
        private static String signatureToString(int sig) {
            return String.format("%c%c%c%c", (char) (0xff & (sig >> 24)), (char) (0xff & (sig >> 16)),
                (char) (0xff & (sig >> 8)), (char) (0xff & (sig)));
        }
    }
}
