...
public class MaskFill extends GraphicsPrimitive {
     ...
     */
    public native void MaskFill(SunGraphics2D sg2d, SurfaceData sData, Composite comp, int x, int y, int w, int h,
        byte[] mask, int maskoff, int maskscan);

    public native void FillAAPgram(SunGraphics2D sg2d, SurfaceData sData, Composite comp, double x, double y,
        double dx1, double dy1, double dx2, double dy2);

    public native void DrawAAPgram(SunGraphics2D sg2d, SurfaceData sData, Composite comp, double x, double y,
    ...
    private static class General extends MaskFill {
        ...
        public void MaskFill(SunGraphics2D sg2d, SurfaceData sData, Composite comp, int x, int y, int w, int h,
    ...
    private static class TraceMaskFill extends MaskFill {
        ...
        public TraceMaskFill(MaskFill target) {
            ...
            this.target = target;
            this.fillPgramTarget = new MaskFill(fillPgramSignature, target.getSourceType(), target.getCompositeType(),
                target.getDestType());
            this.drawPgramTarget = new MaskFill(drawPgramSignature, target.getSourceType(), target.getCompositeType(),
                target.getDestType());
        }
        ...
        public void FillAAPgram(SunGraphics2D sg2d, SurfaceData sData, Composite comp, double x, double y, double dx1,
            double dy1, double dx2, double dy2) {
            tracePrimitive(fillPgramTarget);
            target.FillAAPgram(sg2d, sData, comp, x, y, dx1, dy1, dx2, dy2);
        }

        public void DrawAAPgram(SunGraphics2D sg2d, SurfaceData sData, Composite comp, double x, double y, double dx1,
