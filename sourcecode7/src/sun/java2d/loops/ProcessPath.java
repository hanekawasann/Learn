...
public class ProcessPath {
    ...
    public static abstract class ProcessHandler implements EndSubPathHandler {
        ...
        public abstract void processFixedLine(int x1, int y1, int x2, int y2, int[] pixelInfo, boolean checkBounds,
            boolean endSubPath);
    }
    ...
    public static boolean drawPath(DrawHandler dhnd, EndSubPathHandler endSubPath, Path2D.Float p2df, int transX,
        int transY) {
        return doProcessPath(new DrawProcessHandler(dhnd, endSubPath), p2df, transX, transY);
    }

    public static boolean drawPath(DrawHandler dhnd, Path2D.Float p2df, int transX, int transY) {
        return doProcessPath(new DrawProcessHandler(dhnd, noopEndSubPathHandler), p2df, transX, transY);
    }
    ...
    private static final int DF_QUAD_DEC_BND = 1 << DF_QUAD_STEPS * 2 + FWD_PREC + 2;
     ...
     *
     */
    private static int CLIPCLAMP(float LINE_MIN, float LINE_MAX, float[] c, int a1, int b1, int a2, int b2, int a3,
        int b3) {
        c[a3] = c[a1];
        c[b3] = c[b1];
    ...
    private static class DrawProcessHandler extends ProcessHandler {
        ...
        void PROCESS_LINE(int fX0, int fY0, int fX1, int fY1, boolean checkBounds, int[] pixelInfo) {
            ...
            if (((X0 ^ X1) | (Y0 ^ Y1)) == 0) {
                if (checkBounds && (dhnd.yMin > Y0 || dhnd.yMax <= Y0 || dhnd.xMin > X0 || dhnd.xMax <= X0)) { return; }
            ...
            if (!checkBounds || (dhnd.yMin <= Y0 && dhnd.yMax > Y0 && dhnd.xMin <= X0 && dhnd.xMax > X0)) {
                /* Switch off first pixel of the line before drawing */
                if (pixelInfo[0] == 1 &&
                    ((pixelInfo[1] == X0 && pixelInfo[2] == Y0) || (pixelInfo[3] == X0 && pixelInfo[4] == Y0))) {
                    dhnd.drawPixel(X0, Y0);
                }
            }
            ...
            if ((pixelInfo[1] == X1 && pixelInfo[2] == Y1) || (pixelInfo[3] == X1 && pixelInfo[4] == Y1)) {
                if (checkBounds && (dhnd.yMin > Y1 || dhnd.yMax <= Y1 || dhnd.xMin > X1 || dhnd.xMax <= X1)) {
                    return;
                }
        ...
        void PROCESS_POINT(int fX, int fY, boolean checkBounds, int[] pixelInfo) {
            int _X = fX >> MDP_PREC;
            int _Y = fY >> MDP_PREC;
            if (checkBounds && (dhnd.yMin > _Y || dhnd.yMax <= _Y || dhnd.xMin > _X || dhnd.xMax <= _X)) { return; }
            /*
         ...
         *
         */
        public void processFixedLine(int x1, int y1, int x2, int y2, int[] pixelInfo, boolean checkBounds,
            boolean endSubPath) {
            ...
            if ((c & MDP_W_MASK) == 0) {
                 ...
                 */
                if (c == 0) {
                    PROCESS_POINT(x1 + MDP_HALF_MULT, y1 + MDP_HALF_MULT, checkBounds, pixelInfo);
                }
                return;
            }
    ...
    private static void DrawMonotonicQuad(ProcessHandler hnd, float[] coords, boolean checkBounds, int[] pixelInfo) {
        ...
        int ax = (int) ((coords[0] - 2 * coords[2] + coords[4]) * QUAD_A_MDP_MULT);
        int ay = (int) ((coords[1] - 2 * coords[3] + coords[5]) * QUAD_A_MDP_MULT);
    ...
    private static void ProcessMonotonicQuad(ProcessHandler hnd, float[] coords, int[] pixelInfo) {
        ...
        if (xMax - xMin > MAX_QUAD_SIZE || yMax - yMin > MAX_QUAD_SIZE) {
            ...
            DrawMonotonicQuad(hnd, coords,
                 ...
                 * simple
                 */
                hnd.dhnd.xMinf >= xMin || hnd.dhnd.xMaxf <= xMax || hnd.dhnd.yMinf >= yMin || hnd.dhnd.yMaxf <= yMax,
                pixelInfo);
        }
    }
    ...
    private static void ProcessQuad(ProcessHandler hnd, float[] coords, int[] pixelInfo) {
         ...
         * ordered.
         */
        if ((coords[0] > coords[2] || coords[2] > coords[4]) && (coords[0] < coords[2] || coords[2] < coords[4])) {
         ...
         * ordered.
         */
        if ((coords[1] > coords[3] || coords[3] > coords[5]) && (coords[1] < coords[3] || coords[3] < coords[5])) {
        ...
        switch (cnt) {
            case 0:
                break;
            case 1:
                ProcessFirstMonotonicPartOfQuad(hnd, coords, pixelInfo, (float) params[0]);
                break;
            case 2:
                ProcessFirstMonotonicPartOfQuad(hnd, coords, pixelInfo, (float) params[0]);
    ...
    private static void DrawMonotonicCubic(ProcessHandler hnd, float[] coords, boolean checkBounds, int[] pixelInfo) {
        ...
        int ax = (int) ((-coords[0] + 3 * coords[2] - 3 * coords[4] + coords[6]) * CUB_A_MDP_MULT);
        int ay = (int) ((-coords[1] + 3 * coords[3] - 3 * coords[5] + coords[7]) * CUB_A_MDP_MULT);
        ...
        while (count > 0) {
            ...
            while ((count & 1) == 0 && shift > DF_CUB_SHIFT && Math.abs(dpx) <= incStepBnd &&
    ...
    private static void ProcessMonotonicCubic(ProcessHandler hnd, float[] coords, int[] pixelInfo) {
        ...
        if (hnd.clipMode == PH_MODE_DRAW_CLIP) {
            ...
            if (hnd.dhnd.xMinf > xMax) {
                coords[0] = coords[2] = coords[4] = coords[6] = hnd.dhnd.xMinf;
            }
        }
    ...
    private static void ProcessCubic(ProcessHandler hnd, float[] coords, int[] pixelInfo) {
         ...
         * ordered.
         */
        if ((coords[0] > coords[2] || coords[2] > coords[4] || coords[4] > coords[6]) &&
            (coords[0] < coords[2] || coords[2] < coords[4] || coords[4] < coords[6])) {
            /* Searching for extreme points of the X(t) function  by solving
             * dX(t)
             * ----  = 0 equation
             *  dt
         ...
         * ordered.
         */
        if ((coords[1] > coords[3] || coords[3] > coords[5] || coords[5] > coords[7]) &&
            (coords[1] < coords[3] || coords[3] < coords[5] || coords[5] < coords[7])) {
            /* Searching for extreme points of the Y(t) function by solving
             * dY(t)
             * ----- = 0 equation
             *  dt
        ...
        if (cnt > 0) {
            ...
            /* Processing obtained monotonic parts */
            ProcessFirstMonotonicPartOfCubic(hnd, coords, pixelInfo, (float) params[0]);
            for (int i = 1; i < cnt; i++) {
    ...
    private static void ProcessLine(ProcessHandler hnd, float x1, float y1, float x2, float y2, int[] pixelInfo) {
        ...
        if (hnd.clipMode == PH_MODE_DRAW_CLIP) {
            ...
            hnd.processFixedLine(X1, Y1, X2, Y2, pixelInfo, clipped, /* enable boundary checking in
        ...
        while (!pi.isDone()) {
            switch (pi.currentSegment(coords)) {
                case PathIterator.SEG_MOVETO:
                    ...
                    if (tCoords[0] < UPPER_BND && tCoords[0] > LOWER_BND && tCoords[1] < UPPER_BND &&
                        tCoords[1] > LOWER_BND) {
                        subpathStarted = true;
                        skip = false;
                ...
                case PathIterator.SEG_QUADTO:
                    ...
                    if (lastX < UPPER_BND && lastX > LOWER_BND && lastY < UPPER_BND && lastY > LOWER_BND) {
                        if (skip) {
                            ...
                            if (tCoords[2] < UPPER_BND && tCoords[2] > LOWER_BND && tCoords[3] < UPPER_BND &&
                                tCoords[3] > LOWER_BND) {
                ...
                case PathIterator.SEG_CUBICTO:
                    ...
                    if (lastX < UPPER_BND && lastX > LOWER_BND && lastY < UPPER_BND && lastY > LOWER_BND) {
                        if (skip) {
                            ...
                            if (tCoords[2] < UPPER_BND && tCoords[2] > LOWER_BND && tCoords[3] < UPPER_BND &&
                                tCoords[3] > LOWER_BND && tCoords[4] < UPPER_BND && tCoords[4] > LOWER_BND &&
                                tCoords[5] < UPPER_BND && tCoords[5] > LOWER_BND) {
    ...
    private static void FillPolygon(FillProcessHandler hnd, int fillRule) {
        ...
        for (y = hashOffset + MDP_MULT, k = 0; y <= yMax && k < hashSize; y += MDP_MULT, k++) {
    ...
    private static class FillProcessHandler extends ProcessHandler {
        ...
        public void processFixedLine(int x1, int y1, int x2, int y2, int[] pixelInfo, boolean checkBounds,
            ...
            if (checkBounds) {
                ...
                /* Clamping only by left boundary */
                if (res == CRES_MIN_CLIPPED) {
                    processFixedLine(c[4], c[5], c[0], c[1], pixelInfo, false, lastClipped);
                ...
                /* Clamping only by left boundary */
                if (res == CRES_MIN_CLIPPED) {
                    processFixedLine(c[2], c[3], c[4], c[5], pixelInfo, false, lastClipped);
                }
