...
public class Blit extends GraphicsPrimitive {
     ...
     */
    public native void Blit(SurfaceData src, SurfaceData dst, Composite comp, Region clip, int srcx, int srcy, int dstx,
    ...
    private static class AnyBlit extends Blit {
        ...
        public void Blit(SurfaceData srcData, SurfaceData dstData, Composite comp, Region clip, int srcx, int srcy,
            ...
            while (si.nextSpan(span)) {
                int w = span[2] - span[0];
                int h = span[3] - span[1];
                srcRas = srcRas.createChild(srcx + span[0], srcy + span[1], w, h, 0, 0, null);
                dstRas = dstRas.createWritableChild(span[0], span[1], w, h, 0, 0, null);
    ...
    private static class GeneralMaskBlit extends Blit {
        ...
        public void Blit(SurfaceData srcData, SurfaceData dstData, Composite comp, Region clip, int srcx, int srcy,
            int dstx, int dsty, int width, int height) {
            performop.MaskBlit(srcData, dstData, comp, clip, srcx, srcy, dstx, dsty, width, height, null, 0, 0);
        }
    }
    ...
    private static class GeneralXorBlit extends Blit implements GeneralBinaryOp {
        ...
        public void setPrimitives(Blit srcconverter, Blit dstconverter, GraphicsPrimitive genericop,
            Blit resconverter) {
        ...
        public synchronized void Blit(SurfaceData srcData, SurfaceData dstData, Composite comp, Region clip, int srcx,
            ...
            if (convertsrc == null) {
                ...
                src = convertFrom(convertsrc, srcData, srcx, srcy, width, height, cachedSrc);
                sx = 0;
                sy = 0;
                if (src != cachedSrc) {
            ...
            if (convertdst == null) {
                ...
                dst = convertFrom(convertdst, dstData, dstx, dsty, width, height, cachedDst);
                dx = 0;
                dy = 0;
                opclip = null;
                if (dst != cachedDst) {
            ...
            performop.Blit(src, dst, comp, opclip, sx, sy, dx, dy, width, height);

            if (convertresult != null) {
                // assert: convertdst != null
                convertTo(convertresult, dst, dstData, clip, dstx, dsty, width, height);
            }
        }
    }
    ...
    private static class TraceBlit extends Blit {
        ...
        public void Blit(SurfaceData src, SurfaceData dst, Composite comp, Region clip, int srcx, int srcy, int dstx,
            int dsty, int width, int height) {
            tracePrimitive(target);
            target.Blit(src, dst, comp, clip, srcx, srcy, dstx, dsty, width, height);
        }
    }
}
