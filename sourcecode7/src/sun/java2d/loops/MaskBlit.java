...
public class MaskBlit extends GraphicsPrimitive {
     ...
     */
    public native void MaskBlit(SurfaceData src, SurfaceData dst, Composite comp, Region clip, int srcx, int srcy,
        int dstx, int dsty, int width, int height, byte[] mask, int maskoff, int maskscan);
    ...
    private static class General extends MaskBlit implements GeneralBinaryOp {
        ...
        public synchronized void MaskBlit(SurfaceData srcData, SurfaceData dstData, Composite comp, Region clip,
            int srcx, int srcy, int dstx, int dsty, int width, int height, byte mask[], int offset, int scan) {
            SurfaceData src, dst;
            Region opclip;
            int sx, sy, dx, dy;
            ...
            performop.MaskBlit(src, dst, comp, opclip, sx, sy, dx, dy, width, height, mask, offset, scan);
    ...
    private static class TraceMaskBlit extends MaskBlit {
        ...
        public void MaskBlit(SurfaceData src, SurfaceData dst, Composite comp, Region clip, int srcx, int srcy,
            int dstx, int dsty, int width, int height, byte[] mask, int maskoff, int maskscan) {
            tracePrimitive(target);
            target.MaskBlit(src, dst, comp, clip, srcx, srcy, dstx, dsty, width, height, mask, maskoff, maskscan);
        }
    }
}
