...
public abstract class SurfaceData implements Transparency, DisposerTarget, StateTrackable, Surface {
    ...
    static class PixelToPgramLoopConverter extends PixelToParallelogramConverter implements LoopBasedPipe {
        public PixelToPgramLoopConverter(ShapeDrawPipe shapepipe, ParallelogramPipe pgrampipe, double minPenSize,
    ...
    public boolean canRenderLCDText(SunGraphics2D sg2d) {
        ...
        if (sg2d.compositeState <= SunGraphics2D.COMP_ISCOPY && sg2d.paintState <= SunGraphics2D.PAINT_ALPHACOLOR &&
            sg2d.clipState <= SunGraphics2D.CLIP_RECTANGULAR &&
            sg2d.surfaceData.getTransparency() == Transparency.OPAQUE) {
            if (haveLCDLoop == LOOP_UNKNOWN) {
                DrawGlyphListLCD loop = DrawGlyphListLCD
                    .locate(SurfaceType.AnyColor, CompositeType.SrcNoEa, getSurfaceType());
        ...
        if (sg2d.paintState <= sg2d.PAINT_ALPHACOLOR) {
            if (sg2d.compositeState == sg2d.COMP_XOR) {
                if (havePgramXORLoop == LOOP_UNKNOWN) {
                    FillParallelogram loop = FillParallelogram
                        .locate(SurfaceType.AnyColor, CompositeType.Xor, getSurfaceType());
                    havePgramXORLoop = (loop != null) ? LOOP_FOUND : LOOP_NOTFOUND;
                }
                return havePgramXORLoop == LOOP_FOUND;
            } else if (sg2d.compositeState <= sg2d.COMP_ISCOPY && sg2d.antialiasHint != SunHints.INTVAL_ANTIALIAS_ON &&
    ...
    public void validatePipe(SunGraphics2D sg2d) {
        ...
        if (sg2d.compositeState == sg2d.COMP_XOR) {
            ...
            if (sg2d.alphafill != null) {
                if (sg2d.clipState == sg2d.CLIP_SHAPE) {
                    ...
                    PixelToParallelogramConverter converter = (sg2d.alphafill.canDoParallelograms() ? AAColorViaPgram
                        : AAColorViaShape);
                    sg2d.drawpipe = converter;
                    sg2d.fillpipe = converter;
        ...
        } else if (sg2d.paintState > sg2d.PAINT_ALPHACOLOR || sg2d.compositeState > sg2d.COMP_ISCOPY ||
            sg2d.clipState == sg2d.CLIP_SHAPE) {
            sg2d.drawpipe = paintViaShape;
            sg2d.fillpipe = paintViaShape;
            sg2d.shapepipe = paintShape;
        ...
        // check for loops
        if (sg2d.textpipe instanceof LoopBasedPipe || sg2d.shapepipe instanceof LoopBasedPipe ||
            sg2d.fillpipe instanceof LoopBasedPipe || sg2d.drawpipe instanceof LoopBasedPipe ||
