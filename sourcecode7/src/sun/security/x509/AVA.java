...
public class AVA implements DerEncoder {
    ...
    private static final boolean PRESERVE_OLD_DC_ENCODING = AccessController
        .doPrivileged(new GetBooleanAction("com.sun.security.preserveOldDCEncoding"));
    ...
    private DerValue parseString(Reader in, int c, int format, StringBuilder temp) throws IOException {
        ...
        do {
            ...
            if (c == '\\') {
                ...
                if ((format == DEFAULT && specialCharsAll.indexOf((char) c) == -1) ||
                    (format == RFC1779 && specialChars.indexOf((char) c) == -1 && c != '\\' && c != '\"')) {
                    ...
                    if (c == ' ') {
                        ...
                        if (!leadingChar && !trailingSpace(in)) {
                            throw new IOException(
                                "Invalid escaped space character " + "in AVA.  Only a leading or trailing " +
                        ...
                        if (!leadingChar) {
                            throw new IOException(
                                "Invalid escaped '#' character in AVA.  " + "Only a leading '#' can be escaped.");
                        }
    ...
    public String toString() {
        return toKeywordValueString(toKeyword(DEFAULT, Collections.<String, String>emptyMap()));
    }
            ...
            for (int i = 0; i < valStr.length(); i++) {
                ...
                if (DerValue.isPrintableStringChar(c) || escapees.indexOf(c) >= 0) {
                    ...
                    for (int j = 0; j < valueBytes.length; j++) {
                        sbuffer.append('\\');
                        char hexChar = Character.forDigit(0xF & (valueBytes[j] >>> 4), 16);
                        sbuffer.append(Character.toUpperCase(hexChar));
                        hexChar = Character.forDigit(0xF & (valueBytes[j]), 16);
                        sbuffer.append(Character.toUpperCase(hexChar));
                    }
                } else {
...
class AVAKeyword {
    ...
    static ObjectIdentifier getOID(String keyword, int standard) throws IOException {
        return getOID(keyword, standard, Collections.<String, String>emptyMap());
    }
    ...
    static String getKeyword(ObjectIdentifier oid, int standard) {
        return getKeyword(oid, standard, Collections.<String, String>emptyMap());
    }
    ...
    static String getKeyword(ObjectIdentifier oid, int standard, Map<String, String> extraOidMap) {
        ...
        if (keywordString == null) {
            ...
            for (int i = 1; i < keywordString.length(); i++) {
                ...
                if ((c < 65 || c > 122 || (c > 90 && c < 97)) && (c < 48 || c > 57) && c != '_') {
