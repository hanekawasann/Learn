...
public class IPAddressName implements GeneralNameInterface {
    ...
    private void parseIPv6(String name) throws IOException {
        ...
        if (slashNdx == -1) {
            ...
            if (prefixLen > 128) { throw new IOException("IPv6Address prefix is longer than 128"); }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof IPAddressName)) { return false; }
        ...
        if (address.length == 8 || address.length == 32) {
            ...
            // Now compare masks
            for (int i = maskLen; i < address.length; i++) { if (address[i] != other[i]) { return false; } }
            return true;
        } else {
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < address.length; i++) { retval += address[i] * i; }
    ...
    public int constrains(GeneralNameInterface inputName) throws UnsupportedOperationException {
        int constraintType;
        if (inputName == null) { constraintType = NAME_DIFF_TYPE; } else if (inputName.getType() != NAME_IP) {
            constraintType = NAME_DIFF_TYPE;
        } else if (((IPAddressName) inputName).equals(this)) { constraintType = NAME_MATCH; } else {
            byte[] otherAddress = ((IPAddressName) inputName).getBytes();
            if (otherAddress.length == 4 && address.length == 4)
            // Two host addresses
            { constraintType = NAME_SAME_TYPE; } else if ((otherAddress.length == 8 && address.length == 8) ||
                ...
                for (int i = 0; i < maskOffset; i++) {
                    if ((byte) (address[i] & address[i + maskOffset]) != address[i]) { thisEmpty = true; }
                    if ((byte) (otherAddress[i] & otherAddress[i + maskOffset]) != otherAddress[i]) {
                        otherEmpty = true;
                    }
                    if (!(
                        ((byte) (address[i + maskOffset] & otherAddress[i + maskOffset]) == address[i + maskOffset]) &&
                            ((byte) (address[i] & address[i + maskOffset]) ==
                                (byte) (otherAddress[i] & address[i + maskOffset])))) {
                        otherSubsetOfThis = false;
                    }
                    if (!(((byte) (otherAddress[i + maskOffset] & address[i + maskOffset]) ==
                        otherAddress[i + maskOffset]) && ((byte) (otherAddress[i] & otherAddress[i + maskOffset]) ==
                        (byte) (address[i] & otherAddress[i + maskOffset])))) {
                        thisSubsetOfOther = false;
                    }
                }
                if (thisEmpty || otherEmpty) {
                    if (thisEmpty && otherEmpty) { constraintType = NAME_MATCH; } else if (thisEmpty) {
                        constraintType = NAME_WIDENS;
                    } else { constraintType = NAME_NARROWS; }
                } else if (otherSubsetOfThis) { constraintType = NAME_NARROWS; } else if (thisSubsetOfOther) {
                    constraintType = NAME_WIDENS;
                } else { constraintType = NAME_SAME_TYPE; }
                ...
                for (; i < maskOffset; i++) {
                    ...
                    if ((address[i] & otherAddress[i + maskOffset]) != otherAddress[i]) { break; }
                }
                if (i == maskOffset) { constraintType = NAME_WIDENS; } else { constraintType = NAME_SAME_TYPE; }
                ...
                for (; i < maskOffset; i++) {
                    ...
                    if ((otherAddress[i] & address[i + maskOffset]) != address[i]) { break; }
                }
                if (i == maskOffset) { constraintType = NAME_NARROWS; } else { constraintType = NAME_SAME_TYPE; }
            } else {
