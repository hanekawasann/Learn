...
public class InhibitAnyPolicyExtension extends Extension implements CertAttrSet<String> {
    ...
    public InhibitAnyPolicyExtension(int skipCerts) throws IOException {
        if (skipCerts < -1) { throw new IOException("Invalid value for skipCerts"); }
        if (skipCerts == -1) { this.skipCerts = Integer.MAX_VALUE; } else { this.skipCerts = skipCerts; }
    ...
    public InhibitAnyPolicyExtension(Boolean critical, Object value) throws IOException {
        ...
        if (val.data == null) { throw new IOException("Invalid encoding of InhibitAnyPolicy: " + "null data"); }
        int skipCertsValue = val.getInteger();
        if (skipCertsValue < -1) { throw new IOException("Invalid value for skipCerts"); }
        if (skipCertsValue == -1) {
    ...
    public void set(String name, Object obj) throws IOException {
        if (name.equalsIgnoreCase(SKIP_CERTS)) {
            if (!(obj instanceof Integer)) { throw new IOException("Attribute value should be of type Integer."); }
    ...
    public Object get(String name) throws IOException {
        if (name.equalsIgnoreCase(SKIP_CERTS)) { return (new Integer(skipCerts)); } else {
