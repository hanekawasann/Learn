...
public class PolicyConstraintsExtension extends Extension implements CertAttrSet<String> {
            ...
            if (next.isContextSpecific(TAG_REQUIRE) && !next.isConstructed()) {
                if (this.require != -1) {
                    throw new IOException(
                        "Duplicate requireExplicitPolicy" + "found in the PolicyConstraintsExtension");
                }
                next.resetTag(DerValue.tag_Integer);
                this.require = next.getInteger();

            } else if (next.isContextSpecific(TAG_INHIBIT) && !next.isConstructed()) {
                if (this.inhibit != -1) {
                    throw new IOException("Duplicate inhibitPolicyMapping" + "found in the PolicyConstraintsExtension");
                }
                next.resetTag(DerValue.tag_Integer);
                this.inhibit = next.getInteger();
            } else { throw new IOException("Invalid encoding of PolicyConstraint"); }
        }
    }
    ...
    public String toString() {
        ...
        if (require == -1) { s += "unspecified;"; } else { s += require + ";"; }
        s += "\tInhibit: ";
        if (inhibit == -1) { s += "unspecified"; } else { s += inhibit; }
        s += " ]\n";
        return s;
    }
    ...
    public void set(String name, Object obj) throws IOException {
        ...
        if (name.equalsIgnoreCase(REQUIRE)) {
            ...
            throw new IOException(
                "Attribute name " + "[" + name + "]" + " not recognized by " + "CertAttrSet:PolicyConstraints.");
        }
        encodeThis();
    }
