...
public class EDIPartyName implements GeneralNameInterface {
    ...
    public EDIPartyName(DerValue derValue) throws IOException {
        ...
        int len = seq.length;
        if (len < 1 || len > 2) { throw new IOException("Invalid encoding of EDIPartyName"); }

        for (int i = 0; i < len; i++) {
            DerValue opt = seq[i];
            if (opt.isContextSpecific(TAG_ASSIGNER) && !opt.isConstructed()) {
                if (assigner != null) { throw new IOException("Duplicate nameAssigner found in" + " EDIPartyName"); }
                opt = opt.data.getDerValue();
                assigner = opt.getAsString();
            }
            if (opt.isContextSpecific(TAG_PARTYNAME) && !opt.isConstructed()) {
                if (party != null) { throw new IOException("Duplicate partyName found in" + " EDIPartyName"); }
                opt = opt.data.getDerValue();
    ...
    public void encode(DerOutputStream out) throws IOException {
        ...
        if (party == null) { throw new IOException("Cannot have null partyName"); }
    ...
    public boolean equals(Object other) {
        if (!(other instanceof EDIPartyName)) { return false; }
        String otherAssigner = ((EDIPartyName) other).assigner;
        if (this.assigner == null) {
            if (otherAssigner != null) { return false; }
        } else {
            if (!(this.assigner.equals(otherAssigner))) { return false; }
        }
        String otherParty = ((EDIPartyName) other).party;
        if (this.party == null) {
            if (otherParty != null) { return false; }
        } else {
            if (!(this.party.equals(otherParty))) { return false; }
        }
        return true;
    }
    ...
    public String toString() {
        return ("EDIPartyName: " + ((assigner == null) ? "" : ("  nameAssigner = " + assigner + ",")) +
            "  partyName = " + party);
    }
    ...
    public int constrains(GeneralNameInterface inputName) throws UnsupportedOperationException {
        int constraintType;
        if (inputName == null) { constraintType = NAME_DIFF_TYPE; } else if (inputName.getType() != NAME_EDI) {
