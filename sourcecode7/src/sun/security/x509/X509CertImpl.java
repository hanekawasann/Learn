...
public class X509CertImpl extends X509Certificate implements DerEncoder {
    ...
    public static final String SUBJECT_DN = NAME + DOT + INFO + DOT + X509CertInfo.SUBJECT + DOT +
        CertificateSubjectName.DN_NAME;
    // x509.info.issuer.dname
    public static final String ISSUER_DN = NAME + DOT + INFO + DOT + X509CertInfo.ISSUER + DOT +
            ...
            try {
                ...
                CertificateException ce = new CertificateException(
                    "Input stream must be " + "either DER-encoded bytes " + "or RFC1421 hex-encoded " +
                        "DER-encoded bytes: " + ioe1.getMessage());
                ce.initCause(ioe1);
                throw ce;
            }
        }
    ...
    public Principal getSubjectDN() {
        if (info == null) { return null; }
        try {
            Principal subject = (Principal) info
                .get(CertificateSubjectName.NAME + DOT + CertificateSubjectName.DN_NAME);
            return subject;
        } catch (Exception e) {
            return null;
        }
    ...
    public X500Principal getSubjectX500Principal() {
        ...
        try {
            X500Principal subject = (X500Principal) info
                .get(CertificateSubjectName.NAME + DOT + CertificateSubjectName.DN_PRINCIPAL);
            return subject;
        } catch (Exception e) {
            return null;
        }
    ...
    public X500Principal getIssuerX500Principal() {
        ...
        try {
            X500Principal issuer = (X500Principal) info
                .get(CertificateIssuerName.NAME + DOT + CertificateIssuerName.DN_PRINCIPAL);
            return issuer;
        } catch (Exception e) {
            return null;
        }
    ...
    public boolean[] getIssuerUniqueID() {
        if (info == null) { return null; }
        try {
            UniqueIdentity id = (UniqueIdentity) info
                .get(CertificateIssuerUniqueIdentity.NAME + DOT + CertificateIssuerUniqueIdentity.ID);
    ...
    public boolean[] getSubjectUniqueID() {
        ...
        try {
            UniqueIdentity id = (UniqueIdentity) info
                .get(CertificateSubjectUniqueIdentity.NAME + DOT + CertificateSubjectUniqueIdentity.ID);
            if (id == null) { return null; } else { return (id.getId()); }
        } catch (Exception e) {
            return null;
        }
