...
public class NameConstraintsExtension extends Extension implements CertAttrSet<String>, Cloneable {
    ...
    private void calcMinMax() throws IOException {
        ...
        if (excluded != null) {
            for (int i = 0; i < excluded.size(); i++) {
                ...
                if (subtree.getMinimum() != 0) { hasMin = true; }
                if (subtree.getMaximum() != -1) { hasMax = true; }
            }
        }
    ...
    public NameConstraintsExtension(Boolean critical, Object value) throws IOException {
        ...
        while (val.data.available() != 0) {
            ...
            } else if (opt.isContextSpecific(TAG_EXCLUDED) && opt.isConstructed()) {
                if (excluded != null) {
                    throw new IOException("Duplicate excluded " + "GeneralSubtrees in NameConstraintsExtension.");
                }
                opt.resetTag(DerValue.tag_Sequence);
                excluded = new GeneralSubtrees(opt);
            } else { throw new IOException("Invalid encoding of " + "NameConstraintsExtension."); }
        }
        minMaxValid = false;
    }
    ...
    public boolean verify(X509Certificate cert) throws IOException {
        ...
        try {
            ...
            if (altNameExt != null) {
                ...
                altNames = (GeneralNames) (altNameExt.get(altNameExt.SUBJECT_NAME));
            }
    ...
    public Object clone() {
        try {
            ...
            throw new RuntimeException(
                "CloneNotSupportedException while " + "cloning NameConstraintsException. This should never happen.");
        }
    }
}
