...
public final class RSAKeyFactory extends KeyFactorySpi {
    ...
    private final static Class<?> rsaPublicKeySpecClass = RSAPublicKeySpec.class;
    private final static Class<?> rsaPrivateKeySpecClass = RSAPrivateKeySpec.class;
    private final static Class<?> rsaPrivateCrtKeySpecClass = RSAPrivateCrtKeySpec.class;
    ...
    private static final boolean restrictExpLen = "true".equalsIgnoreCase(
        AccessController.doPrivileged(new GetPropertyAction("sun.security.rsa.restrictRSAExponent", "true")));
    ...
    public static RSAKey toRSAKey(Key key) throws InvalidKeyException {
        if ((key instanceof RSAPrivateKeyImpl) || (key instanceof RSAPrivateCrtKeyImpl) ||
    ...
    public static void checkKeyLengths(int modulusLen, BigInteger exponent, int minModulusLen, int maxModulusLen)
        ...
        if (restrictExpLen && (exponent != null) && (modulusLen > MAX_MODLEN_RESTRICT_EXP) &&
            ...
            throw new InvalidKeyException("RSA exponents can be no longer than " + MAX_RESTRICTED_EXPLEN + " bits " +
    ...
    private PrivateKey translatePrivateKey(PrivateKey key) throws InvalidKeyException {
        if (key instanceof RSAPrivateCrtKey) {
            ...
            try {
                return new RSAPrivateCrtKeyImpl(rsaKey.getModulus(), rsaKey.getPublicExponent(),
    ...
    private PrivateKey generatePrivate(KeySpec keySpec) throws GeneralSecurityException {
        if (keySpec instanceof PKCS8EncodedKeySpec) {
            ...
            return new RSAPrivateCrtKeyImpl(rsaSpec.getModulus(), rsaSpec.getPublicExponent(),
                rsaSpec.getPrivateExponent(), rsaSpec.getPrimeP(), rsaSpec.getPrimeQ(), rsaSpec.getPrimeExponentP(),
                rsaSpec.getPrimeExponentQ(), rsaSpec.getCrtCoefficient());
        ...
        if (key instanceof RSAPublicKey) {
            ...
            if (pkcs8KeySpecClass.isAssignableFrom(keySpec)) {
                ...
                if (key instanceof RSAPrivateCrtKey) {
                    ...
                    return (T) new RSAPrivateCrtKeySpec(crtKey.getModulus(), crtKey.getPublicExponent(),
                ...
                throw new InvalidKeySpecException(
                    "KeySpec must be RSAPrivate(Crt)KeySpec or " + "PKCS8EncodedKeySpec for RSA private keys");
            }
        } else {
