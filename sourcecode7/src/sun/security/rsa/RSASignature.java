...
public abstract class RSASignature extends SignatureSpi {
    ...
    private void initCommon(RSAKey rsaKey, SecureRandom random) throws InvalidKeyException {
        ...
        try {
            padding = RSAPadding.getInstance(RSAPadding.PAD_BLOCKTYPE_1, keySize, random);
    ...
    protected boolean engineVerify(byte[] sigBytes) throws SignatureException {
        if (sigBytes.length != RSACore.getByteLength(publicKey)) {
            throw new SignatureException(
                "Signature length not correct: got " + sigBytes.length + " but was expecting " +
     ...
     */
    public static byte[] decodeSignature(ObjectIdentifier oid, byte[] signature) throws IOException {
