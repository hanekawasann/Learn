...
public class GSSUtil {
    ...
    static {
        DEBUG = (AccessController.doPrivileged(new GetBooleanAction("sun.security.jgss.debug"))).
            booleanValue();
    }
        ...
        while (elements.hasNext()) {
            ...
            if (cred instanceof KerberosTicket) {
                if (!cred.getClass().getName().equals("javax.security.auth.kerberos.KerberosTicket")) {
                    ...
                    cred = new KerberosTicket(tempTkt.getEncoded(), tempTkt.getClient(), tempTkt.getServer(),
                        tempTkt.getSessionKey().getEncoded(), tempTkt.getSessionKeyType(), tempTkt.getFlags(),
                        tempTkt.getAuthTime(), tempTkt.getStartTime(), tempTkt.getEndTime(), tempTkt.getRenewTill(),
                ...
                if (!cred.getClass().getName().equals("javax.security.auth.kerberos.KerberosKey")) {
                    ...
                    cred = new KerberosKey(tempKey.getPrincipal(), tempKey.getEncoded(), tempKey.getKeyType(),
    ...
    public static boolean useMSInterop() {
         ...
         */
        String propValue = AccessController
            .doPrivileged(new GetPropertyAction("sun.security.spnego.msinterop", "true"));
        /*
         * This property has to be explicitly set to "false". Invalid
         * values should be ignored and the default "true" assumed.
         */
     ...
     */
    public static Vector searchSubject(final GSSNameSpi name, final Oid mech, final boolean initiate,
        final Class credCls) {
        debug("Search Subject for " + getMechStr(mech) + (initiate ? " INIT" : " ACCEPT") + " cred (" +
            (name == null ? "<<DEF>>" : name.toString()) + ", " + credCls.getName() + ")");
        final AccessControlContext acc = AccessController.getContext();
        try {
            Vector creds = AccessController.doPrivileged(new PrivilegedExceptionAction<Vector>() {
                public Vector run() throws Exception {
                    Subject accSubj = Subject.getSubject(acc);
                    Vector<GSSCredentialSpi> result = null;
                    if (accSubj != null) {
                        result = new Vector<GSSCredentialSpi>();
                        Iterator<GSSCredentialImpl> iterator = accSubj.getPrivateCredentials(GSSCredentialImpl.class)
                            .iterator();
                        while (iterator.hasNext()) {
                            ...
                            try {
                                ...
                                if (ce.getClass().equals(credCls) &&
                                    (name == null || name.equals((Object) ce.getName()))) {
                                    result.add(ce);
                                } else {
