...
public final class ProviderList {
    ...
    private ArrayList<PreferencesEntry> preferences = new ArrayList<PreferencesEntry>(5);
    private HashMap<PreferencesEntry, MechanismFactory> factories = new HashMap<PreferencesEntry, MechanismFactory>(5);
    ...
    private boolean isMechFactoryProperty(String prop) {
        return (prop.startsWith(PROV_PROP_PREFIX) || prop.regionMatches(true, 0, // Try ignoring case
            PROV_PROP_PREFIX, 0, PROV_PROP_PREFIX_LEN));
    }
    ...
    synchronized public MechanismFactory getMechFactory(Oid mechOid) throws GSSException {
     ...
     */
    synchronized public MechanismFactory getMechFactory(Oid mechOid, Provider p) throws GSSException {
            ...
            if (className != null) {
                ...
                if (e.getOid() != null) {
                    throw new GSSExceptionImpl(GSSException.BAD_MECH,
                        "Provider " + p.getName() + " does not support mechanism " + mechOid);
                }
            }
        }
        return retVal;
     ...
     */
    private static MechanismFactory getMechFactoryImpl(Provider p, String className, Oid mechOid, GSSCaller caller)
        throws GSSException {
        ...
        try {
            ...
            if (baseClass.isAssignableFrom(implClass)) {
                ...
                java.lang.reflect.Constructor<?> c = implClass.getConstructor(GSSCaller.class);
                ...
                throw createGSSException(p, className, "is not a " + SPI_MECH_FACTORY_TYPE, null);
            }
    ...
    private static GSSException createGSSException(Provider p, String className, String trailingMsg, Exception cause) {
        ...
        return new GSSExceptionImpl(GSSException.BAD_MECH, errClassInfo + trailingMsg, cause);
    }
    ...
    synchronized public void addProviderAtFront(Provider p, Oid mechOid) throws GSSException {
        ...
        while (list.hasNext()) {
            oldEntry = list.next();
            if (newEntry.implies(oldEntry)) { list.remove(); }
        }
        ...
        if (mechOid == null) {
            ...
            if (p.getProperty(PROV_PROP_PREFIX + oidStr) == null) {
                throw new GSSExceptionImpl(GSSException.BAD_MECH,
                    "Provider " + p.getName() + " does not support " + oidStr);
            }
            mechs.add(mechOid);
            foundSomeMech = true;
        }
    ...
    synchronized public void addProviderAtEnd(Provider p, Oid mechOid) throws GSSException {
        ...
        while (list.hasNext()) {
            oldEntry = list.next();
            if (oldEntry.implies(newEntry)) { return; }
        }

        // System.out.println("addProviderAtEnd: No it is not redundant");

        if (mechOid == null) { foundSomeMech = addAllMechsFromProvider(p); } else {
            String oidStr = mechOid.toString();
            if (p.getProperty(PROV_PROP_PREFIX + oidStr) == null) {
                throw new GSSExceptionImpl(GSSException.BAD_MECH,
                    "Provider " + p.getName() + " does not support " + oidStr);
    ...
    private static final class PreferencesEntry {
        ...
        boolean implies(Object other) {
            ...
            if (other instanceof PreferencesEntry) {
                ...
                return (equals(temp) || p.getName().equals(temp.p.getName()) && oid == null);
            } else {
                return false;
            }
        }
