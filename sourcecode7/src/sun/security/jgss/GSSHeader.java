...
public class GSSHeader {
    ...
    public GSSHeader(InputStream is) throws IOException, GSSException {
        ...
        if (tag != TOKEN_ID) {
            throw new GSSException(GSSException.DEFECTIVE_TOKEN, -1, "GSSHeader did not find the right tag");
        }
    ...
    private int getLength(int lenByte, InputStream in) throws IOException {
        ...
        if ((tmp & 0x080) == 0x00) { // short form, 1 byte datum
            ...
            if (tmp == 0) { return -1; }
            if (tmp < 0 || tmp > 4) {
                throw new IOException("DerInputStream.getLength(): lengthTag=" + tmp + ", " +
