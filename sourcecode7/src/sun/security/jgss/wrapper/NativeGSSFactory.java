...
public final class NativeGSSFactory implements MechanismFactory {
    ...
    private GSSCredElement getCredFromSubject(GSSNameElement name, boolean initiate) throws GSSException {
        ...
        Vector<GSSCredElement> creds = GSSUtil.searchSubject(name, mech, initiate, GSSCredElement.class);
        ...
        GSSCredElement result = ((creds == null || creds.isEmpty()) ? null : creds.firstElement());
    ...
    public GSSCredentialSpi getCredentialElement(GSSNameSpi name, int initLifetime, int acceptLifetime, int usage)
        ...
        if (name != null && !(name instanceof GSSNameElement)) {
            nname = (GSSNameElement) getNameElement(name.toString(), name.getStringNameType());
        ...
        GSSCredElement credElement = getCredFromSubject((GSSNameElement) nname, (usage == GSSCredential.INITIATE_ONLY));

        if (credElement == null) {
            // No cred in the Subject
            if (usage == GSSCredential.INITIATE_ONLY) {
                credElement = new GSSCredElement(nname, initLifetime, usage, cStub);
            } else if (usage == GSSCredential.ACCEPT_ONLY) {
                if (nname == null) {
                    nname = GSSNameElement.DEF_ACCEPTOR;
                }
                credElement = new GSSCredElement(nname, acceptLifetime, usage, cStub);
            } else {
    ...
    public GSSContextSpi getMechanismContext(GSSNameSpi peer, GSSCredentialSpi myCred, int lifetime)
        throws GSSException {
        if (peer == null) {
            throw new GSSException(GSSException.BAD_NAME);
        } else if (!(peer instanceof GSSNameElement)) {
            peer = (GSSNameElement) getNameElement(peer.toString(), peer.getStringNameType());
        }
        if (myCred == null) {
        ...
        return new NativeGSSContext((GSSNameElement) peer, (GSSCredElement) myCred, lifetime, cStub);
    }
