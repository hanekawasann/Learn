...
abstract class MessageToken extends Krb5Token {
    ...
    /* cipher instance used by the corresponding GSSContext */ CipherHelper cipherHelper = null;
     ...
     */
    MessageToken(int tokenId, Krb5Context context, byte[] tokenBytes, int tokenOffset, int tokenLen, MessageProp prop)
        throws GSSException {
     ...
     */
    public void genSignAndSeqNumber(MessageProp prop, byte[] optionalHeader, byte[] data, int offset, int len,
        byte[] optionalTrailer) throws GSSException {
     ...
     *                      encryption sequence number calculation.
     */
    public final boolean verifySignAndSeqNumber(byte[] optionalHeader, byte[] data, int offset, int len,
        byte[] optionalTrailer) throws GSSException {
        // debug("\tIn verifySign:\n");
        ...
        if (MessageDigest.isEqual(checksum, myChecksum)) {
            ...
            if ((seqNumberData[4] == directionByte) && (seqNumberData[5] == directionByte) &&
                (seqNumberData[6] == directionByte) && (seqNumberData[7] == directionByte)) { return true; }
        }
     ...
     */
    private byte[] getChecksum(byte[] optionalHeader, byte[] data, int offset, int len, byte[] optionalTrailer)
        throws GSSException {
        ...
        return cipherHelper
            .calculateChecksum(tokenHeader.getSignAlg(), checksumDataHeader, optionalTrailer, data, offset, len,
                tokenId);
    }
