...
public class Krb5NameElement implements GSSNameSpi {
    ...
    static Krb5NameElement getInstance(String gssNameStr, Oid gssNameType) throws GSSException {
         ...
         */
        if (gssNameType == null) { gssNameType = Krb5MechFactory.NT_GSS_KRB5_PRINCIPAL; } else if (!gssNameType.equals(GSSName.NT_USER_NAME) && !gssNameType.equals(GSSName.NT_HOSTBASED_SERVICE) &&
            !gssNameType.equals(Krb5MechFactory.NT_GSS_KRB5_PRINCIPAL) && !gssNameType.equals(GSSName.NT_EXPORT_NAME)) {
            throw new GSSException(GSSException.BAD_NAMETYPE, -1,
                gssNameType.toString() + " is an unsupported nametype");
        }
        ...
        try {
            ...
            if (gssNameType.equals(GSSName.NT_EXPORT_NAME) ||
                ...
                if (gssNameType.equals(GSSName.NT_USER_NAME)) {
                    ...
                    if (components.length >= 2) { hostName = components[1]; }

                    String principal = getHostBasedInstance(service, hostName);
                    principalName = new ServiceName(principal, PrincipalName.KRB_NT_SRV_HST);
                }
            }
    ...
    private static String[] getComponents(String gssNameStr) throws GSSException {
        ...
        if ((separatorPos > 0) && (gssNameStr.charAt(separatorPos - 1) == '\\')) {
            // Is the `\` character escaped itself?
            if ((separatorPos - 2 < 0) || (gssNameStr.charAt(separatorPos - 2) != '\\')) { separatorPos = -1; }
        }
     ...
     * @throws GSSException with major codes of BAD_NAMETYPE,
     *                      BAD_NAME, FAILURE
    ...
    public boolean equals(Object another) {
        ...
        try {
            if (another instanceof Krb5NameElement) { return equals((Krb5NameElement) another); }
