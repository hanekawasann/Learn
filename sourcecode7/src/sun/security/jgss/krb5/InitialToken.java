...
abstract class InitialToken extends Krb5Token {
    ...
    private final byte[] CHECKSUM_FIRST_BYTES = { (byte) 0x10, (byte) 0x00, (byte) 0x00, (byte) 0x00 };
    ...
    protected class OverloadedChecksum {
         ...
         * InitSecContextToken.
         */
        public OverloadedChecksum(Krb5Context context, Credentials tgt, Credentials serviceTicket)
            ...
            if (context.getCredDelegState()) {
                ...
                if (krbCredMessage.length > 0x0000ffff) {
                    throw new GSSException(GSSException.FAILURE, -1, "Incorrect messsage length");
                }
        ...
        // internal.
        public OverloadedChecksum(Krb5Context context, Checksum checksum, EncryptionKey key, EncryptionKey subKey)
            ...
            if ((checksumBytes[0] != CHECKSUM_FIRST_BYTES[0]) || (checksumBytes[1] != CHECKSUM_FIRST_BYTES[1]) ||
                (checksumBytes[2] != CHECKSUM_FIRST_BYTES[2]) || (checksumBytes[3] != CHECKSUM_FIRST_BYTES[3])) {
            ...
            if ((flags & CHECKSUM_DELEG_FLAG) > 0) {
                ...
                if (useNullKey(cipherHelper)) {
                    delegCreds = new KrbCred(credBytes, EncryptionKey.NULL_KEY).
                        getDelegatedCreds()[0];
                } else {
                    KrbCred cred;
                    try {
        ...
        public void setContextFlags(Krb5Context context) {
            ...
            if ((flags & CHECKSUM_DELEG_FLAG) > 0) { context.setCredDelegState(true); }
    ...
    private int getAddrType(InetAddress addr) {
        ...
        } else if (addr instanceof Inet6Address) { addressType = CHANNEL_BINDING_AF_INET6; }
        return (addressType);
    }
