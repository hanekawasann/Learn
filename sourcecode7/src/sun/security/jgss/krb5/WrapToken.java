...
class WrapToken extends MessageToken {
     ...
     */
    static final byte[][] pads = { null, // No, no one escapes padding
     ...
     */
    public WrapToken(Krb5Context context, byte[] tokenBytes, int tokenOffset, int tokenLen, MessageProp prop)
        throws GSSException {
    ...
    public int getData(byte[] dataBuf, int dataBufOffset) throws GSSException {
        ...
        if (readTokenFromInputStream) { getDataFromStream(dataBuf, dataBufOffset); } else {
    ...
    private void getDataFromBuffer(byte[] dataBuf, int dataBufOffset) throws GSSException {
        ...
        if (dataPos + dataSize > tokenOffset + tokenLen) {
            throw new GSSException(GSSException.DEFECTIVE_TOKEN, -1,
                "Insufficient data in " + getTokenName(getTokenId()));
        }
        ...
        if (privacy) {
            ...
            */
            ...
            System.arraycopy(tokenBytes, dataPos + CONFOUNDER_SIZE, dataBuf, dataBufOffset,
                dataSize - CONFOUNDER_SIZE - padSize);
        ...
        if (!verifySignAndSeqNumber(confounder, dataBuf, dataBufOffset, dataSize - CONFOUNDER_SIZE - padding.length,
            padding)) {
            throw new GSSException(GSSException.BAD_MIC, -1, "Corrupt checksum or sequence number in Wrap token");
        }
    }
    ...
    private void getDataFromStream(byte[] dataBuf, int dataBufOffset) throws GSSException {
        ...
        try {
            ...
            throw new GSSException(GSSException.DEFECTIVE_TOKEN, -1,
                getTokenName(getTokenId()) + ": " + e.getMessage());
        }
    ...
    public WrapToken(Krb5Context context, MessageProp prop, byte[] dataBytes, int dataOffset, int dataLen)
        throws GSSException {
    ...
    static int getSizeLimit(int qop, boolean confReq, int maxTokenSize, CipherHelper ch) throws GSSException {
        return (GSSHeader.getMaxMechTokenSize(OID, maxTokenSize) - (getTokenSize(ch) + CONFOUNDER_SIZE) -
            8); /* safety */
    }
