...
public class GSSManagerImpl extends GSSManager {
    ...
    static {
        USE_NATIVE = AccessController.doPrivileged(new PrivilegedAction<Boolean>() {
            public Boolean run() {
                String osname = System.getProperty("os.name");
                if (osname.startsWith("SunOS") || osname.startsWith("Linux")) {
                    return new Boolean(System.getProperty(USE_NATIVE_PROP));
                }
    ...
    public Oid[] getMechsForName(Oid nameType) {
        ...
        if (pos < retVal.length) {
            ...
            for (int i = 0; i < pos; i++) { temp[i] = retVal[i]; }
            retVal = temp;
        }
    ...
    public GSSCredentialSpi getCredentialElement(GSSNameSpi name, int initLifetime, int acceptLifetime, Oid mech,
    ...
    GSSContextSpi getMechanismContext(GSSNameSpi peer, GSSCredentialSpi myInitiatorCred, int lifetime, Oid mech)
        throws GSSException {
        Provider p = null;
