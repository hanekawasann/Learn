...
public class PKCS10 {
     ...
     * @throws NoSuchAlgorithmException when the signature
    ...
    public PKCS10(byte[] data) throws IOException, SignatureException, NoSuchAlgorithmException {
        ...
        if (seq.length != 3) { throw new IllegalArgumentException("not a PKCS #10 request"); }
        ...
        if (!serial.equals(BigInteger.ZERO)) { throw new IllegalArgumentException("not PKCS #10 v1"); }

        subject = new X500Name(seq[0].data);
        subjectPublicKeyInfo = X509Key.parse(seq[0].data.getDerValue());

        // Cope with a somewhat common illegal PKCS #10 format
        if (seq[0].data.available() != 0) { attributeSet = new PKCS10Attributes(seq[0].data); } else {
            attributeSet = new PKCS10Attributes();
        }

        if (seq[0].data.available() != 0) { throw new IllegalArgumentException("illegal PKCS #10 data"); }
        ...
        try {
            ...
            sig.update(data);
            if (!sig.verify(sigData)) { throw new SignatureException("Invalid PKCS #10 signature"); }
    ...
    public byte[] getEncoded() {
        if (encoded != null) { return encoded.clone(); } else { return null; }
    }
     ...
     *            will be printed.
     * @throws IOException        when an output operation failed
     * @throws SignatureException when the certificate request was
     *                            not yet signed.
     */
    public void print(PrintStream out) throws IOException, SignatureException {
        if (encoded == null) { throw new SignatureException("Cert request was not signed"); }
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof PKCS10)) { return false; }
        if (encoded == null) // not signed yet
        { return false; }
        byte[] otherEncoded = ((PKCS10) other).getEncoded();
        if (otherEncoded == null) { return false; }
    ...
    public int hashCode() {
        int retval = 0;
        if (encoded != null) { for (int i = 1; i < encoded.length; i++) { retval += encoded[i] * i; } }
        return (retval);
    }
