...
public class HostnameChecker {
    ...
    private final static HostnameChecker INSTANCE_TLS = new HostnameChecker(TYPE_TLS);

    // Constant for a HostnameChecker for LDAP
    public final static byte TYPE_LDAP = 2;
    private final static HostnameChecker INSTANCE_LDAP = new HostnameChecker(TYPE_LDAP);
    ...
    private static boolean matchLeftmostWildcard(String name, String template) {
        ...
        if (templateIdx == -1) { templateIdx = template.length(); }
        if (nameIdx == -1) { nameIdx = name.length(); }
