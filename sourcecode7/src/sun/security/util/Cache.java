...
class MemoryCache extends Cache {
    ...
    public MemoryCache(boolean soft, int maxSize, int lifetime) {
        ...
        cacheMap = new LinkedHashMap<Object, CacheEntry>(buckets, LOAD_FACTOR, true);
    }
    ...
    public synchronized void setCapacity(int size) {
        ...
        if (size > 0 && cacheMap.size() > size) {
            ...
            for (int i = cacheMap.size() - size; i > 0; i--) {
                CacheEntry lruEntry = t.next();
                if (DEBUG) {
                    System.out.println("** capacity reset removal " + lruEntry.getKey() + " | " + lruEntry.getValue());
                }
                t.remove();
                lruEntry.invalidate();
            }
    ...
    private static class SoftCacheEntry extends SoftReference implements CacheEntry {
