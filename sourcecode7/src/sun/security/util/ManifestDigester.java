...
public class ManifestDigester {
    ...
    public ManifestDigester(byte bytes[]) {
        ...
        entries.put(MF_MAIN_ATTRS, new Entry(0, pos.endOfSection + 1, pos.startOfNext, rawBytes));
                    ...
                    try {
                        ...
                        while ((i - start) < sectionLen) {
                            if (bytes[i++] == ' ') {
                                ...
                                int wrapLen;
                                if (bytes[i - 2] == '\r') { wrapLen = i - wrapStart - 2; } else {
                        ...
                        entries.put(nameBuf.toString(), new Entry(start, sectionLen, sectionLenWithBlank, rawBytes));
    ...
    private boolean isNameAttr(byte bytes[], int start) {
        return ((bytes[start] == 'N') || (bytes[start] == 'n')) &&
            ...
            ((bytes[start + 3] == 'e') || (bytes[start + 3] == 'E')) && (bytes[start + 4] == ':') &&
            (bytes[start + 5] == ' ');
    }
