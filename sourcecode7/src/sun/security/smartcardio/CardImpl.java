...
final class CardImpl extends Card {
    ...
    public CardChannel openLogicalChannel() throws CardException {
        ...
        checkExclusive();
        try {
            byte[] response = SCardTransmit(cardId, protocol, commandOpenChannel, 0, commandOpenChannel.length);
    ...
    public synchronized void beginExclusive() throws CardException {
        ...
        if (exclusiveThread != null) {
            throw new CardException(
                "Exclusive access has already been assigned to Thread " + exclusiveThread.getName());
        }
        try {
    ...
    public String toString() {
        return "PC/SC card in " + terminal.getName() + ", protocol " + getProtocol() + ", state " + state;
    }
