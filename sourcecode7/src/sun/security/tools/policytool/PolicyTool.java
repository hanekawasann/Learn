...
public class PolicyTool {
    ...
    // for i18n
    static final java.util.ResourceBundle rb = java.util.ResourceBundle.getBundle("sun.security.util.Resources");
    ...
    void openPolicy(String filename)
        throws FileNotFoundException, PolicyParser.ParsingException, KeyStoreException, CertificateException,
        InstantiationException, MalformedURLException, IOException, NoSuchAlgorithmException, IllegalAccessException,
        NoSuchMethodException, UnrecoverableKeyException, NoSuchProviderException, ClassNotFoundException,
        PropertyExpander.ExpandException, InvocationTargetException {
        ...
        // open the keystore
        openKeyStore(parser.getKeyStoreUrl(), parser.getKeyStoreType(), parser.getKeyStoreProvider(),
            parser.getStorePassURL());
            ...
            while (perms.hasMoreElements()) {
                ...
                try {
                    ...
                    MessageFormat form = new MessageFormat(
                        rb.getString("Warning.Invalid.argument.s.for.constructor.arg"));
                    Object[] source = { pe.permission };
                    warnings.addElement(form.format(source));
                }
     ...
     * Open the KeyStore
     */
    void openKeyStore(String name, String type, String provider, String pwdURL)
        throws KeyStoreException, NoSuchAlgorithmException, UnrecoverableKeyException, IOException,
        CertificateException, NoSuchProviderException, ExpandException {
        ...
        try {
            ...
            if (cause != null && cause instanceof javax.security.auth.login.LoginException &&
    ...
    boolean addPrinEntry(PolicyEntry pe, PolicyParser.PrincipalEntry newPrin, int index) {
        ...
        if (index != -1) { prinList.set(index, newPrin); } else { prinList.add(newPrin); }
    ...
    boolean addPermEntry(PolicyEntry pe, PolicyParser.PermissionEntry newPerm, int index) {
        ...
        PolicyParser.GrantEntry grantEntry = pe.getGrantEntry();
        if (grantEntry.contains(newPerm) == true) { return false; }

        Vector<PolicyParser.PermissionEntry> permList = grantEntry.permissionEntries;
        if (index != -1) { permList.setElementAt(newPerm, index); } else { permList.addElement(newPerm); }
    ...
    void verifyPrincipal(String type, String name) throws ClassNotFoundException, InstantiationException {
        if (type.equals(PolicyParser.PrincipalEntry.WILDCARD_CLASS) || type.equals(PolicyParser.REPLACE_NAME)) {
            return;
        }
        ;
     ...
     */
    void verifyPermission(String type, String name, String actions)
...
class PolicyEntry {
    ...
    PolicyEntry(PolicyTool tool, PolicyParser.GrantEntry ge)
        throws MalformedURLException, NoSuchMethodException, ClassNotFoundException, InstantiationException,
        IllegalAccessException, InvocationTargetException, CertificateException, IOException, NoSuchAlgorithmException,
    ...
    String codebaseToString() {
        ...
        if (grantEntry.signedBy != null && grantEntry.signedBy.equals("") == false) {
            stringEntry = ((stringEntry.length() > 0) ? stringEntry.concat(", SignedBy \"" + grantEntry.signedBy + "\"")
                : stringEntry.concat("SignedBy \"" + grantEntry.signedBy + "\""));
        }

        if (stringEntry.length() == 0) { return new String("CodeBase <ALL>"); }
        return stringEntry;
    }
    ...
    PolicyParser.PermissionEntry toPermissionEntry(Permission perm) {
        ...
        // get the actions
        if (perm.getActions() != null && perm.getActions().trim() != "") { actions = perm.getActions(); }

        PolicyParser.PermissionEntry pe = new PolicyParser.PermissionEntry(perm.getClass().getName(), perm.getName(),
            actions);
        return pe;
    }
}
...
class ToolWindow extends Frame {
    ...
    private void initWindow() {
        ...
        addNewComponent(this, label, MW_FILENAME_LABEL, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            TOP_BOTTOM_PADDING);
        TextField tf = new TextField(50);
        tf.getAccessibleContext().setAccessibleName(PolicyTool.rb.getString("Policy.File."));
        tf.setEditable(false);
        addNewComponent(this, tf, MW_FILENAME_TEXTFIELD, 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            TOP_BOTTOM_PADDING);
        ...
        if (policyFile == null) {
            String userHome;
            userHome = java.security.AccessController
                .doPrivileged(new sun.security.action.GetPropertyAction("user.home"));
        ...
        try {
            ...
            // display the error
            MessageFormat form = new MessageFormat(
                PolicyTool.rb.getString("Could.not.open.policy.file.policyFile.e.toString."));
     ...
     */
    void addNewComponent(Container container, Component component, int index, int gridx, int gridy, int gridwidth,
        int gridheight, double weightx, double weighty, int fill, Insets is) {
...
class ToolDialog extends Dialog {
    ...
    void displayPolicyEntryDialog(boolean edit) {
        ...
        if (edit) {
            ...
            // get principal list
            LinkedList principals = entries[listIndex].getGrantEntry().principals;
            for (int i = 0; i < principals.size(); i++) {
                String prinString = null;
                PolicyParser.PrincipalEntry nextPrin = (PolicyParser.PrincipalEntry) principals.get(i);
        ...
        tw.addNewComponent(this, label, PE_PRIN_LABEL, 0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.BOTTOM_PADDING);
        tw.addNewComponent(this, prinList, PE_PRIN_LIST, 1, 3, 3, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.BOTTOM_PADDING);
        ...
        tw.addNewComponent(this, panel, PE_PANEL1, 0, 4, 2, 1, 0.0, 0.0, GridBagConstraints.HORIZONTAL,
            tw.LITE_BOTTOM_PADDING);

        // permission list
        tw.addNewComponent(this, permList, PE_PERM_LIST, 0, 5, 3, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.BOTTOM_PADDING);
        ...
        tw.addNewComponent(panel, button, PE_DONE_BUTTON, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.LR_PADDING);

        // Cancel Button
        button = new Button(PolicyTool.rb.getString("Cancel"));
        button.addActionListener(new CancelButtonListener(this));
        tw.addNewComponent(panel, button, PE_CANCEL_BUTTON, 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.LR_PADDING);
    ...
    PolicyEntry getPolicyEntryFromDialog()
        throws InvalidParameterException, MalformedURLException, NoSuchMethodException, ClassNotFoundException,
        InstantiationException, IllegalAccessException, InvocationTargetException, CertificateException, IOException,
        Exception {

        // get the Codebase
        TextField tf = (TextField) getComponent(PE_CODEBASE_TEXTFIELD);
        String codebase = null;
        if (tf.getText().trim().equals("") == false) { codebase = new String(tf.getText().trim()); }

        // get the SignedBy
        tf = (TextField) getComponent(PE_SIGNEDBY_TEXTFIELD);
        String signedby = null;
        if (tf.getText().trim().equals("") == false) { signedby = new String(tf.getText().trim()); }
    ...
    void keyStoreDialog(int mode) {
        ...
        if (mode == EDIT_KEYSTORE) {
            ...
            tw.addNewComponent(this, label, KSD_NAME_LABEL, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);
            ...
            tw.addNewComponent(this, tf, KSD_NAME_TEXTFIELD, 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);

            // KeyStore type and textfield
            label = new Label(PolicyTool.rb.getString("KeyStore.Type."));
            tw.addNewComponent(this, label, KSD_TYPE_LABEL, 0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);
            tf = new TextField(tool.getKeyStoreType(), 30);
            tf.getAccessibleContext().setAccessibleName(PolicyTool.rb.getString("KeyStore.Type."));
            tw.addNewComponent(this, tf, KSD_TYPE_TEXTFIELD, 1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);

            // KeyStore provider and textfield
            label = new Label(PolicyTool.rb.getString("KeyStore.Provider."));
            tw.addNewComponent(this, label, KSD_PROVIDER_LABEL, 0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);
            ...
            tw.addNewComponent(this, tf, KSD_PROVIDER_TEXTFIELD, 1, 2, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);

            // KeyStore password URL and textfield
            label = new Label(PolicyTool.rb.getString("KeyStore.Password.URL."));
            tw.addNewComponent(this, label, KSD_PWD_URL_LABEL, 0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);
            ...
            tw.addNewComponent(this, tf, KSD_PWD_URL_TEXTFIELD, 1, 3, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
                tw.BOTTOM_PADDING);
            ...
            tw.addNewComponent(this, cancelButton, KSD_CANCEL_BUTTON, 1, 4, 1, 1, 0.0, 0.0,
    ...
    void displayPrincipalDialog(boolean editPolicyEntry, boolean edit) {
        ...
        // description label
        Label label = (edit ? new Label(PolicyTool.rb.getString(".Edit.Principal."))
            : new Label(PolicyTool.rb.getString(".Add.New.Principal.")));
        tw.addNewComponent(newTD, label, PRD_DESC_LABEL, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
        ...
        tw.addNewComponent(newTD, choice, PRD_PRIN_CHOICE, 0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.LR_PADDING);
        ...
        tw.addNewComponent(newTD, tf, PRD_PRIN_TEXTFIELD, 1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.LR_PADDING);
        ...
        tw.addNewComponent(newTD, label, PRD_NAME_LABEL, 0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.LR_PADDING);
        tw.addNewComponent(newTD, tf, PRD_NAME_TEXTFIELD, 1, 2, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.LR_PADDING);

        // OK button
        Button okButton = new Button(PolicyTool.rb.getString("OK"));
        okButton.addActionListener(new NewPolicyPrinOKButtonListener(tool, tw, this, newTD, edit));
        tw.addNewComponent(newTD, okButton, PRD_OK_BUTTON, 0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.TOP_BOTTOM_PADDING);
        // cancel button
        Button cancelButton = new Button(PolicyTool.rb.getString("Cancel"));
        cancelButton.addActionListener(new CancelButtonListener(newTD));
        tw.addNewComponent(newTD, cancelButton, PRD_CANCEL_BUTTON, 1, 3, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.TOP_BOTTOM_PADDING);
    ...
    void displayPermissionDialog(boolean editPolicyEntry, boolean edit) {
        ...
        // description label
        Label label = (edit ? new Label(PolicyTool.rb.getString(".Edit.Permission."))
            : new Label(PolicyTool.rb.getString(".Add.New.Permission.")));
        tw.addNewComponent(newTD, label, PD_DESC_LABEL, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.TOP_BOTTOM_PADDING);
        ...
        tw.addNewComponent(newTD, choice, PD_PERM_CHOICE, 0, 1, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.LR_PADDING);
        ...
        tw.addNewComponent(newTD, tf, PD_PERM_TEXTFIELD, 1, 1, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.LR_PADDING);
        ...
        tw.addNewComponent(newTD, choice, PD_NAME_CHOICE, 0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.LR_PADDING);
        tw.addNewComponent(newTD, tf, PD_NAME_TEXTFIELD, 1, 2, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.LR_PADDING);
        ...
        tw.addNewComponent(newTD, choice, PD_ACTIONS_CHOICE, 0, 3, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.LR_PADDING);
        tw.addNewComponent(newTD, tf, PD_ACTIONS_TEXTFIELD, 1, 3, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.LR_PADDING);

        // signedby label and textfield
        label = new Label(PolicyTool.rb.getString("Signed.By."));
        tw.addNewComponent(newTD, label, PD_SIGNEDBY_LABEL, 0, 4, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.LR_PADDING);
        tf = (edit ? new TextField(editMe.signedBy, 40) : new TextField(40));
        tf.getAccessibleContext().setAccessibleName(PolicyTool.rb.getString("Signed.By."));
        tw.addNewComponent(newTD, tf, PD_SIGNEDBY_TEXTFIELD, 1, 4, 1, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            tw.LR_PADDING);

        // OK button
        Button okButton = new Button(PolicyTool.rb.getString("OK"));
        okButton.addActionListener(new NewPolicyPermOKButtonListener(tool, tw, this, newTD, edit));
        tw.addNewComponent(newTD, okButton, PD_OK_BUTTON, 0, 5, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.TOP_BOTTOM_PADDING);

        // cancel button
        Button cancelButton = new Button(PolicyTool.rb.getString("Cancel"));
        cancelButton.addActionListener(new CancelButtonListener(newTD));
        tw.addNewComponent(newTD, cancelButton, PD_CANCEL_BUTTON, 1, 5, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.TOP_BOTTOM_PADDING);
    ...
    PolicyParser.PrincipalEntry getPrinFromDialog() throws Exception {
        ...
        if ((pclass.equals(PolicyParser.PrincipalEntry.WILDCARD_CLASS)) &&
            ...
            tool.warnings.addElement(
                "Warning: Principal name '" + pname + "' specified without a Principal class.\n" + "\t'" + pname +
                    "' will be interpreted " + "as a key store alias.\n" + "\tThe final principal class will be " +
            ...
            tw.displayStatusDialog(this,
                "'" + pname + "' will be interpreted as a key " + "store alias.  View Warning Log for details.");
        }
    ...
    PolicyParser.PermissionEntry getPermFromDialog() {
        ...
        String name = null;
        if (tf.getText().trim().equals("") == false) { name = new String(tf.getText().trim()); }
        if (permission.equals("") || (!permission.equals(ALL_PERM_CLASS) && name == null)) {
            throw new InvalidParameterException(
                PolicyTool.rb.getString("Permission.and.Target.Name.must.have.a.value"));
        }
        ...
        if (permission.equals(FILE_PERM_CLASS) && name.lastIndexOf("\\\\") > 0) {
            char result = tw.displayYesNoDialog(this, PolicyTool.rb.getString("Warning"), PolicyTool.rb.getString(
                "Warning.File.name.may.include.escaped.backslash.characters.It.is.not.necessary.to.escape.backslash.characters.the.tool.escapes"),
                PolicyTool.rb.getString("Retain"), PolicyTool.rb.getString("Edit"));
            if (result != 'Y') {
        ...
        String actions = null;
        if (tf.getText().trim().equals("") == false) { actions = new String(tf.getText().trim()); }
    ...
    void displayConfirmRemovePolicyEntry() {
        ...
        tw.addNewComponent(this, label, CRPE_LABEL1, 0, 0, 2, 1, 0.0, 0.0, GridBagConstraints.BOTH, tw.BOTTOM_PADDING);
        ...
        tw.addNewComponent(panel, okButton, CRPE_PANEL_OK, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.LR_PADDING);

        // cancel button
        Button cancelButton = new Button(PolicyTool.rb.getString("Cancel"));
        cancelButton.addActionListener(new CancelButtonListener(this));
        tw.addNewComponent(panel, cancelButton, CRPE_PANEL_CANCEL, 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            tw.LR_PADDING);

        tw.addNewComponent(this, panel, CRPE_LABEL2 + 2 + perms.size(), 0, 3 + perms.size(), 2, 1, 0.0, 0.0,
    ...
    void displaySaveAsDialog(int nextEvent) {
        ...
        if (fd.getFile() == null || fd.getFile().equals("")) { return; }
        ...
        try {
            ...
            TextField newFilename = (TextField) tw.getComponent(tw.MW_FILENAME_TEXTFIELD);
    ...
    void displayUserSave(int select) {
        ...
        if (tool.modified == true) {
            ...
            tw.addNewComponent(this, label, USC_LABEL, 0, 0, 3, 1, 0.0, 0.0, GridBagConstraints.BOTH,
            ...
            tw.addNewComponent(panel, yesButton, USC_YES_BUTTON, 0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
            ...
            tw.addNewComponent(panel, noButton, USC_NO_BUTTON, 1, 0, 1, 1, 0.0, 0.0, GridBagConstraints.VERTICAL,
                tw.LR_BOTTOM_PADDING);
            Button cancelButton = new Button(PolicyTool.rb.getString("Cancel"));
            cancelButton.addActionListener(new UserSaveCancelButtonListener(this));
            tw.addNewComponent(panel, cancelButton, USC_CANCEL_BUTTON, 2, 0, 1, 1, 0.0, 0.0,
                GridBagConstraints.VERTICAL, tw.LR_BOTTOM_PADDING);
...
class FileMenuListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (PolicyTool.collator.compare(e.getActionCommand(), tw.QUIT) == 0) {
        ...
        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.NEW_POLICY_FILE) == 0) {
        ...
        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.OPEN_POLICY_FILE) == 0) {
        ...
        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.SAVE_POLICY_FILE) == 0) {

            // get the previously entered filename
            String filename = ((TextField) tw.getComponent(tw.MW_FILENAME_TEXTFIELD)).getText();
            ...
            if (filename == null || filename.length() == 0) {
                ...
                try {
                    ...
                    // display status
                    MessageFormat form = new MessageFormat(
                        PolicyTool.rb.getString("Policy.successfully.written.to.filename"));
                    Object[] source = { filename };
                    tw.displayStatusDialog(null, form.format(source));
                } catch (FileNotFoundException fnfe) {
                    if (filename == null || filename.equals("")) {
                        tw.displayErrorDialog(null,
                            new FileNotFoundException(PolicyTool.rb.getString("null.filename")));
                    } else {
                        tw.displayErrorDialog(null, fnfe);
                    }
                } catch (Exception ee) {
        ...
        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.SAVE_AS_POLICY_FILE) == 0) {

            // user wants to SAVE AS
            ToolDialog td = new ToolDialog(PolicyTool.rb.getString("Save.As"), tool, tw, true);
            td.displaySaveAsDialog(td.NOACTION);

        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.VIEW_WARNINGS) == 0) {
...
class MainWindowListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (PolicyTool.collator.compare(e.getActionCommand(), tw.ADD_POLICY_ENTRY) == 0) {

            // display a dialog box for the user to enter policy info
            ToolDialog td = new ToolDialog(PolicyTool.rb.getString("Policy.Entry"), tool, tw, true);
            td.displayPolicyEntryDialog(false);

        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.REMOVE_POLICY_ENTRY) == 0) {
        ...
        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.EDIT_POLICY_ENTRY) == 0) {
        ...
        } else if (PolicyTool.collator.compare(e.getActionCommand(), tw.EDIT_KEYSTORE) == 0) {
...
class ChangeKeyStoreOKButtonListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        String URLString = ((TextField) td.getComponent(td.KSD_NAME_TEXTFIELD)).getText().trim();
        String type = ((TextField) td.getComponent(td.KSD_TYPE_TEXTFIELD)).getText().trim();
        String provider = ((TextField) td.getComponent(td.KSD_PROVIDER_TEXTFIELD)).getText().trim();
        String pwdURL = ((TextField) td.getComponent(td.KSD_PWD_URL_TEXTFIELD)).getText().trim();

        try {
            tool.openKeyStore((URLString.length() == 0 ? null : URLString), (type.length() == 0 ? null : type),
                (provider.length() == 0 ? null : provider), (pwdURL.length() == 0 ? null : pwdURL));
            tool.modified = true;
        } catch (Exception ex) {
...
class NewPolicyPrinOKButtonListener implements ActionListener {
    ...
    NewPolicyPrinOKButtonListener(PolicyTool tool, ToolWindow tw, ToolDialog listDialog, ToolDialog infoDialog,
        boolean edit) {
        this.tool = tool;
        this.tw = tw;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            if (pppe != null) {
                ...
                TaggedList prinList = (TaggedList) listDialog.getComponent(listDialog.PE_PRIN_LIST);
...
class NewPolicyPermOKButtonListener implements ActionListener {
    ...
    NewPolicyPermOKButtonListener(PolicyTool tool, ToolWindow tw, ToolDialog listDialog, ToolDialog infoDialog,
        boolean edit) {
        this.tool = tool;
        this.tw = tw;
        this.listDialog = listDialog;
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        try {
            ...
            TaggedList permList = (TaggedList) listDialog.getComponent(listDialog.PE_PERM_LIST);
...
class PrincipalTypeMenuListener implements ItemListener {
    ...
    public void itemStateChanged(ItemEvent e) {
        ...
        TextField prinField = (TextField) td.getComponent(td.PRD_PRIN_TEXTFIELD);
        TextField nameField = (TextField) td.getComponent(td.PRD_NAME_TEXTFIELD);
...
class PermissionMenuListener implements ItemListener {
    ...
    public void itemStateChanged(ItemEvent e) {
        ...
        TextField nameField = (TextField) td.getComponent(td.PD_NAME_TEXTFIELD);
        TextField actionsField = (TextField) td.getComponent(td.PD_ACTIONS_TEXTFIELD);
        TextField permField = (TextField) td.getComponent(td.PD_PERM_TEXTFIELD);
        TextField signedbyField = (TextField) td.getComponent(td.PD_SIGNEDBY_TEXTFIELD);
...
class PermissionNameMenuListener implements ItemListener {
    ...
    public void itemStateChanged(ItemEvent e) {
        ...
        if (((String) e.getItem()).indexOf(td.PERM_NAME) != -1) { return; }
...
class PermissionActionsMenuListener implements ItemListener {
    ...
    public void itemStateChanged(ItemEvent e) {
        ...
        if (((String) e.getItem()).indexOf(td.PERM_ACTIONS) != -1) { return; }
    ...
    public AuthPerm() {
        super("AuthPermission", "javax.security.auth.AuthPermission",
            new String[] { "doAs", "doAsPrivileged", "getSubject", "getSubjectFromDomainCombiner", "setReadOnly",
                "modifyPrincipals", "modifyPublicCredentials", "modifyPrivateCredentials", "refreshCredential",
                "destroyCredential", "createLoginContext.<" + PolicyTool.rb.getString("name") + ">",
                "getLoginConfiguration", "setLoginConfiguration",
                "createLoginConfiguration.<" + PolicyTool.rb.getString("configuration.type") + ">",
...
class DelegationPerm extends Perm {
    public DelegationPerm() {
        super("DelegationPermission", "javax.security.auth.kerberos.DelegationPermission", new String[] {
            // allow user input
        }, null);
    }
}
...
class FilePerm extends Perm {
    public FilePerm() {
        super("FilePermission", "java.io.FilePermission", new String[] { "<<ALL FILES>>" },
...
class InqSecContextPerm extends Perm {
    public InqSecContextPerm() {
        super("InquireSecContextPermission", "com.sun.security.jgss.InquireSecContextPermission",
            new String[] { "KRB5_GET_SESSION_KEY", "KRB5_GET_TKT_FLAGS", "KRB5_GET_AUTHZ_DATA", "KRB5_GET_AUTHTIME" },
            null);
    }
}
...
class MBeanPerm extends Perm {
    public MBeanPerm() {
        super("MBeanPermission", "javax.management.MBeanPermission", new String[] {
            // allow user input
        }, new String[] { "addNotificationListener", "getAttribute", "getClassLoader", "getClassLoaderFor",
            "getClassLoaderRepository", "getDomains", "getMBeanInfo", "getObjectInstance", "instantiate", "invoke",
            "isInstanceOf", "queryMBeans", "queryNames", "registerMBean", "removeNotificationListener", "setAttribute",
            "unregisterMBean" });
    }
}
...
class MBeanSvrPerm extends Perm {
    public MBeanSvrPerm() {
        super("MBeanServerPermission", "javax.management.MBeanServerPermission",
            new String[] { "createMBeanServer", "findMBeanServer", "newMBeanServer", "releaseMBeanServer" }, null);
    }
}
...
class PrivCredPerm extends Perm {
    public PrivCredPerm() {
        super("PrivateCredentialPermission", "javax.security.auth.PrivateCredentialPermission", new String[] {
            // allow user input
        }, new String[] { "read" });
    }
}
...
class PropPerm extends Perm {
    public PropPerm() {
        super("PropertyPermission", "java.util.PropertyPermission", new String[] {
            // allow user input
...
class ReflectPerm extends Perm {
    public ReflectPerm() {
        super("ReflectPermission", "java.lang.reflect.ReflectPermission", new String[] { "suppressAccessChecks" },
            null);
    }
}
...
class RuntimePerm extends Perm {
    public RuntimePerm() {
        super("RuntimePermission", "java.lang.RuntimePermission",
            new String[] { "createClassLoader", "getClassLoader", "setContextClassLoader",
                "enableContextClassLoaderOverride", "setSecurityManager", "createSecurityManager",
                "getenv.<" + PolicyTool.rb.getString("environment.variable.name") + ">", "exitVM", "shutdownHooks",
                "setFactory", "setIO", "modifyThread", "stopThread", "modifyThreadGroup", "getProtectionDomain",
                ...
                "defineClassInPackage.<" + PolicyTool.rb.getString("package.name") + ">", "accessDeclaredMembers",
                "queuePrintJob", "getStackTrace", "setDefaultUncaughtExceptionHandler", "preferences", "usePolicy",
                // "inheritedChannel"
            }, null);
    }
}
    ...
    public SecurityPerm() {
        super("SecurityPermission", "java.security.SecurityPermission",
            new String[] { "createAccessControlContext", "getDomainCombiner", "getPolicy", "setPolicy",
                "createPolicy.<" + PolicyTool.rb.getString("policy.type") + ">",
...
class ServicePerm extends Perm {
    public ServicePerm() {
        super("ServicePermission", "javax.security.auth.kerberos.ServicePermission", new String[] {
            // allow user input
...
class SocketPerm extends Perm {
    public SocketPerm() {
        super("SocketPermission", "java.net.SocketPermission", new String[] {
            // allow user input
        }, new String[] { "accept", "connect", "listen", "resolve" });
    }
}
...
class SQLPerm extends Perm {
    public SQLPerm() {
        super("SQLPermission", "java.sql.SQLPermission",
            new String[] { "setLog", "callAbort", "setSyncFactory", "setNetworkTimeout", }, null);
    }
}
...
class SubjDelegPerm extends Perm {
    public SubjDelegPerm() {
        super("SubjectDelegationPermission", "javax.management.remote.SubjectDelegationPermission", new String[] {
            // allow user input
        }, null);
    }
}
