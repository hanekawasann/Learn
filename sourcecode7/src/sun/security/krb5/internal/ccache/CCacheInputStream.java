...
public class CCacheInputStream extends KrbDataInputStream implements FileCCacheConstants {
    ...
    public PrincipalName readPrincipal(int version) throws IOException, RealmException {
         ...
         * does not.
         */
        if (version == KRB5_FCC_FVNO_1) { length--; }
    ...
    EncryptionKey readKey(int version) throws IOException {
        int keyType, keyLen;
        keyType = read(2);
        if (version == KRB5_FCC_FVNO_3) { read(2); /* keytype recorded twice in fvno 3 */ }
        keyLen = read(4);
    ...
    HostAddress[] readAddr() throws IOException, KrbApErrException {
        ...
        if (numAddrs > 0) {
            ...
            for (int i = 0; i < numAddrs; i++) {
                ...
                for (int j = 0; j < addrLength; j++) { result[j] = (byte) read(1); }
    ...
    boolean[] readFlags() throws IOException {
        ...
        int ticketFlags;
        ticketFlags = read(4);
        if ((ticketFlags & 0x40000000) == TKT_FLG_FORWARDABLE) { flags[1] = true; }
        if ((ticketFlags & 0x20000000) == TKT_FLG_FORWARDED) { flags[2] = true; }
        if ((ticketFlags & 0x10000000) == TKT_FLG_PROXIABLE) { flags[3] = true; }
        if ((ticketFlags & 0x08000000) == TKT_FLG_PROXY) { flags[4] = true; }
        if ((ticketFlags & 0x04000000) == TKT_FLG_MAY_POSTDATE) { flags[5] = true; }
        if ((ticketFlags & 0x02000000) == TKT_FLG_POSTDATED) { flags[6] = true; }
        if ((ticketFlags & 0x01000000) == TKT_FLG_INVALID) { flags[7] = true; }
        if ((ticketFlags & 0x00800000) == TKT_FLG_RENEWABLE) { flags[8] = true; }
        if ((ticketFlags & 0x00400000) == TKT_FLG_INITIAL) { flags[9] = true; }
        if ((ticketFlags & 0x00200000) == TKT_FLG_PRE_AUTH) { flags[10] = true; }
        if ((ticketFlags & 0x00100000) == TKT_FLG_HW_AUTH) { flags[11] = true; }
        if (DEBUG) {
    ...
    Credentials readCred(int version) throws IOException, RealmException, KrbApErrException, Asn1Exception {
        ...
        if (DEBUG) { System.out.println(">>>DEBUG <CCacheInputStream>  client principal is " + cpname.toString()); }
        PrincipalName spname = readPrincipal(version);
        if (DEBUG) { System.out.println(">>>DEBUG <CCacheInputStream> server principal is " + spname.toString()); }
        EncryptionKey key = readKey(version);
        if (DEBUG) { System.out.println(">>>DEBUG <CCacheInputStream> key type: " + key.getEType()); }
        long times[] = readTimes();
        KerberosTime authtime = new KerberosTime(times[0]);
        KerberosTime starttime = (times[1] == 0) ? null : new KerberosTime(times[1]);
        KerberosTime endtime = new KerberosTime(times[2]);
        KerberosTime renewTill = (times[3] == 0) ? null : new KerberosTime(times[3]);
        ...
        try {
            return new Credentials(cpname, spname, key, authtime, starttime, endtime, renewTill, skey, tFlags, addrs,
                auData, ticketData != null ? new Ticket(ticketData) : null,
