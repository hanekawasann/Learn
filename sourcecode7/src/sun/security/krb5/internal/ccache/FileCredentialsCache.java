...
public class FileCredentialsCache extends CredentialsCache implements FileCCacheConstants {
        ...
        if (credentialsList != null) {
            if (credentialsList.isEmpty()) {
                ...
                for (int i = 0; i < credentialsList.size(); i++) {
                    ...
                    if (match(c.sname.getNameStrings(), tmp.sname.getNameStrings()) &&
                        ((c.sname.getRealmString()).equalsIgnoreCase(tmp.sname.getRealmString()))) {
                        matched = true;
                        if (c.endtime.getTime() >= tmp.endtime.getTime()) {
                            if (DEBUG) {
                                System.out.println(
                                    " >>> FileCredentialsCache " + "Ticket matched, overwrite " + "the old one.");
                            }
                ...
                if (matched == false) {
                    if (DEBUG) {
                        System.out.println(
                            " >>> FileCredentialsCache Ticket " + "not exactly matched, " + "add new one into cache.");
                    }
        ...
        if (options == null) {
            ...
            if (list == null) {
                ...
                for (int i = 0; i < list.length; i++) {
                    if (sname.match(list[i].sname) && (srealm.toString().equals(list[i].srealm.toString()))) {
    ...
    public static String getDefaultCacheName() {
        ...
        name = java.security.AccessController.doPrivileged(new java.security.PrivilegedAction<String>() {
            @Override
            public String run() {
        ...
        if (osname != null) {
            ...
            if (osname.startsWith("SunOS") || (osname.startsWith("Linux"))) {
                try {
                    ...
                    if (DEBUG) {
                        System.out.println(
                            "Exception in obtaining uid " + "for Unix platforms " + "Using user's home directory");
        ...
        String user_name = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("user.name"));

        String user_home = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("user.home"));

        if (user_home == null) {
            user_home = java.security.AccessController
                .doPrivileged(new sun.security.action.GetPropertyAction("user.dir"));
        }

        if (user_name != null) {
            name = user_home + File.separator + stdCacheNameComponent + "_" + user_name;
        } else {
    ...
    public static String checkValidation(String name) {
        ...
        try {
            ...
            if (!(fCheck.exists())) {
                ...
                if (!(temp.isDirectory())) { fullname = null; }
                temp = null;
            }
            fCheck = null;
    ...
    private static String exec(String c) {
        ...
        try {
            ...
            Process p = java.security.AccessController.doPrivileged(new java.security.PrivilegedAction<Process>() {
                public Process run() {
                    try {
            ...
            String s1 = null;
            if ((command.length == 1) && (command[0].equals("/usr/bin/env"))) {
                while ((s1 = commandResult.readLine()) != null) {
                    if (s1.length() >= 11) {
                        if ((s1.substring(0, 11)).equalsIgnoreCase("KRB5CCNAME=")) {
                            s1 = s1.substring(11);
                            break;
                        }
                    }
