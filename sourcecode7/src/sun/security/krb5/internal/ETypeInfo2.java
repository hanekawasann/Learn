...
public class ETypeInfo2 {
    ...
    public Object clone() {
        ...
        if (s2kparams != null) {
            ...
            System.arraycopy(s2kparams, 0, etypeInfo2.s2kparams, 0, s2kparams.length);
        }
        return etypeInfo2;
    }
    ...
    public ETypeInfo2(DerValue encoding) throws Asn1Exception, IOException {
        ...
        if (encoding.getData().available() > 0) {
            if ((encoding.getData().peekByte() & 0x1F) == 0x01) {
                der = encoding.getData().getDerValue();
                this.saltStr = new KerberosString(der.getData().getDerValue()).toString();
            }
        }
     ...
     * @throws IOException   if an I/O error occurs while reading encoded data.
     * @throws Asn1Exception on encoding errors.
     */
    public byte[] asn1Encode() throws Asn1Exception, IOException {
        ...
        bytes.write(DerValue.createTag(DerValue.TAG_CONTEXT, true, TAG_TYPE), temp);

        if (saltStr != null) {
            temp = new DerOutputStream();
            temp.putDerValue(new KerberosString(saltStr).toDerValue());
            bytes.write(DerValue.createTag(DerValue.TAG_CONTEXT, true, TAG_VALUE1), temp);
        }
        if (s2kparams != null) {
            temp = new DerOutputStream();
            temp.putOctetString(s2kparams);
            bytes.write(DerValue.createTag(DerValue.TAG_CONTEXT, true, TAG_VALUE2), temp);
        }
