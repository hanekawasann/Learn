...
public class KRBError implements java.io.Serializable {
    ...
    public KRBError(APOptions new_apOptions, KerberosTime new_cTime, Integer new_cuSec, KerberosTime new_sTime,
        Integer new_suSec, int new_errorCode, Realm new_crealm, PrincipalName new_cname, Realm new_realm,
        PrincipalName new_sname, String new_eText, byte[] new_eData) throws IOException, Asn1Exception {
        pvno = Krb5.PVNO;
    ...
    public KRBError(APOptions new_apOptions, KerberosTime new_cTime, Integer new_cuSec, KerberosTime new_sTime,
        Integer new_suSec, int new_errorCode, Realm new_crealm, PrincipalName new_cname, Realm new_realm,
        PrincipalName new_sname, String new_eText, byte[] new_eData, Checksum new_eCksum)
    ...
    private void parseEData(byte[] data) throws IOException {
        ...
        if (errorCode == Krb5.KDC_ERR_PREAUTH_REQUIRED || errorCode == Krb5.KDC_ERR_PREAUTH_FAILED) {
            try {
            ...
            if (DEBUG) {
                System.out
                    .println("Unknown eData field of KRB-ERROR:\n" + new sun.misc.HexDumpEncoder().encodeBuffer(data));
            }
        }
    }
     ...
     * @throws KrbApErrException if the value read from the DER-encoded data
    ...
    private void init(DerValue encoding) throws Asn1Exception, RealmException, KrbApErrException, IOException {
        DerValue der, subDer;
        if (((encoding.getTag() & (byte) 0x1F) != (byte) 0x1E) || (encoding.isApplication() != true) ||
    ...
    public boolean equals(Object obj) {
        ...
        return pvno == other.pvno && msgType == other.msgType && isEqual(cTime, other.cTime) &&
            isEqual(cuSec, other.cuSec) && isEqual(sTime, other.sTime) && isEqual(suSec, other.suSec) &&
            errorCode == other.errorCode && isEqual(crealm, other.crealm) && isEqual(cname, other.cname) &&
            isEqual(realm, other.realm) && isEqual(sname, other.sname) && isEqual(eText, other.eText) &&
            java.util.Arrays.equals(eData, other.eData) && isEqual(eCksum, other.eCksum);
    }
