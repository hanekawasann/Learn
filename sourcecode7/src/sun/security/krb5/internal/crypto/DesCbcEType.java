...
abstract class DesCbcEType extends EType {
    ...
    public byte[] decrypt(byte[] cipher, byte[] key, byte[] ivec, int usage)
        ...
        if (!isChecksumValid(data)) { throw new KrbApErrException(Krb5.KRB_AP_ERR_BAD_INTEGRITY); }
        return data;
    }

    private void copyChecksumField(byte[] data, byte[] cksum) {
        for (int i = 0; i < checksumSize(); i++) { data[startOfChecksum() + i] = cksum[i]; }
    }

    private byte[] checksumField(byte[] data) {
        byte[] result = new byte[checksumSize()];
        for (int i = 0; i < checksumSize(); i++) { result[i] = data[startOfChecksum() + i]; }
        return result;
    }

    private void resetChecksumField(byte[] data) {
        for (int i = startOfChecksum(); i < startOfChecksum() + checksumSize(); i++) { data[i] = 0; }
    }
