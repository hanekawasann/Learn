...
public abstract class EType {
    ...
    static {
        ...
        try {
            ...
            if (DEBUG) {
                System.out
                    .println("Exception in getting allow_weak_crypto, " + "using default value " + exc.getMessage());
            }
        }
    ...
    public static EType getInstance(int eTypeConst) throws KdcErrException {
        ...
        switch (eTypeConst) {
            ...
            default:
                String msg = "encryption type = " + toString(eTypeConst) + " (" + eTypeConst + ")";
    ...
    public abstract byte[] decrypt(byte[] cipher, byte[] key, byte[] ivec, int usage)
    ...
    public int padSize(byte[] data) {
        return data.length - confounderSize() - checksumSize() - dataSize(data);
    }
    ...
    private static final int[] BUILTIN_ETYPES = new int[] { EncryptedData.ETYPE_AES256_CTS_HMAC_SHA1_96,
        EncryptedData.ETYPE_AES128_CTS_HMAC_SHA1_96, EncryptedData.ETYPE_DES3_CBC_HMAC_SHA1_KD,
        EncryptedData.ETYPE_ARCFOUR_HMAC, EncryptedData.ETYPE_DES_CBC_CRC, EncryptedData.ETYPE_DES_CBC_MD5, };

    private static final int[] BUILTIN_ETYPES_NOAES256 = new int[] { EncryptedData.ETYPE_AES128_CTS_HMAC_SHA1_96,
        EncryptedData.ETYPE_DES3_CBC_HMAC_SHA1_KD, EncryptedData.ETYPE_ARCFOUR_HMAC, EncryptedData.ETYPE_DES_CBC_CRC,
    ...
    public static int[] getDefaults(String configName, EncryptionKey[] keys) throws KrbException {
        ...
        if (len <= 0) {
            ...
            throw new KrbException(
                "Do not have keys of types listed in " + configName + " available; only have keys of following type: " +
                    keystr.toString());
        } else {
            answer = new int[len];
            for (int i = 0; i < len; i++) {
