...
public class AesDkCrypto extends DkCrypto {
    ...
    public byte[] encrypt(byte[] baseKey, int usage, byte[] ivec, byte[] new_ivec, byte[] plaintext, int start, int len)
        ...
        byte[] output = encryptCTS(baseKey, usage, ivec, new_ivec, plaintext, start, len, true);
        return output;
    }
    ...
    public byte[] decrypt(byte[] baseKey, int usage, byte[] ivec, byte[] ciphertext, int start, int len)
        ...
        byte[] output = decryptCTS(baseKey, usage, ivec, ciphertext, start, len, true);
        return output;
    }
     ...
     */
    private byte[] encryptCTS(byte[] baseKey, int usage, byte[] ivec, byte[] new_ivec, byte[] plaintext, int start,
        int len, boolean confounder_exists) throws GeneralSecurityException, KrbCryptoException {
     ...
     */
    private byte[] decryptCTS(byte[] baseKey, int usage, byte[] ivec, byte[] ciphertext, int start, int len,
        ...
        try {
            ...
            if (debug) {
                traceOutput("AES PlainText", plaintext, 0, Math.min(plaintext.length, 32));
            }
