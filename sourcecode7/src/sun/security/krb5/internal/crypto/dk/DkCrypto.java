...
public abstract class DkCrypto {
    ...
    static final byte[] KERBEROS_CONSTANT = { 0x6b, 0x65, 0x72, 0x62, 0x65, 0x72, 0x6f, 0x73 };

    protected abstract int getKeySeedLength();  // in bits

    protected abstract byte[] randomToKey(byte[] in);

    protected abstract Cipher getCipher(byte[] key, byte[] ivec, int mode) throws GeneralSecurityException;

    public abstract int getChecksumLength();  // in bytes

    protected abstract byte[] getHmac(byte[] key, byte[] plaintext) throws GeneralSecurityException;
     ...
     */
    public byte[] encrypt(byte[] baseKey, int usage, byte[] ivec, byte[] new_ivec, byte[] plaintext, int start, int len)
        ...
        try {
            ...
            if (debug) {
                System.err.println("confounder = " + confounder.length + "; plaintext = " + len + "; padding = " +
                    (plainSize - confounder.length - len) + "; total = " + plainSize);
            ...
            if (debug) {
                ...
                traceOutput("ciphertext", ciphertext, 0, Math.min(ciphertext.length, 32));
            }
     ...
     * Ignore key usage.
     */
    public byte[] encryptRaw(byte[] baseKey, int usage, byte[] ivec, byte[] plaintext, int start, int len)
        throws GeneralSecurityException, KrbCryptoException {
        ...
        if ((len % blockSize) != 0) {
            throw new GeneralSecurityException(
                "length of data to be encrypted (" + len + ") is not a multiple of the blocksize (" + blockSize + ")");
        }
    ...
    public byte[] decryptRaw(byte[] baseKey, int usage, byte[] ivec, byte[] ciphertext, int start, int len)
        ...
        if ((len % blockSize) != 0) {
            throw new GeneralSecurityException(
                "length of data to be decrypted (" + len + ") is not a multiple of the blocksize (" + blockSize + ")");
        }

        byte[] decrypted = decCipher.doFinal(ciphertext, start, len);

        if (debug) {
            traceOutput("decrypted", decrypted, 0, Math.min(decrypted.length, 32));
        }
     ...
     */
    public byte[] decrypt(byte[] baseKey, int usage, byte[] ivec, byte[] ciphertext, int start, int len)
        throws GeneralSecurityException {
        ...
        try {
            ...
            if (debug) {
                traceOutput("calculated Hmac", calculatedHmac, 0, calculatedHmac.length);
                traceOutput("message Hmac", ciphertext, cipherSize, cksumSize);
            }
    ...
    public byte[] calculateChecksum(byte[] baseKey, int usage, byte[] input, int start, int len)
        throws GeneralSecurityException {
    ...
    private byte[] dr(byte[] key, byte[] constant) throws GeneralSecurityException {
        ...
        while (n < keybytes) {
            if (debug) {
                System.err.println("Encrypting: " + bytesToString(toBeEncrypted));
            }

            byte[] cipherBlock = encCipher.doFinal(toBeEncrypted);
            if (debug) {
                System.err.println("K: " + ++posn + " = " + bytesToString(cipherBlock));
            }
        ...
        for (i = lcm - 1; i >= 0; i--) {
            ...
            msbit = (/* first, start with msbit in the first, unrotated byte */
                ((inbits << 3) - 1)
                    ...
                    /* last, pick out correct byte within that shifted repetition */ + ((inbits - (i % inbits)) << 3)) %
                (inbits << 3);
            ...
            bval = ((((in[((inbits - 1) - (msbit >>> 3)) % inbits] & 0xff) << 8) |
                (in[((inbits) - (msbit >>> 3)) % inbits] & 0xff)) >>> ((msbit & 7) + 1)) & 0xff;
            ...
            */
            ...
            if (debug) {
                System.err.println("msbit[" + i + "] = " + msbit + "\tbval=" + Integer.toHexString(bval) + "\toval=" +
                    Integer.toHexString(oval) + "\tsum = " + Integer.toHexString(thisbyte));
            }
