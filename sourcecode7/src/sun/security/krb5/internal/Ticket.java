...
public class Ticket implements Cloneable {
    ...
    public Ticket(Realm new_realm, PrincipalName new_sname, EncryptedData new_encPart) {
     ...
     * @throws KrbApErrException if the value read from the DER-encoded data stream does not match the pre-defined value.
    ...
    private void init(DerValue encoding) throws Asn1Exception, RealmException, KrbApErrException, IOException {
        DerValue der;
        DerValue subDer;
        if (((encoding.getTag() & (byte) 0x1F) != Krb5.KRB_TKT) || (encoding.isApplication() != true) ||
            (encoding.isConstructed() != true)) { throw new Asn1Exception(Krb5.ASN1_BAD_ID); }
        ...
        if (tkt_vno != Krb5.TICKET_VNO) { throw new KrbApErrException(Krb5.KRB_AP_ERR_BADVERSION); }
     ...
     */
    public static Ticket parse(DerInputStream data, byte explicitTag, boolean optional)
