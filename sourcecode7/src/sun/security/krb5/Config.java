...
public class Config {
    ...
    private Config() throws KrbException {
         ...
         */
        String tmp = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("java.security.krb5.kdc"));
        if (tmp != null) {
        ...
        defaultRealm = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("java.security.krb5.realm"));
        if ((defaultKDC == null && defaultRealm != null) || (defaultRealm == null && defaultKDC != null)) {
            throw new KrbException(
                "System property java.security.krb5.kdc and " + "java.security.krb5.realm both must be set or " +
                    "neither must be set.");
        }
    ...
    public int getDefaultIntValue(String name, String section) {
        ...
        if (result != null) {
            try {
                ...
                if (DEBUG) {
                    System.out.println(
                        "Exception in getting value of " + name + " in section " + section + " " + e.getMessage());
                    System.out.println("Setting " + name + " to minimum value");
                }
                value = Integer.MIN_VALUE;
            }
                ...
                if (ob instanceof Hashtable) {
                    ...
                    if (ob instanceof String) {
                        ...
                        for (int i = 0; i < length; i++) {
                            if (i == length - 1) {
                                result += (String) (((Vector) ob).elementAt(i));
                            } else {
                                result += (String) (((Vector) ob).elementAt(i)) + " ";
                            }
                        }
                    ...
                    if (ob instanceof Hashtable) {
                        ...
                        if (temp.containsKey(name)) {
                            ...
                            if (object instanceof Vector) {
                                ...
                                for (int i = 0; i < length; i++) {
                                    if (i == length - 1) {
                                        result += (String) (((Vector) object).elementAt(i));
                                    } else {
                                        result += (String) (((Vector) object).elementAt(i)) + " ";
                                    }
                                }
     ...
     * @throws NumberFormationException if the String does not contain a
     *                                  parsable integer.
     */
    ...
    private String find(String key1, String key2) {
        String result;
        if ((stanzaTable != null) && ((result = (String) (((Hashtable) (stanzaTable.get(key1))).get(key2))) != null)) {
            return result;
        } else {
            return "";
        }
    ...
    private Vector<String> loadConfigFile() throws IOException {
        try {
            ...
            if (!fileName.equals("")) {
                BufferedReader br = new BufferedReader(new InputStreamReader(java.security.AccessController
                    .doPrivileged(new java.security.PrivilegedExceptionAction<FileInputStream>() {
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            if (line.equalsIgnoreCase("[realms]")) {
                for (int count = i + 1; count < v.size() + 1; count++) {
                    ...
                    if ((count == v.size()) || (v.elementAt(count).startsWith("["))) {
                        Hashtable<String, Hashtable<String, Vector<String>>> temp = new Hashtable<>();
                ...
                for (int count = i + 1; count < v.size() + 1; count++) {
                    // find the next stanza name
                    if ((count == v.size()) || (v.elementAt(count).startsWith("["))) {
                        Hashtable<String, String> temp = parseField(v, i + 1, count);
                        table.put(key, temp);
                        i = count - 1;
                        break;
                    }
    ...
    private String getFileName() {
        String name = java.security.AccessController.doPrivileged(new sun.security.action.
            GetPropertyAction("java.security.krb5.conf"));
        if (name == null) {
            name = java.security.AccessController.doPrivileged(new sun.security.action.
                GetPropertyAction("java.home")) + File.separator + "lib" + File.separator + "security" +
                File.separator + "krb5.conf";
            if (!fileExists(name)) {
                name = null;
                String osname = java.security.AccessController
        ...
        if (stanzaTable != null) {
            for (Enumeration e = table.keys(); e.hasMoreElements(); ) {
                ...
                if (object instanceof Hashtable) {
                    if (table != stanzaTable) { System.out.println("\t" + key + " = {"); }
                    listTable((Hashtable) object);
                    if (table != stanzaTable) { System.out.println("\t}"); }
    ...
    public int getType(String input) {
        ...
        if (input.startsWith("d") || (input.startsWith("D"))) {
            ...
            } else if (input.equalsIgnoreCase("des3-cbc-sha1") || input.equalsIgnoreCase("des3-hmac-sha1") ||
                input.equalsIgnoreCase("des3-cbc-sha1-kd") || input.equalsIgnoreCase("des3-cbc-hmac-sha1-kd")) {
        ...
        } else if (input.equalsIgnoreCase("hmac-md5-rc4") || input.equalsIgnoreCase("hmac-md5-arcfour") ||
    ...
    public String getDefaultRealm() throws KrbException {
        ...
        if (realm == null) {
            realm = java.security.AccessController.doPrivileged(new java.security.PrivilegedAction<String>() {
                @Override
                public String run() {
    ...
    private String getRealmFromDNS() throws KrbException {
        ...
        if (realm == null) {
            throw new KrbException(Krb5.KRB_ERR_GENERIC, "Unable to locate Kerberos realm");
        }
        return realm;
    }
    ...
    private String getKDCFromDNS(String realm) throws KrbException {
        ...
        if (srvs == null) {
            // no DNS SRV records
            throw new KrbException(Krb5.KRB_ERR_GENERIC, "Unable to locate KDC for realm " + realm);
        }
    ...
    private boolean fileExists(String name) {
        return java.security.AccessController.doPrivileged(new FileExistsAction(name));
    }

    static class FileExistsAction implements java.security.PrivilegedAction<Boolean> {
    ...
    private static void toStringIndented(String prefix, Object obj, StringBuffer sb) {
