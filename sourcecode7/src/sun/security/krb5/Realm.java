...
public class Realm implements Cloneable {
    ...
    private static String[] parseCapaths(String cRealm, String sRealm) throws KrbException {
        ...
        do {
            ...
            if (intermediaries != null && !intermediaries.equals(".") && !intermediaries.equals(cRealm)) {
                if (DEBUG) {
                    System.out
                        .println(">>> Realm parseCapaths: loop " + count + ": intermediaries=[" + intermediaries + "]");
                }
                ...
                for (int i = ints.length - 1; i >= 0; i--) {
                    ...
                    if (!tempList.contains(tempRealm)) {
                        ...
                        if (DEBUG) {
                            System.out.println(
                                ">>> Realm parseCapaths: loop " + count + ": pushed realm on to stack: " + tempRealm);
                        }
                    } else if (DEBUG) {
                        System.out
                            .println(">>> Realm parseCapaths: loop " + count + ": ignoring realm: [" + tempRealm + "]");
                    }
                }
            } else {
                if (DEBUG) {
            ...
            if (DEBUG) {
                System.out
                    .println(">>> Realm parseCapaths: loop " + count + ": added intermediary to list: " + tempTarget);
            }
    ...
    private static String[] parseHierarchy(String cRealm, String sRealm) throws KrbException {
        ...
        StringTokenizer strTok = new StringTokenizer(cRealm, PrincipalName.REALM_COMPONENT_SEPARATOR_STR);
        ...
        strTok = new StringTokenizer(sRealm, PrincipalName.REALM_COMPONENT_SEPARATOR_STR);
        ...
        for (sCount--, cCount--;
            sCount >= 0 && cCount >= 0 && sComponents[sCount].equals(cComponents[cCount]); sCount--, cCount--) {
            commonComponents++;
        }
        ...
        if (DEBUG) {
            if (commonComponents > 0) {
                System.out.println(">>> Realm parseHierarchy: " + commonComponents + " common component" +
                    (commonComponents > 1 ? "s" : " "));

                System.out.println(
                    ">>> Realm parseHierarchy: common part " + "in cRealm (starts at index " + cCommonStart + ")");
                System.out
                    .println(">>> Realm parseHierarchy: common part in sRealm (starts at index " + sCommonStart + ")");
            ...
            } else { System.out.println(">>> Realm parseHierarchy: no common part"); }
        }
    ...
    private static String substring(String realm, int componentIndex) {
        ...
        while (i < len && j != componentIndex) {
            if (realm.charAt(i++) != PrincipalName.REALM_COMPONENT_SEPARATOR) { continue; }
            j++;
        }
