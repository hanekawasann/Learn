...
public final class KrbAsReqBuilder {
    ...
    public EncryptionKey[] getKeys() throws KrbException {
        ...
        if (password != null) {
            ...
            for (int i = 0; i < eTypes.length; i++) {
                PAData.SaltAndParams snp = PAData.getSaltAndParams(eTypes[i], paList);
                // First round, only calculate those with new salt
                if (snp.salt != null) {
                    salt = snp.salt;
                    result[i] = EncryptionKey.acquireSecretKey(password, snp.salt, eTypes[i], snp.params);
                }
            }
            ...
            for (int i = 0; i < eTypes.length; i++) {
                ...
                if (result[i] == null) {
                    PAData.SaltAndParams snp = PAData.getSaltAndParams(eTypes[i], paList);
                    result[i] = EncryptionKey.acquireSecretKey(password, salt, eTypes[i], snp.params);
                }
            }
            return result;
        } else {
    ...
    private KrbAsReq build(EncryptionKey key) throws KrbException, IOException {
        ...
        return new KrbAsReq(key, options, cname, sname, from, till, rtime, eTypes, addresses);
    }
    ...
    private KrbAsReqBuilder send() throws KrbException, IOException {
        ...
        while (true) {
            try {
                ...
                if (!preAuthFailedOnce && (ke.returnCode() == Krb5.KDC_ERR_PREAUTH_FAILED ||
                    ...
                    if (password == null) {
                        ...
                        PAData.SaltAndParams snp = PAData.getSaltAndParams(kerr.getEType(), kerr.getPA());
                        if (kerr.getEType() == 0) {
                            // Possible if PA-PW-SALT is in KRB-ERROR. RFC
                            // does not recommend this
                            pakey = EncryptionKey
                                .acquireSecretKey(password, snp.salt == null ? cname.getSalt() : snp.salt,
                                    EType.getDefaults("default_tkt_enctypes")[0], null);
                        } else {
                            pakey = EncryptionKey
                                .acquireSecretKey(password, snp.salt == null ? cname.getSalt() : snp.salt,
                                    kerr.getEType(), snp.params);
                        }
                    }
