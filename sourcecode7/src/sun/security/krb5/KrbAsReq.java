...
public class KrbAsReq {
    ...
    public KrbAsReq(EncryptionKey pakey,        // ok
        ...
        if (options.get(KDCOptions.FORWARDED) || options.get(KDCOptions.PROXY) ||
            options.get(KDCOptions.ENC_TKT_IN_SKEY) || options.get(KDCOptions.RENEW) ||
        ...
        if (sname == null) {
            sname = new PrincipalName("krbtgt" + PrincipalName.NAME_COMPONENT_SEPARATOR + cname.getRealmAsString(),
        ...
        KDCReqBody kdc_req_body = new KDCReqBody(options, cname, cname.getRealm(), sname, from, till, rtime,
            Nonce.value(), eTypes, addresses, null, null);
