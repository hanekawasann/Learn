...
public class KrbApRep {
    ...
    public KrbApRep(KrbApReq incomingReq, boolean useSeqNumber, boolean useSubKey) throws KrbException, IOException {

        EncryptionKey subKey = (useSubKey ? new EncryptionKey(incomingReq.getCreds().getSessionKey()) : null);
    ...
    private void init(KrbApReq apReq, EncryptionKey subKey, SeqNumber seqNumber) throws KrbException, IOException {
        createMessage(apReq.getCreds().key, apReq.getCtime(), apReq.cusec(), subKey, seqNumber);
    ...
    private KrbApRep(DerValue encoding, Credentials tgs_creds) throws KrbException, IOException {
        ...
        byte[] temp = rep.encPart.decrypt(tgs_creds.key, KeyUsage.KU_ENC_AP_REP_PART);
    ...
    private void createMessage(EncryptionKey key, KerberosTime ctime, int cusec, EncryptionKey subKey,
        SeqNumber seqNumber) throws Asn1Exception, IOException, KdcErrException, KrbCryptoException {

        Integer seqno = null;

        if (seqNumber != null) { seqno = new Integer(seqNumber.current()); }

        encPart = new EncAPRepPart(ctime, cusec, subKey, seqno);

        byte[] encPartEncoding = encPart.asn1Encode();

        EncryptedData encEncPart = new EncryptedData(key, encPartEncoding, KeyUsage.KU_ENC_AP_REP_PART);
