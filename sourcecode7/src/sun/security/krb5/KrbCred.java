...
public class KrbCred {
    ...
    public KrbCred(Credentials tgt, Credentials serviceTicket, EncryptionKey key) throws KrbException, IOException {
        ...
        if (!serviceTicket.getClient().equals(client)) {
            throw new KrbException(Krb5.KRB_ERR_GENERIC, "Client principal does not match");
        }
        ...
        if (server.getNameType() == PrincipalName.KRB_NT_SRV_HST) { sAddrs = new HostAddresses(server); }

        KrbTgsReq tgsReq = new KrbTgsReq(options, tgt, tgService, null, null, null, null, sAddrs, null, null, null);
    ...
    KRBCred createMessage(Credentials delegatedCreds, EncryptionKey key) throws KrbException, IOException {
        ...
        KrbCredInfo credInfo = new KrbCredInfo(sessionKey, realm, princ, delegatedCreds.flags, delegatedCreds.authTime,
            delegatedCreds.startTime, delegatedCreds.endTime, delegatedCreds.renewTill, tgsRealm, tgService,
            delegatedCreds.cAddr);

        timeStamp = new KerberosTime(KerberosTime.NOW);
        KrbCredInfo[] credInfos = { credInfo };
        EncKrbCredPart encPart = new EncKrbCredPart(credInfos, timeStamp, null, null, null, null);

        EncryptedData encEncPart = new EncryptedData(key, encPart.asn1Encode(), KeyUsage.KU_ENC_KRB_CRED_PART);
    ...
    public KrbCred(byte[] asn1Message, EncryptionKey key) throws KrbException, IOException {
        ...
        byte[] temp = credMessg.encPart.decrypt(key, KeyUsage.KU_ENC_KRB_CRED_PART);
        ...
        if (DEBUG) {
            System.out.println(
                ">>>Delegated Creds have pname=" + pname + " sname=" + sname + " authtime=" + authtime + " starttime=" +
                    starttime + " endtime=" + endtime + "renewTill=" + renewTill);
        }
        creds = new Credentials(ticket, pname, sname, credInfoKey, flags, authtime, starttime, endtime, renewTill,
            caddr);
    }
