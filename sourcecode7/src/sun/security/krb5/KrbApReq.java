...
*/
    ...
    public KrbApReq(Credentials tgsCred, boolean mutualRequired, boolean useSubKey, boolean useSeqNumber,
        Checksum cksum) throws Asn1Exception, KrbCryptoException, KrbException, IOException {

        APOptions apOptions = (mutualRequired ? new APOptions(Krb5.AP_OPTS_MUTUAL_REQUIRED) : new APOptions());
        if (DEBUG) { System.out.println(">>> KrbApReq: APOptions are " + apOptions); }

        EncryptionKey subKey = (useSubKey ? new EncryptionKey(tgsCred.getSessionKey()) : null);

        SeqNumber seqNum = new LocalSeqNumber();

        init(apOptions, tgsCred, cksum, subKey, seqNum, null,   // AuthorizationData authzData
    ...
    public KrbApReq(byte[] message, EncryptionKey[] keys, InetAddress initiator) throws KrbException, IOException {
        obuf = message;
        if (apReqMessg == null) { decode(); }
...
*/
    ...
    /** used by KrbTgsReq **/
    KrbApReq(APOptions apOptions, Ticket ticket, EncryptionKey key, Realm crealm, PrincipalName cname, Checksum cksum,
        KerberosTime ctime, EncryptionKey subKey, SeqNumber seqNumber, AuthorizationData authorizationData)
        throws Asn1Exception, IOException, KdcErrException, KrbCryptoException {

        init(apOptions, ticket, key, crealm, cname, cksum, ctime, subKey, seqNumber, authorizationData,
            KeyUsage.KU_PA_TGS_REQ_AUTHENTICATOR);

    }

    private void init(APOptions options, Credentials tgs_creds, Checksum cksum, EncryptionKey subKey,
        SeqNumber seqNumber, AuthorizationData authorizationData, int usage) throws KrbException, IOException {

        ctime = new KerberosTime(KerberosTime.NOW);
        init(options, tgs_creds.ticket, tgs_creds.key, tgs_creds.client.getRealm(), tgs_creds.client, cksum, ctime,
            subKey, seqNumber, authorizationData, usage);
    }

    private void init(APOptions apOptions, Ticket ticket, EncryptionKey key, Realm crealm, PrincipalName cname,
        Checksum cksum, KerberosTime ctime, EncryptionKey subKey, SeqNumber seqNumber,
        AuthorizationData authorizationData, int usage)
        throws Asn1Exception, IOException, KdcErrException, KrbCryptoException {

        createMessage(apOptions, ticket, key, crealm, cname, cksum, ctime, subKey, seqNumber, authorizationData, usage);
    ...
    void decode(DerValue encoding) throws KrbException, IOException {
        ...
        try {
            ...
            String eText;
            if (errStr.charAt(errStr.length() - 1) == 0) { eText = errStr.substring(0, errStr.length() - 1); } else {
                eText = errStr;
            }
    ...
    private void authenticate(EncryptionKey[] keys, InetAddress initiator) throws KrbException, IOException {
        ...
        if (dkey == null) {
            throw new KrbException(Krb5.API_INVALID_ARG,
                "Cannot find key of appropriate type to decrypt AP REP - " + EType.toString(encPartKeyType));
        }
        ...
        if (!authenticator.ctime.inClockSkew(currTime)) { throw new KrbApErrException(Krb5.KRB_AP_ERR_SKEW); }

        // start to check if it is a replay attack.
        AuthTime time = new AuthTime(authenticator.ctime.getTime(), authenticator.cusec);
        ...
        if (initiator != null) {
            ...
            if (enc_ticketPart.caddr != null && !enc_ticketPart.caddr.inList(sender)) {
                if (DEBUG) {
                    System.out.println(">>> KrbApReq: initiator is " + sender.getInetAddress() + ", but caddr is " +
                        Arrays.toString(enc_ticketPart.caddr.getInetAddresses()));
                }
        ...
        if ((enc_ticketPart.starttime != null && enc_ticketPart.starttime.greaterThanWRTClockSkew(now)) ||
            enc_ticketPart.flags.get(Krb5.TKT_OPTS_INVALID)) { throw new KrbApErrException(Krb5.KRB_AP_ERR_TKT_NYV); }
        ...
        creds = new Credentials(apReqMessg.ticket, authenticator.cname, apReqMessg.ticket.sname, enc_ticketPart.key,
            enc_ticketPart.flags, enc_ticketPart.authtime, enc_ticketPart.starttime, enc_ticketPart.endtime,
            enc_ticketPart.renewTill, enc_ticketPart.caddr, enc_ticketPart.authorizationData);
        if (DEBUG) {
    ...
    APOptions getAPOptions() throws KrbException, IOException {
        if (apReqMessg == null) { decode(); }
        if (apReqMessg != null) { return apReqMessg.apOptions; }
        return null;
    }
    ...
    public boolean getMutualAuthRequired() throws KrbException, IOException {
        if (apReqMessg == null) { decode(); }
        if (apReqMessg != null) { return apReqMessg.apOptions.get(Krb5.AP_OPTS_MUTUAL_REQUIRED); }
        return false;
    }

    boolean useSessionKey() throws KrbException, IOException {
        if (apReqMessg == null) { decode(); }
        if (apReqMessg != null) { return apReqMessg.apOptions.get(Krb5.AP_OPTS_USE_SESSION_KEY); }
        return false;
    }
    ...
    private void createMessage(APOptions apOptions, Ticket ticket, EncryptionKey key, Realm crealm, PrincipalName cname,
        Checksum cksum, KerberosTime ctime, EncryptionKey subKey, SeqNumber seqNumber,
        AuthorizationData authorizationData, int usage)
        throws Asn1Exception, IOException, KdcErrException, KrbCryptoException {
        ...
        if (seqNumber != null) { seqno = new Integer(seqNumber.current()); }

        authenticator = new Authenticator(crealm, cname, cksum, ctime.getMicroSeconds(), ctime, subKey, seqno,
            authorizationData);
