...
abstract class KrbAppMessage {
     ...
     */
    void check(KerberosTime packetTimestamp, Integer packetUsec, Integer packetSeqNumber, HostAddress packetSAddress,
        HostAddress packetRAddress, SeqNumber seqNumber, HostAddress sAddress, HostAddress rAddress,
        boolean timestampRequired, boolean seqNumberRequired, PrincipalName packetPrincipal, Realm packetRealm)
        throws KrbApErrException {

        if (!Krb5.AP_EMPTY_ADDRESSES_ALLOWED || sAddress != null) {
            if (packetSAddress == null || sAddress == null || !packetSAddress.equals(sAddress)) {
        ...
        if (!Krb5.AP_EMPTY_ADDRESSES_ALLOWED || rAddress != null) {
            if (packetRAddress == null || rAddress == null || !packetRAddress.equals(rAddress)) {
                throw new KrbApErrException(Krb5.KRB_AP_ERR_BADADDR);
            }
        }
        ...
        if (packetTimestamp != null) {
            ...
            if (!packetTimestamp.inClockSkew()) { throw new KrbApErrException(Krb5.KRB_AP_ERR_SKEW); }
        } else if (timestampRequired) { throw new KrbApErrException(Krb5.KRB_AP_ERR_SKEW); }
        ...
        if (seqNumber == null && seqNumberRequired == true) { throw new KrbApErrException(Krb5.API_INVALID_ARG); }
