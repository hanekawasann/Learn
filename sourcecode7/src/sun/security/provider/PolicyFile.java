...
public class PolicyFile extends java.security.Policy {
    ...
    private boolean init(URL policy, PolicyInfo newInfo) {
        ...
        try {
            ...
            try {
                keyStore = PolicyUtil
                    .getKeyStore(policy, pp.getKeyStoreUrl(), pp.getKeyStoreType(), pp.getKeyStoreProvider(),
                        pp.getStorePassURL(), debug);
            } catch (Exception e) {
            ...
            public Void run() {
                ...
                if (extCodebases != null && extCodebases.length > 0) {
                    for (int i = 0; i < extCodebases.length; i++) {
                        try {
                            pe = new PolicyEntry(
                                canonicalizeCodebase(new CodeSource(new URL(extCodebases[i]), (Certificate[]) null),
                                    false));
     ...
     */
    private CodeSource getCodeSource(PolicyParser.GrantEntry ge, KeyStore keyStore, PolicyInfo newInfo)
        ...
        try {
            ...
            while (enum_.hasMoreElements()) {
                ...
                try {
                    ...
                    Permission perm;
                    if (pe.permission.equals("javax.security.auth.PrivateCredentialPermission") &&
                    ...
                    MessageFormat form = new MessageFormat(
                        ResourcesMgr.getString(POLICY + ".error.adding.Permission.perm.message"));
...
*/
    ...
    private void addPermissions(Permissions perms, final CodeSource cs, Principal[] principals,
        ...
        if (debug != null) {
            ...
            if (principals != null) {
                for (int i = 0; i < principals.length; i++) {
                    accPs.add(
                        new PolicyParser.PrincipalEntry(principals[i].getClass().getName(), principals[i].getName()));
                }
            }
            debug.println(
        ...
        for (int i = 0; i < entryPs.size(); i++) {
            ...
            try {
                Class<?> pClass = Class
                    .forName(pppe.principalClass, true, Thread.currentThread().getContextClassLoader());
    ...
    private boolean checkEntryPs(Principal[] pList, PolicyParser.PrincipalEntry pppe) {
        ...
        for (int i = 0; i < pList.length; i++) {
            ...
            if (pppe.principalClass.equals(PolicyParser.PrincipalEntry.WILDCARD_CLASS) ||
                pppe.principalClass.equals(pList[i].getClass().getName())) {

                if (pppe.principalName.equals(PolicyParser.PrincipalEntry.WILDCARD_NAME) ||
                    pppe.principalName.equals(pList[i].getName())) {
    ...
    private void expandSelf(SelfPermission sp, List<PolicyParser.PrincipalEntry> entryPs, Principal[] pdp,
        Permissions perms) {
        ...
        if (entryPs == null || entryPs.size() == 0) {
            ...
            if (debug != null) {
                debug.println(
                    "Ignoring permission " + sp.getSelfType() + " with target name (" + sp.getSelfName() + ").  " +
                        "No Principal(s) specified " + "in the grant clause.  " + "SELF-based target names are " +
                        "only valid in the context " + "of a Principal-based grant entry.");
            }
            return;
        }
        int startIndex = 0;
        ...
        try {
            ...
            if (pc == null) {
                ...
                perms
                    .add(new UnresolvedPermission(sp.getSelfType(), sb.toString(), sp.getSelfActions(), sp.getCerts()));
            } else {
                try {
    ...
    private String[][] getPrincipalInfo(PolicyParser.PrincipalEntry pe, Principal[] pdp) {
        ...
        if (!pe.principalClass.equals(PolicyParser.PrincipalEntry.WILDCARD_CLASS) &&
            !pe.principalName.equals(PolicyParser.PrincipalEntry.WILDCARD_NAME)) {
        ...
        } else if (!pe.principalClass.equals(PolicyParser.PrincipalEntry.WILDCARD_CLASS) &&
            pe.principalName.equals(PolicyParser.PrincipalEntry.WILDCARD_NAME)) {
    ...
    protected Certificate[] getSignerCertificates(CodeSource cs) {
        ...
        while (i < certs.length) {
            ...
            while (((i + 1) < certs.length) &&
                ((X509Certificate) certs[i]).getIssuerDN().equals(((X509Certificate) certs[i + 1]).getSubjectDN())) {
                i++;
            }
            i++;
        }
    ...
    private CodeSource canonicalizeCodebase(CodeSource cs, boolean extractSignerCerts) {
        ...
        if (u != null) {
            ...
            if (u.getProtocol().equals("file")) {
                ...
                isLocalFile = (host == null || host.equals("") || host.equals("~") ||
    ...
    private String printPD(ProtectionDomain pd) {
        ...
        if (principals != null && principals.length > 0) {
            ...
            for (int i = 0; i < principals.length; i++) {
                ...
                if (i < principals.length - 1) { palBuf.append(", "); } else { palBuf.append(")"); }
            }
    ...
    private boolean replacePrincipals(List<PolicyParser.PrincipalEntry> principals, KeyStore keystore) {
        ...
        while (i.hasNext()) {
            ...
            if (pppe.principalClass.equals(PolicyParser.REPLACE_NAME)) {
                ...
                if (debug != null) {
                    debug.println(
                        "  Replacing \"" + pppe.principalName + "\" with " + X500PRINCIPAL + "/\"" + name + "\"");
                }
