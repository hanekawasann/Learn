...
class URICertStore extends CertStoreSpi {
    ...
    private Collection<X509Certificate> certs = Collections.<X509Certificate>emptySet();
    ...
    private static class LDAP {
        ...
        private static final CertStoreHelper helper = AccessController
            .doPrivileged(new PrivilegedAction<CertStoreHelper>() {
    ...
    URICertStore(CertStoreParameters params) throws InvalidAlgorithmParameterException, NoSuchAlgorithmException {
        ...
        if (uri.getScheme().toLowerCase(Locale.ENGLISH).equals("ldap")) {
            if (LDAP.helper() == null) { throw new NoSuchAlgorithmException("LDAP not present"); }
            ldap = true;
     ...
     * CertSelector.
     */
    private static Collection<X509Certificate> getMatchingCerts(Collection<X509Certificate> certs,
