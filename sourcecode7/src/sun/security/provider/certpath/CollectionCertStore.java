...
public class CollectionCertStore extends CertStoreSpi {
        ...
        for (int c = 0; c < 10; c++) {
            try {
                ...
                if (selector != null) {
                    ...
                    while (i.hasNext()) {
                        Object o = i.next();
                        if (o instanceof Certificate) { result.add((Certificate) o); }
                    }
                }
                return (result);
    ...
    public Collection<CRL> engineGetCRLs(CRLSelector selector) throws CertStoreException {
        if (coll == null) { throw new CertStoreException("Collection is null"); }
        ...
        for (int c = 0; c < 10; c++) {
            try {
                ...
                if (selector != null) {
                    while (i.hasNext()) {
                        Object o = i.next();
                        if ((o instanceof CRL) && selector.match((CRL) o)) { result.add((CRL) o); }
                    }
                } else {
                    while (i.hasNext()) {
                        Object o = i.next();
                        if (o instanceof CRL) { result.add((CRL) o); }
                    }
                }
                return (result);
            } catch (ConcurrentModificationException e) { }
