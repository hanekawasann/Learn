...
class DistributionPointFetcher {
     ...
     */
    private final static boolean USE_CRLDP = AccessController
        .doPrivileged(new GetBooleanAction("com.sun.security.enableCRLDP"));
     ...
     * empty set.
     */
    Collection<X509CRL> getCRLs(X509CRLSelector selector, boolean signFlag, PublicKey prevKey, String provider,
        List<CertStore> certStores, boolean[] reasonsMask, Set<TrustAnchor> trustAnchors, Date validity)
        ...
        try {
            ...
            List<DistributionPoint> points = (List<DistributionPoint>) ext.get(CRLDistributionPointsExtension.POINTS);
            Set<X509CRL> results = new HashSet<X509CRL>();
            for (Iterator<DistributionPoint> t = points.iterator();
                t.hasNext() && !Arrays.equals(reasonsMask, ALL_REASONS); ) {
                DistributionPoint point = t.next();
                Collection<X509CRL> crls = getCRLs(selector, certImpl, point, reasonsMask, signFlag, prevKey, provider,
     ...
     */
    private Collection<X509CRL> getCRLs(X509CRLSelector selector, X509CertImpl certImpl, DistributionPoint point,
        boolean[] reasonsMask, boolean signFlag, PublicKey prevKey, String provider, List<CertStore> certStores,
        ...
        for (X509CRL crl : possibleCRLs) {
            try {
                ...
                if (selector.match(crl) &&
                    verifyCRL(certImpl, point, crl, reasonsMask, signFlag, prevKey, provider, trustAnchors, certStores,
                        validity)) {
                    crls.add(crl);
                }
            } catch (Exception e) {
     ...
     */
    boolean verifyCRL(X509CertImpl certImpl, DistributionPoint point, X509CRL crl, boolean[] reasonsMask,
        boolean signFlag, PublicKey prevKey, String provider, Set<TrustAnchor> trustAnchors, List<CertStore> certStores,
        ...
        if (unresCritExts != null) {
            ...
            if (!unresCritExts.isEmpty()) {
                if (debug != null) {
                    ...
                    while (i.hasNext()) { debug.println(i.next()); }
                }
                return false;
            }
        }
