...
class BasicChecker extends PKIXCertPathChecker {
     ...
     */
    BasicChecker(TrustAnchor anchor, Date testDate, String sigProvider, boolean sigOnly)
     ...
     *                           extensions
     * @throws CertPathValidatorException Exception thrown if certificate
     *                                    does not verify.
     */
     ...
     *                                    does not verify.
     */
    private void verifySignature(X509Certificate cert, PublicKey prevPubKey, String sigProvider)
        throws CertPathValidatorException {
        String msg = "signature";
        if (debug != null) { debug.println("---checking " + msg + "..."); }
     ...
     */
    private void verifyTimestamp(X509Certificate cert, Date date) throws CertPathValidatorException {
     ...
     */
    private void verifyNameChaining(X509Certificate cert, X500Principal prevSubject) throws CertPathValidatorException {
        if (prevSubject != null) {
            ...
            if (X500Name.asX500Name(currIssuer).isEmpty()) {
                throw new CertPathValidatorException(
                    msg + " check failed: " + "empty/null issuer DN in certificate is invalid", null, null, -1,
    ...
    private void updateState(X509Certificate currCert) throws CertPathValidatorException {
        ...
        if (debug != null) {
            debug.println(
                "BasicChecker.updateState issuer: " + currCert.getIssuerX500Principal().toString() + "; subject: " +
                    currCert.getSubjectX500Principal() + "; serial#: " + currCert.getSerialNumber().toString());
        }
        if (cKey instanceof DSAPublicKey && ((DSAPublicKey) cKey).getParams() == null) {
            //cKey needs to inherit DSA parameters from prev key
            cKey = makeInheritedParamsKey(cKey, prevPubKey);
            if (debug != null) { debug.println("BasicChecker.updateState Made " + "key with inherited params"); }
        }
        prevPubKey = cKey;
     ...
     *                                    for this operation
     */
    static PublicKey makeInheritedParamsKey(PublicKey keyValueKey, PublicKey keyParamsKey)
        throws CertPathValidatorException {
        PublicKey usableKey;
        if (!(keyValueKey instanceof DSAPublicKey) || !(keyParamsKey instanceof DSAPublicKey)) {
            throw new CertPathValidatorException(
                "Input key is not " + "appropriate type for " + "inheriting parameters");
        }
        DSAParams params = ((DSAPublicKey) keyParamsKey).getParams();
        if (params == null) { throw new CertPathValidatorException("Key parameters missing"); }
        try {
            ...
            throw new CertPathValidatorException(
                "Unable to generate key with" + " inherited parameters: " + e.getMessage(), e);
        }
        return usableKey;
    }
