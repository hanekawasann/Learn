...
final class PolicyNodeImpl implements PolicyNode {
     ...
     */
    PolicyNodeImpl(PolicyNodeImpl parent, String validPolicy, Set<PolicyQualifierInfo> qualifierSet,
        boolean criticalityIndicator, Set<String> expectedPolicySet, boolean generatedByPolicyMapping) {
        mParent = parent;
        mChildren = new HashSet<PolicyNodeImpl>();

        if (validPolicy != null) { mValidPolicy = validPolicy; } else { mValidPolicy = ""; }

        if (qualifierSet != null) { mQualifierSet = new HashSet<PolicyQualifierInfo>(qualifierSet); } else {
            mQualifierSet = new HashSet<PolicyQualifierInfo>();
        }

        mCriticalityIndicator = criticalityIndicator;

        if (expectedPolicySet != null) { mExpectedPolicySet = new HashSet<String>(expectedPolicySet); } else {
    ...
    void prune(int depth) {
        if (isImmutable) { throw new IllegalStateException("PolicyNode is immutable"); }
        ...
        while (it.hasNext()) {
            ...
            if ((node.mChildren.size() == 0) && (depth > mDepth + 1)) { it.remove(); }
        }
    }
    ...
    private Set<PolicyNodeImpl> getPolicyNodesExpectedHelper(int depth, String expectedOID, boolean matchAny) {
        ...
        if (mDepth < depth) {
            ...
            if (matchAny) {
                if (mExpectedPolicySet.contains(ANY_POLICY)) { set.add(this); }
            } else {
                if (mExpectedPolicySet.contains(expectedOID)) { set.add(this); }
            }
        }
    ...
    Set<PolicyNodeImpl> getPolicyNodesValid(int depth, String validOID) {
        ...
        if (mDepth < depth) {
            ...
            if (mValidPolicy.equals(validOID)) { set.add(this); }
        }
