...
class OCSPChecker extends PKIXCertPathChecker {
     ...
     * @throws CertPathValidatorException Exception is thrown if the
    ...
    public void check(Certificate cert, Collection<String> unresolvedCritExts) throws CertPathValidatorException {
        ...
        if (remainingCerts < certs.length) {
            ...
            if (!seekResponderCert) {
                ...
                if (DEBUG != null) {
                    DEBUG.println(
                        "Responder's certificate is the same " + "as the issuer of the certificate being validated");
                }
            }
        }
            ...
            while (anchors.hasNext() && (seekIssuerCert || seekResponderCert)) {
                ...
                if (seekIssuerCert && certIssuerName.equals(anchorSubjectName)) {
                    ...
                    if (!seekResponderCert && responderCert == null) {
                        ...
                        if (DEBUG != null) {
                            DEBUG.println("Responder's certificate is the" + " same as the issuer of the certificate " +
                                "being validated");
                        }
                    }
                }
                ...
                if (seekResponderCert) {
                    ...
                    if ((responderSubjectName != null && responderSubjectName.equals(anchorSubjectName)) ||
                        (responderIssuerName != null && responderSerialNumber != null &&
                            responderIssuerName.equals(anchorCert.getIssuerX500Principal()) &&
                            responderSerialNumber.equals(anchorCert.getSerialNumber()))) {
        ...
        if (certStatus == RevocationStatus.CertStatus.REVOKED) {
            Throwable t = new CertificateRevokedException(rs.getRevocationTime(), rs.getRevocationReason(),
                responderCert.getSubjectX500Principal(), rs.getSingleExtensions());
            throw new CertPathValidatorException(t.getMessage(), t, null, -1, BasicReason.REVOKED);
        } else if (certStatus == RevocationStatus.CertStatus.UNKNOWN) {
            throw new CertPathValidatorException("Certificate's revocation status is unknown", null, cp, remainingCerts,
     ...
     *   4. ocsp.responderCertSerialNumber
     */
    private static URI getOCSPServerURI(X509CertImpl currCertImpl, String responderURL)
