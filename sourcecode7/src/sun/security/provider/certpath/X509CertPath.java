...
public class X509CertPath extends CertPath {
    ...
    public X509CertPath(List<? extends Certificate> certs) throws CertificateException {
        ...
        this.certs = Collections.unmodifiableList(new ArrayList<X509Certificate>((List<X509Certificate>) certs));
    }
    ...
    private static List<X509Certificate> parsePKIPATH(InputStream is) throws CertificateException {
        ...
        try {
            ...
            for (int i = seq.length - 1; i >= 0; i--) {
                certList
                    .add((X509Certificate) certFac.generateCertificate(new ByteArrayInputStream(seq[i].toByteArray())));
            }

            return Collections.unmodifiableList(certList);

        } catch (IOException ioe) {
            CertificateException ce = new CertificateException("IOException" + " parsing PkiPath data: " + ioe);
            ce.initCause(ioe);
            throw ce;
        }
    }
    ...
    private byte[] encodePKCS7() throws CertificateEncodingException {
        PKCS7 p7 = new PKCS7(new AlgorithmId[0], new ContentInfo(ContentInfo.DATA_OID, null),
            certs.toArray(new X509Certificate[certs.size()]), new SignerInfo[0]);
        DerOutputStream derout = new DerOutputStream();
        try {
