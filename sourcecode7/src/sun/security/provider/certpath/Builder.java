...
public abstract class Builder {
     ...
     */
    final static boolean USE_AIA = AccessController
        .doPrivileged(new GetBooleanAction("com.sun.security.enableAIAcaIssuers"));
    ...
    Builder(PKIXBuilderParameters buildParams, X500Principal targetSubjectDN) {
        ...
        this.targetCertConstraints = (X509CertSelector) buildParams.getTargetCertConstraints();
    }
     ...
     */
    abstract Collection<X509Certificate> getMatchingCerts(State currentState, List<CertStore> certStores)
     ...
     */
    abstract void verifyCert(X509Certificate cert, State currentState, List<X509Certificate> certPathList)
     ...
     */
    abstract void addCertToPath(X509Certificate cert, LinkedList<X509Certificate> certPathList);
     ...
     * @param certPathList the certification path list
     */
    abstract void removeFinalCertFromPath(LinkedList<X509Certificate> certPathList);
     ...
     */
    static int targetDistance(NameConstraintsExtension constraints, X509Certificate cert, GeneralNameInterface target)
        throws IOException {
        ...
        if (altNameExt != null) {
            GeneralNames altNames = (GeneralNames) altNameExt.get(altNameExt.SUBJECT_NAME);
        ...
        GeneralSubtrees permitted = (GeneralSubtrees) constraints.get(constraints.PERMITTED_SUBTREES);
        GeneralSubtrees excluded = (GeneralSubtrees) constraints.get(constraints.EXCLUDED_SUBTREES);
        if (permitted != null) {
    ...
    Set<String> getMatchingPolicies() {
        if (matchingPolicies != null) {
            ...
            if ((!initialPolicies.isEmpty()) && (!initialPolicies.contains(PolicyChecker.ANY_POLICY)) &&
     ...
     */
    boolean addMatchingCerts(X509CertSelector selector, Collection<CertStore> certStores,
        ...
        if (targetCert != null) {
            ...
            if (selector.match(targetCert) && !X509CertImpl.isSelfSigned(targetCert, buildParams.getSigProvider())) {
                if (debug != null) {
        ...
        for (CertStore store : certStores) {
            try {
                ...
                for (Certificate cert : certs) {
                    if (!X509CertImpl.isSelfSigned((X509Certificate) cert, buildParams.getSigProvider())) {
    ...
    static boolean isLocalCertStore(CertStore certStore) {
        return (certStore.getType().equals("Collection") ||
            certStore.getCertStoreParameters() instanceof CollectionCertStoreParameters);
    }
}
