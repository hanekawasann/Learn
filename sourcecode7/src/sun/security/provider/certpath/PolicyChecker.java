...
class PolicyChecker extends PKIXCertPathChecker {
     ...
     */
    PolicyChecker(Set<String> initialPolicies, int certPathLen, boolean expPolicyRequired, boolean polMappingInhibited,
        boolean anyPolicyInhibited, boolean rejectPolicyQualifiers, PolicyNodeImpl rootNode)
     ...
     * @throws CertPathValidatorException Exception thrown if user
    ...
    private void checkPolicy(X509Certificate currCert) throws CertPathValidatorException {
        ...
        rootNode = processPolicies(certIndex, initPolicies, explicitPolicy, policyMapping, inhibitAnyPolicy,
            rejectPolicyQualifiers, rootNode, currCertImpl, finalCert);
     ...
     * @throws CertPathValidatorException Exception thrown if an error
     *                                    occurs
     */
    static int mergeExplicitPolicy(int explicitPolicy, X509CertImpl currCert, boolean finalCert)
        throws CertPathValidatorException {
        ...
        try {
            ...
            if (polConstExt == null) { return explicitPolicy; }
            int require = ((Integer) polConstExt.get(PolicyConstraintsExtension.REQUIRE)).intValue();
            if (debug != null) {
    ...
    static int mergePolicyMapping(int policyMapping, X509CertImpl currCert) throws CertPathValidatorException {
        ...
        try {
            ...
            int inhibit = ((Integer) polConstExt.get(PolicyConstraintsExtension.INHIBIT)).intValue();
            if (debug != null) {
    ...
    static int mergeInhibitAnyPolicy(int inhibitAnyPolicy, X509CertImpl currCert) throws CertPathValidatorException {
        ...
        try {
            InhibitAnyPolicyExtension inhAnyPolExt = (InhibitAnyPolicyExtension) currCert
                .getExtension(PKIXExtensions.InhibitAnyPolicy_Id);
            if (inhAnyPolExt == null) { return inhibitAnyPolicy; }

            int skipCerts = ((Integer) inhAnyPolExt.get(InhibitAnyPolicyExtension.SKIP_CERTS)).intValue();
            if (debug != null) {
     ...
     * @throws CertPathValidatorException Exception thrown if an
     *                                    error occurs while processing policies.
     */
    static PolicyNodeImpl processPolicies(int certIndex, Set<String> initPolicies, int explicitPolicy,
        int policyMapping, int inhibitAnyPolicy, boolean rejectPolicyQualifiers, PolicyNodeImpl origRootNode,
        X509CertImpl currCert, boolean finalCert) throws CertPathValidatorException {
        ...
        if (origRootNode == null) { rootNode = null; } else { rootNode = origRootNode.copyTree(); }
        ...
        if ((currCertPolicies != null) && (rootNode != null)) {
            ...
            try {
                policyInfo = (List<PolicyInformation>) currCertPolicies.get(CertificatePoliciesExtension.POLICIES);
            ...
            if (debug != null) {
                debug
                    .println("PolicyChecker.processPolicies() " + "rejectPolicyQualifiers = " + rejectPolicyQualifiers);
            }
            ...
            for (PolicyInformation curPolInfo : policyInfo) {
                ...
                if (curPolicy.equals(ANY_POLICY)) {
                    ...
                    if (!pQuals.isEmpty() && rejectPolicyQualifiers && policiesCritical) {
                        throw new CertPathValidatorException("critical policy qualifiers present in certificate", null,
                            null, -1, PKIXReason.INVALID_POLICY);
                    }

                    // PKIX: Section 6.1.3: Step (d)(1)(i)
                    boolean foundMatch = processParents(certIndex, policiesCritical, rejectPolicyQualifiers, rootNode,
                        curPolicy, pQuals, false);

                    if (!foundMatch) {
                        // PKIX: Section 6.1.3: Step (d)(1)(ii)
                        processParents(certIndex, policiesCritical, rejectPolicyQualifiers, rootNode, curPolicy, pQuals,
                            true);
                    }
                }
            }
            ...
            if (foundAnyPolicy) {
                if ((inhibitAnyPolicy > 0) || (!finalCert && X509CertImpl.isSelfIssued(currCert))) {
                    ...
                    processParents(certIndex, policiesCritical, rejectPolicyQualifiers, rootNode, ANY_POLICY, anyQuals,
                        true);
                }
            }
            ...
            if (debug != null) { debug.println("PolicyChecker.processPolicies() " + "no policies present in cert"); }
        ...
        if (rootNode != null) {
            if (!finalCert) {
                ...
                rootNode = processPolicyMappings(currCert, certIndex, policyMapping, rootNode, policiesCritical,
                    anyQuals);
            }
        }
        ...
        if ((explicitPolicy == 0) && (rootNode == null)) {
            throw new CertPathValidatorException("non-null policy tree required and policy tree is null", null, null,
                -1, PKIXReason.INVALID_POLICY);
        }
    ...
    private static PolicyNodeImpl rewriteLeafNodes(int certIndex, Set<String> initPolicies, PolicyNodeImpl rootNode) {
        ...
        if (initial.isEmpty()) {
            ...
            for (String policy : initial) {
                ...
                PolicyNodeImpl node = new PolicyNodeImpl(parentNode, policy, anyQualifiers, anyCritical,
                    expectedPolicies, false);
            }
        }
        return rootNode;
    }
     ...
     * @throws CertPathValidatorException Exception thrown if error occurs.
     */
    private static boolean processParents(int certIndex, boolean policiesCritical, boolean rejectPolicyQualifiers,
        PolicyNodeImpl rootNode, String curPolicy, Set<PolicyQualifierInfo> pQuals, boolean matchAny)
        throws CertPathValidatorException {
        boolean foundMatch = false;

        if (debug != null) { debug.println("PolicyChecker.processParents(): matchAny = " + matchAny); }
                ...
                for (String curParExpPol : parExpPols) {
                    ...
                    while (childIter.hasNext()) {
                        ...
                        if (curParExpPol.equals(childPolicy)) {
                            if (debug != null) {
                                debug.println(
                                    childPolicy + " in parent's " + "expected policy set already appears in " +
                                        "child node");
                            }
                    ...
                    curNode = new PolicyNodeImpl(curParent, curParExpPol, pQuals, policiesCritical, expPols, false);
                }
            } else {
                curExpPols = new HashSet<String>();
                curExpPols.add(curPolicy);

                curNode = new PolicyNodeImpl(curParent, curPolicy, pQuals, policiesCritical, curExpPols, false);
            }
        }
     ...
     */
    private static PolicyNodeImpl processPolicyMappings(X509CertImpl currCert, int certIndex, int policyMapping,
        PolicyNodeImpl rootNode, boolean policiesCritical, Set<PolicyQualifierInfo> anyQuals)
        throws CertPathValidatorException {
        PolicyMappingsExtension polMappingsExt = currCert.getPolicyMappingsExtension();

        if (polMappingsExt == null) { return rootNode; }

        if (debug != null) { debug.println("PolicyChecker.processPolicyMappings() " + "inside policyMapping check"); }
        ...
        for (int j = 0; j < maps.size(); j++) {
            ...
            if (issuerDomain.equals(ANY_POLICY)) {
                throw new CertPathValidatorException("encountered an issuerDomainPolicy of ANY_POLICY", null, null, -1,
                    PKIXReason.INVALID_POLICY);
            }

            if (subjectDomain.equals(ANY_POLICY)) {
                throw new CertPathValidatorException("encountered a subjectDomainPolicy of ANY_POLICY", null, null, -1,
                    PKIXReason.INVALID_POLICY);
            }
            ...
            if (!validNodes.isEmpty()) {
                for (PolicyNodeImpl curNode : validNodes) {
                    if ((policyMapping > 0) || (policyMapping == -1)) {
                        ...
                        if (debug != null) {
                            debug.println("PolicyChecker.processPolicyMappings" + "() before deleting: policy tree = " +
                                rootNode);
                        }
                        ...
                        if (debug != null) {
                            debug.println(
                                "PolicyChecker.processPolicyMappings" + "() after deleting: policy tree = " + rootNode);
                        }
                    }
                }
                ...
                if ((policyMapping > 0) || (policyMapping == -1)) {
                    ...
                    for (PolicyNodeImpl curAnyNode : validAnyNodes) {
                        ...
                        PolicyNodeImpl curNode = new PolicyNodeImpl(curAnyNodeParent, issuerDomain, anyQuals,
        ...
        if (childDeleted) {
            rootNode.prune(certIndex);
            if (!rootNode.getChildren().hasNext()) {
                if (debug != null) { debug.println("setting rootNode to null"); }
                rootNode = null;
            }
        }
     ...
     */
    private static PolicyNodeImpl removeInvalidNodes(PolicyNodeImpl rootNode, int certIndex, Set<String> initPolicies,
        CertificatePoliciesExtension currCertPolicies) throws CertPathValidatorException {
            ...
            for (PolicyNodeImpl curNode : validNodes) {
                ...
                if (parentNode.getValidPolicy().equals(ANY_POLICY)) {
                    if ((!initPolicies.contains(curPolicy)) && (!curPolicy.equals(ANY_POLICY))) {
                        if (debug != null) {
                            debug.println(
                                "PolicyChecker.processPolicies() " + "before deleting: policy tree = " + rootNode);
                        }
                        parentNode.deleteChild(curNode);
                        childDeleted = true;
                        if (debug != null) {
                            debug.println(
                                "PolicyChecker.processPolicies() " + "after deleting: policy tree = " + rootNode);
                        }
                    }
                }
            }
    ...
    PolicyNode getPolicyTree() {
        if (rootNode == null) { return null; } else {
