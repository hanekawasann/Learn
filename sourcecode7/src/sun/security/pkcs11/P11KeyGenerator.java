...
final class P11KeyGenerator extends KeyGeneratorSpi {
            ...
            default:
                ...
                if (keySize < minKeySize || keySize > maxKeySize) {
                    throw new InvalidAlgorithmParameterException(
                        "Key length must be between " + minKeySize + " and " + maxKeySize + " bits");
                }
                if (keyGenMech == CKM_AES_KEY_GEN) {
                    if ((keySize != 128) && (keySize != 192) && (keySize != 256)) {
                        throw new InvalidAlgorithmParameterException(
                            "AES key length must be " + minKeySize + (maxKeySize >= 192 ? ", 192" : "") +
    ...
    P11KeyGenerator(Token token, String algorithm, long mechanism) throws PKCS11Exception {
        ...
        if (this.mechanism == CKM_DES3_KEY_GEN) {
            ...
            */
            supportBothKeySizes = (token.provider.config.isEnabled(CKM_DES2_KEY_GEN) &&
    ...
    protected void engineInit(int keySize, SecureRandom random) {
        ...
        if ((mechanism == CKM_DES2_KEY_GEN) || (mechanism == CKM_DES3_KEY_GEN)) {
            long newMechanism = (newSignificantKeySize == 112 ? CKM_DES2_KEY_GEN : CKM_DES3_KEY_GEN);
            if (mechanism != newMechanism) {
                if (supportBothKeySizes) {
                    mechanism = newMechanism;
                    // Adjust keyType to reflect the mechanism change
                    keyType = (mechanism == CKM_DES2_KEY_GEN ? CKK_DES2 : CKK_DES3);
                } else {
                    throw new InvalidParameterException("Only " + significantKeySize + "-bit DESede is supported");
                }
            }
        }
    ...
    protected SecretKey engineGenerateKey() {
        ...
        try {
            ...
            long keyID = token.p11.C_GenerateKey(session.id(), new CK_MECHANISM(mechanism), attributes);
            return P11Key.secretKey(session, keyID, algorithm, significantKeySize, attributes);
