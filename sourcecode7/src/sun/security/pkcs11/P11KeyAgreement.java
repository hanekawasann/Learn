...
final class P11KeyAgreement extends KeyAgreementSpi {
    ...
    protected Key engineDoPhase(Key key, boolean lastPhase) throws InvalidKeyException, IllegalStateException {
        ...
        if ((multiPartyAgreement != null) || (lastPhase == false)) {
            if (multiPartyAgreement == null) {
                try {
                    multiPartyAgreement = KeyAgreement.getInstance("DH", P11Util.getSunJceProvider());
        ...
        if ((key instanceof PublicKey == false) || (key.getAlgorithm().equals(algorithm) == false)) {
        ...
        if (key instanceof DHPublicKey) {
            ...
            try {
                DHPublicKeySpec spec = (DHPublicKeySpec) kf.engineGetKeySpec(key, DHPublicKeySpec.class);
                y = spec.getY();
                p = spec.getP();
                g = spec.getG();
        ...
        if (privateKey instanceof DHPrivateKey) {
            ...
            DHParameterSpec params = dhKey.getParams();
            if ((p.equals(params.getP()) == false) || (g.equals(params.getG()) == false)) {
    ...
    protected byte[] engineGenerateSecret() throws IllegalStateException {
        ...
        try {
            ...
            CK_ATTRIBUTE[] attributes = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_CLASS, CKO_SECRET_KEY),
                new CK_ATTRIBUTE(CKA_KEY_TYPE, CKK_GENERIC_SECRET), };
            attributes = token.getAttributes(O_GENERATE, CKO_SECRET_KEY, CKK_GENERIC_SECRET, attributes);
            long keyID = token.p11
                .C_DeriveKey(session.id(), new CK_MECHANISM(mechanism, publicValue), privateKey.keyID, attributes);
            attributes = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_VALUE) };
    ...
    protected int engineGenerateSecret(byte[] sharedSecret, int offset)
        ...
        if (offset + secretLen > sharedSecret.length) {
            throw new ShortBufferException(
                "Need " + secretLen + " bytes, only " + (sharedSecret.length - offset) + " available");
        }
    ...
    private SecretKey nativeGenerateSecret(String algorithm)
        ...
        try {
            ...
            long keyID = token.p11
                .C_DeriveKey(session.id(), new CK_MECHANISM(mechanism, publicValue), privateKey.keyID, attributes);
            CK_ATTRIBUTE[] lenAttributes = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_VALUE_LEN), };
