...
final class P11TlsPrfGenerator extends KeyGeneratorSpi {
    ...
    private final static String MSG = "TlsPrfGenerator must be initialized using a TlsPrfParameterSpec";
    ...
    protected SecretKey engineGenerateKey() {
        ...
        if (mechanism == CKM_NSS_TLS_PRF_GENERAL) {
            ...
            try {
                ...
                byte[] out = token.p11.C_SignFinal(session.id(), spec.getOutputLength());
        ...
        try {
            session = token.getOpSession();
            long keyID = token.p11.C_DeriveKey(session.id(), new CK_MECHANISM(mechanism, params), p11Key.keyID, null);
