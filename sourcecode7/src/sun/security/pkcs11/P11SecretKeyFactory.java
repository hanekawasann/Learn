...
final class P11SecretKeyFactory extends SecretKeyFactorySpi {
     ...
     */
    static P11Key convertKey(Token token, Key key, String algo, CK_ATTRIBUTE[] extraAttrs) throws InvalidKeyException {
        token.ensureValid();
        if (key == null) {
            ...
            if (p11Key.token == token) {
                if (extraAttrs != null) {
                    ...
                    try {
                        ...
                        long newKeyID = token.p11.C_CopyObject(session.id(), p11Key.keyID, extraAttrs);
                        p11Key = (P11Key) (P11Key
                            .secretKey(session, newKeyID, p11Key.algorithm, p11Key.keyLength, extraAttrs));
    ...
    private static P11Key createKey(Token token, byte[] encoded, String algorithm, long keyType,
        ...
        try {
            switch ((int) keyType) {
                case (int) CKK_DES:
                    keyLength = P11KeyGenerator.checkKeySize(CKM_DES_KEY_GEN, n, token);
                    fixDESParity(encoded, 0);
                    break;
                case (int) CKK_DES3:
                    keyLength = P11KeyGenerator.checkKeySize(CKM_DES3_KEY_GEN, n, token);
                    fixDESParity(encoded, 0);
                    ...
                    break;
                case (int) CKK_AES:
                    keyLength = P11KeyGenerator.checkKeySize(CKM_AES_KEY_GEN, n, token);
                    break;
                case (int) CKK_RC4:
                    keyLength = P11KeyGenerator.checkKeySize(CKM_RC4_KEY_GEN, n, token);
                    break;
                case (int) CKK_BLOWFISH:
                    keyLength = P11KeyGenerator.checkKeySize(CKM_BLOWFISH_KEY_GEN, n, token);
                    break;
        ...
        try {
            ...
            attributes = token.getAttributes(O_IMPORT, CKO_SECRET_KEY, keyType, attributes);
            session = token.getObjSession();
            long keyID = token.p11.C_CreateObject(session.id(), attributes);
            P11Key p11Key = (P11Key) P11Key.secretKey(session, keyID, algorithm, keyLength, attributes);
            return p11Key;
