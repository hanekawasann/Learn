...
public abstract class MappedMXBeanType {
    private static final WeakHashMap<Type, MappedMXBeanType> convertedTypes = new WeakHashMap<Type, MappedMXBeanType>();
        ...
        Object toJavaTypeData(Object data) throws OpenDataException, InvalidObjectException {
            ...
            try {
                ...
                final InvalidObjectException ioe = new InvalidObjectException(
                    "Enum constant named " + (String) data + " is missing");
                ioe.initCause(e);
                throw ioe;
            }
        }
    ...
    static class MapMXBeanType extends MappedMXBeanType {
        ...
        MapMXBeanType(ParameterizedType pt) throws OpenDataException {
            ...
            final CompositeType rowType = new CompositeType(typeName, typeName, mapItemNames, mapItemNames,
                mapItemTypes);
    ...
    static class CompositeDataMXBeanType extends MappedMXBeanType {
        ...
        CompositeDataMXBeanType(Class c) throws OpenDataException {
            ...
            if (COMPOSITE_DATA_CLASS.isAssignableFrom(c)) {
                ...
                for (int i = 0; i < methods.length; i++) {
                    ...
                    if (rest.equals("") || method.getParameterTypes().length > 0 || type == void.class ||
                        rest.equals("Class")) {
                ...
                openType = new CompositeType(c.getName(), c.getName(), nameArray, // field names
        ...
        Object toJavaTypeData(Object data) throws OpenDataException, InvalidObjectException {
            ...
            try {
                ...
                final OpenDataException ode = new OpenDataException(
                    "Failed to invoke " + fromMethod.getName() + " to convert CompositeData " + " to " +
                        javaClass.getName());
                ode.initCause(e);
                throw ode;
            }
    ...
    private static final OpenType[] simpleTypes = { BIGDECIMAL, BIGINTEGER, BOOLEAN, BYTE, CHARACTER, DATE, DOUBLE,
    ...
    static {
        try {
            for (int i = 0; i < simpleTypes.length; i++) {
                ...
                Class c;
                try {
                    c = Class.forName(t.getClassName(), false, String.class.getClassLoader());
