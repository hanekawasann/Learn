...
public class NotificationTargetImpl implements NotificationTarget {
    ...
    private void parseTarget(String target) throws IllegalArgumentException, UnknownHostException {
        ...
        if (target.startsWith("[")) {
            ...
            if (index == -1) { throw new IllegalArgumentException("Host starts with [ but " + "does not end with ]"); }
            addrStr = target.substring(1, index);
            port = Integer.parseInt(target.substring(index + 2, index2));
            if (!isNumericIPv6Address(addrStr)) {
                throw new IllegalArgumentException("Address inside [...] must " + "be numeric IPv6 address");
            }
            if (addrStr.startsWith("[")) { throw new IllegalArgumentException("More than one [[...]]"); }
        } else {
            final int index = target.indexOf(":");
            final int index2 = target.lastIndexOf(":");
            if (index == -1) { throw new IllegalArgumentException("Missing port separator \":\""); }
            addrStr = target.substring(0, index);

            port = Integer.parseInt(target.substring(index + 1, index2));
        }
