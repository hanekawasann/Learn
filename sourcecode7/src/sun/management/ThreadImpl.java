...
class ThreadImpl implements com.sun.management.ThreadMXBean {
    ...
    private void verifyThreadIds(long[] ids) {
        ...
        for (int i = 0; i < ids.length; i++) {
            if (ids[i] <= 0) {
                throw new IllegalArgumentException("Invalid thread ID parameter: " + ids[i]);
            }
        }
    }
            ...
            for (int i = 0; i < ids.length; i++) {
                if (ids[i] != Thread.currentThread().getId()) {
                    throw new UnsupportedOperationException(
                        "Thread CPU time measurement is only supported" + " for the current thread.");
                }
            }
        }
    ...
    private static native void getThreadInfo1(long[] ids, int maxDepth, ThreadInfo[] result);
    ...
    private static native ThreadInfo[] dumpThreads0(long[] ids, boolean lockedMonitors, boolean lockedSynchronizers);
