...
class JapaneseImperialCalendar extends Calendar {
     ...
     * *: depends on eras
     */
    static final int[] MIN_VALUES = { 0,              // ERA
        -292275055,     // YEAR
        JANUARY,        // MONTH
        1,              // WEEK_OF_YEAR
        0,              // WEEK_OF_MONTH
    ...
    static final int[] LEAST_MAX_VALUES = { 0,              // ERA (initialized later)
    ...
    static final int[] MAX_VALUES = { 0,              // ERA
        292278994,      // YEAR
        DECEMBER,       // MONTH
        53,             // WEEK_OF_YEAR
        6,              // WEEK_OF_MONTH
        ...
        switch (field) {
            ...
            case DAY_OF_MONTH: {
                ...
                assert getEraIndex(d) == internalGetEra() && d.getYear() == internalGet(YEAR) &&
                    d.getMonth() - 1 == internalGet(MONTH);
                set(DAY_OF_MONTH, d.getDayOfMonth());
                return;
            }
                ...
                if (!(transitionYear || isTransitionYear(d.getNormalizedYear()))) {
                    ...
                    if (jdate.getYear() == 1) {
                        ...
                        if (era > HEISEI) {
                            ...
                            d.setMonth(BaseCalendar.JANUARY).setDayOfMonth(1);
                        }
                        jcal.normalize(d);
        ...
        if (!isLenient()) {
            for (int field = 0; field < FIELD_COUNT; field++) {
                ...
                if (originalFields[field] != internalGet(field)) {
                    ...
                    throw new IllegalArgumentException(
                        getFieldName(field) + "=" + wrongValue + ", expected " + originalFields[field]);
                }
            }
        }
    ...
    private final int monthLength(int month, int gregorianYear) {
        return CalendarUtils.isGregorianLeapYear(gregorianYear) ? GregorianCalendar.LEAP_MONTH_LENGTH[month]
            : GregorianCalendar.MONTH_LENGTH[month];
    }
