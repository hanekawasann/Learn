...
class PatternEntry {
    ...
    void addToBuffer(StringBuffer toAddTo, boolean showExtension, boolean showWhiteSpace, PatternEntry lastEntry) {
        if (showWhiteSpace && toAddTo.length() > 0) {
            if (strength == Collator.PRIMARY || lastEntry != null) { toAddTo.append('\n'); } else {
                toAddTo.append(' ');
            }
        }
        if (lastEntry != null) {
    ...
    static class Parser {
        ...
        public PatternEntry next() throws ParseException {
            ...
            while (i < pattern.length()) {
                ...
                if (inQuote) {
                    if (ch == '\'') {
                        ...
                        if (newChars.length() == 0) { newChars.append(ch); } else if (inChars) {
                            newChars.append(ch);
                    ...
                    switch (ch) {
                        case '=':
                            if (newStrength != UNSET) { break mainLoop; }
                        ...
                        default:
                            if (newStrength == UNSET) {
                                throw new ParseException("missing char (=,;<&) : " +
                                    pattern.substring(i, (i + 10 < pattern.length()) ? i + 10 : pattern.length()), i);
                            }
                            if (PatternEntry.isSpecialChar(ch) && (inQuote == false)) {
                                throw new ParseException("Unquoted punctuation character : " + Integer.toString(ch, 16),
                                    i);
                            }
                            if (inChars) {
                                newChars.append(ch);
            ...
            if (newStrength == UNSET) { return null; }
            if (newChars.length() == 0) {
                throw new ParseException("missing chars (=,;<&): " +
                    pattern.substring(i, (i + 10 < pattern.length()) ? i + 10 : pattern.length()), i);
            }
