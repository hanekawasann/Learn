...
public final class Console implements Flushable {
    ...
    public String readLine(String fmt, Object... args) {
        ...
        synchronized (writeLock) {
            synchronized (readLock) {
                if (fmt.length() != 0) { pw.format(fmt, args); }
                try {
                    char[] ca = readline(false);
                    if (ca != null) { line = new String(ca); }
                } catch (IOException x) {
                    throw new IOError(x);
                }
            }
            ...
            synchronized (readLock) {
                ...
                try {
                    ...
                    try {
                        ...
                        if (ioe == null) { ioe = new IOError(x); } else { ioe.addSuppressed(x); }
                    }
    ...
    static {
        try {
            ...
            // it be necessary.
            sun.misc.SharedSecrets.getJavaLangAccess().registerShutdownHook(0 /* shutdown hook invocation order */,
                false /* only register if shutdown is not in progress */, new Runnable() {
                    public void run() {
                        try {
                            if (echoOff) {
                                echo(true);
