...
public abstract class PathGraphics extends ProxyGraphics2D {
    ...
    protected PathGraphics(Graphics2D graphics, PrinterJob printerJob, Printable painter, PageFormat pageFormat,
     ...
     */
    public abstract void redrawRegion(Rectangle2D region, double scaleX, double scaleY, Shape clip,
    ...
    protected static SoftReference<Hashtable<Font2DHandle, Object>> fontMapRef
        = new SoftReference<Hashtable<Font2DHandle, Object>>(null);
    ...
    boolean printedSimpleGlyphVector(GlyphVector g, float x, float y) {
        ...
        boolean noPositionAdjustments = ((flags & GlyphVector.FLAG_HAS_POSITION_ADJUSTMENTS) == 0) ||
    ...
    private boolean samePositions(GlyphVector gv, int[] gvcodes, int[] origCodes, float[] origPositions) {
        ...
        if (numGlyphs != gvcodes.length ||  /* real paranoia here */
            origCodes.length != gvcodes.length || origPositions.length != gvpos.length) {
            return false;
        }
    ...
    private static char[] getGlyphToCharMapForFont(Font2D font2D) {
        ...
        for (char c = 0; c < 0xFFFF; c++) {
            ...
            if (glyph != missingGlyph && glyph >= 0 && glyph < numGlyphs &&
                (glyphToCharMap[glyph] == CharToGlyphMapper.INVISIBLE_GLYPH_ID)) {
    ...
    protected boolean isBitmaskTransparency(BufferedImage bufferedImage) {
        ColorModel colorModel = bufferedImage.getColorModel();
        return (colorModel != null && colorModel.getTransparency() == ColorModel.BITMASK);
    }
     ...
     */
    protected boolean drawBitmaskImage(BufferedImage bufferedImage, AffineTransform xform, Color bgcolor, int srcX,
        ...
        if ((xform.getType() & ~(AffineTransform.TYPE_UNIFORM_SCALE | AffineTransform.TYPE_TRANSLATION |
            AffineTransform.TYPE_QUADRANT_ROTATION)) != 0) {
            return false;
        }

        if ((getTransform().getType() & ~(AffineTransform.TYPE_UNIFORM_SCALE | AffineTransform.TYPE_TRANSLATION |
            AffineTransform.TYPE_QUADRANT_ROTATION)) != 0) {
            return false;
        }
     ...
     */
    protected abstract boolean drawImageToPlatform(Image img, AffineTransform xform, Color bgcolor, int srcX, int srcY,
    ...
    public boolean drawImage(Image img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2,
        ...
        AffineTransform xForm = new AffineTransform(scalex, 0, 0, scaley, dx1 - (sx1 * scalex), dy1 - (sy1 * scaley));
    ...
    public void drawRenderedImage(RenderedImage img, AffineTransform xform) {
        ...
        if (img instanceof BufferedImage) {
            ...
            bufferedImage = new BufferedImage(srcWidth, srcHeight, BufferedImage.TYPE_INT_ARGB);
            Graphics2D imageGraphics = bufferedImage.createGraphics();
            imageGraphics.drawRenderedImage(img, xform);
        }

        drawImageToPlatform(bufferedImage, xform, null, 0, 0, srcWidth, srcHeight, false);
