...
public abstract class RasterPrinterJob extends PrinterJob {
    ...
    static {
         ...
         */
        String forceStr = (String) java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction(FORCE_PIPE_PROP));
        ...
        String shapeTextStr = (String) java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction(SHAPE_TEXT_PROP));
     ...
     * Begin a new page.
     */
    abstract protected void startPage(PageFormat format, Printable painter, int index, boolean paperChanged)
     ...
     */
    abstract protected void printBand(byte[] data, int x, int y, int width, int height) throws PrinterException;
    ...
    protected static PrintService lookupDefaultPrintService() {
        ...
        if (service != null && service.isDocFlavorSupported(DocFlavor.SERVICE_FORMATTED.PAGEABLE) &&
            service.isDocFlavorSupported(DocFlavor.SERVICE_FORMATTED.PRINTABLE)) {
            return service;
        } else {
    ...
    public PrintService getPrintService() {
        if (myService == null) {
            ...
            if (myService == null) {
                PrintService[] svcs = PrintServiceLookup
                    .lookupPrintServices(DocFlavor.SERVICE_FORMATTED.PAGEABLE, null);
                if (svcs.length > 0) {
                    try {
    ...
    public void setPrintService(PrintService service) throws PrinterException {
        if (service == null) {
            ...
            if (prnState == PrinterState.STOPPED) {
                PrinterStateReasons prnStateReasons = (PrinterStateReasons) service
            ...
            if (service.isDocFlavorSupported(DocFlavor.SERVICE_FORMATTED.PAGEABLE) &&
                service.isDocFlavorSupported(DocFlavor.SERVICE_FORMATTED.PRINTABLE)) {
                myService = service;
            } else {
    ...
    protected void updateAttributesWithPageFormat(PrintService service, PageFormat page,
        PrintRequestAttributeSet pageAttributes) {
        if (service == null || page == null || pageAttributes == null) {
            return;
        }

        float x = (float) Math.rint((page.getPaper().getWidth() * Size2DSyntax.INCH) / (72.0)) /
            (float) Size2DSyntax.INCH;
        float y = (float) Math.rint((page.getPaper().getHeight() * Size2DSyntax.INCH) / (72.0)) /
            (float) Size2DSyntax.INCH;
    ...
    public PageFormat pageDialog(PageFormat page) throws HeadlessException {
        ...
        PrintService service = (PrintService) java.security.AccessController
    ...
    public PageFormat pageDialog(final PrintRequestAttributeSet attributes) throws HeadlessException {
        ...
        if (dlg == DialogTypeSelection.NATIVE) {
            ...
            PageFormat page = pageDialog(attributeToPageFormat(pservice, attributes));
        ...
        ServiceDialog pageDialog = new ServiceDialog(gc, x, y, service, DocFlavor.SERVICE_FORMATTED.PAGEABLE,
    ...
    public boolean printDialog(final PrintRequestAttributeSet attributes) throws HeadlessException {
        ...
        if (service instanceof StreamPrintService) {
            ...
            services = (PrintService[]) java.security.AccessController
                .doPrivileged(new java.security.PrivilegedAction() {
                    public Object run() {
        ...
        try {
            newService = ServiceUI
                .printDialog(gc, x, y, services, service, DocFlavor.SERVICE_FORMATTED.PAGEABLE, attributes);
        } catch (IllegalArgumentException iae) {
            newService = ServiceUI
                .printDialog(gc, x, y, services, services[0], DocFlavor.SERVICE_FORMATTED.PAGEABLE, attributes);
        }
    ...
    protected boolean isSupportedValue(Attribute attrval, PrintRequestAttributeSet attrset) {
        PrintService ps = getPrintService();
        return (attrval != null && ps != null &&
            ps.isAttributeValueSupported(attrval, DocFlavor.SERVICE_FORMATTED.PAGEABLE, attrset));
    }
    ...
    protected void setAttributes(PrintRequestAttributeSet attributes) throws PrinterException {
        ...
        if (fidelity == true) {
            AttributeSet unsupported = service
                .getUnsupportedAttributes(DocFlavor.SERVICE_FORMATTED.PAGEABLE, attributes);
            if (unsupported != null) {
        ...
        if ((orientReq != null || media != null || mpa != null) && getPageable() instanceof OpenBook) {
             ...
             */
            if (mpa == null && media != null && service.
                isAttributeCategorySupported(MediaPrintableArea.class)) {
                Object mpaVals = service.
                    getSupportedAttributeValues(MediaPrintableArea.class, null, attributes);
    ...
    public void print(PrintRequestAttributeSet attributes) throws PrinterException {
        ...
        if ((PrinterIsAcceptingJobs) (psvc.getAttribute(PrinterIsAcceptingJobs.class)) ==
        ...
        try {
            ...
            debug_println(
                "after startDoc rangeSelected? " + rangeIsSelected + " numNonCollatedCopies " + numNonCollatedCopies);
            ...
            for (int collated = 0; collated < numCollatedCopies; collated++) {
                for (int i = firstPage, pageResult = Printable.PAGE_EXISTS;
                    (i <= lastPage || lastPage == Pageable.UNKNOWN_NUMBER_OF_PAGES) &&
                        pageResult == Printable.PAGE_EXISTS; i++) {
                    ...
                    for (int nonCollated = 0;
                        nonCollated < numNonCollatedCopies && pageResult == Printable.PAGE_EXISTS; nonCollated++) {
                        if (isCancelled()) {
                            cancelDoc();
                        }
    ...
    public PageFormat defaultPage(PageFormat page) {
        ...
        if (!Locale.getDefault().equals(Locale.ENGLISH) && // ie "C"
            defaultCountry != null && !defaultCountry.equals(Locale.US.getCountry()) &&
    ...
    synchronized void setGraphicsConfigInfo(AffineTransform at, double pw, double ph) {
        ...
        if (pgConfig == null || defaultDeviceTransform == null || !at.equals(defaultDeviceTransform) ||
            deviceWidth != (int) pt.getX() || deviceHeight != (int) pt.getY()) {
    ...
    protected int printPage(Pageable document, int pageIndex) throws PrinterException {
         ...
         * resolution.
         */
        Rectangle2D deviceArea = new Rectangle2D.Double(paper.getImageableX() * xScale, paper.getImageableY() * yScale,
            paper.getImageableWidth() * xScale, paper.getImageableHeight() * yScale);
        ...
        Rectangle2D.Double pageFormatArea = new Rectangle2D.Double(page.getImageableX(), page.getImageableY(),
        ...
        if (pageResult == Printable.PAGE_EXISTS) {
            ...
            boolean paperChanged = previousPaper == null || thisPaper.getWidth() != previousPaper.getWidth() ||
            ...
            if (pathGraphics != null) {
                ...
                for (int i = 0; i < redrawList.size(); i++) {
                    ...
                    ((PathGraphics) pathGraphics)
                        .redrawRegion(gstate.region, gstate.sx, gstate.sy, gstate.theClip, gstate.theTransform);
                }
     ...
     */
    protected Graphics2D createPathGraphics(PeekGraphics graphics, PrinterJob printerJob, Printable painter,
