...
public class PSPrinterJob extends RasterPrinterJob {
    ...
    private static final byte hexDigits[] = { (byte) '0', (byte) '1', (byte) '2', (byte) '3', (byte) '4', (byte) '5',
    ...
    private static final String READIMAGEPROC = "/imStr 0 def /imageSrc " +
        "{currentfile /ASCII85Decode filter /RunLengthDecode filter " + " imStr readstring pop } def";
    ...
    private static final String COORD_PREP = " 0 exch translate " + "1 -1 scale" + "[72 " + PS_XRES + " div " + "0 0 " +
        "72 " + PS_YRES + " div " + "0 0]concat";
    ...
    private class PrinterSpooler implements java.security.PrivilegedAction {
        ...
        private void handleProcessFailure(final Process failedProcess, final String[] execCmd, final int result)
            throws IOException {
            try (StringWriter sw = new StringWriter(); PrintWriter pw = new PrintWriter(sw)) {
                ...
                try (InputStream is = failedProcess.getErrorStream(); InputStreamReader isr = new InputStreamReader(
                    is); BufferedReader br = new BufferedReader(isr)) {
                    while (br.ready()) {
                        pw.println();
     ...
     */
    protected void drawImageBGR(byte[] bgrData, float destX, float destY, float destWidth, float destHeight, float srcX,
        float srcY, float srcWidth, float srcHeight, int srcBitMapWidth, int srcBitMapHeight) {
         ...
         */
        mPSStream.println(
            intSrcWidth + " " + intSrcHeight + " " + 8 + "[" + intSrcWidth + " 0 " + "0 " + intSrcHeight + " 0 " + 0 +
    ...
    protected void printBand(byte[] bgrData, int x, int y, int width, int height) throws PrinterException {
        ...
        /* Color Image invocation.
         */
        mPSStream.println(width + " " + height + " " + 8 + "[" + width + " 0 " + "0 " + -height + " 0 " + height + "]" +
            "/imageSrc load false 3 colorimage");
    ...
    protected Graphics2D createPathGraphics(PeekGraphics peekGraphics, PrinterJob printerJob, Printable painter,
    ...
    protected boolean textOut(Graphics g, String str, float x, float y, Font mLastFont, FontRenderContext frc,
        float width) {
            ...
            if (psFonts != null) {
                ...
                for (int i = 0; i < acs.length; i++) {
                    ...
                    if (acs.length == 1 && width != 0f) {
                        ...
                        Rectangle2D r2d = mLastFont
                            .getStringBounds(cs.charsetChars, cs.offset, cs.offset + cs.length, frc);
                    ...
                    // out String
                    mPSStream
                        .println(nativeStr.toString() + " " + desiredWidth + " " + x + " " + y + " " + DrawStringName);
                    x += desiredWidth;
                }
            } else {
                didText = false;
     ...
     * point.
     */
    protected void bezierTo(float control1x, float control1y, float control2x, float control2y, float endX,
        float endY) {
        ...
        //                        + CURVETO_STR);
        mPSStream.println(
            trunc(control1x) + " " + trunc(control1y) + " " + trunc(control2x) + " " + trunc(control2y) + " " +
                trunc(endX) + " " + trunc(endY) + CURVETO_STR);
    ...
    private String[] printExecCmd(String printer, String options, boolean noJobSheet, String banner, int copies,
        String spoolFile) {
        int PRINTER = 0x1;
        int OPTIONS = 0x2;
        int BANNER = 0x4;
    ...
    private void prepDrawing() {
        ...
        while (isOuterGState() == false &&
            (getGState().canSetClip(mLastClip) == false || getGState().mTransform.equals(mLastTransform) == false)) {
    ...
    private class GState {
        ...
        void emitTransform(AffineTransform transform) {
            ...
            if (transform != null && transform.equals(mTransform) == false) {
                ...
                mPSStream.println("[" + (float) matrix[0] + " " + (float) matrix[1] + " " + (float) matrix[2] + " " +
                    (float) matrix[3] + " " + (float) matrix[4] + " " + (float) matrix[5] + "] concat");
    ...
    private byte[] ascii85Encode(byte[] inArr) {
        ...
        while (i + 3 < inArr.length) {
            val = ((long) ((inArr[i++] & 0xff)) << 24) + ((long) ((inArr[i++] & 0xff)) << 16) +
                ((long) ((inArr[i++] & 0xff)) << 8) + ((long) (inArr[i++] & 0xff));
            if (val == 0) {
                outArr[olen++] = 'z';
            } else {
                rem = val;
