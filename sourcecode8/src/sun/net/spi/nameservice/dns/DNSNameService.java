...
public final class DNSNameService implements NameService {
    ...
    private DirContext getTemporaryContext() throws NamingException {
        ...
        //
        if (nameProviderUrl == null) { nsList = ResolverConfiguration.open().nameservers(); }
        ...
        if (thrCtxt == null) {
            ...
            DirContext dirCtxt;
            try {
                dirCtxt = java.security.AccessController
                    .doPrivileged(new java.security.PrivilegedExceptionAction<DirContext>() {
     ...
     */
    private ArrayList<String> resolve(final DirContext ctx, final String name, final String[] ids, int depth)
        ...
        // do the query
        try {
            attrs = java.security.AccessController
                .doPrivileged(new java.security.PrivilegedExceptionAction<Attributes>() {
    ...
    public DNSNameService() throws Exception {
        ...
        // name servers
        String nameservers = AccessController
            .doPrivileged(new GetPropertyAction("sun.net.spi.nameservice.nameservers"));
        if (nameservers != null && nameservers.length() > 0) {
            ...
            for (String addr : nsList) {
                if (IPAddressUtil.isIPv4LiteralAddress(addr) || IPAddressUtil.isIPv6LiteralAddress(addr)) {
                    found = true;
                    break;
                }
            }
