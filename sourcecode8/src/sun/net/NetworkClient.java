...
public class NetworkClient {
    ...
    public void openServer(String server, int port) throws IOException, UnknownHostException {
        if (serverSocket != null) { closeServer(); }
    ...
    protected Socket doConnect(String server, int port) throws IOException, UnknownHostException {
        ...
        if (readTimeout >= 0) { s.setSoTimeout(readTimeout); } else if (defaultSoTimeout > 0) {
    ...
    protected InetAddress getLocalAddress() throws IOException {
        if (serverSocket == null) { throw new IOException("not connected"); }
    ...
    public void setReadTimeout(int timeout) {
        if (timeout == DEFAULT_READ_TIMEOUT) { timeout = defaultSoTimeout; }
