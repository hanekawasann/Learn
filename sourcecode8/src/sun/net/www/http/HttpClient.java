...
public class HttpClient extends NetworkClient {
    ...
    static private int getDefaultPort(String proto) {
        if ("http".equalsIgnoreCase(proto)) { return 80; }
        if ("https".equalsIgnoreCase(proto)) { return 443; }
        return -1;
    }
    ...
    static {
        String keepAlive = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("http.keepAlive"));

        String retryPost = java.security.AccessController
            .doPrivileged(new sun.security.action.GetPropertyAction("sun.net.http.retryPost"));
    ...
    static protected Proxy newHttpProxy(String proxyHost, int proxyPort, String proto) {
        if (proxyHost == null || proto == null) { return Proxy.NO_PROXY; }
    ...
    public static HttpClient New(URL url, Proxy p, int to, boolean useCache, HttpURLConnection httpuc)
        throws IOException {
        if (p == null) {
            p = Proxy.NO_PROXY;
        }
            ...
            if (ret != null) {
                if ((ret.proxy != null && ret.proxy.equals(p)) || (ret.proxy == null && p == null)) {
                    synchronized (ret) {
                        ...
                        assert ret.inCache;
                        ret.inCache = false;
                        if (httpuc != null && ret.needsTunneling()) { httpuc.setTunnelState(TUNNELING); }
    ...
    public void finished() {
        if (reuse) /* will be reused */ { return; }
    ...
    protected synchronized boolean available() {
        ...
        try {
            try {
                ...
                if (old != -1) { serverSocket.setSoTimeout(old); }
            }
        } catch (IOException e) {
    ...
    public void writeRequests(MessageHeader head, PosterOutputStream pos) throws IOException {
        ...
        requests.print(serverOutput);
        poster = pos;
        if (poster != null) { poster.writeTo(serverOutput); }
        serverOutput.flush();
    }
        ...
        try {
            ...
            if (!failedOnce && requests != null) {
                ...
                if (getRequestMethod().equals("CONNECT") ||
                    (httpuc.getRequestMethod().equals("POST") && (!retryPostProp || streaming))) {
    ...
    private boolean parseHTTPHeader(MessageHeader responses, ProgressSource pi, HttpURLConnection httpuc)
        ...
        try {
            ...
            if (ret) { // is valid HTTP - response started w/ "HTTP/1."
                ...
                if (cookieHandler != null) {
                    ...
                    // it is resolved.
                    if (uri != null) { cookieHandler.put(uri, responses.getHeaders()); }
                }
        ...
        if (te != null && te.equalsIgnoreCase("chunked")) {
            ...
            if (keepAliveConnections > 1 &&
                (cl >= 0 || code == HttpURLConnection.HTTP_NOT_MODIFIED || code == HttpURLConnection.HTTP_NO_CONTENT)) {
                keepingAlive = true;
                failedOnce = false;
    ...
    public int getProxyPortUsed() {
        if (usingProxy) { return ((InetSocketAddress) proxy.address()).getPort(); }
        return -1;
    }
}
