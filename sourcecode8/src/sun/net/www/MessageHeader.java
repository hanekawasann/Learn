...
public class MessageHeader {
    ...
    public synchronized int getKey(String k) {
        for (int i = nkeys; --i >= 0; ) {
            if ((keys[i] == k) || (k != null && k.equalsIgnoreCase(keys[i]))) { return i; }
        }
        return -1;
    }
    ...
    public synchronized String findNextValue(String k, String v) {
        ...
        if (k == null) {
            for (int i = nkeys; --i >= 0; ) {
                if (keys[i] == null) { if (foundV) { return values[i]; } else if (values[i] == v) { foundV = true; } }
            }
        } else {
            for (int i = nkeys; --i >= 0; ) {
                if (k.equalsIgnoreCase(keys[i])) {
                    if (foundV) { return values[i]; } else if (values[i] == v) { foundV = true; }
                }
            }
        }
        return null;
    ...
    public boolean filterNTLMResponses(String k) {
        boolean found = false;
        for (int i = 0; i < nkeys; i++) {
            if (k.equalsIgnoreCase(keys[i]) && values[i] != null && values[i].length() > 5 &&
        ...
        if (found) {
            ...
            for (int i = 0; i < nkeys; i++) {
                if (k.equalsIgnoreCase(keys[i]) &&
                    ("Negotiate".equalsIgnoreCase(values[i]) || "Kerberos".equalsIgnoreCase(values[i]))) {
                    continue;
                }
                if (i != j) {
                    keys[j] = keys[i];
    ...
    public void mergeHeader(InputStream is) throws java.io.IOException {
        ...
        while (firstc != '\n' && firstc != '\r' && firstc >= 0) {
            ...
            String v;
            if (keyend >= len) { v = new String(); } else { v = String.copyValueOf(s, keyend, len - keyend); }
            add(k, v);
        }
    }
