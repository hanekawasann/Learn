...
public abstract class AuthenticationInfo extends AuthCacheValue implements Cloneable {
    ...
    static {
        serializeAuth = java.security.AccessController
            .doPrivileged(new sun.security.action.GetBooleanAction("http.auth.serializeRequests")).booleanValue();
    }
    ...
    public AuthenticationInfo(char type, AuthScheme authScheme, URL url, String realm) {
        ...
        if (urlPath.length() == 0) { this.path = urlPath; } else {
    ...
    static String reducePath(String urlPath) {
        ...
        if (sepIndex != -1) {
            if (sepIndex < targetSuffixIndex) { return urlPath.substring(0, sepIndex + 1); } else { return urlPath; }
        } else { return urlPath; }
    }
