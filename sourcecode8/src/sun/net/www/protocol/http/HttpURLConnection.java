...
public class HttpURLConnection extends java.net.HttpURLConnection {
    ...
    static {
        maxRedirects = java.security.AccessController
            .doPrivileged(new sun.security.action.GetIntegerAction("http.maxRedirects", defaultmaxRedirects))
            .intValue();
        version = java.security.AccessController
        ...
        userAgent = agent;
        validateProxy = java.security.AccessController
            .doPrivileged(new sun.security.action.GetBooleanAction("http.auth.digest.validateProxy")).booleanValue();
        validateServer = java.security.AccessController
            .doPrivileged(new sun.security.action.GetBooleanAction("http.auth.digest.validateServer")).booleanValue();

        enableESBuffer = java.security.AccessController
            .doPrivileged(new sun.security.action.GetBooleanAction("sun.net.http.errorstream.enableBuffering"))
            .booleanValue();
        timeout4ESBuffer = java.security.AccessController
            .doPrivileged(new sun.security.action.GetIntegerAction("sun.net.http.errorstream.timeout", 300)).intValue();
        if (timeout4ESBuffer <= 0) {
            timeout4ESBuffer = 300; // use the default
        }

        bufSize4ES = java.security.AccessController
            .doPrivileged(new sun.security.action.GetIntegerAction("sun.net.http.errorstream.bufferSize", 4096))
            .intValue();
        if (bufSize4ES <= 0) {
            bufSize4ES = 4096; // use the default
        }

        allowRestrictedHeaders = java.security.AccessController
            .doPrivileged(new sun.security.action.GetBooleanAction("sun.net.http.allowRestrictedHeaders"))
            .booleanValue();
    ...
    /* The following two fields are only used with Digest Authentication */ String
        domain;      /* The list of authentication domains */
    DigestAuthentication.Parameters digestparams;

    /* Current credentials in use */ AuthenticationInfo currentProxyCredentials = null;
    ...
    /* Set if the user is manually setting the Authorization or Proxy-Authorization headers */ boolean isUserServerAuth;
    ...
    private static PasswordAuthentication privilegedRequestPasswordAuthentication(final String host,
        final InetAddress addr, final int port, final String protocol, final String prompt, final String scheme,
        ...
        return java.security.AccessController
            .doPrivileged(new java.security.PrivilegedAction<PasswordAuthentication>() {
                public PasswordAuthentication run() {
                    ...
                    PasswordAuthentication pass = Authenticator
    ...
    private void writeRequests() throws IOException {
        ...
        if (!setRequests) {
            ...
            if (!failedOnce) {
                checkURLFile();
                requests.prepend(method + " " + getRequestURI() + " " + httpVersion, null);
            }
            if (!getUseCaches()) {
            ...
            if (reqHost == null || (!reqHost.equalsIgnoreCase(host) && !checkSetHost())) {
    ...
    protected void proxiedConnect(URL url, String proxyHost, int proxyPort, boolean useCache) throws IOException {
        http = HttpClient.New(url, proxyHost, proxyPort, useCache, connectTimeout, this);
        http.setReadTimeout(readTimeout);
    }
        ...
        do {
            ...
            if (c instanceof HttpURLConnection) {
                ...
                if (stat >= 300 && stat <= 307 && stat != 306 && stat != HttpURLConnection.HTTP_NOT_MODIFIED) {
                    ...
                    http.disconnect();
                    if (target == null || !base.getProtocol().equals(target.getProtocol()) ||
                        base.getPort() != target.getPort() || !hostsEqual(base, target) || redirects >= 5) {
            ...
            do {
                ...
                if (respCode == HTTP_PROXY_AUTH) {
                    ...
                    AuthenticationHeader authhdr = new AuthenticationHeader("Proxy-Authenticate", responses,
                        new HttpCallerInfo(url, http.getProxyHostUsed(), http.getProxyPortUsed()), dontUseNegotiate);
                    ...
                    if (!doingNTLMp2ndStage) {
                        ...
                        if (serverAuthentication != null && srvHdr != null &&
                            !serverAuthentication.setHeaders(this, srvHdr.headerParser(), raw)) {
                            disconnectInternal();
                            throw new IOException("Authentication failure");
                        }
                        authObj = null;
                ...
                if (respCode == HTTP_UNAUTHORIZED) {
                    ...
                    srvHdr = new AuthenticationHeader("WWW-Authenticate", responses, new HttpCallerInfo(url),
                        dontUseNegotiate);
    ...
    private IOException getChainedException(final IOException rememberedException) {
        try {
            ...
            IOException chainedException = java.security.AccessController
                .doPrivileged(new java.security.PrivilegedExceptionAction<IOException>() {
                    public IOException run() throws Exception {
                        return (IOException) rememberedException.getClass()
                            .getConstructor(new Class<?>[] { String.class }).newInstance(args);
                    }
                });
    ...
    private AuthenticationInfo getHttpProxyAuthentication(AuthenticationHeader authhdr) {
            ...
            if (ret == null) {
                switch (authScheme) {
                    case BASIC:
                        ...
                        PasswordAuthentication a = privilegedRequestPasswordAuthentication(host, addr, port, "http",
                        ...
                        break;
                    case DIGEST:
                        a = privilegedRequestPasswordAuthentication(host, null, port, url.getProtocol(), realm, scheme,
                    ...
                    case NTLM:
                        if (NTLMAuthenticationProxy.supported) {
                            ...
                            if (tryTransparentNTLMProxy) {
                                ...
                                a = privilegedRequestPasswordAuthentication(host, null, port, url.getProtocol(), "",
        ...
        if (logger.isLoggable(PlatformLogger.Level.FINER)) {
            logger.finer("Proxy Authentication for " + authhdr.toString() + " returned " +
    ...
    private AuthenticationInfo getServerAuthentication(AuthenticationHeader authhdr) {
            ...
            if (ret == null) {
                switch (authScheme) {
                    ...
                    case NEGOTIATE:
                        ...
                        break;
                    case BASIC:
                        PasswordAuthentication a = privilegedRequestPasswordAuthentication(url.getHost(), addr, port,
                            url.getProtocol(), realm, scheme, url, RequestorType.SERVER);
                        if (a != null) {
                            ret = new BasicAuthentication(false, url, realm, a);
                        }
                        break;
                    case DIGEST:
                        a = privilegedRequestPasswordAuthentication(url.getHost(), addr, port, url.getProtocol(), realm,
                    ...
                    case NTLM:
                        if (NTLMAuthenticationProxy.supported) {
                            ...
                            if (tryTransparentNTLMServer) {
                                ...
                                a = privilegedRequestPasswordAuthentication(url.getHost(), addr, port,
                                    url.getProtocol(), "", scheme, url, RequestorType.SERVER);
                            }
        ...
        if (logger.isLoggable(PlatformLogger.Level.FINER)) {
            logger.finer("Server Authentication for " + authhdr.toString() + " returned " +
                (ret != null ? ret.toString() : "null"));
        }
        return ret;
    }
    ...
    /* dummy byte buffer for reading off socket prior to closing */ byte[] cdata = new byte[128];
    ...
    private String filterHeaderField(String name, String value) {
        ...
        if (SET_COOKIE.equalsIgnoreCase(name) || SET_COOKIE2.equalsIgnoreCase(name)) {
            ...
            if (cookieHandler == null || value.length() == 0) { return value; }
    ...
    private Map<String, List<String>> getFilteredHeaderFields() {
        if (filteredHeaders != null) { return filteredHeaders; }

        Map<String, List<String>> headers, tmpMap = new HashMap<>();

        if (cachedHeaders != null) { headers = cachedHeaders.getHeaders(); } else { headers = responses.getHeaders(); }
        ...
        for (Map.Entry<String, List<String>> e : headers.entrySet()) {
            ...
            for (String value : values) {
                ...
                if (fVal != null) { filteredVals.add(fVal); }
            }
            if (!filteredVals.isEmpty()) { tmpMap.put(key, Collections.unmodifiableList(filteredVals)); }
        }
