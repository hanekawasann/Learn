...
public class SmtpClient extends TransferProtocolClient {
    ...
    private void toCanonical(String s) throws IOException {
        if (s.startsWith("<")) { issueCommand("rcpt to: " + s + "\r\n", 250); } else {
    ...
    public void to(String s) throws IOException {
        ...
        while (pos < limit) {
            ...
            if (parendepth > 0) {
                ...
                if (parendepth == 0) { if (lastnonsp > st) { ignore = true; } else { st = pos + 1; } }
            } else if (c == '(') { parendepth++; } else if (c == '<') { st = lastnonsp = pos + 1; } else if (c == '>') {
                ignore = true;
            } else if (c == ',') {
                ...
                if (c > ' ' && !ignore) { lastnonsp = pos + 1; } else if (st == pos) { st++; }
            }
            pos++;
        }
        if (lastnonsp > st) { toCanonical(s.substring(st, lastnonsp)); }
    ...
    public void from(String s) throws IOException {
        if (s.startsWith("<")) { issueCommand("mail from: " + s + "\r\n", 250); } else {
    ...
    public SmtpClient(String host) throws IOException {
        ...
        try {
            String s;
            mailhost = java.security.AccessController
                .doPrivileged(new sun.security.action.GetPropertyAction("mail.host"));
            if (mailhost != null) {
                openServer(mailhost);
                return;
            }
