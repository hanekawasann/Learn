...
public class ConstantPoolParser {
    ...
    public int getEndOffset() {
        if (endOffset == 0) { throw new IllegalStateException("class file has not yet been parsed"); }
        return endOffset;
    }
    ...
    private static char[] parseHeader(byte[] classFile) throws InvalidConstantPoolFormatException {
        ...
        for (int i = 0; i < result.length; i++) { result[i] = (char) getUnsignedShort(buffer); }
        int magic = result[0] << 16 | result[1] << 0;
        if (magic != 0xCAFEBABE) { throw new InvalidConstantPoolFormatException("invalid magic number " + magic); }
        // skip major, minor version
        int len = result[4];
        if (len < 1) { throw new InvalidConstantPoolFormatException("constant pool length < 1"); }
        return result;
    }
    ...
    private char[] getCharArray(int utfLength) {
        if (utfLength <= charArray.length) { return charArray; }
        return charArray = new char[utfLength];
    }

    private void parseConstantPool(ByteBuffer buffer, Object[] values, ConstantPoolVisitor visitor)
        throws InvalidConstantPoolFormatException {
    ...
    private void resolveConstantPool(Object[] values, ConstantPoolVisitor visitor) {
        ...
        for (int beg = 1, end = values.length - 1, beg2, end2; beg <= end; beg = beg2, end = end2) {
            beg2 = end;
            end2 = beg - 1;
            ...
            for (int i = beg; i <= end; i++) {
                ...
                switch (tag) {
                    ...
                    case CONSTANT_InterfaceMethodref: {
                        ...
                        visitor.visitMemberRef(i, tag, (String) className, nameAndTypeArray[0], nameAndTypeArray[1],
                            array[0], array[1]);
                        values[i] = null;
                    }
                    break;
    ...
    private static String getUTF8(ByteBuffer buffer, int utfLen, char[] charArray)
        throws InvalidConstantPoolFormatException {
    ...
    private static String getUTF8Extended(ByteBuffer buffer, int utfLimit, char[] charArray, int index)
        throws InvalidConstantPoolFormatException {
        int c, c2, c3;
        while (buffer.position() < utfLimit) {
            c = buffer.get() & 0xff;
            switch (c >> 4) {
                ...
                case 14:
                    ...
                    charArray[index++] = (char) (((c & 0x0F) << 12) | ((c2 & 0x3F) << 6) | ((c3 & 0x3F) << 0));
                    break;
                default:
                    /* 10xx xxxx,  1111 xxxx */
