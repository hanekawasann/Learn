...
public class ConstantPoolPatch {
    ...
    public Object getPatch(int index) {
        ...
        switch (getTag(index)) {
            ...
            case CONSTANT_InterfaceMethodref:
                if (value instanceof String) { value = stripSemis(2, (String) value); }
                break;
            case CONSTANT_NameAndType:
                if (value instanceof String) { value = stripSemis(1, (String) value); }
                break;
        }
        return value;
    }
     ...
     */
    public void putPatches(final Map<String, String> utf8Map, final Map<String, Object> classMap,
        final Map<Object, Object> valueMap, boolean deleteUsedEntries) throws InvalidConstantPoolFormatException {
    ...
    Object[] getOriginalCP(final int startIndex, final int endIndex, final int tagMask)
        ...
        outer.parse(new ConstantPoolVisitor() {
            ...
            @Override
            public void visitMemberRef(int index, byte tag, String className, String memberName, String signature,
                int j, int k) {
    ...
    private void checkConstantTag(byte tag, Object value) {
        if (value == null) { throw new IllegalArgumentException("invalid null constant value"); }
        if (classForTag(tag) != value.getClass()) {
            throw new IllegalArgumentException(
                "invalid constant value" + (tag == CONSTANT_None ? "" : " for tag " + tagName(tag)) + " of class " +
                    value.getClass());
        }
    }
    ...
    private void checkTag(int index, byte putTag) {
        ...
        if (tag != putTag) {
            throw new IllegalArgumentException(
                "invalid put operation" + " for " + tagName(putTag) + " at index " + index + " found " + tagName(tag));
        }
    }
    ...
    private void checkTagMask(int index, int tagBitMask) {
        ...
        if ((tagBit & tagBitMask) == 0) {
            throw new IllegalArgumentException(
                "invalid put operation" + " at index " + index + " found " + tagName(tag));
        }
    }
    ...
    static private final int CONSTANT_MemberRef_MASK = CONSTANT_Fieldref | CONSTANT_Methodref |
        CONSTANT_InterfaceMethodref;

    private static final Map<Class<?>, Byte> CONSTANT_VALUE_CLASS_TAG = new IdentityHashMap<Class<?>, Byte>();
    private static final Class<?>[] CONSTANT_VALUE_CLASS = new Class<?>[16];

    static {
        Object[][] values = { { Integer.class, CONSTANT_Integer }, { Long.class, CONSTANT_Long },
            { Float.class, CONSTANT_Float }, { Double.class, CONSTANT_Double }, { String.class, CONSTANT_String },
    ...
    static Class<?> classForTag(byte tag) {
        if ((tag & 0xFF) >= CONSTANT_VALUE_CLASS.length) { return null; }
    ...
    static String addSemis(String name, String... names) {
        ...
        assert (names.length == 1 || stripSemis(names.length, res)[2].equals(names[1]));
        return res;
    }
