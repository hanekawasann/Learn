...
public class VerifyAccess {
    ...
    public static boolean isMemberAccessible(Class<?> refc,  // symbolic ref class
        ...
        assert ((allowedModes & PUBLIC) != 0 && (allowedModes & ~(ALL_ACCESS_MODES | PACKAGE_ALLOWED)) == 0);
        // The symbolic reference class (refc) must always be fully verified.
        if (!isClassAccessible(refc, lookupClass, allowedModes)) {
            return false;
        }
        // Usually refc and defc are the same, but verify defc also in case they differ.
        if (defc == lookupClass && (allowedModes & PRIVATE) != 0) {
        ...
        switch (mods & ALL_ACCESS_MODES) {
            ...
            case PROTECTED:
                if ((allowedModes & PROTECTED_OR_PACKAGE_ALLOWED) != 0 && isSamePackage(defc, lookupClass)) {
                    return true;
                }
                if ((allowedModes & PROTECTED) == 0) { return false; }
                if ((mods & STATIC) != 0 && !isRelatedClass(refc, lookupClass)) { return false; }
                if ((allowedModes & PROTECTED) != 0 && isSuperClass(defc, lookupClass)) { return true; }
                return false;
            case PACKAGE_ONLY:  // That is, zero.  Unmarked member is package-only access.
                return ((allowedModes & PACKAGE_ALLOWED) != 0 && isSamePackage(defc, lookupClass));
            case PRIVATE:
                // Loosened rules for privates follows access rules for inner classes.
                return (ALLOW_NESTMATE_ACCESS && (allowedModes & PRIVATE) != 0 &&
    ...
    static int getClassModifiers(Class<?> c) {
        ...
        if (c.isArray() || c.isPrimitive()) { return c.getModifiers(); }
    ...
    public static boolean isClassAccessible(Class<?> refc, Class<?> lookupClass, int allowedModes) {
        ...
        if ((allowedModes & PACKAGE_ALLOWED) != 0 && isSamePackage(lookupClass, refc)) { return true; }
        return false;
    }
    ...
    public static boolean isTypeVisible(Class<?> type, Class<?> refc) {
        ...
        if (parent == child || loadersAreRelated(parent, child, true)) { return true; }
    ...
    public static boolean isTypeVisible(java.lang.invoke.MethodType type, Class<?> refc) {
        for (int n = -1, max = type.parameterCount(); n < max; n++) {
            ...
            if (!isTypeVisible(ptype, refc)) { return false; }
        }
        return true;
    }
    ...
    public static boolean isSamePackage(Class<?> class1, Class<?> class2) {
        ...
        if (class1.getClassLoader() != class2.getClassLoader()) { return false; }
