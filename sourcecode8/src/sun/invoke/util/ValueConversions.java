...
public class ValueConversions {
    ...
    private static EnumMap<Wrapper, MethodHandle>[] newWrapperCaches(int n) {
        @SuppressWarnings("unchecked")  // generic array creation
            EnumMap<Wrapper, MethodHandle>[] caches = (EnumMap<Wrapper, MethodHandle>[]) new EnumMap<?, ?>[n];
    ...
    private static MethodHandle unbox(Wrapper wrap, boolean cast) {
        ...
        throw new IllegalArgumentException("cannot find unbox adapter for " + wrap + (cast ? " (cast)" : ""));
    }
    ...
    public static int widenSubword(Object x) {
        if (x instanceof Integer) { return (int) x; } else if (x instanceof Boolean) {
            return fromBoolean((boolean) x);
        } else if (x instanceof Character) {
            return (char) x;
        } else if (x instanceof Short) {
            return (short) x;
        } else if (x instanceof Byte) { return (byte) x; } else
    ...
    private static MethodHandle box(Wrapper wrap, boolean exact) {
        ...
        throw new IllegalArgumentException("cannot find box adapter for " + wrap + (exact ? " (exact)" : ""));
    }
    ...
    private static final MethodHandle IDENTITY, CAST_REFERENCE, ZERO_OBJECT, IGNORE, EMPTY, ARRAY_IDENTITY,
        FILL_NEW_TYPED_ARRAY, FILL_NEW_ARRAY;
    ...
    static {
        try {
            ...
            ARRAY_IDENTITY = IMPL_LOOKUP
                .findStatic(THIS_CLASS, "identity", MethodType.methodType(Object[].class, Object[].class));
            FILL_NEW_ARRAY = IMPL_LOOKUP.findStatic(THIS_CLASS, "fillNewArray",
                MethodType.methodType(Object[].class, Integer.class, Object[].class));
            FILL_NEW_TYPED_ARRAY = IMPL_LOOKUP.findStatic(THIS_CLASS, "fillNewTypedArray",
    ...
    private static final EnumMap<Wrapper, MethodHandle>[] CONVERT_PRIMITIVE_FUNCTIONS = newWrapperCaches(
        Wrapper.values().length);
    ...
    public static MethodHandle convertPrimitive(Wrapper wsrc, Wrapper wdst) {
        ...
        throw new IllegalArgumentException(
            "cannot find primitive conversion function for " + src.getSimpleName() + " -> " + dst.getSimpleName());
    }
    ...
    private static MethodHandle buildVarargsArray(MethodHandle newArray, MethodHandle finisher, int nargs) {
        ...
        if (rightLen > 0) {
            ...
            if (mh == ARRAY_IDENTITY) { mh = rightFiller; } else {
                mh = MethodHandles.collectArguments(mh, 0, rightFiller);
            }
        }
        if (mh == ARRAY_IDENTITY) { mh = leftCollector; } else {
    ...
    private static MethodHandle buildFiller(int nargs) {
        ...
        if (midLen == LEFT_ARGS) { return rightFill; } else {
