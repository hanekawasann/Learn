...
class ExecutableInputMethodManager extends InputMethodManager implements Runnable {
    ...
    ExecutableInputMethodManager() {
        ...
        try {
            if (toolkit instanceof InputMethodSupport) {
                InputMethodDescriptor hostAdapterDescriptor = ((InputMethodSupport) toolkit)
    ...
    public synchronized void notifyChangeRequest(Component comp) {
        if (!(comp instanceof Frame || comp instanceof Dialog)) { return; }

        // if busy with the current request, ignore this request.
        if (requestComponent != null) { return; }
        ...
        synchronized (javaInputMethodLocatorList) {
            ...
            try {
                AccessController.doPrivileged(new PrivilegedExceptionAction<Object>() {
                    public Object run() {
                        for (InputMethodDescriptor descriptor : ServiceLoader
                            .loadInstalled(InputMethodDescriptor.class)) {
