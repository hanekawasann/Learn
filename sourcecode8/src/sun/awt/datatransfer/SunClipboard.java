...
public abstract class SunClipboard extends Clipboard implements PropertyChangeListener {
    ...
    public synchronized FlavorListener[] getFlavorListeners() {
        EventListenerAggregate contextFlavorListeners = (EventListenerAggregate) AppContext.getAppContext()
            .get(CLIPBOARD_FLAVOR_LISTENER_KEY);
    ...
    public void checkChange(long[] formats) {
        ...
        if ((prevDataFlavors != null) && (currentDataFlavors != null) && prevDataFlavors.equals(currentDataFlavors)) {
        ...
        for (Iterator it = AppContext.getAppContexts().iterator(); it.hasNext(); ) {
            ...
            EventListenerAggregate flavorListeners = (EventListenerAggregate) appContext
                .get(CLIPBOARD_FLAVOR_LISTENER_KEY);
            if (flavorListeners != null) {
                ...
                for (int i = 0; i < flavorListenerArray.length; i++) {
                    SunToolkit.postEvent(appContext,
                        new PeerEvent(this, new SunFlavorChangeNotifier(flavorListenerArray[i]),
