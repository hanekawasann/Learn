...
public class ByteBandedRaster extends SunWritableRaster {
     ...
     */
    public ByteBandedRaster(SampleModel sampleModel, DataBuffer dataBuffer, Rectangle aRegion, Point origin,
        ...
        if (sampleModel instanceof BandedSampleModel) {
            ...
            for (int i = 0; i < bankIndices.length; i++) {
                ...
                dataOffsets[i] = dOffsets[bankIndices[i]] + xOffset + yOffset * scanlineStride + bandOffsets[i];
            }
        } else {
    ...
    public WritableRaster createWritableChild(int x, int y, int width, int height, int x0, int y0, int bandList[]) {
        ...
        return new ByteBandedRaster(sm, dataBuffer, new Rectangle(x0, y0, width, height),
        ...
        if (data.length == 1) {
            if (data[0].length <= maxIndex * numDataElements) {
                throw new RasterFormatException(
                    "Data array too small " + "(it is " + data[0].length + " and should be > " +
            ...
            for (int i = 0; i < numDataElements; i++) {
                if (data[i].length <= maxIndex) {
                    throw new RasterFormatException(
                        "Data array too small " + "(it is " + data[i].length + " and should be > " + maxIndex + " )");
                }
            }
        }
    }
    ...
    public String toString() {
        return new String(
            "ByteBandedRaster: width = " + width + " height = " + height + " #bands " + numDataElements + " minX = " +
                minX + " minY = " + minY);
    }
