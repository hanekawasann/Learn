...
public class GifImageDecoder extends ImageDecoder {
     ...
     */
    private static final int normalflags = ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES |
        ImageConsumer.SINGLEPASS | ImageConsumer.SINGLEFRAME;

    /**
     * The ImageConsumer hints flag for an interlaced GIF image.
     */
    private static final int interlaceflags = ImageConsumer.RANDOMPIXELORDER | ImageConsumer.COMPLETESCANLINES |
        ImageConsumer.SINGLEPASS | ImageConsumer.SINGLEFRAME;
    ...
    private native boolean parseImage(int x, int y, int width, int height, boolean interlace, int initCodeSize,
    ...
    private boolean readImage(boolean first, int disposal_method, int delay) throws IOException {
        ...
        if (disposal_method == GifFrame.DISPOSAL_SAVE && saved_image == null) {
            ...
            if ((height < global_height) && (model != null)) {
                ...
                if (tpix >= 0) {
                    ...
                    setPixels(0, 0, global_width, y, model, trans_rasline, 0, 0);
                    setPixels(0, y + height, global_width, global_height - height - y, model, trans_rasline, 0, 0);
                }
            }
        }
        ...
        curframe = new GifFrame(this, disposal_method, delay, (curframe == null), model, x, y, width, height);
        ...
        if (verbose) {
            System.out.print(
                "Reading a " + width + " by " + height + " " + (interlace ? "" : "non-") + "interlaced image...");
        }
        int initCodeSize = ExtractByte(block, 9);
        if (initCodeSize >= 12) {
            if (verbose) {
                System.out.println("Invalid initial code size: " + initCodeSize);
            }
            return false;
        }
        ...
        if (decoder.imageComplete(ImageConsumer.SINGLEFRAMEDONE, false) == 0) {
            ...
            switch (disposal_method) {
                case DISPOSAL_PREVIOUS:
                    ...
                    if (saved_image != null) {
                        setPixels(x, y, width, height, saved_model, saved_image, y * global_width + x, global_width);
                    }
                    break;
                case DISPOSAL_BGCOLOR:
                    ...
                    if (decoder.saved_image != null) {
                        for (int i = 0; i < global_width * global_height; i++) { decoder.saved_image[i] = tpix; }
                    }
