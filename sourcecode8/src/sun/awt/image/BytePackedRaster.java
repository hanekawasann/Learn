...
public class BytePackedRaster extends SunWritableRaster {
     ...
     */
    public BytePackedRaster(SampleModel sampleModel, DataBuffer dataBuffer, Rectangle aRegion, Point origin,
        ...
        if (sampleModel instanceof MultiPixelPackedSampleModel) {
            ...
            throw new RasterFormatException("BytePackedRasters must have" + "MultiPixelPackedSampleModel");
        }
        verify(false);
    }
    ...
    public void setDataElements(int x, int y, Raster inRaster) {
        ...
        if (!(inRaster instanceof BytePackedRaster) || ((BytePackedRaster) inRaster).pixelBitStride != pixelBitStride) {
        ...
        if ((dstOffX < this.minX) || (dstOffY < this.minY) || (dstOffX + width > this.maxX) ||
     ...
     */
    private void setDataElements(int dstX, int dstY, int srcX, int srcY, int width, int height,
        ...
        int inbit = inRaster.dataBitOffset + 8 * (srcY - inRaster.minY) * inscan +
            (srcX - inRaster.minX) * inRaster.pixelBitStride;
        int outbit = (this.dataBitOffset + 8 * (dstY - minY) * outscan + (dstX - minX) * this.pixelBitStride);
    ...
    public void setRect(int dx, int dy, Raster srcRaster) {
        ...
        setDataElements(dstOffX, dstOffY, srcOffX, srcOffY, width, height, (BytePackedRaster) srcRaster);
    }
    ...
    public WritableRaster createWritableChild(int x, int y, int width, int height, int x0, int y0, int[] bandList) {
        ...
        return new BytePackedRaster(sm, dataBuffer, new Rectangle(x0, y0, width, height),
    ...
    private void verify(boolean strictCheck) {
        ...
        if (scanlineStride < 0 || scanlineStride > (Integer.MAX_VALUE / height) || scanlineStride > data.length) {
            throw new RasterFormatException("Invalid scanline stride");
        }

        int lastbit = (
            dataBitOffset + (height - 1) * scanlineStride * 8 + (width - 1) * pixelBitStride + pixelBitStride - 1);
    ...
    public String toString() {
        return new String(
            "BytePackedRaster: width = " + width + " height = " + height + " #channels " + numBands + " xOff = " +
                sampleModelTranslateX + " yOff = " + sampleModelTranslateY);
    }
}
