  ...
  /* this is not needed
        ...
        switch (key) {
            ...
            case cHRMChunk:
                property("chromaticities",
                    new Chromaticities(getInt(st), getInt(st + 4), getInt(st + 8), getInt(st + 12), getInt(st + 16),
                        getInt(st + 20), getInt(st + 24), getInt(st + 28)));
                break;
            case gAMAChunk:
            ...
            case tIMEChunk:
                property("modtime",
                    new GregorianCalendar(getShort(st + 0), getByte(st + 2) - 1, getByte(st + 3), getByte(st + 4),
                        getByte(st + 5), getByte(st + 6)).getTime());
                break;
            case tRNSChunk:
                switch (colorType) {
                    case PALETTE | COLOR:
                    ...
                    case COLOR | ALPHA: // doesn't deal with 16 bit colors properly
                        ...
                        if (bitDepth == 16) {
                            ...
                            transparentPixel = ((getShort(st + 0) & 0xFF) << 16) | ((getShort(st + 2) & 0xFF) << 8) |
                                ((getShort(st + 4) & 0xFF));
                        }
                        break;
    ...
    /* this is not needed
       ...
       */
        try {
            ...
            setColorModel(cm);
            int flags = (interlaceMethod != 0 ? ImageConsumer.TOPDOWNLEFTRIGHT | ImageConsumer.COMPLETESCANLINES
            ...
            while (++pass <= passLimit) {
                ...
                while (row < height) {
                    ...
                    filterRow(rowByteBuffer, firstRow ? null : prevRowByteBuffer, rowFilter, rowByteWidth,
                        bytesPerPixel);
                    int col = sCol;
                    int spos = 0;
                    int pixel = 0;
                        ...
                        if (wPixels != null) {
                            switch (combinedType) {
                                case COLOR | ALPHA | (8 << 3):
                                    wPixels[col + rowOffset] = ((rowByteBuffer[spos] & 0xFF) << 16) |
                                        ((rowByteBuffer[spos + 1] & 0xFF) << 8) | ((rowByteBuffer[spos + 2] & 0xFF)) |
                                        ((rowByteBuffer[spos + 3] & 0xFF) << 24);
                                    spos += 4;
                                    break;
                                case COLOR | ALPHA | (16 << 3):
                                    wPixels[col + rowOffset] = ((rowByteBuffer[spos] & 0xFF) << 16) |
                                        ((rowByteBuffer[spos + 2] & 0xFF) << 8) | ((rowByteBuffer[spos + 4] & 0xFF)) |
                                        ((rowByteBuffer[spos + 6] & 0xFF) << 24);
                                    spos += 8;
                                    break;
                                case COLOR | (8 << 3):
                                    pixel = ((rowByteBuffer[spos] & 0xFF) << 16) |
                                        ((rowByteBuffer[spos + 1] & 0xFF) << 8) | ((rowByteBuffer[spos + 2] & 0xFF));
                                ...
                                case COLOR | (16 << 3):
                                    ...
                                    for (int i = 0; isTransparent && (i < 6); i++) {
                                        isTransparent &= (rowByteBuffer[spos + i] & 0xFF) ==
                                            (transparentPixel_16[i] & 0xFF);
                                    }
                                    if (!isTransparent) {
                                        pixel |= 0xff000000;
                                    }
                                ...
                                case GRAY | ALPHA | (8 << 3): {
                                    ...
                                    wPixels[col + rowOffset] = (tx << 16) | (tx << 8) | tx |
                            ...
                            switch (bitDepth) {
                                case 1:
                                    bPixels[col + rowOffset] = (byte) ((rowByteBuffer[spos >> 3] >> (7 - (spos & 7))) &
                                        1);
                                    spos++;
                                    break;
                                case 2:
                                    bPixels[col + rowOffset] = (byte) (
                                        (rowByteBuffer[spos >> 2] >> ((3 - (spos & 3)) * 2)) & 3);
                                    spos++;
                                    break;
                                case 4:
                                    bPixels[col + rowOffset] = (byte) (
                                        (rowByteBuffer[spos >> 1] >> ((1 - (spos & 1)) * 4)) & 15);
                                    spos++;
                                    break;
                                case 8:
   ...
   /* Here, the function "visit(row,column,height,width)" obtains the
    ...
    private boolean sendPixels(int x, int y, int w, int h, int[] pixels, int offset, int pixlength) {
        int count = setPixels(x, y, w, h, cm, pixels, offset, pixlength);
        if (count <= 0) {
            aborted = true;
        }
        return !aborted;
    ...
    private void filterRow(byte rowByteBuffer[], byte[] prevRow, int rowFilter, int rowByteWidth, int bytesPerSample)
        ...
        switch (rowFilter) {
            ...
            case 1:
                ...
                break;
            case 2:
                if (prevRow != null) { for (; x < rowByteWidth; x++) { rowByteBuffer[x] += prevRow[x]; } }
                break;
            case 3:
                if (prevRow != null) {
                    for (; x < bytesPerSample; x++) { rowByteBuffer[x] += (0xff & prevRow[x]) >> 1; }
            ...
            case 4:
                if (prevRow != null) {
                    for (; x < bytesPerSample; x++) { rowByteBuffer[x] += prevRow[x]; }
                    for (; x < rowByteWidth; x++) {
    ...
    private static final byte[] signature = { (byte) 137, (byte) 80, (byte) 78, (byte) 71, (byte) 13, (byte) 10,
        (byte) 26, (byte) 10 };
    ...
    private final int getInt(int pos) {
        return ((inbuf[pos] & 0xFF) << 24) | ((inbuf[pos + 1] & 0xFF) << 16) | ((inbuf[pos + 2] & 0xFF) << 8) |
            ((inbuf[pos + 3] & 0xFF));
    }

    private final int getShort(int pos) {
        return (short) (((inbuf[pos] & 0xFF) << 8) | ((inbuf[pos + 1] & 0xFF)));
    }
    ...
    boolean getData() throws IOException {
        while (chunkLength == 0 && getChunk()) {
            if (handleChunk(chunkKey, inbuf, chunkStart, chunkLength)) { chunkLength = 0; }
        }
    ...
    static {
        for (int n = 0; n < 256; n++) {
            int c = n;
            for (int k = 0; k < 8; k++) { if ((c & 1) != 0) { c = 0xedb88320 ^ (c >>> 1); } else { c = c >>> 1; } }
            crc_table[n] = c;
        }
    }
    ...
    static private int update_crc(int crc, byte[] buf, int offset, int len) {
        int c = crc;
        while (--len >= 0) { c = crc_table[(c ^ buf[offset++]) & 0xff] ^ (c >>> 8); }
        return c;
    }
    ...
    public static class Chromaticities {
        ...
        public String toString() {
            return "Chromaticities(white=" + whiteX + "," + whiteY + ";red=" + redX + "," + redY + ";green=" + greenX +
                "," + greenY + ";blue=" + blueX + "," + blueY + ")";
        }
    }
}
