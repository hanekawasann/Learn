...
public class Instruction implements Constants {
    ...
    void collect(ConstantPool tab) {
        switch (opc) {
            ...
            case opc_nop:
                if ((value != null) && (value instanceof ClassDeclaration)) { tab.put(value); }
                return;
        }
    }
    ...
    int size(ConstantPool tab) {
        switch (opc) {
            ...
            case opc_dstore:
            case opc_astore: {
                int v = (value instanceof Number) ? ((Number) value).intValue() : ((LocalVariable) value).slot;
                if (v < 4) {
                    if (v < 0) {
                        throw new CompilerError("invalid slot: " + toString());
                    }
            ...
            case opc_nop:
                if ((value != null) && !(value instanceof Integer)) { return 2; } else { return 1; }
        }
    ...
    void write(DataOutputStream out, ConstantPool tab) throws IOException {
        switch (opc) {
            ...
            case opc_astore:
                out.writeByte(opc);
                out.writeByte((value instanceof Number) ? ((Number) value).intValue() : ((LocalVariable) value).slot);
                break;
            ...
            case opc_astore + 256:
                out.writeByte(opc_wide);
                out.writeByte(opc - 256);
                out.writeShort((value instanceof Number) ? ((Number) value).intValue() : ((LocalVariable) value).slot);
                break;
            ...
            case opc_nop:
                if (value != null) {
                    if (value instanceof Integer) { out.writeByte(((Integer) value).intValue()); } else {
                        out.writeShort(tab.index(value));
                    }
                    return;
                }
