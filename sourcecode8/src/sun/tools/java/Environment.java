...
public class Environment implements Constants {
        ...
        switch (t.getTypeCode()) {
            case TC_CLASS: {
                ...
                try {
                    ...
                    try {
                        if (e.name.isInner() && getPackage(e.name.getTopName()).exists()) {
                            env.error(where, "class.and.package", e.name.getTopName());
                        }
     ...
     */
    public final ClassDefinition getQualifiedClassDefinition(long where, Identifier nm, ClassDefinition ctxClass,
                ...
                if (hname.length() > 0 && Character.isDigit(hname.charAt(0))) {
                    ...
                    for (MemberDefinition f = c.getFirstMatch(head); f != null; f = f.getNextMatch()) {
                        if (f.isInnerClass()) {
                            ...
                            if ((!isExtends && !ctxClass.canAccess(env, fdecl)) ||
                                (isExtends && !ctxClass.extendsCanAccess(env, fdecl))) {
    ...
    public Identifier resolveName(Identifier name) {
        ...
        try {
            ...
            if (imports != null) { return imports.forceResolve(this, name); }
        }
        return name;
    }
    ...
    public final Identifier resolvePackageQualifiedName(Identifier name) {
        ...
        if (tail != null) { name = Identifier.lookupInner(name, tail); }
        return name;
    }
     ...
     * Create a new class.
     */
    public ClassDefinition makeClassDefinition(Environment origEnv, long where, IdentifierToken name, String doc,
        int modifiers, IdentifierToken superClass, IdentifierToken interfaces[], ClassDefinition outerClass) {
        if (env == null) {
            return null; // lame default
        }
        return env.makeClassDefinition(origEnv, where, name, doc, modifiers, superClass, interfaces, outerClass);
    }

    /**
     * Create a new field.
     */
    public MemberDefinition makeMemberDefinition(Environment origEnv, long where, ClassDefinition clazz, String doc,
        int modifiers, Type type, Identifier name, IdentifierToken argNames[], IdentifierToken expIds[], Object value) {
        if (env == null) {
            return null; // lame default
        }
        return env.makeMemberDefinition(origEnv, where, clazz, doc, modifiers, type, name, argNames, expIds, value);
    }
    ...
    public boolean isApplicable(MemberDefinition m, Type args[]) throws ClassNotFound {
        ...
        for (int i = args.length; --i >= 0; ) { if (!isMoreSpecific(args[i], mArgs[i])) { return false; } }
        return true;
    }
    ...
    public boolean isMoreSpecific(MemberDefinition best, MemberDefinition other) throws ClassNotFound {
        ...
        boolean result = isMoreSpecific(bestType, otherType) && isApplicable(other, best.getType().getArgumentTypes());
    ...
    public boolean implicitCast(Type from, Type to) throws ClassNotFound {
        ...
        switch (from.getTypeCode()) {
            case TC_BYTE:
                if (toTypeCode == TC_SHORT) { return true; }
            case TC_SHORT:
            case TC_CHAR:
            ...
            case TC_ARRAY:
                if (!to.isType(TC_ARRAY)) {
                    ...
                    if (from.inMask(TM_ARRAY | TM_CLASS) && to.inMask(TM_ARRAY | TM_CLASS)) {
    ...
    public boolean explicitCast(Type from, Type to) throws ClassNotFound {
        ...
        if (to.isType(TC_ARRAY)) {
            if (from.isType(TC_ARRAY)) {
                ...
                while ((t1.getTypeCode() == TC_ARRAY) && (t2.getTypeCode() == TC_ARRAY)) {
                    t1 = t1.getElementType();
                    t2 = t2.getElementType();
                }
                if (t1.inMask(TM_ARRAY | TM_CLASS) && t2.inMask(TM_ARRAY | TM_CLASS)) {
                    return explicitCast(t1, t2);
                }
            } else if (from == Type.tObject || from == Type.tCloneable || from == Type.tSerializable) { return true; }
        }
        return false;
    }
    ...
    public static void debugOutput(Object msg) {
        if (Environment.debugging) { System.out.println(msg.toString()); }
    }
