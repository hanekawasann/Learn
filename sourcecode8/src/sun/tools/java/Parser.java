...
public class Parser extends Scanner implements ParserActions, Constants {
     ...
     */
    @Deprecated
    public ClassDefinition beginClass(long off, String doc, int mod, IdentifierToken nm, IdentifierToken sup,
     ...
     */
    @Deprecated
    public void defineField(long where, ClassDefinition c, String doc, int mod, Type t, IdentifierToken nm,
        IdentifierToken args[], IdentifierToken exp[], Node val) {
        // By default, call the deprecated version.
     ...
     */
    @Deprecated
    protected void defineField(long where, String doc, int mod, Type t, Identifier nm, Identifier args[],
    ...
    protected Expression parseNewInstanceExpression(long p, Expression outerArg, Expression type)
        ...
        if (token == LBRACE && !(type instanceof TypeExpression)) {
            ...
            body = parseClassBody(new IdentifierToken(tp, idNull), M_ANONYMOUS | M_LOCAL, EXPR, null, ext, impl,
                type.getWhere());
        }
    ...
    protected void parseField() throws SyntaxError, IOException {
        ...
        if ((mod == (mod & M_STATIC)) && (token == LBRACE)) {
            // static initializer
            actions.defineField(p, curClass, doc, mod, Type.tMethod(Type.tVoid), new IdentifierToken(idClassInit), null,
                null, parseStatement());
            return;
        }
     ...
     */
    protected ClassDefinition parseClassBody(IdentifierToken nm, int mod, int ctx, String doc, Vector ext, Vector impl,
        long p) throws SyntaxError, IOException {
