...
public class MemberDefinition implements Constants {
     ...
     * Constructor
     */
    public MemberDefinition(long where, ClassDefinition clazz, int modifiers, Type type, Identifier name,
    ...
    public MemberDefinition(ClassDefinition innerClass) {
        this(innerClass.getWhere(), innerClass.getOuterClass(), innerClass.getModifiers(), innerClass.getType(),
            innerClass.getName().getFlatName().getName(), null, null);
    ...
    public static MemberDefinition makeProxyMember(MemberDefinition field, ClassDefinition classDef, Environment env) {
        ...
        proxy = new MemberDefinition(field.getWhere(), classDef, field.getModifiers(), field.getType(), field.getName(),
            field.getExceptionIds(), null);
    ...
    public ClassDeclaration[] getExceptions(Environment env) {
        if (expIds != null && exp == null) {
            if (expIds.length == 0) { exp = new ClassDeclaration[0]; } else
    ...
    public boolean isUplevelValue() {
        ...
        return name.startsWith(prefixVal) || name.startsWith(prefixLoc) || name.startsWith(prefixThis);
    }
    ...
    private void reportError(Environment env, String errorString, ClassDeclaration clazz, MemberDefinition method) {
        ...
        if (clazz == null) {
            ...
            env.error(clazz.getClassDefinition().getWhere(),
                ...
                errorString,
                //clazz,
                this, getClassDeclaration(), method.getClassDeclaration());
        }
    }
    ...
    public boolean checkMeet(Environment env, MemberDefinition method, ClassDeclaration clazz) {
        ...
        if (!sameReturnType(method)) {
            // More args?
            env.error(clazz.getClassDefinition().getWhere(), "meet.different.return", this, this.getClassDeclaration(),
    ...
    public void cleanup(Environment env) {
        ...
        if (isMethod() && value != null) {
            ...
            } else if ((cost = ((Statement) value).costInline(Statement.MAXINLINECOST, null, null)) >=
