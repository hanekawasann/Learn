...
public final class BinaryClass extends ClassDefinition implements Constants {
     ...
     * Constructor
     */
    public BinaryClass(Object source, ClassDeclaration declaration, int modifiers, ClassDeclaration superClass,
        ClassDeclaration interfaces[], Vector dependencies) {
    ...
    public static BinaryClass load(Environment env, DataInputStream in, int mask) throws IOException {
        ...
        if (version < JAVA_MIN_SUPPORTED_VERSION) {
            throw new ClassFormatError(
                sun.tools.javac.Main.getText("javac.err.version.too.old", String.valueOf(version)));
        } else if ((version > JAVA_MAX_SUPPORTED_VERSION) ||
            (version == JAVA_MAX_SUPPORTED_VERSION && minor_version > JAVA_MAX_SUPPORTED_MINOR_VERSION)) {
            throw new ClassFormatError(
                sun.tools.javac.Main.getText("javac.err.version.too.recent", version + "." + minor_version));
        }
    ...
    private void initInnerClasses(Environment env, byte data[], int flags) throws IOException {
        ...
        for (int i = 0; i < nrec; i++) {
            ...
            boolean accessible = (outer != null) && (!inner_nm.equals(idNull)) &&
                ((mods & M_PRIVATE) == 0 || (flags & ATT_ALLCLASSES) != 0);
    ...
    private void initInner(ClassDefinition outerClass, int mods) {
        ...
        for (MemberDefinition field = getFirstMember(); field != null; field = field.getNextMember()) {
            if (field.isUplevelValue() && outerClass.getType().equals(field.getType()) &&
