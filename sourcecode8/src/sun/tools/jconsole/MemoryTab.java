...
class MemoryTab extends Tab implements ActionListener, ItemListener {
    ...
    */
    ...
    public MemoryTab(VMPanel vmPanel) {
        ...
        controlPanel.add(
            new LabeledComponent(Messages.CHART_COLON, Resources.getMnemonicInt(Messages.CHART_COLON), plotterChoice));

        // Range control
        timeComboBox = new TimeComboBox();
        controlPanel.add(
            new LabeledComponent(Messages.TIME_RANGE_COLON, Resources.getMnemonicInt(Messages.TIME_RANGE_COLON),
                timeComboBox));
    ...
    private void createPlotters() throws IOException {
        ...
        setAccessibleName(heapPlotter, Messages.MEMORY_TAB_HEAP_PLOTTER_ACCESSIBLE_NAME);
        setAccessibleName(nonHeapPlotter, Messages.MEMORY_TAB_NON_HEAP_PLOTTER_ACCESSIBLE_NAME);
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    try {
                        if (plotter instanceof PoolPlotter) {
                            ...
                            AttributeList al = proxyClient
                                .getAttributes(objectName, new String[] { "Usage", "UsageThreshold" });
                            if (al.size() > 0) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (used[i] >= 0L) {
                        if (plotter instanceof PoolPlotter) {
                            ...
                            poolChart.setValue(poolCount++, (PoolPlotter) plotter, used[i], threshold[i], max[i]);
                        } else {
    ...
    private String formatDetails() {
        ...
        try {
            ...
            for (GarbageCollectorMXBean garbageCollectorMBean : garbageCollectors) {
                ...
                String str = Resources.format(Messages.GC_TIME_DETAILS, justify(formatTime(gcTime), 14), gcName,
    ...
    private class PoolPlotter extends Plotter {
        ...
        public PoolPlotter(ObjectName objectName, String name, boolean isHeap) {
            ...
            setAccessibleName(this, Resources.format(Messages.MEMORY_TAB_POOL_PLOTTER_ACCESSIBLE_NAME, name));
        }
    ...
    private class PoolChart extends BorderedComponent implements Accessible, MouseListener {
        ...
        public void paintComponent(Graphics g) {
            ...
            for (int pc : new int[] { 0, 25, 50, 75, 100 }) {
                String str = pc + "% --";
                g.drawString(str, leftMargin - fm.stringWidth(str) - 4,
        ...
        private Rectangle getBarRect(PoolPlotter poolPlotter) {
            return new Rectangle(leftMargin + poolPlotter.barX, height - bottomMargin - barHeight, barWidth, barHeight);
        }

        public Dimension getPreferredSize() {
            return new Dimension(nonHeapRect.x + nonHeapRect.width + rightMargin, height);
        }
        ...
        protected class AccessiblePoolChart extends AccessibleJPanel {
            public String getAccessibleName() {
                ...
                for (PoolPlotter poolPlotter : poolPlotters) {
                    ...
                    keyValueList += Resources
                        .format(Messages.PLOTTER_ACCESSIBLE_NAME_KEY_AND_VALUE, poolPlotter.toString(), value);
                    if (poolPlotter.threshold > 0L) {
                        String threshold = (poolPlotter.threshold * 100 / poolPlotter.max) + "%";
                        if (poolPlotter.value > poolPlotter.threshold) {
                            keyValueList += Resources.format(Messages.MEMORY_TAB_POOL_CHART_ABOVE_THRESHOLD, threshold);
                        } else {
                            keyValueList += Resources.format(Messages.MEMORY_TAB_POOL_CHART_BELOW_THRESHOLD, threshold);
                        }
                    }
                }
    ...
    private static class MemoryOverviewPanel extends OverviewPanel {
        ...
        private void updateMemoryInfo(long used, long committed, long max) {
            getInfoLabel().setText(Resources
                .format(Messages.MEMORY_TAB_INFO_LABEL_FORMAT, formatBytes(used, true), formatBytes(committed, true),
