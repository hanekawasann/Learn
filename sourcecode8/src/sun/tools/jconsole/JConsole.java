...
public class JConsole extends JFrame implements ActionListener, InternalFrameListener {
    ...
    private final static String title = Messages.JAVA_MONITORING___MANAGEMENT_CONSOLE;
    public final static String ROOT_URL = "service:jmx:";
    ...
    public JConsole(boolean hotspot) {
        ...
        setAccessibleDescription(this, Messages.JCONSOLE_ACCESSIBLE_DESCRIPTION);
        ...
        if (hotspot) {
            ...
            hotspotMI.setAccelerator(KeyStroke.
                getKeyStroke(KeyEvent.VK_H, InputEvent.CTRL_MASK));
        ...
        connectMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N, InputEvent.CTRL_MASK));
    ...
    private void createMDI() {
        ...
        Component oldCenter = ((BorderLayout) cp.getLayout()).
    ...
    private class WindowMenu extends JMenu {
        ...
        WindowMenu(String text) {
            ...
            tileMI.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_T, InputEvent.CTRL_MASK));
        ...
        private void add(VMInternalFrame vmIF) {
            ...
            String text = SwingUtilities
                .layoutCompoundLabel(this, getGraphics().getFontMetrics(getFont()), indexString + " " + vmName, null, 0,
                    0, 0, 0, viewR, iconR, textR, 0);
    ...
    public void actionPerformed(ActionEvent ev) {
        ...
        if (src == connectButton || src == connectMI) {
            ...
            if (vmPanel != null) {
                ...
                if (vmPanel.getUrl() != null) { url = vmPanel.getUrl(); }
            }
    ...
    public void internalFrameClosing(InternalFrameEvent e) {
        ...
        if (!isProxyClientUsed(client)) { client.markAsDead(); }
        if (windows.size() == 0) {
    ...
    private static void mainInit(final List<String> urls, final List<String> hostNames, final List<Integer> ports,
        final List<LocalVirtualMachine> vmids, final ProxyClient proxyClient, final boolean noTile,
        ...
        SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                jConsole
                    .setBounds(scrBounds.x + (scrBounds.width - w) / 2, scrBounds.y + (scrBounds.height - h) / 2, w, h);

                jConsole.setVisible(true);
                jConsole.createMDI();

                for (int i = 0; i < hostNames.size(); i++) {
                    jConsole.addHost(hostNames.get(i), ports.get(i), null, null,
                        (i == hostNames.size() - 1) ? !noTile : false);
                }
    ...
    private static void initPluginService(String pluginPath) {
        if (pluginPath.length() > 0) {
            try {
                ...
                System.out.println(Resources.format(Messages.FAIL_TO_LOAD_PLUGIN, e.getMessage()));
            } catch (MalformedURLException e) {
                if (JConsole.isDebug()) {
                    e.printStackTrace();
                }
                System.out.println(Resources.format(Messages.INVALID_PLUGIN_PATH, e.getMessage()));
            }
        }
