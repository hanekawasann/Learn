...
class ThreadTab extends Tab implements ActionListener, DocumentListener, ListSelectionListener {
    ...
    */
    ...
    public ThreadTab(VMPanel vmPanel) {
        ...
        threadMeter = new PlotterPanel(Messages.NUMBER_OF_THREADS, Plotter.Unit.NONE, true);
        ...
        setAccessibleName(threadMeter.plotter, Messages.THREAD_TAB_THREAD_PLOTTER_ACCESSIBLE_NAME);
        ...
        setAccessibleName(textArea, Messages.THREAD_TAB_THREAD_INFO_ACCESSIBLE_NAME);
        ...
        threadsSplitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, threadlistSP, textAreaSP);
        ...
        separator
            .setPreferredSize(new Dimension(separator.getPreferredSize().width, filterTF.getPreferredSize().height));
        ...
        return new SwingWorker<Boolean, Object>() {
            ...
            protected void done() {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (doFilter && name != null && name.toLowerCase(Locale.ENGLISH).indexOf(filter) < 0) {
                    ...
                    try {
                        ...
                        if (ti != null) {
                            if (ti.getLockName() == null) {
                                sb.append(Resources
                                    .format(Messages.NAME_STATE, ti.getThreadName(), ti.getThreadState().toString()));
                            } else if (ti.getLockOwnerName() == null) {
                                sb.append(Resources.format(Messages.NAME_STATE_LOCK_NAME, ti.getThreadName(),
                                    ti.getThreadState().toString(), ti.getLockName()));
                            } else {
                                sb.append(Resources.format(Messages.NAME_STATE_LOCK_NAME_LOCK_OWNER, ti.getThreadName(),
                                    ti.getThreadState().toString(), ti.getLockName(), ti.getLockOwnerName()));
                            }
                            sb.append(Resources.format(Messages.BLOCKED_COUNT_WAITED_COUNT, ti.getBlockedCount(),
                                ti.getWaitedCount()));
                        ...
                        public void run() {
                            ...
                            if (deadlockedThreads != null) {
                                for (int i = 0; i < deadlockedThreads.length; i++) {
                                    ...
                                    threadlistSP.setBorder(null);
                                    textAreaSP.setBorder(null);
                                    JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, threadlistSP,
                                        textAreaSP);
