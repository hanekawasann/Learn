...
abstract class OverviewPanel extends PlotterPanel {
    ...
    public OverviewPanel(String title, String plotterKey, String plotterName, Plotter.Unit plotterUnit) {
        ...
        if (plotterKey != null && plotterName != null) {
            ...
            setAccessibleName(plotter, Resources.format(Messages.OVERVIEW_PANEL_PLOTTER_ACCESSIBLE_NAME, title));
            setPlotter(plotter);
        }
    }
            ...
            infoLabel = new JLabel("", CENTER) {
                ...
                public void setText(String text) {
                    if (text.startsWith("<html>")) {
                        ...
                        int n = chars.length;
                        for (int i = 0; i < n; i++) {
                            if (chars[i] == ' ' && ((i < n - 1 && chars[i + 1] == ' ') ||
                                ((i == 0 || chars[i - 1] != ' ') && (i == n - 1 || chars[i + 1] != ' ')))) {
                                buf.append("&nbsp;");
                            } else {
                                buf.append(chars[i]);
                            }
