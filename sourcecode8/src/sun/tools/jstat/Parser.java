...
public class Parser {
    ...
    private static char[] delimiters = { OPENBLOCK, CLOSEBLOCK, PERCENT_CHAR, OPENPAREN, CLOSEPAREN };
    ...
    private Token matchOne(Set keyWords) throws ParserException, IOException {
        if ((lookahead.ttype == StreamTokenizer.TT_WORD) && keyWords.contains(lookahead.sval)) {
            Token t = lookahead;
            nextToken();
            return t;
        }
    ...
    private void match(int ttype, String token) throws ParserException, IOException {
        if (lookahead.ttype == ttype && lookahead.sval.compareTo(token) == 0) {
            ...
            throw new SyntaxException(st.lineno(), new Token(ttype, token), lookahead);
        }
    }
    ...
    private void match(char ttype) throws ParserException, IOException {
        if (lookahead.ttype == (int) ttype) {
            ...
            throw new SyntaxException(st.lineno(), new Token((int) ttype), lookahead);
        }
    }
    ...
    private Expression primary() throws ParserException, IOException {
        ...
        switch (lookahead.ttype) {
            ...
            case StreamTokenizer.TT_WORD:
                ...
                if (isReservedWord(s)) {
                    throw new SyntaxException(st.lineno(), "IDENTIFIER", "Reserved Word: " + lookahead.sval);
                }
                matchID();
                e = new Identifier(s);
    ...
    public OptionFormat parse(String option) throws ParserException, IOException {
        ...
        while (lookahead.ttype != StreamTokenizer.TT_EOF) {
            ...
            if ((lookahead.ttype != StreamTokenizer.TT_WORD) || (lookahead.sval.compareTo(START) != 0)) {
            ...
            if ((lookahead.ttype == StreamTokenizer.TT_WORD) && (lookahead.sval.compareTo(option) == 0)) {
