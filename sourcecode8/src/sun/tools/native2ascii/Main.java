...
public class Main {
    ...
    public synchronized boolean convert(String argv[]) {
        ...
        if (encodingString == null) { defaultEncoding = Charset.defaultCharset().name(); }

        char[] lineBreak = System.getProperty("line.separator").toCharArray();
        try {
            initializeConverter();

            if (v.size() == 1) { inputFileName = v.get(0); }
    ...
    private BufferedReader getN2AInput(String inFile) throws Exception {
        ...
        if (inFile == null) { forwardIn = System.in; } else {
        ...
        BufferedReader r = (encodingString != null) ? new BufferedReader(
            new InputStreamReader(forwardIn, encodingString)) : new BufferedReader(new InputStreamReader(forwardIn));
        return r;
    }
    ...
    private BufferedWriter getN2AOutput(String outFile) throws Exception {
        ...
        if (outFile == null) { output = new OutputStreamWriter(System.out, "US-ASCII"); } else {
            File f = new File(outFile);

            File tempDir = f.getParentFile();

            if (tempDir == null) { tempDir = new File(System.getProperty("user.dir")); }
    ...
    private BufferedReader getA2NInput(String inFile) throws Exception {
        ...
        if (inFile == null) { in = new InputStreamReader(System.in, "US-ASCII"); } else {
            File f = new File(inFile);
            if (!f.canRead()) {
                throw new Exception(formatMsg("err.cannot.read", f.getName()));
            }
    ...
    private Writer getA2NOutput(String outFile) throws Exception {
        ...
        if (outFile == null) { output = System.out; } else {
            File f = new File(outFile);
        ...
        w = (encodingString != null) ? new OutputStreamWriter(output, encodingString) : new OutputStreamWriter(output);
