...
public class NewArrayExpression extends NaryExpression {
    ...
    public Expression inline(Environment env, Context ctx) {
        ...
        if (init != null) { e = (e != null) ? new CommaExpression(where, e, init) : init; }
    ...
    public void codeValue(Environment env, Context ctx, Assembler asm) {
        ...
        switch (type.getElementType().getTypeCode()) {
            case TC_BOOLEAN:
                asm.add(where, opc_newarray, new Integer(T_BOOLEAN));
                break;
            case TC_BYTE:
                asm.add(where, opc_newarray, new Integer(T_BYTE));
                break;
            case TC_SHORT:
                asm.add(where, opc_newarray, new Integer(T_SHORT));
                break;
            case TC_CHAR:
                asm.add(where, opc_newarray, new Integer(T_CHAR));
                break;
            case TC_INT:
                asm.add(where, opc_newarray, new Integer(T_INT));
                break;
            case TC_LONG:
                asm.add(where, opc_newarray, new Integer(T_LONG));
                break;
            case TC_FLOAT:
                asm.add(where, opc_newarray, new Integer(T_FLOAT));
                break;
            case TC_DOUBLE:
                asm.add(where, opc_newarray, new Integer(T_DOUBLE));
                break;
            case TC_ARRAY:
