...
public class BitAndExpression extends BinaryBitExpression {
     ...
     */
    Expression simplify() {
        if (left.equals(true)) { return right; }
        if (right.equals(true)) { return left; }
        if (left.equals(false) || left.equals(0)) { return new CommaExpression(where, right, left).simplify(); }
        if (right.equals(false) || right.equals(0)) { return new CommaExpression(where, left, right).simplify(); }
        return this;
    }
