...
public class ConditionalExpression extends BinaryExpression {
    ...
    public Vset checkValue(Environment env, Context ctx, Vset vset, Hashtable exp) {
        ...
        } else if (((tm & TM_CHAR) != 0) && left.fitsType(env, ctx, Type.tChar) &&
            right.fitsType(env, ctx, Type.tChar)) {
            type = Type.tChar;
        } else if (((tm & TM_SHORT) != 0) && left.fitsType(env, ctx, Type.tShort) &&
            right.fitsType(env, ctx, Type.tShort)) {
            type = Type.tShort;
        } else if (((tm & TM_BYTE) != 0) && left.fitsType(env, ctx, Type.tByte) &&
            right.fitsType(env, ctx, Type.tByte)) {
            type = Type.tByte;
        } else {
            type = Type.tInt;
        }
    ...
    public int costInline(int thresh, Environment env, Context ctx) {
        ...
        // (Fix for 4069861).
        return 1 + cond.costInline(thresh, env, ctx) + left.costInline(thresh, env, ctx) +
