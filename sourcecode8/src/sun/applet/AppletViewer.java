    ...
    private void initEventQueue() {
        ...
        if (eventList == null) {
            ...
            for (int i = 0; i < events.length; i++) {
                ...
                if (events[i].equals("dispose")) { panel.sendEvent(AppletPanel.APPLET_DISPOSE); } else if (events[i]
                    .equals("load")) { panel.sendEvent(AppletPanel.APPLET_LOAD); } else if (events[i].equals("init")) {
                    panel.sendEvent(AppletPanel.APPLET_INIT);
                } else if (events[i].equals("start")) {
                    panel.sendEvent(AppletPanel.APPLET_START);
                } else if (events[i].equals("stop")) { panel.sendEvent(AppletPanel.APPLET_STOP); } else if (events[i]
                    .equals("destroy")) {
                    panel.sendEvent(AppletPanel.APPLET_DESTROY);
                } else if (events[i].equals("quit")) { panel.sendEvent(AppletPanel.APPLET_QUIT); } else if (events[i]
                    .equals("error")) { panel.sendEvent(AppletPanel.APPLET_ERROR); } else
    ...
    void appletSave() {
        AccessController.doPrivileged(new PrivilegedAction() {
            ...
            public Object run() {
                ...
                panel.sendEvent(AppletPanel.APPLET_STOP);
                FileDialog fd = new FileDialog(AppletViewer.this, amh.getMessage("appletsave.filedialogtitle"),
                    FileDialog.SAVE);
                ...
                try (FileOutputStream fos = new FileOutputStream(
                    file); BufferedOutputStream bos = new BufferedOutputStream(
                    fos); ObjectOutputStream os = new ObjectOutputStream(bos)) {
        ...
        while (true) {
            ...
            if (c == '<') {
                ...
                if (c == '/') {
                    ...
                    if (nm.equalsIgnoreCase("applet") || nm.equalsIgnoreCase("object") ||
    ...
    private static void checkConnect(URL url) {
        ...
        if (security != null) {
            try {
                ...
                if (perm != null) { security.checkPermission(perm); } else {
