...
public abstract class AppletPanel extends Panel implements AppletStub, Runnable {
    ...
    synchronized void createAppletThread() {
        ...
        if (param != null && param.equals("false")) { loader.setCodebaseLookup(false); } else {
            ...
            try {
                switch (evt.getID()) {
                    ...
                    case APPLET_INIT:
                        ...
                        Font f = getFont();
                        if (f == null ||
                            "dialog".equals(f.getFamily().toLowerCase(Locale.ENGLISH)) && f.getSize() == 12 &&
    ...
    private Component getMostRecentFocusOwnerForWindow(Window w) {
        Method meth = (Method) AccessController.doPrivileged(new PrivilegedAction() {
            public Object run() {
                Method meth = null;
                try {
                    meth = KeyboardFocusManager.class
                        .getDeclaredMethod("getMostRecentFocusOwner", new Class[] { Window.class });
    ...
    protected Applet createApplet(final AppletClassLoader loader)
        ...
        if (code != null) {
            ...
            // serName is not null;
            try (InputStream is = AccessController.doPrivileged((PrivilegedAction<InputStream>) () -> loader
                .getResourceAsStream(serName)); ObjectInputStream ois = new AppletObjectInputStream(is, loader)) {
    ...
    public static void changeFrameAppContext(Frame frame, AppContext newAppContext) {
        ...
        synchronized (Window.class) {
            ...
            // Remove frame from the Window list in wrong AppContext
            {
                ...
                Vector<WeakReference<Window>> windowList = (Vector<WeakReference<Window>>) oldAppContext
                    .get(Window.class);
                if (windowList != null) {
    ...
    private void findAppletJDKLevel(Applet applet) {
        ...
        synchronized (appletClass) {
            ...
            try (InputStream is = AccessController
                .doPrivileged((PrivilegedAction<InputStream>) () -> loader.getResourceAsStream(resourceName))) {
            ...
            //   48 - JDK 1.4
            //   49 - JDK 1.5
            if (major_version < 46) { jdk11Applet = true; } else if (major_version == 46) { jdk12Applet = true; }
