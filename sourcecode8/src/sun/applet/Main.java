...
public class Main {
    ...
    static final String[][] avDefaultUserProps = {
        ...
        { "http.proxyHost", "" }, { "http.proxyPort", "80" }, { "package.restrict.access.sun", "true" } };
     ...
     * decoded.
     * @throws ParseException Thrown when there is a problem with something in the
     *                        argument array.
     */
     ...
     * @throws ParseException Thrown when we are unable to construct a proper URL from the
     *                        passed in string.
     */
    private URL parseURL(String url) throws ParseException {
        ...
        try {
            ...
            } else if (url.startsWith(prefix) && url.length() != prefix.length() &&
    ...
    private void init() {
        ...
        for (Enumeration e = sysProps.propertyNames(); e.hasMoreElements(); ) {
            ...
            if ((oldVal = (String) avProps.setProperty(key, val)) != null) {
                System.err.println(lookup("main.warn.prop.overwrite", key, oldVal, val));
            }
        }
