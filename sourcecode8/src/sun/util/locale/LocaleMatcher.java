    ...
    public static List<String> filterTags(List<LanguageRange> priorityList, Collection<String> tags,
        ...
        if (mode == EXTENDED_FILTERING) {
            ...
            for (LanguageRange lr : priorityList) {
                ...
                if (range.startsWith("*-") || range.indexOf("-*") != -1) { // Extended range
                    if (mode == AUTOSELECT_FILTERING) {
                        ...
                        throw new IllegalArgumentException(
                            "An extended range \"" + range + "\" found in REJECT_EXTENDED_RANGES mode.");
                    }
                } else { // Basic range
                    list.add(lr);
                }
            ...
            if (range.equals("*")) {
                ...
                for (String tag : tags) {
                    ...
                    if (tag.startsWith(range)) {
                        ...
                        if ((tag.length() == len || tag.charAt(len) == '-') && !list.contains(tag)) {
                            list.add(tag);
                        }
                    }
                }
            ...
            for (String tag : tags) {
                ...
                while (rangeIndex < rangeSubtags.length && tagIndex < tagSubtags.length) {
                    if (rangeSubtags[rangeIndex].equals("*")) {
                        ...
                        rangeIndex++;
                        tagIndex++;
                    } else if (tagSubtags[tagIndex].length() == 1 && !tagSubtags[tagIndex].equals("*")) {
                        break;
                    } else {
                        tagIndex++;
                    }
        ...
        for (String range : langRanges) {
            ...
            if ((index = range.indexOf(";q=")) == -1) {
                ...
                try {
                    ...
                    throw new IllegalArgumentException(
                        "weight=\"" + range.substring(index) + "\" for language range \"" + r + "\"");
                }

                if (w < MIN_WEIGHT || w > MAX_WEIGHT) {
                    throw new IllegalArgumentException(
                        "weight=" + w + " for language range \"" + r + "\". It must be between " + MIN_WEIGHT +
                            " and " + MAX_WEIGHT + ".");
                }
            }
            ...
            if (!tempList.contains(r)) {
                ...
                // aa-XX -> aa-YY
                String equivalent;
                if ((equivalent = getEquivalentForRegionAndVariant(r)) != null && !tempList.contains(equivalent)) {
                ...
                if (keyMap.containsKey(r)) {
                    ...
                    if (equivalents != null) {
                        ...
                        for (String equivalent : equivalents) {
                            list.add(
                                new LanguageRange(equivalent.toLowerCase() + range.substring(len), lr.getWeight()));
                        }
                    }
