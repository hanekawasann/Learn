...
public class CalendarNameProviderImpl extends CalendarNameProvider implements AvailableLanguageTags {
    ...
    public String getDisplayNameImpl(String calendarType, int field, int value, int style, Locale locale,
        boolean javatime) {
        String name = null;
        ...
        if (key != null) {
            ...
            if (strings != null && strings.length > 0) {
                ...
                if (name.length() == 0 &&
                    (style == SHORT_STANDALONE || style == LONG_STANDALONE || style == NARROW_STANDALONE)) {
                    name = getDisplayName(calendarType, field, value, getBaseStyle(style), locale);
                }
            }
        }
        return name;
    ...
    private static int[] REST_OF_STYLES = { SHORT_STANDALONE, LONG_FORMAT, LONG_STANDALONE, NARROW_FORMAT,
        NARROW_STANDALONE };
    ...
    private Map<String, Integer> getDisplayNamesImpl(String calendarType, int field, int style, Locale locale,
        boolean javatime) {
        String key = getResourceKey(calendarType, field, style, javatime);
