...
public final class ZoneInfoFile {
    ...
    private static String[][] oldMappings = new String[][] { { "ACT", "Australia/Darwin" },
        { "AET", "Australia/Sydney" }, { "AGT", "America/Argentina/Buenos_Aires" }, { "ART", "Africa/Cairo" },
        { "AST", "America/Anchorage" }, { "BET", "America/Sao_Paulo" }, { "BST", "Asia/Dhaka" },
        { "CAT", "Africa/Harare" }, { "CNT", "America/St_Johns" }, { "CST", "America/Chicago" },
        { "CTT", "Asia/Shanghai" }, { "EAT", "Africa/Addis_Ababa" }, { "ECT", "Europe/Paris" },
        { "IET", "America/Indiana/Indianapolis" }, { "IST", "Asia/Kolkata" }, { "JST", "Asia/Tokyo" },
        { "MIT", "Pacific/Apia" }, { "NET", "Asia/Yerevan" }, { "NST", "Pacific/Auckland" }, { "PLT", "Asia/Karachi" },
        { "PNT", "America/Phoenix" }, { "PRT", "America/Puerto_Rico" }, { "PST", "America/Los_Angeles" },
        { "SST", "Pacific/Guadalcanal" }, { "VST", "Asia/Ho_Chi_Minh" }, };

    static {
        String oldmapping = AccessController.doPrivileged(new GetPropertyAction("sun.timezone.ids.oldmapping", "false"))
     ...
     */
    private static ZoneInfo getZoneInfo(String zoneId, long[] standardTransitions, int[] standardOffsets,
        long[] savingsInstantTransitions, int[] wallOffsets, ZoneOffsetTransitionRule[] lastRules) {
        int rawOffset = 0;
        int dstSavings = 0;
        int checksum = 0;
        int[] params = null;
        ...
        if (savingsInstantTransitions.length != 0) {
            ...
            if (i < savingsInstantTransitions.length) {
                ...
                nOffsets = addTrans(transitions, nTrans++, offsets, nOffsets, UTC1900, wallOffsets[i],
            ...
            for (; i < savingsInstantTransitions.length; i++) {
                ...
                while (k < standardTransitions.length) {
                    ...
                    if (trans_s >= UTC1900) {
                        ...
                        if (trans_s < trans) {
                            ...
                            nOffsets = addTrans(transitions, nTrans++, offsets, nOffsets, trans_s, wallOffsets[i],
                                standardOffsets[k + 1]);
                ...
                nOffsets = addTrans(transitions, nTrans++, offsets, nOffsets, trans, wallOffsets[i + 1],
            ...
            while (k < standardTransitions.length) {
                ...
                if (trans >= UTC1900) {
                    ...
                    transitions[nTrans++] = ((trans * 1000) << TRANSITION_NSHIFT) | (offsetIndex & OFFSET_MASK);
                }
                k++;
            }
            if (lastRules.length > 1) {
                ...
                while (lastyear++ < LASTYEAR) {
                    for (ZoneOffsetTransitionRule zotr : lastRules) {
                        ...
                        nOffsets = addTrans(transitions, nTrans++, offsets, nOffsets, trans, zotr.offsetAfter,
                            zotr.standardOffset);
                    }
                }
                ...
                if (params[2] == 6 && params[3] == 0 &&
                    (zoneId.equals("Asia/Amman") || zoneId.equals("Asia/Gaza") || zoneId.equals("Asia/Hebron"))) {
                    params[2] = 5;
                    params[3] = 86400000;
                }
    ...
    private static final int toCalendarDOW[] = new int[] { -1, Calendar.MONDAY, Calendar.TUESDAY, Calendar.WEDNESDAY,
        Calendar.THURSDAY, Calendar.FRIDAY, Calendar.SATURDAY, Calendar.SUNDAY };

    private static final int toSTZTime[] = new int[] { SimpleTimeZone.UTC_TIME, SimpleTimeZone.WALL_TIME,
    ...
    private static int addTrans(long transitions[], int nTrans, int offsets[], int nOffsets, long trans, int offset,
        int stdOffset) {
        int offsetIndex = indexOf(offsets, 0, nOffsets, offset);
        if (offsetIndex == nOffsets) { nOffsets++; }
        int dstIndex = 0;
        ...
        transitions[nTrans] = ((trans * 1000) << TRANSITION_NSHIFT) | ((dstIndex << DST_NSHIFT) & DST_MASK) |
