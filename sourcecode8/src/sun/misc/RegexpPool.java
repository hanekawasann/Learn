...
public class RegexpPool {
    ...
    public Object delete(String re) {
        ...
        if (!re.startsWith("*") || !re.endsWith("*")) { len++; }
    ...
    private void add(String re, Object ret, boolean replace) throws REException {
        ...
        if (re.charAt(0) == '*') {
            p = suffixMachine;
            while (len > 1) { p = p.add(re.charAt(--len)); }
        } else {
            boolean exact = false;
            if (re.charAt(len - 1) == '*') { len--; } else { exact = true; }
            p = prefixMachine;
            for (int i = 0; i < len; i++) { p = p.add(re.charAt(i)); }
            p.exact = exact;
        }

        if (p.result != null && !replace) { throw new REException(re + " is a duplicate"); }
    ...
    private Object matchAfter(String s, int lastMatchDepth) {
        ...
        if (o != null && o instanceof RegexpTarget) { o = ((RegexpTarget) o).found(s.substring(bst, bend)); }
        return o;
    }
...
class RegexpNode {
    ...
    RegexpNode add(char C) {
        ...
        if (p == null) { p = new RegexpNode(C, depth + 1); } else {
            while (p != null) { if (p.c == C) { return p; } else { p = p.nextsibling; } }
    ...
    RegexpNode find(char C) {
        for (RegexpNode p = firstchild; p != null; p = p.nextsibling) { if (p.c == C) { return p; } }
        return null;
    }
    ...
    void print(PrintStream out) {
        if (nextsibling != null) {
            ...
            while (p != null) {
                out.write(p.c);
                if (p.firstchild != null) { p.firstchild.print(out); }
                p = p.nextsibling;
                out.write(p != null ? '|' : ')');
            }
        } else {
            out.write(c);
            if (firstchild != null) { firstchild.print(out); }
        }
    }
}
