        ...
        private CoderResult decodeArrayLoop(ByteBuffer src, CharBuffer dst) {
            ...
            while (sp < sl) {
                ...
                if (b1 >= 0) {
                    ...
                    if (Character.isSurrogate(c)) { return malformedForLength(src, sp, dst, dp, 3); }
                    da[dp++] = c;
                    sp += 3;
                } else if ((b1 >> 3) == -2) {
        ...
        private CoderResult decodeBufferLoop(ByteBuffer src, CharBuffer dst) {
            ...
            while (mark < limit) {
                ...
                if (b1 >= 0) {
                    ...
                    char c = (char) ((b1 << 12) ^ (b2 << 6) ^
                        (b3 ^ (((byte) 0xE0 << 12) ^ ((byte) 0x80 << 6) ^ ((byte) 0x80 << 0))));
                    if (Character.isSurrogate(c)) { return malformedForLength(src, mark, 3); }
                    dst.put(c);
                    mark += 3;
                } else if ((b1 >> 3) == -2) {
                    // 4 bytes, 21 bits: 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx
                    ...
                    if (srcRemaining < 4 || dst.remaining() < 2) {
                        ...
                        if (srcRemaining > 2 && isMalformed4_3(src.get())) { return malformedForLength(src, mark, 2); }
