...
public class SingleByte {
    ...
    final public static class Decoder extends CharsetDecoder implements ArrayDecoder {
    ...
    final public static class Encoder extends CharsetEncoder implements ArrayEncoder {
        ...
        public boolean isLegalReplacement(byte[] repl) {
            return ((repl.length == 1 && repl[0] == (byte) '?') || super.isLegalReplacement(repl));
        }
            ...
            while (sp < sl) {
                ...
                if (b == UNMAPPABLE_ENCODING) {
                    if (Character.isSurrogate(c)) {
                        ...
                        if (sgp.parse(c, sa, sp, sl) < 0) { return withResult(sgp.error(), src, sp, dst, dp); }
    ...
    public static void initC2B(char[] b2c, char[] c2bNR, char[] c2b, char[] c2bIndex) {
        for (int i = 0; i < c2bIndex.length; i++) { c2bIndex[i] = UNMAPPABLE_ENCODING; }
        for (int i = 0; i < c2b.length; i++) { c2b[i] = UNMAPPABLE_ENCODING; }
        int off = 0;
