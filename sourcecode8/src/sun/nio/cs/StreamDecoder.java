...
public class StreamDecoder extends Reader {
    ...
    StreamDecoder(InputStream in, Object lock, Charset cs) {
        this(in, lock, cs.newDecoder().onMalformedInput(CodingErrorAction.REPLACE)
    ...
    StreamDecoder(ReadableByteChannel ch, CharsetDecoder dec, int mbc) {
        ...
        this.cs = dec.charset();
        this.bb = ByteBuffer
            .allocate(mbc < 0 ? DEFAULT_BYTE_BUFFER_SIZE : (mbc < MIN_BYTE_BUFFER_SIZE ? MIN_BYTE_BUFFER_SIZE : mbc));
        bb.flip();
    }
    ...
    void implClose() throws IOException {
        if (ch != null) { ch.close(); } else { in.close(); }
    }
