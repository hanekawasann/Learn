...
public class DoubleByte {
    ...
    public static class Decoder extends CharsetDecoder implements DelegatableDecoder, ArrayDecoder {
        ...
        public char decodeDouble(int b1, int b2) {
            if (b1 < 0 || b1 > b2c.length || b2 < b2Min || b2 > b2Max) { return UNMAPPABLE_DECODING; }
            return b2c[b1][b2 - b2Min];
        }
    }
    ...
    public static class Decoder_EBCDIC extends Decoder {
            ...
            try {
                ...
                while (sp < sl) {
                    ...
                    if (b1 == SO) {  // Shift out
                        ...
                        if (currentState != DBCS) { return CoderResult.malformedForLength(1); } else {
                            currentState = SBCS;
                        }
                    } else {
                        ...
                        if (currentState == SBCS) {
                            c = b2cSB[b1];
                            if (c == UNMAPPABLE_DECODING) { return CoderResult.unmappableForLength(1); }
                        } else {
        ...
        public int decode(byte[] src, int sp, int len, char[] dst) {
            ...
            while (sp < sl) {
                int b1 = src[sp++] & 0xff;
                if (b1 == SO) {  // Shift out
                    if (currentState != SBCS) { dst[dp++] = repl; } else { currentState = DBCS; }
                } else if (b1 == SI) {
                    if (currentState != DBCS) { dst[dp++] = repl; } else { currentState = SBCS; }
                } else {
                    char c = UNMAPPABLE_DECODING;
                    if (currentState == SBCS) {
                        c = b2cSB[b1];
                        if (c == UNMAPPABLE_DECODING) { c = repl; }
                    } else {
                        if (sl == sp) {
                            c = repl;
                        } else {
    ...
    public static class Encoder extends CharsetEncoder implements ArrayEncoder {
        ...
        public int encode(char[] src, int sp, int len, byte[] dst) {
            ...
            while (sp < sl) {
                ...
                if (bb == UNMAPPABLE_ENCODING) {
                    ...
                    dst[dp++] = repl[0];
                    if (repl.length > 1) { dst[dp++] = repl[1]; }
                    continue;
                } //else
                if (bb > MAX_SINGLEBYTE) { // DoubleByte
                    dst[dp++] = (byte) (bb >> 8);
        ...
        static void initC2B(String[] b2c, String b2cSB, String b2cNR, String c2bNR, int b2Min, int b2Max, char[] c2b,
            char[] c2bIndex) {
            ...
            if (b2cNR != null) {
                ...
                while (j < b2cNR.length()) {
                    ...
                    if (b < 0x100 && b2cSB_ca != null) {
                        if (b2cSB_ca[b] == c) { b2cSB_ca[b] = UNMAPPABLE_DECODING; }
                    } else {
            ...
            if (b2cSB_ca != null) {      // SingleByte
                for (int b = 0; b < b2cSB_ca.length; b++) {
                    char c = b2cSB_ca[b];
                    if (c == UNMAPPABLE_DECODING) { continue; }
    ...
    public static class Encoder_DBCSONLY extends Encoder {
        Encoder_DBCSONLY(Charset cs, byte[] repl, char[] c2b, char[] c2bIndex) {
            super(cs, 2.0f, 2.0f, repl, c2b, c2bIndex);
        }

        public int encodeChar(char ch) {
            int bb = super.encodeChar(ch);
            if (bb <= MAX_SINGLEBYTE) { return UNMAPPABLE_ENCODING; }
            return bb;
        }
    }
