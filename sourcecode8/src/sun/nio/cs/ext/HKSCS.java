...
public class HKSCS {
    ...
    public static class Decoder extends DoubleByte.Decoder {
            ...
            try {
                while (sp < sl) {
                    ...
                    if (c == UNMAPPABLE_DECODING) {
                        ...
                        int b2 = sa[sp + 1] & 0xff;
                        inSize++;
                        if (b2 < b2Min || b2 > b2Max) { return CoderResult.unmappableForLength(2); }
                        c = decodeDouble(b1, b2);           //bmp
                        if (c == UNMAPPABLE_DECODING) {
                            c = decodeDoubleEx(b1, b2);     //supp
                            if (c == UNMAPPABLE_DECODING) {
                                c = decodeBig5(b1, b2);     //big5
                                if (c == UNMAPPABLE_DECODING) { return CoderResult.unmappableForLength(2); }
                            } else {
        ...
        static void initb2c(char[][] b2c, String[] b2cStr) {
            for (int i = 0; i < b2cStr.length; i++) {
                if (b2cStr[i] == null) { b2c[i] = DoubleByte.B2C_UNMAPPABLE; } else {
    ...
    public static class Encoder extends DoubleByte.Encoder {
        ...
        public int encodeChar(char ch) {
            ...
            if (bb == UNMAPPABLE_ENCODING) { return encodeBig5(ch); }
            return bb;
        }

        public int encodeSupp(int cp) {
            if ((cp & 0xf0000) != 0x20000) { return UNMAPPABLE_ENCODING; }
                ...
                while (sp < sl) {
                    ...
                    if (bb == UNMAPPABLE_ENCODING) {
                        if (Character.isSurrogate(c)) {
                            int cp;
                            if ((cp = sgp().parse(c, sa, sp, sl)) < 0) { return sgp.error(); }
                            bb = encodeSupp(cp);
                            if (bb == UNMAPPABLE_ENCODING) { return CoderResult.unmappableForLength(2); }
                            inSize = 2;
                        } else {
            ...
            try {
                while (src.hasRemaining()) {
                    ...
                    if (bb == UNMAPPABLE_ENCODING) {
                        if (Character.isSurrogate(c)) {
                            int cp;
                            if ((cp = sgp().parse(c, src)) < 0) { return sgp.error(); }
                            bb = encodeSupp(cp);
                            if (bb == UNMAPPABLE_ENCODING) { return CoderResult.unmappableForLength(2); }
                            inSize = 2;
                        } else {
            ...
            while (sp < sl) {
                ...
                if (bb == UNMAPPABLE_ENCODING) {
                    if (!Character.isHighSurrogate(c) || sp == sl || !Character.isLowSurrogate(src[sp]) ||
                        (bb = encodeSupp(Character.toCodePoint(c, src[sp++]))) == UNMAPPABLE_ENCODING) {
                        dst[dp++] = repl[0];
