...
public class SJIS_0213 extends Charset {
    ...
    static CharsetMapping mapping = AccessController.doPrivileged(new PrivilegedAction<CharsetMapping>() {
    ...
    protected static class Decoder extends CharsetDecoder {
        ...
        protected char[] decodeDoubleEx(int b1, int b2) {
            int db = (b1 << 8) | b2;
            if (mapping.decodeSurrogate(db, cc) != null) { return cc; }
            comp.bs = db;
            if (mapping.decodeComposite(comp, cc) != null) { return cc; }
            return null;
        }
    }
            ...
            try {
                while (sp < sl) {
                    ...
                    if (leftoverBase != 0) {
                        ...
                        if (db == UNMAPPABLE) { db = encodeChar(leftoverBase); } else { isComp = true; }
                    ...
                    if (isCompositeBase(c)) {
                        ...
                        if (db <= MAX_SINGLEBYTE) {      // SingleByte
                            if (dl <= dp) { return CoderResult.OVERFLOW; }
                            da[dp++] = (byte) db;
                            ...
                            if (db == UNMAPPABLE) { return CoderResult.unmappableForLength(2); }
                            if (dl - dp < 2) { return CoderResult.OVERFLOW; }
                            da[dp++] = (byte) (db >> 8);
                            da[dp++] = (byte) db;
                            sp++;
