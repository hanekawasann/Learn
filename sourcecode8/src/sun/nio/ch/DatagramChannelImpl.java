...
class DatagramChannelImpl extends DatagramChannel implements SelChImpl {
    ...
    public DatagramChannelImpl(SelectorProvider sp, ProtocolFamily family) throws IOException {
        ...
        if ((family != StandardProtocolFamily.INET) && (family != StandardProtocolFamily.INET6)) {
            if (family == null) { throw new NullPointerException("'family' is null"); } else {
    ...
    public <T> DatagramChannel setOption(SocketOption<T> name, T value) throws IOException {
        ...
        synchronized (stateLock) {
            ...
            if (name == StandardSocketOptions.IP_MULTICAST_TTL || name == StandardSocketOptions.IP_MULTICAST_LOOP) {
    ...
    private int sendFromNativeBuffer(FileDescriptor fd, ByteBuffer bb, InetSocketAddress target) throws IOException {
        ...
        int written;
        try {
            written = send0(preferIPv6, fd, ((DirectBuffer) bb).address() + pos, rem, target.getAddress(),
                target.getPort());
    ...
    private MembershipKey innerJoin(InetAddress group, NetworkInterface interf, InetAddress source) throws IOException {
        ...
        synchronized (stateLock) {
            ...
            if ((family == StandardProtocolFamily.INET6) &&
                ...
                key = new MembershipKeyImpl.Type4(this, group, interf, source, groupAddress, targetAddress,
                    sourceAddress);
            }
    ...
    private native int send0(boolean preferIPv6, FileDescriptor fd, long address, int len, InetAddress addr, int port)
        throws IOException;
