...
public class SimpleAsynchronousFileChannelImpl extends AsynchronousFileChannelImpl {
    ...
    SimpleAsynchronousFileChannelImpl(FileDescriptor fdObj, boolean reading, boolean writing,
    ...
    @Override
    <A> Future<FileLock> implLock(final long position, final long size, final boolean shared, final A attachment,
        ...
        final PendingFuture<FileLock, A> result = (handler == null) ? new PendingFuture<FileLock, A>(this) : null;
    ...
    public FileLock tryLock(long position, long size, boolean shared) throws IOException {
        ...
        try {
            ...
            if (n == FileDispatcher.INTERRUPTED) { throw new AsynchronousCloseException(); }
            // should not get here
    ...
    @Override
    <A> Future<Integer> implRead(final ByteBuffer dst, final long position, final A attachment,
        ...
        if (!isOpen() || (dst.remaining() == 0)) {
            ...
            if (handler == null) { return CompletedFuture.withResult(0, exc); }
            Invoker.invokeIndirectly(handler, attachment, 0, exc, executor);
            return null;
        }

        final PendingFuture<Integer, A> result = (handler == null) ? new PendingFuture<Integer, A>(this) : null;
        Runnable task = new Runnable() {
            public void run() {
                int n = 0;
                Throwable exc = null;
                ...
                try {
                    ...
                    if (n < 0 && !isOpen()) { throw new AsynchronousCloseException(); }
                } catch (IOException x) {
                    if (!isOpen()) { x = new AsynchronousCloseException(); }
                    exc = x;
                } finally {
    ...
    @Override
    <A> Future<Integer> implWrite(final ByteBuffer src, final long position, final A attachment,
        final CompletionHandler<Integer, ? super A> handler) {
        if (position < 0) { throw new IllegalArgumentException("Negative position"); }
        if (!writing) { throw new NonWritableChannelException(); }
