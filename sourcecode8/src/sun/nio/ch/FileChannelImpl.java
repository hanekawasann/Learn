...
public class FileChannelImpl extends FileChannel {
    ...
    private FileChannelImpl(FileDescriptor fd, boolean readable, boolean writable, boolean append, Object parent) {
        this.fd = fd;
    ...
    private long transferToTrustedChannel(long position, long count, WritableByteChannel target) throws IOException {
        ...
        if (!((target instanceof FileChannelImpl) || isSelChImpl)) { return IOStatus.UNSUPPORTED; }
    ...
    private long transferFromFileChannel(FileChannelImpl src, long position, long count) throws IOException {
        if (!src.readable) { throw new NonReadableChannelException(); }
    ...
    public MappedByteBuffer map(MapMode mode, long position, long size) throws IOException {
        ...
        int imode = -1;
        if (mode == MapMode.READ_ONLY) { imode = MAP_RO; } else if (mode == MapMode.READ_WRITE) {
            imode = MAP_RW;
        } else if (mode == MapMode.PRIVATE) { imode = MAP_PV; }
        assert (imode >= 0);
        ...
        try {
            ...
            if (size == 0) {
                ...
                if ((!writable) || (imode == MAP_RO)) { return Util.newMappedByteBufferR(0, 0, dummy, null); } else {
            ...
            synchronized (FileChannelImpl.class) {
                if (!propertyChecked) {
                    String value = AccessController
                        .doPrivileged(new GetPropertyAction("sun.nio.ch.disableSystemWideOverlappingFileLockCheck"));
