...
class SocketChannelImpl extends SocketChannel implements SelChImpl {
    ...
    SocketChannelImpl(SelectorProvider sp, FileDescriptor fd, boolean bound) throws IOException {
        ...
        this.fdVal = IOUtil.fdVal(fd);
        this.state = ST_UNCONNECTED;
        if (bound) { this.localAddress = Net.localAddress(fd); }
    }
    ...
    public Socket socket() {
        synchronized (stateLock) {
            if (socket == null) { socket = SocketAdaptor.create(this); }
            return socket;
        }
    }
    ...
    public <T> SocketChannel setOption(SocketOption<T> name, T value) throws IOException {
        ...
        synchronized (stateLock) {
            ...
            if (name == StandardSocketOptions.IP_TOS) {
                if (!Net.isIPv6Available()) { Net.setSocketOption(fd, StandardProtocolFamily.INET, name, value); }
                return this;
    ...
    private boolean ensureReadOpen() throws ClosedChannelException {
        synchronized (stateLock) {
            ...
            if (!isInputOpen) { return false; } else { return true; }
        }
    }
    ...
    private void ensureWriteOpen() throws ClosedChannelException {
        synchronized (stateLock) {
            if (!isOpen()) { throw new ClosedChannelException(); }
            if (!isOutputOpen) { throw new ClosedChannelException(); }
            if (!isConnected()) { throw new NotYetConnectedException(); }
        }
    }
            ...
            synchronized (writeLock) {
                ...
                synchronized (blockingLock()) {
                    ...
                    synchronized (stateLock) {
                        ...
                        if (!isBlocking()) { state = ST_PENDING; } else { assert false; }
                    }
                }
                return false;
            }
    ...
    public boolean translateReadyOps(int ops, int initialOps, SelectionKeyImpl sk) {
        ...
        if (((ops & PollArrayWrapper.POLLCONN) != 0) && ((intOps & SelectionKey.OP_CONNECT) != 0) &&
    ...
    public String toString() {
        ...
        sb.append('[');
        if (!isOpen()) { sb.append("closed"); } else {
            synchronized (stateLock) {
                switch (state) {
                    case ST_UNCONNECTED:
