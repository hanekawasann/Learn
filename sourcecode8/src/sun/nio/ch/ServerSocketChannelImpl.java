...
class ServerSocketChannelImpl extends ServerSocketChannel implements SelChImpl {
    ...
    ServerSocketChannelImpl(SelectorProvider sp, FileDescriptor fd, boolean bound) throws IOException {
        ...
        this.fdVal = IOUtil.fdVal(fd);
        this.state = ST_INUSE;
        if (bound) { localAddress = Net.localAddress(fd); }
    }

    public ServerSocket socket() {
        synchronized (stateLock) {
            if (socket == null) { socket = ServerSocketAdaptor.create(this); }
            return socket;
        }
    }
    ...
    public SocketAddress getLocalAddress() throws IOException {
        synchronized (stateLock) {
            ...
            return localAddress == null ? localAddress
                : Net.getRevealedLocalAddress(Net.asInetSocketAddress(localAddress));
        }
    }
    ...
    public void translateAndSetInterestOps(int ops, SelectionKeyImpl sk) {
        ...
        // Translate ops
        if ((ops & SelectionKey.OP_ACCEPT) != 0) { newOps |= PollArrayWrapper.POLLIN; }
