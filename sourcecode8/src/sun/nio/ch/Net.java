...
public class Net {
    ...
    public static InetSocketAddress checkAddress(SocketAddress sa) {
        if (sa == null) { throw new NullPointerException(); }
    ...
    static InetSocketAddress asInetSocketAddress(SocketAddress sa) {
        if (!(sa instanceof InetSocketAddress)) { throw new UnsupportedAddressTypeException(); }
        return (InetSocketAddress) sa;
    }

    static void translateToSocketException(Exception x) throws SocketException {
        if (x instanceof SocketException) { throw (SocketException) x; }
        Exception nx = x;
        ...
        if (nx != x) { nx.initCause(x); }

        if (nx instanceof SocketException) { throw (SocketException) nx; } else if (nx instanceof RuntimeException) {
    ...
    static InetSocketAddress getRevealedLocalAddress(InetSocketAddress addr) {
        ...
        if (addr == null || sm == null) { return addr; }
    ...
    static void setSocketOption(FileDescriptor fd, ProtocolFamily family, SocketOption<?> name, Object value)
        throws IOException {
        if (value == null) { throw new IllegalArgumentException("Invalid option value"); }
        ...
        if (name == StandardSocketOptions.SO_RCVBUF || name == StandardSocketOptions.SO_SNDBUF) {
            ...
            if (i < 0) { throw new IllegalArgumentException("Invalid send/receive buffer size"); }
        }
        if (name == StandardSocketOptions.SO_LINGER) {
            int i = ((Integer) value).intValue();
            if (i < 0) { value = Integer.valueOf(-1); }
            if (i > 65535) { value = Integer.valueOf(65535); }
        }
        if (name == StandardSocketOptions.IP_TOS) {
            int i = ((Integer) value).intValue();
            if (i < 0 || i > 255) { throw new IllegalArgumentException("Invalid IP_TOS value"); }
        }
        if (name == StandardSocketOptions.IP_MULTICAST_TTL) {
            int i = ((Integer) value).intValue();
            if (i < 0 || i > 255) { throw new IllegalArgumentException("Invalid TTL/hop value"); }
        }

        // map option name to platform level/name
        OptionKey key = SocketOptionRegistry.findOption(name, family);
        if (key == null) { throw new AssertionError("Option not found"); }
    ...
    private static native void bind0(FileDescriptor fd, boolean preferIPv6, boolean useExclBind, InetAddress addr,
    ...
    private static native int connect0(boolean preferIPv6, FileDescriptor fd, InetAddress remote, int remotePort)
        throws IOException;
    ...
    private static native int getIntOption0(FileDescriptor fd, boolean mayNeedConversion, int level, int opt)
        throws IOException;
    ...
    private static native int blockOrUnblock4(boolean block, FileDescriptor fd, int group, int interf, int source)
        throws IOException;
