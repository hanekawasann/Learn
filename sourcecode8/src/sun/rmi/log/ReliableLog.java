...
public class ReliableLog {
     ...
     *                recovery
     * @param pad     ignored
     * @throws IOException If a directory creation error has
     ...
     *
     * @throws IOException If recovery fails due to serious log
     ...
     * @throws IOException If an exception occurred during a
     ...
     * @throws IOException If force-write to log failed or an
     ...
     * @throws IOException If an exception occurred during the
     ...
     *
     * @throws IOException If an I/O error occurs when the log is
     *                     closed
     */
     ...
     * @throws IOException If new version file couldn't be removed
     */
    ...
    private void writeVersionFile(boolean newVersion) throws IOException {
        ...
        try (FileOutputStream fos = new FileOutputStream(fName(name)); DataOutputStream out = new DataOutputStream(
            fos)) {
     ...
     * @throws IOException If serious log corruption is detected or
    ...
    private Object recoverUpdates(Object state) throws IOException {
        ...
        if (majorFormatVersion != PreferredMajorVersion) {
            if (Debug) {
                System.err
                    .println("log.debug: major version mismatch: " + majorFormatVersion + "." + minorFormatVersion);
            }
            throw new IOException(
                "Log file " + logName + " has a " + "version " + majorFormatVersion + "." + minorFormatVersion +
                    " format, and this implementation " + " understands only version " + PreferredMajorVersion + "." +
        ...
        try {
            while (in != null) {
                ...
                if (updateLen <= 0) {/* crashed while writing last log entry */
                    if (Debug) {
                        System.err.println(
                            "log.debug: last update incomplete, " + "updateLen = 0x" + Integer.toHexString(updateLen));
                    }
                    break;
                }
