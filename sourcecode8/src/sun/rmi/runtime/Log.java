...
public abstract class Log {
    ...
    static {
        boolean useOld = Boolean.valueOf(java.security.AccessController.
            doPrivileged(new sun.security.action.GetPropertyAction("sun.rmi.log.useOld"))).booleanValue();

        /* set factory to select the logging facility to use */
        logFactory = (useOld ? (LogFactory) new LogStreamLogFactory() : (LogFactory) new LoggerLogFactory());
    }
    ...
    private static class LoggerLog extends Log {
        ...
        private static final Handler alternateConsole = java.security.AccessController
            .doPrivileged(new java.security.PrivilegedAction<Handler>() {
                public Handler run() {
        ...
        public void log(Level level, String message) {
            if (isLoggable(level)) {
                ...
                logger.logp(level, source[0], source[1], Thread.currentThread().getName() + ": " + message);
            }
        }
        ...
        public void log(Level level, String message, Throwable thrown) {
            if (isLoggable(level)) {
                String[] source = getSource();
                logger.logp(level, source[0], source[1], Thread.currentThread().getName() + ": " + message, thrown);
            }
        }
        ...
        public void write(int b) {
            if ((last == '\r') && (b == '\n')) {
                ...
                try {
                    ...
                    String message = Thread.currentThread().getName() + ": " + bufOut.toString();
