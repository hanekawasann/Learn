...
public class TCPEndpoint implements Endpoint {
    ...
    public static TCPEndpoint getLocalEndpoint(int port, RMIClientSocketFactory csf, RMIServerSocketFactory ssf) {
        ...
        synchronized (localEndpoints) {
            ...
            if (epList == null) {
                ...
                if (TCPTransport.tcpLog.isLoggable(Log.BRIEF)) {
                    TCPTransport.tcpLog
                        .log(Log.BRIEF, "created local endpoint for socket factory " + ssf + " on port " + port);
                }
            } else {
                synchronized (epList) {
                    ep = epList.getLast();
        ...
        synchronized (localEndpoints) {
            ...
            if (TCPTransport.tcpLog.isLoggable(Log.BRIEF)) {
                TCPTransport.tcpLog.log(Log.BRIEF,
                    "default port for server socket factory " + ssf + " and client socket factory " + csf + " set to " +
                        port);
            }
        }
    }
    ...
    public boolean equals(Object obj) {
        if ((obj != null) && (obj instanceof TCPEndpoint)) {
            ...
            if (port != ep.port || !host.equals(ep.host)) { return false; }
            if (((csf == null) ^ (ep.csf == null)) || ((ssf == null) ^ (ep.ssf == null))) { return false; }
            /*
             ...
             */
            if ((csf != null) && !(csf.getClass() == ep.csf.getClass() && csf.equals(ep.csf))) { return false; }
            if ((ssf != null) && !(ssf.getClass() == ep.ssf.getClass() && ssf.equals(ep.ssf))) { return false; }
            return true;
        } else {
            return false;
        }
     ...
     * @throws IOException If id could not be read (due to stream failure)
     */
    ...
    public void writeHostPortFormat(DataOutput out) throws IOException {
        if (csf != null) {
            throw new InternalError(
                "TCPEndpoint.writeHostPortFormat: " + "called for endpoint with non-null socket factory");
        }
        out.writeUTF(host);
        out.writeInt(port);
    }
    ...
    private static class FQDN implements Runnable {
        ...
        static String attemptFQDN(InetAddress localAddr) throws java.net.UnknownHostException {
            ...
            if (hostName.indexOf('.') < 0) {
                ...
                if ((hostName == null) || (hostName.equals("")) || (hostName.indexOf('.') < 0)) {
        ...
        private void getFQDN() {
            ...
            Thread t = AccessController.doPrivileged(new NewThreadAction(FQDN.this, "FQDN Finder", true));
            t.start();
        }
