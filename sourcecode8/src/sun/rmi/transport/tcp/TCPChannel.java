...
public class TCPChannel implements Channel {
    ...
    private static final long idleTimeout =             // default 15 seconds
        AccessController.doPrivileged(new GetLongAction("sun.rmi.transport.connectionTimeout", 15000));

    /** client-side connection handshake read timeout */
    private static final int handshakeTimeout =         // default 1 minute
        AccessController.doPrivileged(new GetIntegerAction("sun.rmi.transport.tcp.handshakeTimeout", 60000));

    /** client-side connection response read timeout (after handshake) */
    private static final int responseTimeout =          // default infinity
        AccessController.doPrivileged(new GetIntegerAction("sun.rmi.transport.tcp.responseTimeout", 0));
     ...
     *
     * @throws SecurityException if caller is not allowed to use this
     *                           Channel.
     */
    ...
    private Connection createConnection() throws RemoteException {
        ...
        if (!usingMultiplexer) {
            ...
            try {
                ...
                if (!conn.isReusable()) {
                    ...
                    if (ack != TransportConstants.ProtocolAck) {
                        throw new ConnectIOException(
                            ack == TransportConstants.ProtocolNack ? "JRMP StreamProtocol not supported by server"
                ...
                if (e instanceof RemoteException) { throw (RemoteException) e; } else {
...
class ConnectionAcceptor implements Runnable {
    ...
    public void startNewAcceptor() {
        Thread t = AccessController
            .doPrivileged(new NewThreadAction(ConnectionAcceptor.this, "Multiplex Accept-" + ++threadNum, true));
        t.start();
    }
