...
class HttpInputStream extends FilterInputStream {
    ...
    public int available() throws IOException {
        ...
        if (bytesAvailable > bytesLeft) { bytesAvailable = bytesLeft; }
        ...
        if (bytesLeft > 0) {
            ...
            if (RMIMasterSocketFactory.proxyLog.isLoggable(Log.VERBOSE)) {
                RMIMasterSocketFactory.proxyLog.log(Log.VERBOSE,
                    "received byte: '" + ((data & 0x7F) < ' ' ? " " : String.valueOf((char) data)) + "' " + data);
            }
    ...
    public int read(byte b[], int off, int len) throws IOException {
        ...
        if (RMIMasterSocketFactory.proxyLog.isLoggable(Log.VERBOSE)) {
            RMIMasterSocketFactory.proxyLog
                .log(Log.VERBOSE, "read " + bytesRead + " bytes, " + bytesLeft + " remaining");
        }
    ...
    public void mark(int readlimit) {
        in.mark(readlimit);
        if (in.markSupported()) { bytesLeftAtMark = bytesLeft; }
    }
