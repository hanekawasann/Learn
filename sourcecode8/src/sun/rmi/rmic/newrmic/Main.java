...
public class Main {
    ...
    public static boolean start(RootDoc rootDoc) {
        ...
        try {
            Constructor<? extends BatchEnvironment> cons = batch.envClass
                .getConstructor(new Class<?>[] { RootDoc.class });
    ...
    private boolean invokeJavadoc(Batch batch, long batchID) {
        ...
        int status = com.sun.tools.javadoc.Main
            .execute(program, new PrintWriter(out, true), new PrintWriter(out, true), new PrintWriter(out, true),
    ...
    private boolean invokeJavac(Batch batch, List<File> files) {
        ...
        int status = com.sun.tools.javac.Main
            .compile(javacArgs.toArray(new String[javacArgs.size()]), new PrintWriter(out, true));
        return status == 0;
    }
