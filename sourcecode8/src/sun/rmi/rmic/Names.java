...
public class Names {
    ...
    static final public Identifier mangleClass(Identifier className) {
         ...
         * each '.' with '$'.
         */
        Identifier mangled = Identifier
            .lookup(className.getFlatName().toString().replace('.', sun.tools.java.Constants.SIGC_INNERCLASS));
        if (mangled.isInner()) { throw new Error("failed to mangle inner class name"); }
