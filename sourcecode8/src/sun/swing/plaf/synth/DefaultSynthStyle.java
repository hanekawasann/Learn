...
public class DefaultSynthStyle extends SynthStyle implements Cloneable {
    ...
    public DefaultSynthStyle(DefaultSynthStyle style) {
        ...
        if (style.insets != null) {
            insets = new Insets(style.insets.top, style.insets.left, style.insets.bottom, style.insets.right);
        }
        if (style.states != null) {
            states = new StateInfo[style.states.length];
            for (int counter = style.states.length - 1; counter >= 0; counter--) {
    ...
    public Color getColor(SynthContext context, ColorType type) {
        return getColor(context.getComponent(), context.getRegion(), context.getComponentState(), type);
    }
    ...
    protected Color getColorForState(SynthContext context, ColorType type) {
        return getColorForState(context.getComponent(), context.getRegion(), context.getComponentState(), type);
    }
    ...
    public Font getFont(SynthContext state) {
        return getFont(state.getComponent(), state.getRegion(), state.getComponentState());
    }
    ...
    protected Font getFontForState(SynthContext context) {
        return getFontForState(context.getComponent(), context.getRegion(), context.getComponentState());
    }
    ...
    public Object get(SynthContext state, Object key) {
        ...
        if (getKeyFromData(data, key) != null) { return getKeyFromData(data, key); }
    ...
    public DefaultSynthStyle addTo(DefaultSynthStyle style) {
        ...
        if (states != null) {
            if (style.states == null) {
                ...
                for (int thisCounter = states.length - 1; thisCounter >= 0; thisCounter--) {
                    int state = states[thisCounter].getComponentState();
                    boolean found = false;

                    for (int oCounter = maxOStyles - 1 - matchCount; oCounter >= 0; oCounter--) {
                ...
                if (unique != 0) {
                    ...
                    for (int thisCounter = states.length - 1; thisCounter >= 0; thisCounter--) {
                        ...
                        for (int oCounter = maxOStyles - 1; oCounter >= 0; oCounter--) {
                            if (state == style.states[oCounter].
                                getComponentState()) {
                                found = true;
                                break;
                        ...
                        if (!found) {
                            newStates[newIndex++] = (StateInfo) states[thisCounter].clone();
                        }
                    }
        ...
        public StateInfo addTo(StateInfo info) {
            ...
            if (colors != null) {
                if (info.colors == null) {
                    ...
                    System.arraycopy(colors, 0, info.colors, 0, colors.length);
                } else {
