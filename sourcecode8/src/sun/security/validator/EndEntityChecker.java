...
class EndEntityChecker {
    ...
    private final static String OID_EXTENDED_KEY_USAGE = SimpleValidator.OID_EXTENDED_KEY_USAGE;
    ...
    private final static String NSCT_CODE_SIGNING = NetscapeCertTypeExtension.OBJECT_SIGNING;
    ...
    private final static Collection<String> KU_SERVER_SIGNATURE = Arrays
        .asList("DHE_DSS", "DHE_RSA", "ECDHE_ECDSA", "ECDHE_RSA", "RSA_EXPORT", "UNKNOWN");

    // TLS key exchange algorithms requiring keyEncipherment key usage
    private final static Collection<String> KU_SERVER_ENCRYPTION = Arrays.asList("RSA");

    // TLS key exchange algorithms requiring keyAgreement key usage
    private final static Collection<String> KU_SERVER_KEY_AGREEMENT = Arrays
        .asList("DH_DSS", "DH_RSA", "ECDH_ECDSA", "ECDH_RSA");
    ...
    private void checkTLSClient(X509Certificate cert) throws CertificateException {
        ...
        if (checkKeyUsage(cert, KU_SIGNATURE) == false) {
            throw new ValidatorException("KeyUsage does not allow digital signatures",
                ValidatorException.T_EE_EXTENSIONS, cert);
        }

        if (checkEKU(cert, exts, OID_EKU_TLS_CLIENT) == false) {
            throw new ValidatorException("Extended key usage does not " + "permit use for TLS client authentication",
                ValidatorException.T_EE_EXTENSIONS, cert);
        }

        if (!SimpleValidator.getNetscapeCertTypeBit(cert, NSCT_SSL_CLIENT)) {
            throw new ValidatorException("Netscape cert type does not permit use for SSL client",
                ValidatorException.T_EE_EXTENSIONS, cert);
        }
    ...
    private void checkTLSServer(X509Certificate cert, String parameter) throws CertificateException {
        ...
        if (KU_SERVER_ENCRYPTION.contains(parameter)) {
            if (checkKeyUsage(cert, KU_KEY_ENCIPHERMENT) == false) {
                throw new ValidatorException("KeyUsage does not allow key encipherment",
                    ValidatorException.T_EE_EXTENSIONS, cert);
            }
            ...
            if (checkKeyUsage(cert, KU_KEY_AGREEMENT) == false) {
                throw new ValidatorException("KeyUsage does not allow key agreement",
                    ValidatorException.T_EE_EXTENSIONS, cert);
            }
        } else {
        ...
        if (checkEKU(cert, exts, OID_EKU_TLS_SERVER) == false) {
            ...
            if ((checkEKU(cert, exts, OID_EKU_MS_SGC) == false) && (checkEKU(cert, exts, OID_EKU_NS_SGC) == false)) {
        ...
        if (!SimpleValidator.getNetscapeCertTypeBit(cert, NSCT_SSL_SERVER)) {
            throw new ValidatorException("Netscape cert type does not permit use for SSL server",
                ValidatorException.T_EE_EXTENSIONS, cert);
        }
    ...
    private void checkCodeSigning(X509Certificate cert) throws CertificateException {
        ...
        if (checkEKU(cert, exts, OID_EKU_CODE_SIGNING) == false) {
            throw new ValidatorException("Extended key usage does not permit use for code signing",
                ValidatorException.T_EE_EXTENSIONS, cert);
        }
        ...
        if (variant.equals(Validator.VAR_JCE_SIGNING) == false) {
            if (!SimpleValidator.getNetscapeCertTypeBit(cert, NSCT_CODE_SIGNING)) {
                throw new ValidatorException("Netscape cert type does not permit use for code signing",
                    ValidatorException.T_EE_EXTENSIONS, cert);
            }
            exts.remove(SimpleValidator.OID_NETSCAPE_CERT_TYPE);
        }
    ...
    private void checkTSAServer(X509Certificate cert) throws CertificateException {
        ...
        if (cert.getExtendedKeyUsage() == null) {
            throw new ValidatorException(
                "Certificate does not contain an extended key usage " + "extension required for a TSA server",
                ValidatorException.T_EE_EXTENSIONS, cert);
        }

        if (checkEKU(cert, exts, OID_EKU_TIME_STAMPING) == false) {
            throw new ValidatorException("Extended key usage does not permit use for TSA server",
                ValidatorException.T_EE_EXTENSIONS, cert);
        }
