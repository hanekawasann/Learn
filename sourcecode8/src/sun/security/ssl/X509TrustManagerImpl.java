...
final class X509TrustManagerImpl extends X509ExtendedTrustManager implements X509TrustManager {
    ...
    private void checkTrusted(X509Certificate[] chain, String authType, SSLEngine engine, boolean isClient)
        ...
        if (engine != null) {
            ...
            String identityAlg = engine.getSSLParameters().
                getEndpointIdentificationAlgorithm();
            if (identityAlg != null && identityAlg.length() != 0) {
                checkIdentity(session, chain[0], identityAlg, isClient, getRequestedServerNames(engine));
            }
    ...
    private void showTrustedCerts() {
        if (debug != null && Debug.isOn("trustmanager")) {
            for (X509Certificate cert : trustedCerts) {
                ...
                System.out.println("  Algorithm: " + cert.getPublicKey().getAlgorithm() + "; Serial number: 0x" +
    ...
    private static X509Certificate[] validate(Validator v, X509Certificate[] chain, AlgorithmConstraints constraints,
     ...
     */
    private static void checkIdentity(SSLSession session, X509Certificate cert, String algorithm, boolean isClient,
