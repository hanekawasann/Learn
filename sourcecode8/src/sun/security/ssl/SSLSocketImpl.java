...
final public class SSLSocketImpl extends BaseSSLSocketImpl {
     ...
     */
    SSLSocketImpl(SSLContextImpl context, String host, int port, InetAddress localAddr, int localPort)
     ...
     * @param localPort the local port the socket is bound to
     */
    SSLSocketImpl(SSLContextImpl context, InetAddress host, int port, InetAddress localAddr, int localPort)
        throws IOException {
        super();
        init(context, false);
        bind(new InetSocketAddress(localAddr, localPort));
     ...
     */
    SSLSocketImpl(SSLContextImpl context, boolean serverMode, CipherSuiteList suites, byte clientAuth,
        boolean sessionCreation, ProtocolList protocols, String identificationProtocol,
        AlgorithmConstraints algorithmConstraints, Collection<SNIMatcher> sniMatchers, boolean preferLocalCipherSuites)
        throws IOException {
                ...
                try {
                    if (writeLock.tryLock(getSoLinger(), TimeUnit.SECONDS)) {
                        ...
                        SSLException ssle = new SSLException(
                            "SO_LINGER timeout," + " close_notify message cannot be sent.");
    ...
    private void writeRecordInternal(OutputRecord r, boolean holdRecord) throws IOException {
        ...
        if (isFirstAppOutputRecord && r.contentType() == Record.ct_application_data) {
    ...
    boolean needToSplitPayload() {
        writeLock.lock();
        try {
            return (protocolVersion.v <= ProtocolVersion.TLS10.v) && writeCipher.isCBCMode() &&
                !isFirstAppOutputRecord && Record.enableCBCProtection;
        } finally {
            writeLock.unlock();
        }
    }
            ...
            while (((state = getConnectionState()) != cs_CLOSED) && (state != cs_ERROR) && (state != cs_APP_CLOSED)) {
                ...
                try {
                    ...
                    if ((debug != null) && Debug.isOn("ssl")) {
                        System.out.println(Thread.currentThread().getName() + ", received EOFException: " +
                ...
                try {
                    ...
                    byte alertType = (r.contentType() == Record.ct_handshake) ? Alerts.alert_handshake_failure
                ...
                synchronized (this) {
                    switch (r.contentType()) {
                        ...
                        case Record.ct_application_data:
                            ...
                            if (connectionState != cs_DATA && connectionState != cs_RENEGOTIATE &&
                        ...
                        case Record.ct_change_cipher_spec:
                            if ((connectionState != cs_HANDSHAKE && connectionState != cs_RENEGOTIATE) ||
                                r.available() != 1 || r.read() != 1) {
                                fatal(Alerts.alert_unexpected_message,
                                    "illegal change cipher spec msg, state = " + connectionState);
                            }
                        ...
                        default:
                            ...
                            if (debug != null && Debug.isOn("ssl")) {
                                System.out.println(
                                    Thread.currentThread().getName() + ", Received record type: " + r.contentType());
                            }
                            continue;
                    } // switch
    ...
    private void checkSequenceNumber(Authenticator authenticator, byte type) throws IOException {
        ...
        if (authenticator.seqNumOverflow()) {
             ...
             */
            if (debug != null && Debug.isOn("ssl")) {
                System.out.println(Thread.currentThread().getName() + ", sequence number extremely close to overflow " +
        ...
        if ((type != Record.ct_handshake) && authenticator.seqNumIsHuge()) {
            if (debug != null && Debug.isOn("ssl")) {
                System.out.println(Thread.currentThread().getName() + ", request renegotiation " +
    ...
    private void initHandshaker() {
        ...
        if (roleIsServer) {
            handshaker = new ServerHandshaker(this, sslContext, enabledProtocols, doClientAuth, protocolVersion,
                connectionState == cs_HANDSHAKE, secureRenegotiation, clientVerifyData, serverVerifyData);
            handshaker.setSNIMatchers(sniMatchers);
            handshaker.setUseCipherSuitesOrder(preferLocalCipherSuites);
        } else {
            handshaker = new ClientHandshaker(this, sslContext, enabledProtocols, protocolVersion,
                connectionState == cs_HANDSHAKE, secureRenegotiation, clientVerifyData, serverVerifyData);
    ...
    private void closeInternal(boolean selfInitiated) throws IOException {
        ...
        try {
            switch (state) {
                ...
                default:
                    synchronized (this) {
                        if (((state = getConnectionState()) == cs_CLOSED) || (state == cs_ERROR) ||
                            (state == cs_APP_CLOSED)) {
    ...
    void waitForClose(boolean rethrow) throws IOException {
        if (debug != null && Debug.isOn("ssl")) {
            System.out.println(Thread.currentThread().getName() + ", waiting for close_notify or alert: state " +
                getConnectionState());
        }
    ...
    synchronized public void setUseClientMode(boolean flag) {
        switch (connectionState) {
            ...
            default:
                if (debug != null && Debug.isOn("ssl")) {
                    System.out.println(Thread.currentThread().getName() + ", setUseClientMode() invoked in state = " +
                        connectionState);
                }
    ...
    private static class NotifyHandshakeThread extends Thread {
        ...
        private Set<Map.Entry<HandshakeCompletedListener, AccessControlContext>> targets;        // who gets notified
        private HandshakeCompletedEvent event;          // the notification

        NotifyHandshakeThread(Set<Map.Entry<HandshakeCompletedListener, AccessControlContext>> entrySet,
