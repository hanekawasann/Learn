...
public abstract class SSLContextImpl extends SSLContextSpi {
        ...
        if (sr == null) {
            ...
            if (SunJSSE.isFIPS() && (sr.getProvider() != SunJSSE.cryptoProvider)) {
                throw new KeyManagementException(
                    "FIPS mode: SecureRandom must be from provider " + SunJSSE.cryptoProvider.getName());
            }
            secureRandom = sr;
        }
    ...
    private X509ExtendedKeyManager chooseKeyManager(KeyManager[] kms) throws KeyManagementException {
        for (int i = 0; kms != null && i < kms.length; i++) {
            ...
            if (debug != null && Debug.isOn("sslctx")) {
                System.out.println("X509KeyManager passed to " + "SSLContext.init():  need an " +
    ...
    CipherSuiteList getSupportedCipherSuiteList() {
        ...
        synchronized (this) {
            ...
            if (supportedCipherSuiteList == null) {
                supportedCipherSuiteList = getApplicableCipherSuiteList(getSuportedProtocolList(), false);
            }
        ...
        synchronized (this) {
            ...
            if (roleIsServer) {
                if (defaultServerCipherSuiteList == null) {
                    defaultServerCipherSuiteList = getApplicableCipherSuiteList(getDefaultProtocolList(true), true);
                }

                return defaultServerCipherSuiteList;
            } else {
                if (defaultClientCipherSuiteList == null) {
                    defaultClientCipherSuiteList = getApplicableCipherSuiteList(getDefaultProtocolList(false), true);
                }
    ...
    boolean isDefaultProtocolList(ProtocolList protocols) {
        return (protocols == defaultServerProtocolList) || (protocols == defaultClientProtocolList);
    }
        ...
        if (!(protocols.collection().isEmpty()) && protocols.min.v != ProtocolVersion.NONE.v) {
            for (CipherSuite suite : allowedCipherSuites) {
                ...
                if (suite.isAvailable() && suite.obsoleted > protocols.min.v && suite.supported <= protocols.max.v) {
                    if (defaultAlgorithmConstraints
                        .permits(EnumSet.of(CryptoPrimitive.KEY_AGREEMENT), suite.name, null)) {
                        suites.add(suite);
                    }
    ...
    private abstract static class AbstractSSLContext extends SSLContextImpl {
        ...
        static {
            ...
            if (SunJSSE.isFIPS()) {
                supportedSSLParams.setProtocols(new String[] { ProtocolVersion.TLS10.name, ProtocolVersion.TLS11.name,
                    ProtocolVersion.TLS12.name });

                defaultServerSSLParams = supportedSSLParams;
            } else {
                supportedSSLParams.setProtocols(
                    new String[] { ProtocolVersion.SSL20Hello.name, ProtocolVersion.SSL30.name,
                        ProtocolVersion.TLS10.name, ProtocolVersion.TLS11.name, ProtocolVersion.TLS12.name });
        ...
        static {
            ...
            if (SunJSSE.isFIPS()) {
                ...
                defaultClientSSLParams
                    .setProtocols(new String[] { ProtocolVersion.SSL30.name, ProtocolVersion.TLS10.name });
            }
        }
    ...
    public static final class TLS11Context extends AbstractSSLContext {
        ...
        static {
            ...
            if (SunJSSE.isFIPS()) {
                defaultClientSSLParams
                    .setProtocols(new String[] { ProtocolVersion.TLS10.name, ProtocolVersion.TLS11.name });

            } else {
                defaultClientSSLParams.setProtocols(
                    new String[] { ProtocolVersion.SSL30.name, ProtocolVersion.TLS10.name,
    ...
    public static final class TLS12Context extends AbstractSSLContext {
        ...
        static {
            ...
            if (SunJSSE.isFIPS()) {
                defaultClientSSLParams.setProtocols(
                    new String[] { ProtocolVersion.TLS10.name, ProtocolVersion.TLS11.name,
                        ProtocolVersion.TLS12.name });

            } else {
                defaultClientSSLParams.setProtocols(
                    new String[] { ProtocolVersion.SSL30.name, ProtocolVersion.TLS10.name, ProtocolVersion.TLS11.name,
                        ProtocolVersion.TLS12.name });
            }
        }
        ...
        static {
            ...
            if (property == null || property.length() == 0) {
                ...
                if (SunJSSE.isFIPS()) {
                    ...
                    defaultClientSSLParams.setProtocols(
                        new String[] { ProtocolVersion.SSL30.name, ProtocolVersion.TLS10.name,
                            ProtocolVersion.TLS11.name, ProtocolVersion.TLS12.name });
                }
            } else {
                ...
                for (int i = 0; i < protocols.length; i++) {
                    ...
                    try {
                        ...
                        reservedException = new IllegalArgumentException(
                            PROPERTY_NAME + ": " + protocols[i] + " is not a standard SSL protocol name", iae);
                    }
                }
                ...
                if ((reservedException == null) && SunJSSE.isFIPS()) {
                    for (String protocol : protocols) {
                        if (ProtocolVersion.SSL20Hello.name.equals(protocol) ||
                            ...
                            reservedException = new IllegalArgumentException(
                                PROPERTY_NAME + ": " + protocol + " is not FIPS compliant");
                        }
                    }
                }
        ...
        private static synchronized KeyManager[] getDefaultKeyManager() throws Exception {
            ...
            if (P11KEYSTORE.equals(defaultKeyStoreType) && !NONE.equals(defaultKeyStore)) {
                throw new IllegalArgumentException(
                    "if keyStoreType is " + P11KEYSTORE + ", then keyStore must be " + NONE);
            }
...
final class AbstractTrustManagerWrapper extends X509ExtendedTrustManager implements X509TrustManager {
    ...
    private void checkAdditionalTrust(X509Certificate[] chain, String authType, Socket socket, boolean isClient)
        throws CertificateException {
    ...
    private void checkAdditionalTrust(X509Certificate[] chain, String authType, SSLEngine engine, boolean isClient)
        throws CertificateException {
        if (engine != null) {
    ...
    private void checkAlgorithmConstraints(X509Certificate[] chain, AlgorithmConstraints constraints)
        throws CertificateException {
