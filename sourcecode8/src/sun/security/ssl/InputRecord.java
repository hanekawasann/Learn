...
class InputRecord extends ByteArrayInputStream implements Record {
    ...
    void decrypt(Authenticator authenticator, CipherBox box) throws BadPaddingException {
        ...
        if (!box.isNullCipher()) {
            try {
                ...
                int nonceSize = box.applyExplicitNonce(authenticator, contentType(), buf, headerSize, cipheredLength);
     ...
     */
    static boolean checkMacTags(byte contentType, byte[] buffer, int offset, int contentLen, MAC signer,
        boolean isSimulated) {
    ...
    static int calculateRemainingLen(MAC signer, int fullLen, int usedLen) {
        ...
        return 0x01 + (int) (Math.ceil(fullLen / (1.0d * blockLen)) - Math.ceil(usedLen / (1.0d * blockLen))) *
            signer.hashBlockLen();
    }
    ...
    private void readV3Record(InputStream s, OutputStream o) throws IOException {
        ...
        if (contentLen < 0 || contentLen > maxLargeRecordSize - headerSize) {
            throw new SSLProtocolException(
                "Bad InputRecord size" + ", count = " + contentLen + ", buf.length = " + buf.length);
        }
        ...
        if (debug != null && Debug.isOn("record")) {
            if (count < 0 || count > (maxRecordSize - headerSize)) {
                System.out.println(Thread.currentThread().getName() + ", Bad InputRecord size" + ", count = " + count);
            }
            System.out.println(
                Thread.currentThread().getName() + ", READ: " + recordVersion + " " + contentName(contentType()) +
                    ", length = " + available());
        }
        /*
    ...
    private void handleUnknownRecord(InputStream s, OutputStream o) throws IOException {
        ...
        if (((buf[0] & 0x080) != 0) && buf[2] == 1) {
            ...
            if (debug != null && Debug.isOn("record")) {
                System.out.println(
                    Thread.currentThread().getName() + ", READ:  SSL v2, contentType = " + contentName(contentType()) +
    ...
    private void V2toV3ClientHello(byte v2Msg[]) throws SSLException {
        ...
        if (nonceLen < 32) {
            for (i = 0; i < (32 - nonceLen); i++) { buf[count++] = 0; }
