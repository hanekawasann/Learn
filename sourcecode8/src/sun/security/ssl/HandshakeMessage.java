...
public abstract class HandshakeMessage {
    ...
    static final class ClientHello extends HandshakeMessage {
        ...
        ClientHello(SecureRandom generator, ProtocolVersion protocolVersion, SessionId sessionId,
        ...
        int messageLength() {
             ...
             */
            return (2 + 32 + 1 + 2 + 1 + sessionId.length()                /* ... + variable parts */ +
                (cipherSuites.size() * 2) + compression_methods.length) + extensions.length();
        }
    ...
    static final class RSA_ServerKeyExchange extends ServerKeyExchange {
         ...
         */
        RSA_ServerKeyExchange(PublicKey ephemeralKey, PrivateKey privateKey, RandomCookie clntNonce,
            RandomCookie svrNonce, SecureRandom sr) throws GeneralSecurityException {
        ...
        PublicKey getPublicKey() {
            try {
                ...
                RSAPublicKeySpec kspec = new RSAPublicKeySpec(new BigInteger(1, rsa_modulus),
         ...
         */
        boolean verify(PublicKey certifiedKey, RandomCookie clntNonce, RandomCookie svrNonce)
    ...
    static final class DH_ServerKeyExchange extends ServerKeyExchange {
        ...
        private final static boolean dhKeyExchangeFix = Debug
            .getBooleanProperty("com.sun.net.ssl.dhKeyExchangeFix", true);
         ...
         */
        DH_ServerKeyExchange(DHCrypt obj, PrivateKey key, byte clntNonce[], byte svrNonce[], SecureRandom sr,
            SignatureAndHashAlgorithm signAlgorithm, ProtocolVersion protocolVersion) throws GeneralSecurityException {
         ...
         */
        DH_ServerKeyExchange(HandshakeInStream input, ProtocolVersion protocolVersion)
            ...
            KeyUtil.validate(
                new DHPublicKeySpec(new BigInteger(1, dh_Ys), new BigInteger(1, dh_p), new BigInteger(1, dh_g)));
         ...
         */
        DH_ServerKeyExchange(HandshakeInStream input, PublicKey publicKey, byte clntNonce[], byte svrNonce[],
            int messageSize, Collection<SignatureAndHashAlgorithm> localSupportedSignAlgs,
            ProtocolVersion protocolVersion) throws IOException, GeneralSecurityException {
    ...
    static final class ECDH_ServerKeyExchange extends ServerKeyExchange {
        ...
        ECDH_ServerKeyExchange(ECDHCrypt obj, PrivateKey privateKey, byte[] clntNonce, byte[] svrNonce, SecureRandom sr,
            SignatureAndHashAlgorithm signAlgorithm, ProtocolVersion protocolVersion) throws GeneralSecurityException {
         ...
         */
        ECDH_ServerKeyExchange(HandshakeInStream input, PublicKey signingKey, byte[] clntNonce, byte[] svrNonce,
            Collection<SignatureAndHashAlgorithm> localSupportedSignAlgs, ProtocolVersion protocolVersion)
            throws IOException, GeneralSecurityException {
    ...
    static final class CertificateRequest extends HandshakeMessage {
        ...
        CertificateRequest(X509Certificate ca[], KeyExchange keyExchange,
            Collection<SignatureAndHashAlgorithm> signAlgs, ProtocolVersion protocolVersion) throws IOException {
        ...
        CertificateRequest(HandshakeInStream input, ProtocolVersion protocolVersion) throws IOException {
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS12.v) {
                ...
                while (remains > 1) {    // needs at least two bytes
                    ...
                    SignatureAndHashAlgorithm algorithm = SignatureAndHashAlgorithm
                        .valueOf(hash, signature, ++sequence);
    ...
    static final class CertificateVerify extends HandshakeMessage {
         ...
         */
        CertificateVerify(ProtocolVersion protocolVersion, HandshakeHash handshakeHash, PrivateKey privateKey,
            SecretKey masterSecret, SecureRandom sr, SignatureAndHashAlgorithm signAlgorithm)
            throws GeneralSecurityException {
        ...
        //
        CertificateVerify(HandshakeInStream input, Collection<SignatureAndHashAlgorithm> localSupportedSignAlgs,
         ...
         */
        boolean verify(ProtocolVersion protocolVersion, HandshakeHash handshakeHash, PublicKey publicKey,
         ...
         */
        private static Signature getSignature(ProtocolVersion protocolVersion, String algorithm)
            throws GeneralSecurityException {
            switch (algorithm) {
                case "RSA":
         ...
         */
        private static void updateSignature(Signature sig, ProtocolVersion protocolVersion, HandshakeHash handshakeHash,
            String algorithm, SecretKey masterKey) throws SignatureException {
    ...
    static final class Finished extends HandshakeMessage {
        ...
        private byte[] getFinished(HandshakeHash handshakeHash, int sender, SecretKey masterKey) {
            ...
            if (protocolVersion.v >= ProtocolVersion.TLS10.v) {
                ...
                try {
                     ...
                     */
                    TlsPrfParameterSpec spec = new TlsPrfParameterSpec(masterKey, tlsLabel, seed, 12, prfHashAlg,
         ...
         */
        private static void updateDigest(MessageDigest md, byte[] sender, byte[] pad1, byte[] pad2,
