...
final class CipherSuite implements Comparable<CipherSuite> {
     ...
     */
    private CipherSuite(String name, int id, int priority, KeyExchange keyExchange, BulkCipher cipher, boolean allowed,
        int obsoleted, int supported, PRF prfAlg) {
        this.name = name;
        this.id = id;
     ...
     *
     * @throws IllegalArgumentException if the CipherSuite is unknown or
     *                                  unsupported.
     */
     ...
     */
    private static void add(String name, int id, int priority, KeyExchange keyExchange, BulkCipher cipher,
    ...
    final static class BulkCipher {
        ...
        private final static Map<BulkCipher, Boolean> availableCache = new HashMap<>(8);
        ...
        BulkCipher(String transformation, CipherType cipherType, int keySize, int expandedKeySize, int ivSize,
        ...
        BulkCipher(String transformation, CipherType cipherType, int keySize, int ivSize, int fixedIvSize,
            boolean allowed) {
            this.transformation = transformation;
            String[] splits = transformation.split("/");
            this.algorithm = splits[0];
         ...
         */
        CipherBox newCipher(ProtocolVersion version, SecretKey key, IvParameterSpec iv, SecureRandom random,
        ...
        boolean isAvailable() {
            ...
            if ((this == B_AES_256) || (this.cipherType == CipherType.AEAD_CIPHER)) {
            ...
            if (b == null) {
                ...
                if (b == null) {
                    ...
                    try {
                        ...
                        IvParameterSpec iv;
                        if (cipher.cipherType == CipherType.AEAD_CIPHER) {
                            iv = new IvParameterSpec(new byte[cipher.fixedIvSize]);
                        } else {
                            iv = new IvParameterSpec(new byte[cipher.ivSize]);
                        }
                        temporary = cipher.newCipher(ProtocolVersion.DEFAULT, key, iv, secureRandom, true);
    ...
    final static BulkCipher B_NULL = new BulkCipher("NULL", STREAM_CIPHER, 0, 0, 0, 0, true);
    final static BulkCipher B_RC4_40 = new BulkCipher(CIPHER_RC4, STREAM_CIPHER, 5, 16, 0, 0, true);
    final static BulkCipher B_RC2_40 = new BulkCipher("RC2", BLOCK_CIPHER, 5, 16, 8, 0, false);
    final static BulkCipher B_DES_40 = new BulkCipher(CIPHER_DES, BLOCK_CIPHER, 5, 8, 8, 0, true);

    // domestic strength ciphers
    final static BulkCipher B_RC4_128 = new BulkCipher(CIPHER_RC4, STREAM_CIPHER, 16, 0, 0, true);
    final static BulkCipher B_DES = new BulkCipher(CIPHER_DES, BLOCK_CIPHER, 8, 8, 0, true);
    final static BulkCipher B_3DES = new BulkCipher(CIPHER_3DES, BLOCK_CIPHER, 24, 8, 0, true);
    final static BulkCipher B_IDEA = new BulkCipher("IDEA", BLOCK_CIPHER, 16, 8, 0, false);
    final static BulkCipher B_AES_128 = new BulkCipher(CIPHER_AES, BLOCK_CIPHER, 16, 16, 0, true);
    final static BulkCipher B_AES_256 = new BulkCipher(CIPHER_AES, BLOCK_CIPHER, 32, 16, 0, true);
    final static BulkCipher B_AES_128_GCM = new BulkCipher(CIPHER_AES_GCM, AEAD_CIPHER, 16, 12, 4, true);
    final static BulkCipher B_AES_256_GCM = new BulkCipher(CIPHER_AES_GCM, AEAD_CIPHER, 32, 12, 4, true);
    ...
    static {
        ...
        // of RFC 6460.
        add("TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384", 0xc02c, --p, K_ECDHE_ECDSA, B_AES_256_GCM, T, max, tls12,
            P_SHA384);
        add("TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256", 0xc02b, --p, K_ECDHE_ECDSA, B_AES_128_GCM, T, max, tls12,
            P_SHA256);

        // AES_256(GCM)
        add("TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384", 0xc030, --p, K_ECDHE_RSA, B_AES_256_GCM, T, max, tls12, P_SHA384);
        add("TLS_RSA_WITH_AES_256_GCM_SHA384", 0x009d, --p, K_RSA, B_AES_256_GCM, T, max, tls12, P_SHA384);
        add("TLS_ECDH_ECDSA_WITH_AES_256_GCM_SHA384", 0xc02e, --p, K_ECDH_ECDSA, B_AES_256_GCM, T, max, tls12,
            P_SHA384);
        ...
        // AES_128(GCM)
        add("TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256", 0xc02f, --p, K_ECDHE_RSA, B_AES_128_GCM, T, max, tls12, P_SHA256);
        add("TLS_RSA_WITH_AES_128_GCM_SHA256", 0x009c, --p, K_RSA, B_AES_128_GCM, T, max, tls12, P_SHA256);
        add("TLS_ECDH_ECDSA_WITH_AES_128_GCM_SHA256", 0xc02d, --p, K_ECDH_ECDSA, B_AES_128_GCM, T, max, tls12,
            P_SHA256);
        add("TLS_ECDH_RSA_WITH_AES_128_GCM_SHA256", 0xc031, --p, K_ECDH_RSA, B_AES_128_GCM, T, max, tls12, P_SHA256);
        add("TLS_DHE_RSA_WITH_AES_128_GCM_SHA256", 0x009e, --p, K_DHE_RSA, B_AES_128_GCM, T, max, tls12, P_SHA256);
        add("TLS_DHE_DSS_WITH_AES_128_GCM_SHA256", 0x00a2, --p, K_DHE_DSS, B_AES_128_GCM, T, max, tls12, P_SHA256);
        ...
        add("TLS_DH_anon_WITH_AES_256_GCM_SHA384", 0x00a7, --p, K_DH_ANON, B_AES_256_GCM, N, max, tls12, P_SHA384);
        add("TLS_DH_anon_WITH_AES_128_GCM_SHA256", 0x00a6, --p, K_DH_ANON, B_AES_128_GCM, N, max, tls12, P_SHA256);

        add("TLS_DH_anon_WITH_AES_256_CBC_SHA256", 0x006d, --p, K_DH_ANON, B_AES_256, N, max, tls12, P_SHA256);
        add("TLS_ECDH_anon_WITH_AES_256_CBC_SHA", 0xC019, --p, K_ECDH_ANON, B_AES_256, N);
        ...
        add("TLS_ECDH_anon_WITH_AES_128_CBC_SHA", 0xC018, --p, K_ECDH_ANON, B_AES_128, N);
        ...
        add("TLS_ECDH_anon_WITH_3DES_EDE_CBC_SHA", 0xC017, --p, K_ECDH_ANON, B_3DES, N);
