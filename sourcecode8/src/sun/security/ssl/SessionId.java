...
final class SessionId {
    ...
    SessionId(boolean isRejoinable, SecureRandom generator) {
        ...
        { sessionId = new RandomCookie(generator).random_bytes; } else { sessionId = new byte[0]; }
    }
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < sessionId.length; i++) { retval += sessionId[i]; }
        return retval;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (b.length != sessionId.length) { return false; }
        for (int i = 0; i < sessionId.length; i++) {
            if (b[i] != sessionId[i]) { return false; }
        }
        return true;
    }
}
