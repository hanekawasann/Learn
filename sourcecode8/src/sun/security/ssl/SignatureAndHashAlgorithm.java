...
final class SignatureAndHashAlgorithm {
    ...
    private SignatureAndHashAlgorithm(HashAlgorithm hash, SignatureAlgorithm signature, String algorithm,
        int priority) {
        this.hash = hash;
    ...
    static SignatureAndHashAlgorithm valueOf(int hash, int signature, int sequence) {
        ...
        if (signAlg == null) {
            ...
            signAlg = new SignatureAndHashAlgorithm(
                "Unknown (hash:0x" + Integer.toString(hash, 16) + ", signature:0x" + Integer.toString(signature, 16) +
                    ")", id, sequence);
        }
    ...
    static SignatureAndHashAlgorithm getPreferableAlgorithm(Collection<SignatureAndHashAlgorithm> algorithms,
        ...
        for (SignatureAndHashAlgorithm algorithm : algorithms) {
            ...
            } else if ((expected.equalsIgnoreCase("dsa") && signValue == SignatureAlgorithm.DSA.value) ||
                (expected.equalsIgnoreCase("ecdsa") && signValue == SignatureAlgorithm.ECDSA.value) ||
                (expected.equalsIgnoreCase("ec") && signValue == SignatureAlgorithm.ECDSA.value)) {
                return algorithm;
            }
        }
