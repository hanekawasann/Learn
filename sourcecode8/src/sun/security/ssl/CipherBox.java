...
final class CipherBox {
     ...
     *
     * @throws NoSuchAlgorithmException if no appropriate JCE Cipher
     *                                  implementation could be found.
     */
    private CipherBox(ProtocolVersion protocolVersion, BulkCipher bulkCipher, SecretKey key, IvParameterSpec iv,
        SecureRandom random, boolean encrypt) throws NoSuchAlgorithmException {
        try {
             ...
             * mask for decryption.
             */
            if (iv == null && bulkCipher.ivSize != 0 && mode == Cipher.DECRYPT_MODE &&
            ...
            if (cipherType == AEAD_CIPHER) {
                ...
                fixedIv = iv.getIV();
                if (fixedIv == null || fixedIv.length != bulkCipher.fixedIvSize) {
     ...
     */
    static CipherBox newCipherBox(ProtocolVersion version, BulkCipher cipher, SecretKey key, IvParameterSpec iv,
        SecureRandom random, boolean encrypt) throws NoSuchAlgorithmException {
            ...
            if (cipherType == AEAD_CIPHER) {
                try {
                    ...
                    throw new RuntimeException(
                        "Cipher error in AEAD mode in JCE provider " + cipher.getProvider().getName(), ibe);
                }
            } else {
                ...
                if (newLen != len) {
                    ...
                    throw new RuntimeException(
                        "Cipher buffering error " + "in JCE provider " + cipher.getProvider().getName());
                }
                return newLen;
            }
    ...
    int encrypt(ByteBuffer bb, int outLimit) {
        ...
        if (cipherType == AEAD_CIPHER) {
            try {
                ...
                // unlikely to happen
                throw new RuntimeException(
                    "Cipher error in AEAD mode in JCE provider " + cipher.getProvider().getName(), ibse);
            }
        } else {
            int newLen;
            try {
        ...
        try {
            ...
            if (cipherType == AEAD_CIPHER) {
                try {
                    ...
                    // unlikely to happen
                    throw new RuntimeException(
                        "Cipher error in AEAD mode \"" + ibse.getMessage() + " \"in JCE provider " +
     ...
     */
    private static int removePadding(byte[] buf, int offset, int len, int tagLen, int blockSize,
     ...
     */
    private static int removePadding(ByteBuffer bb, int tagLen, int blockSize, ProtocolVersion protocolVersion)
        ...
        // The padding data should be filled with the padding length value.
        int[] results = checkPadding((ByteBuffer) bb.duplicate().position(offset + newLen), (byte) (padLen & 0xFF));
        if (protocolVersion.v >= ProtocolVersion.TLS10.v) {
            if (results[0] != 0) {          // padding data has invalid bytes
                throw new BadPaddingException("Invalid TLS padding data");
            }
    ...
    int applyExplicitNonce(Authenticator authenticator, byte contentType, ByteBuffer bb) throws BadPaddingException {
        switch (cipherType) {
            ...
            case AEAD_CIPHER:
                ...
                byte[] aad = authenticator
                    .acquireAuthenticationBytes(contentType, bb.remaining() - recordIvSize - tagSize);
     ...
     * @return the explicit nonce size of the cipher.
     */
    int applyExplicitNonce(Authenticator authenticator, byte contentType, byte[] buf, int offset, int cipheredLength)
        throws BadPaddingException {
    ...
    Boolean isAvailable() {
        ...
        if (cipherType == AEAD_CIPHER) {
            try {
                Authenticator authenticator = new Authenticator(protocolVersion);
