...
final class X509KeyManagerImpl extends X509ExtendedKeyManager implements X509KeyManager {
    ...
    private String chooseAlias(List<KeyType> keyTypeList, Principal[] issuers, CheckType checkType,
        AlgorithmConstraints constraints, List<SNIServerName> requestedServerNames, String idAlgorithm) {
    ...
    public String[] getAliases(String keyType, Principal[] issuers, CheckType checkType,
        ...
        for (int i = 0, n = builders.size(); i < n; i++) {
            try {
                List<EntryStatus> results = getAliases(i, keyTypeList, issuerSet, true, checkType, constraints, null,
                    null);
                if (results != null) {
                    if (allResults == null) {
                        allResults = new ArrayList<EntryStatus>();
    ...
    private static enum CheckType {
        ...
        CLIENT(new HashSet<String>(Arrays.asList(new String[] { "2.5.29.37.0", "1.3.6.1.5.5.7.3.2" }))),

        // enum constant for "tls server" check
        // valid EKU for TLS server: any, tls_server, ns_sgc, ms_sgc
        SERVER(new HashSet<String>(Arrays.asList(
            new String[] { "2.5.29.37.0", "1.3.6.1.5.5.7.3.1", "2.16.840.1.113730.4.1", "1.3.6.1.4.1.311.10.3.3" })));
                ...
                for (SNIServerName serverName : serverNames) {
                    if (serverName.getType() == StandardConstants.SNI_HOST_NAME) {
                        ...
                        try {
                            ...
                            if (useDebug) {
                                debug.println(
                                    "Certificate identity does not match " + "Server Name Inidication (SNI): " +
                                        hostname);
                            }
                            return CheckResult.INSENSITIVE;
                        }
     ...
     *     matches
     */
    private List<EntryStatus> getAliases(int builderIndex, List<KeyType> keyTypes, Set<Principal> issuerSet,
        boolean findAll, CheckType checkType, AlgorithmConstraints constraints,
        List<SNIServerName> requestedServerNames, String idAlgorithm) throws Exception {
        ...
        for (Enumeration<String> e = ks.aliases(); e.hasMoreElements(); ) {
            ...
            if (constraints != null && !conformsToAlgorithmConstraints(constraints, chain)) {
                ...
                if (useDebug) {
                    debug.println("Ignoring alias " + alias + ": certificate list does not conform to " +
            ...
            CheckResult checkResult = checkType
                .check((X509Certificate) chain[0], date, requestedServerNames, idAlgorithm);
