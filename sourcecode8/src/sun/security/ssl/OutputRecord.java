...
class OutputRecord extends ByteArrayOutputStream implements Record {
    ...
    void encrypt(Authenticator authenticator, CipherBox box) throws IOException {
        ...
        if (authenticator instanceof MAC) {
            ...
            if (signer.MAClen() != 0) {
                byte[] hash = signer.compute(contentType, buf, headerPlusMaxIVSize, count - headerPlusMaxIVSize, false);
                write(hash);
            }
        }
        ...
        if (!box.isNullCipher()) {
            ...
            // TLS 1.1 or later.
            if ((protocolVersion.v >= ProtocolVersion.TLS11.v) && (box.isCBCMode() || box.isAEADMode())) {
                byte[] nonce = box.createExplicitNonce(authenticator, contentType, count - headerPlusMaxIVSize);
    ...
    void write(OutputStream s, boolean holdRecord, ByteArrayOutputStream heldRecordBuffer) throws IOException {
        ...
        if (debug != null && (Debug.isOn("record") || Debug.isOn("handshake"))) {
            if ((debug != null && Debug.isOn("record")) || contentType() == ct_change_cipher_spec) {
                System.out.println(Thread.currentThread().getName()
                    // v3.0/v3.1 ...
                    + ", WRITE: " + protocolVersion + " " + InputRecord.contentName(contentType()) + ", length = " +
                    length);
            }
        }
        ...
        if (firstMessage && useV2Hello()) {
            ...
            if (debug != null && Debug.isOn("record")) {
                System.out.println(
                    Thread.currentThread().getName() + ", WRITE: SSLv2 client hello message" + ", length = " +
                        (count - 2)); // 2 byte SSLv2 header
            }
        } else {
            /*
    ...
    private boolean useV2Hello() {
        return firstMessage && (helloVersion == ProtocolVersion.SSL20Hello) && (contentType == ct_handshake) &&
     ...
     */
    private static int[] V3toV2CipherMap1 = { -1, -1, -1, 0x02, 0x01, -1, 0x04, 0x05, -1, 0x06, 0x07 };
    private static int[] V3toV2CipherMap3 = { -1, -1, -1, 0x80, 0x80, -1, 0x80, 0x80, -1, 0x40, 0xC0 };
