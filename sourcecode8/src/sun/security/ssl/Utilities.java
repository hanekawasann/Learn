...
final class Utilities {
    ...
    static List<SNIServerName> addToSNIServerNameList(List<SNIServerName> serverNames, String hostname) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (serverName.getType() == StandardConstants.SNI_HOST_NAME) {
                ...
                if (Debug.isOn("ssl")) {
                    System.out.println(
                        Thread.currentThread().getName() + ", the previous server name in SNI (" + serverName +
    ...
    private static SNIHostName rawToSNIHostName(String hostname) {
        ...
        if (hostname != null && hostname.indexOf('.') > 0 && !hostname.endsWith(".") &&
            !IPAddressUtil.isIPv4LiteralAddress(hostname) && !IPAddressUtil.isIPv6LiteralAddress(hostname)) {
