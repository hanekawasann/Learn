...
final public class SSLEngineImpl extends SSLEngine {
    ...
    private void init(SSLContextImpl ctx) {
        ...
        serverNames = Utilities.addToSNIServerNameList(serverNames, getPeerHost());
        ...
        if (packetLen > sess.getPacketBufferSize()) {
            if (packetLen > Record.maxLargeRecordSize) {
                throw new SSLProtocolException(
                    "Input SSL/TLS record too big: max = " + Record.maxLargeRecordSize + " len = " + packetLen);
            } else {
    ...
    private HandshakeStatus readRecord(EngineArgs ea) throws IOException {
        ...
        if (getConnectionState() != cs_ERROR) {
             ...
             */
            try {
                decryptedBB = inputRecord.decrypt(readAuthenticator, readCipher, readBB);
            } catch (BadPaddingException e) {
                byte alertType = (inputRecord.contentType() == Record.ct_handshake) ? Alerts.alert_handshake_failure
            ...
            synchronized (this) {
                switch (inputRecord.contentType()) {
                    ...
                    case Record.ct_change_cipher_spec:
                        if ((connectionState != cs_HANDSHAKE && connectionState != cs_RENEGOTIATE) ||
                            inputRecord.available() != 1 || inputRecord.read() != 1) {
                    ...
                    default:
                        ...
                        //
                        if (debug != null && Debug.isOn("ssl")) {
                            System.out.println(Thread.currentThread().getName() + ", Received record type: " +
                ...
                if (connectionState < cs_ERROR && !isInboundDone() && (hsStatus == HandshakeStatus.NOT_HANDSHAKING)) {
                    if (checkSequenceNumber(readAuthenticator, inputRecord.contentType())) {
                        hsStatus = getHSStatus(null);
                    }
                }
            } // synchronized (this)
    ...
    public SSLEngineResult wrap(ByteBuffer[] appData, int offset, int length, ByteBuffer netData) throws SSLException {
        ...
        try {
            ...
            fatal(Alerts.alert_internal_error, "problem wrapping app data", e);
            return null;  // make compiler happy
        } finally {
            /*
    ...
    private HandshakeStatus writeRecord(EngineOutputRecord eor, EngineArgs ea) throws IOException {
        ...
        HandshakeStatus hsStatus = writer.writeRecord(eor, ea, writeAuthenticator, writeCipher);
    ...
    boolean needToSplitPayload(CipherBox cipher, ProtocolVersion protocol) {
        return (protocol.v <= ProtocolVersion.TLS10.v) && cipher.isCBCMode() && !isFirstAppOutputRecord &&
     ...
     */
    private boolean checkSequenceNumber(Authenticator authenticator, byte type) throws IOException {
    ...
    synchronized void fatal(byte description, String diagnostic, Throwable cause) throws SSLException {
        ...
        if (closeReason != null) {
            if ((debug != null) && Debug.isOn("ssl")) {
                System.out.println(Thread.currentThread().getName() + ", fatal: engine already closed.  Rethrowing " +
                    cause.toString());
            }
        ...
        if ((debug != null) && Debug.isOn("ssl")) {
            System.out.println(
                Thread.currentThread().getName() + ", fatal error: " + description + ": " + diagnostic + "\n" +
                    cause.toString());
        }
