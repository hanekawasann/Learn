...
public final class KerberosClientKeyExchangeImpl extends sun.security.ssl.KerberosClientKeyExchange {
     ...
     */
    @Override
    public void init(String serverName, AccessControlContext acc, ProtocolVersion protocolVersion, SecureRandom rand)
        throws IOException {
        ...
        try {
            ...
            if (serverKeys.length == 0) {
                throw new IOException("Found no key for " + princ +
                    (creds.getName() == null ? "" : (", this keytab is for " + creds.getName() + " only")));
            }
    ...
    private static KerberosTicket getServiceTicket(String serverName, final AccessControlContext acc)
        throws IOException {
