...
public class Krb5InitCredential extends KerberosTicket implements Krb5CredElement {
    ...
    private Krb5InitCredential(Krb5NameElement name, byte[] asn1Encoding, KerberosPrincipal client,
        KerberosPrincipal server, byte[] sessionKey, int keyType, boolean[] flags, Date authTime, Date startTime,
        Date endTime, Date renewTill, InetAddress[] clientAddresses) throws GSSException {
        super(asn1Encoding, client, server, sessionKey, keyType, flags, authTime, startTime, endTime, renewTill,
            clientAddresses);
        ...
        try {
            ...
            krb5Credentials = new Credentials(asn1Encoding, client.getName(), server.getName(), sessionKey, keyType,
    ...
    private Krb5InitCredential(Krb5NameElement name, Credentials delegatedCred, byte[] asn1Encoding,
        KerberosPrincipal client, KerberosPrincipal server, byte[] sessionKey, int keyType, boolean[] flags,
        Date authTime, Date startTime, Date endTime, Date renewTill, InetAddress[] clientAddresses)
        throws GSSException {
        super(asn1Encoding, client, server, sessionKey, keyType, flags, authTime, startTime, endTime, renewTill,
            clientAddresses);
    ...
    static Krb5InitCredential getInstance(GSSCaller caller, Krb5NameElement name, int initLifetime)
        ...
        if (tgt == null) { throw new GSSException(GSSException.NO_CRED, -1, "Failed to find any Kerberos tgt"); }
        ...
        return new Krb5InitCredential(name, tgt.getEncoded(), tgt.getClient(), tgt.getServer(),
            tgt.getSessionKey().getEncoded(), tgt.getSessionKeyType(), tgt.getFlags(), tgt.getAuthTime(),
            tgt.getStartTime(), tgt.getEndTime(), tgt.getRenewTill(), tgt.getClientAddresses());
    }
    ...
    static Krb5InitCredential getInstance(Krb5NameElement name, Credentials delegatedCred) throws GSSException {
        ...
        return new Krb5InitCredential(credName, delegatedCred, delegatedCred.getEncoded(), client, server,
            sessionKey.getBytes(), sessionKey.getEType(), delegatedCred.getFlags(), delegatedCred.getAuthTime(),
            delegatedCred.getStartTime(), delegatedCred.getEndTime(), delegatedCred.getRenewTill(),
    ...
    public int getInitLifetime() throws GSSException {
        int retVal = 0;
        retVal = (int) (getEndTime().getTime() - (new Date().getTime()));
    ...
    public void dispose() throws GSSException {
        try {
            ...
            GSSException gssException = new GSSException(GSSException.FAILURE, -1,
    ...
    public GSSCredentialSpi impersonate(GSSNameSpi name) throws GSSException {
        try {
            ...
            Credentials newCred = Credentials.acquireS4U2selfCreds(kname.getKrb5PrincipalName(), krb5Credentials);
