...
public class Krb5AcceptCredential implements Krb5CredElement {
        ...
        try {
            creds = AccessController.doPrivileged(new PrivilegedExceptionAction<ServiceCreds>() {
                public ServiceCreds run() throws Exception {
                    return Krb5Util
                        .getServiceCreds(caller == GSSCaller.CALLER_UNKNOWN ? GSSCaller.CALLER_ACCEPT : caller,
                            serverPrinc, acc);
                }
            });
    ...
    public GSSCredentialSpi impersonate(GSSNameSpi name) throws GSSException {
        ...
        if (cred != null) {
            ...
            throw new GSSException(GSSException.FAILURE, -1, "Only an initiate credentials can impersonate");
        }
    }
}
