...
class Krb5Context implements GSSContextSpi {
            ...
            if (delegatedCred == null) {
                ...
                try {
                    delegatedCred = new Krb5ProxyCredential(
                        Krb5InitCredential.getInstance(GSSCaller.CALLER_ACCEPT, myName, lifetime), peerName,
                        serviceTicket);
                    ...
                    try {
                        ...
                        kerbTicket = AccessController.doPrivileged(new PrivilegedExceptionAction<KerberosTicket>() {
                            public KerberosTicket run() throws Exception {
                                ...
                                return Krb5Util.getTicket(GSSCaller.CALLER_UNKNOWN,
                                    // since it's useSubjectCredsOnly here,
                                    // don't worry about the null
                                    second == null ? myName.getKrb5PrincipalName().getName()
                                        : second.getName().getKrb5PrincipalName().getName(),
                                    peerName.getKrb5PrincipalName().getName(), acc);
                            }
                        });
                ...
                if (serviceCreds == null) {
                    ...
                    if (second == null) {
                        ...
                        serviceCreds = Credentials
                            .acquireS4U2proxyCreds(peerName.getKrb5PrincipalName().getName(), second.tkt,
                                second.getName().getKrb5PrincipalName(), tgt);
                    }
                    if (GSSUtil.useSubjectCredsOnly(caller)) {
                        final Subject subject = AccessController
                            .doPrivileged(new java.security.PrivilegedAction<Subject>() {
                                public Subject run() {
    ...
    public final int wrap(byte inBuf[], int inOffset, int len, byte[] outBuf, int outOffset, MessageProp msgProp)
        throws GSSException {
    ...
    public final void wrap(byte inBuf[], int offset, int len, OutputStream os, MessageProp msgProp)
        throws GSSException {
    ...
    public final int unwrap(byte inBuf[], int inOffset, int len, byte[] outBuf, int outOffset, MessageProp msgProp)
        throws GSSException {
    ...
    private int getMIC(byte[] inMsg, int offset, int len, byte[] outBuf, int outOffset, MessageProp msgProp)
        throws GSSException {
    ...
    private void verifyMIC(InputStream is, byte[] inMsg, int msgOffset, int msgLen, MessageProp msgProp)
        throws GSSException {
     ...
     *
     * @throws GSSException with major codes NO_CONTEXT, FAILURE.
     */
