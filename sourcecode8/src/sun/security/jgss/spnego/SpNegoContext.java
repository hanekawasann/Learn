...
public class SpNegoContext implements GSSContextSpi {
    ...
    // debug property
    static final boolean DEBUG = java.security.AccessController
        .doPrivileged(new sun.security.action.GetBooleanAction("sun.security.spnego.debug")).booleanValue();
    ...
    public final boolean getDelegPolicyState() {
        if (isInitiator() && mechContext != null && mechContext instanceof ExtendedGSSContext &&
    ...
    public final boolean getCredDelegState() {
        if (isInitiator() && mechContext != null && (state == STATE_IN_PROCESS || state == STATE_DONE)) {
    ...
    public final byte[] initSecContext(InputStream is, int mechTokenSize) throws GSSException {
        ...
        try {
            if (state == STATE_NEW) {
                ...
                initToken = new NegTokenInit(DER_mechTypes, getContextFlags(), mechToken, null);
                if (DEBUG) {
                    System.out.println("SpNegoContext.initSecContext: " + "sending token of type = " +
                ...
                if (DEBUG) {
                    System.out.println("SpNegoContext.initSecContext: " + "process received token = " +
                ...
                if (DEBUG) {
                    System.out.println("SpNegoContext.initSecContext: " + "received token of type = " +
                ...
                if (internal_mech == null) {
                    // return wth failure
                    throw new GSSException(errorCode, -1, "supported mechanism from server is null");
                }
                ...
                if (negoResult == SpNegoToken.NegoResult.REJECT) {
                    throw new GSSException(errorCode, -1, internal_mech.toString());
                }
                ...
                if ((negoResult == SpNegoToken.NegoResult.ACCEPT_COMPLETE) ||
                    ...
                    if (isMechContextEstablished()) {
                        ...
                        retVal = mechToken;
                        if (DEBUG) {
                            System.out.println("SPNEGO Negotiated Mechanism = " + internal_mech + " " +
                        ...
                        if (DEBUG) {
                            System.out.println("SpNegoContext.initSecContext:" + " continue sending token of type = " +
                                SpNegoToken.getTokenName(initToken.getType()));
                        }
                        // get the encoded token
                        retVal = initToken.getEncoded();
            ...
            if (DEBUG) {
                if (retVal != null) {
                    System.out.println(
                        "SNegoContext.initSecContext: " + "sending token = " + SpNegoToken.getHexBytes(retVal));
                }
            }
    ...
    public final byte[] acceptSecContext(InputStream is, int mechTokenSize) throws GSSException {
        ...
        try {
            if (state == STATE_NEW) {
                ...
                if (DEBUG) {
                    System.out.println(
                        "SpNegoContext.acceptSecContext: " + "receiving token = " + SpNegoToken.getHexBytes(token));
                }
                ...
                if (DEBUG) {
                    System.out.println("SpNegoContext.acceptSecContext: " + "received token of type = " +
                        SpNegoToken.getTokenName(initToken.getType()));
                }
                ...
                Oid mech_wanted = negotiate_mech_type(supported_mechSet, mechList);
                if (mech_wanted == null) {
                    valid = false;
                }
                ...
                if (!GSSUtil.useMSInterop() && valid) {
                    valid = verifyMechListMIC(DER_mechTypes, initToken.getMechListMIC());
                }
                ...
                if (DEBUG) {
                    System.out.println("SpNegoContext.acceptSecContext: " + "sending token of type = " +
                        SpNegoToken.getTokenName(targToken.getType()));
                }
                // get the encoded token
            ...
            if (DEBUG) {
                System.out
                    .println("SpNegoContext.acceptSecContext: " + "sending token = " + SpNegoToken.getHexBytes(retVal));
            }
        } catch (IOException e) {
            GSSException gssException = new GSSException(GSSException.FAILURE, -1, e.getMessage());
            gssException.initCause(e);
    ...
    private boolean verifyMechListMIC(byte[] mechTypes, byte[] token) throws GSSException {
        ...
        if (!mechContext.getIntegState()) {
            if (DEBUG) {
                System.out
                    .println("SpNegoContext: no MIC token validation" + " - mechanism does not support integrity");
            }
            return true;
        }
        ...
        try {
            MessageProp prop = new MessageProp(0, true);
            verifyMIC(token, 0, token.length, mechTypes, 0, mechTypes.length, prop);
            valid = true;
        } catch (GSSException e) {
            valid = false;
            if (DEBUG) {
    ...
    private byte[] GSS_initSecContext(byte[] token) throws GSSException {
        ...
        if (mechContext == null) {
            ...
            GSSName serverName = factory.manager
                .createName(peerName.toString(), peerName.getStringNameType(), internal_mech);
            GSSCredential cred = null;
            if (myCred != null) {
                // create context with provided credential
                cred = new GSSCredentialImpl(factory.manager, myCred.getInternalCred());
            }
            mechContext = factory.manager.createContext(serverName, internal_mech, cred, GSSContext.DEFAULT_LIFETIME);
    ...
    private byte[] GSS_acceptSecContext(byte[] token) throws GSSException {
        ...
        byte[] accept_token = mechContext.acceptSecContext(token, 0, token.length);
    ...
    public final GSSCredentialSpi getDelegCred() throws GSSException {
        ...
        if (mechContext != null) {
            GSSCredentialImpl delegCred = (GSSCredentialImpl) mechContext.getDelegCred();
            if (delegCred == null) {
                return null;
            }
            ...
            GSSCredentialSpi mechCred = delegCred.getElement(internal_mech, initiate);
            SpNegoCredElement cred = new SpNegoCredElement(mechCred);
            return cred.getInternalCred();
        } else {
            throw new GSSException(GSSException.NO_CONTEXT, -1, "getDelegCred called in invalid state!");
        }
    }
    ...
    public final int getWrapSizeLimit(int qop, boolean confReq, int maxTokSize) throws GSSException {
        if (mechContext != null) {
            ...
            throw new GSSException(GSSException.NO_CONTEXT, -1, "getWrapSizeLimit called in invalid state!");
        }
    }
    ...
    public final byte[] wrap(byte inBuf[], int offset, int len, MessageProp msgProp) throws GSSException {
        if (mechContext != null) {
            ...
            throw new GSSException(GSSException.NO_CONTEXT, -1, "Wrap called in invalid state!");
        }
    }
    ...
    public final byte[] getMIC(byte[] inMsg, int offset, int len, MessageProp msgProp) throws GSSException {
        if (mechContext != null) {
            ...
            throw new GSSException(GSSException.NO_CONTEXT, -1, "getMIC called in invalid state!");
        }
    }
    ...
    public final void verifyMIC(byte[] inTok, int tokOffset, int tokLen, byte[] inMsg, int msgOffset, int msgLen,
        MessageProp msgProp) throws GSSException {
        if (mechContext != null) {
            mechContext.verifyMIC(inTok, tokOffset, tokLen, inMsg, msgOffset, msgLen, msgProp);
        } else {
            throw new GSSException(GSSException.NO_CONTEXT, -1, "verifyMIC called in invalid state!");
        }
    }
    ...
    public Object inquireSecContext(InquireType type) throws GSSException {
        if (mechContext == null) {
            throw new GSSException(GSSException.NO_CONTEXT, -1, "Underlying mech not established.");
        }
        if (mechContext instanceof ExtendedGSSContext) {
            return ((ExtendedGSSContext) mechContext).inquireSecContext(type);
        } else {
            throw new GSSException(GSSException.BAD_MECH, -1, "inquireSecContext not supported by underlying mech.");
        }
    }
}
