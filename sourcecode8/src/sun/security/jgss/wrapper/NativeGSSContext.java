...
class NativeGSSContext implements GSSContextSpi {
        ...
        if (System.getSecurityManager() != null) {
            ...
            if (GSSUtil.isSpNegoMech(cStub.getMech()) && isInitiator && !isEstablished) {
                if (srcName == null) {
                    ...
                    GSSCredElement tempCred = new GSSCredElement(null, lifetime, GSSCredential.INITIATE_ONLY,
    ...
    NativeGSSContext(GSSNameElement peer, GSSCredElement myCred, int time, GSSLibStub stub) throws GSSException {
        ...
        if (GSSUtil.isKerberosMech(cStub.getMech())) {
            doServicePermCheck();
            if (cred == null) {
                cred = new GSSCredElement(null, lifetime, GSSCredential.INITIATE_ONLY, cStub);
            }
    ...
    public byte[] initSecContext(InputStream is, int mechTokenLen) throws GSSException {
        ...
        if ((!isEstablished) && (isInitiator)) {
            ...
            outToken = cStub.initContext(pCred, targetName.pName, cb, inToken, this);
            ...
            if (isEstablished) {
                ...
                if (cred == null) {
                    cred = new GSSCredElement(srcName, lifetime, GSSCredential.INITIATE_ONLY, cStub);
                }
            }
        }
        return outToken;
    ...
    public byte[] acceptSecContext(InputStream is, int mechTokenLen) throws GSSException {
        ...
        if ((!isEstablished) && (!isInitiator)) {
            ...
            if (targetName == null) {
                ...
                cred = new GSSCredElement(targetName, lifetime, GSSCredential.ACCEPT_ONLY, cStub);
            }

            // Only inspect token when the permission check has not
            // been performed
            if (GSSUtil.isSpNegoMech(cStub.getMech()) && (outToken != null) && !skipServicePermCheck) {
    ...
    public int getWrapSizeLimit(int qop, boolean confReq, int maxTokenSize) throws GSSException {
        return cStub.wrapSizeLimit(pContext, (confReq ? 1 : 0), qop, maxTokenSize);
    }
    ...
    public int wrap(byte[] inBuf, int inOffset, int len, byte[] outBuf, int outOffset, MessageProp msgProp)
        throws GSSException {
    ...
    public int unwrap(byte[] inBuf, int inOffset, int len, byte[] outBuf, int outOffset, MessageProp msgProp)
        throws GSSException {
        byte[] result = null;
    ...
    public void verifyMIC(byte[] inToken, int tOffset, int tLen, byte[] inMsg, int mOffset, int mLen,
