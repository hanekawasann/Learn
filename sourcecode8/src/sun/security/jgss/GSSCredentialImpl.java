...
public class GSSCredentialImpl implements ExtendedGSSCredential {
    ...
    GSSCredentialImpl(GSSManagerImpl gssManager, int usage) throws GSSException {
        this(gssManager, null, GSSCredential.DEFAULT_LIFETIME, (Oid[]) null, usage);
    }
    ...
    GSSCredentialImpl(GSSManagerImpl gssManager, GSSName name, int lifetime, Oid mechs[], int usage)
        throws GSSException {
        init(gssManager);
        ...
        if ((hashtable.size() == 0) || (usage != getUsage())) { throw new GSSException(GSSException.NO_CRED); }
    }
    ...
    void init(GSSManagerImpl gssManager) {
        ...
        hashtable = new Hashtable<SearchKey, GSSCredentialSpi>(gssManager.getMechs().length);
    }
    ...
    public GSSCredential impersonate(GSSName name) throws GSSException {
        ...
        return (cred == null ? null : new GSSCredentialImpl(gssManager, cred));
    }
    ...
    public int getRemainingLifetime() throws GSSException {
        ...
        for (Enumeration<SearchKey> e = hashtable.keys(); e.hasMoreElements(); ) {
            tempKey = e.nextElement();
            tempCred = hashtable.get(tempKey);
            if (tempKey.getUsage() == INITIATE_ONLY) { tempLife = tempCred.getInitLifetime(); } else if (tempKey.getUsage() == ACCEPT_ONLY) {
    ...
    public int getRemainingInitLifetime(Oid mech) throws GSSException {
        ...
        if (element != null) {
            found = true;
            if (max < element.getInitLifetime()) { max = element.getInitLifetime(); }
        }
    ...
    public int getRemainingAcceptLifetime(Oid mech) throws GSSException {
        ...
        if (element != null) {
            found = true;
            if (max < element.getAcceptLifetime()) { max = element.getAcceptLifetime(); }
        }
    ...
    public int getUsage() throws GSSException {
        ...
        for (Enumeration<SearchKey> e = hashtable.keys(); e.hasMoreElements(); ) {
            tempKey = e.nextElement();
            if (tempKey.getUsage() == INITIATE_ONLY) { initiate = true; } else if (tempKey.getUsage() == ACCEPT_ONLY) {
                accept = true;
            } else { return INITIATE_AND_ACCEPT; }
        }
        if (initiate) {
            if (accept) { return INITIATE_AND_ACCEPT; } else { return INITIATE_ONLY; }
        } else { return ACCEPT_ONLY; }
    }
    ...
    public int getUsage(Oid mech) throws GSSException {
        ...
        if (initiate && accept) { return GSSCredential.INITIATE_AND_ACCEPT; } else if (initiate) {
    ...
    public void add(GSSName name, int initLifetime, int acceptLifetime, Oid mech, int usage) throws GSSException {
        ...
        if (hashtable.containsKey(key)) {
            throw new GSSExceptionImpl(GSSException.DUPLICATE_ELEMENT,
                "Duplicate element found: " + getElementStr(mech, usage));
        }
        ...
        tempCred = gssManager.getCredentialElement(nameElement, initLifetime, acceptLifetime, mech, usage);
        /*
        ...
        if (tempCred != null) {
            if (usage == GSSCredential.INITIATE_AND_ACCEPT &&
                (!tempCred.isAcceptorCredential() || !tempCred.isInitiatorCredential())) {
                ...
                tempCred = gssManager
                    .getCredentialElement(nameElement, initLifetime, acceptLifetime, mech, desiredUsage);
     ...
     * @throws GSSException of invalid mechanism
     */
    public GSSCredentialSpi getElement(Oid mechOid, boolean initiate) throws GSSException {
        ...
        if (mechOid == null) {
            ...
            key = new SearchKey(mechOid, initiate ? INITIATE_ONLY : ACCEPT_ONLY);
            element = hashtable.get(key);
            if (element == null) {
                ...
                if (element == null) {
                    ...
                    for (int i = 0; i < elements.length; i++) {
                        ...
                        if (element.isInitiatorCredential() == initiate) { break; }
                    } // for loop
                }
            }
        } else {

            if (initiate) { key = new SearchKey(mechOid, INITIATE_ONLY); } else {
        ...
        if (element == null) {
            throw new GSSExceptionImpl(GSSException.NO_CRED,
                "No credential found for: " + getElementStr(mechOid, initiate ? INITIATE_ONLY : ACCEPT_ONLY));
        }
        return element;
    }

    Set<GSSCredentialSpi> getElements() {
        HashSet<GSSCredentialSpi> retVal = new HashSet<GSSCredentialSpi>(hashtable.size());
        Enumeration<GSSCredentialSpi> values = hashtable.elements();
        while (values.hasMoreElements()) {
    ...
    private static String getElementStr(Oid mechOid, int usage) {
        ...
        if (usage == GSSCredential.INITIATE_ONLY) {
            displayString = displayString.concat(" usage: Initiate");
        } else if (usage == GSSCredential.ACCEPT_ONLY) {
            displayString = displayString.concat(" usage: Accept");
        } else {
            displayString = displayString.concat(" usage: Initiate and Accept");
        }
