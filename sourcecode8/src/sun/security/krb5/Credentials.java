...
public class Credentials {
    ...
    public Credentials(Ticket new_ticket, PrincipalName new_client, PrincipalName new_server, EncryptionKey new_key,
        TicketFlags new_flags, KerberosTime authTime, KerberosTime new_startTime, KerberosTime new_endTime,
        KerberosTime renewTill, HostAddresses cAddr, AuthorizationData authzData) {
        this(new_ticket, new_client, new_server, new_key, new_flags, authTime, new_startTime, new_endTime, renewTill,
            cAddr);
    ...
    public Credentials(Ticket new_ticket, PrincipalName new_client, PrincipalName new_server, EncryptionKey new_key,
        TicketFlags new_flags, KerberosTime authTime, KerberosTime new_startTime, KerberosTime new_endTime,
        KerberosTime renewTill, HostAddresses cAddr) {
        ticket = new_ticket;
        client = new_client;
        server = new_server;
        key = new_key;
    ...
    public Credentials(byte[] encoding, String client, String server, byte[] keyBytes, int keyType, boolean[] flags,
        Date authTime, Date startTime, Date endTime, Date renewTill, InetAddress[] cAddrs)
        throws KrbException, IOException {
        this(new Ticket(encoding), new PrincipalName(client, PrincipalName.KRB_NT_PRINCIPAL),
            new PrincipalName(server, PrincipalName.KRB_NT_SRV_INST), new EncryptionKey(keyType, keyBytes),
            (flags == null ? null : new TicketFlags(flags)), (authTime == null ? null : new KerberosTime(authTime)),
            (startTime == null ? null : new KerberosTime(startTime)),
            (endTime == null ? null : new KerberosTime(endTime)),
            (renewTill == null ? null : new KerberosTime(renewTill)), null); // caddrs are in the encoding at this point
    }
    ...
    */
    ...
    public Credentials renew() throws KrbException, IOException {
        ...
        return new KrbTgsReq(options, this, server, null, // from
            null, // till
            null, // rtime
            null, // eTypes
            cAddr, null, null, null).sendAndGetCreds();
    }
    ...
    public static Credentials acquireTGTFromCache(PrincipalName princ, String ticketCache)
        ...
        if (ticketCache == null) {
            ...
            String os = java.security.AccessController
    ...
    public static synchronized Credentials acquireDefaultCreds() {
        ...
        if (cache != null) {
            sun.security.krb5.internal.ccache.Credentials temp = cache.getDefaultCreds();
            if (temp != null) {
                if (DEBUG) {
                    System.out
                        .println(">>> KrbCreds found the default ticket" + " granting ticket in credential cache.");
                }
        ...
        if (result == null) {
            ...
            if (alreadyLoaded) {
                ...
                try {
                    result = acquireDefaultNativeCreds(EType.getDefaults("default_tkt_enctypes"));
     ...
     * @throws IOException  if an error occurs in reading the credentials
     *                      cache
     * @throws KrbException if an error occurs specific to Kerberos
     */
    ...
    public static Credentials acquireS4U2selfCreds(PrincipalName user, Credentials ccreds)
        throws KrbException, IOException {
        return CredentialsUtil.acquireS4U2selfCreds(user, ccreds);
    }

    public static Credentials acquireS4U2proxyCreds(String service, Ticket second, PrincipalName client,
