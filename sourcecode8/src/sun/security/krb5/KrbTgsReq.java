...
public class KrbTgsReq {
    ...
    public KrbTgsReq(Credentials asCreds, PrincipalName sname) throws KrbException, IOException {
        this(new KDCOptions(), asCreds, sname, null, // KerberosTime from
    ...
    // S4U2proxy
    public KrbTgsReq(Credentials asCreds, Ticket second, PrincipalName sname) throws KrbException, IOException {
        this(KDCOptions.with(KDCOptions.CNAME_IN_ADDL_TKT, KDCOptions.FORWARDABLE), asCreds, sname, null, null, null,
            null, null, null, new Ticket[] { second }, // the service ticket
            null);
    }

    // S4U2user
    public KrbTgsReq(Credentials asCreds, PrincipalName sname, PAData extraPA) throws KrbException, IOException {
        this(KDCOptions.with(KDCOptions.FORWARDABLE), asCreds, asCreds.getClient(), sname, null, null, null, null, null,
            null, null, null, extraPA); // the PA-FOR-USER
    }

    // Called by Credentials, KrbCred
    KrbTgsReq(KDCOptions options, Credentials asCreds, PrincipalName sname, KerberosTime from, KerberosTime till,
        KerberosTime rtime, int[] eTypes, HostAddresses addresses, AuthorizationData authorizationData,
        Ticket[] additionalTickets, EncryptionKey subKey) throws KrbException, IOException {
        this(options, asCreds, asCreds.getClient(), sname, from, till, rtime, eTypes, addresses, authorizationData,
            additionalTickets, subKey, null);
    }

    private KrbTgsReq(KDCOptions options, Credentials asCreds, PrincipalName cname, PrincipalName sname,
        KerberosTime from, KerberosTime till, KerberosTime rtime, int[] eTypes, HostAddresses addresses,
        AuthorizationData authorizationData, Ticket[] additionalTickets, EncryptionKey subKey, PAData extraPA)
        ...
        if (options.get(KDCOptions.FORWARDED)) {
            if (!(asCreds.flags.get(KDCOptions.FORWARDABLE))) { throw new KrbException(Krb5.KRB_AP_ERR_REQ_OPTIONS); }
        }
        if (options.get(KDCOptions.PROXIABLE) && (!(asCreds.flags.get(Krb5.TKT_OPTS_PROXIABLE)))) {
            throw new KrbException(Krb5.KRB_AP_ERR_REQ_OPTIONS);
        }
        if (options.get(KDCOptions.PROXY)) {
            if (!(asCreds.flags.get(KDCOptions.PROXIABLE))) { throw new KrbException(Krb5.KRB_AP_ERR_REQ_OPTIONS); }
        }
        ...
        if (options.get(KDCOptions.POSTDATED)) {
            if (!(asCreds.flags.get(KDCOptions.POSTDATED))) { throw new KrbException(Krb5.KRB_AP_ERR_REQ_OPTIONS); }
        } else {
            if (from != null) { from = null; }
        }
        if (options.get(KDCOptions.RENEWABLE)) {
            if (!(asCreds.flags.get(KDCOptions.RENEWABLE))) { throw new KrbException(Krb5.KRB_AP_ERR_REQ_OPTIONS); }
        } else {
            if (rtime != null) { rtime = null; }
        }
        if (options.get(KDCOptions.ENC_TKT_IN_SKEY) || options.get(KDCOptions.CNAME_IN_ADDL_TKT)) {
            if (additionalTickets == null) { throw new KrbException(Krb5.KRB_AP_ERR_REQ_OPTIONS); }
        ...
        tgsReqMessg = createRequest(options, asCreds.ticket, asCreds.key, ctime, princName, servName, from, till, rtime,
            eTypes, addresses, authorizationData, additionalTickets, subKey, extraPA);
         ...
         * FORWARDED flag set.
         */
        if (asCreds.flags.get(KDCOptions.FORWARDED)) { options.set(KDCOptions.FORWARDED, true); }
    ...
    public void send() throws IOException, KrbException {
        ...
        if (servName != null) { realmStr = servName.getRealmString(); }
    ...
    private TGSReq createRequest(KDCOptions kdc_options, Ticket ticket, EncryptionKey key, KerberosTime ctime,
        PrincipalName cname, PrincipalName sname, KerberosTime from, KerberosTime till, KerberosTime rtime,
        int[] eTypes, HostAddresses addresses, AuthorizationData authorizationData, Ticket[] additionalTickets,
        EncryptionKey subKey, PAData extraPA) throws IOException, KrbException, UnknownHostException {
        ...
        if (authorizationData != null) {
            ...
            if (subKey != null) {
                ...
                useSubkey = true;
                encAuthorizationData = new EncryptedData(reqKey, ad, KeyUsage.KU_TGS_REQ_AUTH_DATA_SUBKEY);
            } else { encAuthorizationData = new EncryptedData(key, ad, KeyUsage.KU_TGS_REQ_AUTH_DATA_SESSKEY); }
        }

        KDCReqBody reqBody = new KDCReqBody(kdc_options, cname, sname, from, req_till, rtime, Nonce.value(), req_eTypes,
        ...
        switch (Checksum.CKSUMTYPE_DEFAULT) {
            ...
            case Checksum.CKSUMTYPE_HMAC_SHA1_96_AES256:
                cksum = new Checksum(Checksum.CKSUMTYPE_DEFAULT, temp, key, KeyUsage.KU_PA_TGS_REQ_CKSUM);
                break;
        ...
        byte[] tgs_ap_req = new KrbApReq(new APOptions(), ticket, key, cname, cksum, ctime, reqKey, null, null)
            .getMessage();

        PAData tgsPAData = new PAData(Krb5.PA_TGS_REQ, tgs_ap_req);
        return new TGSReq(extraPA != null ? new PAData[] { extraPA, tgsPAData } : new PAData[] { tgsPAData }, reqBody);
    }
