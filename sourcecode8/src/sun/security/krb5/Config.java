...
public class Config {
    ...
    public int getIntValue(String... keys) {
        ...
        if (result != null) {
            try {
                ...
                if (DEBUG) {
                    System.out.println("Exception in getting value of " + Arrays.toString(keys) + " " + e.getMessage());
                    System.out.println("Setting " + Arrays.toString(keys) + " to minimum value");
                }
    ...
    private List<String> loadConfigFile(final String fileName) throws IOException, KrbException {
        try {
            ...
            try (BufferedReader br = new BufferedReader(
                new InputStreamReader(AccessController.doPrivileged(new PrivilegedExceptionAction<FileInputStream>() {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if (line.startsWith("[")) {
                        ...
                        if (previous == null) {
                            throw new KrbException("Config file must starts with a section");
                        }
                        v.add(previous);
                        previous = line;
                    }
                ...
                if (value.equals("{")) {
                    ...
                    if (current.containsKey(key)) {
                        ...
                        if (!(obj instanceof Vector)) {
                            throw new KrbException("Key " + key + "used for both value and section");
                        }
    ...
    private String getJavaFileName() {
        ...
        if (name == null) {
            name = getProperty("java.home") + File.separator + "lib" + File.separator + "security" + File.separator +
                "krb5.conf";
            if (!fileExists(name)) {
                name = null;
            }
    ...
    public int[] defaultEtype(String configName) throws KrbException {
        ...
        if (default_enctypes == null) {
            if (DEBUG) {
                System.out.println("Using builtin default etypes for " + configName);
            }
            ...
            if (ls.isEmpty()) {
                throw new KrbException("no supported default etypes for " + configName);
            } else {
