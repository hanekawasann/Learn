...
public class EncryptionKey implements Cloneable {
    ...
    public static final EncryptionKey NULL_KEY = new EncryptionKey(new byte[] {}, EncryptedData.ETYPE_NULL, null);
    ...
    public static EncryptionKey[] acquireSecretKeys(PrincipalName princ, String keytab) {
        ...
        if (princ == null) { throw new IllegalArgumentException("Cannot have null pricipal name to look in keytab."); }
     ...
     * @returns never null
     */
    public static EncryptionKey acquireSecretKey(PrincipalName cname, char[] password, int etype,
        PAData.SaltAndParams snp) throws KrbException {
        String salt;
        byte[] s2kparams;
        if (snp != null) {
     ...
     * @returns never null
     */
    public static EncryptionKey acquireSecretKey(char[] password, String salt, int etype, byte[] s2kparams)
        throws KrbException {

        return new EncryptionKey(stringToKey(password, salt, s2kparams, etype), etype, null);
    }
     ...
     */
    public static EncryptionKey[] acquireSecretKeys(char[] password, String salt) throws KrbException {
        ...
        for (int i = 0; i < etypes.length; i++) {
            if (EType.isSupported(etypes[i])) {
                encKeys[i] = new EncryptionKey(stringToKey(password, salt, null, etypes[i]), etypes[i], null);
            } else {
                if (DEBUG) {
                    System.out.println("Encryption Type " + EType.toString(etypes[i]) + " is not supported/enabled");
                }
            }
        }
        return encKeys;
    ...
    private static byte[] stringToKey(char[] password, String salt, byte[] s2kparams, int keyType)
    ...
    public EncryptionKey(EncryptionKey key) throws KrbCryptoException {
        ...
        try {
            // check for DES key
            if ((keyType == EncryptedData.ETYPE_DES_CBC_MD5) || (keyType == EncryptedData.ETYPE_DES_CBC_CRC)) {
                // fix DES key parity
     ...
     *                       encoded data.
     * @throws IOException   if an I/O error occurs while reading encoded
     *                       data.
     */
    // Used in javax.security.auth.kerberos.KeyImpl
    ...
    public synchronized void destroy() {
        if (keyValue != null) { for (int i = 0; i < keyValue.length; i++) { keyValue[i] = 0; } }
    }
    ...
    public String toString() {
        return new String("EncryptionKey: keyType=" + keyType + " kvno=" + kvno + " keyValue (hex dump)=" +
            (keyValue == null || keyValue.length == 0 ? " Empty Key"
     ...
     */
    public static EncryptionKey findKey(int etype, Integer kvno, EncryptionKey[] keys) throws KrbException {
