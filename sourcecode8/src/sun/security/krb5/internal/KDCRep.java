...
public class KDCRep {
    ...
    public KDCRep(PAData[] new_pAData, PrincipalName new_cname, Ticket new_ticket, EncryptedData new_encPart,
    ...
    public KDCRep(DerValue encoding, int req_type)
     ...
     * @throws RealmException    if an error occurs while constructing
     *                           a Realm object from DER-encoded data.
     * @throws KrbApErrException if the value read from the DER-encoded
    ...
    protected void init(DerValue encoding, int req_type)
        ...
        if ((encoding.getTag() & 0x1F) != req_type) {
            if (DEBUG) {
                System.out.println(
                    ">>> KDCRep: init() " + "encoding tag is " + encoding.getTag() + " req type is " + req_type);
            }
    ...
    public byte[] asn1Encode() throws Asn1Exception, IOException {
        ...
        bytes.write(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0x03), cname.getRealm().asn1Encode());
