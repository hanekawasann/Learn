...
public class KDCReqBody {
    ...
    public KDCReqBody(KDCOptions new_kdcOptions, PrincipalName new_cname, //optional in ASReq only
        PrincipalName new_sname, //optional
        KerberosTime new_from, //optional
        KerberosTime new_till, KerberosTime new_rtime, //optional
        int new_nonce, int[] new_eType, //a sequence; not optional
     ...
     * @throws RealmException if an error occurs while constructing a Realm object from the encoded data.
     */
    public KDCReqBody(DerValue encoding, int msgType) throws Asn1Exception, RealmException, KrbException, IOException {
        ...
        cname = PrincipalName.parse(encoding.getData(), (byte) 0x01, true, new Realm("PLACEHOLDER"));
        if ((msgType != Krb5.KRB_AS_REQ) && (cname != null)) {
            throw new Asn1Exception(Krb5.ASN1_BAD_ID);
        }
        Realm realm = Realm.parse(encoding.getData(), (byte) 0x02, false);
        if (cname != null) {
            cname = new PrincipalName(cname.getNameType(), cname.getNameStrings(), realm);
        }
    ...
    public byte[] asn1Encode(int msgType) throws Asn1Exception, IOException {
        ...
        v.addElement(
            new DerValue(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0x00), kdcOptions.asn1Encode()));
        if (msgType == Krb5.KRB_AS_REQ) {
            if (cname != null) {
                v.addElement(
                    new DerValue(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0x01), cname.asn1Encode()));
            }
        }
        if (sname != null) {
        ...
        v.addElement(
            new DerValue(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0x08), eTypetemp.toByteArray()));
        if (addresses != null) {
            v.addElement(
                new DerValue(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0x09), addresses.asn1Encode()));
        }
        if (encAuthorizationData != null) {
            v.addElement(new DerValue(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0x0A),
        ...
        if (additionalTickets != null && additionalTickets.length > 0) {
            ...
            v.addElement(
                new DerValue(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0x0B), ticketsTemp.toByteArray()));
        }
