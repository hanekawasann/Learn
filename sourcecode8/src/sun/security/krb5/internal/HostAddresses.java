...
public class HostAddresses implements Cloneable {
    ...
    public HostAddresses(PrincipalName serverPrincipal) throws UnknownHostException, KrbException {
        ...
        if (serverPrincipal.getNameType() != PrincipalName.KRB_NT_SRV_HST || components.length < 2) {
    ...
    public Object clone() {
        ...
        if (addresses != null) {
            ...
            for (int i = 0; i < addresses.length; i++) {
                new_hostAddresses.addresses[i] = (HostAddress) addresses[i].clone();
            }
        }
    ...
    public boolean inList(HostAddress addr) {
        if (addresses != null) {
            for (int i = 0; i < addresses.length; i++) { if (addresses[i].equals(addr)) { return true; } }
        }
        return false;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if ((addresses == null && addrs.addresses != null) || (addresses != null && addrs.addresses == null)) {
            return false;
        }
        if (addresses != null && addrs.addresses != null) {
            if (addresses.length != addrs.addresses.length) { return false; }
            for (int i = 0; i < addresses.length; i++) {
                if (!addresses[i].equals(addrs.addresses[i])) { return false; }
            }
        }
        return true;
    }
    ...
    public byte[] asn1Encode() throws Asn1Exception, IOException {
        ...
        if (addresses != null && addresses.length > 0) {
            for (int i = 0; i < addresses.length; i++) { bytes.write(addresses[i].asn1Encode()); }
        }
     ...
     * @throws Asn1Exception if an Asn1Exception occurs.
     */
    public static HostAddresses parse(DerInputStream data, byte explicitTag, boolean optional)
    ...
    public void writeAddrs(CCacheOutputStream cos) throws IOException {
        ...
        for (int i = 0; i < addresses.length; i++) {
            ...
            cos.write(addresses[i].address, 0, addresses[i].address.length);
        }
    }
    ...
    public InetAddress[] getInetAddresses() {
        ...
        for (int i = 0; i < addresses.length; i++) {
            try {
                if ((addresses[i].addrType == Krb5.ADDRTYPE_INET) || (addresses[i].addrType == Krb5.ADDRTYPE_INET6)) {
    ...
    public static HostAddresses getLocalAddresses() throws IOException {
        ...
        try {
            ...
            if (DEBUG) {
                ...
                for (int i = 0; i < inetAddresses.length; i++) {
                    System.out.println("\n\t" + inetAddresses[i]);
                    if (inetAddresses[i] instanceof Inet4Address) { System.out.println("IPv4 address"); }
                    if (inetAddresses[i] instanceof Inet6Address) { System.out.println("IPv6 address"); }
                }
            }
    ...
    public HostAddresses(InetAddress[] inetAddresses) {
        ...
        for (int i = 0; i < inetAddresses.length; i++) { addresses[i] = new HostAddress(inetAddresses[i]); }
    }
}
