...
public class KerberosTime {
    ...
    private static long toKerberosTime(String time) throws Asn1Exception {
        ...
        if (time.length() != 15) { throw new Asn1Exception(Krb5.ASN1_BAD_TIMEFORMAT); }
        if (time.charAt(14) != 'Z') { throw new Asn1Exception(Krb5.ASN1_BAD_TIMEFORMAT); }
        int year = Integer.parseInt(time.substring(0, 4));
        Calendar calendar = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
        calendar.clear(); // so that millisecond is zero
        calendar.set(year, Integer.parseInt(time.substring(4, 6)) - 1, Integer.parseInt(time.substring(6, 8)),
            Integer.parseInt(time.substring(8, 10)), Integer.parseInt(time.substring(10, 12)),
    ...
    public String toGeneralizedTimeString() {
        ...
        return String.format("%04d%02d%02d%02d%02d%02dZ", calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH) + 1,
    ...
    public KerberosTime withMicroSeconds(int usec) {
        return new KerberosTime(kerberosTime - kerberosTime % 1000L + usec / 1000L, usec % 1000);
    }

    private boolean inClockSkew(int clockSkew) {
        return java.lang.Math.abs(kerberosTime - System.currentTimeMillis()) <= clockSkew * 1000L;
    }
    ...
    public boolean greaterThanWRTClockSkew(KerberosTime time, int clockSkew) {
        if ((kerberosTime - time.kerberosTime) > clockSkew * 1000L) { return true; }
        return false;
    }
    ...
    public boolean greaterThan(KerberosTime time) {
        return kerberosTime > time.kerberosTime ||
            kerberosTime == time.kerberosTime && microSeconds > time.microSeconds;
    }
    ...
    public boolean equals(Object obj) {
        ...
        return kerberosTime == ((KerberosTime) obj).kerberosTime && microSeconds == ((KerberosTime) obj).microSeconds;
    }
     ...
     */
    public static KerberosTime parse(DerInputStream data, byte explicitTag, boolean optional)
    ...
    public static int getDefaultSkew() {
        ...
        try {
            if ((tdiff = Config.getInstance().getIntValue("libdefaults", "clockskew")) ==
                Integer.MIN_VALUE) {   //value is not defined
                tdiff = Krb5.DEFAULT_ALLOWABLE_CLOCKSKEW;
            }
        } catch (KrbException e) {
            if (DEBUG) {
                System.out.println("Exception in getting clockskew from " + "Configuration " + "using default value " +
                    e.getMessage());
            }
        }
        return tdiff;
