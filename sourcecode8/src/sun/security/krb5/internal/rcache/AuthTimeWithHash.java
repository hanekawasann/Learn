...
public class AuthTimeWithHash extends AuthTime implements Comparable<AuthTimeWithHash> {
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(hash, that.hash) && Objects.equals(client, that.client) &&
            Objects.equals(server, that.server) && ctime == that.ctime && cusec == that.cusec;
    }
    ...
    public boolean isSameIgnoresHash(AuthTime old) {
        return client.equals(old.client) && server.equals(old.server) && ctime == old.ctime && cusec == old.cusec;
    }
    ...
    public byte[] encode(boolean withHash) {
        ...
        if (withHash) {
            cstring = "";
            sstring = String.format("HASH:%s %d:%s %d:%s", hash, client.length(), client, server.length(), server);
        } else {
            cstring = client;
            sstring = server;
        }
