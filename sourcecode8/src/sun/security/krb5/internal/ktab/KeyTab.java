...
public class KeyTab implements KeyTabConstants {
    ...
    private void load(KeyTabInputStream kis) throws IOException, RealmException {
        ...
        while (kis.available() > 0) {
            ...
            if (DEBUG) {
                System.out.println(">>> KeyTab: load() entry length: " + entryLength + "; type: " +
    ...
    // Called by KDC test
    public void addEntry(PrincipalName service, String salt, char[] psswd, int kvno, boolean append)
        throws KrbException {
        ...
        for (int i = 0; encKeys != null && i < encKeys.length; i++) {
            ...
            KeyTabEntry newEntry = new KeyTabEntry(service, service.getRealm(),
                new KerberosTime(System.currentTimeMillis()), kvno, keyType, keyValue);
