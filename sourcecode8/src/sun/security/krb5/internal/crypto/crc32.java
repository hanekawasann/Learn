...
public final class crc32 extends MessageDigestSpi implements Cloneable {
    ...
    private static int[] crc32Table = { 0x00000000, 0x77073096, 0xee0e612c, 0x990951ba, 0x076dc419, 0x706af48f,
    ...
    private void processData(byte[] data, int off, int len) {
        int result = seed;
        for (int i = 0; i < len; i++) { result = (result >>> 8) ^ crc32Table[(result ^ data[i]) & 0xff]; }
        seed = result;
    }
    ...
    public static void printcrc32Table() {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            System.out.print("0x" + zerofill.substring(temp.length()) + temp);
    ...
    public static int byte2crc32sum(int seed, byte[] data, int size) {
        ...
        for (int i = 0; i < size; i++) { crc = (crc >>> 8) ^ crc32Table[(crc ^ data[i]) & 0xff]; }
        return crc;
    }
