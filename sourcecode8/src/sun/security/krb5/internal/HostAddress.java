...
public class HostAddress implements Cloneable {
    ...
    public boolean equals(Object obj) {
        ...
        if (addrType != h.addrType || (address != null && h.address == null) ||
            (address == null && h.address != null)) { return false; }
        if (address != null && h.address != null) {
            if (address.length != h.address.length) { return false; }
            for (int i = 0; i < address.length; i++) { if (address[i] != h.address[i]) { return false; } }
        }
        return true;
    }
     ...
     * @throws if no IP address for the host could be found.
     */
    public InetAddress getInetAddress() throws UnknownHostException {
        // the type of internet addresses is 2.
        if (addrType == Krb5.ADDRTYPE_INET || addrType == Krb5.ADDRTYPE_INET6) {
    ...
    private int getAddrType(InetAddress inetAddress) {
        ...
        } else if (inetAddress instanceof Inet6Address) { addressType = Krb5.ADDRTYPE_INET6; }
        return (addressType);
    }
     ...
     * @throws KrbApErrException if address type and address length do not match defined value.
     */
    public HostAddress(int new_addrType, byte[] new_address) throws KrbApErrException, UnknownHostException {
        switch (new_addrType) {
            case Krb5.ADDRTYPE_INET:        //Internet address
                if (new_address.length != 4) { throw new KrbApErrException(0, "Invalid Internet address"); }
                break;
            ...
            case Krb5.ADDRTYPE_IPX:   // XNS address
                if (new_address.length != 6) { throw new KrbApErrException(0, "Invalid XNS address"); }
                break;
            case Krb5.ADDRTYPE_APPLETALK:  //AppleTalk DDP address
                if (new_address.length != 3) { throw new KrbApErrException(0, "Invalid DDP address"); }
                break;
            ...
            case Krb5.ADDRTYPE_INET6:     //Internet IPv6 address
                if (new_address.length != 16) { throw new KrbApErrException(0, "Invalid Internet IPv6 address"); }
                break;
        }
     ...
     * @throws IOException   if an I/O error occurs while reading encoded data.
     */
    public static HostAddress parse(DerInputStream data, byte explicitTag, boolean optional)
