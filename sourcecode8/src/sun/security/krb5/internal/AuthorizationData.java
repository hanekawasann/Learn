...
public class AuthorizationData implements Cloneable {
    ...
    public Object clone() {
        ...
        if (entry != null) {
            new_authorizationData.entry = new AuthorizationDataEntry[entry.length];
            for (int i = 0; i < entry.length; i++) {
                new_authorizationData.entry[i] = (AuthorizationDataEntry) entry[i].clone();
            }
        }
     ...
     * @throws IOException   if an I/O error occurs while reading encoded data.
     */
    public static AuthorizationData parse(DerInputStream data, byte explicitTag, boolean optional)
