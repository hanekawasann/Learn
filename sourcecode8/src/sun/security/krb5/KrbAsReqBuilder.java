            ...
            try {
                for (int i = 0; i < eTypes.length; i++) {
                    ...
                    if (snp != null) {
                        ...
                        // a salt at all
                        if (eTypes[i] != EncryptedData.ETYPE_ARCFOUR_HMAC && snp.salt != null) {
                            salt = snp.salt;
                        }
                        result[i] = EncryptionKey.acquireSecretKey(cname, password, eTypes[i], snp);
                    }
                }
                ...
                for (int i = 0; i < eTypes.length; i++) {
                    ...
                    if (result[i] == null) {
                        result[i] = EncryptionKey.acquireSecretKey(password, salt, eTypes[i], null);
                    }
                }
            ...
            try {
                ...
                if (!preAuthFailedOnce && (ke.returnCode() == Krb5.KDC_ERR_PREAUTH_FAILED ||
                    ...
                    if (password == null) {
                        ...
                        pakey = EncryptionKey
                            .acquireSecretKey(cname, password, paEType, PAData.getSaltAndParams(paEType, kerr.getPA()));
                    }
