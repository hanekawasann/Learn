...
public class Main {
    ...
    private static final java.util.ResourceBundle rb = java.util.ResourceBundle
        .getBundle("sun.security.tools.jarsigner.Resources");
    ...
    void parseArgs(String args[]) {
        ...
        try {
            if (signedjar != null &&
                new File(signedjar).getCanonicalPath().equals(new File(jarfile).getCanonicalPath())) {
                signedjar = null;
            }
        ...
        if (token && keypass != null) {
            System.err.println(
                MessageFormat.format(rb.getString(".keypass.can.not.be.specified.if.storetype.is.{0}"), storetype));
            usage();
        }
        ...
        if (protectedPath) {
            if (storepass != null || keypass != null) {
                System.err.println(
                    rb.getString("If.protected.is.specified.then.storepass.and.keypass.must.not.be.specified"));
                usage();
            }
        }
        ...
        try {
            ...
            if (man != null) {
                ...
                while (e.hasMoreElements()) {
                    ...
                    if (verbose != null) {
                        sb = new StringBuffer();
                        boolean inManifest = ((man.getAttributes(name) != null) ||
                        ...
                        sb.append((isSigned ? rb.getString("s") : rb.getString("SPACE")) +
                            ...
                            ((inStoreOrScope & NOT_ALIAS) != 0 ? "X" : " ") + rb.getString("SPACE"));
                        sb.append("|");
                    }
                ...
                for (Entry<String, List<String>> s : output.entrySet()) {
                    ...
                    if (verbose.equals("all")) {
                        ...
                        if (verbose.equals("grouped")) {
                            ...
                            if (files.size() > 1) {
                                System.out.println(
                                    files.get(0) + " " + String.format(rb.getString(".and.d.more."), files.size() - 1));
                            } else {
            ...
            if (!anySigned) {
                ...
                if (badKeyUsage || badExtendedKeyUsage || badNetscapeCertType || notYetValidCert || chainNotValidated ||
                    hasExpiredCert || hasUnsignedEntry || aliasNotInStore || notSignedByAlias) {
                    ...
                    if (hasUnsignedEntry) {
                        System.out.println(
                            rb.getString("This.jar.contains.unsigned.entries.which.have.not.been.integrity.checked."));
                    }
                    if (hasExpiredCert) {
                        System.out
                            .println(rb.getString("This.jar.contains.entries.whose.signer.certificate.has.expired."));
                    }
                    if (notYetValidCert) {
                        System.out.println(
    ...
    String printCert(String tab, Certificate c, boolean checkValidityPeriod, Date timestamp, boolean checkUsage) {
        ...
        if (c instanceof X509Certificate) {
            ...
            certStr.append(tab).append(x509Cert.getType()).append(rb.getString("COMMA"))
        ...
        if (checkValidityPeriod && x509Cert != null) {
            ...
            if (checkUsage) {
                ...
                if (bad[0] || bad[1] || bad[2]) {
                    ...
                    certStr.append("\n").append(tab)
                        .append(MessageFormat.format(rb.getString(".{0}.extension.does.not.support.code.signing."), x));
                }
            }
        }
    ...
    void signJar(String jarName, String alias, String[] args) throws Exception {
            ...
            if (!((c >= 'A' && c <= 'Z') || (c >= '0' && c <= '9') || (c == '-') || (c == '_'))) {
                if (aliasUsed) {
                    ...
                    throw new RuntimeException(
                        rb.getString("signature.filename.must.consist.of.the.following.characters.A.Z.0.9.or."));
                }
            }
            tmpSigFile.append(c);
        }
        ...
        try {
            ...
            for (Enumeration<? extends ZipEntry> enum_ = zipFile.entries(); enum_.hasMoreElements(); ) {
                ...
                if (manifest.getAttributes(ze.getName()) != null) {
                    ...
                    if (updateDigests(ze, zipFile, digests, manifest) == true) {
                        mfModified = true;
                    }
            ...
            try {
                block = sf.generateBlock(privateKey, sigalg, certChain, externalSF, tsaUrl, tsaCert, tSAPolicyID,
                    signingMechanism, args, zipFile);
            } catch (SocketTimeoutException e) {
                // Provide a helpful message when TSA is beyond a firewall
                error(
                    rb.getString("unable.to.sign.jar.") + rb.getString("no.response.from.the.Timestamping.Authority.") +
            ...
            if (verbose != null) {
                ...
                if (tsaCert != null) {
                    ...
                    if (tsaURI != null) {
                        System.out.println(rb.getString("TSA.location.") + tsaURI);
                    }
                    System.out.println(rb.getString("TSA.certificate.") + printCert("", tsaCert, false, null, false));
                }
            ...
            for (int i = 0; i < mfFiles.size(); i++) {
                ...
                if (!ze.getName().equalsIgnoreCase(JarFile.MANIFEST_NAME) &&
                    !ze.getName().equalsIgnoreCase(sfFilename) && !ze.getName().equalsIgnoreCase(bkFilename)) {
            ...
            for (Enumeration<? extends ZipEntry> enum_ = zipFile.entries(); enum_.hasMoreElements(); ) {
                ...
                if (!ze.getName().startsWith(META_INF)) {
                    if (verbose != null) {
                        ...
                        } else { System.out.println(rb.getString(".adding.") + ze.getName()); }
                    }
                    writeEntry(zipFile, zos, ze);
                }
            }
                ...
                if (jarFile.renameTo(origJar)) {
                    if (signedJarFile.renameTo(jarFile)) {
                        ...
                        MessageFormat form = new MessageFormat(
                            rb.getString("attempt.to.rename.signedJarFile.to.jarFile.failed"));
        ...
        if (badKeyUsage || badExtendedKeyUsage || badNetscapeCertType || notYetValidCert || chainNotValidated ||
            ...
            if (badKeyUsage) {
                System.out
                    .println(rb.getString("The.signer.certificate.s.KeyUsage.extension.doesn.t.allow.code.signing."));
            }
        ...
        if (hasExpiringCert || noTimestamp) {
            ...
            if (noTimestamp) {
                System.out.println(String.format(rb.getString("no.timestamp.signing"), expireDate));
            }
        }
    ...
    private String signerInfo(CodeSigner signer, String tab) {
        ...
        try {
            ...
            if (e.getCause() != null && (e.getCause() instanceof CertificateExpiredException ||
    ...
    void getAliasInfo(String alias) {
        ...
        try {
            ...
            try {
                if (!token && keypass == null) { key = store.getKey(alias, storepass); } else {
     ...
     */
    private synchronized String[] getDigests(ZipEntry ze, ZipFile zf, MessageDigest[] digests) throws IOException {
...
class SignatureFile {
    ...
    public SignatureFile(MessageDigest digests[], Manifest mf, ManifestDigester md, String baseName,
        ...
        if (mde != null) {
            for (int i = 0; i < digests.length; i++) {
                mattr.putValue(digests[i].getAlgorithm() + "-Digest-" + ManifestDigester.MF_MAIN_ATTRS,
     ...
     */
    public Block generateBlock(PrivateKey privateKey, String sigalg, X509Certificate[] certChain, boolean externalSF,
        String tsaUrl, X509Certificate tsaCert, String tSAPolicyID, ContentSigner signingMechanism, String[] args,
        ZipFile zipFile)
        throws NoSuchAlgorithmException, InvalidKeyException, IOException, SignatureException, CertificateException {
        return new Block(this, privateKey, sigalg, certChain, externalSF, tsaUrl, tsaCert, tSAPolicyID,
            signingMechanism, args, zipFile);
    }
    ...
    public static class Block {
         ...
         */
        Block(SignatureFile sfg, PrivateKey privateKey, String sigalg, X509Certificate[] certChain, boolean externalSF,
            String tsaUrl, X509Certificate tsaCert, String tSAPolicyID, ContentSigner signingMechanism, String[] args,
            ZipFile zipFile) throws NoSuchAlgorithmException, InvalidKeyException, IOException, SignatureException,
            ...
            if (!(issuerName instanceof X500Name)) {
                ...
                issuerName = (Principal) tbsCert.get(X509CertInfo.ISSUER + "." + X509CertInfo.DN_NAME);
            }
            ...
            if (sigalg == null) {
                ...
                if (keyAlgorithm.equalsIgnoreCase("DSA")) { signatureAlgorithm = "SHA1withDSA"; } else if (keyAlgorithm
                    .equalsIgnoreCase("RSA")) {
                    signatureAlgorithm = "SHA256withRSA";
                } else if (keyAlgorithm.equalsIgnoreCase("EC")) { signatureAlgorithm = "SHA256withECDSA"; } else {
            ...
            if ((sigAlgUpperCase.endsWith("WITHRSA") && !keyAlgorithm.equalsIgnoreCase("RSA")) ||
                (sigAlgUpperCase.endsWith("WITHECDSA") && !keyAlgorithm.equalsIgnoreCase("EC")) ||
                (sigAlgUpperCase.endsWith("WITHDSA") && !keyAlgorithm.equalsIgnoreCase("DSA"))) {
            ...
            ContentSignerParameters params = new JarSignerParameters(args, tsaUri, tsaCert, tSAPolicyID, signature,
...
class JarSignerParameters implements ContentSignerParameters {
     ...
     * Create a new object.
     */
    JarSignerParameters(String[] args, URI tsa, X509Certificate tsaCertificate, String tSAPolicyID, byte[] signature,
        String signatureAlgorithm, X509Certificate[] signerCertificateChain, byte[] content, ZipFile source) {
