...
public class PolicyInformation {
    ...
    public PolicyInformation(DerValue val) throws IOException {
        ...
        if (val.data.available() != 0) {
            ...
            if (opt.tag != DerValue.tag_Sequence) { throw new IOException("Invalid encoding of PolicyInformation"); }
            if (opt.data.available() == 0) { throw new IOException("No data available in policyQualifiers"); }
    ...
    public void set(String name, Object obj) throws IOException {
        if (name.equalsIgnoreCase(ID)) {
            if (obj instanceof CertificatePolicyId) { policyIdentifier = (CertificatePolicyId) obj; } else {
                throw new IOException("Attribute value must be instance " + "of CertificatePolicyId.");
            }
        } else if (name.equalsIgnoreCase(QUALIFIERS)) {
            if (policyIdentifier == null) {
                throw new IOException("Attribute must have a " + "CertificatePolicyIdentifier value before " +
            ...
            if (obj instanceof Set) {
                ...
                while (i.hasNext()) {
                    ...
                    if (!(obj1 instanceof PolicyQualifierInfo)) {
                        throw new IOException("Attribute value must be a" + "Set of PolicyQualifierInfo objects.");
                    }
                }
