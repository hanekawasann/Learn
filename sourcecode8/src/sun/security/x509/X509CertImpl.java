...
public class X509CertImpl extends X509Certificate implements DerEncoder {
    ...
    public static final String SUBJECT_DN = NAME + DOT + INFO + DOT + X509CertInfo.SUBJECT + DOT + X509CertInfo.DN_NAME;
    // x509.info.issuer.dname
    public static final String ISSUER_DN = NAME + DOT + INFO + DOT + X509CertInfo.ISSUER + DOT + X509CertInfo.DN_NAME;
    // x509.info.serialNumber.number
    public static final String SERIAL_ID = NAME + DOT + INFO + DOT + X509CertInfo.SERIAL_NUMBER + DOT +
    ...
    public static final String VERSION = NAME + DOT + INFO + DOT + X509CertInfo.VERSION + DOT +
        ...
        try {
            ...
            try {
                ...
                throw new CertificateException(
                    "Input stream must be " + "either DER-encoded bytes " + "or RFC1421 hex-encoded " +
                        "DER-encoded bytes: " + ioe1.getMessage(), ioe1);
            }
        }
        try {
            parse(der);
     ...
     */
    public static void verify(X509Certificate cert, PublicKey key, Provider sigProvider)
        throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
    ...
    public void sign(PrivateKey key, String algorithm, String provider)
        ...
        try {
            ...
            if ((provider == null) || (provider.length() == 0)) { sigEngine = Signature.getInstance(algorithm); } else {
    ...
    public Object get(String name) throws CertificateParsingException {
        ...
        if (!(id.equalsIgnoreCase(NAME))) {
            throw new CertificateParsingException(
                "Invalid root of " + "attribute name, expected [" + NAME + "], received " + "[" + id + "]");
        }
    ...
    public void set(String name, Object obj) throws CertificateException, IOException {
        ...
        if (id.equalsIgnoreCase(INFO)) {
            ...
            throw new CertificateException(
                "Attribute name not recognized or " + "set() not allowed for the same: " + id);
        }
    }
    ...
    public void delete(String name) throws CertificateException, IOException {
        ...
        if (id.equalsIgnoreCase(INFO)) {
            ...
            throw new CertificateException(
                "Attribute name not recognized or " + "delete() not allowed for the same: " + id);
        }
    }
    ...
    public SerialNumber getSerialNumberObject() {
        if (info == null) { return null; }
        try {
            SerialNumber ser = (SerialNumber) info
                .get(CertificateSerialNumber.NAME + DOT + CertificateSerialNumber.NUMBER);
            return ser;
        } catch (Exception e) {
            return null;
        }
    ...
    public Principal getSubjectDN() {
        if (info == null) { return null; }
        try {
            Principal subject = (Principal) info.get(X509CertInfo.SUBJECT + DOT + X509CertInfo.DN_NAME);
            return subject;
        } catch (Exception e) {
            return null;
        }
    ...
    public X500Principal getSubjectX500Principal() {
        ...
        try {
            X500Principal subject = (X500Principal) info.get(X509CertInfo.SUBJECT + DOT + "x500principal");
            return subject;
        } catch (Exception e) {
            return null;
        }
    ...
    public Principal getIssuerDN() {
        if (info == null) { return null; }
        try {
            Principal issuer = (Principal) info.get(X509CertInfo.ISSUER + DOT + X509CertInfo.DN_NAME);
            return issuer;
        } catch (Exception e) {
            return null;
        }
    ...
    public X500Principal getIssuerX500Principal() {
        ...
        try {
            X500Principal issuer = (X500Principal) info.get(X509CertInfo.ISSUER + DOT + "x500principal");
            return issuer;
        } catch (Exception e) {
            return null;
        }
    ...
    public boolean[] getIssuerUniqueID() {
        if (info == null) { return null; }
        try {
            UniqueIdentity id = (UniqueIdentity) info.get(X509CertInfo.ISSUER_ID);
    ...
    public boolean[] getSubjectUniqueID() {
        if (info == null) { return null; }
        try {
            UniqueIdentity id = (UniqueIdentity) info.get(X509CertInfo.SUBJECT_ID);
            if (id == null) { return null; } else { return (id.getId()); }
        } catch (Exception e) {
            return null;
        }
    ...
    public KeyIdentifier getSubjectKeyId() {
        ...
        if (ski != null) {
            try {
                return (KeyIdentifier) ski.get(SubjectKeyIdentifierExtension.KEY_ID);
            } catch (IOException ioe) {} // not possible
        }
        return null;
    }
    ...
    private void parse(DerValue val) throws CertificateException, IOException {
        ...
        AlgorithmId infoSigAlg = (AlgorithmId) info
            .get(CertificateAlgorithmId.NAME + DOT + CertificateAlgorithmId.ALGORITHM);
    ...
    public String getFingerprint(String algorithm) {
        return fingerprints.computeIfAbsent(algorithm, x -> getCertificateFingerPrint(x));
    }
