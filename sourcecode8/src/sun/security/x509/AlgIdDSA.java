...
public final class AlgIdDSA extends AlgorithmId implements DSAParams {
    ...
    AlgIdDSA(DerValue val) throws IOException { super(val.getOID()); }

    /**
     * Construct an AlgIdDSA from an X.509 encoded byte array.
     */
    public AlgIdDSA(byte[] encodedAlg) throws IOException { super(new DerValue(encodedAlg).getOID()); }
    ...
    protected void decodeParams() throws IOException {
        if (params == null) { throw new IOException("DSA alg params are null"); }
        if (params.tag != DerValue.tag_Sequence) { throw new IOException("DSA alg parsing error"); }
