...
public class CertificateValidity implements CertAttrSet<String> {
    ...
    private void construct(DerValue derVal) throws IOException {
        ...
        if (derVal.data.available() == 0) { throw new IOException("No data encoded for CertificateValidity"); }

        DerInputStream derIn = new DerInputStream(derVal.toByteArray());
        DerValue[] seq = derIn.getSequence(2);
        if (seq.length != 2) { throw new IOException("Invalid encoding for CertificateValidity"); }
    ...
    public void encode(OutputStream out) throws IOException {
        ...
        } else { pair.putGeneralizedTime(notBefore); }
