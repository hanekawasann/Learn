...
public class X509CRLImpl extends X509CRL implements DerEncoder {
    ...
    public void encodeInfo(OutputStream out) throws CRLException {
        try {
            ...
            if (thisUpdate.getTime() < YR_2050) { tmp.putUTCTime(thisUpdate); } else {
                tmp.putGeneralizedTime(thisUpdate);
            }

            if (nextUpdate != null) {
                if (nextUpdate.getTime() < YR_2050) { tmp.putUTCTime(nextUpdate); } else {
            ...
            if (extensions != null) { extensions.encode(tmp, isExplicit); }
     ...
     */
    public static void verify(X509CRL crl, PublicKey key, Provider sigProvider)
        throws CRLException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
     ...
     * @throws NoSuchProviderException  on incorrect provider.
     * @throws SignatureException       on signature errors.
     * @throws CRLException             if any mandatory data was omitted.
     */
    ...
    public byte[] getTBSCertList() throws CRLException {
        if (tbsCertList == null) { throw new CRLException("Uninitialized CRL"); }
    ...
    public KeyIdentifier getAuthKeyId() throws IOException {
        ...
        if (aki != null) {
            KeyIdentifier keyId = (KeyIdentifier) aki.get(AuthorityKeyIdentifierExtension.KEY_ID);
            return keyId;
        } else {
            return null;
        }
    ...
    public void derEncode(OutputStream out) throws IOException {
        if (signedCRL == null) { throw new IOException("Null CRL to encode"); }
     ...
     */
    private final static class X509IssuerSerial implements Comparable<X509IssuerSerial> {
