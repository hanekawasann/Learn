...
public class CRLExtensions {
    ...
    private Map<String, Extension> map = Collections.synchronizedMap(new TreeMap<String, Extension>());
    ...
    private void parseExtension(Extension ext) throws CRLException {
        try {
            ...
            if (extClass == null) {   // Unsupported extension
                if (ext.isCritical()) { unsupportedCritExt = true; }
    ...
    public void encode(OutputStream out, boolean isExplicit) throws CRLException {
        try {
            ...
            for (int i = 0; i < objs.length; i++) {
                ...
                } else if (objs[i] instanceof Extension) { ((Extension) objs[i]).encode(extOut); } else {
            ...
            if (isExplicit) { tmp.write(DerValue.createTag(DerValue.TAG_CONTEXT, true, (byte) 0), seq); } else {
                tmp = seq;
            }
