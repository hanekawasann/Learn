    ...
    public AuthorityKeyIdentifierExtension(Boolean critical, Object value) throws IOException {
        ...
        while ((val.data != null) && (val.data.available() != 0)) {
            ...
            if (opt.isContextSpecific(TAG_ID) && !opt.isConstructed()) {
                if (id != null) { throw new IOException("Duplicate KeyIdentifier in " + "AuthorityKeyIdentifier."); }
                opt.resetTag(DerValue.tag_OctetString);
                id = new KeyIdentifier(opt);

            } else if (opt.isContextSpecific(TAG_NAMES) && opt.isConstructed()) {
                if (names != null) { throw new IOException("Duplicate GeneralNames in " + "AuthorityKeyIdentifier."); }
                opt.resetTag(DerValue.tag_Sequence);
                names = new GeneralNames(opt);

            } else if (opt.isContextSpecific(TAG_SERIAL_NUM) && !opt.isConstructed()) {
                if (serialNum != null) {
            ...
            } else { throw new IOException("Invalid encoding of " + "AuthorityKeyIdentifierExtension."); }
        }
    }
