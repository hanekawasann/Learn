...
public class DNSName implements GeneralNameInterface {
    ...
    public DNSName(String name) throws IOException {
        if (name == null || name.length() == 0) { throw new IOException("DNS name must not be null"); }
        if (name.indexOf(' ') != -1) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof DNSName)) { return false; }
    ...
    public int constrains(GeneralNameInterface inputName) throws UnsupportedOperationException {
        int constraintType;
        if (inputName == null) { constraintType = NAME_DIFF_TYPE; } else if (inputName.getType() != NAME_DNS) {
            constraintType = NAME_DIFF_TYPE;
        } else {
            String inName = (((DNSName) inputName).getName()).toLowerCase(Locale.ENGLISH);
            String thisName = name.toLowerCase(Locale.ENGLISH);
            if (inName.equals(thisName)) { constraintType = NAME_MATCH; } else if (thisName.endsWith(inName)) {
                int inNdx = thisName.lastIndexOf(inName);
                if (thisName.charAt(inNdx - 1) == '.') { constraintType = NAME_WIDENS; } else {
                    constraintType = NAME_SAME_TYPE;
                }
            } else if (inName.endsWith(thisName)) {
                int ndx = inName.lastIndexOf(thisName);
                if (inName.charAt(ndx - 1) == '.') { constraintType = NAME_NARROWS; } else {
                    constraintType = NAME_SAME_TYPE;
                }
            } else {
                constraintType = NAME_SAME_TYPE;
