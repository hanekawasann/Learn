...
public final class PKCS12KeyStore extends KeyStoreSpi {
    ...
    private static final String[] KEY_PROTECTION_ALGORITHM = { "keystore.pkcs12.keyProtectionAlgorithm",
        "keystore.PKCS12.keyProtectionAlgorithm" };

    // friendlyName, localKeyId, trustedKeyUsage
    private static final String[] CORE_ATTRIBUTES = { "1.2.840.113549.1.9.20", "1.2.840.113549.1.9.21",
    ...
    private static class CertEntry extends Entry {
        ...
        CertEntry(X509Certificate cert, byte[] keyId, String alias, ObjectIdentifier[] trustedKeyUsage,
     ...
     */
    private Map<String, Entry> entries = Collections.synchronizedMap(new LinkedHashMap<String, Entry>());
    ...
    public Key engineGetKey(String alias, char[] password) throws NoSuchAlgorithmException, UnrecoverableKeyException {
        ...
        try {
            ...
            if (entry instanceof PrivateKeyEntry) {
                ...
                if (debug != null) {
                    debug.println(
                        "Retrieved a protected private key (" + key.getClass().getName() + ") at alias '" + alias +
                            "'");
                }
                ...
                if (debug != null) {
                    debug.println(
                        "Retrieved a protected secret key (" + key.getClass().getName() + ") at alias '" + alias + "'");
                }
            }
        } catch (Exception e) {
    ...
    public Certificate[] engineGetCertificateChain(String alias) {
        ...
        if (entry != null && entry instanceof PrivateKeyEntry) {
            if (((PrivateKeyEntry) entry).chain == null) {
                ...
                if (debug != null) {
                    debug.println(
                        "Retrieved a " + ((PrivateKeyEntry) entry).chain.length + "-certificate chain at alias '" +
                            alias + "'");
                }
     ...
     */
    private void setKeyEntry(String alias, Key key, KeyStore.PasswordProtection passwordProtection, Certificate[] chain,
        ...
        try {
            ...
            if (key instanceof PrivateKey) {
                ...
                if ((key.getFormat().equals("PKCS#8")) || (key.getFormat().equals("PKCS8"))) {
                    ...
                    if (debug != null) {
                        debug.println(
                            "Setting a protected private key (" + key.getClass().getName() + ") at alias '" + alias +
                                "'");
                    }
                ...
                if (debug != null) {
                    debug.println(
                        "Setting a protected secret key (" + key.getClass().getName() + ") at alias '" + alias + "'");
                }
                secretKeyCount++;
                entry = keyEntry;
    ...
    public synchronized void engineSetKeyEntry(String alias, byte[] key, Certificate[] chain) throws KeyStoreException {
        ...
        try {
            ...
            throw new KeyStoreException("Private key is not stored" + " as PKCS#8 EncryptedPrivateKeyInfo: " + ioe,
                ioe);
        }
    ...
    private byte[] encryptPrivateKey(byte[] data, KeyStore.PasswordProtection passwordProtection)
        ...
        try {
            ...
            UnrecoverableKeyException uke = new UnrecoverableKeyException(
                "Encrypt Private Key failed: " + e.getMessage());
            uke.initCause(e);
            throw uke;
        }
     ...
     */
    private static String mapPBEParamsToAlgorithm(ObjectIdentifier algorithm, AlgorithmParameters algParams)
        ...
        if (protParam instanceof KeyStore.PasswordProtection) {
            if (engineIsCertificateEntry(alias)) {
                ...
                if (key instanceof PrivateKey) {
                    ...
                    return new KeyStore.PrivateKeyEntry((PrivateKey) key, chain, getAttributes(entry));
                ...
                throw new UnsupportedOperationException(
                    "untrusted certificate entries are not " + "password-protected");
            }
        }
    ...
    public synchronized void engineSetEntry(String alias, KeyStore.Entry entry, KeyStore.ProtectionParameter protParam)
        ...
        if (entry instanceof KeyStore.TrustedCertificateEntry) {
            if (protParam != null && pProtect.getPassword() != null) {
                ...
                setCertEntry(alias, tce.getTrustedCertificate(), tce.getAttributes());
            ...
            if (pProtect == null || pProtect.getPassword() == null) {
                ...
                setKeyEntry(alias, pke.getPrivateKey(), pProtect, pke.getCertificateChain(), pke.getAttributes());
            ...
            if (pProtect == null || pProtect.getPassword() == null) {
                ...
                setKeyEntry(alias, ske.getSecretKey(), pProtect, (Certificate[]) null, ske.getAttributes());
        ...
        if (entry instanceof CertEntry) {
            ...
            if (trustedKeyUsageValue != null) {
                if (trustedKeyUsageValue.length == 1) { // omit brackets
                    entry.attributes
                        .add(new PKCS12Attribute(TrustedKeyUsage_OID.toString(), trustedKeyUsageValue[0].toString()));
                } else { // multi-valued
                    entry.attributes.add(
                        new PKCS12Attribute(TrustedKeyUsage_OID.toString(), Arrays.toString(trustedKeyUsageValue)));
                }
            }
        }
    ...
    private byte[] getBagAttributes(String alias, byte[] keyId, ObjectIdentifier[] trustedUsage,
        ...
        if (attributes != null) {
            for (KeyStore.Entry.Attribute attribute : attributes) {
                ...
                if (CORE_ATTRIBUTES[0].equals(attributeName) || CORE_ATTRIBUTES[1].equals(attributeName) ||
            ...
            for (int i = 0; i < chainLen; i++) {
                ...
                if (i == 0) {
                    ...
                    if (entry instanceof KeyEntry) {
                        ...
                        bagAttrs = getBagAttributes(certEntry.alias, certEntry.keyId, certEntry.trustedKeyUsage,
            ...
            if (keyEntry instanceof PrivateKeyEntry) {
                ...
                try {
                    ...
                    throw new IOException(
                        "Private key not stored as " + "PKCS#8 EncryptedPrivateKeyInfo" + ioe.getMessage());
                }
    ...
    private X509Certificate findMatchedCertificate(PrivateKeyEntry entry) {
        ...
        if (keyIdMatch != null) { return keyIdMatch.cert; } else if (aliasMatch != null) {
