...
public class PKCS8Key implements PrivateKey {
    ...
    public static PKCS8Key parse(DerValue in) throws IOException {
        ...
        key = parseKey(in);
        if (key instanceof PKCS8Key) { return (PKCS8Key) key; }
    ...
    public static PrivateKey parseKey(DerValue in) throws IOException {
        ...
        if (in.tag != DerValue.tag_Sequence) { throw new IOException("corrupt private key"); }

        BigInteger parsedVersion = in.data.getBigInteger();
        if (!version.equals(parsedVersion)) {
            throw new IOException("version mismatch: (supported: " + Debug.toHexString(version) + ", parsed: " +
        ...
        if (in.data.available() != 0) { throw new IOException("excess private key"); }
        return privKey;
    }
        ...
        try {
            ...
            if (!version.equals(PKCS8Key.version)) {
                throw new IOException(
                    "version mismatch: (supported: " + Debug.toHexString(PKCS8Key.version) + ", parsed: " +
