...
public class EncryptedPrivateKeyInfo {
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof EncryptedPrivateKeyInfo)) { return false; }
        try {
            byte[] thisEncrInfo = this.getEncoded();
            byte[] otherEncrInfo = ((EncryptedPrivateKeyInfo) other).getEncoded();

            if (thisEncrInfo.length != otherEncrInfo.length) { return false; }
            for (int i = 0; i < thisEncrInfo.length; i++) { if (thisEncrInfo[i] != otherEncrInfo[i]) { return false; } }
            return true;
        } catch (IOException e) {
            return false;
        }
    ...
    public int hashCode() {
        ...
        for (int i = 0; i < this.encryptedData.length; i++) { retval += this.encryptedData[i] * i; }
        return retval;
    }
}
