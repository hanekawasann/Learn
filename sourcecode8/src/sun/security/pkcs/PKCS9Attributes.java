...
public class PKCS9Attributes {
        ...
        for (int i = 0; i < attribs.length; i++) {
            ...
            if (attributes.containsKey(oid)) {
                throw new IllegalArgumentException(
                    "PKCSAttribute " + attribs[i].getOID() + " duplicated while constructing " + "PKCS9Attributes.");
            }
    ...
    public Object getAttributeValue(String name) throws IOException {
        ...
        if (oid == null) { throw new IOException("Attribute name " + name + " not recognized or not supported."); }
    ...
    static DerEncoder[] castToDerEncoder(Object[] objs) {
        ...
        for (int i = 0; i < encoders.length; i++) { encoders[i] = (DerEncoder) objs[i]; }
