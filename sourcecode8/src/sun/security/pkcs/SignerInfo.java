...
public class SignerInfo implements DerEncoder {
    ...
    public SignerInfo(X500Name issuerName, BigInteger serial, AlgorithmId digestAlgorithmId,
    ...
    public SignerInfo(X500Name issuerName, BigInteger serial, AlgorithmId digestAlgorithmId,
        PKCS9Attributes authenticatedAttributes, AlgorithmId digestEncryptionAlgorithmId, byte[] encryptedDigest,
        PKCS9Attributes unauthenticatedAttributes) {
        this.version = BigInteger.ONE;
        this.issuerName = issuerName;
        this.certificateSerialNumber = serial;
    ...
    public SignerInfo(DerInputStream derin, boolean oldStyle) throws IOException, ParsingException {
        ...
        if (oldStyle) {
            ...
            if (derin.available() != 0 && (byte) (derin.peekByte()) == (byte) 0xA1) {
                unauthenticatedAttributes = new PKCS9Attributes(derin, true);// ignore unsupported attrs
            }
        }
    ...
    public Timestamp getTimestamp()
        ...
        if (timestamp != null || !hasTimestamp) { return timestamp; }

        if (unauthenticatedAttributes == null) {
            hasTimestamp = false;
            return null;
        }
        PKCS9Attribute tsTokenAttr = unauthenticatedAttributes
    ...
    private void verifyTimestamp(TimestampToken token) throws NoSuchAlgorithmException, SignatureException {
        ...
        if (!Arrays.equals(token.getHashedMessage(), md.digest(encryptedDigest))) {
            ...
            throw new SignatureException(
                "Signature timestamp (#" + token.getSerialNumber() + ") generated on " + token.getDate() +
                    " is inapplicable");
        }
        ...
        if (debug != null) {
            debug.println();
            debug.println(
                "Detected signature timestamp (#" + token.getSerialNumber() + ") generated on " + token.getDate());
            debug.println();
        }
    }
