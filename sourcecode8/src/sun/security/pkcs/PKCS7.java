...
public class PKCS7 {
     ...
     */
    public PKCS7(AlgorithmId[] digestAlgorithmIds, ContentInfo contentInfo, X509Certificate[] certificates,
        X509CRL[] crls, SignerInfo[] signerInfos) {
    ...
    private void parseNetscapeCertChain(DerValue val) throws ParsingException, IOException {
        ...
        for (int i = 0; i < contents.length; i++) {
            ...
            try {
                if (certfac == null) { certificates[i] = new X509CertImpl(contents[i]); } else {
    ...
    private void parseSignedData(DerValue val) throws ParsingException, IOException {
        ...
        if ((byte) (dis.peekByte()) == (byte) 0xA1) {
            ...
            for (int i = 0; i < len; i++) {
                ByteArrayInputStream bais = null;
                try {
                    if (certfac == null) { crls[i] = new X509CRLImpl(crlVals[i]); } else {
    ...
    private void parseOldSignedData(DerValue val) throws ParsingException, IOException {
        ...
        for (int i = 0; i < len; i++) {
            ByteArrayInputStream bais = null;
            try {
                if (certfac == null) { certificates[i] = new X509CertImpl(certVals[i]); } else {
                    byte[] encoded = certVals[i].toByteArray();
                    bais = new ByteArrayInputStream(encoded);
                    certificates[i] = (X509Certificate) certfac.generateCertificate(bais);
                    bais.close();
    ...
    public void encodeSignedData(DerOutputStream out) throws IOException {
        ...
        if (certificates != null && certificates.length != 0) {
            ...
            for (int i = 0; i < certificates.length; i++) {
                if (certificates[i] instanceof X509CertImpl) { implCerts[i] = (X509CertImpl) certificates[i]; } else {
                    try {
        ...
        if (crls != null && crls.length != 0) {
            ...
            for (X509CRL crl : crls) {
                if (crl instanceof X509CRLImpl) { implCRLs.add((X509CRLImpl) crl); } else {
                    try {
    ...
    public X509Certificate[] getCertificates() {
        if (certificates != null) { return certificates.clone(); } else { return null; }
    }
    ...
    public X509CRL[] getCRLs() {
        if (crls != null) { return crls.clone(); } else { return null; }
    }
     ...
     *                                  data message.
     */
    public static byte[] generateSignedData(byte[] signature, X509Certificate[] signerChain, byte[] content,
        String signatureAlgorithm, URI tsaURI, String tSAPolicyID)
        ...
        if (tsaURI != null) {
            ...
            unauthAttrs = new PKCS9Attributes(
                new PKCS9Attribute[] { new PKCS9Attribute(PKCS9Attribute.SIGNATURE_TIMESTAMP_TOKEN_STR, tsToken) });
        }
        ...
        SignerInfo signerInfo = new SignerInfo(issuerName, serialNumber, AlgorithmId.get(digAlg), null,
            AlgorithmId.get(encAlg), signature, unauthAttrs);

        // Create the PKCS #7 signed data message
        SignerInfo[] signerInfos = { signerInfo };
        AlgorithmId[] algorithms = { signerInfo.getDigestAlgorithmId() };
        // Include or exclude content
        ContentInfo contentInfo = (content == null) ? new ContentInfo(ContentInfo.DATA_OID, null)
     ...
     */
    private static byte[] generateTimestampToken(Timestamper tsa, String tSAPolicyID, byte[] toBeTimestamped)
        ...
        if (status != 0 && status != 1) {
            throw new IOException(
                "Error generating timestamp: " + tsReply.getStatusCodeAsText() + " " + tsReply.getFailureCodeAsText());
        }
