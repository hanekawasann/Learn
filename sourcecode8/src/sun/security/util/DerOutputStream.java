...
public class DerOutputStream extends ByteArrayOutputStream implements DerEncoder {
    ...
    private void putIntegerContents(int i) throws IOException {
        ...
        if (bytes[0] == (byte) 0xff) {
            ...
            for (int j = 0; j < 3; j++) {
                if ((bytes[j] == (byte) 0xff) && ((bytes[j + 1] & 0x80) == 0x80)) { start++; } else { break; }
            }
            ...
            for (int j = 0; j < 3; j++) {
                if ((bytes[j] == 0x00) && ((bytes[j + 1] & 0x80) == 0)) { start++; } else { break; }
            }
        }

        putLength(4 - start);
        for (int k = start; k < 4; k++) { write(bytes[k]); }
    }
    ...
    public void putSequence(DerValue[] seq) throws IOException {
        ...
        for (i = 0; i < seq.length; i++) { seq[i].encode(bytes); }
    ...
    public void putSet(DerValue[] set) throws IOException {
        ...
        for (i = 0; i < set.length; i++) { set[i].encode(bytes); }
