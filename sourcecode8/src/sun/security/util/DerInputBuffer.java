...
class DerInputBuffer extends ByteArrayInputStream implements Cloneable {
    ...
    public boolean equals(Object other) {
        if (other instanceof DerInputBuffer) { return equals((DerInputBuffer) other); } else { return false; }
    }
    ...
    boolean equals(DerInputBuffer other) {
        ...
        if (other.available() != max) { return false; }
        for (int i = 0; i < max; i++) {
    ...
    void truncate(int len) throws IOException {
        if (len > available()) { throw new IOException("insufficient data"); }
        count = pos + len;
    }
    ...
    BigInteger getBigInteger(int len, boolean makePositive) throws IOException {
        if (len > available()) { throw new IOException("short read of integer"); }
    ...
    public byte[] getBitString(int len) throws IOException {
        if (len > available()) { throw new IOException("short read of bit string"); }
    ...
    public Date getUTCTime(int len) throws IOException {
        if (len > available()) { throw new IOException("short read of DER UTC Time"); }

        if (len < 11 || len > 17) { throw new IOException("DER UTC Time length error"); }
    ...
    public Date getGeneralizedTime(int len) throws IOException {
        if (len > available()) { throw new IOException("short read of DER Generalized Time"); }

        if (len < 13 || len > 23) { throw new IOException("DER Generalized Time length error"); }
    ...
    private Date getTime(int len, boolean generalized) throws IOException {
         ...
         */
        if (!(len == 1 || len == 5)) { throw new IOException("Parse " + type + " time, invalid offset"); }
        ...
        switch (buf[pos++]) {
            case '+':
                ...
                if (hr >= 24 || min >= 60) { throw new IOException("Parse " + type + " time, +hhmm"); }
