...
public class DerValue {
    ...
    DerValue(DerInputBuffer in) throws IOException {
        ...
        if (length == -1) {  // indefinite length encoding found
            ...
            if (tag != inbuf.read()) { throw new IOException("Indefinite length encoding not supported"); }
    ...
    private DerInputStream init(boolean fullyBuffered, InputStream in) throws IOException {
        ...
        if (length == -1) { // indefinite length encoding found
            ...
            if (tag != in.read()) { throw new IOException("Indefinite length encoding not supported"); }
    ...
    public ObjectIdentifier getOID() throws IOException {
        if (tag != tag_ObjectId) { throw new IOException("DerValue.getOID, not an OID " + tag); }
    ...
    public byte[] getOctetString() throws IOException {
        ...
        if (buffer.read(bytes) != length) { throw new IOException("short read on DerValue buffer"); }
        if (isConstructed()) {
    ...
    public BigInteger getBigInteger() throws IOException {
        if (tag != tag_Integer) { throw new IOException("DerValue.getBigInteger, not an int " + tag); }
    ...
    public byte[] getBitString() throws IOException {
        if (tag != tag_BitString) { throw new IOException("DerValue.getBitString, not a bit string " + tag); }
    ...
    public String getAsString() throws IOException {
        ...
        */
        else if (tag == tag_BMPString) { return getBMPString(); } else if (tag == tag_GeneralString) {
    ...
    public String getPrintableString() throws IOException {
        if (tag != tag_PrintableString) { throw new IOException("DerValue.getPrintableString, not a string " + tag); }
    ...
    public String getT61String() throws IOException {
        if (tag != tag_T61String) { throw new IOException("DerValue.getT61String, not T61 " + tag); }
    ...
    public String getIA5String() throws IOException {
        if (tag != tag_IA5String) { throw new IOException("DerValue.getIA5String, not IA5 " + tag); }
    ...
    public String getBMPString() throws IOException {
        if (tag != tag_BMPString) { throw new IOException("DerValue.getBMPString, not BMP " + tag); }
    ...
    public String getUTF8String() throws IOException {
        if (tag != tag_UTF8String) { throw new IOException("DerValue.getUTF8String, not UTF-8 " + tag); }
    ...
    public String getGeneralString() throws IOException {
        if (tag != tag_GeneralString) { throw new IOException("DerValue.getGeneralString, not GeneralString " + tag); }
    ...
    public boolean equals(Object other) {
        if (other instanceof DerValue) { return equals((DerValue) other); } else { return false; }
    }
    ...
    public boolean equals(DerValue other) {
        ...
        return (System.identityHashCode(this.data) > System.identityHashCode(other.data)) ? doEquals(this, other)
            : doEquals(other, this);
    }
    ...
    public String toString() {
        try {
            ...
            if (tag == tag_Null) { return "[DerValue, null]"; }
            if (tag == tag_ObjectId) { return "OID." + getOID(); }

            // integers
            else { return "[DerValue, tag = " + tag + ", length = " + length + "]"; }
        } catch (IOException e) {
    ...
    public DerInputStream toDerInputStream() throws IOException {
        if (tag == tag_Sequence || tag == tag_Set) { return new DerInputStream(buffer); }
