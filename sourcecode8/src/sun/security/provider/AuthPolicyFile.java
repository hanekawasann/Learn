...
public class AuthPolicyFile extends javax.security.auth.Policy {
    ...
    private KeyStore initKeyStore(URL policyUrl, String keyStoreName, String keyStoreType) {
        if (keyStoreName != null) {
            try {
                ...
                InputStream inStream = new BufferedInputStream(PolicyUtil.getInputStream(keyStoreUrl));

                KeyStore ks;
                if (keyStoreType != null) { ks = KeyStore.getInstance(keyStoreType); } else {
        ...
        try {
            ...
            while (enum_.hasMoreElements()) {
                ...
                try {
                    ...
                    System.err.println(AUTH_POLICY + rb.getString(".error.adding.Permission.") + pe.permission +
                        rb.getString("SPACE") + ite.getTargetException());
                } catch (Exception e) {
                    System.err.println(AUTH_POLICY + rb.getString(".error.adding.Permission.") + pe.permission +
                        rb.getString("SPACE") + e);
                }
            }
    ...
    public PermissionCollection getPermissions(final Subject subject, final CodeSource codesource) {
        ...
        return AccessController.doPrivileged(new PrivilegedAction<PermissionCollection>() {
            ...
            public PermissionCollection run() {
                SubjectCodeSource scs = new SubjectCodeSource(subject, null,
    ...
    private boolean addSelfPermissions(final Permission p, CodeSource entryCs, CodeSource accCs, Permissions perms) {
        ...
        if (pPrincipals.length <= 0 || !pPrincipals[0][0].equalsIgnoreCase("self") ||
            ...
            for (PrincipalEntry principal : scs.getPrincipals()) {
                ...
                for (int i = 0; i < principalInfo.length; i++) {
                    ...
                    PrivateCredentialPermission newPcp = new PrivateCredentialPermission(
                        pcp.getCredentialClass() + " " + principalInfo[i][0] + " " + "\"" + principalInfo[i][1] + "\"",
                        "read");
     ...
     */
    private String[][] getPrincipalInfo(PrincipalEntry principal, final CodeSource accCs) {
        ...
        if (!principal.getPrincipalClass().equals(PrincipalEntry.WILDCARD_CLASS) &&
            !principal.getPrincipalName().equals(PrincipalEntry.WILDCARD_NAME)) {
        ...
        } else if (!principal.getPrincipalClass().equals(PrincipalEntry.WILDCARD_CLASS) &&
            principal.getPrincipalName().equals(PrincipalEntry.WILDCARD_NAME)) {
            ...
            try {
                ...
                Class<? extends Principal> pClass = (Class<? extends Principal>) Class
                    .forName(principal.getPrincipalClass(), false, ClassLoader.getSystemClassLoader());
                principalSet = scs.getSubject().getPrincipals(pClass);
            } catch (Exception e) {
                if (debug != null) {
                    debug.println(
                        "problem finding Principal Class " + "when expanding SELF permission: " + e.toString());
                }
            }
            ...
            try {
                ...
                if (cs instanceof SubjectCodeSource) {
                    ...
                    if (extractSignerCerts) {
                        canonCs = new SubjectCodeSource(scs.getSubject(), scs.getPrincipals(), csUrl,
                ...
                if (extractSignerCerts) {
                    if (!(cs instanceof SubjectCodeSource)) {
                        ...
                        SubjectCodeSource scs = (SubjectCodeSource) cs;
                        canonCs = new SubjectCodeSource(scs.getSubject(), scs.getPrincipals(), scs.getLocation(),
                            getSignerCertificates(scs));
                    }
                }
            }
...
class PolicyPermissions extends PermissionCollection {
    ...
    PolicyPermissions(AuthPolicyFile policy, CodeSource codesource) {
    ...
    public void add(Permission permission) {
        if (isReadOnly()) {
            throw new SecurityException(
                AuthPolicyFile.rb.getString("attempt.to.add.a.Permission.to.a.readonly.PermissionCollection"));
        }
