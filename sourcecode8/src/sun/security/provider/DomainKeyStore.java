...
abstract class DomainKeyStore extends KeyStoreSpi {
    ...
    public void engineSetKeyEntry(String alias, Key key, char[] password, Certificate[] chain)
        ...
        AbstractMap.SimpleEntry<String, AbstractMap.SimpleEntry<String, KeyStore>> pair = getKeystoreForWriting(alias);
    ...
    private AbstractMap.SimpleEntry<String, Collection<KeyStore>> getKeystoresForReading(String alias) {
        ...
        if (splits.length == 2) { // prefixed alias
            ...
            if (keystore != null) {
                return new AbstractMap.SimpleEntry<>(splits[1], (Collection<KeyStore>) Collections.singleton(keystore));
            }
        ...
        return new AbstractMap.SimpleEntry<>("", (Collection<KeyStore>) Collections.<KeyStore>emptyList());
    }
     ...
     */
    private AbstractMap.SimpleEntry<String, AbstractMap.SimpleEntry<String, KeyStore>> getKeystoreForWriting(
        String alias) {
        ...
        if (splits.length == 2) { // prefixed alias
            KeyStore keystore = keystores.get(splits[0]);
            if (keystore != null) {
                return new AbstractMap.SimpleEntry<>(splits[1], new AbstractMap.SimpleEntry<>(splits[0], keystore));
            }
        }
        return null;
    }
    ...
    public void engineStore(KeyStore.LoadStoreParameter param)
        ...
        if (param instanceof DomainLoadStoreParameter) {
            ...
            List<KeyStoreBuilderComponents> builders = getBuilders(domainParameter.getConfiguration(),
            ...
            for (KeyStoreBuilderComponents builder : builders) {
                ...
                try {
                    ...
                    if (!(pp instanceof KeyStore.PasswordProtection)) {
                        throw new KeyStoreException(new IllegalArgumentException(
            ...
            for (KeyStoreBuilderComponents builder : builders) {
                ...
                try {
                    ...
                    if (builder.file != null) {
                        keystores.put(builder.name, KeyStore.Builder
                            .newInstance(builder.type, builder.provider, builder.file, builder.protection)
                            .getKeyStore());
                    } else {
    ...
    private List<KeyStoreBuilderComponents> getBuilders(URI configuration,
        Map<String, KeyStore.ProtectionParameter> passwords) throws IOException {
        ...
        try (InputStreamReader configurationReader = new InputStreamReader(
            ...
            for (PolicyParser.KeyStoreEntry keystore : keystores) {
                ...
                if (properties.containsKey(KEYSTORE_URI)) {
                    ...
                    try {
                        ...
                        throw new IOException("Error processing keystore property: " + "keystoreURI=\"" + uri + "\"",
                            e);
                    }
                }
                ...
                builders.add(new KeyStoreBuilderComponents(keystoreName, keystoreType, keystoreProvider, keystoreFile,
                    keystoreProtection));
            }
    ...
    class KeyStoreBuilderComponents {
        ...
        KeyStoreBuilderComponents(String name, String type, Provider provider, File file,
            KeyStore.ProtectionParameter protection) {
            this.name = name;
            this.type = type;
