...
public class PolicyParser {
                ...
                if (de != null) {
                    ...
                    if (!domainEntries.containsKey(domainName)) {
                        ...
                        MessageFormat form = new MessageFormat(
                            ResourcesMgr.getString("duplicate.keystore.domain.name"));
        ...
        while (!peek("{")) {
            ...
            if (peekAndMatch("Codebase")) {
                ...
                while (aliases.hasMoreTokens()) {
                    ...
                    if (alias.equals(",")) { cctr++; } else if (alias.length() > 0) { actr++; }
                }
                if (actr <= cctr) {
                ...
                if (peek("\"")) {
                    ...
                    if (principalClass.equals(PrincipalEntry.WILDCARD_CLASS) &&
                        ...
                        throw new ParsingException(st.lineno(), ResourcesMgr
                            .getString("can.not.specify.Principal.with.a.wildcard.class.without.a.wildcard.name"));
                    }
                }
                ...
                throw new ParsingException(st.lineno(),
                    ResourcesMgr.getString("expected.codeBase.or.SignedBy.or.Principal"));
            }
        }
                ...
                if ((es = e.codeBase.indexOf(EXTDIRS_EXPANSION)) < 0) {
                    ...
                    if (extDirs != null && extDirs.length > 0) {
                        for (int i = 0; i < extDirs.length; i++) {
                            ...
                            if (debug != null) {
                                debug.println(
                                    "creating policy entry for " + "expanded java.ext.dirs path:\n\t\t" + extDirs[i]);
                            }
                        }
                    }
                    ignoreEntry = true;
        ...
        switch (lookahead) {
            case StreamTokenizer.TT_NUMBER:
                throw new ParsingException(st.lineno(), expect,
                    ResourcesMgr.getString("number.") + String.valueOf(st.nval));
            ...
            case StreamTokenizer.TT_WORD:
                if (expect.equalsIgnoreCase(st.sval)) {
                    ...
                    value = st.sval;
                    lookahead = st.nextToken();
                } else if (expect.equalsIgnoreCase("domain name") || expect.equalsIgnoreCase("keystore name") ||
            ...
            case '"':
                ...
                break;
            case ',':
                if (expect.equalsIgnoreCase(",")) { lookahead = st.nextToken(); } else {
                    throw new ParsingException(st.lineno(), expect, ",");
                }
                break;
            case '{':
    ...
    public static class PrincipalEntry implements Principal {
        ...
        public String getDisplayName(boolean addQuote) {
            if (isWildcardName()) {
                ...
                if (addQuote) { return "\"" + principalName + "\""; } else { return principalName; }
            }
        }
    ...
    static class DomainEntry {
        ...
        void add(KeyStoreEntry entry) throws ParsingException {
            ...
            if (!entries.containsKey(keystoreName)) {
                ...
                MessageFormat form = new MessageFormat(ResourcesMgr.getString("duplicate.keystore.name"));
    ...
    public static void main(String arg[]) throws Exception {
        try (FileReader fr = new FileReader(arg[0]); FileWriter fw = new FileWriter(arg[1])) {
