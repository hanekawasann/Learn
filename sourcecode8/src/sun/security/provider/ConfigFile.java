    ...
    public final static class Spi extends ConfigurationSpi {
        ...
        private void init(URL config, Map<String, List<AppConfigurationEntry>> newConfig) throws IOException {
            ...
            try (InputStreamReader isr = new InputStreamReader(getInputStream(config), "UTF-8")) {
                ...
                throw new IOException(ResourcesMgr
                    .getString("Configuration.Error.No.such.file.or.directory", "sun.security.util.AuthResources"));
            }
        }
        ...
        private void parseLoginEntry(Map<String, List<AppConfigurationEntry>> newConfig) throws IOException {
            ...
            if (newConfig.containsKey(appName)) {
                throw ioException("Configuration.Error.Can.not.specify.multiple.entries.for.appName", appName);
            }
        ...
        private String match(String expect) throws IOException {
            ...
            switch (lookahead) {
                ...
                case StreamTokenizer.TT_WORD:
                    if (expect.equalsIgnoreCase("module class name") || expect.equalsIgnoreCase("controlFlag") ||
                        expect.equalsIgnoreCase("option key") || expect.equalsIgnoreCase("option value")) {
                        value = st.sval;
                        lookahead = nextToken();
                    } else {
                        throw ioException("Configuration.Error.Line.line.expected.expect.found.value.",
                ...
                case '{':
                    if (expect.equalsIgnoreCase("{")) {
                        ...
                        throw ioException("Configuration.Error.Line.line.expected.expect.", new Integer(linenum),
                            expect, st.sval);
                    }
                    break;
        ...
        private String expand(String value) throws PropertyExpander.ExpandException, IOException {
            ...
            if (s == null || s.length() == 0) {
                throw ioException("Configuration.Error.Line.line.system.property.value.expanded.to.empty.value",
