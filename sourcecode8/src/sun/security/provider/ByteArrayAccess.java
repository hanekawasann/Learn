...
final class ByteArrayAccess {
    ...
    static {
        boolean scaleOK = ((unsafe.arrayIndexScale(byte[].class) == 1) && (unsafe.arrayIndexScale(int[].class) == 4) &&
            (unsafe.arrayIndexScale(long[].class) == 8) && ((byteArrayOfs & 3) == 0));
    ...
    private static boolean unaligned() {
        String arch = java.security.AccessController
    ...
    static void b2iLittle(byte[] in, int inOfs, int[] out, int outOfs, int len) {
        ...
        if (littleEndianUnaligned) {
            ...
            len += inOfs;
            while (inOfs < len) {
                out[outOfs++] = ((in[inOfs] & 0xff)) | ((in[inOfs + 1] & 0xff) << 8) | ((in[inOfs + 2] & 0xff) << 16) |
                    ((in[inOfs + 3]) << 24);
                inOfs += 4;
            }
        }
    ...
    static void b2iBig(byte[] in, int inOfs, int[] out, int outOfs, int len) {
        ...
        if (littleEndianUnaligned) {
            ...
            len += inOfs;
            while (inOfs < len) {
                out[outOfs++] = ((in[inOfs + 3] & 0xff)) | ((in[inOfs + 2] & 0xff) << 8) |
    ...
    static void b2lBig(byte[] in, int inOfs, long[] out, int outOfs, int len) {
        ...
        if (littleEndianUnaligned) {
            ...
            len += inOfs;
            while (inOfs < len) {
                out[outOfs++] = ((long) unsafe.getInt(in, (long) inOfs) << 32) |
            ...
            len += inOfs;
            while (inOfs < len) {
                int i1 = ((in[inOfs + 3] & 0xff)) | ((in[inOfs + 2] & 0xff) << 8) | ((in[inOfs + 1] & 0xff) << 16) |
                    ((in[inOfs]) << 24);
                inOfs += 4;
                int i2 = ((in[inOfs + 3] & 0xff)) | ((in[inOfs + 2] & 0xff) << 8) | ((in[inOfs + 1] & 0xff) << 16) |
                    ((in[inOfs]) << 24);
