...
class ForwardBuilder extends Builder {
     ...
     */
    @Override
    Collection<X509Certificate> getMatchingCerts(State currentState, List<CertStore> certStores)
     ...
     */
    private void getMatchingEECerts(ForwardState currentState, List<CertStore> certStores,
        Collection<X509Certificate> eeCerts) throws IOException {
        if (debug != null) {
     ...
     * and requirements specified in the parameters and PKIX state.
     */
    private void getMatchingCACerts(ForwardState currentState, List<CertStore> certStores,
        Collection<X509Certificate> caCerts) throws IOException {
        if (debug != null) {
        ...
        if (currentState.isInitial()) {
            ...
            if (caTargetSelector == null) {
                 ...
                 * Policy processing optimizations
                 */
                if (buildParams.explicitPolicyRequired()) { caTargetSelector.setPolicy(getMatchingPolicies()); }
            }
            ...
            if (caSelector == null) {
                 ...
                 * Policy processing optimizations
                 */
                if (buildParams.explicitPolicyRequired()) { caSelector.setPolicy(getMatchingPolicies()); }
            }
         ...
         * certificate pairs.
         */
        if (currentState.isInitial() || (buildParams.maxPathLength() == -1) ||
     ...
     */
    @Override
    void verifyCert(X509Certificate cert, State currentState, List<X509Certificate> certPathList)
        throws GeneralSecurityException {
        if (debug != null) {
            debug.println(
                "ForwardBuilder.verifyCert(SN: " + Debug.toHexString(cert.getSerialNumber()) + "\n  Issuer: " +
