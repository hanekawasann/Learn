...
public abstract class Builder {
    ...
    boolean addMatchingCerts(X509CertSelector selector, Collection<CertStore> certStores,
        ...
        if (targetCert != null) {
            ...
            if (selector.match(targetCert) && !X509CertImpl.isSelfSigned(targetCert, buildParams.sigProvider())) {
                if (debug != null) {
        ...
        for (CertStore store : certStores) {
            try {
                ...
                for (Certificate cert : certs) {
                    if (!X509CertImpl.isSelfSigned((X509Certificate) cert, buildParams.sigProvider())) {
