...
class PKIX {
    ...
    static BuilderParams checkBuilderParams(CertPathParameters params) throws InvalidAlgorithmParameterException {
        if (!(params instanceof PKIXBuilderParameters)) {
            throw new InvalidAlgorithmParameterException(
                "inappropriate " + "params, must be an instance of PKIXBuilderParameters");
        }
    ...
    static class ValidatorParams {
        ...
        List<X509Certificate> certificates() {
            if (certs == null) {
                if (certPath == null) {
                    ...
                    List<X509Certificate> xc = new ArrayList<>((List<X509Certificate>) certPath.getCertificates());
        ...
        List<PKIXCertPathChecker> certPathCheckers() {
            if (checkers == null) { checkers = params.getCertPathCheckers(); }
            return checkers;
        }

        List<CertStore> certStores() {
            if (stores == null) { stores = params.getCertStores(); }
            return stores;
        }
        ...
        Set<String> initialPolicies() {
            if (policies == null) { policies = params.getInitialPolicies(); }
            return policies;
        }
    ...
    static class BuilderParams extends ValidatorParams {
        ...
        private static X500Principal getTargetSubject(List<CertStore> stores, X509CertSelector sel)
            ...
            for (CertStore store : stores) {
                try {
                    Collection<? extends Certificate> certs = (Collection<? extends Certificate>) store
                        .getCertificates(sel);
                    if (!certs.isEmpty()) {
                    ...
                    if (debug != null) {
                        debug.println(
                            "BuilderParams.getTargetSubjectDN: " + "non-fatal exception retrieving certs: " + e);
                        e.printStackTrace();
                    }
                }
            }
    ...
    private static class CertStoreComparator implements Comparator<CertStore> {
        ...
        public int compare(CertStore store1, CertStore store2) {
            if (store1.getType().equals("Collection") ||
                store1.getCertStoreParameters() instanceof CollectionCertStoreParameters) {
                return -1;
            } else {
                return 1;
            }
