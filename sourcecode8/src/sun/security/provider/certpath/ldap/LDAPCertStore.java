...
public final class LDAPCertStore extends CertStoreSpi {
    ...
    private final static String PROP_LIFETIME = "sun.security.certpath.ldap.cache.lifetime";
    ...
    private void createInitialDirContext(String server, int port) throws InvalidAlgorithmParameterException {
        ...
        boolean disableAppResourceFiles = AccessController
            .doPrivileged(new GetBooleanAction(PROP_DISABLE_APP_RESOURCE_FILES));
     ...
     */
    private Collection<X509Certificate> getCertificates(LDAPRequest request, String id, X509CertSelector sel)
    ...
    private Collection<X509CertificatePair> getCertPairs(LDAPRequest request, String id) throws CertStoreException {
        ...
        for (int i = 0; i < n; i++) {
            try {
                ...
                if (debug != null) {
                    debug.println("LDAPCertStore.getCertPairs() encountered exception " +
     ...
     * @throws CertStoreException       if an exception occurs
     */
    private Collection<X509Certificate> getMatchingCrossCerts(LDAPRequest request, X509CertSelector forward,
        X509CertSelector reverse) throws CertStoreException {
        // Get the cert pairs
    ...
    public synchronized Collection<X509Certificate> engineGetCertificates(CertSelector selector)
            ...
            if (basicConstraints > -2) {
                ...
                if (debug != null) {
                    debug.println("LDAPCertStore.engineGetCertificates() after " +
                        "getMatchingCrossCerts(subject,xsel,null),certs.size(): " + certs.size());
                }
                certs.addAll(getCertificates(request, CA_CERT, xsel));
                if (debug != null) {
                    debug.println("LDAPCertStore.engineGetCertificates() after " +
                        "getCertificates(subject,CA_CERT,xsel),certs.size(): " + certs.size());
                }
            }
            if (basicConstraints < 0) {
                ...
                if (debug != null) {
                    debug.println("LDAPCertStore.engineGetCertificates() after " +
                        "getCertificates(subject,USER_CERT, xsel),certs.size(): " + certs.size());
                }
            }
        } else {
            if (debug != null) {
        ...
        if ((issuer != null) && (basicConstraints > -2)) {
            ...
            if (debug != null) {
                debug.println("LDAPCertStore.engineGetCertificates() after " +
                    "getMatchingCrossCerts(issuer,null,xsel),certs.size(): " + certs.size());
            }
            certs.addAll(getCertificates(request, CA_CERT, xsel));
            if (debug != null) {
                debug.println("LDAPCertStore.engineGetCertificates() after " +
                    "getCertificates(issuer,CA_CERT,xsel),certs.size(): " + certs.size());
            }
        }
        if (debug != null) {
    ...
    private static class SunLDAPCertStoreParameters extends LDAPCertStoreParameters {
        ...
        public boolean equals(Object obj) {
            ...
            return (getPort() == params.getPort() && getServerName().equalsIgnoreCase(params.getServerName()));
        }
        ...
        public int hashCode() {
            if (hashCode == 0) {
                ...
                result = 37 * result + getServerName().toLowerCase(Locale.ENGLISH).hashCode();
                hashCode = result;
            }
            return hashCode;
        }
    ...
    static class LDAPCRLSelector extends X509CRLSelector {
         ...
         */
        LDAPCRLSelector(X509CRLSelector selector, Collection<X500Principal> certIssuers, String ldapDN)
            throws IOException {
