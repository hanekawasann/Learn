...
class RevocationChecker extends PKIXRevocationChecker {
    ...
    void init(TrustAnchor anchor, ValidatorParams params) throws CertPathValidatorException {
        ...
        try {
            this.certStores
                .add(CertStore.getInstance("Collection", new CollectionCertStoreParameters(params.certificates())));
    ...
    private static X509Certificate getResponderCert(RevocationProperties rp, Set<TrustAnchor> anchors,
        List<CertStore> stores) throws CertPathValidatorException {
        if (rp.ocspSubject != null) {
            ...
            throw new CertPathValidatorException(
                "Must specify both ocsp.responderCertIssuerName and " + "ocsp.responderCertSerialNumber properties");
        }
        return null;
    }

    private static X509Certificate getResponderCert(String subject, Set<TrustAnchor> anchors, List<CertStore> stores)
    ...
    private static X509Certificate getResponderCert(String issuer, String serial, Set<TrustAnchor> anchors,
        List<CertStore> stores) throws CertPathValidatorException {
        X509CertSelector sel = new X509CertSelector();
        try {
    ...
    private static X509Certificate getResponderCert(X509CertSelector sel, Set<TrustAnchor> anchors,
        List<CertStore> stores) throws CertPathValidatorException {
        ...
        throw new CertPathValidatorException(
            "Cannot find the responder's certificate " + "(set using the OCSP security properties).");
    }
    ...
    public void init(boolean forward) throws CertPathValidatorException {
        ...
        if (anchor != null) {
            ...
            prevPubKey = (issuerCert != null) ? issuerCert.getPublicKey() : anchor.getCAPublicKey();
        }
        crlSignFlag = true;
    ...
    public void check(Certificate cert, Collection<String> unresolvedCritExts) throws CertPathValidatorException {
        check((X509Certificate) cert, unresolvedCritExts, prevPubKey, crlSignFlag);
    }

    private void check(X509Certificate xcert, Collection<String> unresolvedCritExts, PublicKey pubKey,
        boolean crlSignFlag) throws CertPathValidatorException {
        try {
    ...
    private boolean isSoftFailException(CertPathValidatorException e) {
        if (softFail && e.getReason() == BasicReason.UNDETERMINED_REVOCATION_STATUS) {
            // recreate exception with correct index
            CertPathValidatorException e2 = new CertPathValidatorException(e.getMessage(), e.getCause(),
                params.certPath(), certIndex, e.getReason());
    ...
    private void checkCRLs(X509Certificate cert, Collection<String> unresolvedCritExts,
        Set<X509Certificate> stackedCerts, PublicKey pubKey, boolean signFlag) throws CertPathValidatorException {
        checkCRLs(cert, pubKey, null, signFlag, true, stackedCerts, params.trustAnchors());
    }

    private void checkCRLs(X509Certificate cert, PublicKey prevKey, X509Certificate prevCert, boolean signFlag,
        boolean allowSeparateKey, Set<X509Certificate> stackedCerts, Set<TrustAnchor> anchors)
        throws CertPathValidatorException {
        if (debug != null) {
        ...
        if (stackedCerts != null && stackedCerts.contains(cert)) {
            ...
            throw new CertPathValidatorException("Could not determine revocation status", null, null, -1,
            ...
            try {
                ...
                if (networkFailureException == null && CertStoreHelper.isCausedByNetworkIssue(store.getType(), e)) {
                    ...
                    networkFailureException = new CertPathValidatorException(
                        "Unable to determine revocation status due to " + "network error", e, null, -1,
                        BasicReason.UNDETERMINED_REVOCATION_STATUS);
                }
            }
        }
        ...
        if (!approvedCRLs.isEmpty() && Arrays.equals(reasonsMask, ALL_REASONS)) {
            ...
            try {
                if (crlDP) {
                    approvedCRLs.addAll(DistributionPointFetcher
                        .getCRLs(sel, signFlag, prevKey, prevCert, params.sigProvider(), certStores, reasonsMask,
                            anchors, null));
                }
            } catch (CertStoreException e) {
            ...
            if (e != null) {
                ...
                if (unresCritExts != null && !unresCritExts.isEmpty()) {
                    ...
                    if (!unresCritExts.isEmpty()) {
                        throw new CertPathValidatorException(
                            "Unrecognized critical extension(s) in revoked " + "CRL entry");
                    }
                }
                ...
                if (revocationDate.before(params.date())) {
                    Throwable t = new CertificateRevokedException(revocationDate, reasonCode,
    ...
    private void checkOCSP(X509Certificate cert, Collection<String> unresolvedCritExts)
        throws CertPathValidatorException {
        ...
        try {
            ...
            if (responseBytes != null) {
                ...
                response = OCSP.check(Collections.singletonList(certId), responderURI, issuerCert, responderCert, null,
                    ocspExtensions);
            }
        } catch (IOException e) {
            throw new CertPathValidatorException("Unable to determine revocation status due to network error", e, null,
        ...
        if (certStatus == RevocationStatus.CertStatus.REVOKED) {
            ...
            if (revocationTime.before(params.date())) {
                Throwable t = new CertificateRevokedException(revocationTime, rs.getRevocationReason(),
                    response.getSignerCertificate().getSubjectX500Principal(), rs.getSingleExtensions());
                throw new CertPathValidatorException(t.getMessage(), t, null, -1, BasicReason.REVOKED);
            }
        } else if (certStatus == RevocationStatus.CertStatus.UNKNOWN) {
            throw new CertPathValidatorException("Certificate's revocation status is unknown", null, params.certPath(),
                -1, BasicReason.UNDETERMINED_REVOCATION_STATUS);
        }
    }
    ...
    private Collection<X509CRL> verifyPossibleCRLs(Set<X509CRL> crls, X509Certificate cert, PublicKey prevKey,
        boolean signFlag, boolean[] reasonsMask, Set<TrustAnchor> anchors) throws CertPathValidatorException {
        try {
            X509CertImpl certImpl = X509CertImpl.toImpl(cert);
            if (debug != null) {
                debug.println("RevocationChecker.verifyPossibleCRLs: " + "Checking CRLDPs for " +
            ...
            if (ext == null) {
                ...
                DistributionPoint point = new DistributionPoint(new GeneralNames().add(new GeneralName(certIssuer)),
                    null, null);
                ...
                for (X509CRL crl : crls) {
                    if (DistributionPointFetcher
                        .verifyCRL(certImpl, point, crl, reasonsMask, signFlag, prevKey, null, params.sigProvider(),
                            anchors, certStores, params.date())) {
                        results.add(crl);
                    }
                }
                if (Arrays.equals(reasonsMask, ALL_REASONS)) { break; }
            }
            return results;
     ...
     */
    private void verifyWithSeparateSigningKey(X509Certificate cert, PublicKey prevKey, boolean signFlag,
    ...
    private void buildToNewKey(X509Certificate currCert, PublicKey prevKey, Set<X509Certificate> stackedCerts)
        throws CertPathValidatorException {
        ...
        Set<TrustAnchor> newAnchors = anchor == null ? params.trustAnchors() : Collections.singleton(anchor);
        ...
        builderParams.setExplicitPolicyRequired(params.explicitPolicyRequired());
        builderParams.setPolicyMappingInhibited(params.policyMappingInhibited());
        ...
        builderParams.setCertPathCheckers(params.getPKIXParameters().getCertPathCheckers());
        ...
        while (true) {
            try {
                ...
                try {
                    checkCRLs(currCert, newKey, (X509Certificate) cpList.get(0), true, false, null,
                        params.trustAnchors());
    ...
    private static class RejectKeySelector extends X509CertSelector {
        ...
        public boolean match(Certificate cert) {
            ...
            if (badKeySet.contains(cert.getPublicKey())) {
                if (debug != null) { debug.println("RejectKeySelector.match: bad key"); }
                return false;
            }

            if (debug != null) { debug.println("RejectKeySelector.match: returning true"); }
            return true;
        }
