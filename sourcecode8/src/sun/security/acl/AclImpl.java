...
public class AclImpl extends OwnerImpl implements Acl {
     ...
     * @throws NotOwnerException if the caller principal is
    ...
    public synchronized boolean addEntry(Principal caller, AclEntry entry) throws NotOwnerException {
        ...
        if (aclTable.get(key) != null) { return false; }
    ...
    private Hashtable<Principal, AclEntry> findTable(AclEntry entry) {
        ...
        if (p instanceof Group) {
            if (entry.isNegative()) { aclTable = deniedGroupsTable; } else { aclTable = allowedGroupsTable; }
        } else {
            if (entry.isNegative()) { aclTable = deniedUsersTable; } else { aclTable = allowedUsersTable; }
        }
        return aclTable;
    }
    ...
    private Enumeration<Permission> getIndividualPositive(Principal user) {
        ...
        if (ae != null) { individualPositive = ae.permissions(); }
    ...
    private Enumeration<Permission> getIndividualNegative(Principal user) {
        ...
        if (ae != null) { individualNegative = ae.permissions(); }
...
final class AclEnumerator implements Enumeration<AclEntry> {
    ...
    AclEnumerator(Acl acl, Hashtable<?, AclEntry> u1, Hashtable<?, AclEntry> g1, Hashtable<?, AclEntry> u2,
        Hashtable<?, AclEntry> g2) {
        this.acl = acl;
        this.u1 = u1.elements();
        this.u2 = u2.elements();
        this.g1 = g1.elements();
