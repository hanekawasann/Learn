...
final class P11Signature extends SignatureSpi {
    ...
    private void initialize() {
        try {
            ...
            if (mode == M_SIGN) {
                token.p11.C_SignInit(session.id(), new CK_MECHANISM(mechanism), p11Key.keyID);
            } else {
                token.p11.C_VerifyInit(session.id(), new CK_MECHANISM(mechanism), p11Key.keyID);
            }
            initialized = true;
        } catch (PKCS11Exception e) {
    ...
    private void checkKeySize(String keyAlgo, Key key) throws InvalidKeyException {
        ...
        if ((minKeySize != -1) && (keySize < minKeySize)) {
            throw new InvalidKeyException(keyAlgo + " key must be at least " + minKeySize + " bits");
        }
        if ((maxKeySize != -1) && (keySize > maxKeySize)) {
            throw new InvalidKeyException(keyAlgo + " key must be at most " + maxKeySize + " bits");
        }
        if (keyAlgo.equals("RSA")) {
    ...
    private void checkRSAKeyLength(int len) throws InvalidKeyException {
        RSAPadding padding;
        try {
            padding = RSAPadding.getInstance(RSAPadding.PAD_BLOCKTYPE_1, (len + 7) >> 3);
        } catch (InvalidAlgorithmParameterException iape) {
            throw new InvalidKeyException(iape.getMessage());
        }
        int maxDataSize = padding.getMaxDataSize();
        int encodedLength;
        if (algorithm.equals("MD5withRSA") || algorithm.equals("MD2withRSA")) {
            encodedLength = 34;
        } else if (algorithm.equals("SHA1withRSA")) {
            encodedLength = 35;
        } else if (algorithm.equals("SHA224withRSA")) {
                    ...
                    if (mechanism == CKM_DSA) {
                        ...
                        if (bytesProcessed > buffer.length) {
                            throw new SignatureException(
                                "Data for NONEwithECDSA" + " must be at most " + RAW_ECDSA_MAX + " bytes long");
                        }
    ...
    private byte[] pkcs1Pad(byte[] data) {
        try {
            ...
            RSAPadding padding = RSAPadding.getInstance(RSAPadding.PAD_BLOCKTYPE_1, len);
