...
public final class Secmod {
    ...
    public synchronized void initialize(DbMode dbMode, String configDir, String nssLibDir, boolean nssOptimizeSpace)
        throws IOException {
        ...
        boolean initok = nssInitialize(dbMode.functionName, nssHandle, configDir, nssOptimizeSpace);
    ...
    public synchronized List<Module> getModules() {
        ...
        if (modules == null) {
            ...
            List<Module> modules = (List<Module>) nssGetModuleList(nssHandle, nssLibDir);
            ...
            if ((libraryName == null) || (libraryName.length() == 0)) {
                ...
                if (fips) {
                    throw new RuntimeException(
                        "FIPS flag set for non-internal " + "module: " + libraryName + ", " + commonName);
                }
            }
    ...
    static class TrustAttributes {
        ...
        TrustAttributes(Token token, X509Certificate cert, Bytes bytes, long trustValue) {
            ...
            try {
                ...
                // attributes to set
                CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_TOKEN, true),
        ...
        TrustAttributes(Token token, Session session, long handle) throws PKCS11Exception {
            this.handle = handle;
            CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_NETSCAPE_TRUST_SERVER_AUTH),
                new CK_ATTRIBUTE(CKA_NETSCAPE_TRUST_CODE_SIGNING),
                new CK_ATTRIBUTE(CKA_NETSCAPE_TRUST_EMAIL_PROTECTION), new CK_ATTRIBUTE(CKA_NETSCAPE_CERT_SHA1_HASH), };
        ...
        boolean isTrusted(TrustType type) {
            switch (type) {
                ...
                case ALL:
                    return isTrusted(TrustType.CLIENT_AUTH) && isTrusted(TrustType.SERVER_AUTH) &&
                        isTrusted(TrustType.CODE_SIGNING) && isTrusted(TrustType.EMAIL_PROTECTION);
                default:
                    return false;
            }
        }
    ...
    private static native boolean nssInitialize(String functionName, long handle, String configDir,
