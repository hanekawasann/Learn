...
final class P11Cipher extends CipherSpi {
    ...
    private static interface Padding {
        ...
        // padded data
        int unpad(byte[] paddedData, int len) throws BadPaddingException, IllegalBlockSizeException;
    }
    ...
    private static class PKCS5Padding implements Padding {
        ...
        public int unpad(byte[] paddedData, int len) throws BadPaddingException, IllegalBlockSizeException {
            if ((len < 1) || (len % blockSize != 0)) {
                throw new IllegalBlockSizeException("Input length must be multiples of " + blockSize);
            }
    ...
    P11Cipher(Token token, String algorithm, long mechanism) throws PKCS11Exception, NoSuchAlgorithmException {
        ...
        if (algoParts[0].startsWith("AES")) {
            ...
            if (keyAlgorithm.equals("RC4") || keyAlgorithm.equals("ARCFOUR")) {
                blockSize = 0;
            } else { // DES, DESede, Blowfish
                blockSize = 8;
            }
        }
        this.blockMode = (algoParts.length > 1 ? parseMode(algoParts[1]) : MODE_ECB);
    ...
    protected void engineSetPadding(String padding) throws NoSuchPaddingException {
        ...
        if (padding.equals("NOPADDING")) {
            ...
            if (mechanism != CKM_DES_CBC_PAD && mechanism != CKM_DES3_CBC_PAD && mechanism != CKM_AES_CBC_PAD) {
    ...
    protected AlgorithmParameters engineGetParameters() {
        ...
        try {
            AlgorithmParameters params = AlgorithmParameters.getInstance(keyAlgorithm, P11Util.getSunJceProvider());
            params.init(ivSpec);
            return params;
    ...
    private void implInit(int opmode, Key key, byte[] iv, SecureRandom random)
        ...
        if (blockMode == MODE_ECB) { // ECB or stream cipher
            ...
            if (iv == null) {
                if (encrypt == false) {
                    String exMsg = (blockMode == MODE_CBC ? "IV must be specified for decryption in CBC mode"
    ...
    private void initialize() throws PKCS11Exception {
        ...
        CK_MECHANISM mechParams = (blockMode == MODE_CTR ? new CK_MECHANISM(mechanism, new CK_AES_CTR_PARAMS(iv))
            ...
            if (encrypt) {
                ...
                if (paddingObj != null) {
                    if (padBufferLen != 0) {
                        ...
                        k = token.p11
                            .C_DecryptUpdate(session.id(), 0, padBuffer, 0, padBufferLen, 0, out, outOfs, outLen);
                        padBufferLen = 0;
                    }
                ...
                if (inLen > 0) {
                    k += token.p11
                        .C_DecryptUpdate(session.id(), 0, in, inOfs, inLen, 0, out, (outOfs + k), (outLen - k));
                }
    ...
    private int implUpdate(ByteBuffer inBuffer, ByteBuffer outBuffer) throws ShortBufferException {
        ...
        try {
            ...
            if (encrypt) {
                ...
                k = token.p11
                    .C_EncryptUpdate(session.id(), inAddr, inArray, inOfs, inLen, outAddr, outArray, outOfs, outLen);
            } else {
                int newPadBufferLen = 0;
                if (paddingObj != null) {
                    if (padBufferLen != 0) {
                        ...
                        k = token.p11
                            .C_DecryptUpdate(session.id(), 0, padBuffer, 0, padBufferLen, outAddr, outArray, outOfs,
                                outLen);
                        padBufferLen = 0;
                    }
                    newPadBufferLen = inLen & (blockSize - 1);
                ...
                if (inLen > 0) {
                    ...
                    k += token.p11
                        .C_DecryptUpdate(session.id(), inAddr, inArray, inOfs, inLen, outAddr, outArray, (outOfs + k),
                            (outLen - k));
                }
                // update 'padBuffer' if using our own padding impl.
        ...
        try {
            ...
            if (encrypt) {
                if (paddingObj != null) {
                    ...
                    k = token.p11.C_EncryptUpdate(session.id(), 0, padBuffer, 0, actualPadLen, 0, out, outOfs, outLen);
                }
                k += token.p11.C_EncryptFinal(session.id(), 0, out, (outOfs + k), (outLen - k));
            } else {
                if (paddingObj != null) {
                    if (padBufferLen != 0) {
                        k = token.p11.C_DecryptUpdate(session.id(), 0, padBuffer, 0, padBufferLen, 0, padBuffer, 0,
                            padBuffer.length);
                    }
        ...
        try {
            ...
            if (encrypt) {
                if (paddingObj != null) {
                    int actualPadLen = paddingObj.setPaddingBytes(padBuffer, requiredOutLen - bytesBuffered);
                    k = token.p11
                        .C_EncryptUpdate(session.id(), 0, padBuffer, 0, actualPadLen, outAddr, outArray, outOfs,
                            outLen);
                }
            ...
            if ((!encrypt && paddingObj != null) || (!(outBuffer instanceof DirectBuffer) && !outBuffer.hasArray())) {
                outBuffer.put(outArray, outOfs, k);
            } else {
                outBuffer.position(outBuffer.position() + k);
            }
    ...
    protected int engineGetKeySize(Key key) throws InvalidKeyException {
        int n = P11SecretKeyFactory.convertKey(token, key, keyAlgorithm).length();
        return n;
    }
