...
final class P11KeyPairGenerator extends KeyPairGeneratorSpi {
    ...
    P11KeyPairGenerator(Token token, String algorithm, long mechanism) throws PKCS11Exception {
        ...
        try {
            ...
            // Should never happen
            throw new ProviderException("Unexpected error while getting mechanism info", p11e);
        }
    ...
    private void checkKeySize(int keySize, RSAKeyGenParameterSpec params) throws InvalidAlgorithmParameterException {
        // check native range first
        if ((minKeySize != -1) && (keySize < minKeySize)) {
            throw new InvalidAlgorithmParameterException(algorithm + " key must be at least " + minKeySize + " bits");
        }
        if ((maxKeySize != -1) && (keySize > maxKeySize)) {
            throw new InvalidAlgorithmParameterException(algorithm + " key must be at most " + maxKeySize + " bits");
        }
        ...
        if (algorithm.equals("EC")) {
            ...
            if (algorithm.equals("RSA")) {
                ...
                try {
                    ...
                    RSAKeyFactory.checkKeyLengths(keySize, tmpExponent, minKeySize,
                ...
                if (algorithm.equals("DH") && (params != null)) {
                    ...
                    if ((keySize != 2048) && ((keySize > 1024) || ((keySize & 0x3f) != 0))) {
                        throw new InvalidAlgorithmParameterException(
                            algorithm + " key must be multiples of 64 if less than 1024 bits" + ", or 2048 bits");
                    }
                }
            }
        }
    ...
    public KeyPair generateKeyPair() {
        ...
        if (algorithm.equals("RSA")) {
            keyType = CKK_RSA;
            publicKeyTemplate = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_MODULUS_BITS, keySize),
            ...
            publicKeyTemplate = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_PRIME, dsaParams.getP()),
                new CK_ATTRIBUTE(CKA_SUBPRIME, dsaParams.getQ()), new CK_ATTRIBUTE(CKA_BASE, dsaParams.getG()), };
            privateKeyTemplate = new CK_ATTRIBUTE[] {
                // empty
            };
            ...
            publicKeyTemplate = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_PRIME, dhParams.getP()),
        ...
        try {
            ...
            long[] keyIDs = token.p11
                .C_GenerateKeyPair(session.id(), new CK_MECHANISM(mechanism), publicKeyTemplate, privateKeyTemplate);
