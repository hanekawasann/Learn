...
final class P11KeyStore extends KeyStoreSpi {
    ...
    private static final CK_ATTRIBUTE ATTR_CLASS_SKEY = new CK_ATTRIBUTE(CKA_CLASS, CKO_SECRET_KEY);

    private static final CK_ATTRIBUTE ATTR_X509_CERT_TYPE = new CK_ATTRIBUTE(CKA_CERTIFICATE_TYPE, CKC_X_509);
    ...
    private static final CK_ATTRIBUTE ATTR_TRUSTED_TRUE = new CK_ATTRIBUTE(CKA_TRUSTED, true);
    private static final CK_ATTRIBUTE ATTR_PRIVATE_TRUE = new CK_ATTRIBUTE(CKA_PRIVATE, true);
    ...
    private static class AliasInfo {
        ...
        public String toString() {
            ...
            if (cert == null) {
                ...
                sb.append("\tcert=[\tsubject: " + cert.getSubjectX500Principal() + "\n\t\tissuer: " +
                    cert.getIssuerX500Principal() + "\n\t\tserialNum: " + cert.getSerialNumber().toString() + "]");
            }
            return sb.toString();
        }
    }
    ...
    public synchronized Key engineGetKey(String alias, char[] password)
        ...
        try {
            ...
            if (aliasInfo.type == ATTR_CLASS_PKEY) {
                THandle h = getTokenObject(session, aliasInfo.type, aliasInfo.id, null);
    ...
    public synchronized void engineSetKeyEntry(String alias, Key key, char[] password, Certificate[] chain)
        ...
        } else if (password != null && !token.config.getKeyStoreCompatibilityMode()) {
     ...
     * @throws IOException if the given <code>stream</code> is not
     ...
     * @throws IOException              if the token supports a
    ...
    public synchronized void engineLoad(KeyStore.LoadStoreParameter param)
        ...
        if (pp instanceof PasswordProtection) {
            ...
            throw new IllegalArgumentException(
                "ProtectionParameter must be either " + "PasswordProtection or CallbackHandlerProtection");
        }
    ...
    private void login(CallbackHandler handler) throws LoginException {
        if ((token.tokenInfo.flags & CKF_PROTECTED_AUTHENTICATION_PATH) == 0) {
            ...
            if (handler != null && !token.config.getKeyStoreCompatibilityMode()) {
                throw new LoginException(
                    "can not specify password if token " + "supports protected authentication path");
            }
    ...
    public synchronized KeyStore.Entry engineGetEntry(String alias, KeyStore.ProtectionParameter protParam)
        ...
        try {
            ...
            if (aliasInfo.type == ATTR_CLASS_CERT) {
                ...
                THandle h = getTokenObject(session, ATTR_CLASS_SKEY, null, aliasInfo.label);
                if (h.type != ATTR_CLASS_SKEY) {
                ...
                THandle h = getTokenObject(session, ATTR_CLASS_PKEY, aliasInfo.id, null);
                if (h.type != ATTR_CLASS_PKEY) {
     ...
     * @since 1.5
     */
    public synchronized void engineSetEntry(String alias, KeyStore.Entry entry, KeyStore.ProtectionParameter protParam)
        throws KeyStoreException {
        ...
        if (entry instanceof KeyStore.TrustedCertificateEntry) {
            ...
            if (entry instanceof KeyStore.PrivateKeyEntry) {
                ...
                if (!(key instanceof P11Key) && !(key instanceof RSAPrivateKey) && !(key instanceof DSAPrivateKey) &&
                    !(key instanceof DHPrivateKey) && !(key instanceof ECPrivateKey)) {
                ...
                if (!(chain instanceof X509Certificate[])) {
                    throw new KeyStoreException(new UnsupportedOperationException(
                        "unsupported certificate array type: " + chain.getClass().getName()));
                }
                ...
                try {
                    ...
                    for (String oldAlias : aliases) {
                        ...
                        if (aliasInfo.type == ATTR_CLASS_PKEY &&
                            aliasInfo.cert.getPublicKey().equals(chain[0].getPublicKey())) {
                            ...
                            updatePkey(alias, aliasInfo.id, (X509Certificate[]) chain,
                ...
                throw new KeyStoreException(
                    new UnsupportedOperationException("unsupported entry type: " + entry.getClass().getName()));
            }
        ...
        if (debug != null) {
            debug.println("engineSetEntry added new entry for [" + alias + "] to token");
        }
    }
    ...
    private X509Certificate[] loadChain(Session session, X509Certificate endCert)
        ...
        while (true) {
            CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, ATTR_CLASS_CERT,
                new CK_ATTRIBUTE(CKA_SUBJECT, next.getIssuerX500Principal().getEncoded()) };
            ...
            if (ch == null || ch.length == 0) {
                ...
                if (debug != null && ch.length > 1) {
                    debug.println("engineGetEntry found " + ch.length + " certificate entries for subject [" +
                        next.getIssuerX500Principal().toString() + "] in token - using first entry");
                }
    ...
    private SecretKey loadSkey(Session session, long oHandle) throws PKCS11Exception {
        ...
        CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_KEY_TYPE) };
        token.p11.C_GetAttributeValue(session.id(), oHandle, attrs);
    ...
    private PrivateKey loadPkey(Session session, long oHandle) throws PKCS11Exception, KeyStoreException {
        ...
        if (kType == CKK_RSA) {
            ...
            attrs = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_EC_PARAMS), };
            token.p11.C_GetAttributeValue(session.id(), oHandle, attrs);
     ...
     */
    private void updatePkey(String alias, byte[] cka_id, X509Certificate[] chain, boolean replaceCert)
        throws KeyStoreException, CertificateException, PKCS11Exception {
        ...
        try {
            ...
            if (h.type == ATTR_CLASS_PKEY) {
                ...
                throw new KeyStoreException(
                    "expected but could not find private key " + "with CKA_ID " + getID(cka_id));
            }
            ...
            if (h.type != ATTR_CLASS_CERT) {
                throw new KeyStoreException(
                    "expected but could not find certificate " + "with CKA_ID " + getID(cka_id));
            } else {
                if (replaceCert) {
                    ...
                    CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_LABEL, alias),
            ...
            CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_ID, alias) };
    ...
    private void updateP11Pkey(String alias, CK_ATTRIBUTE attribute, P11Key key) throws PKCS11Exception {
        ...
        try {
            ...
            if (key.tokenObject == true) {
                ...
                CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_ID, alias) };
                token.p11.C_SetAttributeValue(session.id(), key.keyID, attrs);
                if (debug != null) {
                ...
                CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, new CK_ATTRIBUTE(CKA_ID, alias), };
                if (attribute != null) {
    ...
    private void storeCert(String alias, X509Certificate cert) throws PKCS11Exception, CertificateException {
        ...
        attrList.add(new CK_ATTRIBUTE(CKA_SUBJECT, cert.getSubjectX500Principal().getEncoded()));
        attrList.add(new CK_ATTRIBUTE(CKA_ISSUER, cert.getIssuerX500Principal().getEncoded()));
        attrList.add(new CK_ATTRIBUTE(CKA_SERIAL_NUMBER, cert.getSerialNumber().toByteArray()));
        ...
        if (alias != null) {
            ...
            attrList.add(
                new CK_ATTRIBUTE(CKA_ID, getID(cert.getSubjectX500Principal().getName(X500Principal.CANONICAL), cert)));
        }

        Session session = null;
        try {
            session = token.getOpSession();
            token.p11.C_CreateObject(session.id(), attrList.toArray(new CK_ATTRIBUTE[attrList.size()]));
        } finally {
            token.releaseSession(session);
        }
    }
    ...
    private void storeSkey(String alias, KeyStore.SecretKeyEntry ske) throws PKCS11Exception, KeyStoreException {
        ...
        CK_ATTRIBUTE[] attrs = new CK_ATTRIBUTE[] { ATTR_SKEY_TOKEN_TRUE, ATTR_PRIVATE_TRUE,
    ...
    private void storePkey(String alias, KeyStore.PrivateKeyEntry pke)
        ...
        if (key instanceof RSAPrivateKey) {
            ...
            attrs = getRsaPrivKeyAttrs(alias, (RSAPrivateKey) key, cert.getSubjectX500Principal());
            ...
            attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, ATTR_CLASS_PKEY, ATTR_PRIVATE_TRUE,
                new CK_ATTRIBUTE(CKA_KEY_TYPE, CKK_DSA), idAttrs[0],
                new CK_ATTRIBUTE(CKA_PRIME, dsaKey.getParams().getP()),
                new CK_ATTRIBUTE(CKA_SUBPRIME, dsaKey.getParams().getQ()),
                new CK_ATTRIBUTE(CKA_BASE, dsaKey.getParams().getG()), new CK_ATTRIBUTE(CKA_VALUE, dsaKey.getX()), };
            if (idAttrs[1] != null) {
                attrs = addAttribute(attrs, idAttrs[1]);
            }

            attrs = token.getAttributes(TemplateManager.O_IMPORT, CKO_PRIVATE_KEY, CKK_DSA, attrs);
            ...
            attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, ATTR_CLASS_PKEY, ATTR_PRIVATE_TRUE,
                new CK_ATTRIBUTE(CKA_KEY_TYPE, CKK_DH), idAttrs[0],
                new CK_ATTRIBUTE(CKA_PRIME, dhKey.getParams().getP()),
                new CK_ATTRIBUTE(CKA_BASE, dhKey.getParams().getG()), new CK_ATTRIBUTE(CKA_VALUE, dhKey.getX()), };
            if (idAttrs[1] != null) {
                attrs = addAttribute(attrs, idAttrs[1]);
            }

            attrs = token.getAttributes(TemplateManager.O_IMPORT, CKO_PRIVATE_KEY, CKK_DH, attrs);
            ...
            attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, ATTR_CLASS_PKEY, ATTR_PRIVATE_TRUE,
                new CK_ATTRIBUTE(CKA_KEY_TYPE, CKK_EC), idAttrs[0], new CK_ATTRIBUTE(CKA_VALUE, ecKey.getS()),
                new CK_ATTRIBUTE(CKA_EC_PARAMS, encodedParams), };
            if (idAttrs[1] != null) {
                attrs = addAttribute(attrs, idAttrs[1]);
            }

            attrs = token.getAttributes(TemplateManager.O_IMPORT, CKO_PRIVATE_KEY, CKK_EC, attrs);
    ...
    private CK_ATTRIBUTE[] getRsaPrivKeyAttrs(String alias, RSAPrivateKey key, X500Principal subject)
        ...
        if (key instanceof RSAPrivateCrtKey) {
            ...
            attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, ATTR_CLASS_PKEY, ATTR_PRIVATE_TRUE,
                new CK_ATTRIBUTE(CKA_KEY_TYPE, CKK_RSA), new CK_ATTRIBUTE(CKA_ID, alias),
                new CK_ATTRIBUTE(CKA_MODULUS, rsaKey.getModulus()),
                new CK_ATTRIBUTE(CKA_PRIVATE_EXPONENT, rsaKey.getPrivateExponent()),
                new CK_ATTRIBUTE(CKA_PUBLIC_EXPONENT, rsaKey.getPublicExponent()),
                new CK_ATTRIBUTE(CKA_PRIME_1, rsaKey.getPrimeP()), new CK_ATTRIBUTE(CKA_PRIME_2, rsaKey.getPrimeQ()),
                new CK_ATTRIBUTE(CKA_EXPONENT_1, rsaKey.getPrimeExponentP()),
                new CK_ATTRIBUTE(CKA_EXPONENT_2, rsaKey.getPrimeExponentQ()),
                new CK_ATTRIBUTE(CKA_COEFFICIENT, rsaKey.getCrtCoefficient()) };
            attrs = token.getAttributes(TemplateManager.O_IMPORT, CKO_PRIVATE_KEY, CKK_RSA, attrs);
    ...
    private boolean destroyCert(byte[] cka_id) throws PKCS11Exception, KeyStoreException {
        ...
        try {
            ...
            if (debug != null) {
                debug.println("destroyCert destroyed cert with CKA_ID [" + getID(cka_id) + "]");
            }
            return true;
        } finally {
            token.releaseSession(session);
    ...
    private boolean destroyChain(byte[] cka_id) throws PKCS11Exception, CertificateException, KeyStoreException {
        ...
        try {
            ...
            if (h.type != ATTR_CLASS_CERT) {
                if (debug != null) {
                    debug.println("destroyChain could not find " + "end entity cert with CKA_ID [0x" +
            ...
            token.p11.C_DestroyObject(session.id(), h.handle);
            if (debug != null) {
                debug.println("destroyChain destroyed end entity cert " + "with CKA_ID [" + getID(cka_id) + "]");
            }
            ...
            while (true) {
                ...
                if (ch == null || ch.length == 0) {
                    ...
                    // if more than one found, use first
                    if (debug != null && ch.length > 1) {
                        debug.println("destroyChain found " + ch.length + " certificate entries for subject [" +
                            next.getIssuerX500Principal() + "] in token - using first entry");
                    }
                    ...
                    attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, ATTR_CLASS_CERT,
                        new CK_ATTRIBUTE(CKA_ISSUER, next.getSubjectX500Principal().getEncoded()) };
                    ...
                    if (destroyIt) {
                        ...
                        if (debug != null) {
                            debug.println("destroyChain destroyed cert in chain " + "with subject [" +
                                next.getSubjectX500Principal() + "]");
                        }
                    } else {
                        if (debug != null) {
                            debug.println("destroyChain did not destroy " + "shared cert in chain with subject [" +
                                next.getSubjectX500Principal() + "]");
                        }
                    }
                }
    ...
    private boolean destroyPkey(byte[] cka_id) throws PKCS11Exception, KeyStoreException {
        ...
        try {
            ...
            if (h.type != ATTR_CLASS_PKEY) {
                if (debug != null) {
                    debug.println("destroyPkey did not find private key with CKA_ID [" + getID(cka_id) + "]");
                }
                return false;
            }
            token.p11.C_DestroyObject(session.id(), h.handle);
    ...
    private String getID(String alias, X509Certificate cert) {
        ...
        return alias + ALIAS_SEP + issuer.getName(X500Principal.CANONICAL) + ALIAS_SEP + serialNum.toString();
    }
     ...
     */
    private THandle getTokenObject(Session session, CK_ATTRIBUTE type, byte[] cka_id, String cka_label)
        throws PKCS11Exception, KeyStoreException {

        CK_ATTRIBUTE[] attrs;
        if (type == ATTR_CLASS_SKEY) {
            attrs = new CK_ATTRIBUTE[] { ATTR_SKEY_TOKEN_TRUE, new CK_ATTRIBUTE(CKA_LABEL, cka_label), type };
        } else {
            attrs = new CK_ATTRIBUTE[] { ATTR_TOKEN_TRUE, new CK_ATTRIBUTE(CKA_ID, cka_id), type };
        }
        ...
        if (h.length == 0) {
            if (debug != null) {
                if (type == ATTR_CLASS_SKEY) {
                    ...
                    debug.println("getTokenObject did not find cert with CKA_ID [" + getID(cka_id) + "]");
                } else {
                    debug.println("getTokenObject did not find private key " + "with CKA_ID [" + getID(cka_id) + "]");
                }
            }
        } else if (h.length == 1) {
            ...
            if (type == ATTR_CLASS_SKEY) {
                ...
                for (int i = 0; i < h.length; i++) {
                    ...
                    CK_ATTRIBUTE[] label = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_LABEL) };
                    token.p11.C_GetAttributeValue(session.id(), h[i], label);
                    if (label[0].pValue != null && cka_label.equals(new String(label[0].getCharArray()))) {
                ...
                if (list.size() == 1) {
                    ...
                    throw new KeyStoreException(
                        "invalid KeyStore state: " + "found " + list.size() + " secret keys sharing CKA_LABEL [" +
                            cka_label + "]");
                }
            } else if (type == ATTR_CLASS_CERT) {
                throw new KeyStoreException(
                    "invalid KeyStore state: " + "found " + h.length + " certificates sharing CKA_ID " + getID(cka_id));
            } else {
                throw new KeyStoreException(
                    "invalid KeyStore state: " + "found " + h.length + " private keys sharing CKA_ID " + getID(cka_id));
            }
        }
    ...
    private boolean mapLabels() throws PKCS11Exception, CertificateException, KeyStoreException {
        ...
        CK_ATTRIBUTE[] trustedAttr = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_TRUSTED) };
        ...
        try {
            ...
            for (long handle : handles) {
                ...
                if (useSecmodTrust) {
                    ...
                    if (CKA_TRUSTED_SUPPORTED) {
                        try {
                            token.p11.C_GetAttributeValue(session.id(), handle, trustedAttr);
            ...
            for (long handle : handles) {
                ...
                if (attrs[0].pValue != null) {
                    ...
                    if (sKeyMap.get(cka_label) == null) {
                        ...
                        throw new KeyStoreException(
                            "invalid KeyStore state: " + "found multiple secret keys sharing same " + "CKA_LABEL [" +
                                cka_label + "]");
                    }
                }
            }
            ...
            ArrayList<AliasInfo> matchedCerts = mapPrivateKeys(pkeyIDs, certMap);
     ...
     */
    private ArrayList<AliasInfo> mapPrivateKeys(ArrayList<byte[]> pkeyIDs, HashMap<String, HashSet<AliasInfo>> certMap)
        throws PKCS11Exception, CertificateException {
        ...
        for (byte[] pkeyID : pkeyIDs) {
            ...
            if (!foundMatch) {
                if (debug != null) {
                    debug.println(
                        "did not find match for private key with CKA_ID [" + getID(pkeyID) + "] (ignoring entry)");
                }
            }
        }
     ...
     * @return true if multiple certs found sharing the same CKA_LABEL
     */
    private boolean mapCerts(ArrayList<AliasInfo> matchedCerts, HashMap<String, HashSet<AliasInfo>> certMap)
        throws PKCS11Exception, CertificateException {
    ...
    private void mapSecretKeys(HashMap<String, AliasInfo> sKeyMap) throws KeyStoreException {
        for (String label : sKeyMap.keySet()) {
            if (aliasMap.containsKey(label)) {
                throw new KeyStoreException(
                    "invalid KeyStore state: " + "found secret key sharing CKA_LABEL [" + label +
