...
final class P11Digest extends MessageDigestSpi implements Cloneable {
    ...
    protected int engineDigest(byte[] digest, int ofs, int len) throws DigestException {
        ...
        try {
            int n;
            if (state == S_BUFFERED) {
                n = token.p11.C_DigestSingle(session.id(), mechanism, buffer, 0, bufOfs, digest, ofs, len);
                bufOfs = 0;
            } else {
                if (bufOfs != 0) {
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        try {
            ...
            if (state == S_INIT) {
                byte[] stateValues = token.p11.C_GetOperationState(session.id());
                token.p11.C_SetOperationState(copy.session.id(), stateValues, 0, 0);
            }
        } catch (PKCS11Exception e) {
            throw (CloneNotSupportedException) (new CloneNotSupportedException(algorithm).initCause(e));
        }
        return copy;
    }
}
