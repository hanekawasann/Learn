...
public final class P11Util {
    ...
    static Provider getSunProvider() {
        ...
        if (p == null) {
            synchronized (LOCK) {
                p = getProvider(sun, "SUN", "sun.security.provider.Sun");
                sun = p;
            }
        }
        return p;
    ...
    static Provider getSunRsaSignProvider() {
        ...
        if (p == null) {
            synchronized (LOCK) {
                p = getProvider(sunRsaSign, "SunRsaSign", "sun.security.rsa.SunRsaSign");
                sunRsaSign = p;
            }
        }
        return p;
    ...
    static Provider getSunJceProvider() {
        ...
        if (p == null) {
            synchronized (LOCK) {
                p = getProvider(sunJce, "SunJCE", "com.sun.crypto.provider.SunJCE");
                sunJce = p;
            }
        }
        return p;
    ...
    private static Provider getProvider(Provider p, String providerName, String className) {
        ...
        if (p == null) {
            try {
                ...
                throw new ProviderException("Could not find provider " + providerName, e);
            }
        }
        return p;
    }
