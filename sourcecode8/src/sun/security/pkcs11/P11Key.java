...
abstract class P11Key implements Key, Length {
    ...
    public String toString() {
        token.ensureValid();
        String s1 = token.provider.getName() + " " + algorithm + " " + type + " key, " + keyLength + " bits";
        s1 += " (id " + keyID + ", " + (tokenObject ? "token" : "session") + " object";
        if (isPublic()) {
            s1 += ")";
        } else {
    ...
    private static CK_ATTRIBUTE[] getAttributes(Session session, long keyID, CK_ATTRIBUTE[] knownAttributes,
        CK_ATTRIBUTE[] desiredAttributes) {
    ...
    static SecretKey secretKey(Session session, long keyID, String algorithm, int keyLength,
        CK_ATTRIBUTE[] attributes) {
        attributes = getAttributes(session, keyID, attributes,
            new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_TOKEN), new CK_ATTRIBUTE(CKA_SENSITIVE),
                new CK_ATTRIBUTE(CKA_EXTRACTABLE), });
        return new P11SecretKey(session, keyID, algorithm, keyLength, attributes);
    }

    static SecretKey masterSecretKey(Session session, long keyID, String algorithm, int keyLength,
        CK_ATTRIBUTE[] attributes, int major, int minor) {
        attributes = getAttributes(session, keyID, attributes,
            new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_TOKEN), new CK_ATTRIBUTE(CKA_SENSITIVE),
                new CK_ATTRIBUTE(CKA_EXTRACTABLE), });
    ...
    static PublicKey publicKey(Session session, long keyID, String algorithm, int keyLength,
        CK_ATTRIBUTE[] attributes) {
        switch (algorithm) {
            case "RSA":
    ...
    static PrivateKey privateKey(Session session, long keyID, String algorithm, int keyLength,
        CK_ATTRIBUTE[] attributes) {
        attributes = getAttributes(session, keyID, attributes,
            new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_TOKEN), new CK_ATTRIBUTE(CKA_SENSITIVE),
                new CK_ATTRIBUTE(CKA_EXTRACTABLE), });
        if (attributes[1].getBoolean() || (attributes[2].getBoolean() == false)) {
            ...
            switch (algorithm) {
                case "RSA":
                    ...
                    CK_ATTRIBUTE[] attrs2 = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_PUBLIC_EXPONENT), };
                    boolean crtKey;
                    try {
            ...
            if (b == null) {
                synchronized (this) {
                    ...
                    if (b == null) {
                        ...
                        try {
                            tempSession = token.getOpSession();
                            CK_ATTRIBUTE[] attributes = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_VALUE), };
                            token.p11.C_GetAttributeValue(tempSession.id(), keyID, attributes);
    ...
    private static class P11TlsMasterSecretKey extends P11SecretKey implements TlsMasterSecret {
        ...
        P11TlsMasterSecretKey(Session session, long keyID, String algorithm, int keyLength, CK_ATTRIBUTE[] attributes,
            int major, int minor) {
    ...
    private static final class P11RSAPrivateKey extends P11Key implements RSAPrivateCrtKey {
        ...
        private synchronized void fetchValues() {
            ...
            CK_ATTRIBUTE[] attributes = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_MODULUS),
                new CK_ATTRIBUTE(CKA_PUBLIC_EXPONENT), new CK_ATTRIBUTE(CKA_PRIVATE_EXPONENT),
                new CK_ATTRIBUTE(CKA_PRIME_1), new CK_ATTRIBUTE(CKA_PRIME_2), new CK_ATTRIBUTE(CKA_EXPONENT_1),
                new CK_ATTRIBUTE(CKA_EXPONENT_2), new CK_ATTRIBUTE(CKA_COEFFICIENT), };
    ...
    private static final class P11RSAPrivateNonCRTKey extends P11Key implements RSAPrivateKey {
        ...
        P11RSAPrivateNonCRTKey(Session session, long keyID, String algorithm, int keyLength,
            CK_ATTRIBUTE[] attributes) {
            super(PRIVATE, session, keyID, algorithm, keyLength, attributes);
        }
    ...
    private static final class P11DSAPublicKey extends P11Key implements DSAPublicKey {
        ...
        private synchronized void fetchValues() {
            ...
            CK_ATTRIBUTE[] attributes = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_VALUE), new CK_ATTRIBUTE(CKA_PRIME),
                new CK_ATTRIBUTE(CKA_SUBPRIME), new CK_ATTRIBUTE(CKA_BASE), };
            fetchAttributes(attributes);
            y = attributes[0].getBigInteger();
            params = new DSAParameterSpec(attributes[1].getBigInteger(), attributes[2].getBigInteger(),
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    Key key = new sun.security.provider.DSAPublicKey(y, params.getP(), params.getQ(), params.getG());
    ...
    private static final class P11DSAPrivateKey extends P11Key implements DSAPrivateKey {
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    Key key = new sun.security.provider.DSAPrivateKey(x, params.getP(), params.getQ(), params.getG());
                    encoded = key.getEncoded();
                } catch (InvalidKeyException e) {
                    throw new ProviderException(e);
                }
    ...
    private static final class P11DHPrivateKey extends P11Key implements DHPrivateKey {
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    DHPrivateKeySpec spec = new DHPrivateKeySpec(x, params.getP(), params.getG());
        ...
        public boolean equals(Object obj) {
            ...
            return ((this.x.compareTo(other.getX()) == 0) && (this.params.getP().compareTo(otherParams.getP()) == 0) &&
    ...
    private static final class P11DHPublicKey extends P11Key implements DHPublicKey {
        ...
        synchronized byte[] getEncodedInternal() {
            ...
            if (encoded == null) {
                fetchValues();
                try {
                    DHPublicKeySpec spec = new DHPublicKeySpec(y, params.getP(), params.getG());
                    KeyFactory kf = KeyFactory.getInstance("DH", P11Util.getSunJceProvider());
        ...
        public boolean equals(Object obj) {
            ...
            DHParameterSpec otherParams = other.getParams();
            return ((this.y.compareTo(other.getY()) == 0) && (this.params.getP().compareTo(otherParams.getP()) == 0) &&
                (this.params.getG().compareTo(otherParams.getG()) == 0));
        }
    }
    ...
    private static final class P11ECPrivateKey extends P11Key implements ECPrivateKey {
        ...
        private synchronized void fetchValues() {
            ...
            try {
                params = P11ECKeyFactory.decodeParameters(attributes[1].getByteArray());
            } catch (Exception e) {
    ...
    private static final class P11ECPublicKey extends P11Key implements ECPublicKey {
        ...
        private synchronized void fetchValues() {
            ...
            CK_ATTRIBUTE[] attributes = new CK_ATTRIBUTE[] { new CK_ATTRIBUTE(CKA_EC_POINT),
                ...
                if (!token.config.getUseEcX963Encoding()) {
                    ...
                    if (wECPoint.getTag() != DerValue.tag_OctetString) {
                        throw new IOException(
                            "Could not DER decode EC point." + " Unexpected tag: " + wECPoint.getTag());
                    }
                    w = P11ECKeyFactory.decodePoint(wECPoint.getDataBytes(), params.getCurve());
...
final class SessionKeyRef extends PhantomReference<P11Key> implements Comparable<SessionKeyRef> {
    private static ReferenceQueue<P11Key> refQueue = new ReferenceQueue<P11Key>();
    private static Set<SessionKeyRef> refList = Collections.synchronizedSortedSet(new TreeSet<SessionKeyRef>());
