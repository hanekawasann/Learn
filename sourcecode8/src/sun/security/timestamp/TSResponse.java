...
public class TSResponse {
    ...
    public String getFailureCodeAsText() {
        ...
        try {
            if (isSet(BAD_ALG)) { return "Unrecognized or unsupported algorithm identifier."; }
            if (isSet(BAD_REQUEST)) { return "The requested transaction is not permitted or " + "supported."; }
            if (isSet(BAD_DATA_FORMAT)) { return "The data submitted has the wrong format."; }
            if (isSet(TIME_NOT_AVAILABLE)) { return "The TSA's time source is not available."; }
            if (isSet(UNACCEPTED_POLICY)) { return "The requested TSA policy is not supported by the TSA."; }
            if (isSet(UNACCEPTED_EXTENSION)) { return "The requested extension is not supported by the TSA."; }
            if (isSet(ADD_INFO_NOT_AVAILABLE)) {
                return "The additional information requested could not be " + "understood or is not available.";
            }
            if (isSet(SYSTEM_FAILURE)) { return "The request cannot be handled due to system failure."; }
        ...
        if (this.status == 0 || this.status == 1) {
            if (tsToken == null) {
                throw new TimestampException(
                    "Bad encoding for timestamp response: " + "expected a timeStampToken element to be present");
            }
        } else if (tsToken != null) {
            throw new TimestampException(
                "Bad encoding for timestamp response: " + "expected no timeStampToken element to be present");
        }
    }
