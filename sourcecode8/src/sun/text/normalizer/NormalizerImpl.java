...
public final class NormalizerImpl {
    ...
    static final class NormTrieImpl implements Trie.DataManipulate {
        ...
        public int getFoldingOffset(int value) {
            return BMP_INDEX_LENGTH +
                ((value >> (EXTRA_SHIFT - SURROGATE_BLOCK_BITS)) & (0x3ff << SURROGATE_BLOCK_BITS));
        }
     ...
     * Constructor
     *
     * @throws thrown when data reading fails or data corrupted
     */
    ...
    public static long/*unsigned*/ getNorm32FromSurrogatePair(long norm32, char c2) {
        /*
     ...
     */
    private static long/*unsigned*/ getNorm32(char[] p, int start, int/*unsigned*/ mask) {
     ...
     */
    private static int/*index*/ decompose(long/*unsigned*/ norm32, int/*unsigned*/ qcMask, DecomposeArgs args) {
     ...
     * @return index into the extraData array
     */
    private static int decompose(long/*unsigned*/ norm32, DecomposeArgs args) {
     ...
     */
    private static long /*unsigned*/ getPrevNorm32(PrevArgs args, int/*unsigned*/ minC, int/*unsigned*/ mask) {
        long/*unsigned*/ norm32;
     ...
     * (lead cc==0)
     */
    public static boolean isNFDSafe(long/*unsigned*/ norm32, int/*unsigned*/ccOrQCMask, int/*unsigned*/ decompQCMask) {
     ...
     * (cc==0 and NF*C_YES)
     */
    public static boolean isTrueStarter(long/*unsigned*/ norm32, int/*unsigned*/ ccOrQCMask,
     ...
     */
    private static int/*unsigned byte*/ insertOrdered(char[] source, int start, int current, int p, char c, char c2,
        int/*unsigned byte*/ cc) {
        int back, preBack;
        int r;
    ...
    private static int /*unsigned byte*/ mergeOrdered(char[] source, int start, int current, char[] data,
        final int next, final int limit) {
        return mergeOrdered(source, start, current, data, next, limit, true);
    }

    public static NormalizerBase.QuickCheckResult quickCheck(char[] src, int srcStart, int srcLimit, int minNoMaybe,
        int qcMask, int options, boolean allowMaybe, UnicodeSet nx) {
        ...
        for (; ; ) {
            ...
            if ((qcNorm32 & QC_ANY_NO) >= 1) {
                ...
                if (allowMaybe) {
                    ...
                    prevStarter = findPreviousStarter(src, start, prevStarter, ccOrQCMask, decompQCMask,
                        (char) minNoMaybe);
    ...
    public static int decompose(char[] src, int srcStart, int srcLimit, char[] dest, int destStart, int destLimit,
        boolean compat, int[] outTrailCC, UnicodeSet nx) {
        ...
        for (; ; ) {
            ...
            while (srcIndex != srcLimit &&
                ((c = src[srcIndex]) < minNoMaybe || ((norm32 = getNorm32(c)) & ccOrQCMask) == 0)) {
                prevCC = 0;
                ++srcIndex;
            }
    ...
    private static int /*unsigned*/    getNextCombining(NextCombiningArgs args, int limit, UnicodeSet nx) {
        long/*unsigned*/ norm32;
        int combineFlags;
        /* get properties */
     ...
     */
    private static int/*unsigned*/ combine(char[] table, int tableStart, int/*unsinged*/ combineBackIndex,
        int[] outValues) {
        int/*unsigned*/ key;
        int value, value2;
            ...
            if (((combineFlags & COMBINES_BACK) != 0) && starter != -1) {
                if ((combineBackIndex & 0x8000) != 0) {
                    ...
                    if ((options & BEFORE_PRI_29) != 0 || prevCC == 0) {
                        ...
                        if (combineBackIndex == 0xfff2) {
                            ...
                            if (ncArg.c2 < JAMO_L_COUNT) {
                                remove = args.start - 1;
                                ncArg.c = (char) (HANGUL_BASE +
                                    (ncArg.c2 * JAMO_V_COUNT + (ncArg.c - JAMO_V_BASE)) * JAMO_T_COUNT);
                    ...
                    !((combineFwdIndex & 0x8000) != 0) &&
                        ...
                        0 != (result = combine(combiningTable, combineFwdIndex, combineBackIndex, outValues)) &&
    ...
    private static int findPreviousStarter(char[] src, int srcStart, int current, int/*unsigned*/ ccOrQCMask,
     ...
     * pointer to it
     */
    private static int/*index*/    findNextStarter(char[] src, int start, int limit, int/*unsigned*/ qcMask,
        int/*unsigned*/ decompQCMask, char minNoMaybe) {
        int p;
        long/*unsigned*/ norm32;
        int ccOrQCMask;
    ...
    /* decompose and recompose [prevStarter..src[ */
    private static char[] composePart(ComposePartArgs args, int prevStarter, char[] src, int start, int limit,
    ...
    private static boolean composeHangul(char prev, char c, long/*unsigned*/ norm32, char[] src, int[] srcIndex,
        int limit, boolean compat, char[] dest, int destIndex, UnicodeSet nx) {
    ...
    */
    ...
    public static int compose(char[] src, int srcStart, int srcLimit, char[] dest, int destStart, int destLimit,
        int options, UnicodeSet nx) {
        ...
        for (; ; ) {
            ...
            if (srcIndex != prevSrc) {
                ...
                if (UTF16.isTrailSurrogate(src[prevStarter]) && prevSrc < prevStarter &&
            ...
            if (isNorm32HangulOrJamo(norm32)) {
                ...
                reorderStartIndex = destIndex;
                ioIndex[0] = srcIndex;
                if (destIndex > 0 && composeHangul(src[(prevSrc - 1)], c, norm32, src, ioIndex, srcLimit,
                    (options & OPTIONS_COMPAT) != 0, dest, destIndex <= destLimit ? destIndex - 1 : 0, nx)) {
                    srcIndex = ioIndex[0];
                    prevStarter = srcIndex;
                    continue;
                }
    ...
    public static final int quickCheck(int c, int modeValue) {
        final int qcMask[/*UNORM_MODE_COUNT*/] = { 0, 0, QC_NFD, QC_NFKD, QC_NFC, QC_NFKC };
    ...
    private static int strCompare(char[] s1, int s1Start, int s1Limit, char[] s2, int s2Start, int s2Limit,
        ...
        if (c1 >= 0xd800 && c2 >= 0xd800 && codePointOrder) {
             ...
             *  supplementary ones */
            if ((c1 <= 0xdbff && (s1Start + 1) != limit1 && UTF16.isTrailSurrogate(s1[(s1Start + 1)])) ||
                (UTF16.isTrailSurrogate(c1) && start1 != s1Start && UTF16.isLeadSurrogate(s1[(s1Start - 1)]))) {
            ...
            if ((c2 <= 0xdbff && (s2Start + 1) != limit2 && UTF16.isTrailSurrogate(s2[(s2Start + 1)])) ||
                (UTF16.isTrailSurrogate(c2) && start2 != s2Start && UTF16.isLeadSurrogate(s2[(s2Start - 1)]))) {
                /* part of a surrogate pair, leave >=d800 */
            } else {
                /* BMP code point - may be surrogate code point - make <d800 */
                c2 -= 0x2800;
    ...
    public static int getDecompose(int chars[], String decomps[]) {
        ...
        while (++ch < 0x2fa1e) {   //no cannoical above 0x3ffff
            ...
            if (ch == 0x30ff) { ch = 0xf900; } else if (ch == 0x10000) { ch = 0x1d15e; } else if (ch == 0x1d1c1) {
                ch = 0x2f800;
            }
    ...
    private static final char[][] corrigendum4MappingTable = { { '\uD844', '\uDF6A' },  // 0x2F868
        { '\u5F33' },            // 0x2F874
        { '\u43AB' },            // 0x2F91F
        { '\u7AAE' },            // 0x2F95F
        { '\u4D57' } };           // 0x2F9BF
