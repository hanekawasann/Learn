...
public class SnmpCachedData implements SnmpTableHandler {
    ...
    public static final Comparator<SnmpOid> oidComparator = new Comparator<SnmpOid>() {
        ...
        public boolean equals(Object o1, Object o2) {
            if (o1 == o2) { return true; } else { return o1.equals(o2); }
        }
    };
    ...
    public SnmpCachedData(long lastUpdated, TreeMap<SnmpOid, Object> indexMap, boolean b) {
        ...
        } else { indexMap.values().toArray(this.datas); }
    }
    ...
    public SnmpOid getNext(SnmpOid index) {
        if (index == null) {
            if (indexes.length > 0) { return indexes[0]; } else { return null; }
        }
        final int pos = find(index);
        if (pos > -1) {
            if (pos < (indexes.length - 1)) { return indexes[pos + 1]; } else { return null; }
        }
        final int insertion = -pos - 1;
        if ((insertion > -1) && (insertion < indexes.length)) { return indexes[insertion]; } else { return null; }
    }
