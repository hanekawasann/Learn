...
public final class AdaptorBootstrap {
    ...
    private static List<NotificationTarget> getTargetList(InetAddressAcl acl, int defaultTrapPort) {
        final ArrayList<NotificationTarget> result = new ArrayList<>();
        if (acl != null) {
            if (log.isDebugOn()) {
            ...
            for (; td.hasMoreElements(); ) {
                ...
                final Enumeration<String> tc = acl.getTrapCommunities(targetAddr);
                for (; tc.hasMoreElements(); ) {
                    final String community = tc.nextElement();
                    final NotificationTarget target = new NotificationTargetImpl(targetAddr, defaultTrapPort,
                        community);
                    if (log.isDebugOn()) {
                        log.debug("getTargetList",
                            Agent.getText("jmxremote.AdaptorBootstrap.getTargetList.adding", target.toString()));
                    }
                    result.add(target);
                }
            }
    ...
    private static AdaptorBootstrap getAdaptorBootstrap(int port, int trapPort, String bindAddress, boolean useAcl,
        String aclFileName) {
        ...
        try {
            ...
            throw new AgentConfigurationError(SNMP_ADAPTOR_START_FAILED, t, address + ":" + port,
                "(" + t.getMessage() + ")");
        }
        ...
        log.config("initialize", Agent
            .getText("jmxremote.AdaptorBootstrap.getTargetList.initialize2", address.toString(),
                java.lang.Integer.toString(adaptor.getPort())));
