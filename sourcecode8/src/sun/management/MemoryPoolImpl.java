...
class MemoryPoolImpl implements MemoryPoolMXBean {
    ...
    public void setUsageThreshold(long newThreshold) {
        ...
        if (usage.getMax() != -1 && newThreshold > usage.getMax()) {
            throw new IllegalArgumentException(
                "Invalid threshold: " + newThreshold + " must be <= maxSize." + " Committed = " + usage.getCommitted() +
                    " Max = " + usage.getMax());
        }
    ...
    public boolean isUsageThresholdExceeded() {
        ...
        return (u.getUsed() >= usageThreshold || usageSensor.isOn());
    }
    ...
    public void setCollectionUsageThreshold(long newThreshold) {
        ...
        if (usage.getMax() != -1 && newThreshold > usage.getMax()) {
            throw new IllegalArgumentException(
                "Invalid threshold: " + newThreshold + " > max (" + usage.getMax() + ").");
        }
    ...
    class PoolSensor extends Sensor {
        ...
        void triggerAction(MemoryUsage usage) {
            ...
            MemoryImpl.createNotification(MEMORY_THRESHOLD_EXCEEDED, pool.getName(), usage, getCount());
        }
    ...
    class CollectionSensor extends Sensor {
        ...
        void triggerAction(MemoryUsage usage) {
            MemoryImpl
                .createNotification(MEMORY_COLLECTION_THRESHOLD_EXCEEDED, pool.getName(), usage, gcSensor.getCount());
        }
