...
public class GarbageCollectionNotifInfoCompositeData extends LazyCompositeData {
    ...
    private CompositeType getCompositeTypeByBuilder() {
        ...
        synchronized (compositeTypeByBuilder) {
            ...
            if (gict == null) {
                OpenType<?>[] gcNotifInfoItemTypes = new OpenType<?>[] { SimpleType.STRING, SimpleType.STRING,
                    SimpleType.STRING, builder.getGcInfoCompositeType(), };
                try {
                    final String typeName = "sun.management.GarbageCollectionNotifInfoCompositeType";
                    gict = new CompositeType(typeName, "CompositeType for GC notification info", gcNotifInfoItemNames,
                        gcNotifInfoItemNames, gcNotifInfoItemTypes);
    ...
    protected CompositeData getCompositeData() {
        ...
        gcNotifInfoItemValues = new Object[] { gcNotifInfo.getGcName(), gcNotifInfo.getGcAction(),
            gcNotifInfo.getGcCause(), GcInfoCompositeData.toCompositeData(gcNotifInfo.getGcInfo()) };
    ...
    public static String getGcName(CompositeData cd) {
        ...
        if (gcname == null) {
            throw new IllegalArgumentException("Invalid composite data: " + "Attribute " + GC_NAME + " has null value");
        }
        return gcname;
    }
    ...
    public static String getGcAction(CompositeData cd) {
        ...
        if (gcaction == null) {
            throw new IllegalArgumentException(
                "Invalid composite data: " + "Attribute " + GC_ACTION + " has null value");
        }
        return gcaction;
    }
    ...
    public static String getGcCause(CompositeData cd) {
        ...
        if (gccause == null) {
            throw new IllegalArgumentException(
                "Invalid composite data: " + "Attribute " + GC_CAUSE + " has null value");
        }
        return gccause;
    }
    ...
    private static synchronized CompositeType getBaseGcNotifInfoCompositeType() {
        if (baseGcNotifInfoCompositeType == null) {
            try {
                OpenType<?>[] baseGcNotifInfoItemTypes = new OpenType<?>[] { SimpleType.STRING, SimpleType.STRING,
                    SimpleType.STRING, GcInfoCompositeData.getBaseGcInfoCompositeType() };
                baseGcNotifInfoCompositeType = new CompositeType(
                    "sun.management.BaseGarbageCollectionNotifInfoCompositeType",
                    "CompositeType for Base GarbageCollectionNotificationInfo", gcNotifInfoItemNames,
                    gcNotifInfoItemNames, baseGcNotifInfoItemTypes);
            } catch (OpenDataException e) {
                // shouldn't reach here
                throw Util.newException(e);
            }
