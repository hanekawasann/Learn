...
public final class CompositeStrike extends FontStrike {
    ...
    CompositeStrike(CompositeFont font2D, FontStrikeDesc desc) {
        ...
        if (desc.style != compFont.style) {
            algoStyle = true;
            if ((desc.style & Font.BOLD) == Font.BOLD && ((compFont.style & Font.BOLD) == 0)) {
                boldness = 1.33f;
            }
            if ((desc.style & Font.ITALIC) == Font.ITALIC && (compFont.style & Font.ITALIC) == 0) {
                italic = 0.7f;
            }
        }
    ...
    PhysicalStrike getStrikeForSlot(int slot) {
        ...
        if (strike == null) {
            strike = (PhysicalStrike) (compFont.getSlotFont(slot).getStrike(desc));
