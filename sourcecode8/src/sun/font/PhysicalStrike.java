...
public abstract class PhysicalStrike extends FontStrike {
    ...
    protected static final int complexTX = AffineTransform.TYPE_FLIP | AffineTransform.TYPE_GENERAL_SCALE |
    ...
    Point2D.Float getGlyphPoint(int glyphCode, int ptNumber) {
        ...
        if (glyphPointMapCache == null) {
            synchronized (this) {
                if (glyphPointMapCache == null) {
                    glyphPointMapCache = new ConcurrentHashMap<Integer, Point2D.Float>();
                }
            }
        } else {
