...
public class StandardGlyphVector extends GlyphVector {
    ...
    public StandardGlyphVector(Font font, FontRenderContext frc, int[] glyphs, float[] positions, int[] indices,
        int flags) {
    ...
    public void initGlyphVector(Font font, FontRenderContext frc, int[] glyphs, float[] positions, int[] indices,
        int flags) {
        this.font = font;
        this.frc = frc;
        this.glyphs = glyphs;
    ...
    public static StandardGlyphVector getStandardGV(GlyphVector gv, FontInfo info) {
        if (info.aaHint == SunHints.INTVAL_TEXT_ANTIALIAS_ON) {
            ...
            if (aaHint != VALUE_TEXT_ANTIALIAS_ON && aaHint != VALUE_TEXT_ANTIALIAS_GASP) {
                /* We need to create a new GV with AA==ON for rendering */
                FontRenderContext frc = gv.getFontRenderContext();
                frc = new FontRenderContext(frc.getTransform(), VALUE_TEXT_ANTIALIAS_ON,
    ...
    public int[] getGlyphCharIndices(int start, int count, int[] result) {
        ...
        if (charIndices == null) {
            if ((getLayoutFlags() & FLAG_RUN_RTL) != 0) {
                for (int i = 0, n = glyphs.length - 1 - start; i < count; ++i, --n) {
                    result[i] = n;
                }
            } else {
    ...
    boolean needsPositions(double[] devTX) {
        return gti != null || (getLayoutFlags() & FLAG_HAS_POSITION_ADJUSTMENTS) != 0 || !matchTX(devTX, frctx);
    }
    ...
    private static boolean matchTX(double[] lhs, AffineTransform rhs) {
        return lhs[0] == rhs.getScaleX() && lhs[1] == rhs.getShearY() && lhs[2] == rhs.getShearX() &&
            lhs[3] == rhs.getScaleY();
    }
    ...
    private static AffineTransform getNonTranslateTX(AffineTransform tx) {
        if (tx.getTranslateX() != 0 || tx.getTranslateY() != 0) {
            tx = new AffineTransform(tx.getScaleX(), tx.getShearY(), tx.getShearX(), tx.getScaleY(), 0, 0);
        }
        return tx;
    }

    private static boolean equalNonTranslateTX(AffineTransform lhs, AffineTransform rhs) {
        return lhs.getScaleX() == rhs.getScaleX() && lhs.getShearY() == rhs.getShearY() &&
            lhs.getShearX() == rhs.getShearX() && lhs.getScaleY() == rhs.getScaleY();
    }
    ...
    static final class GlyphTransformInfo {
        ...
        AffineTransform getGlyphTransform(int ix) {
            ...
            int x = (index - 1) * 6;
            return new AffineTransform(transforms[x + 0], transforms[x + 1], transforms[x + 2], transforms[x + 3],
                transforms[x + 4], transforms[x + 5]);
        }
        ...
        private GlyphStrike getStrikeAtIndex(GlyphStrike[] strikes, int strikeIndex) {
            ...
            if (strike == null) {
                if (strikeIndex == 0) {
                    ...
                    AffineTransform gtx = new AffineTransform(transforms[ix], transforms[ix + 1], transforms[ix + 2],
                        transforms[ix + 3], transforms[ix + 4], transforms[ix + 5]);
    ...
    public static final class GlyphStrike {
        ...
        static GlyphStrike create(StandardGlyphVector sgv, AffineTransform dtx, AffineTransform gtx) {
            ...
            if (aaHint == VALUE_TEXT_ANTIALIAS_GASP) {
                 ...
                 */
                if (!tx.isIdentity() && (tx.getType() & ~AffineTransform.TYPE_TRANSLATION) != 0) {
        ...
        Rectangle2D getGlyphOutlineBounds(int glyphID, float x, float y) {
             ...
             */
            if (!result.isEmpty()) {
                result.setRect(result.getMinX() + x + dx, result.getMinY() + y + dy, result.getWidth(),
                    result.getHeight());
            }
            return result;
        }
