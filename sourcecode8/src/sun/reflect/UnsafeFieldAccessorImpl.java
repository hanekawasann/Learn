...
abstract class UnsafeFieldAccessorImpl extends FieldAccessorImpl {
    ...
    UnsafeFieldAccessorImpl(Field field) {
        this.field = field;
        if (Modifier.isStatic(field.getModifiers())) { fieldOffset = unsafe.staticFieldOffset(field); } else {
    ...
    protected IllegalArgumentException newGetIllegalArgumentException(String type) {
        return new IllegalArgumentException(
            "Attempt to get " + field.getType().getName() + " field \"" + getQualifiedFieldName() +
                "\" with illegal data type conversion to " + type);
    }

    protected void throwFinalFieldIllegalAccessException(String attemptedType, String attemptedValue)
    ...
    protected String getSetMessage(String attemptedType, String attemptedValue) {
        ...
        if (Modifier.isStatic(field.getModifiers())) { err += " static"; }
        if (isFinal) { err += " final"; }
        err += " " + field.getType().getName() + " field " + getQualifiedFieldName() + " to ";
        if (attemptedValue.length() > 0) {
            err += "(" + attemptedType + ")" + attemptedValue;
        } else {
            if (attemptedType.length() > 0) { err += attemptedType; } else { err += "null value"; }
        }
        return err;
    }
