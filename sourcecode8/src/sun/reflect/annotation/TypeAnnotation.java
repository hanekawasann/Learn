...
public final class TypeAnnotation {
    ...
    public TypeAnnotation(TypeAnnotationTargetInfo targetInfo, LocationInfo loc, Annotation annotation,
    ...
    public static List<TypeAnnotation> filter(TypeAnnotation[] typeAnnotations, TypeAnnotationTarget predicate) {
        ...
        for (TypeAnnotation t : typeAnnotations) {
            if (t.getTargetInfo().getTarget() == predicate) { typeAnnos.add(t); }
        }
    ...
    public static final class LocationInfo {
        ...
        public static LocationInfo parseLocationInfo(ByteBuffer buf) {
            ...
            if (depth == 0) { return BASE_LOCATION; }
        ...
        public TypeAnnotation[] filter(TypeAnnotation[] ta) {
            ...
            for (TypeAnnotation t : ta) {
                if (isSameLocationInfo(t.getLocationInfo())) { l.add(t); }
            }
        ...
        boolean isSameLocationInfo(LocationInfo other) {
            ...
            for (int i = 0; i < depth; i++) { if (!locations[i].isSameLocation(other.locations[i])) { return false; } }
            return true;
        }
    ...
    @Override
    public String toString() {
        return annotation.toString() + " with Targetnfo: " + targetInfo.toString() + " on base declaration: " +
