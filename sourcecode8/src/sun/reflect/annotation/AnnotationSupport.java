...
public final class AnnotationSupport {
    ...
    public static <A extends Annotation> A[] getDirectlyAndIndirectlyPresent(
        Map<Class<? extends Annotation>, Annotation> annotations, Class<A> annoClass) {
    ...
    public static <A extends Annotation> A[] getAssociatedAnnotations(
        Map<Class<? extends Annotation>, Annotation> declaredAnnotations, Class<?> decl, Class<A> annoClass) {
    ...
    private static <A extends Annotation> A[] getValueArray(Annotation container) {
        try {
            ...
            if (annoType == null) { throw invalidContainerException(container, null); }

            Method m = annoType.members().get("value");
            if (m == null) { throw invalidContainerException(container, null); }
        ...
        for (A a : annotations) {
            if (!annoClass.isInstance(a)) {
                throw new AnnotationFormatError(String
                    .format("%s is an invalid container for " + "repeating annotations of type: %s", container,
                        annoClass));
            }
        }
    }
