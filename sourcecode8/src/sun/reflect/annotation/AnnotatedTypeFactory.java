...
public final class AnnotatedTypeFactory {
     ...
     *                        corresponds to
     */
    public static AnnotatedType buildAnnotatedType(Type type, LocationInfo currentLoc, TypeAnnotation[] actualTypeAnnos,
        TypeAnnotation[] allOnSameTarget, AnnotatedElement decl) {
        if (type == null) {
        ...
        if (type instanceof Class) {
            return new AnnotatedTypeBaseImpl(type, addNesting(type, currentLoc), actualTypeAnnos, allOnSameTarget,
                decl);
        } else if (type instanceof TypeVariable) {
            return new AnnotatedTypeVariableImpl((TypeVariable) type, currentLoc, actualTypeAnnos, allOnSameTarget,
                decl);
        } else if (type instanceof ParameterizedType) {
            return new AnnotatedParameterizedTypeImpl((ParameterizedType) type, addNesting(type, currentLoc),
                actualTypeAnnos, allOnSameTarget, decl);
        } else if (type instanceof WildcardType) {
            return new AnnotatedWildcardTypeImpl((WildcardType) type, currentLoc, actualTypeAnnos, allOnSameTarget,
                decl);
        }
    ...
    private static LocationInfo addNesting(Type type, LocationInfo addTo) {
        if (isArray(type)) { return addTo; }
        if (type instanceof Class) {
            Class<?> clz = (Class) type;
            if (clz.getEnclosingClass() == null) { return addTo; }
            if (Modifier.isStatic(clz.getModifiers())) { return addNesting(clz.getEnclosingClass(), addTo); }
            return addNesting(clz.getEnclosingClass(), addTo.pushInner());
        } else if (type instanceof ParameterizedType) {
            ParameterizedType t = (ParameterizedType) type;
            if (t.getOwnerType() == null) { return addTo; }
    ...
    private static class AnnotatedTypeBaseImpl implements AnnotatedType {
        ...
        AnnotatedTypeBaseImpl(Type type, LocationInfo location, TypeAnnotation[] actualTypeAnnotations,
            TypeAnnotation[] allOnSameTargetTypeAnnotations, AnnotatedElement decl) {
            this.type = type;
            this.decl = decl;
    ...
    private static final class AnnotatedArrayTypeImpl extends AnnotatedTypeBaseImpl implements AnnotatedArrayType {
        AnnotatedArrayTypeImpl(Type type, LocationInfo location, TypeAnnotation[] actualTypeAnnotations,
            TypeAnnotation[] allOnSameTargetTypeAnnotations, AnnotatedElement decl) {
        ...
        public AnnotatedType getAnnotatedGenericComponentType() {
            return AnnotatedTypeFactory
                .buildAnnotatedType(getComponentType(), getLocation().pushArray(), getTypeAnnotations(),
                    getTypeAnnotations(), getDecl());
        }
    ...
    private static final class AnnotatedTypeVariableImpl extends AnnotatedTypeBaseImpl
        implements AnnotatedTypeVariable {
        AnnotatedTypeVariableImpl(TypeVariable<?> type, LocationInfo location, TypeAnnotation[] actualTypeAnnotations,
            TypeAnnotation[] allOnSameTargetTypeAnnotations, AnnotatedElement decl) {
            super(type, location, actualTypeAnnotations, allOnSameTargetTypeAnnotations, decl);
        }
        ...
        public AnnotatedType[] getAnnotatedActualTypeArguments() {
            ...
            for (int i = 0; i < res.length; i++) {
                ...
                for (TypeAnnotation t : getTypeAnnotations()) {
                    if (t.getLocationInfo().isSameLocationInfo(newLoc)) { l.add(t); }
                }
                res[i] = buildAnnotatedType(arguments[i], newLoc, l.toArray(new TypeAnnotation[0]),
                    getTypeAnnotations(), getDecl());
            }
            return res;
        }
    ...
    private static final class AnnotatedWildcardTypeImpl extends AnnotatedTypeBaseImpl
        implements AnnotatedWildcardType {
        private final boolean hasUpperBounds;

        AnnotatedWildcardTypeImpl(WildcardType type, LocationInfo location, TypeAnnotation[] actualTypeAnnotations,
            TypeAnnotation[] allOnSameTargetTypeAnnotations, AnnotatedElement decl) {
            super(type, location, actualTypeAnnotations, allOnSameTargetTypeAnnotations, decl);
        ...
        public AnnotatedType[] getAnnotatedUpperBounds() {
            if (!hasUpperBounds()) { return new AnnotatedType[0]; }
        ...
        private AnnotatedType[] getAnnotatedBounds(Type[] bounds) {
            ...
            for (int i = 0; i < res.length; i++) {
                ...
                res[i] = buildAnnotatedType(bounds[i], newLoc, l.toArray(new TypeAnnotation[0]), getTypeAnnotations(),
                    getDecl());
            }
            return res;
        }
