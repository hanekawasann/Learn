...
public final class TypeAnnotationParser {
     ...
     */
    public static AnnotatedType buildAnnotatedType(byte[] rawAnnotations, ConstantPool cp, AnnotatedElement decl,
        Class<?> container, Type type, TypeAnnotationTarget filter) {
        ...
        for (TypeAnnotation t : tas) {
            ...
            if (ti.getTarget() == filter) { l.add(t); }
        }
        TypeAnnotation[] typeAnnotations = l.toArray(new TypeAnnotation[0]);
        return AnnotatedTypeFactory
            .buildAnnotatedType(type, LocationInfo.BASE_LOCATION, typeAnnotations, typeAnnotations, decl);
    }
     ...
     */
    public static AnnotatedType[] buildAnnotatedTypes(byte[] rawAnnotations, ConstantPool cp, AnnotatedElement decl,
        Class<?> container, Type[] types, TypeAnnotationTarget filter) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            result[i] = AnnotatedTypeFactory
                .buildAnnotatedType(types[i], LocationInfo.BASE_LOCATION, typeAnnotations, typeAnnotations, decl);
    ...
    public static AnnotatedType buildAnnotatedSuperclass(byte[] rawAnnotations, ConstantPool cp, Class<?> decl) {
        ...
        if (supertype == null) { return AnnotatedTypeFactory.EMPTY_ANNOTATED_TYPE; }
        return buildAnnotatedType(rawAnnotations, cp, decl, decl, supertype, TypeAnnotationTarget.CLASS_EXTENDS);
    }
    ...
    public static AnnotatedType[] buildAnnotatedInterfaces(byte[] rawAnnotations, ConstantPool cp, Class<?> decl) {
        ...
        return buildAnnotatedTypes(rawAnnotations, cp, decl, decl, decl.getGenericInterfaces(),
    ...
    public static <D extends GenericDeclaration> Annotation[] parseTypeVariableAnnotations(D genericDecl,
        ...
        List<TypeAnnotation> typeVarAnnos = TypeAnnotation.filter(parseAllTypeAnnotations(decl), predicate);
        List<Annotation> res = new ArrayList<>(typeVarAnnos.size());
        for (TypeAnnotation t : typeVarAnnos) {
            if (t.getTargetInfo().getCount() == typeVarIndex) { res.add(t.getAnnotation()); }
        }
     ...
     */
    public static <D extends GenericDeclaration> AnnotatedType[] parseAnnotatedBounds(Type[] bounds, D decl,
        int typeVarIndex) {
        return parseAnnotatedBounds(bounds, decl, typeVarIndex, LocationInfo.BASE_LOCATION);
    }

    //helper for above
    private static <D extends GenericDeclaration> AnnotatedType[] parseAnnotatedBounds(Type[] bounds, D decl,
        int typeVarIndex, LocationInfo loc) {
        ...
        if (bounds != null) {
            ...
            for (int i = 0; i < bounds.length; i++) {
                ...
                for (TypeAnnotation t : candidates) {
                    ...
                    res[i] = AnnotatedTypeFactory.buildAnnotatedType(bounds[i], loc, l.toArray(new TypeAnnotation[0]),
                        candidates.toArray(new TypeAnnotation[0]), (AnnotatedElement) decl);
                }
            }
            return res;
        }
    ...
    private static TypeAnnotation[] parseTypeAnnotations(byte[] rawAnnotations, ConstantPool cp,
        AnnotatedElement baseDecl, Class<?> container) {
        if (rawAnnotations == null) { return EMPTY_TYPE_ANNOTATION_ARRAY; }
        ...
        for (int i = 0; i < annotationCount; i++) {
            ...
            if (ta != null) { typeAnnotations.add(ta); }
        }
    ...
    private static TypeAnnotation parseTypeAnnotation(ByteBuffer buf, ConstantPool cp, AnnotatedElement baseDecl,
        Class<?> container) {
        try {
        ...
        switch (posCode) {
            ...
            case METHOD_TYPE_PARAMETER: {
                ...
                if (posCode == CLASS_TYPE_PARAMETER) {
                    res = new TypeAnnotationTargetInfo(TypeAnnotationTarget.CLASS_TYPE_PARAMETER, index);
                } else { res = new TypeAnnotationTargetInfo(TypeAnnotationTarget.METHOD_TYPE_PARAMETER, index); }
                return res;
            } // unreachable break;
            case CLASS_EXTENDS: {
            ...
            case METHOD_FORMAL_PARAMETER: {
                int index = buf.get() & 0xFF;
                return new TypeAnnotationTargetInfo(TypeAnnotationTarget.METHOD_FORMAL_PARAMETER, index);
            } //unreachable break;
            case THROWS:
    ...
    private static TypeAnnotationTargetInfo parse2ByteTarget(TypeAnnotationTarget target, ByteBuffer buf) {
        ...
        return new TypeAnnotationTargetInfo(target, count, secondaryIndex);
    }
}
