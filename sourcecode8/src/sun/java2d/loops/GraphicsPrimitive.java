...
public abstract class GraphicsPrimitive {
    ...
    public synchronized static final int makeUniqueID(int primTypeID, SurfaceType src, CompositeType cmp,
        SurfaceType dst) {
     ...
     */
    protected GraphicsPrimitive(String methodSignature, int primTypeID, SurfaceType sourceType,
     ...
     */
    protected GraphicsPrimitive(long pNativePrim, String methodSignature, int primTypeID, SurfaceType sourceType,
        CompositeType compositeType, SurfaceType destType) {
    ...
    final boolean satisfiesSameAs(GraphicsPrimitive other) {
        return (methodSignature == other.methodSignature && sourceType.equals(other.sourceType) &&
            compositeType.equals(other.compositeType) && destType.equals(other.destType));
    }
        ...
        if (trace != null) {
            ...
            while (st.hasMoreTokens()) {
                ...
                if (tok.equalsIgnoreCase("count")) {
                    ...
                    System.err.println("usage: -Dsun.java2d.trace=" + "[log[,timestamp]],[count]," +
    ...
    private static PrintStream getTraceOutputFile() {
        if (traceout == null) {
            if (tracefile != null) {
                FileOutputStream o = AccessController.doPrivileged(new PrivilegedAction<FileOutputStream>() {
    ...
    protected void setupGeneralBinaryOp(GeneralBinaryOp gbo) {
        ...
        if (performop != null) {
            ...
            if (performop == null) {
                throw new InternalError("Cannot construct general op for " + methodSignature + " " + comptype);
            }
    ...
    protected void setupGeneralUnaryOp(GeneralUnaryOp guo) {
        ...
        if (convertdst == null || performop == null || convertres == null) {
            throw new InternalError("Cannot construct binary op for " + comptype + " " + dsttype);
        }
    ...
    protected static Blit createConverter(SurfaceType srctype, SurfaceType dsttype) {
        ...
        if (cv == null) {
            throw new InternalError("Cannot construct converter for " + srctype + "=>" + dsttype);
        }
        return cv;
    }

    protected static SurfaceData convertFrom(Blit ob, SurfaceData srcData, int srcX, int srcY, int w, int h,
        SurfaceData dstData) {
    ...
    protected static void convertTo(Blit ob, SurfaceData srcImg, SurfaceData dstImg, Region clip, int dstX, int dstY,
        int w, int h) {
        if (ob != null) {
    ...
    public String toString() {
        if (cachedname == null) {
            ...
            cachedname = (getClass().getName() + "::" + sig + "(" + simplename(sourceType) + ", " +
                simplename(compositeType) + ", " + simplename(destType) + ")");
        }
        return cachedname;
    }
}
