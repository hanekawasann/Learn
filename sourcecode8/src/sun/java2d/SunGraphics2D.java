...
public final class SunGraphics2D extends Graphics2D implements ConstrainableGraphics, Cloneable, DestSurfaceProvider {
    ...
    Shape intersectRectShape(Rectangle2D r, Shape s, boolean keep1, boolean keep2) {
        if (s instanceof Rectangle2D) {
            ...
            { outrect.setFrameFromDiagonal(0, 0, 0, 0); } else { outrect.setFrameFromDiagonal(x1, y1, x2, y2); }
            return outrect;
        }
        ...
        if (aahint == SunHints.INTVAL_TEXT_ANTIALIAS_DEFAULT) {
            ...
            if (aahint == SunHints.INTVAL_TEXT_ANTIALIAS_GASP) {
                ...
                if (!surfaceData.canRenderLCDText(this)
                     ...
                     */
                    info.lcdSubPixPos = fmhint == SunHints.INTVAL_FRACTIONALMETRICS_ON &&
                        aahint == SunHints.INTVAL_TEXT_ANTIALIAS_LCD_HRGB;
                }
            }
        }
    ...
    public void setFont(Font font) {
        ...
        if (font != null && font != this.font/*!font.equals(this.font)*/) {
             ...
             */
            if (textAntialiasHint == SunHints.INTVAL_TEXT_ANTIALIAS_GASP && textpipe != invalidpipe &&
                (transformState > TRANSFORM_ANY_TRANSLATE || font.isTransformed() || fontInfo == null ||
                    // Precaution, if true shouldn't get here
                    (fontInfo.aaHint == SunHints.INTVAL_TEXT_ANTIALIAS_ON) != FontUtilities.getFont2D(font).
    ...
    public void setComposite(Composite comp) {
        ...
        if (comp instanceof AlphaComposite) {
            ...
            } else if (newCompType == CompositeType.SrcNoEa || newCompType == CompositeType.Src ||
    ...
    public void setRenderingHints(Map<?, ?> hints) {
        ...
        while (iter.hasNext()) {
            ...
            if (key == SunHints.KEY_RENDERING || key == SunHints.KEY_ANTIALIASING ||
                key == SunHints.KEY_TEXT_ANTIALIASING || key == SunHints.KEY_FRACTIONALMETRICS ||
                key == SunHints.KEY_TEXT_ANTIALIAS_LCD_CONTRAST || key == SunHints.KEY_STROKE_CONTROL ||
    ...
    public void setTransform(AffineTransform Tx) {
        if ((constrainX | constrainY) == 0 && devScale == 1) {
            ...
            transform.setTransform(devScale, 0, 0, devScale, constrainX, constrainY);
    ...
    public AffineTransform getTransform() {
        ...
        AffineTransform tx = new AffineTransform(invScale, 0, 0, invScale, -constrainX * invScale,
            -constrainY * invScale);
    ...
    final void validateColor() {
        ...
        if (imageComp == CompositeType.Clear) {
            ...
            if (compositeState <= COMP_ALPHA && imageComp != CompositeType.SrcNoEa &&
                imageComp != CompositeType.SrcOverNoEa) {
    ...
    */
     ...
     * be used.
     */
    private static Rectangle getImageRegion(RenderedImage img, Region compClip, AffineTransform transform,
        AffineTransform xform, int padX, int padY) {
    ...
    private void drawTranslatedRenderedImage(RenderedImage img, Rectangle region, int i2uTransX, int i2uTransY) {
        ...
        for (int ty = minTileY; ty <= maxTileY; ty++) {
            for (int tx = minTileX; tx <= maxTileX; tx++) {
                ...
                wRaster = wRaster
                    .createWritableChild(tileRect.x, tileRect.y, tileRect.width, tileRect.height, 0, 0, null);
                ...
                copyImage(bufImg, tileRect.x + i2uTransX, tileRect.y + i2uTransY, 0, 0, tileRect.width, tileRect.height,
                    null, null);
            }
        }
    }
    ...
    private boolean drawHiDPIImage(Image img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2,
     ...
     */
    public boolean copyImage(Image img, int dx, int dy, int sx, int sy, int width, int height, Color bgcolor,
        ...
        if (cachedFRC == null) {
            ...
            if (transformState >= TRANSFORM_TRANSLATESCALE) {
                if (transform.getTranslateX() == 0 && transform.getTranslateY() == 0) {
                    ...
                    tx = new AffineTransform(transform.getScaleX(), transform.getShearY(), transform.getShearX(),
                        transform.getScaleY(), 0, 0);
                }
            }
            cachedFRC = new FontRenderContext(tx, SunHints.Value.get(SunHints.INTKEY_TEXT_ANTIALIASING, aahint),
                SunHints.Value.get(SunHints.INTKEY_FRACTIONALMETRICS, fractionalMetricsHint));
        }
        return cachedFRC;
    }
