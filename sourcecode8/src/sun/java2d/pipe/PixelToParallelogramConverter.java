...
public class PixelToParallelogramConverter extends PixelToShapeConverter implements ShapeDrawPipe {
     ...
     */
    public PixelToParallelogramConverter(ShapeDrawPipe shapepipe, ParallelogramPipe pgrampipe, double minPenSize,
        double normPosition, boolean adjustfill) {
        super(shapepipe);
    ...
    public void draw(SunGraphics2D sg2d, Shape s) {
        if (sg2d.strokeState < SunGraphics2D.STROKE_CUSTOM) {
            ...
            if (s instanceof Rectangle2D) {
                ...
                Line2D l2d = (Line2D) s;
                if (drawGeneralLine(sg2d, l2d.getX1(), l2d.getY1(), l2d.getX2(), l2d.getY2())) {
                    return;
                }
            }
        }
    ...
    public boolean drawGeneralLine(SunGraphics2D sg2d, double ux1, double uy1, double ux2, double uy2) {
        if (sg2d.strokeState == SunGraphics2D.STROKE_CUSTOM || sg2d.strokeState == SunGraphics2D.STROKE_THINDASHED) {
            return false;
        }
        BasicStroke bs = (BasicStroke) sg2d.stroke;
        ...
        if (sg2d.strokeHint != SunHints.INTVAL_STROKE_PURE) {
            if (sg2d.strokeState == SunGraphics2D.STROKE_THIN && outrenderer instanceof PixelDrawPipe) {
        ...
        outrenderer.fillParallelogram(sg2d, ux1, uy1, ux2, uy2, px, py, -udy, udx, dx, dy);
        return true;
    }
    ...
    public void fillRectangle(SunGraphics2D sg2d, double rx, double ry, double rw, double rh) {
        ...
        dx2 *= rh;
        dy2 *= rh;
        if (adjustfill && sg2d.strokeState < SunGraphics2D.STROKE_CUSTOM &&
        ...
        outrenderer.fillParallelogram(sg2d, rx, ry, rx + rw, ry + rh, px, py, dx1, dy1, dx2, dy2);
    }
    ...
    public void drawRectangle(SunGraphics2D sg2d, double rx, double ry, double rw, double rh, double lw) {
        ...
        if (lw1 >= len1 || lw2 >= len2) {
            ...
            outrenderer
                .drawParallelogram(sg2d, rx, ry, rx + rw, ry + rh, px, py, dx1, dy1, dx2, dy2, lw1 / len1, lw2 / len2);
        }
    }
     ...
     */
    public void fillOuterParallelogram(SunGraphics2D sg2d, double ux1, double uy1, double ux2, double uy2, double px,
        double py, double dx1, double dy1, double dx2, double dy2, double len1, double len2, double lw1, double lw2) {
        ...
        outrenderer.fillParallelogram(sg2d, ux1, uy1, ux2, uy2, px, py, dx1, dy1, dx2, dy2);
    }
}
