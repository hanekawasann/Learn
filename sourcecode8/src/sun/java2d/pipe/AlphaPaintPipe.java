...
public class AlphaPaintPipe implements CompositePipe {
    ...
    public Object startSequence(SunGraphics2D sg, Shape s, Rectangle devR, int[] abox) {
        PaintContext paintContext = sg.paint
            .createContext(sg.getDeviceColorModel(), devR, s.getBounds2D(), sg.cloneTransform(),
        ...
        for (int rely = 0; rely < h; rely += TILE_SIZE) {
            ...
            for (int relx = 0; relx < w; relx += TILE_SIZE) {
                ...
                if (lastRas != srcRaster) {
                    ...
                    BufferedImage bImg = new BufferedImage(paintModel, (WritableRaster) srcRaster,
                        paintModel.isAlphaPremultiplied(), null);
                ...
                if (atile == null) {
                    if (context.lastBlit == null) {
                        ...
                        context.lastBlit = Blit
                            .getFromCache(srcData.getSurfaceType(), comptype, dstData.getSurfaceType());
                    }
                    ...
                    if (context.lastMask == null) {
                        ...
                        context.lastMask = MaskBlit
                            .getFromCache(srcData.getSurfaceType(), comptype, dstData.getSurfaceType());
                    }

                    int toff = offset + rely * tilesize + relx;
                    context.lastMask
                        .MaskBlit(srcData, dstData, sg.composite, null, 0, 0, tx, ty, tw, th, atile, toff, tilesize);
                }
            }
        }
    }
