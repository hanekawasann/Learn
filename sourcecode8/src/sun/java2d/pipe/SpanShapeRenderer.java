...
public abstract class SpanShapeRenderer implements ShapeDrawPipe {
    ...
    public void fill(SunGraphics2D sg, Shape s) {
        if (s instanceof Rectangle2D && (sg.transform.getType() & NON_RECTILINEAR_TRANSFORM_MASK) == 0) {
    ...
    public void renderRect(SunGraphics2D sg, Rectangle2D r) {
        double corners[] = { r.getX(), r.getY(), r.getWidth(), r.getHeight(), };
        corners[2] += corners[0];
        corners[3] += corners[1];
        ...
        int abox[] = { (int) corners[0], (int) corners[1], (int) corners[2], (int) corners[3], };
        ...
        if (clipRegion.isRectangular()) {
            renderBox(context, abox[0], abox[1], abox[2] - abox[0], abox[3] - abox[1]);
        } else {
