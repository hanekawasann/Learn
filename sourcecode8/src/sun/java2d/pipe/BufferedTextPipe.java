...
public abstract class BufferedTextPipe extends GlyphListPipe {
    ...
    private static int createPackedParams(SunGraphics2D sg2d, GlyphList gl) {
        return (((gl.usePositions() ? 1 : 0) << OFFSET_POSITIONS) | ((gl.isSubPixPos() ? 1 : 0) << OFFSET_SUBPIXPOS) |
            ((gl.isRGBOrder() ? 1 : 0) << OFFSET_RGBORDER) | ((sg2d.lcdTextContrast & 0xff) << OFFSET_CONTRAST));
    }
    ...
    private void enqueueGlyphList(final SunGraphics2D sg2d, final GlyphList gl) {
        ...
        if (totalBytesRequired <= buf.capacity()) {
            ...
            rq.flushAndInvokeNow(new Runnable() {
                public void run() {
                    drawGlyphList(totalGlyphs, gl.usePositions(), gl.isSubPixPos(), gl.isRGBOrder(),
                        sg2d.lcdTextContrast, glyphListOrigX, glyphListOrigY, images, gl.getPositions());
                }
            });
        }
    }
     ...
     */
    protected abstract void drawGlyphList(int numGlyphs, boolean usePositions, boolean subPixPos, boolean rgbOrder,
        int lcdContrast, float glOrigX, float glOrigY, long[] images, float[] positions);
