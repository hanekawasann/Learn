...
public class BufferedBufImgOps {
    ...
    public static void enableBufImgOp(RenderQueue rq, SurfaceData srcData, BufferedImage srcImg, BufferedImageOp biop) {
    ...
    public static boolean isRescaleOpValid(RescaleOp rop, BufferedImage srcImg) {
    ...
    private static void enableRescaleOp(RenderQueue rq, SurfaceData srcData, BufferedImage srcImg, RescaleOp rop) {
        // assert rq.lock.isHeldByCurrentThread();
        ColorModel srcCM = srcImg.getColorModel();
    ...
    public static boolean isLookupOpValid(LookupOp lop, BufferedImage srcImg) {
        ...
        if (numComps != 1 && numComps != srcCM.getNumComponents() && numComps != srcCM.getNumColorComponents()) {
            throw new IllegalArgumentException(
                "Number of arrays in the " + " lookup table (" + numComps + ") is not compatible with" +
