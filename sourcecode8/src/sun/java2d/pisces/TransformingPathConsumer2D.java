...
final class TransformingPathConsumer2D {
    ...
    public static PathConsumer2D inverseDeltaTransformConsumer(PathConsumer2D out, AffineTransform at) {
        if (at == null) {
            return out;
        }
        float Mxx = (float) at.getScaleX();
        ...
        if (Mxy == 0f && Myx == 0f) {
            ...
            return new DeltaTransformFilter(out, Myy / det, -Mxy / det, -Myx / det, Mxx / det);
        }
    }
    ...
    static final class ScaleFilter implements PathConsumer2D {
        ...
        public void curveTo(float x1, float y1, float x2, float y2, float x3, float y3) {
            out.curveTo(x1 * sx + tx, y1 * sy + ty, x2 * sx + tx, y2 * sy + ty, x3 * sx + tx, y3 * sy + ty);
        }
    ...
    static final class TransformFilter implements PathConsumer2D {
        ...
        public void quadTo(float x1, float y1, float x2, float y2) {
            out.quadTo(x1 * Mxx + y1 * Mxy + Mxt, x1 * Myx + y1 * Myy + Myt, x2 * Mxx + y2 * Mxy + Mxt,
                x2 * Myx + y2 * Myy + Myt);
        }

        public void curveTo(float x1, float y1, float x2, float y2, float x3, float y3) {
            out.curveTo(x1 * Mxx + y1 * Mxy + Mxt, x1 * Myx + y1 * Myy + Myt, x2 * Mxx + y2 * Mxy + Mxt,
                x2 * Myx + y2 * Myy + Myt, x3 * Mxx + y3 * Mxy + Mxt, x3 * Myx + y3 * Myy + Myt);
        }
    ...
    static final class DeltaTransformFilter implements PathConsumer2D {
        ...
        public void quadTo(float x1, float y1, float x2, float y2) {
            out.quadTo(x1 * Mxx + y1 * Mxy, x1 * Myx + y1 * Myy, x2 * Mxx + y2 * Mxy, x2 * Myx + y2 * Myy);
        }

        public void curveTo(float x1, float y1, float x2, float y2, float x3, float y3) {
            out.curveTo(x1 * Mxx + y1 * Mxy, x1 * Myx + y1 * Myy, x2 * Mxx + y2 * Mxy, x2 * Myx + y2 * Myy,
                x3 * Mxx + y3 * Mxy, x3 * Myx + y3 * Myy);
        }
