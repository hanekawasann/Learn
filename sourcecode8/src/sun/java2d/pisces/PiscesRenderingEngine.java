...
public class PiscesRenderingEngine extends RenderingEngine {
     ...
     * @since 1.7
     */
    public Shape createStrokedShape(Shape src, float width, int caps, int join, float miterlimit, float dashes[],
        float dashphase) {
     ...
     * @since 1.7
     */
    public void strokeTo(Shape src, AffineTransform at, BasicStroke bs, boolean thin, boolean normalize,
    ...
    void strokeTo(Shape src, AffineTransform at, BasicStroke bs, boolean thin, NormMode normalize, boolean antialias,
        PathConsumer2D pc2d) {
        float lw;
        if (thin) {
            if (antialias) {
        ...
        strokeTo(src, at, lw, normalize, bs.getEndCap(), bs.getLineJoin(), bs.getMiterLimit(), bs.getDashArray(),
            bs.getDashPhase(), pc2d);
    }
    ...
    private float userSpaceLineWidth(AffineTransform at, float lw) {
        ...
        if ((at.getType() & (AffineTransform.TYPE_GENERAL_TRANSFORM | AffineTransform.TYPE_GENERAL_SCALE)) != 0) {
    ...
    void strokeTo(Shape src, AffineTransform at, float width, NormMode normalize, int caps, int join, float miterlimit,
        float dashes[], float dashphase, PathConsumer2D pc2d) {
    ...
    private static class NormalizingPathIterator implements PathIterator {
            ...
            switch (mode) {
                ...
                case OFF:
                    throw new InternalError(
                        "A NormalizingPathIterator should " + "not be created if no normalization is being done");
                default:
        ...
        public int currentSegment(float[] coords) {
            ...
            // normalize endpoint
            float x_adjust = (float) Math.floor(coords[lastCoord] + lval) + rval - coords[lastCoord];
            float y_adjust = (float) Math.floor(coords[lastCoord + 1] + lval) + rval - coords[lastCoord + 1];
     ...
     * @since 1.7
     */
    public AATileGenerator getAATileGenerator(Shape s, AffineTransform at, Region clip, BasicStroke bs, boolean thin,
        ...
        if (bs == null) {
            ...
            r = new Renderer(3, 3, clip.getLoX(), clip.getLoY(), clip.getWidth(), clip.getHeight(),
                pi.getWindingRule());
            pathTo(pi, r);
        } else {
            r = new Renderer(3, 3, clip.getLoX(), clip.getLoY(), clip.getWidth(), clip.getHeight(),
    ...
    public AATileGenerator getAATileGenerator(double x, double y, double dx1, double dy1, double dx2, double dy2,
        double lw1, double lw2, Region clip, int bbox[]) {
        ...
        Renderer r = new Renderer(3, 3, clip.getLoX(), clip.getLoY(), clip.getWidth(), clip.getHeight(),
    ...
    static {
        if (PathIterator.WIND_NON_ZERO != Renderer.WIND_NON_ZERO ||
            PathIterator.WIND_EVEN_ODD != Renderer.WIND_EVEN_ODD || BasicStroke.JOIN_MITER != Stroker.JOIN_MITER ||
            BasicStroke.JOIN_ROUND != Stroker.JOIN_ROUND || BasicStroke.JOIN_BEVEL != Stroker.JOIN_BEVEL ||
            BasicStroke.CAP_BUTT != Stroker.CAP_BUTT || BasicStroke.CAP_ROUND != Stroker.CAP_ROUND ||
