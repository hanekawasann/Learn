...
abstract class LongPipeline<E_IN> extends AbstractPipeline<E_IN, Long, LongStream> implements LongStream {
    ...
    private static LongConsumer adapt(Sink<Long> sink) {
        if (sink instanceof LongConsumer) {
            ...
            if (Tripwire.ENABLED) { Tripwire.trip(AbstractPipeline.class, "using LongStream.adapt(Sink<Long> s)"); }
            return sink::accept;
        }
    }
    ...
    @Override
    final <P_IN> Node<Long> evaluateToNode(PipelineHelper<Long> helper, Spliterator<P_IN> spliterator,
        boolean flattenTree, IntFunction<Long[]> generator) {
        return Nodes.collectLong(helper, spliterator, flattenTree);
    }

    @Override
    final <P_IN> Spliterator<Long> wrap(PipelineHelper<Long> ph, Supplier<Spliterator<P_IN>> supplier,
        boolean isParallel) {
    ...
    public final LongStream skip(long n) {
        ...
        if (n == 0) { return this; } else { return SliceOps.makeLong(this, n, -1); }
    }
    ...
    abstract static class StatefulOp<E_IN> extends LongPipeline<E_IN> {
        ...
        @Override
        abstract <P_IN> Node<Long> opEvaluateParallel(PipelineHelper<Long> helper, Spliterator<P_IN> spliterator,
