...
public class JarFile extends ZipFile {
    ...
    public Stream<JarEntry> stream() {
        return StreamSupport.stream(Spliterators.spliterator(new JarEntryIterator(), size(),
            Spliterator.ORDERED | Spliterator.DISTINCT | Spliterator.IMMUTABLE | Spliterator.NONNULL), false);
    }
    ...
    public synchronized InputStream getInputStream(ZipEntry ze) throws IOException {
        ...
        return new JarVerifier.VerifierStream(getManifestFromReference(),
            ze instanceof JarFileEntry ? (JarEntry) ze : getJarEntry(ze.getName()), super.getInputStream(ze), jv);
    }
    ...
    private void checkForSpecialAttributes() throws IOException {
        ...
        if (!isKnownNotToHaveSpecialAttributes()) {
            ...
            if (manEntry != null) {
                byte[] b = getBytes(manEntry);
                if (match(CLASSPATH_CHARS, b, CLASSPATH_LASTOCC, CLASSPATH_OPTOSFT)) { hasClassPathAttribute = true; }
            }
        }
