...
public class StampedLock implements java.io.Serializable {
    ...
    public long writeLockInterruptibly() throws InterruptedException {
        long next;
        if (!Thread.interrupted() && (next = acquireWrite(true, 0L)) != INTERRUPTED) { return next; }
        throw new InterruptedException();
    }
    ...
    public long readLock() {
        ...
        return ((whead == wtail && (s & ABITS) < RFULL && U.compareAndSwapLong(this, STATE, s, next = s + RUNIT)) ? next
    ...
    public long tryReadLock() {
        for (; ; ) {
            long s, m, next;
            if ((m = (s = state) & ABITS) == WBIT) { return 0L; } else if (m < RFULL) {
    ...
    public long readLockInterruptibly() throws InterruptedException {
        long next;
        if (!Thread.interrupted() && (next = acquireRead(true, 0L)) != INTERRUPTED) { return next; }
        throw new InterruptedException();
    }
    ...
    public void unlockRead(long stamp) {
        ...
        for (; ; ) {
            if (((s = state) & SBITS) != (stamp & SBITS) || (stamp & ABITS) == 0L || (m = s & ABITS) == 0L ||
                m == WBIT) { throw new IllegalMonitorStateException(); }
            if (m < RFULL) {
    ...
    public void unlock(long stamp) {
        ...
        while (((s = state) & SBITS) == (stamp & SBITS)) {
            if ((m = s & ABITS) == 0L) { break; } else if (m == WBIT) {
                if (a != m) { break; }
                state = (s += WBIT) == 0L ? ORIGIN : s;
                if ((h = whead) != null && h.status != 0) { release(h); }
                return;
            } else if (a == 0L || a >= WBIT) { break; } else if (m < RFULL) {
                if (U.compareAndSwapLong(this, STATE, s, s - RUNIT)) {
                    if (m == RUNIT && (h = whead) != null && h.status != 0) { release(h); }
                    return;
                }
    ...
    public long tryConvertToReadLock(long stamp) {
        ...
        while (((s = state) & SBITS) == (stamp & SBITS)) {
            if ((m = s & ABITS) == 0L) {
                ...
                if ((h = whead) != null && h.status != 0) { release(h); }
                return next;
            } else if (a != 0L && a < WBIT) { return stamp; } else { break; }
        }
        return 0L;
    }
    ...
    private int getReadLockCount(long s) {
        long readers;
        if ((readers = s & RBITS) >= RFULL) { readers = RFULL + readerOverflow; }
        return (int) readers;
    }
    ...
    final void unstampedUnlockWrite() {
        WNode h;
        long s;
        if (((s = state) & WBIT) == 0L) { throw new IllegalMonitorStateException(); }
        state = (s += WBIT) == 0L ? ORIGIN : s;
        if ((h = whead) != null && h.status != 0) { release(h); }
    }
    ...
    final void unstampedUnlockRead() {
        for (; ; ) {
            long s, m;
            WNode h;
            if ((m = (s = state) & ABITS) == 0L || m >= WBIT) { throw new IllegalMonitorStateException(); } else if (m <
                RFULL) {
                if (U.compareAndSwapLong(this, STATE, s, s - RUNIT)) {
                    if (m == RUNIT && (h = whead) != null && h.status != 0) { release(h); }
                    break;
    ...
    private long acquireWrite(boolean interruptible, long deadline) {
        ...
        for (int spins = -1; ; ) { // spin while enqueuing
            ...
            } else if (spins < 0) { spins = (m == WBIT && wtail == whead) ? SPINS : 0; } else if (spins > 0) {
                if (LockSupport.nextSecondarySeed() >= 0) { --spins; }
            } else if ((p = wtail) == null) { // initialize queue
                WNode hd = new WNode(WMODE, null);
                if (U.compareAndSwapObject(this, WHEAD, null, hd)) { wtail = hd; }
            } else if (node == null) { node = new WNode(WMODE, p); } else if (node.prev != p) {
                node.prev = p;
        ...
        for (int spins = -1; ; ) {
            ...
            int ps;
            if ((h = whead) == p) {
                if (spins < 0) { spins = HEAD_SPINS; } else if (spins < MAX_HEAD_SPINS) { spins <<= 1; }
            ...
            if (whead == h) {
                if ((np = node.prev) != p) {
                ...
                } else if ((ps = p.status) == 0) { U.compareAndSwapInt(p, WSTATUS, 0, WAITING); } else if (ps ==
                    CANCELLED) {
                    if ((pp = p.prev) != null) {
                        node.prev = pp;
                        pp.next = node;
                    ...
                    if (deadline == 0L) { time = 0L; } else if ((time = deadline - System.nanoTime()) <= 0L) {
    ...
    private long acquireRead(boolean interruptible, long deadline) {
        ...
        for (int spins = -1; ; ) {
            ...
            if ((h = whead) == (p = wtail)) {
                for (long m, s, ns; ; ) {
                    if ((m = (s = state) & ABITS) < RFULL ? U.compareAndSwapLong(this, STATE, s, ns = s + RUNIT)
                        : (m < WBIT && (ns = tryIncReaderOverflow(s)) != 0L)) { return ns; } else if (m >= WBIT) {
                        if (spins > 0) {
                            if (LockSupport.nextSecondarySeed() >= 0) { --spins; }
                        } else {
                            if (spins == 0) {
            ...
            if (p == null) { // initialize queue
                WNode hd = new WNode(WMODE, null);
                if (U.compareAndSwapObject(this, WHEAD, null, hd)) { wtail = hd; }
            } else if (node == null) { node = new WNode(RMODE, p); } else if (h == p || p.mode != RMODE) {
                if (node.prev != p) { node.prev = p; } else if (U.compareAndSwapObject(this, WTAIL, p, node)) {
                    p.next = node;
                    break;
                }
            } else if (!U.compareAndSwapObject(p, WCOWAIT, node.cowait = p.cowait, node)) { node.cowait = null; } else {
                for (; ; ) {
                    WNode pp, c;
                    Thread w;
                    if ((h = whead) != null && (c = h.cowait) != null &&
                        U.compareAndSwapObject(h, WCOWAIT, c, c.cowait) && (w = c.thread) != null) // help release
                    { U.unpark(w); }
                    if (h == (pp = p.prev) || h == p || pp == null) {
                        long m, s, ns;
                        do {
                            if ((m = (s = state) & ABITS) < RFULL ? U.compareAndSwapLong(this, STATE, s, ns = s + RUNIT)
                                : (m < WBIT && (ns = tryIncReaderOverflow(s)) != 0L)) { return ns; }
                        } while (m < WBIT);
                    }
