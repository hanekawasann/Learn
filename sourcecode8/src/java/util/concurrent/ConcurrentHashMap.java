...
public class ConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K, V> e = f, pred = null; ; ) {
                                ...
                                if (e.hash == hash && ((ek = e.key) == key || (ek != null && key.equals(ek)))) {
                                    ...
                                    if (cv == null || cv == ev || (ev != null && cv.equals(ev))) {
                                        oldVal = ev;
                                        if (value != null) { e.val = value; } else if (pred != null) {
                                            pred.next = e.next;
                                        } else {
                            ...
                            if ((r = t.root) != null && (p = r.findTreeNode(hash, key, null)) != null) {
                                ...
                                if (cv == null || cv == pv || (pv != null && cv.equals(pv))) {
                                    oldVal = pv;
                                    if (value != null) { p.val = value; } else if (t.removeTreeNode(p)) {
    ...
    private void readObject(java.io.ObjectInputStream s) throws java.io.IOException, ClassNotFoundException {
        ...
        if (size == 0L) { sizeCtl = 0; } else {
            int n;
            if (size >= (long) (MAXIMUM_CAPACITY >>> 1)) { n = MAXIMUM_CAPACITY; } else {
                int sz = (int) size;
            ...
            while (p != null) {
                ...
                if ((first = tabAt(tab, j)) == null) { insertAtFront = true; } else {
                    K k = p.key;
                    if (first.hash < 0) {
                        ...
                        if (insertAtFront && binCount >= TREEIFY_THRESHOLD) {
                            ...
                            for (q = p; q != null; q = q.next) {
                                ...
                                if ((t.prev = tl) == null) { hd = t; } else { tl.next = t; }
                                tl = t;
                            }
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K, V> e = f, pred = null; ; ++binCount) {
                                ...
                                if (e.hash == h && ((ek = e.key) == key || (ek != null && key.equals(ek)))) {
                                    ...
                                    if (val != null) { e.val = val; } else {
                                        delta = -1;
                                        Node<K, V> en = e.next;
                                        if (pred != null) { pred.next = en; } else { setTabAt(tab, i, en); }
                                    }
                                    break;
                                }
                                pred = e;
                ...
                synchronized (f) {
                    if (tabAt(tab, i) == f) {
                        if (fh >= 0) {
                            ...
                            TreeBin<K, V> t = (TreeBin<K, V>) f;
                            TreeNode<K, V> r, p;
                            if ((r = t.root) != null) { p = r.findTreeNode(h, key, null); } else { p = null; }
                            V pv = (p == null) ? null : p.val;
                            val = remappingFunction.apply(key, pv);
                            if (val != null) {
                                if (p != null) { p.val = val; } else {
                                    delta = 1;
                                    t.putTreeVal(h, key, val);
                                }
                            } else if (p != null) {
        ...
        while ((sc = sizeCtl) >= 0) {
            ...
            if (tab == null || (n = tab.length) == 0) {
                ...
                if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                    // yukms note: 初始化tab的时候，把sizeCtl设为-1
                    try {
                ...
                // yukms note: “tab == table”？？？

                // yukms note: ？？？下面的代码也很懵逼
                int rs = resizeStamp(n);
                if (sc < 0) {
                    // yukms note: 如果正在扩容Table的话，则帮助扩容
                    Node<K, V>[] nt;
                    if ((sc >>> RESIZE_STAMP_SHIFT) != rs//
                        || sc == rs + 1 //
                        || sc == rs + MAX_RESIZERS//
     ...
     */
    // yukms note: 将每个bin中的节点移动和/或复制到新表中
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        ...
        for (; ; ) {
            ...
            if ((as = counterCells) != null && (n = as.length) > 0) {
                ...
                } else if (U.compareAndSwapLong(a, CELLVALUE, v = a.value, v + x)) { break; } else if (
                    counterCells != as || n >= NCPU) {
                    collide = false;            // At max size or stale
                } else if (!collide) { collide = true; } else if (cellsBusy == 0 &&
    ...
    private final void treeifyBin(Node<K, V>[] tab, int index) {
        ...
        int n, sc;
        if (tab != null) {
            if ((n = tab.length) < MIN_TREEIFY_CAPACITY) { tryPresize(n << 1); } else if (
        ...
        final TreeNode<K, V> findTreeNode(int h, Object k, Class<?> kc) {
            if (k != null) {
                ...
                do {
                    ...
                    if ((ph = p.hash) > h) { p = pl; } else if (ph < h) { p = pr; } else if ((pk = p.key) == k ||
                        (pk != null && k.equals(pk))) {
                        return p;
                    } else if (pl == null) {
                        p = pr;
                    } else if (pr == null) { p = pl; } else if ((kc != null || (kc = comparableClassFor(k)) != null) &&
                        (dir = compareComparables(kc, k, pk)) != 0) { p = (dir < 0) ? pl : pr; } else if (
                        (q = pr.findTreeNode(h, k, kc)) != null) { return q; } else { p = pl; }
                } while (p != null);
            }
            return null;
        }
    ...
    static final class TreeBin<K, V> extends Node<K, V> {
            ...
            for (TreeNode<K, V> x = b, next; x != null; x = next) {
                ...
                if (r == null) {
                    ...
                    for (TreeNode<K, V> p = r; ; ) {
                        int dir, ph;
                        K pk = p.key;
                        if ((ph = p.hash) > h) { dir = -1; } else if (ph < h) { dir = 1; } else if (
        ...
        final TreeNode<K, V> putTreeVal(int h, K k, V v) {
            ...
            for (TreeNode<K, V> p = root; ; ) {
                ...
                if (p == null) {
                    ...
                    break;
                } else if ((ph = p.hash) > h) { dir = -1; } else if (ph < h) { dir = 1; } else if ((pk = p.key) == k ||
                    (pk != null && k.equals(pk))) {
                    return p;
                    ...
                    if (!searched) {
                        ...
                        if (((ch = p.left) != null && (q = ch.findTreeNode(h, k, kc)) != null) ||
                            ((ch = p.right) != null && (q = ch.findTreeNode(h, k, kc)) != null)) { return q; }
                    }
        ...
        final boolean removeTreeNode(TreeNode<K, V> p) {
            ...
            TreeNode<K, V> r, rl;
            if (pred == null) { first = next; } else { pred.next = next; }
            ...
            try {
                ...
                if (pl != null && pr != null) {
                    ...
                    if ((s.parent = pp) == null) { r = s; } else if (p == pp.left) { pp.left = s; } else {
                        pp.right = s;
                    }
                ...
                if (replacement != p) {
                    ...
                    if (pp == null) { r = replacement; } else if (p == pp.left) { pp.left = replacement; } else {
    ...
    static class Traverser<K, V> {
            ...
            for (; ; ) {
                ...
                if ((e = tabAt(t, i)) != null && e.hash < 0) {
                    if (e instanceof ForwardingNode) {
                        ...
                        pushState(t, i, n);
                        continue;
                    } else if (e instanceof TreeBin) { e = ((TreeBin<K, V>) e).first; } else { e = null; }
                }
                if (stack != null) { recoverState(n); } else if ((index = i + baseSize) >= n) {
        ...
        private void pushState(Node<K, V>[] t, int i, int n) {
            ...
            if (s != null) { spare = s.next; } else { s = new TableStack<K, V>(); }
            s.tab = t;
            s.length = n;
            s.index = i;
            s.next = stack;
        ...
        public final Object[] toArray() {
            ...
            for (E e : this) {
                if (i == n) {
                    ...
                    if (n >= MAX_ARRAY_SIZE - (MAX_ARRAY_SIZE >>> 1) - 1) { n = MAX_ARRAY_SIZE; } else {
                        n += (n >>> 1) + 1;
                    }
                    r = Arrays.copyOf(r, n);
                }
    ...
    abstract static class BulkTask<K, V, R> extends CountedCompleter<R> {
        ...
        BulkTask(BulkTask<K, V, ?> par, int b, int i, int f, Node<K, V>[] t) {
            ...
            if ((this.tab = t) == null) { this.baseSize = this.baseLimit = 0; } else if (par == null) {
        ...
        final Node<K, V> advance() {
            ...
            for (; ; ) {
                ...
                if (stack != null) { recoverState(n); } else if ((index = i + baseSize) >= n) { index = ++baseIndex; }
            }
        }
    ...
    static final class MapReduceEntriesToDoubleTask<K, V> extends BulkTask<K, V, Double> {
        ...
        MapReduceEntriesToDoubleTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
            MapReduceEntriesToDoubleTask<K, V> nextRight, ToDoubleFunction<Map.Entry<K, V>> transformer, double basis,
            DoubleBinaryOperator reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.transformer = transformer;
    ...
    static final class MapReduceMappingsToLongTask<K, V> extends BulkTask<K, V, Long> {
        ...
        MapReduceMappingsToLongTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
            MapReduceMappingsToLongTask<K, V> nextRight, ToLongBiFunction<? super K, ? super V> transformer, long basis,
            LongBinaryOperator reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.transformer = transformer;
    ...
    static final class MapReduceMappingsToIntTask<K, V> extends BulkTask<K, V, Integer> {
        ...
        MapReduceMappingsToIntTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
            MapReduceMappingsToIntTask<K, V> nextRight, ToIntBiFunction<? super K, ? super V> transformer, int basis,
            IntBinaryOperator reducer) {
            super(p, b, i, f, t);
            this.nextRight = nextRight;
            this.transformer = transformer;
