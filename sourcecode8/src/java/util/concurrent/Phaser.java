...
public class Phaser {
            ...
            if (counts != EMPTY) {                  // not 1st registration
                if (parent == null || reconcileState() == s) {
                    ...
                    { root.internalAwaitAdvance(phase, null); } else if (UNSAFE
                        .compareAndSwapLong(this, stateOffset, s, s + adjust)) { break; }
                }
                ...
                synchronized (this) {               // 1st sub registration
                    if (state == s) {               // recheck under lock
                        ...
                        while (!UNSAFE
                            .compareAndSwapLong(this, stateOffset, s, ((long) phase << PHASE_SHIFT) | adjust)) {
                            s = state;
    ...
    private long reconcileState() {
        ...
        if (root != this) {
            ...
            while ((phase = (int) (root.state >>> PHASE_SHIFT)) != (int) (s >>> PHASE_SHIFT) && !UNSAFE
                .compareAndSwapLong(this, stateOffset, s, s = (((long) phase << PHASE_SHIFT) |
                    ((phase < 0) ? (s & COUNTS_MASK)
                        : (((p = (int) s >>> PARTIES_SHIFT) == 0) ? EMPTY : ((s & PARTIES_MASK) | p)))))) { s = state; }
        }
        return s;
    }
    ...
    static final class QNode implements ForkJoinPool.ManagedBlocker {
        ...
        public boolean block() {
            if (isReleasable()) { return true; } else if (!timed) { LockSupport.park(this); } else if (nanos > 0L) {
