...
public class TreeMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Cloneable, java.io.Serializable {
    ...
    static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
        ...
        public Iterator<E> iterator() {
            if (m instanceof TreeMap) { return ((TreeMap<E, ?>) m).keyIterator(); } else {
                return ((TreeMap.NavigableSubMap<E, ?>) m).keyIterator();
            }
        }

        public Iterator<E> descendingIterator() {
            if (m instanceof TreeMap) { return ((TreeMap<E, ?>) m).descendingKeyIterator(); } else {
    ...
    private void rotateLeft(Entry<K, V> p) {
        if (p != null) {
            ...
            r.parent = p.parent;
            if (p.parent == null) { root = r; } else if (p.parent.left == p) { p.parent.left = r; } else {
                p.parent.right = r;
            }
            r.left = p;
            p.parent = r;
    ...
    private void rotateRight(Entry<K, V> p) {
        if (p != null) {
            ...
            l.parent = p.parent;
            if (p.parent == null) { root = l; } else if (p.parent.right == p) { p.parent.right = l; } else {
                p.parent.left = l;
            }
            l.right = p;
            p.parent = l;
    ...
    static class TreeMapSpliterator<K, V> {
        ...
        TreeMapSpliterator(TreeMap<K, V> tree, TreeMap.Entry<K, V> origin, TreeMap.Entry<K, V> fence, int side, int est,
            int expectedModCount) {
            this.tree = tree;
            this.current = origin;
            this.fence = fence;
