...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    public BigInteger(int bitLength, int certainty, Random rnd) {
        ...
        prime = (bitLength < SMALL_PRIME_THRESHOLD ? smallPrime(bitLength, certainty, rnd)
        ...
        while (true) {
            ...
            if (bitLength > 6) {
                ...
                if ((r % 3 == 0) || (r % 5 == 0) || (r % 7 == 0) || (r % 11 == 0) || (r % 13 == 0) || (r % 17 == 0) ||
                    (r % 19 == 0) || (r % 23 == 0) || (r % 29 == 0) || (r % 31 == 0) || (r % 37 == 0) ||
                    (r % 41 == 0)) {
    ...
    private static final BigInteger SMALL_PRIME_PRODUCT = valueOf(
    ...
    public static BigInteger valueOf(long val) {
        ...
        if (val > 0 && val <= MAX_CONSTANT) { return posConst[(int) val]; } else if (val < 0 && val >= -MAX_CONSTANT) {
    ...
    BigInteger add(long val) {
        ...
        if (Long.signum(val) == signum) { return new BigInteger(add(mag, Math.abs(val)), signum); }
        int cmp = compareMagnitude(val);
        if (cmp == 0) { return ZERO; }
    ...
    BigInteger multiply(long v) {
        ...
        if (carry == 0L) { rmag = java.util.Arrays.copyOfRange(rmag, 1, rmag.length); }
    ...
    private BigInteger divideKnuth(BigInteger val) {
        MutableBigInteger q = new MutableBigInteger(), a = new MutableBigInteger(this.mag), b = new MutableBigInteger(
            val.mag);
    ...
    final int compareMagnitude(BigInteger val) {
        ...
        for (int i = 0; i < len1; i++) {
            int a = m1[i];
            int b = m2[i];
            if (a != b) { return ((a & LONG_MASK) < (b & LONG_MASK)) ? -1 : 1; }
        }
        return 0;
    }
    ...
    public boolean equals(Object x) {
        ...
        for (int i = 0; i < len; i++) { if (xm[i] != m[i]) { return false; } }
    ...
    public String toString(int radix) {
        ...
        if (mag.length <= SCHOENHAGE_BASE_CONVERSION_THRESHOLD) { return smallToString(radix); }
    ...
    private String smallToString(int radix) {
        ...
        while (tmp.signum != 0) {
            ...
            MutableBigInteger q = new MutableBigInteger(), a = new MutableBigInteger(tmp.mag), b
                = new MutableBigInteger(d.mag);
     ...
     */
    private static final ObjectStreamField[] serialPersistentFields = { new ObjectStreamField("signum", Integer.TYPE),
        new ObjectStreamField("magnitude", byte[].class), new ObjectStreamField("bitCount", Integer.TYPE),
        new ObjectStreamField("bitLength", Integer.TYPE), new ObjectStreamField("firstNonzeroByteNum", Integer.TYPE),
    ...
    public long longValueExact() {
        if (mag.length <= 2 && bitLength() <= 63) { return longValue(); } else {
    ...
    public int intValueExact() {
        if (mag.length <= 1 && bitLength() <= 31) { return intValue(); } else {
    ...
    public short shortValueExact() {
        if (mag.length <= 1 && bitLength() <= 31) {
            int value = intValue();
            if (value >= Short.MIN_VALUE && value <= Short.MAX_VALUE) { return shortValue(); }
        }
    ...
    public byte byteValueExact() {
        if (mag.length <= 1 && bitLength() <= 31) {
            int value = intValue();
            if (value >= Byte.MIN_VALUE && value <= Byte.MAX_VALUE) { return byteValue(); }
        }
